...
const moveDoc = async (product, platform, version) => {
  ...
  const destFilename = match(context, 
    ...
    when({product: {name: "EDB pgBouncer", version: 1.16}, platform: {name: "Debian 11"}}, 
      (ctx) => `pgbouncer/1.16/01_installation/install_on_linux/${fmtArchPath(ctx)}/pgbouncer_deb11_${fmtArchFilename(ctx)}.mdx`),
    ...
    when({product: {name: "EDB pgBouncer", version: 1.16}, platform: {name: "Ubuntu 22.04"}}, 
      (ctx) => `pgbouncer/1.16/01_installation/install_on_linux/${fmtArchPath(ctx)}/pgbouncer_ubuntu22_${fmtArchFilename(ctx)}.mdx`),
