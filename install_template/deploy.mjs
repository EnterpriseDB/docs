...
const moveDoc = async (product, platform, version) => {
  ...
  const destFilename = match(context, 
    ...
    when({product: {name: "Failover Manager", version: 4.4}, platform: {name: "CentOS 7"}}, 
      (ctx) => `efm/4/03_installing_efm/${fmtArchPath(ctx)}/efm4_centos7_${fmtArchFilename(ctx)}.mdx`),
    when({product: {name: "Failover Manager", version: 4.4}, platform: {name: "AlmaLinux 8 or Rocky Linux 8"}}, 
      (ctx) => `efm/4/03_installing_efm/${fmtArchPath(ctx)}/efm4_other_linux8_${fmtArchFilename(ctx)}.mdx`),
    when({product: {name: "Failover Manager", version: 4.4}, platform: {name: "RHEL 7 or OL 7"}}, 
      (ctx) => `efm/4/03_installing_efm/${fmtArchPath(ctx)}/efm4_rhel7_${fmtArchFilename(ctx)}.mdx`),
    when({product: {name: "Failover Manager", version: 4.4}, platform: {name: "RHEL 8 or OL 8"}}, 
      (ctx) => `efm/4/03_installing_efm/${fmtArchPath(ctx)}/efm4_rhel_8_${fmtArchFilename(ctx)}.mdx`),
    when({product: {name: "Failover Manager", version: 4.4}, platform: {name: "RHEL 8"}}, 
