...
const moveDoc = async (product, platform, version) => {
  ...
  const destFilename = match(context, 
    ...
    when({product: {name: "EDB Postgres Advanced Server"}, platform: {name: "SLES 12"}}, 
      (ctx) => `epas/14/epas_inst_linux/installing_epas_using_edb_repository/${fmtArchPath(ctx)}/${prefix[plat]}_epas_sles12_${fmtArchFilename(ctx)}.mdx`),
    when({product: {name: "EDB Postgres Advanced Server"}, platform: {name: "SLES 15"}}, 
      (ctx) => `epas/14/epas_inst_linux/installing_epas_using_edb_repository//${fmtArchPath(ctx)}/${prefix[plat]}_epas_sles15_${fmtArchFilename(ctx)}.mdx`),
