...
const moveDoc = async (product, platform, version) => {
  ...
  const destFilename = match(context, 
    ...
    when({product: {name: "PostGIS", version: 3.1}, platform: {name: "AlmaLinux 8 or Rocky Linux 8"}}, 
      (ctx) => `postgis/3.1/01a_installing_postgis/installing_on_linux/${fmtArchPath(ctx)}/postgis_other_linux8_${fmtArchFilename(ctx)}.mdx`),
    when({product: {name: "PostGIS", version: 3.1}, platform: {name: "RHEL 7 or OL 7"}}, 
      (ctx) => `postgis/3.1/01a_installing_postgis/installing_on_linux/${fmtArchPath(ctx)}/postgis_rhel7_${fmtArchFilename(ctx)}.mdx`),
    when({product: {name: "PostGIS", version: 3.1}, platform: {name: "RHEL 8 or OL 8"}}, 
      (ctx) => `postgis/3.1/01a_installing_postgis/installing_on_linux/${fmtArchPath(ctx)}/postgis_rhel_8_${fmtArchFilename(ctx)}.mdx`),
    when({product: {name: "PostGIS", version: 3.1}, platform: {name: "RHEL 8"}}, 
      (ctx) => `postgis/3.1/01a_installing_postgis/installing_on_linux/${fmtArchPath(ctx)}/postgis_rhel8_${fmtArchFilename(ctx)}.mdx`),
