# Installing EDB Postgres Advanced Server 9.6 on CentOS 7 ppc64le
...
Installing the server package creates an operating system user named `enterprisedb`. The user is assigned a user ID (UID) and a group ID (GID). The user has no default password. Use the `passwd` command to assign a password for the user. The default shell for the user is `bash` and the user's home directory is `/var/lib/edb/as9.6`.
...
1. Initialize the database cluster:
   ...
   ```sh
   PGSETUP_INITDB_OPTIONS="-E UTF-8" /usr/edb/as9.6/bin/edb-as-96-setup initdb
   ```
...
1. To ensure the installation and initial configuration was successful, check the status of your cluster:
   ...
   ```sh
   /usr/edb/as9.6/bin/pg_ctl status -D /var/lib/edb/as9.6/data
   ```
