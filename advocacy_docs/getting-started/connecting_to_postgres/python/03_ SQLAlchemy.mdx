---
title: Connecting to Postgres from Python using SQLAlchemy
navTitle: SQLAlchemy (WIP)
...
Connect to Postgres by calling `create_engine` to define the connection, then calling connect() on that:
```python
import sqlalchemy 
