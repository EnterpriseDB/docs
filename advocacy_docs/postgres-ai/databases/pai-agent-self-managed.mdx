...
1. We are running the agent on the same server as the Postgres instance we're monitoring. So it's faster and more secure to have the EDB Postgres AI agent use Postgres local auth rather than set up password auth over TCP/IP.
    ...
    !!! Note
    In this example, we use the 'ubuntu' user that is created by default on the EC2 with a default Ubuntu (22.04) machine image. In production, you'd want to use a minimally privileged user explicitly created for the purposes of running the Beacon agent on the server.
    !!!
