...
Cloud Service calculates the estimated monthly price for your cluster and displays it at the bottom of the page while you're [creating a cluster](/edb-postgres-ai/cloud-service/getting_started/creating_cluster/creating_a_cluster/). This estimate includes database costs. If you're using Cloud Service's cloud account, it also includes infrastructure costs.
...
| Azure Monitor                  | Cloud Service feeds all [logs and metrics](/edb-postgres-ai/cloud-service/using_cluster/monitoring_and_logging/) to Azure Monitor.          |
...
To get a better sense of your Microsoft Azure costs, check out the Microsoft Azure pricing [calculator](https://azure.microsoft.com/en-us/pricing/calculator/) and reach out to [Support](/edb-postgres-ai/cloud-service/support_services/). 
...
| AWS CloudWatch                   | Cloud Service feeds all [logs and metrics](/edb-postgres-ai/cloud-service/using_cluster/monitoring_and_logging/) to AWS CloudWatch.         |
...
To get a better sense of your AWS costs, check out the AWS pricing [calculator](https://calculator.aws/#/) and reach out to [Support](/edb-postgres-ai/cloud-service/support_services/).  
...
Enabling [PgBouncer](/edb-postgres-ai/cloud-service/getting_started/creating_cluster/creating_a_cluster/#pgbouncer) to pool your connections incurs additional costs that depend on your cloud provider. In addition to the cloud provider costs, PgBouncer connects to your primary server and requires an IP address. Cloud Service provisions up to three instances per PgBouncer-enabled cluster to ensure that performance is unaffected, so each availability zone receives its own instance of PgBouncer. The extra VM costs are the 2vcpu SKU times the number of PgBouncer instances. For AWS, the instance type is c5.large. For Azure, the instance type is F2s_v2.
...
EDB charges the credit card for your [EDB account](https://www.enterprisedb.com/accounts/register) month to month and sends invoices to your email. This invoice includes database costs. If you're using EDB Hosted Cloud Service, it also includes infrastructure costs.
