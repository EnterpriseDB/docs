...
BigAnimal requires you to check the readiness of your Google Cloud (GCP) account before you deploy your clusters. (You don't need to perform this check if you're using BigAnimal's cloud account as your [deployment option](/biganimal/latest/planning/deployment_options/). The checks that you perform ensure that your Google Cloud account is prepared to meet your clusters' requirements and resource limits.
...
2.  From Google Cloud Shell, run the following command:
    ...
    | &lt;region>     | Google Cloud region where your clusters are being deployed. See [Supported regions](/edb-postgres-ai/cloud-service/references/supported_regions/#google-cloud-regions-1) for a list of possible regions. |
