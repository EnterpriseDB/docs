...
You can schedule a time in UTC for the backup to start. You can choose hours and minutes in 24-hour format or choose now to start the backup immediately.
...
-   To enable and use TDE for a cluster, you must first enable the encryption key and add it at the project level before creating a cluster. 
    To add a key, see [Adding a TDE key at project level](/edb-postgres-ai/console/using/projects/settings/security/#adding-a-tde-key).
