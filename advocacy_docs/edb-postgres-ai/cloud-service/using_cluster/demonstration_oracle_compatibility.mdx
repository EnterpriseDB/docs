...
redirects:
  - /biganimal/latest/using_cluster/06_demonstration_oracle_compatibility/ #generated for BigAnimal URL path removal branch
  - /purl/upm/oracle-compatibility-demo #UPM (BA / Cloud Service) permalink
---

Cloud Service lets you run Oracle SQL queries in the cloud using [EDB Postgres Advanced Server](/epas/latest/fundamentals/epas_fundamentals/epas_compat_ora_dev_guide/). This demonstration shows two Oracle SQL-syntax queries running unmodified on a Cloud Service test cluster, populated with the [Chinook sample database](https://github.com/lerocha/chinook-database).
...
1.  [Connect to your EDB Postgres Advanced Server cluster as edb_admin](connecting_your_cluster/) using psql. You can get the command for this from your cluster's **Overview** tab. It looks like this, where `<YOUR CLUSTER HOSTNAME>` is specific to your cluster:
...
3.  Create a limited-privilege user for connecting to this database:
    ...
    See [Details on managing access in Cloud Service databases](postgres_access/database_authentication/) for more information.
...
7.  Get the host name of your cluster from the **Connect** tab on the Console. Use it in place of `<YOUR CLUSTER HOSTNAME>` to invoke pg_restore:
    ...
    !!! Note
        ...
        and is installed by default on Cloud Service clusters. However, since you're connecting as
...
-   Learn about [migrating existing databases to Cloud Service](/edb-postgres-ai/cloud-service/managing_your_cluster/migration/).
