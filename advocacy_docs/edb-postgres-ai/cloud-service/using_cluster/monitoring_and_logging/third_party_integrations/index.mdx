...
redirects:
  - /purl/upm/service-now-docs/
  - /biganimal/latest/using_cluster/05_monitoring_and_logging/third_party_integrations/ #generated for BigAnimal URL path removal branch
---
...
All the metrics collected from all the clusters in the project are sent to the integrated tool and displayed in the Cloud Service **Monitoring and logging** tab [using Cloud Service Observability](../monitoring_using_biganimal_observability). The collected logs are exported to the object storage by default.

You can enable in-app inbox or email notifications so that you're notified if third-party monitoring integration fails. For more information, see [Managing notifications](/edb-postgres-ai/cloud-service/console/using/notifications/#managing-notifications).
...
The following table provides a mapping between [Cloud Service metric names](../metrics/) 
