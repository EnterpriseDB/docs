...
!!!note
   ...
   In addition to the Cloud Service `pgd` commands, you can switch over and use commands available in the [EDB Postgres Distributed CLI](/pgd/latest/reference/cli/) to perform PGD-specific operations. The only EDB Postgres Distributed CLI commands that don't apply to Cloud Service are `create-proxy` and `delete-proxy`.
