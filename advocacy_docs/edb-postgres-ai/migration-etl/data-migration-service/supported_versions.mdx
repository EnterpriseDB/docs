...
The EDB Data Migration Service (EDB DMS) is currently a public preview capability that is only available to users of EDB Postgres AI Hybrid Manager.
...
!!!note
    ...
    If the destination is an HM-managed database, AWS must be the provider for the Hybrid Manager cluster. Other Cloud Service providers are currently not supported.
