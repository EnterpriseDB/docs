...
Accordingly, EDB recommends [applying foreign key, check, and exclusion constraints](apply_constraints#foreign-key-) on the target database only after you have signalized the end of the CDC stream by [marking the migration as completed](mark_completed) in the Console.
