---
title: "Installing EDB Data Migration Service Writer on Linux AArch64 (ARM64)"
navTitle: "On Linux ARM64"
# This topic is generated from templates. If you have feedback on it, instead of
# editing the page and creating a pull request, please enter a GitHub issue and
# the documentation team will update the templates accordingly.
# Leaf template: products/edb-data-migration-service-writer/arm64_index.njk

redirects:
  - /advocacy_docs/edb-postgres-ai/migration-etl/data-migration-service/getting_started/installing2/linux_arm64/index
  - /advocacy_docs/edb-postgres-ai/hybrid-manager/using_hybrid_manager/migration/installing2/linux_arm64/index
indexCards: none

navigation:
  - edb-dms-writer_debian_12
---

Operating system-specific install instructions are described in the corresponding documentation:

### Debian and derivatives

- [Debian 12](edb-dms-writer_debian_12)
