...
1.  Unzip the downloaded folder and copy it to the host where the writer is installed.
...
In versions 1.1 and earlier, configuring object storage for the writer is a manual process that requires pointing at the AWS S3 bucket or other S3-compatible object storage (like MinIO) that you already have configured for the HCP. In this case, fill out the `AWS_` values with the information of your object storage.
 
In versions 1.2 and later, an autodiscovery mechanism will automatically use the same object storage you configured for the HCP. In this case, you won't need to provide any of the `AWS_` values.
