...
Create migration credentials to enable the EDB DMS Writer on your [auxiliary machine](../terminology.mdx#auxiliary-machine) to connect to the HCP for migrations. You need one set of credentials per destination [HCP project](../../../hybrid-manager/using-hybrid-manager/projects).

For example, if you are using a single HCP project for all migrations, you only need one set of credentials on your auxiliary machine.
...
2.  Go to the HCP Portal, and verify that a destination with the `DBCONFIG_ID` name is displayed in **Migrate** > **Destinations**.
