...
1.  Unzip the downloaded zip file and copy the generated folder to the host where the writer is installed.
...
These parameters define the information the EDB DMS Writer requires to access the HCP instance and project that you are using for the migration.
...
Currently (HCP version 1.1), configuring object storage for the writer is a manual process that requires pointing at the AWS S3 bucket or other S3-compatible object storage (like MinIO) that you already have configured for the HCP. In this case, fill out the `AWS_` values with the information of your object storage.
 
In later HCP versions, an autodiscovery mechanism will automatically use the same object storage you configured for the HCP. In this case, you won't need to provide any of the `AWS_` values.
