...
You only need to configure your destination database if you are migrating to a Postgres instance created outside of the HCP (externally-managed or self-managed Postgres instance). If you are migrating to an HCP-managed database, you can skip this page, as you won't be using the EDB DMS Writer.
...
1.  Grant permissions so the user can control the behavior of replication sessions:
    ...
    !!!note
        Granting control over replication sessions is only supported on Postgres 16 and later versions. If your destination database is on Postgres 15 or earlier, you will have to use a superuser for the migration.
