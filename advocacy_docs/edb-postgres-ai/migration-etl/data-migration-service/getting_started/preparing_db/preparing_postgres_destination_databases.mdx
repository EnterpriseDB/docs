...
-   Ensure you have [created a database in your destination Postgres instance](../create_database.mdx).

-   Ensure you have [prepared and imported the database schema](../prepare_schema.mdx) to the destination database.
...
For **testing** purposes, the following example creates a self-signed certificate and key with OpenSSL for a PostgreSQL 17 server on Ubuntu. The bin and data directories may vary depending on your OS, [Postgres distribution](https://www.enterprisedb.com/docs/edb-postgres-ai/databases/postgres_distributions/), and version.
...
This is the user you will provide to the EDB DMS Writer [during configuration](../config_writer.mdx) via the `DBCONFIG_DATABASES_0__USERNAME` and `DBCONFIG_DATABASES_0__PASSWORD` values.
