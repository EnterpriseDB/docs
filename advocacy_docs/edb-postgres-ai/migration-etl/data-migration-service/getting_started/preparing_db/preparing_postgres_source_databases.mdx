...
1.  [Set the Postgres configuration](preparing_postgres_source_databases.mdx#set-the-postgres-configuration).
2.  [Create new roles and grant acccess for CDC migration](preparing_postgres_source_databases.mdx#create-new-roles-and-grant-acccess-for-cdc-migration).
3.  [Grant usage of the source schemas to the CDC migration role](preparing_postgres_source_databases.mdx#grant-usage-of-the-source-schemas-to-the-cdc-migration-role).
4.  [Grant `SELECT` on source tables to the CDC migration role](preparing_postgres_source_databases.mdx#grant-select-on-source-tables-to-the-cdc-migration-role).
5.  [Grant `CREATE` privileges on the source database to the CDC migration role](preparing_postgres_source_databases.mdx#grant-create-privileges-on-the-source-database-to-the-cdc-migration-role).
...
Now that you have stepped through the entire configuration process, validate your Postgres configuration using the [config validation script](preparing_postgres_source_databases.mdx#config-validation-script) with the correct parameters. To illustrate, consider the following example:
