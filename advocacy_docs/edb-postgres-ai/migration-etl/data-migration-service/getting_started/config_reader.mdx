...
Create migration credentials to enable the EDB DMS Reader on your [auxiliary machine](../terminology.mdx#auxiliary-machine) to connect to the HCP for migrations. You need one set of credentials per destination [HCP project](../../../hybrid-manager/using-hybrid-manager/projects). 
...
2.  Go to the HCP Portal, and verify that a source with the `DBZ_ID` name is displayed in **Migrate** > **Sources**.
