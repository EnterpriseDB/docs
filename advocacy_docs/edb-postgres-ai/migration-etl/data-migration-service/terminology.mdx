...
When you select the **Snapshot** option, DMS captures an image of your database at a specific moment. This static copy is then used for the migration. Any new inserts and updates performed on the source database after the snapshot is taken won't be replicated to the destination database.
...
Managed Postgres destinations are databases created and managed through the Hybrid Control Plane. All database clusters created in within the project where you are performing the migration can be selected as possible migration destinations. 
