...
While it can be the same host as where you have a source or destination database, as well as the same host where you have the HCP running, its only requirement is a Linux operating system, since it is the only platform currently supporting the EDB DMS Reader and Writer.
