---
title: EDB Postgres AI databases
navTitle: Databases
indexCards: simple
iconName: Database
description: About the Postgres databases that are the core of the EDB Postgres AI platform.
deepToC: true
navigation: 
  - postgres_distributions
  - feature_support_per_db
  - advanced_replication
  - advanced_security
  - advanced_sql
  - advanced_performance
  - oracle_compatibility
  - tools_extensions
  - deployment_options
---

## Postgres options 

Building on decades of Postgres expertise, the EDB Postgres® AI databases are the core of the EDB Postgres AI platform. 

-   [EDB Postgres Advanced Server](/epas/latest) can take on Oracle workloads.
-   [EDB Postgres Extended Server](/pge/latest) designed for large-scale, mission-critical enterprise workloads.

All of them are built on the same Postgres core, so you can use the same tools and skills across all of them.

- [PostgreSQL](/supported-open-source/postgresql) is the world's most advanced open source database, and the foundation for all EDB Postgres databases.

And for distributed high-availability and fault tolerance, you can use EDB Postgres Advanced Server and EDB Postgres Extended server in combination with EDB Postgres Distributed: 

-   [EDB Postgres Distributed](/pgd/latest) provides distributed high availability and fault tolerance for mission-critical workloads.

## Differences between Postgres options

Learn more about the differences between **EDB Postgres Advanced Server**, **EDB Extended Server** and open-source **PostgreSQL**: 