...
The `aidb.PipelineProcessingMode` type is an enum used to define which processing mode is configured for a Pipeline (e.g. a Knowledge Base or a Preparer).
...
| source_data_format   | [aidb.PipelineDataFormat](#aidbpipelinedataformat) | Required     | Type of data in that column ("Text"."Image","PDF") |
...
Sets the processing mode for this Knowledge Base. This function is used to enable and disable auto-embedding: `Live` mode enables auto-embedding and `Disabled` disables it.
...
__OUTPUT__
INFO:  bulk_embedding_text found 3 rows in knowledge base test_knowledge_base
 bulk_embedding
----------------
