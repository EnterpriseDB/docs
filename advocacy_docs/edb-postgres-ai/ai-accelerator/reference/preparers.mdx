...
This section provides reference documentation for Pipelines Preparers. It includes information on the functions and views available in the aidb extension related to [Preparers](../preparers/). See the [Usage](../preparers/usage.mdx) and [Examples](../preparers/example.mdx) sections for more details.
...
Deletes only the preparer's configuration from the database.
