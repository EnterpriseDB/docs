...
If the source table already has data/rows when the knowledge base is created, then you need to make a manual *bulk embedding* call. This call generates the embeddings for all the existing data in the source table.
...
While auto-embedding is enabled, the embeddings are always up to date, and applications can use the knowledge base to query the data as needed.
