...
Processing happens asynchronously based on a configurable `background_sync_interval`. See [change detection below](#change_detection) for details on how the pipelines are processed.
