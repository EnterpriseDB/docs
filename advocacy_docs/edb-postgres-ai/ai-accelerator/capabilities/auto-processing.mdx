...
### Table sources
When background auto-processing is configured, we set up Postgres Triggers on the source table. These triggers are very lightweight.
...
### Volume sources
This source type provides a `last_modified` timestamp for each source record. We keep track of those timestammps in a "state" table.
In each pipeline execution, we list the contents of the volume and compare the timestamps to see whether any records have changed or were added.

This mechanism works in disabled and in background auto-processing.

We detect deleted objects after a full listing is complete. Only then can we be certain that a perviously processed record is no longer present in the source.
