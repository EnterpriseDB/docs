...
With data preparation, Pipelines can perform common preprocessing steps in the AI Accelerator directly from Postgres.
...
The application can trigger bulk data preparation processing for all existing data in the source location using `aidb.bulk_data_preparation()`. The output from the preparers can then be fed into the input for retrievers.
