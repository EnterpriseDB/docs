...
<tr><td><details><summary>Added support for writing preparer output to object storage volumes with TLS configuration.</summary><hr/><p>Preparer pipelines can now write output files directly to object storage volumes. Supported storage backends include local file systems and remote S3-compatible stores configured via foreign tables.
TLS support has been added for secure HTTPS endpoints, with options to disable certificate verification for development or provide a custom CA certificate using the <code>ca_path</code> field in the model configuration.
These improvements enable secure, configurable integration with external storage systems for output persistence.</p>
</details></td><td></td></tr>
</tbody></table>
