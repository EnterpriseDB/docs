...
This high-level overview describes the lifecycle of a pipeline in the Pipelines system.
...
If you want to use an external storage location, to access the data, you must create a storage location.
...
Create a [model](models) with the Pipelines system. This model can be a machine learning model, a deep learning model, or any other type of model that can be used for AI tasks.

### Create a retriever

Create a retriever with the Pipelines system. A retriever is a function that retrieves data from a table or volume and returns it in a format that the model can use.
