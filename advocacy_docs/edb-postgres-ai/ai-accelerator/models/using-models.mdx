...
Pipelines comes with a set of models that you can use out of the box. 
...
The `bert`, `clip`, and `t5` models are all preloaded and ready to use. The `dummy` model is a placeholder model that you can use for testing purposes.
...
Replace the `api_key` value with your own OpenAI API key. Then you can use the `my_openai_model` model in your Pipelines functions and, in this example, leverage the GPT-4o model from OpenAI.

You can create the OpenAI Embeddings model in a similar way:
...
Uf you only needed to change the credentials, you can delete the new model using `SELECT aidb.delete_model('my_new_openai_model')`.

## Using models with OpenAI-compatible APIs

The `completions` and `embeddings` models work with any OpenAI-compatible API. This compatibility allows you to connect and use an even wider range of models. Just pass the appropriate API endpoint to the `url` option in the `aidb.create_model` function's options.
