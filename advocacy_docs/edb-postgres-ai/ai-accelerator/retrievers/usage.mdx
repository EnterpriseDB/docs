...
The [aidb.register_retriever_for_table](../reference/retrievers#aidbregister_retriever_for_table) function is used to create a retriever for a table data source. This is the function signature, you can see many of those are optional and have defaults.
...
If you are only using Postgres tables, [skip to the next section](#creating-the-embeddings).
...
Before we can register a retriever for a volume, we need to create a volume. The [aidb.create_volume](../reference/retrievers#aidbcreate_volume) function is used to create a volume. This is the function signature, you can see many of those are optional and have defaults.
...
The [aidb.register_retriever_for_volume](../reference/retrievers#aidbregister_retriever_for_volume) function is used to create a retriever for a volume data source. This is the function signature, you can see many of those are optional and have defaults.

```
aidb.register_retriever_for_volume(
-------------------------------------------------------------------------------
    p_name                              TEXT,
    p_source_volume_name                TEXT,
    p_vector_table_name                 TEXT DEFAULT NULL,
    p_vector_table_vector_column        TEXT DEFAULT 'embeddings',
    p_vector_table_key_column           TEXT DEFAULT 'id',
    p_model_name,                       TEXT,
...
SELECT aidb.register_retriever_for_volume(
               p_name => 'test_retriever_volume',
               p_source_volume_name => 'test_volume',
               p_model_name => 'simple_model'
       );
```
