...
navTitle : Concepts
deepToC: true
description: Concepts for using retrievers in AI Accelerator.
---
...
Types and sources are generally independent but not all combinations might be supported. Today, it’s not possible to discover supported combinations. Creating a retriever will fail if a combination is not supported.
