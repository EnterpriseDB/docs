...
Simplify data processing with Automated Pipelines for fetching data from Postgres or object storage. These pipelines can generate vector embeddings as new data is ingested, and triggering updates to embeddings when source data changes — meaning always-up-to-date data for query and retrieval without tedious maintenance. Pipelines can also perform pre-processing steps directly with the AI Accelerator to make data suitable for embedding generation, eliminating the need for many external ETL workflows and dependencies.
