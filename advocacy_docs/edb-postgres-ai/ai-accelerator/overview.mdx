...
Simplify data processing with Automated Pipelines for fetching data from Postgres or object storage. Pipelines can perform preprocessing steps directly with the AI Accelerator to make data suitable for embedding generation. This capability eliminates the need for many external ETL workflows and dependencies. Pipelines can also generate vector embeddings as new data is ingested, triggering updates to embeddings when source data changes. This means data is always up to date for query and retrieval without tedious maintenance.

### GenAI query engine

Leverage semantic search across text and images that’s 4.22X faster[^1] than purpose-built vector databases and an intelligent knowledge base that abstracts away the complexities of vector similarity calculations. These capabilities transform your Postgres database into a powerful GenAI search engine across multiple storage locations.
