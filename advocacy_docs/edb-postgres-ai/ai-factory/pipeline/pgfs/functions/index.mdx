...
Note that there is also a `pgfs.create_storage_location` function with a uuid as the third parameter for a managed storage location id. The uuid is optional, and it not specified, set to null.
