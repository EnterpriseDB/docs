...
description: How-to guide for performing a Lakehouse Sync.
---

## Overview

A Lakehouse Sync is a way to capture information from a transactional database at a point in time and sync that information to a Managed Store Location in an AWS region.

Once synced, the data in the MSL is organized into Lakehouse tables. These Lakehouse tables can then be queried by a Lakehouse node optimized for higher-performance queries using a vectorized query engine for Lakehouse tables.

## Perfroming a Lakehouse Sync
...
1. Login to your EDB Postgres AI Console.
...
6. If you have already created a MSL you want to use, select that MSL from the list of available MSLs and skip to step 10. If not, select the **Create New Managed Storage Location** button to bring up the **Add Managed Storage Location** modal.
