...
You can then query this table in the Lakehouse node by creating an external table that references the Delta Table in the `my_schema/my_table` path. See [External tables](../external_tables.mdx) for details.
