...
For the Beacon Agent to be able to read your database, you must provide the connection details in the form of a DSN/connection string in the `beacon_agent.yaml` configuration file.
...
As DSNs can contain sensitive information such as the password, its best practice to create an environment variable for each database you want to monitor and set each to the a corresponding DSN.
