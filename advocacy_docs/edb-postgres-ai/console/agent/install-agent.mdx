...
1. Enable your system to download packages from the EDB repositories for EDB Postgres AI.
...
2. You can now install packages from EDB's repositories. Install the `beacon-agent` package:
...
3. Configure Beacon Agent and database connections.
    ...
    First, create a Beacon configuration directory in your home directory:
...
4. Create DSN/connection strings for each database you want to monitor. These should include the database name, the user, the password, the host, and the port. For example:
...
5. As DSNs can contain sensitive information such as the password, its best practice to create an environmental variable for each database you want to monitor and set each to the a corresponding DSN.

    ```shell
    export DSN1=<dsn1>
    ```

6. Edit your `$HOME/.beacon/beacon_agent.yaml` configuration file and add an entry for each database you want to connect to, including the environmental variable for each database's DSN. Precede the DSN with a `$` to indicate that it's an environmental variable.
...
7. Configure Beacon Agent to send data to the EDB Postgres AI control plane.
...
8. Check the EDB Postgres AI Console to see the data from the agent.
