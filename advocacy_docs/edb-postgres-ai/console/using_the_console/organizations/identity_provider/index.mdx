...
See [Setting up your Azure Marketplace account](/biganimal/latest/getting_started/02_azure_market_setup/).
!!!
...
2. In the **Configure SAML response** section, configure your identity provider to send the SAML response. The SAML response must include the following attributes:
   - `givenname` &mdash; EDB Postgres AI uses the value as the given name in the profile of the authenticated user.
   - `surname` &mdash; EDB Postgres AI uses the value as the surname in the profile of the authenticated user.
   - `name` &mdash; EDB Postgres AI uses the value as the full name (`givenname` joined with `surname`) in the profile of the authenticated user.
   - `emailaddress` &mdash; EDB Postgres AI uses the value for the email address in the profile of the authenticated user. 
