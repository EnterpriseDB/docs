...
The integrations page allows you to view and edit the integrations available to the project. The integrations page shows the available integrations and the status of each integration. Selecting the switch at the bottom of each integration tile allows you to enable or disable the integration. When you enable an integration, you are prompted to enter the required details for that integration.

#### Settings->Profile

The profile page allows you to view and edit the profile of the project. The profile page shows the project name as an editable field. Change it to a new unique project name and select **Save** to make the change.

#### Settings->Security

The security page allows you to add a TDE key to the project. Existing key details are also shown. Selecting **Add Key** allows you to add a new key to the project. Select an appropriate cloud service provider and, if required, a region. See [managing project security](managing_project_security) for more information on adding and deleting TDE keys and details on using TDE keys to create a cluster.

#### Migrate->Migrations

This page allows you to configure a data migration from one cluster to another. Select the source cluster and the destination cluster from the dropdown menus. You can also select the tables you want to migrate. Select **Migrate** to start the migration.
