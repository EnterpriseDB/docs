...
Host metrics display resource utilization for the underlying server infrastructure hosting your database instances. They are visible through the [**Estate**](../../estate) view, and also through the cluster view ► [**Monitoring**](../../projects/project/clusters/cluster/cluster-monitoring-tab.mdx) tab ► **Host** section.
...
Postgres metrics show performance and operational statistics specific to the Postgres database server itself. They are visible through the [**Estate**](../../estate) view, and also through the cluster view ► [**Monitoring**](../../projects/project/clusters/cluster/cluster-monitoring-tab.mdx) tab.
...
Cluster metrics are host and postgres metrics, individual and aggregated for database deployments spanning multiple nodes (HA, AHA, and DHA topologies). They are visible through the [**Estate**](../../estate) view, and also through the cluster view ► [**Monitoring**](../../projects/project/clusters/cluster/cluster-monitoring-tab.mdx) tab ► **Topology** and **Replication** sections. Cluster metrics are not displayed for single node deployments.
...
Query metrics offer insights into the performance and characteristics of the database Postgres queries being executed. They are visible through the cluster view ► [**Query diagnostics**](../../projects/project/clusters/cluster/cluster-query-diagnostics-tab.mdx) tab.
...
Logs present raw and processed log data from various components of your database system for troubleshooting and auditing. They are visible through the cluster view ► [**Logs**](../../projects/project/clusters/cluster/cluster-logs-tab.mdx) tab.
