...
You can monitor your Amazon Web Service (AWS) Relational Database Service deployments with your EDB AI Hybrid Control Plane (HCP).

## Prerequisites

You must have access to the Kubernetes environment where the HCP resides, and also have sufficient administrator privileges to change the settings on the entire HCP cluster.

This is different than other external database monitoring situations with HCP, such as [self-managed](../mon_with_agent) deployments of Postgres, where less privileges are required.
...
## Setting up the Beacon Agent to monitor AWS RDS deployments on HCP

The process of setting up the Beacon Agent to monitor all your AWS RDS deployments on HCP involves the following steps.
...
1.  Add your `beacon-csp-credentials` secret.
    ...
    Add your beacon-csp-credentials secret in your HCP's Kubernetes cluster's `upm-beacon` namespace:
    ...
    EOF
    ```
2.  Restart the Beacon agent and external metrics receiver:
...
## Disabling AWS RDS Monitoring in HCP
...
You are now no longer seeing your AWS RDS deployments on your [Estate](../../../estate) and are no longer monitoring them with HCP.
