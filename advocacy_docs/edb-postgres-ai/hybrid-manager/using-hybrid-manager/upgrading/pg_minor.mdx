---
title: Prerequisites
navTitle: Postgres minor version upgrades on HCP
...
Users have full control over when new versions are available with Hybrid Control Plane (HCP).
Once validated and published, new version images are available in HCP's image library and available for upgrades on any existing cluster.
...
The prerequisites to get started are to have a cluster created with HCP up and in a healthy state.
...
## Upgrade the PG minor version of an existing instance/cluster created with HCP

1.  First, from the cluster's main page, select **Quick Action>Edit Cluster** and then navigate to the **Cluster Settings** tab.

2.  Review the list of available versions under **Software Image** further down the page.

3.  Select your preferred new minor version image, then select the **Save** button at the bottom of the page to confirm.

4.  Review the summary of changes displayed and confirm by selecting the **Confirm changes** button.

5.  The upgrade is applied automatically. 
