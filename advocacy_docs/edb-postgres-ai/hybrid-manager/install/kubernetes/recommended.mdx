...
When considering compute requirements for Hybrid Manager, it is important to understand that PGAIHM itself is relatively lightweight out of the box and it's only the workloads from different use cases which may require scaling.
