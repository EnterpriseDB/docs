---
title: Minimum recommended specifications for PGAI Hybrid Manager
...
When considering compute requirements for PGAI Hybrid Manager (PGAIHM), it is important to understand that PGAIHM itself is relatively lightweight out of the box and it's only the workloads from different use cases which may require scaling.
...
-   **6 Kubernetes worker nodes**, 3 for HCP control components, 3 for HCP database nodes, each with:
...
-   **Storage classes**
    ...
    -   SAN
        -   Must use a Storage Area Network (SAN) for centralized and high-performance needs of HCP.
    ...
    -   Snapshot support
        -   Ensure the CSI driver supports volume snapshots if using snapshot backups in HCP

## Configuration settings

The following configuration settings are recommended for running HCP on Kubernetes:
...
-   **External access**
    -   If accessing HCP using the internet, these ingress/egress requirements apply
-   **DNS**
...
-   **Authentication**
    ```
      ...
      - Multi-factor Authentication (MFA)
          - Enforce MFA for all administrator accounts accessing the HCP.
    ```
-   **Network security**
    ...
    -   Encrypted communication
        -   Use TLS for all communication between HCP components and external clients.
...
-   **Data security**
    -   Encryption at rest
        -   Ensure persistent volumes used by HCP have encryption enabled
    ...
    -   Encryption in transit
        -   Enable encryption for all data transfer between HCP components and the database clusters they manage.
    -   Secrets management
...
-   **Compliance**
    -   Ensure HCP deployment adheres to organizational or industry-specific security standards (e.g. SOC 2, HIPAA, GDPR).
    -   Regularly review compliance and update configurations as standards evolve.

Please see capcacity planning document for guidance on resources based on HCP functionality or use cases.

## 2. Kubernetes Platform and Version Requirements

HCP is designed to run on various conformant Kubernetes distributions. Supported platforms include:

-   Amazon Elastic Kubernetes Service (EKS)
-   Google Kubernetes Engine (GKE)
-   Red Hat OpenShift (RHOS)

(Note: Specific version compatibility should be checked against the HCP release notes.)
...
-   **Processor Architecture:** HCP components primarily target AMD64/x86-64 architecture. While PostgreSQL itself can run on ARM64, mixed-architecture Kubernetes clusters for HCP deployments are not standard and may present support complexities.
...
-   **Node Allocation for HCP:**
    -   A minimum of 3 Kubernetes nodes are required to host the HCP control plane components, ensuring high availability. These can be Kubernetes control plane nodes or worker nodes, labeled appropriately.
-   **Node Allocation for PostgreSQL (Optional but Recommended):**
    -   While not a strict HCP requirement (PostgreSQL can be co-located if resources and labels permit), dedicating at least 3 additional Kubernetes worker nodes for PostgreSQL instances is recommended for production workloads to ensure resource isolation and performance.

## 4. External Services Requirements

HCP, running within Kubernetes, relies on several external services that must be provisioned and accessible to the Kubernetes cluster:

-   **Networking:**
    -   Local network infrastructure (on-premises) or a Virtual Private Cloud (VPC) in a CSP.
    -   **Load Balancer:** Required for exposing HCP and PostgreSQL services. This is typically a managed service in CSPs. For on-premises RHOS, a solution like MetalLB or an enterprise load balancer must be integrated.
...
-   **Block Storage:** Persistent, high-performance block storage is required for PostgreSQL data volumes and potentially other stateful components of HCP.
...
The Kubernetes cluster nodes running HCP components and EDB Postgres instances require outbound internet access to the following EDB services:
