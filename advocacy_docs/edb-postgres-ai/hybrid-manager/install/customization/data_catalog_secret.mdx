...
For EKS installations using the `eks-install-secrets.sh` script, you can skip key and secret creation and go to [Backing up the confounding key](#backing-up-the-confounding-key). Other EKS setups and operating systems require manual secret creation with the confounding key.
...
Store the key safely according to your company's protocols.