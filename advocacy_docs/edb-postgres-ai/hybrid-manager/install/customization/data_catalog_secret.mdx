...
For EKS installations using the `eks-install-secrets.sh` script, you can skip key and secret creation and go straight to [Disaster recovery impact](#loss-of-encrypted-data-without-the-confounding-key). Other EKS setups and operating systems require manual secret creation with the confounding key.
...
1.  Create a secret that references the created confounding key and is stored in the dedicated namespace:
