...
For EKS installations using the `eks-install-secrets.sh` script, you can skip key and secret creation and go straight to [Configure DataLake object storage](#configure-datalake-object-storage-for-genai-builder) and [CORS configuration](#cors-configuration). Other EKS setups and operating systems require manual secret creation with the Fernet key.
...
## Configure DataLake object storage for GenAI builder
