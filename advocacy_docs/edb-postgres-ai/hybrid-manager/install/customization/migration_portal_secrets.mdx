...
Whereas this section provides an overview of the secrets and their functions, [Overriding the default secrets](#overriding-the-default-secrets) provides an example on how to configure custom secrets and override the default ones.
...
- `stringData.username`: User that connects to the database. This user must also be entered in the `parameters.edb-migration-portal.db_owner` [helm value](#apply-the-secrets-to-your-installation).  
