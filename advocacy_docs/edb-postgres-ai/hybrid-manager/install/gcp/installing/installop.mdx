...
1. The following environmental variable is required for authentication:
...
- [Install the operator on GKE in a network-connected environment](installop.mdx#__tabbed_1_1)
- [Install the operator on GKE in an air-gapped environment](/install/rhos/installing/#install-the-operator-on-an-air-gapped-envirnoment)

<TabContainer>
<Tab title="Network-connected">

### Install on network-connected
...
1. Prepare the image pull secret for accessing the EDB Cloudsmith repository:
...
1. Prepare the image pull secret for accessing the private registry accessible by the air-gapped kube:
...
1. Install Skopeo, a command line utility that can perform various operations on container images and image repositories.

1. Log in to EDB Cloudsmith and the private registry:

```bash
skopeo login docker.enterprisedb.com
skopeo login $CONTAINER_REGISTRY_URI
```
...
skopeo --override-os linux copy \
    --multi-arch all \
    docker://docker.enterprisedb.com/pgai-platform/edb-hcp-operator/manager:${OPERATOR_VERSION} \
    docker://${CONTAINER_REGISTRY_URI}/edb-hcp-operator/manager:${OPERATOR_VERSION} \
    --retry-times 3
```

1. Create the image pull secret for the operator against the private registry:
...
helm upgrade \
    ...
    --set "imagePullSecrets[0].name=edb-hcp-operator-pull-secret" \
    --set controllerManager.manager.image.tag=$OPERATOR_VERSION \
    --set controllerManager.manager.image.repository=${CONTAINER_REGISTRY_URI}/edb-hcp-operator/manager \
    --version $OPERATOR_VERSION \
    edb-hcp-operator enterprisedb-edbpgai/edb-hcp-operator
```
