...
You install HCP using a bootstrap kit. This kit is a collection of scripts and Helm charts that are used to install the EDB Software Deployment platform.
...
=== "Create a simple public VPC"

```
1. Select **Create VPC**.

1. On the **Create VPC** page, under **Resources to create**, select **VPC and more**.
...
1. Select **Create VPC**. The **Create VPC workflow** page appears and steps through creating the VPC. When complete, select **View VPC**.
...
1. Select **Actions > Edit subnet settings**.

1. Select **Auto-assign public IPv4 address > Enable auto-assign public IPv4 address**.
...
1. Select **Add Tag**
1. Enter `kubernetes.io/role/elb` in the **Key** field, and `1` in the **Value** field.
1. Select **Save**
1. Close the tab/window and return to the VPC view.
...
1. Select **Manage Tags**
1. Select **Add Tag**
1. Enter `kubernetes.io/role/internal-elb` in the **Key** field, and `1` in the **Value** field.
1. Select **Save**
1. Close the tab/window and return to the VPC view.
```
...
HCP requires an S3 bucket to store backups and other data. Create an S3 bucket and connect it to the cluster.
...
The EKS cluster created by the AWS Management Console isn't fully compatible with HCP, so you need to make some changes to the cluster. A script is available to make those changes for you.
...
Next, [Install the HCP on EKS](../installing.mdx).
