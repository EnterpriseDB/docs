---
title: Installing HCP on AWS EKS
navTitle: Installing with Helm
description: Step-by-step guide to install the Hybrid Control Plane (HCP) on AWS EKS using Helm charts.
---
...
HCP is installed using a bootstrap kit. This kit is a collection of scripts and Helm charts that are used to install the EDB Software Deployment platform.
...
```console
Enter the password for staging_pgai-platform@docker.enterprisedb.com
...
```shell
helm repo add edbpgai "https://downloads.enterprisedb.com/${EDB_SUBSCRIPTION_TOKEN}/staging_pgai-platform/helm/charts"
```
