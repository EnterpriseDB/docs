...
1.  Set `parameters.transporter-dp-agent.rw_service_url` to `https://` followed by $TRANSPORTER_RW_SERVICE_DOMAIN_NAME followed by `/transporter`. Following the first of the previously provided examples, this would be `dms.examplecompany.com`.
