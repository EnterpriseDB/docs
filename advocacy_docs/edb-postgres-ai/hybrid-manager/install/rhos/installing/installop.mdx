...
-   [Install the operator on RHOS in a network-connected environment](installop.mdx#__tabbed_1_1)
-   [Install the operator on RHOS in an air-gapped environment](/install/rhos/installing/#install-the-operator-on-an-air-gapped-envirnoment)

<TabContainer>
<Tab title="Install on a network-connected environment">
...
<Tab title="Install on an air-gapped environment">
...
2.  Select **Operators > OperatorHub**
3.  In the filter box, enter `EDB Postgres AI`.
4.  Filter operators by the CatalogSource that was created.
5.  Select **EDB Postgres AI Operator** and select **Install**.
