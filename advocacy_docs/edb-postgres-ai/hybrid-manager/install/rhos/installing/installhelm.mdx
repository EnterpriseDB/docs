---
title: Install Hybrid Manager on RHOS using Helm
...
bootstrapImageName: "${CONTAINER_REGISTRY_URI}/edbpgai-bootstrap/bootstrap-rhos"
bootstrapImageTag: "${EDBPGAI_BOOTSTRAP_IMAGE_VERSION}"
  ...
  upm-istio-gateway: # For HCP version 1.2.0 and newer
  ...
  # upm-istio:  # For HCP version 1.1.0
  #   cookie_aeskey: "${AES_256_KEY}"
  #   openshift_console_domain_name: "${OPENSHIFT_CONSOLE_DOMAIN_NAME}"
  #   ingress_http_node_port: "32542"
  #   ingress_https_node_port: "30288"
  #   ingress_grpc_tls_node_port: "30290"
  #upm-beaco-ff-base:
  #  cookie_aeskey: "${AES_256_KEY}" # For HCP version 1.0.x
  #  openshift_console_domain_name: "${OPENSHIFT_CONSOLE_DOMAIN_NAME}"
  upm-trust-manager:
...
See [Image discovery configuration](https://docs.redhat.com/en/documentation/assisted_installer_for_openshift_container_platform/2023/html/assisted_installer_for_openshift_container_platform/assembly_configuring-the-discovery-image) for detailed instructions on how to do this.
...
beaconAgent:
  ...
  provisioning:
    ...
    imagesetDiscoveryContainerRegistryURL: "us-east1-docker.pkg.dev" # For HCP version 1.2.0 and newer
  providers:
    - appliance
beaconServer:
  logLevel: "debug"
...
```bash
oc logs -f -l app=edbpgai-bootstrap -n edbpgai-bootstrap
```
