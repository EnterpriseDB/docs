---
title: Hybrid Manager HA/DR recovery
...
5.   After the cluster is successfully restored and in a healthy state, restart the `accm-server`  in the namespace `upm-beaco-ff-base`. 
