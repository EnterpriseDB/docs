...
1.  Paste the following template into the created `beacon-agent.service` file and replace the information in placeholders `< >`.
    ...
    Environment="PATH=</path/to/sqlplus/binary>"
    ...
    | `Environment=`      | `"PATH=</path/to/sqlplus/binary>"`     | Set to the directory of the `sqlplus` utility to enable schema extraction during runtime.                                   |
    | `Environment=`      | `"ORACLE_HOME=</path/to/oracle/home>"` | Set to the directory of the Oracle database server to enable `sqlplus` to extract schemas from the desired Oracle database. |
