...
-  [Enable archive log mode](#step-1-enable-archive-log-mode).
...
Log in to Oracle Container Database (CDB) as sysdba:
...
If you've already Archivelog Mode, you can skip this step. If not, log out from your current SQL session first, then run the following commands to enable it.
...
Then switch to the **PDB** that you want to migrate:
...
To perform data migration, both DMS and its underlying Debezium library require a dedicated **common user**. Before you can create this user, you'll need to set up two distinct tablespaces for it to use.
...
Create a **CDB** tablespace using the SQL command:
...
/opt/cdcreader/reader/oracleConfigValidation.sh
