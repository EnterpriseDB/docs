...
To migrate data from an AWS RDS Postgres database to an HCP-managed database:
...
You can either use an existing HCP-managed cluster or create a new cluster to use it as destination for the migration.  
...
To configure and run the EDB DMS Reader on your auxiliary machine, you need to obtain HCP migration credentials, and then provide the EDB DMS Reader with access to this credentials and other settings. 

This will allow the EDB DMS Reader to have access the source database, and connect it to the HCP migration service.
...
After you have configured the EDB DMS Reader to establish connections to the HCP, create a new migration in the HCP Portal. A migration establishes a sync between the source database and a destination database with the help of EDB DMS.
