...
Migrations are processes you can start in a project of HCP to move data from one database to another. These processes include migrating an external Oracle database to an HCP-managed cluster and to an externally managed Postgres instance, among others.
...
-   **Destination** — Name of the destination, either a database in an HCP-managed cluster or a destination configured through the **Destinations** page.

-   **Status** — Current status of the migration.

### Status

HCP can display the following statuses for a migration.

| Status    | Description                                                                                                                                                                   |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Creating  | HCP is creating resources required to perform a migration (for example, Kafka nodes).                                                                                         |
...
| Failed    | The migration encountered an error and could not be completed. Investigate HCP UI logs to understand the specific cause.                                                      |
