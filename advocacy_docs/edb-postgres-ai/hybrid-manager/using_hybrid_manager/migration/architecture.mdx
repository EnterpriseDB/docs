...
**Agent:** A component that connects to the Oracle source to read and extract schemas for migration. Additionally, registering the source with Hybrid Manager, provides a view of the source database within the HM Console for monitoring, and migration assessment.
...
**Migration Portal:** A service within Hybrid Manager that handles Oracle-specific schema assessment, conversion, and import to the destination database, while ensuring compatibility with Postgres. It can also assist with schema-related issues using an AI Copilot.
