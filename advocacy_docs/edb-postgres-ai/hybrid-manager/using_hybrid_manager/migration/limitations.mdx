...
Although we don't support the migration of `TSVECTOR` type columns, you can apply this workaround to enable their migration. For example, this `messages` table contains a `TSVECTOR` column:
...
The first option is to create a trigger that automatically updates the `tsvector` field. Create the trigger on the destination table before migration and configure it to fire before `INSERT` or `UPDATE` statements.
...
Since the first option won't compute the `TSVECTOR` field for existing rows, the second option is to run the following `UPDATE` statement after the migration is complete to populate the column for all migrated data:
