...
And then when you create the migration, you need to exclude the `TSVECTOR` column in the UI when you review the tables in the `Review your Tables` page by unchecking the `tsv` column.

After the migration finished, you will get the `title` and `body` columns migrated to the target database. Now you can add the `TSVECTOR` column to the target table like this:
