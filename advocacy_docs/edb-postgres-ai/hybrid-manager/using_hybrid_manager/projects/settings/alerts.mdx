...
Alerting is integral to the Hybrid Control Plane (HCP) Observability ecosystem, which proactively notifies about critical events that impact the performance of the Postgres cluster. It continuously monitors essential performance metrics and triggers alerts whenever predefined thresholds are exceeded. This robust alerting system integrates seamlessly into the HCP ecosystem, leveraging real-time data collected from Postgres clusters and their infrastructure across all regions. Alerts are enabled by default for each project created in the HCP.
...
1. For HCP clusters:
      ...
      - Go to Console, and on the top select Projects
...
2. For external HCP clusters:
  
      - Go to Console, and on the top select Estate
...
**Download CSV** &mdash; You can download the alert details in CSV format using the Download button in the top-right corner. The CSV file includes these columns:
...
1. On top of the console, Select Projects
1. Select any project from the list
1. In the left navigation, go to Setting
1. Select Alerts to access/edit the alerts configuration

For each alert, select the Edit button, to:
      
- Enable/Disable &mdash; To activate or deactivate a specific alert as required
- Notify After &mdash; To set the time duration after which the alert is triggered
- Threshold &mdash; To specify the percentage or values that will trigger an alert when they are exceeded.
...
!!!note
    ...
    - All the alerts are not available for external HCP clusters due to the technical limitations of making them available.