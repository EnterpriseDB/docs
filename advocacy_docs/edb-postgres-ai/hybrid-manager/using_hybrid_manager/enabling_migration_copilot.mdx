...
After deploying the EDB Postgres AI Platform with Hybrid Control Plane, [Migration Portal](https://www.enterprisedb.com/docs/migration_portal/latest/), a Launchpad tool to help you migrate external databases, works out of the box. 
...
You can enable the AI Copilot before or after you install the Hybrid Control Plane.
...
1.  Check if the `edb-migration-copilot` exists:
    ...
    The namespace is created during the installation of HCP. If you are enabling the AI Copilot before installing the HCP, you must create the namespace in advance.
...
5.  Create a new file called `migration-portal-values.yaml` with the following helm value to override the default AI vendor secrets with the secret created in the previous step. 
    ...
    - The LLM version specified in `chat_model` is `gpt-4o` by default as it is the recommender version for the copilot.
