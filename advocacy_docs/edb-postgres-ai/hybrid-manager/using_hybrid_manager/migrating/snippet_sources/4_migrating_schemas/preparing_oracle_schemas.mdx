...
Use the collaborative [project-scoped workspace](access_mp.mdx#project-scoped-workspace-vs-users-workspace) of the Migration Portal to convert and migrate schemas. The Hybrid Manager provides an integration that lets you use the HM Console to create a Migration Portal project where you can easily add source database schemas pre-loaded by the [Agent](../migration_guides/agent_for_oracle_readiness.mdx) while you are performing a database migration.
...
3.  Select the schemas you want to migrate, but disable **Constraints** from the list of objects underneath the database. Select **Next**.
    ...
    !!!note
        ...
        Ensure you exclude foreign key, check, and exclusion constraints from the SQL-formatted DDL before importing the schema to the destination database. Review [Schema integrity and performance considerations](../../../../migration-etl/data-migration-service/getting_started/prepare_schema.mdx#schema-integrity-and-performance-considerations) to learn more about why constraints must be disabled and to review other considerations. 
