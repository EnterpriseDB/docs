...
```
edbctl credential import-access-key [options]
```
...
```
-n, --name string         Credential name
-k, --access-key string   Access Key
-a, --address string      Authenticator host address
-o, --port string         Authenticator host port
...
# Import the specific Access API key using the name, access-key, address, and authenticator's host port number:
edbctl credential import-access-key --name cred1 --access-key <your-key-here> --address <authenticator-host-address> --port <authenticator-host-port>

# Import the specific Access API key using options `-n`, `-k`, `-a`, and `-o`:
edbctl credential import-access-key -n cred1 -k <your-key-here> -a <authenticator-host-address> -o <authenticator-host-port>
```
