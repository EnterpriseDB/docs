---
title: edbctl
navTitle: Operations using CLI

---

`edbctl` is a command line tool to manage HCP resources and perform database cluster operations. 

To use `edbctl` you must download the package from EnterpriseDB's repository. 
...
After installing the package, configure an access key using the "credential" command to proceed using this tool. These credentials allows you to access the supported HCP resources and manage them using the edbctl CLI. The CLI supports various commands for managing projects, clusters, backups, credentials, and more.
