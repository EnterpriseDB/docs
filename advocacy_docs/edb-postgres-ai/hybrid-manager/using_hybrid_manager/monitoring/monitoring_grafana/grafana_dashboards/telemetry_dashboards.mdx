...
These dashboards provide insights into the Hybrid Manager (HM) platform itself and its underlying components.
...
**Description:** Provides metrics about the FluentBit instance itself, which is used to collect logs from the nodes, and ship them to Loki, which in turn informs Hybrid Manager.
...
**Usage:** Provides several views of the hardware resources of the servers hosting Hybrid Manager. Useful for identifying resource bottlenecks at the node level.
...
**Description:** Monitors the health and performance of the Prometheus monitoring system,  which is used to collect and ship metrics within Hybrid Manager.
...
-   **Overview** provides a high-level overview of the Thanos deployment within Hybrid Manager.
