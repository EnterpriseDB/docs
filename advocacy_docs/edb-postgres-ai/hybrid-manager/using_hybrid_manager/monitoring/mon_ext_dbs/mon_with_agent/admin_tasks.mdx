...
To configure the Beacon Agent, the user requires access to some resources that they can’t obtain by themselves. However, these resources are available to administrators, as they have the rights and permissions to configure certificate and host-related settings during or after the installation of the Hybrid Control Plane.

## What is the `beacon_server`?

To configure the Beacon Agent, the user must set the `beacon_server` value in the `beacon_agent.yaml` file. This is a string consisting of two values: host and port. 
