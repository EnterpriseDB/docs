...
The Agent makes a secure connection to the `beacon_server` URL using TLS. It is therefore necessary that the machine on which the Beacon Agent is running trusts the server certificate provided by `beacon_server`.
