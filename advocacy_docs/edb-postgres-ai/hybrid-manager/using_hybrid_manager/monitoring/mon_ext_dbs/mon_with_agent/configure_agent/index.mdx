...
1.  Ensure that following environment variables are available in your configuration.
    ...
    - Set the `DSN` environment variable to the Data Source Name (DSN) of one database in the server that you want to report usage about.
      ...
      ```
      export DSN="<postgresql/oracle>://<user>:<password>@localhost:5432/<database_name>"
      ```
...
For an initial test of the Agent, you can get it to send the data that it would normally send to the Hybrid Manager to standard output, your terminal session, instead. This allows you to quickly confirm if the agent is successfully able to gather data from the source database and what that data looks like.
