...
- “Max Execution Time”,
- “CPU AAS”, CPU usage 
- “Wait AAS”, time spent on some wait events for average active sessions (AAS),
- “Total AAS”, time spent by CPU for running the query for average active sessions (AAS),
- “Count”, number of executions for users, databases and nodes.
...
For example, if a single session runs the same query continously for an hour, that query would be said to occupy 1 AAS for that hour.
