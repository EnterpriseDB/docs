...
description: Learn about settings to configure when creating a PG AI Hybrid Manager cluster.
---
...
**Password** — Enter a password for your cluster. This is the password for the user edb_admin. This password is needed to connect to the database and isn't stored on the Hybrid Manager (HM).
...
**Deployment Location** — Select the location for the cluster to be deployed into. The default location is in the Kubernetes cluster hosting the EDB Postgres AI service. Where the HM has been configured in a multi-HM environment, you can select the location for the cluster to be deployed into.
...
1.  In the **Postgres Type** field, select the type of Postgres you want to use:
    ...
    -   **[EDB Postgres Advanced Server](/epas/latest/) is EDB's Oracle-compatible database offering. View [a quick demonstration of Oracle compatibility on EDB Postgres AI](../../migration/migrations/). EDB Postgres Advanced Server is compatible with all three cluster types.
...
**Use a separate storage volume for Write-Ahead Logs** — By default, HM uses a separate storage volume for database storage and write-ahead logs (WAL) files. This default gives best performance and reliability. Clear the check box to store WAL files on the same volume as the database storage.

By default, HM uses a separate storage volume for database storage and Write-Ahead Logs (WAL) files. This default gives best performance and resilience. If you deselect it, WAL files will be stored on the same volume as the database storage.
