...
Rollback after a major version upgrad is not possible with these types of clusters. If you anticipate the need to roll back, create 
To undo the major upgrade, create a logical dump of your databases using the `pg_dump` or `pg_dumpall` tools, create a new cluster using the desired Postgres version, use `pg_restore` to load the logical dump into it, and switch application workloads over to the new cluster.
