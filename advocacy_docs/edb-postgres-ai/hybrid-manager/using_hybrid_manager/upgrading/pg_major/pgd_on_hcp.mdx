---
title: Postgres major upgrades on PGD with HM
navTitle: PGD with HM
description: How to perform PG major upgrades on PGD clusters with HM
deepToC: true

---

Upgrading Postgres major versions on Postgres Distributed (PGD) with Hybrid Control Plane (HM) with zero downtime is much simpler than previous methods of doing this with PGD at scale.
...
The prerequisites to get started are to have a PGD cluster with the old version data group on HM—up and running in a healthy state.
...
Furthermore, you can [view data group state](/pgd/latest/reference/cli/command_ref/group/show/) or perform [deeper monitoring](/pgd/latest/reference/monitoring/sql/#monitoring-replication-peers) on the replication peers to ensure all data is being synchronized as expected.
