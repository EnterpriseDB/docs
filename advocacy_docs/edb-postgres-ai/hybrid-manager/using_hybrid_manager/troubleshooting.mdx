...
In most cases, if you are having disk issues with `stats-collector-db`, then you are getting (some sort of alert that needs to be explicated here).
You are in one of two scenarios when this happens: either there is high disk usage and the cluster is still running, or the disk is full and the cluster is crashing.
