...
HCP disables synchronous replication during maintenance operations of a two-node cluster to ensure write availability. You can also change from the default synchronous replication for a two-node cluster to asynchronous replication on a per-session or per-transaction basis.
...
Since HCP replicates to only one node synchronously, some standby replicas in three-node clusters might experience replication lag. Also, if you override the HCP synchronous replication configuration, then the standby replicas are inconsistent.
