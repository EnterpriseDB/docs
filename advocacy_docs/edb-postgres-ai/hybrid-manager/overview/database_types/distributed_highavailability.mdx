...
Distributed high-availability clusters are powered by [EDB Postgres Distributed](https://enterprisedb.com/docs/pgd/latest/). They use multi-master logical replication to deliver more advanced cluster management compared to a physical replication-based system. Distributed high-availability clusters allows you deploy a cluster across multiple regions or a single region. For use cases where high availability across regions is a requirement, a cluster deployment with distributed high availability can provide up to two data groups in (2) distinct regions along with a witness group in a third separate region.
