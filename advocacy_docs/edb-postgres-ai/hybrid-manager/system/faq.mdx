...
Yes. The entire platform can be deployed and operated in a fully air-gapped environment. All model inference, data processing, and lifecycle management tools run on the installed hardware.
...
You can—but it's not required or assumed. The platform is designed to be fully self-contained. You can operate it entirely offline or selectively enable outbound traffic depending on your internal policies.
...
You can order and rack additional Compute Nodes or GPU-enabled AI Nodes. Once added, Hybrid Manager automatically incorporates them into the cluster and makes them available for workload scheduling.
