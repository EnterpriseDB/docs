...
In managed or multi-tenant deployments where the ability to configure subscriptions has been delegated to non-superuser roles, this crosses a tenant isolation boundary, and the resulting superuser access can be further leveraged against the surrounding database host or orchestration environment. Exploitation requires the attacker to control or impersonate the publisher endpoint for a subscription.
