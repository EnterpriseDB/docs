...
The use case for this approach is for applications that don’t need further development but that need to move out of the data center. Investing in replatforming, which is more resource intensive, might not make sense if no further development is planned. 
...
Although you can use Postgres as the target databases for applications that are restructured, this approach typically requires a fair amount of redesign as well as code refactoring. 
