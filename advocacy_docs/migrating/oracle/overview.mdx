...
Organizations migrate their Oracle based applications to use other database technologies for a variety of reasons including the following:
- cost reduction  
- license flexibility  
...
The scheme is the structure of the database. Schemas include objects for tables and other structures, like indexes for accessing the tables data and database code. This code is written directly in the database. 
...
There are often syntax differences between how the source and target databases interact with the data. Oracle has certain conventions and keywords that it uses in its query that might not be supported in Postgres that you'll need to be aware of.
