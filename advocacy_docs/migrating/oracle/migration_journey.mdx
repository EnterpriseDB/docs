...
- Updating the application connection strings and drivers (e.g., JDBC, ODBC, OCI, .NET). Applications that use open standard connectivity such as JDBC or ODBC normally require changes only to the database connection strings and selection of the EnterpriseDB driver. Review the following driver specific documentation for more information on installing the driver and configuring the application to connect to the EDB Postgres Advanced Server database:
  ...
  Note that applications that use the Oracle .NET driver will need to be updated to replace calls to Oracle driver specific class names to their EDB driver equivalent names.
- Converting embedded SQL in application. The SQL might have been built to work with the source database. Sometimes there's a specific syntax that Oracle supports that might not be supported in Postgres. In this case, you might need to do some conversion of that SQL so that it works with Postgres.  EDB's compatibility for Oracle features built into EDB Postgres Advanced Server and into the EDB drivers reduces the amount of conversion that will be required.
- Updating application code as necessary.  In some cases, an application may contain code that includes Oracle driver specific calls that are not available in the Postgres driver.  Such code will need to be updated to implement Postgres based workarounds.
- Migrating applications. Depending on the migration project, there might be some movement of the actaul application&mdash;where it's hosted and its configuration&mdash;that needs to take place along with migrating the source code.  In general, this should not result in any compatibility concerns.  However, it will be important to verify the application's database connectivity, functionality, and performance in its new environment.  In addition, when migrating an Oracle application to Postgres, different techiniques, tools, and configurations may be needed to address connection pooling and load balancing needs.

## 6. Migrate reports and management tools

In this phase, you’ll migrate reports, DBA utilities, and scripts. Organizations often have reporting mechanisms and other management tools that also query and interact with the database in support of the core applications. Since some of these artifacts were built specifically to support Oracle based processes, they may not all be applicable or needed in the post-migrated system.  As such, it is a good idea to first review the inventory of reports, utilities, scripts, and similar artifacts to determine which can be left behind, which should be replaced, and which can and should be updated. 

Updating a script or utility will often involve updating any incompatible SQL built for Oracle to be compatible with the target database.  It may also involve updating them to work with the command line interfaces provided by the target database instead of the Oracle command line interfaces.  For example, SQL\*Plus is Oracle's primary command line interface; whereas, psql is the one provided with Postgres.  Although SQL statements can be issued in both interfaces, the other supporting commands and keywords they provide are dramatically different. Many Oracle scripts and utilities are built using SQL\*Plus commands.  To get these scripts to run with psql, they would need to be converted to use psql commands instead.  

Fortunately, EDB provides EDB\*Plus, which is a SQL\*Plus work-alike utility that understands some of the most commonly used SQL\*Plus commands.  This capability coupled with EDB Postgres Advanced Server's other built in compatibility for Oracle features provides the option to run many existing scripts built for Oracle with EPAS.  EPAS users also have the option of using a version of psql that has been extended to work with the compatiblity for Oracle features built into EPAS.  After becoming familiar with psql, users will often decide to use it instead of EDB\*Plus to take advantage of its capabilities.  Therefore, they may still choose to convert their SQL\*Plus based scripts and utilities to psql based ones.

Another popular interface provided by Oracle is SQL\*Loader, which is a tool used to bulk load data into Oracle.  EDB provides a work-alike utility called EDB\*Loader that can be used to load data into EDB Postgres Advanced Server.  Like EDB\*Plus, EDB\*Loader provides a farmiliar interface to Oracle users that can be used with EPAS, thus reducing the amount of changes to existing data loading procedures that may be required.  Of course, Postgres native methods for loading data are also available to EPAS users.  If these are chosen, any data loading processes based on Oracle specific capabilities will need to be updated.

There may be other reports or management tools used by the organization that are used to retrieve information from the Oracle database.  The SQL queries issued by these tools should be assessed to determine whether they are compatible with the target database.  Assuming the queries are applicable for the new system, they will need to be updated if SQL incompatibilities or other operational differences are identified.
...
Whatever the size of your migration, you’ll need to validate that the migrated application operates as expected. You'll be testing all along, but at some point, you need to do a formal set of tests to:
 ...
 -  Validate that the data has been fully migrated to the target database and consistent with the source database
...
For this step, it is important to set up a test environment that will allow the functionality and performance of the database and application to be adequately tested. These application and database tests should be run with the the actual application or a copy of the application under workloads simulating the expected production workloads and in an operating environment that will be similar to the production environment. The functional tests should provide sufficient coverage to be able exercise application code and interact with database objects that may be less frequently utilized under normal workloads.  

When it comes to data validation, the tools and processes used to compare the data in the source and target databases need to make it easy to identify differences.  If differences do exist, some analysis will be required to understand why they have occurred and whether some changes in the migration process are required. Depending on how often the data in the target system is refreshed with data from the source system, data validation checks may need to be performed more than once.  In the case of an ongoing CDC based data migration, a plan for validating the data periodically should be established.

In this step, it is also a good idea to setup and exercise the tools and processes that will be used to meet availability requirements, perform backup and recovery operations, and monitor and manage the database.  Doing so, will help identify any changes that may be required to operational procedures or the configuration of the tools and related systems in preparation for their use in the production environment.

One of the main goals of this step is to identify possible issues that need to be addressed before proceding with a final migration to the production system.  Needless to say, if issues are found and corrected some amount of additional testing will be required.  When the results of the pre-production migration tests are satisfactory, final prepartion of the production environment can be performed. 

## 8. Optimize and configure the post migration system

In this phase, you'll ensure that all necessary software is installed and update the production environment with changes identified and verified during the testing phase. You'll also configure and tune the system based on recommendations derived from previous performance testing results. Database parameters and application settings may need to be adjusted to prepare the system to run optimally.  Any query tuning related updates that were identified and verified in the testing phase should also be applied.

This phase is also the time to finish addressing your high availability, disaster recovery, and security requirements, including how users and applications  authenticate to the database and who's authorized to perfom each action. Standard operating procedures (SOPs) for ensuring these requirements are met and for  monitoring and maintaining the database should be updated as necessary.

## 9. Complete cutover/go live

This phase consists of the following:

-  Completing the migration of data into the production database.  If data is being migrated from the source database using a CDC based approach, further changes to the source databases should be disabled to ensure that the new database becomes in synch with the old database.  Note that in general it should be fine to continue to allow read-only queries of the source database if desired.
-  Seting up a rollback option. Set up a rollback configuration so that the old database is receiving updates from the new database and it'll be there as a fallback position in case some issue with running the applications with the new database occurs. This way, you can go back to running the application against the original source database. Moving data back to the target database is often done through a change data capture mechanism.
...
-  Testing the new system until a Go/no-go decision is made. Test the new application and database for some period of time.  Once you're confident that the application is performing well in the new environment, you can declare that the application is a "go" for production.
-  Finishing cutover to production. Once you complete the cutover and are satisfied with it, usually you'll remove the fallback position of sending the change data capture back to the target data source database. At that point, you'll run only in the new production system.
