...
[^1]: PEM can monitor an EDB Postgres® AI Cloud Services cluster (formerly BigAnimal cluster) but with some limitations. The monitoring services provided by the cloud service provider are used in conjunction with PEM or instead of it. PEM isn't applicable for monitoring Cloud Native Postgres instances.  
...
[^3]: EDB Postgres Distributed isn't available for the EDB Postgres® AI Cloud (formerly BigAnimal) and Cloud Native PostgreSQL offerings. When it becomes available, its capabilities will be integrated as an option directly in the EDB Postgres® AI Cloud Service (formerly BigAnimal) and Cloud Native Postgres operators.  
