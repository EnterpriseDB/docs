...
These features simplify migrations from Oracle. EDB Postgres Advanced Server compatibility features mean there are fewer things to convert because things that worked in Oracle will work in EDB Postgres Advanced Server. Although EPAS doesn't fully implement all Oracle features, the compatibility with Oracle features that are implemented are some of the most commonly used constructs. In addition, EDB continues to add compatibility features with each major release. See [Comparison of EDB Postgres Advanced Server with Oracle](consider_compatibility) for information on compatibility between Oracle and EDB Postgres Advanced Server. 
