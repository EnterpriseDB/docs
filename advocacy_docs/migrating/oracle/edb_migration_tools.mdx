...
[^1]: PEM can monitor a cluster managed with EDB Postgres AI Cloud Services (formerly BigAnimal) but with some limitations. The monitoring services provided by the cloud service provider are used in conjunction with PEM or instead of it. PEM isn't applicable for monitoring Cloud Native Postgres instances.  
[^2]: Failover Manager isn't applicable for EDB Postgres AI Cloud Services (formerly BigAnimal) or Cloud Native PostgreSQL (CNP) deployments as the capabilities it provides are built natively into the AI Cloud service or the CNP operator.  
[^3]: EDB Postgres Distributed isn't available for EDB Postgres AI Cloud (formerly BigAnimal) and Cloud Native PostgreSQL offerings. When it becomes available, its capabilities will be integrated as an option directly in the EDB Postgres AI Cloud Services (formerly BigAnimal) and Cloud Native Postgres operators.  
