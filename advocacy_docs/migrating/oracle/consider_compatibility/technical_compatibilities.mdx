...
|-----------------------------|-------------------------------|-------------------------------------------|
| Temporary tables            | Partial                       | Global temporary tables are not supported in Postgres. |
| Views                       | Yes                           |                                           |
| Materialized views          | Yes                           | - Postgres does not provide an automatic refresh feature. Triggers can be used as part of a Postgres solution. <br/> - Postgres does not provide an incremental refresh option, only full refresh. |
| Partitioning                | Yes ✓                         |                                           |
...
| Index organized tables      | No                            | In Postgres, can cluster a table by an index providing similar performance boosts when reading data from a presorted structure |
| Sequences                   | Yes  ✓                        | - In Postgres, some keyword options not available <br/> - Some difference in maximum sequence value |
...
| Bitmap             | Yes              | No                                | Use of BRIN index may be suitable in some use cases |
