...
For information about modifying the `pg_hba.conf` file, see the [*PEM Administrator's Guide*](/pem/latest/).

Link capitalization can be either title or sentence case:

-   **Use title case** and *italics* when referring to the linked doc by name. For example. “For information about modifying the `pg_hba.conf` file, see the [*PEM Administrator's Guide*](/pem/latest/).”).
...
**PGD cluster**: A [PGD cluster](/pgd/latest/overview/terminology/#pgd-cluster) is a [Postgres Distributed](/pgd/latest/) system with a group of multiple redundant database systems in different locations.
