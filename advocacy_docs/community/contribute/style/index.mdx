...
Readers may find your article from a search engine. If your article is split into separate pages, keep in mind that readers may view the pages out of order. It might be useful to alert them to important details that previous pages cover. To do so, we recommend using the `<Attention>` component at the top of your article where prior context would be useful:

```markdown
<Attention>Don't forget to [install prerequisite](/link/article) before trying to connect!</Attention>
```
