...
The following instructions are for yum based systems, such as RHEL, CentOS, and Fedora. 
...
You should now see files in `/var/www/html/repos/enterprisedb-enterprise` if this command is successful. By default, SELinux will block access to these files. To enable httpd to access these files, you can use either run `httpd` in permissive mode or change the SELinux context of the files. To change the SELinux context of the files, use the following command:
...
You will now be able to access the web server from other systems, enabling its use as a mirror.
...
The locally managed repository needs relatively light maintenance, however, you should configure some mechanism to keep the locally managed yum repository up to date, easiest option will be to create a cron job that runs the relevant `dnf reposync` command.
