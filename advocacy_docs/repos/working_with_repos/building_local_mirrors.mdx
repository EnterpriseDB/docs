...
!!! Note
    ...
    The `--repoid` and `--arch` options each target a single repository and architecture at a time. The command above only mirrors the architecture-specific `enterprisedb-enterprise` repository — it doesn't include the separate `enterprisedb-enterprise-noarch` repository shown in the `dnf repolist` output earlier. Some packages, such as architecture-independent Java-based packages, are published only to the `-noarch` repository. If you skip syncing it, those packages won't be present in your local mirror.

    To mirror the `noarch` repository as well, run a second `reposync` command against that repository id. Since `noarch` packages aren't tied to a CPU architecture, omit `--arch` for this one:
    ...
    Repeat this pattern for any other repository IDs from your `dnf repolist` output that you need to mirror, such as `enterprisedb-enterprise-source`.

After the commands complete successfully, there are files in `/var/www/html/repos/enterprisedb-enterprise` (and `/var/www/html/repos/enterprisedb-enterprise-noarch`, if you synced it). By default, SELinux blocks access to these files. To enable httpd to access these files, you can either run `httpd` in permissive mode or change the SELinux context of the files.
...
The locally managed repository needs relatively light maintenance. However, we recommend that you configure some mechanism to keep the locally managed yum repository up to date. The easiest option is to create a cron job that runs the relevant `dnf reposync` commands, including the `-noarch` repository if you're mirroring it.
