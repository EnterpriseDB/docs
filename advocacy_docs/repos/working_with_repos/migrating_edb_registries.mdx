...
description: Migrate an EDB Postgres® AI for CloudNativePG™ Cluster system from a previous tier-based registry configuration to EDB's unified container image registry.
redirects: 
- /postgres_for_kubernetes/latest/migrating_edb_registries
- /postgres_for_kubernetes/1/migrating_edb_registries
---

## What's changing

Previously, EDB published operator and operand images selectively to one of several private repositories: `k8s_standard`, `k8s_enterprise`, `k8s_standard_pgd`, and `k8s_enterprise_pgd`. Your subscription level and which operators you needed determined which repository you configured. Also, you might have used public registries for some operand images. In particular, EDB provided numerous images via public repositories hosted on quay.io.

Now, EDB provides the EDB Postgres® AI for CloudNativePG™ Cluster (CNPG-C) and EDB Postgres® AI for CloudNativePG™ Global Cluster (CNPG-GC) operators and associated operands in a single repository, `k8s`. If your subscription includes access to a given image, you can pull it from this repository. The old repositories remain available for now, but EDB publishes new images to the unified repository. Thus, future operator and operand upgrades will require migrating to the unified repository.
...
-   Your [EDB account token](/repos/getting_started/get_your_token/)
...
For CNPG-C, the default namespace is `postgresql-operator-system`. To create or update the pull secret in this namespace, run:
...
For CNPG-C and operand images, you'll need to update
...
For your CNPG-C Helm releases, you'll need to update override values for both `image.repository` and `image.imageCredentials.username`:
