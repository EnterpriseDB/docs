---
title: License Keys
navTitle: License Keys
description: When and how license keys are used with EDB software
---

Postgres for Kubernetes is deployable from a public registry which requires no pull secret to access. However, when pulled from this registry you will need a license key to use the software. For details on how to obtain a license key, see [License keys](/postgres_for_kubernetes/latest/license_keys/) in the Postgres for Kubernetes documentation.

You do not need a license key to use the EDB Postgres for Kubernetes is you are accessing it through EDB's private registry `docker.enterprisedb.com`. You will need a pull secret to access the repositories.

## Using License Keys with EDB Postgres for Kubernetes
