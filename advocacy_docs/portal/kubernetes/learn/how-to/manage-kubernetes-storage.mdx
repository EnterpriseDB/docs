...
To hide a storage class from the Hybrid Manager cluster-provisioning UI, see [Excluding storage from Hybrid Manager discovery](/edb-postgres-ai/latest/hybrid-manager/install/install_guide/sys_reqs/#62-excluding-storage-from-hybrid-manager-discovery).
