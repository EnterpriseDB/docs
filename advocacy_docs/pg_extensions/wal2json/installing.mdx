---
title: Installing wal2json 
navTitle: Installing
---

`wal2json` is supported on the same platforms as the Postgres distribution you're using. Support for `wal2json` starts with Postgres 12. For details, see:
...
sudo <package-manager> -y install edb-<postgres><postgres_version>-pg-wal2json2
...
For example, to install wal2json for EDB Postgres Advanced Server 15 on a RHEL 8 platform:

```shell
sudo dnf -y install edb-as15-pg-wal2json2
```
