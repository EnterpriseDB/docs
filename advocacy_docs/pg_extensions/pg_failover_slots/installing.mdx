---
title: Installing EDB Failover Slots 
navTitle: Installing
---

EDB Failover Slots is supported on the same platforms as the Postgres distribution you're using. Support for EDB Postgres Tuner starts with Postgres 12. For details, see:
...
```shell
sudo <package-manager> -y install edb-<postgres><postgres_version>-pg-failover_slots<major_version>
```
...
For example, to install EDB Failover Slots 1.0.0 for EDB Postgres Advanced Server 15 on a RHEL 8 platform:

```shell
sudo dnf -y install edb-as15-pg-failover_slots
```

To install EDB Postgres Tuner 1.0.0 for EDB Postgres Advanced Server 15 on a Debian 11 platform:

```shell
sudo apt-get -y install edb-as15-pg-failover_slots-1
```
