...
- `edb_pg_tuner.wal_path` &mdash; Specifies the path to the directory containing the Write-Ahead Log (WAL) files. The default is `/var/lib/edb/pge-14/data/pg_wal` The tuner uses the total size of this directory to limit the suggested `max_wal_size` to 50% of the total `pg_wal` disk size.
