...
- `edb_pg_tuner.wal_path` &mdash; Specifies the path to the directory containing the Write-Ahead Log (WAL) files. The default value is NULL. The tuner uses the total size of the disk where the `pg_wal` directory resides to limit the suggested `max_wal_size` to 50% of the total `pg_wal` disk size.
...
| max_wal_size                   | dynamic  | based on workload and `pg_wal` disk size |         |
