...
EDB Postgres Tuner can be used in two ways: it can automatically apply all tuning recommendations or you can manually apply selected tuning recommendations. 

## Automatic tuning

To automatically apply all tuning recommendations, set the `edb_pg_tuner.autotune` parameter to `true`. Restart Postgres to apply the change and EDB Postgres Tuner begins applying tuning recommendations.  
...
This example demonstrates how to manually apply tuning recommendations to a given `postgresql.conf` file. Here's an example of settings in a `postgresql.conf` file:
...
The tuning recommendations in the `conf` format can be copied and pasted into the `postgresql.conf` file: 
...
The tuning recommendations in the `sql` format can be executed in your preferred Postgres client tool:
...
The following GUCs can be recommended by EDB Postgres Tuner. The `static` category provides fixed recommendation settings. The `dynamic` category uses specific algorithms to suggest a better setting according to your workload or hardware resources.
