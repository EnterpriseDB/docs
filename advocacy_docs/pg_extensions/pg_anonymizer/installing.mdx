---
title: Installing postgresql_anonymizer 
navTitle: Installing
---

`postgresql_anonymizer` is supported on the same platforms as the Postgres distribution you're using. Support for `postgresql_anonymizer` starts with Postgres 13. For details, see:
...
sudo <package-manager> -y install edb-<postgres><postgres_version>-anonymizer
```
...
For example, to install postgresql_anonymizer for EDB Postgres Advanced Server 15 on a RHEL 8 platform:

```shell
sudo dnf -y install edb-as15-anonymizer
```
