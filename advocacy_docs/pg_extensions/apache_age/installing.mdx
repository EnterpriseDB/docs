...
Apache AGE is supported on the same platforms as the Postgres distribution you're using. Support for Apache AGE starts with Postgres 16. For details, see:
...
For example, to install Apache AGE for EDB Postgres Advanced Server 17 on a RHEL 8 platform:

```shell
sudo dnf -y install edb-pg16-age1
```
...
For example, to install Apache AGE for EDB Postgres Advanced Server 17 on a Debian platform:

```shell
sudo apt-get -y install edb-pg17-age-1
```