...
The function generates potential candidates from the multi-column filters of your queries. Then, these candidates are processed by exploring different possible combinations. Currently the focus is on statistics for two columns at a time. It also shows the weights to each candidate.
...
|                             statistics                                                 |       weight       |
+----------------------------------------------------------------------------------------+--------------------+
| CREATE STATISTICS part_p_brand_p_container ON p_brand, p_container FROM part;          | 4940012.436935346  |
| CREATE STATISTICS part_p_brand_p_type ON p_brand, p_type FROM part;                    | 306202.2549564565  |
| CREATE STATISTICS part_p_brand_p_size ON p_brand, p_size FROM part;                    | 2879764.4054564573 |
(3 rows)
```
...
|                             statistics                                                 |
+----------------------------------------------------------------------------------------+
| CREATE STATISTICS part_p_brand_p_container ON p_brand, p_container FROM part;          |
| CREATE STATISTICS part_p_brand_p_type ON p_brand, p_type FROM part;                    |
| CREATE STATISTICS part_p_brand_p_size ON p_brand, p_size FROM part;                    |
(3 rows)
```