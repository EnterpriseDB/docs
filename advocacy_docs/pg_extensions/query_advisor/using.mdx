...
By default, only predicates filtering at least 1000 rows and 30% of the rows in average considered, but you can use the `min_filter` and `min_selectivity` parameters to override the default. 
...
| dbid             | ID of the database in which the query has been executed.                                                                                                                            |     |
| lrelid, lattnum  | ID of the relation and attribute number of the VAR on the left hand side, if any.                                                                                                   |     |
| opno             | ID of the operator used in the expression                                                                                                                                           |     |
| rrelid, rattnum  | ID of the relation and attribute number of the VAR on the right hand side, if any.                                                                                                  |     |
| qualid           | Normalized identifier of the parent AND expression, if any. This identifier is computed excluding the constants. This is useful for identifying predicates that are used together. |     |
...
| occurrences       | Number of times this predicate has been invoked; that is, number of related query execution.                                                                                              |     |
| execution_count  | Number of times this predicate has been executed; that is, the number of rows it processed.                                                                                          |     |
| nbfiltered       | Number of tuples this predicate discarded.                                                                                                                                           |     |
| queryid          | The queryid identifies this query.                                                                                                                                                   |     |
...
Returns the percentage usage of the workload and qual hash table.

### query_advisor_qualstats_reset
Resets the internal counters and forget about every encountered quals and workload entry.

## Views
The extension defines the the following views on top of the pg_qualstats function. 
