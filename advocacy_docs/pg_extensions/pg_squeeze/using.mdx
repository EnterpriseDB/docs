...
First, make sure that your table has either a primary key or unique constraint. This is necessary to process changes resulting from other transactions  while `pg_squeeze` is doing its work.

To make the `pg_squeeze` extension aware of the table, you must insert a record into `squeeze.tables` table. After it is added, statistics of the table are checked periodically. When the table meets the necessary criteria to be squeezed, a task is added to a queue. The tasks are processed sequentially, in the order they were created.
...
The following is the complete description of table metadata.
...
squeeze.worker_role parameter specifies role which does not have the superuser privileges.
