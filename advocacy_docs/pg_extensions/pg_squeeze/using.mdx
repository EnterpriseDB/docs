...
- `schedule` &mdash; Specifies when the table should be checked and possibly squeezed. The schedule is described by a value of the following composite data type, which resembles a crontab entry:
    ...
    The check is performed if the `minutes`, `hours`, and `months` all match the current timestamp, while `NULL` means any minute, hour, and month respectively. Either `days_of_month` or `days_of_week` must match the current timestamp or both must be NULL for the check to take place.
...
- `rel_tablespace` &mdash; Existing tablespace into which the table should be moved. `NULL` means that the table should stay where it is.
...
The next time you start the cluster, two or more workers (that is, one scheduler worker and one or more squeeze workers) are launched for `my_database` and the same for `your_database`. If you take this approach, note that any worker does not start or stop without doing any work if :

- The `pg_squeeze` extension does not exist in the database, or

- The `squeeze.worker_role` parameter specifies a role that  does not have superuser privileges.

The functions and configuration variables explained above use a singular form of the word `worker` although there are actually two workers. This is because only one `worker` existed in the previous versions of `pg_squeeze`, which ensured both scheduling and execution of the tasks. This implementation change is not worth forcing all users to adjust their configuration files during upgrade.
...
This example specifies that the exclusive lock shouldn't be held for more than 0.1 second (100 milliseconds). If more time is needed for the final stage, `pg_squeeze` releases the exclusive lock, processes changes committed by other transactions in between, and tries the final stage again. An error is reported if the lock duration is exceeded a few more times. If that happens, you should either increase the setting or schedule processing of the problematic table for a different time when write activity is lower.

## Running multiple workers per database

If you think that a single squeeze worker does not cope with the load, consider setting the `squeeze.workers_per_database` configuration variable to a value higher than 1. Then the `pg_squeeze` extension is able to process multiple tables simultaneously - one table per squeeze worker. 

However, be aware that this setting affects all databases in which you actively use the `pg_squeeze` extension. The total number of all the squeeze workers in the cluster (including the "scheduler workers") cannot exceed the in-core configuration variable `max_worker_processes`.
...
The `pg_squeeze` extension does not prevent other transactions from altering a table at certain stages of the processing. If a disruptive command (that is, `ALTER TABLE`, `VACUUM FULL`, `CLUSTER` or `TRUNCATE`) manages to commit before the squeeze finishes, the `squeeze_table()` function aborts and all changes made to the table are rolled back. The `max_retry` column of the `squeeze.tables` table determines how many times the squeeze worker retries. Changing your schedule might help avoid these kind of disruptions.
