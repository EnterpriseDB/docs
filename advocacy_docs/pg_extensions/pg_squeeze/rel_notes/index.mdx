---
title: 'pg_squeeze release notes'
navTitle: "Release notes"
indexCards: none
navigation:
  - pg_squeeze_1.9.4.1_rel_notes
  - pg_squeeze_1.9.2.1_rel_notes
  - pg_squeeze_1.9.1.1_rel_notes
  - pg_squeeze_1.8.0_rel_notes
  - pg_squeeze_1.7.0_rel_notes
  - pg_squeeze_1.6.1_rel_notes
---
The pg_squeeze documentation describes the latest version of pg_squeeze, including minor releases and patches. These release notes cover what was new in each release. For new functionality introduced in a minor or patch release, there are also indicators in the content about the release that introduced the feature.

| Version                                 | Release Date |
|-----------------------------------------|--------------|
| [1.9.4.1](pg_squeeze_1.9.4.1_rel_notes) | 03 Aug 2026  |
| [1.9.2.1](pg_squeeze_1.9.2.1_rel_notes) | 17 Jul 2026  |
| [1.9.1.1](pg_squeeze_1.9.1.1_rel_notes) | 30 Mar 2026  |
| [1.8.0](pg_squeeze_1.8.0_rel_notes)     | 05 Feb 2025  |
| [1.7.0](pg_squeeze_1.7.0_rel_notes)     | 22 Nov 2024  |
| [1.6.1](pg_squeeze_1.6.1_rel_notes)     | 15 Feb 2024  |
