...
EDB's diagnostic tools, Lasso and Postgres Workload Report (PWR), access EDB Wait States data using the `pg_monitor` role. For these tools to collect wait-state data successfully, grant the `pg_monitor` role usage on the schema where the extension objects reside:
