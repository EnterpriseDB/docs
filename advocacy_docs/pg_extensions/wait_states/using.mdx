...
In the following examples, a scenario where three queries are executed simultaneously on four different sessions connected to different databases using different users is employed. Those three queries are:
...
You can check whether the EDB Wait States is collecting data using a `SHOW` command. In the following example, EDB Wait States' data collection is enabled:
