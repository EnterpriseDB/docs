...
`edb_stat_monitor` is supported on the same platforms as the Postgres distribution you're using. Support for `edb_stat_monitor` starts with Postgres 12. For details, see:
...
sudo <package-manager> -y install edb-<postgres><postgres_version>-stat-monitor
```
...
```shell
sudo apt-get -y install edb-<postgres><postgres_version>-stat-monitor-1
```
...
For example, to install edb_stat_monitor for EDB Postgres Advanced Server 16 on a RHEL 8 platform:

```shell
sudo dnf -y install edb-as16-stat-monitor
```

Now move to the [Configuring EDB Stat Monitor](configuring) documentation to configure the extension and start using it.
