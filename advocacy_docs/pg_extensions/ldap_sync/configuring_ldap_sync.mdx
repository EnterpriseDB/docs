---
title: Configuring LDAP-sync
---

LDAP-sync is a collection of tools that synchronize credentials from an LDAP or Active Directory server to EDB Postgres Advanced Server. We recommend that you configure the LDAP-sync tools in the following ways.

Find the sample LDAP-sync configuration files at `/usr/lib/python3.6/site-packages/ldap_sync/samples/`.

## Configure pgAgent

pgAgent sets up periodic synchronization tasks. Use `edb-pgagent.conf_tmpl` as the default configuration for pgAgent setup with EDB Postgres Advanced Server 14. Copy it into the configuration directory for pgAgent with EDB Postgres Advanced Server 14. 
