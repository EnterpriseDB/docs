...
When configuring `pg_hba.conf`, the LDAP authentication string must come after the `enterprisedb` user authentication string or, due to the order of evaluation, the `enterprisedb` user can't log into the system i.e. LDAP server will be queried instead of local authentication, LDAP server will return authentication failure and the authentication process will terminate. For this reason, host authentication for the `enterprisedb` user needs to be added if you want to use host authentication as a part of the ldap2pg setup. The following authentication methods are available: peer, host with a password string via `dsn` option, host via reading `.pgpass`. See [Configure ldap2pg](#configure-ldap2pg) for more information. 
