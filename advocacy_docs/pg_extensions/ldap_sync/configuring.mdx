...
Need to create a SQL file to run pg_cron at some interval. This example uses plpython3u to run ldap2pg every minute to ensure that
the PG database is synced with LDAP. Here's a sample file to do this called setup_pg_cron.sql

```text
create function run_ldap() returns void as $$
import os
os.system("/usr/bin/ldap2pg -N")
$$ language plpython3u;

-- run ldap every minute
select cron.schedule('run ldap', '* * * * *', 'select * from run_ldap()');
```

Use psql to run the previous SQL file to enable pg_cron
