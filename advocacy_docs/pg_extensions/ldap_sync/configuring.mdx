...
The permission settings for the database server need to be modified to allow Postgres to query the LDAP host whenever the Postgres authentication procedure is performed. This is accomplished by updating `pg_hba.conf` in the configuration folder of the database server. For EDB Postgres Advanced Server configuration, `pg_hba.conf` is located in `/opt/postgres/data/pg_hba.conf`.

### `pg_hba.conf` settings

!!! Note
The user varies depending on your database service. With PostgreSQL, there is a `postgres` user. With EDB Postgres Advanced Server, there is an `enterprisedb` user. 
!!!

When configuring `pg_hba.conf`, the LDAP authentication string must come after the `enterprisedb` user authentication string or, due to the order of evaluation, the `enterprisedb` user can't log into the system (for example, LDAP server is queried instead of local authentication, LDAP server returns an authentication failure and the authentication process terminates). For this reason, host authentication for the `enterprisedb` user needs to be added if you want to use host authentication as a part of the ldap2pg setup. The following authentication methods are available: peer, host with a password string via `dsn` option, host via reading `.pgpass`. See [Configure ldap2pg](#configure-ldap2pg) for more information. 
...
The following is a sample `pg_hba.conf` for EDB Postgres Advanced Server without any modifications:
...
ldap2pg is a PostgreSQL role and access control list (ACL) management tool. Find the `ldap2pg.yml` in its default location in the `/etc/` folder. For more information on ldap2pg, see its [documentation](https://ldap2pg.readthedocs.io/en/latest/). 
...
And test the connection from the database server to the LDAP server with the `ldapsearch` command:
...
The `role` section defines the user accounts and options that can be mapped in the PostgreSQL database. The default name parameter for each user record is `cn`, which is the account’s full name, so the `uid` name is specified, which is a short username. 
