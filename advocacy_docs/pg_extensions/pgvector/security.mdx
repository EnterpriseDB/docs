...
RLS works with both HNSW and IVFFlat indexes. However, be aware that a nearest-neighbor query with an RLS filter still traverses the index broadly and then filters results — it does not restrict the index traversal itself. For strict tenant isolation, consider [partitioning](https://www.postgresql.org/docs/current/ddl-partitioning.html) the embeddings table by `tenant_id` and creating separate indexes per partition, or use [partial indexes](https://www.postgresql.org/docs/current/indexes-partial.html) to limit each index to a specific tenant's rows. See [Partial indexes](using#partial-indexes) for a pgvector example.
...
Community PostgreSQL does not include TDE. For encryption at rest in community PostgreSQL, use filesystem-level or block-device encryption (for example, LUKS on Linux or encrypted EBS volumes on AWS).
