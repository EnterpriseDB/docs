...
!!! Note 
    There is also a `pgfs.create_storage_location` function with `msl_id`  as the third parameter for a managed storage location id. The `msl_id` with `UUID` type is optional, and if not specified, set to null.
