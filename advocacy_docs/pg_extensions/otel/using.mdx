...
## edb_otel.pg_exporter_base_definitions  (meters) 

This view contains the meter (queries and their metrics) definitions that `pg_exporter` brings by default.  They can be refreshed from [1] or [2] by running yq on the directory, such as:

```yq
yq eval-all -o=json '. as $item ireduce ({}; . * $item)' [...]/config/collector/*.yml
```

[1]: https://github.com/EnterpriseDB/upm-metrics-pg-exporter/tree/main/config/collector
[2]: https://github.com/Vonng/pg_exporter/tree/main/config/collector
