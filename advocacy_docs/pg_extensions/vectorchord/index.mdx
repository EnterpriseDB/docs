...
VectorChord adds a high‑performance vector index to Postgres that works with the `pgvector` `vector` data type.
It is independent of pgvector’s HNSW/IVF indexes and targets large‑scale, low‑latency vector search.
