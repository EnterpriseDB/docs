...
EDB SPL Check can be run in active mode or passive mode. In active mode, checks are started by running `spl_check_function`. In passive mode, functions are checked when executed.

!!! Note
    To update EDB SPL Check, drop the old version of the extension before installing the new version. 

## Active mode

In active mode, you can run checks with `spl_check_function`. Active mode is the default behavior for EDB SPL Check. However, you can change this with the `plpgsql_check.mode` setting. See [Configuring passive mode](#configuring-passive-mode) for more information.
...
When the `compatibility_warnings` flag is active, `plpgsql_check` tries to identify incorrect assigning to refcursor's variable or returning of refcursor's variables:
...
```sql
load 'edbspl'; -- 1.1 and higher doesn't need it
load 'spl_check';
