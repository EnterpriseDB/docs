...
<!-- Check edit (deleted or) Last sentence feels like a non sequitur-->]
...
If you plan to use `plugin_debugger` (the plpgsql debugger) with EDB SPL Check, we recommend that you initialize `plugin_debugger` first. `plugin_debugger` doesn't support sharing the PL/pgSQL debug API. 
