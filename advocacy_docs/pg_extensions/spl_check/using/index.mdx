...
When the GUC `spl_check.validate_function_calls` is set to true, `spl_check_function()` checks the validity of a specified function (its parameters and return type) and then looks inside the function to check the validity of all functions that are subsequently called. For example, if function `a()` calls function `b()` and function `b()` calls function `c()`, `spl_check_function()` checks the validity of all those functions. 

Use the `SET` command to set the value of `spl_check.validate_function_calls`. When `spl_check.validate_function_calls` is set to false, `spl_check_function()` only checks the validity of the specified function and none of the internal functions.

`spl_check_function()` does not check any functions with an OID less than `FirstNormalObjectId` (that is, functions defined in the `sys` or `pg_catalog` schema). They are assumed to be system functions. Also, `spl_check_function()` skips system packages and functions written in languages other than EDB SPL.
