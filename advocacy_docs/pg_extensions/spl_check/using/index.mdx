...
Use the `spl_check_function()` to check all of your functions/procedures and to check all your triggers. By default `spl_check_function()` only checks the validity of parameters and return types. However, a GUC allows you to use `spl_check_function()` to [validate the function body](#checking-function-bodies) of all function calls (except system functions and system package functions).   
...
When the GUC `spl_check.validate_function_calls` is set to true, `spl_check_function()` looks inside a function to check its validity. Use the `SET` command to set the value of `spl_check.validate_function_calls`. When `spl_check.validate_function_calls` is set to false, `spl_check_function()` only checks parameters and return types for function calls.

Note that for any functions with an OID less than `FirstNormalObjectId`, functions defined in the SYS and PG_CATALOG schema are not checked because they are assumed to be the system functions. Also, system packages and functions written in languages other than edb-spl are skipped.
