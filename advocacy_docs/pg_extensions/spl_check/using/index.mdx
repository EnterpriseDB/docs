...
`spl_check` does not check any functions with an OID less than `FirstNormalObjectId` (that is, functions defined in the `sys` or `pg_catalog` schema). They are assumed to be system functions. Also, `spl_check` skips system packages and functions written in languages other than EDB SPL.
