...
Bluefin is best when used with time-range partitioning. The example below shows a table containing logs of trucks, that get inserted periodically when each truck provides updates of its status.
