...
- `edb_job_scheduler.database_list` &mdash; Specifies a comma-separated list of database names for job scheduling. If you modify this parameter you must
  - reload the server for the changes to take effect using `pg_reload_conf()` or `pg_ctl reload` for EDB Postgres Advanced Server version 16 and later. 
  - restart the server for the changes to take effect using `pg_ctl restart` for EDB Postgres Advanced Server version 15 and earlier.
