...
- `sys.dump_schedule()` &mdash; Displays the current job schedule from the scheduler's local memory. The scheduler's local memory dumps all the jobs that weren't executed yet into the server log in JSON format. 
