...
7.  Open the agent configuration file located at:
...
8.  Take a backup of the service file and agent certificates:
...
9.  Uninstall the PEM agent using bitrock uninstaller:

    ```sourceCode text
    /PEM_installation_path/agent/uninstall-pemagent
    ```

10. Use version specific commands to restore the service file backup and agent certificates to original location. For example:
...
11. Enable the `pemagent` service, and start `pemagent` and `httpd`.
...
Upgrading a PEM Server Installed with a Graphical Installer
