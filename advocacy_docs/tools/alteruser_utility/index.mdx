...
The `alteruser` utility helps to alter an existing PostgreSQL role in your EDB Postgres Advanced Server's databases.
...
`-c N` or `--connection-limit=N`  
 ...
 Set a maximum number of connections for an existing role.

`-d` or `--createdb`            

 Allow an existing role to create new databases.

`-D` or `--no-createdb`         

 An existing role cannot create databases.

`-i` or `--inherit`             

 An existing role inherits privileges of roles it is a member of.

`-I` or `--no-inherit`          

 An existing role does not inherit privileges.
  
`-l` or `--login`               
 
 An existing role is allowed to login.

`-L` or `--no-login`            

 An existing role cannot login.
  
`-P` or `--pwprompt`            

 To assign a password to an existing role.
  
`-r` or `--createrole`          

 An existing role will have CREATEROLE privilege. This role is allowed to create, alter, drop, comment on, change the security label for other roles.
  
`-R` or `--no-createrole`       

 An existing role will not be allowed to create new roles.
  
`-s` or `--superuser`           

 An existing role will be a superuser.
  
`-S` or `--no-superuser`        

 An existing role will not be superuser.
...
`--bypassrls`               
 ...
 An existing role can bypass row-level security (RLS) policy.
  
`--no-bypassrls`            
 
 An existing role cannot bypass row-level security (RLS) policy.
...
`--account=MODE`            
 ...
 An existing role mode can be "LOCK", or "UNLOCK".
...
`--rename=ROLENAME`         
 ...
 An existing role is renamed to new name.
