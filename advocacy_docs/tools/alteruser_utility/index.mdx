...
`--encrypt-password=METHOD`
 ...
 An existing role will have selected encryption method for password. This option must be used along with `--pwprompt` option. Encryption method can be "scram-sha-256", "md5", "server_encryption" or "plain" (default). If "server_encryption" is selected, the password will be encrypted using server's `password_encryption` GUC value. If "plain" is selected, the password will not be encrypted and will be sent in the plain text.
...
`--rename=ROLENAME`         
 ...
 An existing role will be renamed to new name. No other options can be used while using `--rename` option.
