...
1.  Omit the password from the DSN, and set the `user` parameter instead:
    ...
    ```shell
    dsn: "postgresql://<host>:<port>/<database>?user=<user>"
    ```
...
1.  On the machine running the agent, provide the client certificate, key, and root certificate. Either place them at the default libpq locations (`~/.postgresql/postgresql.crt`, `~/.postgresql/postgresql.key`, and `~/.postgresql/root.crt`), or reference them explicitly in the DSN:
    ...
    ```shell
    dsn: "postgresql://<host>:<port>/<database>?user=<user>&sslrootcert=/path/to/root.crt&sslcert=/path/to/postgresql.crt&sslkey=/path/to/postgresql.key"
    ```
