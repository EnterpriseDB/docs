...
1.  Set `$DSN` to the Data Source Name (DSN) to of a database of the server that you want to report usage about.
...
1.  You can now [configure the agent to run as a service](/edb-postgres-ai/hybrid-manager/using_hybrid_manager/monitoring/mon_ext_dbs/mon_with_agent/run_as_service/). Reports are generated every 12 hours while it's running, but you'll have to manually submit them . 
