...
Configure the EDB Postgres AI agent (packaged as `beacon-agent`) to allow it to connect to and fetch basic usage data from the source database and send it to EDB.
...
4.  The agent is now configured, proceed to [running the agent](/edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/monitoring/mon_ext_dbs/mon_with_agent/run_agent/). 
