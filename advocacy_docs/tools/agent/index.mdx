...
However you use the Agent, it needs credentials to connect to your source database. By default, this means a username and password in the connection string. If you'd rather not store a password in plain text, refer to [Authenticating to source databases](/tools/agent/authentication/) for password-less alternatives, including Oracle Wallet, pgpass, and Postgres client certificates.
