...
The number of databases you need to register depends on your specific goals:

* **For Hybrid Manager (HM):** To achieve full observability and a centralized monitoring hub, register every database in your organization with the Agent. You must configure a DSN string for each database on that host.
* **For Standalone Databases:** If using the Agent only for basic usage tracking, you only need to provide access to one database per database server instance. You only need to configure a DSN string for that single representative database rather than every database on the server.

!!! Note
    Registering every database is the "best practice" for visibility, but the "one per instance" rule is sufficient for simple license/usage tracking on standalone servers.