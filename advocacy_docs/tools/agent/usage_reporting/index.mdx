...
description: Learn about usage reporting with PGAI agent.
...
Usage reporting with the PGAI agent is an opt-in service. Data is only sent to EDB when you explicitly enable it. When enabled, the PGAI agent sends a new report every 12 hours by default, starting with the first time the PGAI agent runs.
