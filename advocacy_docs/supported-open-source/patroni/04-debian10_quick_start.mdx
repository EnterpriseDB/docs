...
This section walks you through the various steps to create an highly available **demo** cluster (using **EDB Postgres Advanced Server** version 14 on Debian 10) where the etcd hosts will be called **etcd1**, **etcd2** and **etcd3**, and the two EPAS nodes in _Streaming Replication_ are **pg-patroni1** and **pg-patroni2**.
