...
Instead of connecting directly to the database server, you can set up HAProxy so the application connects to the proxy instead. The proxy then forwards the request to PostgreSQL. By using HAproxy, you can also route read-only requests to one or more replicas for load balancing. To do so, set up a separate port for it, as shown in the configuration example that follows.

You can install HAproxy as an independent server. You can also install it on the application server or the database server. We recommend having two independent HAProxy servers running on independent nodes.

To install HAProxy on RedHat Enterprise Linux, log in to each proxy node and run the installation statement:

```bash
sudo dnf install -y haproxy
```

After you install HAProxy, you can configure the connections to two supplied demonstration PostgreSQL nodes:
...
The configuration has two sections: read-write using port 5000 and read-only using port 5001.

Include both PostgreSQL hosts in both sections because they are both potential candidates for either primary or standby. For HAproxy to know each host's role, it queries the Patroni REST API:
