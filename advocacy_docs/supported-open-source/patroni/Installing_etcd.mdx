...
You can check the version installed and the latest available version, you can use the following commands which will output the version of the installed etcd binary and get the latest version from the etcd-io website.
...
Always verify the version installed with:
...
Then, setup the local etcd configuration (we will be placing the etcd.conf file inside /etc/etcd/) and start the service:
...
Note:
* when using the pre-built binaries method downloaded from etcd-io, the configuration file is `/etc/etcd/etcd.conf`
