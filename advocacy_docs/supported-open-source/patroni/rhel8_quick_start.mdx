...
$ sudo firewall-cmd --quiet --zone=public --add-port=5444/tcp --permanent

# EDB Postgres Extended Server
$ sudo firewall-cmd --quiet --zone=public --add-port=5432/tcp --permanent
...
postgresql:
  ...
  parameters:
    unix_socket_directories: "/var/run/edb/as15,/tmp"
...
For **EDB Postgres Extended Server**, change this part in the previous commands:

```yaml
postgresql:
  listen: "0.0.0.0:5432"
  connect_address: "$MY_IP:5432"
  data_dir: /var/lib/edb-pge/15/data
  bin_dir: /usr/edb/pge15/bin
  pgpass: /tmp/pgpass0
  authentication:
    replication:
      username: replicator
    ...
    rewind:
      ...
      password: rewind_password
  parameters:
    unix_socket_directories: '/var/run/edb-pge,/tmp'
```
