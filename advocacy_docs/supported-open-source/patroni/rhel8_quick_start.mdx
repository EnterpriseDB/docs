...
This section walks you through the various steps to create a highly available **demo** cluster using **EDB Postgres Advanced Server** or **EDB Postgres Extended Server** version 15 on RHEL 8. In the steps below the etcd hosts will be called **etcd1**, **etcd2** and **etcd3**, and the two database nodes in _Streaming Replication_ will be **pg-patroni1** and **pg-patroni2**.
...
Patroni will bootstrap (create) the initial Postgres cluster and will be in charge of starting the service, so make sure `systemctl` is disabled for the EPAS/PGE service:
...
Don't forget to configure the firewall on each database node so they accept connections to the Postgres service
