...
This section walks you through the various steps to create a highly available **demo** cluster using **EDB Postgres Advanced Server** or **EDB Postgres Extended Server** version 15 on RHEL 8. In the steps below the etcd hosts will be called **etcd1**, **etcd2** and **etcd3**, and the two database nodes in streaming replication will be **pg-patroni1** and **pg-patroni2**.
...
On both **pg-patroni1** and **pg-patroni2** hosts, install the Postgres flavour you'd like. More information about installing [EDB Postgres Advanced Server](/epas/latest/installing/linux_x86_64/) or [EDB Postgres Extended Server](/pge/latest/installing/linux_x86_64/) using the EDB repository can be found in EDB docs.

To follow this guide, depending on the Postgres flavour you chose, let us define some environment variables.
...
Patroni will bootstrap (create) the initial Postgres cluster and will be in charge of starting the service, so make sure `systemctl` is disabled for this service:
...
!!! Note
    In the `postgresql: authentication:` section above, a **superuser** will be created during initialization (initdb) and later used by Patroni to connect to the database. The **replication** user will be created and used but the standby servers to access the replication source via streaming replication. Finally, a **rewind** user will be created and will be used with `pg_rewind` when needed. We then recommend to change those password in the above configuration to match your password policy.
...
Depending on the Patroni installation source, a [systemd file](https://github.com/zalando/patroni/blob/master/extras/startup-scripts/patroni.service) might have already been created. Define our own file with the accurate system user, group and configuration path:
