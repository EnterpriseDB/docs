...
For sake of simpliticy we will put some notes, when relevant, considering 3 possibilities for an existing cluster, just so we do not need to repeat ourselves regarding similar steps:
...
1. You already have an working `etcd` cluster composed of the following nodes:
    1. `etcd-node-1`.
    2. `etcd-node-2`.
    3. `etcd-node-3`.
...
5. Postgres is running on port `5432`.
6. Postgres data directory is `/var/lib/pgsql/15/data`.
7. Postgres binaries directory is `/usr/pgsql-15/bin`.
8. You already have installed Patroni in the Postgres nodes.
9. The systemd unit names are:
...
10. The configuration files are:
    1. `/etc/patroni.yml` &mdash; for Patroni.
    2. `/etc/repmgr.conf` &mdash; in case repmgr is being used in the existing cluster.
11. The cluster name is `test_cluster`. This is relevant only for EFM and Patroni.

**Note:** please adapt the example names according to your actual cluster.

In the following sub-sections you will find the actual walkthrough.
...
2. Create an YAML configuration file for Patroni (`/etc/patroni.yml`). You can find below a very basic template for the configuration file:
...
3. Schedule a restart of primary `postgres-node-1` to occurr during a maintenance window. On any Postgres node:
...
**Note:** You can use a query like the one below to check the required LSN of these slots. You need to make sure that `restart_lsn` of the slots created by Patroni are `>=` the `restart_lsn` of the original slots. On node `postgres-node-1`, through a `psql` section:
...
Once the above conditions are met we are then able to drop the original slots (`slot_for_postgres_node_2`, and `slot_for_postgres_node_3`). In order to do that, assuming you have not configured any other permament slots other than `slot_for_postgres_node_2` and `slot_for_postgres_node_3`, follow these steps:
...
At this point your cluster is already being completely managed by Patroni, so it would be interesting to test a switchover and switchback operation during a maintenance window to check how Patroni responds to that.
...
If you were able to get here and see the expected outputs, both switchover and switchback operations were successful, and it is very likely that your Patroni cluster is now fully operationable.
