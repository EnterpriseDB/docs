...
**Note:** the above procedure was designed for minimal downtime. In any case we **strongly** recommend that you conduct these steps during a scheduled maintenance window. Also, we **strongly** recommend that you test at least a switchover/switchback operation after all the above steps have been performed.

### More detailed example

In this section we will cover an example with more detailed steps to migrate an existing Postgres cluster to a Patroni managed cluster.

For sake of simplicity we will put some notes, when relevant, considering 3 possibilities for an existing cluster, just so we do not need to repeat ourselves regarding similar steps:
...
1. You already have an working `etcd` cluster &mdash; refer to [Quick start on RHEL 8 - 1. Etcd](rhel8_quick_start#1-etcd)/[Quick start on Debian 11 - 1. Etcd](debian11_quick_start#1-etcd) for instructions on how to set up an `etcd` cluster. The `etcd` cluster here is composed of the following nodes:
    1. `etcd-node-1`.
    2. `etcd-node-2`.
    3. `etcd-node-3`.
2. You have a Postgres cluster that needs to be migrated, which is composed of the following nodes:
    1. `postgres-node-1` &mdash; the primary node.
    2. `postgres-node-2` &mdash; a standby of `postgres-node-1`.
    3. `postgres-node-3` &mdash; a standby of `postgres-node-1`.
3. The existing Postgres cluster uses replication slots to control replication from primary to standbys, and they are named:
    1. `slot_for_postgres_node_2` &mdash; slot used by node `postgres-node-2`.
    2. `slot_for_postgres_node_3` &mdash; slot used by node `postgres-node-3`.
4. You already have a few Postgres users in the cluster:
    1. `postgres` with password `postgres_password` &mdash; Postgres superuser.
    2. `replicator` with password `replicator_password` &mdash; used by replication connections.
    3. `rewind` with password `rewind_password` &mdash; user with rights for executing `pg_rewind`.
5. You are running Postgres 15 with the following characteristics:
    1. Postgres is running on port `5432`.
    2. Postgres data directory is `/var/lib/pgsql/15/data`.
    3. Postgres binaries directory is `/usr/pgsql-15/bin`.
6. You already have installed Patroni in the Postgres nodes.
7. The systemd unit names are:
    1. `postgres` &mdash; for Postgres.
    2. `patroni` &mdash; for Patroni.
    3. `edb-efm` &mdash; in case EFM is being used in the existing cluster.
    4. `repmgrd` &mdash; in case repmgr is being used in the existing cluster.
8. The configuration files are:
    1. `/etc/patroni.yml` &mdash; for Patroni.
    2. `/etc/repmgr.conf` &mdash; in case repmgr is being used in the existing cluster.
9. The cluster name is `test_cluster`. This is relevant only for EFM and Patroni.

**Notes:**

* Please adapt the example names according to your actual cluster.
* Setting up Watchdog and HAProxy are out of the scope of this example. However, as they are part of the recommended architecture, we recommend you read the following documentation:
    * About Watchdog:
        * [Quick start on RHEL 8 - 3. Watchdog](rhel8_quick_start#3-watchdog)/[Quick start on Debian 11 - 3. Watchdog](debian11_quick_start#3-watchdog) &mdash; set up an Watchdog device.
        * [Quick start on RHEL 8 - 4. Patroni](rhel8_quick_start#4-patroni)/[Quick start on Debian 11 - 4. Patroni](debian11_quick_start#4-patroni) &mdash; configure Patroni to use Watchdog.
    * About HAProxy:
        * [Quick start on RHEL 8 - 5. HAProxy](rhel8_quick_start#5-haproxy)/[Quick start on Debian 11 - 5. HAProxy](debian11_quick_start#5-haproxy) &mdash; set up HAProxy.
* This example uses Postgres 15. If you are running EDB Postgres Extended or EDB Postgres Advanced Server, then we recommend you also read the following documentation:
    * [Quick start on RHEL 8 - 2. EDB Postgres Advanced or Extended Server](rhel8_quick_start#2-edb-postgres-advanced-or-extended-server)/[Quick start on Debian 11 - 2. EDB Postgres Advanced or Extended Server](debian11_quick_start#2-edb-postgres-advanced-or-extended-server) &mdash; check differences in ports, binary paths, etc.
    * [Quick start on RHEL 8 - 4. Patroni](rhel8_quick_start#4-patroni)/[Quick start on Debian 11 - 4. Patroni](debian11_quick_start#4-patroni) &mdash; how to configure Patroni to use custom Postgres binary names.

In the following subsections you will find the actual walkthrough.

#### 1. Disable current HA solution

If you have any HA solution currently managing your current cluster, then you need to disable it. If you do not have any HA solution, this section can be skipped.
...
1. Disable Postgres systemd unit. This is required so nothing other than Patroni will try to start Postgres up. Run:
...
Please keep the following notes in mind about the above template configuration file:

* It is just an example template, and you will need to change it according to your setup.
...
* Some key points about the proposed settings:
    * `scope`: it is the name of the Patroni cluster.
    * `name`: it is the name of the managed Postgres node.
    * `etcd3.hosts`: it contains the list of addresses to reach nodes that are part of the `etcd` cluster
    * `bootstrap.dcs.postgresql.slots`: it contains a list of slots that should be permanent, thus not dropped by Patroni when it starts running. We fill this so we can guarantee WALs will be available for replicas until they reach the LSN of the slots that are created by Patroni.
    * `bootstrap.pg_hba`: it contains a few basic HBA rules that Patroni will write to the `pg_hba.conf` file, so users are able to connect. Adapt it to your needs, as the rules are somehow very permissive.
...
3. Create or adjust the Patroni systemd unit. You can find instructions about that in [Quick start on RHEL 8 - 4. Patroni](rhel8_quick_start#4-patroni)/[Quick start on Debian 11 - 4. Patroni](debian11_quick_start#4-patroni).

4. Enable the Patroni systemd unit. This is done so Patroni will automatically start up on boot and bring Postgres up. Run:
...
1. Immediately restart of standby `postgres-node-2`. On any Postgres node:

```bash
patronictl -c /etc/patroni.yml restart test_cluster postgres-node-2 --scheduled now --force
```

2. Immediately restart of standby `postgres-node-3`. On any Postgres node:
...
Once all Postgres instances were restarted, they will be already running through Patroni instead of through the Postgres systemd unit.
...
**Note:** You can use a query like the one below to check the required LSN of these slots. You need to make sure that `restart_lsn` of the slots created by Patroni are `>=` the `restart_lsn` of the original slots. On node `postgres-node-1`, through a `psql` session:
...
Once the above conditions are met we are then able to drop the original slots (`slot_for_postgres_node_2`, and `slot_for_postgres_node_3`). In order to do that, assuming you have not configured any other permanent slots other than `slot_for_postgres_node_2` and `slot_for_postgres_node_3`, follow these steps:
...
2. Wait for about 10 seconds and check that replication slots `slot_for_postgres_node_2` and `slot_for_postgres_node_3` had been dropped from `postgres-node-1` -- query `pg_replication_slots` to check that.

#### 5. Test switchover and switchback

At this point your cluster is completely managed by Patroni, so it is important to perform a switchover and switchback operation during a maintenance window to ensure Patroni is operating correctly.
