...
* You already have an working `etcd` cluster &mdash; refer to [Installing and configuring etcd](installing_etcd) to install and set up `etcd`. The `etcd` cluster here is composed of the following nodes:
    * `etcd-node-1`.
    * `etcd-node-2`.
    * `etcd-node-3`.
* You have a Postgres cluster that needs to be migrated, which is composed of the following nodes:
...
* The existing Postgres cluster uses replication slots to control replication from primary to standbys, and they are named:
    * `slot_for_postgres_node_2` &mdash; slot used by node `postgres-node-2`.
    * `slot_for_postgres_node_3` &mdash; slot used by node `postgres-node-3`.
* You already have a few Postgres users in the cluster:
...
* You already have installed Patroni on the Postgres nodes.
