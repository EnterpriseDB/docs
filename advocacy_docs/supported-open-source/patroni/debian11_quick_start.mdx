...
Patroni will bootstrap (create) the initial Postgres cluster and will be in charge of starting the service, so make sure `systemctl` is disabled for the EPAS/PGE service and remove the Postgres cluster created during the installation:
...
With **EDB Postgres Advanced Server**, now that the database cluster is running, some other systemd dependencies might need a reset. To do so, use the following commands:
