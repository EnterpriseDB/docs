...
This walks you through the various steps to create an highly available **demo** cluster (using **EDB Postgres Advanced Server** or **EDB Postgres Extended Server** version 15 on Debian 11) where the etcd hosts will be called **etcd1**, **etcd2** and **etcd3**, and the two database nodes in _Streaming Replication_ are **pg-patroni1** and **pg-patroni2**.
...
Patroni will bootstrap (create) the initial Postgres cluster and be in charge of starting the service, so be sure `systemctl` is disabled and remove the Postgres cluster created during the installation:
