...
Patroni expects to find the `postgres` binary in the `bin_dir` location, while the EDB Postgres Advanced Server binary is called `edb-postgres`. As `postgresql.bin_name` setting was introduced in [version `3.0.3`](https://github.com/zalando/patroni/blob/master/docs/releases.rst#version-303), if you are running Patroni older than that, then remove the `bin_name` setting from the configuration and create a symbolic link to let Patroni find that binary:
