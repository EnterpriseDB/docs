...
EDB currently only supports using Patroni and Postgres _physical Streaming Replication_ with a primary server and two or more replicas (replicating directly from the primary).
...
!!! Note
    For production environments, it is recommended to setup a cluster of three nodes. One etcd cluster can serve multiple Postgres/Patroni clusters. Etcd nodes should be installed independently from the Postgres nodes (not on the same hosts).
