...
EDB currently only supports using Patroni and PostgreSQL _Streaming Replication_ (physical) with a primary server and two or more replicas.

#### Patroni

Patroni is installed on the PostgreSQL nodes. EDB supports the [latest release](https://patroni.readthedocs.io/en/latest/releases.html#releases).
...
!!! Note
  For production environments, it is recommended to setup at least two HAProxy servers. Using various ports, one HAProxy server can serve multiple PostgreSQL/Patroni clusters.
