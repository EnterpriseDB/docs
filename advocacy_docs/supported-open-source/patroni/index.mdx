...
Patroni is a template to create your own high-availability solution, relying on Postgres's streaming replication and hot standby capabilities, using Python and - for maximum accessibility - a distributed configuration store like etcd or Kubernetes.
...
EDB currently only supports using Patroni and Postgres _physical Streaming Replication_ with a primary server and two replicas (replicating directly from the primary). EDB supports using either PostgreSQL, EDB Postgres Advanced Server or EDB Postgres Extended Server.
