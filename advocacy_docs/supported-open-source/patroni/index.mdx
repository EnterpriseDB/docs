...
Patroni is installed on the Postgres nodes. EDB supports the [latest release](https://patroni.readthedocs.io/en/latest/releases.html#releases) of Patroni.
...
See the [Etcd installation instructions](installing_etcd) to install and set up etcd.
...
Instead of connecting directly to the database server, it is possible to setup HAProxy so the application connects to the proxy instead, which will then forward the request to Postgres. When using HAProxy, it is also possible to route read-only requests to one or more replicas, for load balancing.

EDB strongly suggests using HAProxy to avoid data loss in case of a split-brain.
