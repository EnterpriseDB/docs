---
title: 'Installing Patroni for PostgreSQL'
description: "A walkthrough: setup a demo cluster using Patroni and etcd"
---


This walks you through the various steps to create an highly available demo cluster (using PostgreSQL version 14 on RHEL 8) where the etcd hosts will be called etcd1, etcd2 and etcd3, and the two PostgreSQL nodes in _Streaming Replication_ are pg-patroni1 and pg-patroni2.
...
See [Installing Etcd](installing_etcd) to install and set up etcd.
...
Patroni requires Extra Packages for Enterprise Linux ([EPEL](https://fedoraproject.org/wiki/EPEL)). Configure the [PostgreSQL](https://yum.postgresql.org/) yum repository, and then run: 
