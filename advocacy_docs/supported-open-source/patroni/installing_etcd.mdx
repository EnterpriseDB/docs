...
The PGDG yum [extras](https://yum.postgresql.org/news/new-repo-extra-packages/) repositories should contain etcd packages.
...
Depending on your system, the following repositories have to be enabled:
* RHEL 9: pgdg-rhel9-extras
* RHEL 8: pgdg-rhel8-extras
* RHEL 7: pgdg-rhel7-extras

As an example, the following command installs etcd on RHEL 8:
...
The following commands output the version of the installed etcd binary and get the latest version from the etcd-io website.
...
The easiest way to install the etcd latest release is from pre-built binaries.
...
Note:
* when using the prebuilt binaries method or the RedHat-compatible packages, the configuration file is `/etc/etcd/etcd.conf`
