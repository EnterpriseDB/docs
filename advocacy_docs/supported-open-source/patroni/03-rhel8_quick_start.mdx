...
This section walks you through the various steps to create an highly available **demo** cluster (using **PostgreSQL** version 14 on Rocky Linux 8) where the etcd hosts will be called **etcd1**, **etcd2** and **etcd3**, and the two PostgreSQL nodes in _Streaming Replication_ are **pg-patroni1** and **pg-patroni2**.
