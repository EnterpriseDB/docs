...
WarehousePG Observability is structured into three distinct functional blocks: an internal data collection layer (Extension and Collector) running within the WHPG cluster, the Exporter service that queries the database and transforms the results into a usable metric format, and a storage and visualization block which is supported by third party monitoring services (Prometheus, Loki, and Grafana).

![architechure](images/architecture.png)

### WHPG Collector

The WarehousePG Collector is a service based on Grafana Alloy that runs on the WHPG coordinator, standby, and segments.
...
- **Prometheus**
Endpoint for host metrics and historic SQL metrics. Prometheus receives host level metrics from the Collector, and SQL and cluster metrics from the Exporter.
