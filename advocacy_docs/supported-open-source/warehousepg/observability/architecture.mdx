...
WarehousePG (WHPG) Observability monitors system performance metrics, analyzes cluster health, and enables database administrators to …. on a WarehousePG environment.
...
It collects host metrics and log files from all hosts and pushes them to the Collector service on the coordinator. The coordinator temporarily stores them in memory, then pushes the host metrics to Prometheus, and the log files to Loki.
