...
description: Diagnose and resolve common issues encountered during the installation and configuration of the WarehousePG Observability stack.
---
## Identifying your issue
WarehousePG (WHPG) Observability is composed of several components. Understanding what each component does and where it runs is crucial for narrowing down the location of any issue. For a complete understanding of how each component functions within WHPG Observability, refer to the [Architecture](../overview/architecture) section.

| Issue         | Component to check| Reason 
| ------------- | ------------------ | ------
| SQL metrics   | - [Exporter's connection to WHPG cluster](#troubleshooting-whpg-exporter) <br /> - [Extension installation and database permissions](#troubleshooting-whpg-collector) | Exporter queries the `observability` schema in every database to retrieve SQL metrics. <br /> The Extension needs to be installed in every database that you require metrics for. 
| Host metrics  | [Collector service and its connection to Prometheus](#troubleshooting-whpg-collector) | Collector services gather host metrics from all nodes in the WHPG cluster, and the coordinator pushes them to Prometheus.
| Log data      | [Collector service and its connection to Loki](#troubleshooting-whpg-collector)       | Collector services gather log files from all nodes in the WHPG cluster, and the coordinator pushes them to Loki.
| Live queries in Grafana | [Grafana's connection to the Exporter endpoint](#troubleshooting-grafana) | Grafana directly queries the Exporter service for real-time database metrics.
| Historic data in Grafana | - [Exporter's connection to Prometheus (for SQL data)](#troubleshooting-whpg-exporter) <br /> - [Collector's connection to Prometheus (for host data/logs)](#troubleshooting-whpg-collector) <br /> - [Grafana's connection to Prometheus](#troubleshooting-grafana) | Grafana displays historic data that is stored in Prometheus.<br />  Collector and Exporter perform remote writes to the Prometheus instance. 
...
| Grafana configuration files | `provisioning/datasources/datasource.yaml`<br /> `WHPGClusterDashboard.json` <br /> `WHPGClusterDetails.json` <br /> `WHPGLogDetails.json` <br /> `WHPGQueryDetails.json` <br /> `WHPGRecommendations.json` <br /> `WHPGResourceDetails.json` <br /> `WHPGSegmentDetails.json`
