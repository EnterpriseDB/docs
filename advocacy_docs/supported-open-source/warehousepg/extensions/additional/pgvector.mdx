...
You can check index creation progress in WarehousePG as described in [CREATE INDEX Progress Reporting](/warehousepg/7.5/admin_guide/managing/progress_reporting/#create_index_progress):
...
Use [partitioning](/warehousepg/7.5/admin_guide/ddl/ddl-partition/) to perform an approximate search on many different values of the `WHERE` columns:
...
Use pgvector together with [full-text search](/warehousepg/7.5/admin_guide/query/textsearch/) for a hybrid search:
