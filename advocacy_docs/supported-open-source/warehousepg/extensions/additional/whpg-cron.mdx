...
-   **Unix domain socket.** Limit access to local OS users, since this option doesn't expose the database over the network. Point `cron.host` at the socket directory and restart the database:
    ...
    Then add a matching `local` entry to [`pg_hba.conf`](/warehousepg/7.5/admin_guide/manage_access/client_auth/) on the coordinator to grant trust authentication on that socket:

    ```
    local   all   all   trust
    ```

-   **TCP connection.** Restrict the `pg_hba.conf` entry to the loopback address, since `pg_cron` always connects from the coordinator itself. Leave `cron.host` at its default (`localhost`), and add a `host` entry to [`pg_hba.conf`](/warehousepg/7.5/admin_guide/manage_access/client_auth/) on the coordinator:
