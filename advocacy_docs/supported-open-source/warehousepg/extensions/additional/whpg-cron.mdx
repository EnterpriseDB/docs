...
displayBanner: ''
redirects:
  - /warehousepg/7.3/ref_guide/modules/extensions/whpg-cron
  - /warehousepg/7.4/ref_guide/modules/extensions/whpg-cron
  - /warehousepg/7.5/ref_guide/modules/extensions/whpg-cron
  - /warehousepg/7.6/ref_guide/modules/extensions/whpg-cron
  - /warehousepg/latest/ref_guide/modules/extensions/whpg-cron
description: Learn how to schedule periodic SQL jobs in WarehousePG using the whpg-cron extension.
---

!!!note Version availability

whpg-cron is available starting with WarehousePG 7.3.0.
!!!
...
The WarehousePG `whpg-cron` package installs the extension under its upstream name. You create it, and refer to it in SQL, as `pg_cron`. See [`pg_cron`](https://github.com/citusdata/pg_cron) for the upstream project.

## Downloading, installing, and loading the extension

Refer to [Downloading and installing an extension](/supported-open-source/warehousepg/extensions/additional/#downloading-and-installing-an-extension) for installation and setup instructions. The package name is `edb-whpg7-whpg-cron`.
...
-   **Unix domain socket.** Limit access to local OS users, since this option doesn't expose the database over the network. Point `cron.host` at the socket directory and restart the database:
    ...
    Then add a matching `local` entry to [`pg_hba.conf`](/warehousepg/latest/admin_guide/manage_access/client_auth/) on the coordinator to grant trust authentication on that socket:

    ```
    local   all   all   trust
    ```

-   **TCP connection.** Restrict the `pg_hba.conf` entry to the loopback address, since `pg_cron` always connects from the coordinator itself. Leave `cron.host` at its default (`localhost`), and add a `host` entry to [`pg_hba.conf`](/warehousepg/latest/admin_guide/manage_access/client_auth/) on the coordinator:
...
`pg_cron` stores every scheduled job as a row in `cron.job`, with columns for its `jobid`, `schedule`, `command`, and `username`, among others. `pg_cron` runs each job in the database where you created it, with the same permissions as the user who scheduled it. A row-level security ([RLS](https://www.postgresql.org/docs/current/ddl-rowsecurity.html)) policy limits you to viewing and modifying only the jobs you created, unless you're a superuser or have the `bypassrls` attribute, which lets a role bypass row-level security entirely.

!!!important

We recommend managing jobs with the `cron` schema functions rather than direct `UPDATE`, `INSERT`, or `DELETE` statements against `cron.job`, since WarehousePG doesn't support the `TRIGGER` statement `pg_cron` normally relies on to detect such changes. If the functions can't do what you need, see [Editing cron.job directly](#editing-cronjob-directly).
!!!
...
<a id="editing-cronjob-directly"></a>
...
-   You can only install `pg_cron` in one database per WarehousePG cluster.
-   `pg_cron` and [whpg-anonymizer](whpg-anonymizer.mdx) can't run in the same database. Creating the `pg_cron` extension in a database where `whpg-anonymizer` is enabled fails, because `whpg-anonymizer`'s event trigger doesn't allow another extension to replace the masked views it owns. Schedule jobs in a separate database from the one where you use `whpg-anonymizer`.
