...
title: pgcrypto
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/extensions/bundled/pgcrypto.md
redirects:
  - /warehousepg/6/ref_guide/modules/bundled/pgcrypto
  - /warehousepg/7.5/ref_guide/modules/bundled/pgcrypto
  - /warehousepg/7.4/ref_guide/modules/bundled/pgcrypto
  - /warehousepg/7.3/ref_guide/modules/bundled/pgcrypto
  - /warehousepg/7.5/ref_guide/modules/pgcrypto
  - /warehousepg/7.4/ref_guide/modules/pgcrypto
  - /warehousepg/7.3/ref_guide/modules/pgcrypto
...
The `pgcrypto` module is installed when you install WarehousePG. Before you can use any of the functions defined in the module, you must register the `pgcrypto` extension in each database in which you want to use the functions. Refer to [Installing Extensions](/warehousepg/7.5/docs/7x/install_guide/additional_modules/) for more information.
...
1.  Enable the `pgcrypto` functions as an extension if it is not enabled. See [Installing Extensions](/warehousepg/7.5/docs/7x/install_guide/additional_modules/). This example `psql` command creates the `pgcrypto` extension in the database `testdb`.

    ```
    psql -d testdb -c 'CREATE EXTENSION pgcrypto'
    ```

2.  Configure the WarehousePG server configuration parameter `shared_preload_libraries` to load the `pgcrypto` library. This example uses the `gpconfig` utility to update the parameter in the WarehousePG `postgresql.conf` files.
...
3.  Restart the WarehousePG cluster.
...
2.  Remove `\$libdir/pgcrypto` from the `shared_preload_libraries` parameter, and restart WarehousePG. This `gpconfig` command displays the value of the parameter from the WarehousePG `postgresql.conf` files.
