...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/extensions/bundled/gp_parallel_retrieve_cursor.md
redirects:
  - /warehousepg/6/ref_guide/modules/bundled/gp_parallel_retrieve_cursor
description: >-
  Provides extended cursor functionality to retrieve data, in parallel, directly
  from segments.
...
Refer to [Installing Extensions](/warehousepg/6/docs/6x/install_guide/additional_modules/) for more information.
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| gp_get_endpoints()<br /><br />[gp_endpoints](/warehousepg/6/docs/6x/ref_guide/system_catalogs/system_catalogs_definitions/gp_endpoints.mdx)                         | List the endpoints associated with all active parallel retrieve cursors declared by the current user in the current database. When the WarehousePG superuser invokes this function, it returns a list of all endpoints for all parallel retrieve cursors declared by all users in the current database. |
| gp_get_session_endpoints()<br /><br />[gp_session_endpoints](/warehousepg/6/docs/6x/ref_guide/system_catalogs/system_catalogs_definitions/gp_session_endpoints.mdx) | List the endpoints associated with all parallel retrieve cursors declared in the current session for the current user.                                                                                                                                                                                  |
| gp_get_segment_endpoints()<br /><br />[gp_segment_endpoints](/warehousepg/6/docs/6x/ref_guide/system_catalogs/system_catalogs_definitions/gp_segment_endpoints.mdx) | List the endpoints created in the QE for all active parallel retrieve cursors declared by the current user. When the WarehousePG superuser accesses this view, it returns a list of all endpoints on the QE created for all parallel retrieve cursors declared by all users.                            |
...
You [DECLARE](/warehousepg/6/docs/6x/ref_guide/sql_commands/DECLARE.mdx) a cursor to retrieve a smaller number of rows at a time from a larger query. When you declare a parallel retrieve cursor, you can retrieve the query results directly from the WarehousePG segments.
...
Refer to the [gp_endpoints](/warehousepg/6/docs/6x/ref_guide/system_catalogs/system_catalogs_definitions/gp_endpoints.mdx) view reference page for more information about the endpoint attributes returned by these commands.
...
-   You must specify the [gp_retrieve_conn](/warehousepg/6/docs/6x/ref_guide/config_params/guc-list.mdx#gp_retrieve_conn) server configuration parameter on the connection request, and set the value to `true` .
...
Once you establish a retrieve session, you retrieve the tuples associated with a query result on that endpoint using the [RETRIEVE](/warehousepg/6/docs/6x/ref_guide/sql_commands/RETRIEVE.mdx) command.
...
Refer to the [gp_segment_endpoints](/warehousepg/6/docs/6x/ref_guide/system_catalogs/system_catalogs_definitions/gp_segment_endpoints.mdx) view reference page for more information about the endpoint attributes returned by these commands.
...
By default, WarehousePG does not limit the number of parallel retrieve cursors that are active in the cluster (up to the maximum value of 1024). The WarehousePG superuser can set the [gp_max_parallel_cursors](/warehousepg/6/docs/6x/ref_guide/config_params/guc-list.mdx#gp_max_parallel_cursors) server configuration parameter to limit the number of open cursors.
