...
title: timestamp9
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/extensions/bundled/timestamp9.md
redirects:
  - /warehousepg/6/ref_guide/modules/bundled/timestamp9
description: >-
  Provides an efficient, nanosecond-precision timestamp data type and related
  functions.
...
If no time zone is specified in the input string, then it is presumed to be in the time zone indicated by the system's [`TIMEZONE` server configuration parameter](/warehousepg/6/docs/6x/ref_guide/config_params/guc-list.mdx#timezone) and is converted to UTC using the offset for the time zone.
...
You can set the [TimeZone](/warehousepg/6/docs/6x/ref_guide/config_params/guc-list.mdx#timezone) server configuration parameter to specify the time zone that WarehousePG uses when it prints a `timestamp9` timestamp. When you set this parameter, WarehousePG displays the timestamp value in that time zone. For example:
