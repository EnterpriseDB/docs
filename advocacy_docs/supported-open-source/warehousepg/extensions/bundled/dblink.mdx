...
title: dblink
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/extensions/bundled/dblink.md
redirects:
  - /warehousepg/6/ref_guide/modules/bundled/dblink
  - /warehousepg/7.5/ref_guide/modules/bundled/dblink
  - /warehousepg/7.4/ref_guide/modules/bundled/dblink
  - /warehousepg/7.3/ref_guide/modules/bundled/dblink
  - /warehousepg/7.5/ref_guide/modules/dblink
  - /warehousepg/7.4/ref_guide/modules/dblink
  - /warehousepg/7.3/ref_guide/modules/dblink
description: Provides connections to other WarehousePG databases.
...
WarehousePG supports `dblink` connections between databases in WarehousePG installations with the same major version number. You can also use `dblink` to connect to other WarehousePG installations that use compatible `libpq` libraries.

> **Note** `dblink` is intended for database users to perform short ad hoc queries in other databases. `dblink` is not intended for use as a replacement for external tables.
...
The `dblink` module is installed when you install WarehousePG. Before you can use any of the functions defined in the module, you must register the `dblink` extension in each database in which you want to use the functions. Refer to [Installing Extensions](/warehousepg/7.5/docs/7x/install_guide/additional_modules/) for more information.
...
The WarehousePG version of `dblink` deactivates the following asynchronous functions:
...
3.  Use the `dblink_connect()` function to create either an implicit or a named connection to another database. The connection string that you provide should be a `libpq`-style keyword/value string. This example creates a connection named `mylocalconn` to the `postgres` database on the local WarehousePG cluster:
    ...
    > **Note** If a `user` is not specified, `dblink_connect()` uses the value of the `PGUSER` environment variable when WarehousePG was started. If `PGUSER` is not set, the default is the system user that started WarehousePG.
...
2.  Now `test_user` can create a connection to another local database without a password. For example, `test_user` can log into the `testdb` database and run this command to create a connection named `testconn` to the local `postgres` database.
    ...
    > **Note** If a `user` is not specified, `dblink_connect_u()` uses the value of the `PGUSER` environment variable when WarehousePG was started. If `PGUSER` is not set, the default is the system user that started WarehousePG.
...
> **Note** This section applies to WarehousePG 6.x.

In rare cases you may need to allow non-superusers access to `dblink` without making any authentication checks. The function `dblink_connect_no_auth()` provides this functionality, since it bypasses the `pg_hba.conf` file.

> **Caution** Using this function introduces a security risk. Grant unauthorized access only to trusted user accounts. Also note that `dblink_connect_no_auth()` functions limit connections to the local cluster, and don't permit connections to a remote database.

These functions aren't available by default. The `gpadmin` superuser must grant permission to the non-superuser beforehand:
...
2.  Now `test_user` can create a connection to another local database without providing a password, regardless of what's specified in `pg_hba.conf`. For example, `test_user` can log into the `testdb` database and run this command to create a connection named `testconn` to the local `postgres` database.
...
Refer to [SSL Client Authentication](/warehousepg/7.5/docs/7x/security_guide/authentication.mdx#ssl_postgresql) for information about configuring WarehousePG to use SSL.
