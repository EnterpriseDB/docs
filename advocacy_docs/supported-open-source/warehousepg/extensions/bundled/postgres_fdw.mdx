...
title: postgres_fdw
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/extensions/bundled/postgres_fdw.md
redirects:
  - /warehousepg/6/ref_guide/modules/bundled/postgres_fdw
  - /warehousepg/7.6/ref_guide/modules/bundled/postgres_fdw
  - /warehousepg/7.5/ref_guide/modules/bundled/postgres_fdw
  - /warehousepg/7.4/ref_guide/modules/bundled/postgres_fdw
  - /warehousepg/7.3/ref_guide/modules/bundled/postgres_fdw
  - /warehousepg/7.6/ref_guide/modules/postgres_fdw
  - /warehousepg/7.5/ref_guide/modules/postgres_fdw
  - /warehousepg/7.4/ref_guide/modules/postgres_fdw
  - /warehousepg/7.3/ref_guide/modules/postgres_fdw
description: >-
  Provides a foreign-data wrapper for accessing data stored in a remote
  PostgreSQL or WarehousePG database.
...
The WarehousePG `postgres_fdw` module is a modified version of the PostgreSQL `postgres_fdw` module. The module behaves as described in the PostgreSQL [postgres_fdw](https://www.postgresql.org/docs/12/postgres-fdw.html) documentation when you use it to access a single remote PostgreSQL database. Starting in WarehousePG 7.x, the WarehousePG `postgres_fdw` module also includes enhancements that allow you to access multiple remote PostgreSQL servers from a single foreign table.
...
The `postgres_fdw` module is installed when you install WarehousePG. Before you can use the foreign-data wrapper, you must register the `postgres_fdw` extension in each database in which you want to use the foreign-data wrapper. Refer to [Installing Extensions](/warehousepg/latest/install_guide/additional_modules/) for more information.

<a id="using_mrs"></a>

## Using postgres_fdw to Access Multiple Remote PostgreSQL Servers

> **Note** This feature is available starting in WarehousePG 7.x. It isn't available in WarehousePG 6.x.

Before you [create a foreign table](/warehousepg/latest/ref_guide/sql_commands/CREATE_FOREIGN_TABLE.mdx) using the `postgres_fdw` foreign-data wrapper, you must configure a server with the [CREATE SERVER](/warehousepg/latest/ref_guide/sql_commands/CREATE_SERVER.mdx) command. You can use a `postgres_fdw` foreign table to access data that is distributed across multiple remote PostgreSQL servers when you set certain WarehousePG and `postgres_fdw`-specific options on the `CREATE SERVER` command:
...
| num_segments<sup>1</sup> | WarehousePG option that identifies the number of query executors that WarehousePG spawns on the source WarehousePG cluster. | Set to the number of remote PostgreSQL servers. If this option is not set, defaults to the number of segments in the local WarehousePG cluster. |
...
For more information about using foreign tables in WarehousePG, see [Accessing External Data with Foreign Tables](/warehousepg/latest/admin_guide/external/foreign/).

For detailed information about this module, refer to the [postgres_fdw](https://www.postgresql.org/docs/12/postgres-fdw.html) PostgreSQL documentation.

The `postgres_fdw` foreign-data wrapper is included in the WarehousePG open source github repository. You can view the [source code](https://github.com/warehouse-pg/warehouse-pg/tree/main/contrib/postgres_fdw) for the module which is located in the `contrib/` directory.
