...
The `postgres_fdw` module is installed when you install WarehousePG. Before you can use the foreign-data wrapper, you must register the `postgres_fdw` extension in each database in which you want to use the foreign-data wrapper. Refer to [Installing Extensions](/warehousepg/7.5/install_guide/additional_modules/) for more information.
...
Before you [create a foreign table](/warehousepg/7.5/ref_guide/sql_commands/CREATE_FOREIGN_TABLE.mdx) using the `postgres_fdw` foreign-data wrapper, you must configure a server with the [CREATE SERVER](/warehousepg/7.5/ref_guide/sql_commands/CREATE_SERVER.mdx) command. You can use a `postgres_fdw` foreign table to access data that is distributed across multiple remote PostgreSQL servers when you set certain WarehousePG and `postgres_fdw`-specific options on the `CREATE SERVER` command:
...
For more information about using foreign tables in WarehousePG, see [Accessing External Data with Foreign Tables](/warehousepg/7.5/admin_guide/external/foreign/).
