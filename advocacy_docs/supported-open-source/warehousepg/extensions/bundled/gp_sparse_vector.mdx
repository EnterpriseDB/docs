...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/extensions/bundled/gp_sparse_vector.md
redirects:
  - /warehousepg/latest/ref_guide/modules/bundled/gp_sparse_vector
  - /warehousepg/6/ref_guide/modules/bundled/gp_sparse_vector
  - /warehousepg/7.5/ref_guide/modules/bundled/gp_sparse_vector
  - /warehousepg/7.4/ref_guide/modules/bundled/gp_sparse_vector
  - /warehousepg/7.3/ref_guide/modules/bundled/gp_sparse_vector
  - /warehousepg/7.5/ref_guide/modules/gp_sparse_vector
  - /warehousepg/7.4/ref_guide/modules/gp_sparse_vector
  - /warehousepg/7.3/ref_guide/modules/gp_sparse_vector
description: >-
  Implements a data type that uses compressed storage of zeros to make vector
  computations on floating point numbers faster.
...
The `gp_sparse_vector` module is installed when you install WarehousePG. Before you can use any of the functions defined in the module, you must register the `gp_sparse_vector` extension in each database where you want to use the functions. Refer to [Installing Extensions](/warehousepg/latest/install_guide/additional_modules/) for more information.

<a id="topic_upgrade"></a>

## Upgrading the Module

> **Note** This section applies if you're upgrading WarehousePG 6.x from a version earlier than 6.16.

Starting in WarehousePG 6.16, `gp_sparse_vector` functions and objects are installed in the schema named `sparse_vector` instead of `public`. Upgrading the module requires updating any scripts that reference the module's objects, and adjusting how you reference these objects in a client session. Add the `sparse_vector` schema to a `search_path`, or prepend `sparse_vector.` to all non-`CAST` `gp_sparse_vector` function or object name references.

Update the `gp_sparse_vector` module in each database where you use it:
...
`gp_sparse_vector` functions and objects are installed in the schema named `sparse_vector`. To access `gp_sparse_vector` objects, add `sparse_vector` to a `search_path`, or prepend `sparse_vector.` to the function or object name. For example:

```
SELECT sparse_vector.array_agg( col1 ) FROM table1;
```

`CAST`s that the `gp_sparse_vector` module creates remain in the `public` schema.
...
For example, suppose the following array of `double`s is stored as a `float8[]` in WarehousePG:
...
Refer to the `gp_sparse_vector` READMEs in the [WarehousePG github repository](https://github.com/warehouse-pg/warehouse-pg/tree/master/gpcontrib/gp_sparse_vector/README) for additional information about this module.
