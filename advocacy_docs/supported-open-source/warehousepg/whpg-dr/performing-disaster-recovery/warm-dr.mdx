...
Before the delta schedule can run, the DR cluster needs a baseline state. Run all commands as the `gpadmin` user on the DR coordinator.
...
As the `gpadmin` user, add the following entry to the crontab (`crontab -e`) on the primary to create a restore point, for example every 15 minutes:
...
As the `gpadmin` user, add the following entry to the crontab (`crontab -e`) on the DR coordinator to apply the latest restore point, offset by a few minutes to allow archiving to complete:
...
After promotion, the cluster accepts connections. Extension data is restored as part of the cluster state.
