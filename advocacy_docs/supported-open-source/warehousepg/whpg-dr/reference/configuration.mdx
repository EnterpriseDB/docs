...
Used with `whpg-dr configure restore`. Describes the storage repository and the DR cluster's topology. See [Setting up the DR cluster](../configuring-dr) for setup instructions.
