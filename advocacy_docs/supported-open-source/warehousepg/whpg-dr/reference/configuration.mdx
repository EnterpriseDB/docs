...
| `barman_options.jobs` | No | Number of parallel workers per segment. Defaults to `1`. The `--barman-jobs` flag on [`whpg-dr backup`](commands#whpg-dr-backup) overrides this value per run. Raise it to increase parallelism, sizing it as approximately `host_cores / primaries_per_host`. It's most impactful for S3 storage, where upload concurrency is often the bottleneck. |
