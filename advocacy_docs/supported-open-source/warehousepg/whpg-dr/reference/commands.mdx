...
| Option | Description |
|--------|-------------|
| `--barman-jobs <n>` | Parallel workers per segment. Defaults to `1`. Overrides [`barman_options.jobs`](configuration#backup-configuration-yaml) from the configuration. Raise it to increase parallelism, sizing it as approximately `host_cores / primaries_per_host`. Most impactful for S3 storage, where upload concurrency is often the bottleneck. |
...
| `--barman-jobs <n>` | Parallel workers per segment. Defaults to `1`. Overrides [`barman_options.jobs`](configuration#restore-configuration-yaml) from the configuration. Raise it to increase parallelism. Most impactful for S3 storage, where download concurrency is often the bottleneck. |
