...
Takes a full physical backup of the coordinator and all primary segment data directories in parallel. See [Taking a base backup](../setting-up-primary/backups#performing-a-base-backup) for usage in context.
...
Configures the DR cluster for restore. Reads the restore YAML, maps source segments to DR hosts, generates Barman restore configuration files, and distributes them to all DR segment hosts. See [Setting up the DR cluster](../configuring-dr) for setup instructions.
