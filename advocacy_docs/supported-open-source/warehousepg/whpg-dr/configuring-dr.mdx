...
Prepare the DR cluster for recovery. Install WarehousePG and `whpg-dr`, create a restore configuration file, and run `whpg-dr configure restore` to prepare the cluster to receive restores from the storage repository. Run all commands as `gpadmin` on the DR coordinator. Once configured, proceed to [choose a recovery method](performing-disaster-recovery).
...
For next steps, see [Cold DR](performing-disaster-recovery/cold-dr), [Warm DR](performing-disaster-recovery/warm-dr), or [Point-in-time recovery](performing-disaster-recovery/pitr).