---
title: Setting up the DR cluster for WarehousePG Disaster Recovery
navTitle: Setting up the DR cluster
...
Whether to complete this step in advance depends on your DR mode. For warm DR, complete it before a failure occurs so the cluster can start replaying WAL immediately. For cold DR, you can do it at recovery time, though recording the cluster details in advance saves time when a failure occurs.
...
!!! Note
This page covers installing and configuring `whpg-dr` on the DR cluster, not performing a restore. To perform a restore after configuration, see [Cold DR](performing-disaster-recovery/cold-dr), [Warm DR](performing-disaster-recovery/warm-dr), or [Point-in-time recovery](performing-disaster-recovery/pitr).
!!!
...
Create a YAML file that describes the storage repository and the DR cluster topology, then run `whpg-dr configure restore` to prepare the cluster. The storage block must point to the [same repository the primary cluster writes to](setting-up-primary/configuring-primary#creating-the-backup-configuration-file). The RPM installs ready-to-adapt starting points under `/usr/edb/whpg-dr/examples/restore/`.

The cluster name and storage configuration must match your primary cluster. The restore YAML also includes fields that don't exist in the backup configuration: `coordinator_host`, `coordinator_data_directory`, `segment_hosts`, `data_directory`, and `data_directory_prefix`. These fields describe your DR environment and can be set to match your infrastructure. The DR cluster doesn't need to match the primary's host layout, hostname scheme, or directory structure. `data_directory` accepts either a single path or a list of mount-point directories to spread segments across multiple disks; use `data_directory_overrides` to set a different path for specific hosts.
...
To improve restore performance, set `jobs` to parallelize the base-backup copy within each segment, and `wal_prefetch` to fetch WAL files in parallel during replay. Changes to `barman_options` take effect on the next restore without requiring a new full restore. Re-run `whpg-dr configure restore` after updating the YAML.
...
### Overriding data directories per host

Use `data_directory_overrides` when one or more DR hosts have a different disk layout from the rest. Each value takes the same shape as `data_directory`, either a single path or a list of mount-point directories.

To point a specific host to a different base path:
...
data_directory_overrides:
  sdw3-dr: /scratch/primary
```

To spread a host's segments across multiple mount points:
...
The DR cluster is now ready for restore operations. See [Cold DR](performing-disaster-recovery/cold-dr), [Warm DR](performing-disaster-recovery/warm-dr), or [Point-in-time recovery](performing-disaster-recovery/pitr) for next steps.