...
To improve restore performance, use the Barman options `jobs` to parallelize the base-backup copy within each segment, and `wal_prefetch` to fetch WAL files in parallel during replay. Changes to `barman_options` take effect on the next restore without requiring a new full restore. Re-run `whpg-dr configure restore` after updating the YAML.
