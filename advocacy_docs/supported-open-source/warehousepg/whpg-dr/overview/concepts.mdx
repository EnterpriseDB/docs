...
- **Phase 2 — Set up the DR cluster.** Provision DR hosts with the same OS and WarehousePG patch version as the primary, install `whpg-dr`, and run `whpg-dr configure restore` to prepare the cluster to receive restores from the storage repository. See [Setting up the DR cluster](../configuring-dr).
