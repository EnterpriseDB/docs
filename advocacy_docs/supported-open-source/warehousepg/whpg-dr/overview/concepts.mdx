...
The `whpg-dr` workflow has three phases.

<div style="text-align: center; margin-bottom: 1.5rem;"><img src="../images/whpg-dr-flow.svg" style="max-width: 100%; height: auto;" alt="Three-phase setup flow: Set up primary cluster, Set up DR cluster, Perform DR." /></div>
...
- **Phase 1 — Set up the primary cluster.** Install `whpg-dr` on all primary hosts, configure WAL archiving and storage, take the first base backup, and schedule or run restore points. See [Setting up the primary cluster](../setting-up-primary).

- **Phase 2 — Set up the DR cluster.** In cold DR, provisioning the DR hosts, installing `whpg-dr`, and running `whpg-dr configure restore` all happen at recovery time. In warm DR, the DR cluster is provisioned once in advance and kept running continuously, replaying WAL at regular intervals to stay close to current. See [Setting up the DR cluster](../configuring-dr).

- **Phase 3 — Perform DR.** When a failure occurs or when testing your recovery plan, run `whpg-dr restore` to replay WAL up to the target restore point, then `whpg-dr promote` to bring the cluster online. Optionally add mirrors and a standby coordinator after promotion. See [Performing disaster recovery](../performing-disaster-recovery).
