...
- **Phase 2 — Set up the DR cluster.** Provision DR hosts with the same OS and WarehousePG patch version as the primary, install `whpg-dr`, and run `whpg-dr configure restore` to prepare the cluster to receive restores from the storage repository. See [Installing and configuring the DR cluster](../performing-disaster-recovery/configuring-dr).
