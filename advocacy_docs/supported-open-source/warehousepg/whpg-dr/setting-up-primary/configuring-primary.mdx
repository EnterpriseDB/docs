...
Add this export to `gpadmin`'s `.bashrc` or `.bash_profile` to make it persistent. You can also set `whpg_dr_home` in your backup YAML file, as described in [Configuring backup storage](#configuring-backup-storage). When both are set, the environment variable takes precedence over the YAML field.
...
Create a YAML file that describes your cluster name and storage repository, then run `whpg-dr configure backup` to generate the necessary backup configuration and prepare the cluster for WAL archiving. Run all `whpg-dr` commands as the `gpadmin` user on the coordinator host.
