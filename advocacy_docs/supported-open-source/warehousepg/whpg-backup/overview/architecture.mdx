...
The metadata files contain everything `gprestore` needs to reconstruct a full backup set in parallel, restoring the coordinator's schema and database objects alongside each segment's table data. Because table data is stored as CSV, other utilities, such as [`gpload`](/warehousepg/latest/ref_guide/utility_guide/reference/gpload.mdx), can also load it, in the same cluster or a different one.
