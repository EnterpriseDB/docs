...
1. From the coordinator, download the package from the EDB repository:
    ...
    sudo dnf download edb-<whpg_major_version>-user_profile
    ```
    ...
    export EDB_REPO=gpsupp
    curl -1sSLf "https://downloads.enterprisedb.com/$EDB_SUBSCRIPTION_TOKEN/$EDB_REPO/setup.rpm.sh" | sudo -E bash
    sudo yumdownloader edb-<whpg_major_version>-user_profile
    ```
...
1. From the coordinator, use the `gpsync` utility to transfer the package to all hosts in the cluster and then use use the `gpssh` utility to install the package:
    ...
    ```bash 
    gpsync -f all_hosts <user_profile-package-name> =:/tmp
    gpssh -f all_hosts -e 'sudo dnf install -y /tmp/<user_profile-package-name>'
    ```
    
    Where `<user_profile-package-name>` is the name of the package file you downloaded.
    ...
    ```bash 
    gpsync -f all_hosts <user_profile-package-name> =:/tmp
    gpssh -f all_hosts -e 'sudo yum install -y /tmp/<user_profile-package-name>'
    ```
...
## Understanding the user_profile architecture

user_profile uses an internal SQLite3 database to store security metadata, allowing user profiles and password history to persist and remain consistent across all databases within the cluster.

The extension stores all profile-related information in a file located at `$COORDINATOR_DATA_DIRECTORY/user_profile.db` on the coordinator host.

!!! Important
    The `user_profile.db` file is not synchronized to the standby coordinator automatically by built-in WHPG replication mechanisms.

To ensure continuity of security policies and account states after a coordinator failover or switchover, you must manually copy the `$COORDINATOR_DATA_DIRECTORY/user_profile.db` file from the primary coordinator to the standby coordinator's data directory whenever profiles are created or modified.
