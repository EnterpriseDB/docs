...
originalFilePath: ref_guide/sql_commands/CREATE_EXTERNAL_TABLE.md
...
See [Working with External Data](../../admin_guide/external/working-with-file-based-ext-tables.mdx) for detailed information about working with external tables.
...
You can change the limit for the number of initial rejected rows with the WarehousePG server configuration parameter [gp_initial_bad_row_limit](../config_params/guc_config.mdx#gp_initial_bad_row_limit).
...
When you specify the `LOG ERRORS` clause, WarehousePG captures errors that occur while reading the external table data. For information about the error log format, see [Viewing Bad Rows in the Error Log](../../admin_guide/load/topics/g-viewing-bad-rows-in-the-error-table-or-error-log.mdx#topic58).
