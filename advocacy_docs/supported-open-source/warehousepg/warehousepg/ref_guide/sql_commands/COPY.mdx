...
The limit for the number of initial rejected rows can be changed with the WarehousePG server configuration parameter [gp_initial_bad_row_limit](../config_params/guc_config.mdx#gp_initial_bad_row_limit) server configuration parameter documentation for information about the parameter.
...
-   Use the built-in SQL function `gp_read_error_log('table\_name')`. It requires `SELECT` privilege on table_name. This example displays the error log information for data loaded into table `ext_expenses` with a `COPY` command:
    ...
    For information about the error log format, see [Viewing Bad Rows in the Error Log](../../admin_guide/load/topics/g-viewing-bad-rows-in-the-error-table-or-error-log.mdx#topic58) in the *WarehousePG Administrator Guide*.
...
For information about resource queues, refer to [Using Resource Queues](../../admin_guide/workload_mgmt.mdx) in the *WarehousePG Administrator Guide*.
