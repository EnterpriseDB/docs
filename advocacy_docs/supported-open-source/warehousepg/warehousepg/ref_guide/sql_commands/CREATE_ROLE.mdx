...
WarehousePG includes a program [createuser](../../utility_guide/ref/createuser.mdx) that has the same functionality as `CREATE ROLE` (in fact, it calls this command) but can be run from the command shell.
...
You must exercise caution when specifying an unencrypted password with this command. The password will be transmitted to the server in clear-text, and it might also be logged in the client's command history or the server log. The client program [createuser](../../utility_guide/ref/createuser.mdx), however, transmits the password encrypted. Also, [psql](../../utility_guide/ref/psql.mdx) contains a command `\password` that can be used to safely change the password later.
...
[SET ROLE](SET_ROLE.mdx), [ALTER ROLE](ALTER_ROLE.mdx), [DROP ROLE](DROP_ROLE.mdx), [GRANT](GRANT.mdx), [REVOKE](REVOKE.mdx), [CREATE RESOURCE QUEUE](CREATE_RESOURCE_QUEUE.mdx) [CREATE RESOURCE GROUP](CREATE_RESOURCE_GROUP.mdx), [createuser](../../utility_guide/ref/createuser.mdx)
