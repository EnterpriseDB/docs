...
The Linux sysctl `vm.overcommit_memory` and `vm.overcommit_ratio` variables affect how the operating system manages memory allocation. See the [`/etc/sysctl.conf`](../install_guide/config_os.mdx#topic3__sysctl_file) file parameters guidelines in the *WarehousePG Datatabase Installation Guide* for further details.
...
See [Setting the WarehousePG Recommended OS Parameters](../install_guide/config_os.mdx#topic3/shared_memory_pages) for more details.
