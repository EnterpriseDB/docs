...
-   **vm.overcommit_memory**
    ...
    This is a Linux kernel parameter, set in `/etc/sysctl.conf` and it should always be set to 2. It determines the method the OS uses for determining how much memory can be allocated to processes and 2 is the only safe setting for WarehousePG. Please review the sysctl parameters in the [WarehousePG Installation Guide](../install_guide/config_os.mdx#topic3__sysctl_file).

-   **vm.overcommit_ratio**

    This is a Linux kernel parameter, set in [`/etc/sysctl.conf`](../install_guide/config_os.mdx#topic3__sysctl_file). It is the percentage of RAM that is used for application processes. The remainder is reserved for the operating system. The default on Red Hat is 50.
