...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/admin_guide/workload_mgmt.md
...
The *gp_toolkit* administrative schema has a view called [gp_resq_priority_statement](../ref_guide/system_catalogs/catalog_ref-views.mdx#gp_resq_priority_statement), which lists all statements currently being run and provides the priority, session ID, and other information.
...
To obtain the session ID and statement count parameters required by this function, superusers can use the `gp_toolkit` administrative schema view [gp_resq_priority_statement](../ref_guide/system_catalogs/catalog_ref-views.mdx#gp_resq_priority_statement). From the view, use these values for the function parameters.
