...
The `ALTER TABLE` command changes the definition of a table. Use `ALTER TABLE` to change table attributes such as column definitions, distribution policy, access method, storage parameters, and partition structure (see also [Partitioning Large Tables](ddl-partition.mdx)). For example, to add a not-null constraint to a table column:
