...
WarehousePG can report the progress of `ANALYZE`, `CLUSTER`, `COPY`, `CREATE INDEX`, `REINDEX`, and `VACUUM` commands during command execution. WarehousePG can also report the progress of a running base backup (initiated during [gprecoverseg -F](../../utility_guide/ref/gprecoverseg.mdx)) command invocation, allowing you to monitor the progress of these possibly long-running operations.
