...
<div class="note">Because WarehousePG skips <code>pg_hba.conf</code>-controlled authentication for a retrieve session, for security purposes you may invoke only the <code>RETRIEVE</code> command in the session.</div>
...
<div class="note">When you close a parallel retrieve cursor, WarehousePG terminates any open retrieve sessions associated with the cursor.</div>
