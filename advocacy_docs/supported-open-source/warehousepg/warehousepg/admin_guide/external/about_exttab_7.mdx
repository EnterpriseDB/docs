...
-   WarehousePG 7 uses foreign table data structures and catalogs to internally represent external tables. Use the [pg_foreign_table](../../ref_guide/system_catalogs/pg_foreign_table.mdx) system catalog table and the `ftoptions` column to view the table definition.
...
-   Because an external table is internally represented as a foreign table:
    ...
    -   External tables are included in the foreign table catalogs, for example [pg_foreign_table](../../ref_guide/system_catalogs/pg_foreign_table.mdx).
