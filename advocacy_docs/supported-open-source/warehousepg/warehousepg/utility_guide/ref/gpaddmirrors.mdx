...
When enabling a mirroring configuration that adds hosts to the WarehousePG cluster, `gpaddmirrors` populates the [gp_segment_configuration](../../ref_guide/system_catalogs/gp_segment_configuration.mdx) catalog table with the mirror segment instance information. WarehousePG uses the address value of the `gp_segment_configuration` catalog table when looking up host systems for WarehousePG interconnect (internal) communication between the coordinator and segment instances and between segment instances, and for other internal communication.
