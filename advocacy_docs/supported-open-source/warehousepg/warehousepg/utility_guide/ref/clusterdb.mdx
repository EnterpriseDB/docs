...
`clusterdb` is a wrapper around the SQL command [CLUSTER](../../ref_guide/sql_commands/CLUSTER.mdx). There is no effective difference between clustering databases via this utility and via other methods for accessing the server.
...
In case of difficulty, see [CLUSTER](../../ref_guide/sql_commands/CLUSTER.mdx) and [psql](psql.mdx) for discussions of potential problems and error messages. The database server must be running at the targeted host. Also, any default connection settings and environment variables used by the `libpq` front-end library will apply.
...
[CLUSTER](../../ref_guide/sql_commands/CLUSTER.mdx)
