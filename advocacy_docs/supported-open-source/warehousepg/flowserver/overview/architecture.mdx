...
FlowServer is a gRPC-based streaming engine that serves as a central hub for high-speed ETL tasks, receiving data from clients and managing connections to the WarehousePG cluster. Running on any host with cluster connectivity, it handles FlowCLI requests to fetch data from Kafka or RabbitMQ and load it into WarehousePG. The server supports JSON, Avro, and CSV formats via configuration files that define database settings and Prometheus metrics. During execution, it submits transactions to the coordinator and uses the gpfdist protocol to write data directly into segments via external tables.
...
- A user initiates one or more ETL load jobs via a client application (Kafka or RabbitMQ).
- Flowcli uses the gRPC protocol to submit and start data load jobs to a running FlowServer service instance.
...
- The FlowServer service instance writes the data delivered from the client directly into the segments of the WHPG cluster.
...
- **Format**: FlowServer converts incoming raw data from formats like JSON, CSV, or Avro into a structure compatible with database external tables.
