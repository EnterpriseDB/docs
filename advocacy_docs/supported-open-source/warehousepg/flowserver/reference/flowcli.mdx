...
Manage the WarehousePG streaming server jobs. It allows users to submit, start, stop, and monitor jobs.
...
- `--reset-to-earliest`: Reset to the earliest offset in the source on job starting. This is applicable for jobs that read from Kafka or RabbitMQ stream.
- `--reset-to-latest`: Reset to the latest offset in the source on job starting. This is applicable for jobs that read from Kafka or RabbitMQ stream.
- `--reset-to-timestamp <timestamp>`: Reset to the specified timestamp in the source on job starting. This is applicable for jobs that read from Kafka or RabbitMQ stream.
...
- `--host <host>`: The hostname or IP address of the WarehousePG flow server. Default is localhost.
- `--port <port>`: The port number on which the WarehousePG flow server is running. Default is 6060.
