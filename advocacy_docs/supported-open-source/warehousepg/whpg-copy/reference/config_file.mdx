...
# bytes: connecting to the destination daemon and waiting for the
# destination's receiver to appear. A transfer that can never complete fails
# with an error instead of hanging.
# Set to 0 to wait forever. One exception: the preflight check below is
# always capped at 30 seconds, because a reachability probe that waits
# forever stalls the copy before it starts.
# Default: 120
timeout = 120
...
# host name usually resolves to several addresses, and only some of them can
# be reached from the source hosts. Set this value to the network the source
# hosts can reach. Without it, whichever address DNS returns first is used,
# and the choice can differ between runs.
...
# Default: false
metadata_only = false
# Include object ownership and privilege (GRANT) information in the DDL.
# By default, whpg-copy omits both, equivalent to running pg_dump with
# --no-owner --no-privileges.
# Default: false
with_owner = false
with_privilege = false
# Encrypt data transfer between source and destination using Transport Layer
# Security (TLS).
...
#   "mutual"      : mutual TLS; the daemon also requires each sender to
#                   present a CA-signed client certificate
# When not "disabled", tls_dir must point to a directory that exists at the
# same path on every source and destination host and contains the
# certificates produced by `whpg-copy gen-certs`.
# tls_mode = "mutual"
...
dry_run = false
# Verbosity of the log file written by whpg-copy and the copy daemon it
# starts on the destination cluster. Does not affect what's printed to the
# console, which is controlled by RUST_LOG.
...
Directory containing the TLS certificates used to encrypt data transfer. Required when `tls_mode` isn't `disabled`. The directory must exist at the same path on every source and destination host.

**log_level**

Verbosity of the log file written by `whpg-copy` and the copy daemon it starts on the destination cluster. This setting doesn't affect what's printed to the console. The supported options are `info` and `debug` (default).
