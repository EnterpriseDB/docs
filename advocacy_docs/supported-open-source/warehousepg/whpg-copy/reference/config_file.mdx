...
WarehousePG Copy TOML-based configuration file reference.
...
Connection string for the source database. Supports standard PostgreSQL connection strings. It follows the format `postgres://[user[:password]@]host[:port][/dbname]`.

**dst_url**

Connection string for the destination database. It follows the format `postgres://[user[:password]@]host[:port][/dbname]`
