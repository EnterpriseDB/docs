...
- **Views and materialized views:** `whpg-copy` supports the creation of both regular views and materialized views on the destination cluster. It refreshes each materialized view after the copy operation, but only if every relation it depends on, including other materialized views, finished copying successfully. If a dependency fails, `whpg-copy` skips the refresh and you must manually execute `REFRESH MATERIALIZED VIEW` on the destination once the underlying data is in place.
