...
- While you can specify a password in the connection string, `whpg-copy` currently does not provide full support for password authentication. To ensure stability, we recommend to use a `.pgpass` file or configure trust-based authentication (via `pg_hba.conf`) for the duration of the copy. 
