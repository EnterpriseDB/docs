...
The `whpg-copy` utility provides a high-performance method for migrating data between a source and a destination WarehousePG (WHPG) cluster. 

1. Make sure you meet the [prerequisites](#prerequisites).
1. [Diagnose connectivity between the clusters](#diagnosing-connectivity-between-clusters).
...
- While passwords can be specified in the connection string, `whpg-copy` currently does not currently provide full support for password authentication. To ensure stability, we recommend to use a `.pgpass` file or configure trust-based authentication (via `pg_hba.conf`) for the duration of the copy. 
...
`whpg-copy` supports both direct command-line arguments and TOML-based configuration files.

- Command Line: Ideal for simple tasks or one-off copies.
- TOML File: Recommended for complex setups, reusable pipelines, or when using options available only via TOML. Use `--config-file <PATH>` or `-c <PATH>` to load.

Note Command-line arguments take precedence over settings defined in a TOML file.
...
For a full list of parameters, see [whpg-copy copy](reference/vhpg-copy-utility#copy) and [whpg-copy configuration file](reference/config_file).
...
You can retry the copy operation by running:
