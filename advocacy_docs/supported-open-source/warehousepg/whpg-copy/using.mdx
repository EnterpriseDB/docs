...
- `whpg-copy` currently does not provide full support for password authentication. We recommend to use a `.pgpass` file or configure trust-based authentication (via `pg_hba.conf`) for the duration of the copy. 
