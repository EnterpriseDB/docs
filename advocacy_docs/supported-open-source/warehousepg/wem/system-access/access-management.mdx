...
- **Remote mode** (when WEM is on a different host): select **Generate Commands** to open the **Apply pg_hba.conf Changes** dialog, which provides shell commands you can copy and run directly on the coordinator to apply the changes.
