...
- **Restore factory defaults:** If permissions become misconfigured, use the reset defaults button to instantly revert the role-based access control (RBAC) matrix to the factory-recommended secure state for all roles.
...
- **Establish the database backbone:** Configure the WarehousePG (WHPG) database connection with your coordinator host and credentials.
...
- **Measure custom sql performance:** Use the **Query** type to run specific SQL statements (`SELECT count(*) FROM sales;`) at set intervals. Define warning and critical latency thresholds to trigger alerts if performance degrades.
