...
[PTT-935]: #
- Cancelling an **idle in transaction** session from the **Query Monitor** doesn't release the blocking transaction. WEM uses `pg_cancel_backend`, which terminates the user's connection but leaves the transaction open. The blocked queries remain in a waiting state until the transaction is manually cleaned up. 
