...
The host agent (`acp-host-agent`) is a background service that runs on the WarehousePG coordinator host and executes privileged operations — such as editing `pg_hba.conf` and managing Safeguard — on behalf of WEM. WEM dispatches these operations through its internal job-execution engine, which ensures they're reliable and recoverable even when connectivity is interrupted.

The host agent is installed separately on the WarehousePG coordinator host, alongside WEM. WEM doesn't use host agent features unless both the host agent and WEM's internal job-execution engine are running and connected. See [Installing the host agent](../installing/host-agent).
...
2. **Export & routing:** 
    ...
    - **Inbound metrics (optional):** If the remote write receiver is enabled, Grafana Alloy or other Prometheus-compatible clients can push additional time-series data to WEM via `POST /api/v1/write`. WEM aggregates these with its own metrics at `/prom/metrics`.
    - **Privileged host operations:** When WEM triggers a host-level action (such as updating `pg_hba.conf`), the request is dispatched to the host agent through WEM's internal job-execution engine, which tracks the operation and retries automatically if the connection is interrupted.
