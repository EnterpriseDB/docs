...
The host agent (`acp-host-agent`) is a background service that runs on the WarehousePG coordinator host and executes privileged operations — such as editing `pg_hba.conf` — on behalf of WEM. WEM dispatches these operations through its internal job-execution engine, which ensures they're reliable and recoverable even when connectivity is interrupted.
