...
Check the OTel Collector across every cluster node at once:

```bash
gpssh -f hostfile_whpginitsystem -u gpadmin -e "sudo systemctl is-active edb-otelcol"
```
...
**Cause:** PXF actions dispatch through the host agent on the coordinator rather than running locally on the WEM host, so this usually means the host agent isn't reachable there, or PXF itself isn't installed on the coordinator.
...
**Cause:** ClickHouse is reachable, but the `acp_observability` database doesn't exist yet. The OTel Collector creates it automatically on its first data flush, so this is expected until at least one cluster node's telemetry pipeline is actually running, not necessarily a configuration error on its own.
...
2. Check `/etc/edb/acp-host-agent/acp-host-agent.conf`. `WEM_CONNECT_ADDRESS` must be an IPv4 address in the form `<wem-host>:<wem-port + 1>` (default `8081`), and that port must be reachable from every cluster node. Test it directly:
    ...
    See [Installing the host agent](installing/host-agent) for setup, and [Approving agent registrations](installing/host-agent#approving-agent-registrations) for the next step, a registered agent still needs manual approval before it shows as active.
