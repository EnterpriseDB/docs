...
-   Compare active connections against the maximum limit using the **Connections** card. If connections are near the ceiling, new application requests will be rejected. To prevent rejection, terminate idle sessions or increase the `max_connections` configuration parameter.
