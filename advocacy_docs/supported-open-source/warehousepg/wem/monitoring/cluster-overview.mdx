...
- Stop, Fast Stop, and Restart each prompt for confirmation before running, since these actions are destructive. A cluster whose `WEM_CLUSTER_ENVIRONMENT` is set to `prod` shows an extra production warning in that confirmation. See [Cluster identity](../reference/configuration#cluster-identity).
