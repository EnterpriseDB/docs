...
Search across coordinator and segment logs aggregated by Loki using the **Historical Logs (Coordinator & Segments)** tab. Results include **Time**, **Severity**, **Job**, **Database**, **User**, **Source Host**, **Segment**, and **Message** columns. Use these columns to trace activity back to a specific user or client, and to correlate errors with a particular application or service account.

!!! Note "Loki integration required"
    You must configure a Loki URL in your system environment to enable the **Historical Logs (Coordinator & Segments)** tab. See [Observability endpoints](../installing/wem#observability-endpoints) in the WEM configuration for details.
