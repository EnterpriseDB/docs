...
- `PROMETHEUS_BEARER_TOKEN` (optional): Bearer token to authenticate the Collector's remote write requests to Prometheus. Set this parameter if your Prometheus instance requires authentication. For example:

    ```ini
    PROMETHEUS_BEARER_TOKEN="your-prometheus-token"
    ```

- `LOKI_BEARER_TOKEN` (optional): Bearer token to authenticate the Collector's log push requests to Loki. Set this parameter if your Loki instance requires authentication. For example:
