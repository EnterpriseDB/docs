...
WEM requires a PostgreSQL 12 or later database for its own application state, either your WHPG cluster's own database or a dedicated instance. If you don't already have one running, see [Installing PostgreSQL](observability-stack#installing-postgresql-optional).

WEM 1.3 introduces the `acp-host-agent` service for privileged host operations. See [Installing the host agent](host-agent).

WEM 1.3 also introduces Safeguard, disaster recovery for WarehousePG built on `whpg-dr`, which requires the host agent. If you want to start using it, see [Configuring Safeguard](safeguard).
...
Safeguard is new in WEM 1.3. If you're upgrading from an earlier version, see [Configuring Safeguard](safeguard) for the full procedure.
