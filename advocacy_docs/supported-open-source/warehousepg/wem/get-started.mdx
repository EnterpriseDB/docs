...
1. Navigate to your WEM server URL (for example, `http://your-server:8080`) and enter your administrative credentials. The initial administrator username is `admin`, with the password you set (or that WEM auto-generated) as `WEM_ADMIN_PASSWORD` during installation.
...
| Task | Panel |
| --------------- | ------------ |
| [Verifying and managing cluster health](monitoring/cluster-overview/) | Cluster |
...
| [Securing backups](protecting-data/backups/) | Backups |
| [Managing disaster recovery](protecting-data/safeguard/) | Safeguard |
...
WEM uses role-based access control (RBAC). After the initial bootstrap, all user management and password updates are handled exclusively via the **Management** panel. A user's assigned profile, and the individual actions granted to it, determine which panels and actions are visible. See [Defining role-based access control](system-access/management/#defining-role-based-access-control) for how to configure it, and the [Role permissions matrix](reference#role-permissions-matrix) for the default access each profile has.
