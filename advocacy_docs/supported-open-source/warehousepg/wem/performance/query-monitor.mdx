...
The **Query Monitor** panel on the left sidebar provides a real-time view of every session connected to the WarehousePG (WHPG) cluster. By combining live monitoring with an integrated SQL workbench and AI-assisted diagnostic tools, it allows you to maintain peak cluster performance.
...
Use the **Active Queries** tab to track execution progress and identify resource-heavy statements that could be impacting system stability.
...
The **Active Queries** tab features a status bar that provides a real-time count for each connection state. These metrics offer an immediate snapshot of cluster health and help you quickly identify if the system is under heavy load:
...
- **Idle:** Established connections waiting for the next command. No immediate action is required, as this is standard behavior for established connections.
- **Idle in Transaction:** Open transactions waiting for input. You must monitor these as they can prevent vacuuming and cause table bloat. 
