...
- If you have Admin or Operator privileges, use the **Cancel** action to gracefully stop a statement consuming excessive resources. Use the **Export** feature to save a snapshot of current activity for a performance audit.

**Understanding query statuses**

The **Status** tab provides a real-time count for each connection state. These metrics offer an immediate snapshot of cluster health and help you quickly identify if the system is under heavy load:
...
- **Transaction (abort):** Transactions that have encountered an error and are currently in an aborted state. You must roll back or terminated these sessions to release system locks.
...
!!! Note
    The AI Assistant is an optional feature. An administrator must configure an `ANTHROPIC_API_KEY` for the assistant to be active. See [Configuring WEM](../installing/wem#configuring-wem) and [Configuring WEM](../configuring) for details.
