...
WarehousePG (WHPG) provides two primary strategies for resource management: resource groups and resource queues. They're controlled by the `gp_resource_manager` server configuration parameter. See [Managing resources](/supported-open-source/warehousepg/warehousepg/admin_guide/wlmgmt/) from the WHPG documentation for details about how WHPG provides resource management.
...
Use this section when `gp_resource_manager` is set to `group`. Resource groups use Linux Control Groups (cgroups) to provide hard limits on CPU and memory. WarehousePG Enterprise Manager (WEM) automatically adjusts the **Configuration** tab to match the capabilities of your WHPG version.

- **Configure group limits:**
    - **WHPG 7 settings:**
        - **CPU Max %:** Sets a hard limit on CPU usage. The group can't exceed this even if the cluster is idle.
