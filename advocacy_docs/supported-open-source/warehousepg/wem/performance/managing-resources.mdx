...
WarehousePG (WHPG) provides two primary strategies for resource management: resource groups and resource queues. They're controlled by the `gp_resource_manager` server configuration parameter. See [Managing resources](/warehousepg/latest/admin_guide/wlmgmt/) for details on how WHPG provides resource management.
