...
description: Use the Users panel to manage user accounts, define role-based access controls, and configure system-wide settings for WarehousePG Enterprise Manager.
deepToC: true
---

The **Users** panel is the central administrative hub for WarehousePG Enterprise Manager (WEM). Use these actions to oversee account security, define permission tiers, and configure system-wide integration settings.
...
- **Provision new users:** Select the **Add User** button to create a new identity. Use the **WHPG User Mapping** feature to either link the account to an existing database role (like `gpadmin`) or automatically provision a new database identity that matches the WEM username.
- **Enforce security policies:** When editing a user, use the **Active** checkbox to revoke access immediately without deleting the account's history. Enable **Force Password Change** to ensure users rotate their credentials during their next session.
- **Audit administrative overhead:** Regularly review the **Admins** count in the header. Keeping the number of high-privileged accounts to a minimum is a core security best practice.
...
- **Customize module visibility:** Use the **Role Permissions Matrix** to toggle the visibility of specific tabs for each role. This allows you to simplify the interface for viewers or restrict sensitive configuration pages to admins only.
...
Use the **Settings** tab to establish the core connection between WEM and your infrastructure.
...
Use the **Canary Queries** tab to define automated probes that verify the end-to-end integrity of your cluster.
