...
!!! Note
    The `pg-backup-api` currently only supports Barman packages downloaded from the EDB repositories. It is not compatible with PGDG Barman installations due to differing Python versions, which will prevent the API from starting.
