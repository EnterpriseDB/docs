...
barman cron
__OUTPUT__
Starting WAL archiving for server localhost
```

Give THAT a minute to run, and now the check should pass:
...
(since this is dependant on the timing of various background processes, if you're firing off these commands quickly you may see errors if the machine is slow - give it just a bit to catch up)
