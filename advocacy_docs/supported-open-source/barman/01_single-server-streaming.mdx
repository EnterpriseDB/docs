...
This tutorial provides a quick walk-through of setting up a backup and recovery scenario using Barman on a PostgreSQL server. It covers:
...
Normally, you would never run or store backups on the same server where you're running your database: it adds extra load to the machine already taxed with running the database server, and provides little protection from attacks or hardware failure. 

!!! Warning
    This tutorial is intended to demonstrate the core concepts of backup and recovery of a PostgreSQL database using a Barman backup. For real-world scenarios that involve production architectures, refer to [the Barman Manual](http://docs.pgbarman.org/).
