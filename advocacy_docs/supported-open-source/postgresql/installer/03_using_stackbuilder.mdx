...
You can invoke Stack Builder at any time after the installation has completed by selecting the **Application Stack Builder** menu option from the **PostgreSQL** menu. Enter your system password (if prompted), and the Stack Builder welcome window opens.
...
If the selected PostgreSQL installation has restricted Internet access, use the **Proxy Servers** button on the Welcome window to open the Proxy servers dialog box.

Enter the IP address and port number of the proxy server in the **HTTP proxy** or **FTP proxy** fields on the Proxy servers dialog box. Currently, all Stack Builder modules are distributed via HTTP proxy (FTP proxy information is ignored). Select **OK** to continue.

![The Proxy servers dialog box](images/the_proxy_servers_dialog.png)

Fig. 2: The Proxy servers dialog box
...
-   To add new modules to the selected PostgreSQL installation, check the box to the left of the module name and select **Next**.
...
You can check the box next to **Skip Installation** and select **Next** to exit Stack Builder without installing the downloaded files, or leave the box unchecked and select **Next** to start the installation process.
...
You may occasionally encounter packages that don’t install successfully. If a package fails to install, Stack Builder alerts you to the installation error with a popup dialog box and writes a message to the log file stored in `%TEMP%`.

When the installation is complete, the installer alerts you to the success or failure of the installations of the requested packages. If you are prompted by an installer to restart your computer, re-boot now.
