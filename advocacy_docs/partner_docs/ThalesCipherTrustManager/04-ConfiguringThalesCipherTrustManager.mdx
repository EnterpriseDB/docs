...
A user needs to be created for future authentication when creating a key on Thales CipherTrust Manager with EDB Postgres Advanced Server.  It verifies the username and password against the Thales CipherTurst Manager internal database.
...
3. Select `Add Users` and provide all of the required information:
- Username
- Password
...
7. The user that you created now has the appropriate settings and permissions to create and download certificates in Thales.

## Create and Download Certificates on Thales CipherTrust Manager UI
...
These are the other certificates you will need for your KMIP server and pykmip.conf file.
...
9. Paste the token that you created above into the `Registration Token` box and then press `Save`.
    ![Thales Create New Registration Client](Images/ThalesRegistrationClient.png) 

10. Click on `Save Private Key` and click on `Save Certificate`, make sure to note their saved location for later.
    ![Thales Download Certificate Files](Images/ThalesDownloadCerts.png)

You are now ready to use Thales CipherTrust Manager and EDB Postgres Advanced Server with TDE the three required certificates for key management.