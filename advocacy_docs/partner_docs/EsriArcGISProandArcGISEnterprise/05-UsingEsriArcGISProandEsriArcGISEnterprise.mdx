...
1. Go to the HongKong GDB, or any other geodatabase, as instructed in the Download GIS Database steps, and click `Download`.

2. Locate and unzip HongKong.gdb.zip file that was downloaded.

!!! Note Inside is a file geodatabase: HongKong.gdb. Inside the geodatabase there are a lot of files with mysterious names. These are for functionalities in the geodatabase, do not do anything with them.
!!!
...
!!! Note There will be a few geodatabases listed. If Default.gdb is present, it was created automatically when you created the project. It is empty now, but any new data that you create in this project will land here by default.
!!!

7. Add the HongKong.gbd geodatabase to the map by selecting the down arrow next to it,  and right-clicking each item and selecting `Add to New` then select `Map`.
