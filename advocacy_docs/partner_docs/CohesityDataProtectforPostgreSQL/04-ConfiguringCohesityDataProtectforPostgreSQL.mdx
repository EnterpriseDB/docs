...
!!! Note Note from Cohesity documentation, if you have not set archive_mode=on before a restore is taken, the PostgreSQL database is restarted after the restore to set archive_mode=on.
!!!