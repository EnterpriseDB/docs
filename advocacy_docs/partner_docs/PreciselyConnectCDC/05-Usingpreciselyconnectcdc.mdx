...
After you configure your EDB Postgres distribution, you can use Precisely CDC's replication functionality. Thes examples show the replication functionality using EDB Postgres Advanced Server.

<!-- Didn't edit this lists yet -->
...
With Precisely Connect CDC, you can also have “transform” your data as you replicate it from a source to target using Data Transformation. See the section on “Data Transformation” on how to enable this feature.

- Data Transformation
  - Modify the source data you're distributing or even create or derive the data you distribute to the target system.
...
7. Enter connection information for EDB Postgres Advanced Server as source instance. (**Server Name** is where you give the database name.)
...
10. when prompted on the User ID and Password screen, provide the password of rpuser. Refer to the user name and password given on New Server screen.
...
11. On the Install Source Metabase screen, select **OK**.
  ...
  A success message appears once the source metabase is created on the EDB Postgres Advanced Server source instance.

13. Open pgAdmin and connect to your EDB Postgres Advanced Server Source instance. Navigate to Schemas. Under the schemas, the rpuser schema was created.
...
16. On Database/Schema name screen, provide the schema name for the schema you want to replicate. This example uses the schema `public`.
...
21. Select the Refresh list. Select the schema `public` and click **OK**.

![ Refresh Available Tables](Images/refreshtables.png)

22. To display the available table, select the newly added Server and select **Tables > Available Tables > Public > (Schema name). A list of available tables appears.
...
6. Select **OK** on Install Target Only Metabase screen.
...
9. Right-click the newly added Server and select **Prepare User Database**, this will add the replication user to EDB Postgres Advanced Server Target Instance.
...
12. Select **OK** on the Prepare User Database screen.
...
8. On the Target Tables screen, enter the Target Schema against each table and Select Next.
...
1. To save the Model, click **File > Save** option and provide the name for the Model.
...
5. Select **OK** on Commit Model screen. This will create the Model files.
...
2. Select **Model > New Model** from Menu.
...
5. Select **Process** to see the progress.
...
7. Select **OK** on Commit Model screen. This will create the Model files.
...
2. As the Model is updated in the Connect CDC Director to add the Replication Distribution, click the Model Update, this will add the Replication Distribution to the Connect CDC MonCon Interface.
