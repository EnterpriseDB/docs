...
!!! Note
    You can also connect to your EDB database instance by selecting `New Connection from JDBC URL` from the `Database` menu. DBeaver will automatically select EnterpriseDB in the `Drivers` field when the EDB JDBC URL is provided in the `JDBC URL` field. Click `Proceed` and continue to step 3.
...
!!! Note
    (You can also connect to your PostgreSQL database instance by selecting `New Connection from JDBC URL` from the `Database` menu. DBeaver will automatically select PostgreSQL in the `Drivers` field when the PostgreSQL JDBC URL is provided in the `JDBC URL` field. Click `Proceed` and continue to step 3.)
