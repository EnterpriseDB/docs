...
Liquibase Pro is a development tool that allows changes to be applied to the EDB database using the Liquibase CLI and viewed on the Liquibase Hub.
...
This section provides examples of applying database changes using Liquibase Pro changesets including:
...
!!! Note
    All Liquibase Pro commands in the examples are executed from the directory where Liquibase Pro is installed.
