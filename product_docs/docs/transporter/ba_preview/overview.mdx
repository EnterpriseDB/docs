...
1. Verify that your [database version is compatible with Transporter](supported_versions). 

2. Assess Oracle database sources for schema compatibility using the [EDB Migration Portal](https://www.enterprisedb.com/docs/migration_portal/latest/) before starting the data migration process. Address incompatible schemas or exclude them from the migration and migrate the schema to the new database **without applying constraints**.
...
Configure and run the EDB Transporter source reader. For details, see [Configuring EDB Transporter software](config_reader).
