...
Configuring Postgres for EDB Data Migration Service requires administrative privileges. Create a change data capture (CDC) migration role with limited privileges for data migration.
...
1. [Verify Postgres configuration](#verify-postgres-configuration).
...
4. Ensure `max_wal_size` is configured for adequate WAL LSN lifetime.
    ...
    The streaming migration process also requires changes to be available in the WAL until they can be streamed to durable message storage in the cloud infrastructure of EDB Data Migration Service. Setting `max_wal_size` too small can affect performance. It can also interfere with the migration process by allowing Postgres LSNs to be dropped from the WAL before they can be streamed.
...
The EDB DMS Reader installation (packages as `cdcreader`) comes with a helper script that validates the Postgres configuration and helps you identify any issues. After you configure the database, we recommend running the script and ensuring all checks passed.
...
Where:
 - `<MIGRATION_ROLE>` is the name of the Postgres role or user to use for CDC migration database access.
 - `pgoutput` is the logical decoding plugin supplied by Postgres that EDB Data Migration Service uses.
