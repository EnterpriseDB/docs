...
!!! Note
    From `EnterpriseDB.EDBClient 7.0.4` onwards, the `EDBDataSource` is recommended to connect to EDB Postgres Advanced Server database or execute SQL directly against it. For more information on data source, see [Npgsql docs](https://www.npgsql.org/doc/basic-usage.html).
...
When `Pooling` is enabled and the connection is closed, reopened, and the underlying connection is reused, then some operations are executed to discard the previous connection resources. It is possible to override this behavior by enabling `No Reset On Close`.
...
!!! Note
    For `EnterpriseDB.EDBClient 7.0.4` and later, the `EDBDataSource` is recommended to connect to EDB Postgres Advanced Server database or execute SQL directly against it. For more information, see [Npgsql docs](https://www.npgsql.org/doc/basic-usage.html).
    
    All the examples show the basic steps to connect to an EDB Postgres Advanced Server without a data source. All these examples can also be executed using a data source.
