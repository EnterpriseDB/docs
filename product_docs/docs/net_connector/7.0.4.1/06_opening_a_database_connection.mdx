...
!!! Note
    For `EnterpriseDB.EDBClient 7.0.4` and later, the `EDBDataSource` is recommended to connect to EDB Postgres Advanced Server database or execute SQL directly against it. For more information on data source, see [Npgsql docs](https://www.npgsql.org/doc/basic-usage.html).
    
    All the examples show the basic steps to connect to an EDB Postgres Advanced Server without a data source and can also be executed using a data source.
