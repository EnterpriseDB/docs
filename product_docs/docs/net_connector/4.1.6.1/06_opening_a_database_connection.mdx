...
The following example demonstrates how to open a connection to an instance of Advanced Server and then close the connection. The connection is established using the credentials specified in the `DB_CONN_STRING` configuration parameter (see [Using the .Net Connector](#using_the_net_connector) for an introduction to connection information and also see [Connection String Parameters](#connection_string_parameters) for connection parameters).
