...
```sql
-- in WarehousePG
CREATE WRITABLE EXTERNAL TABLE ch_events_write (
    id         text,
    event_type text,
    event_time timestamp,
    value      float8
...
INSERT INTO ch_events_write VALUES ('evt-1', 'purchase', now(), 99.99);
```
...
CREATE SERVER clickhouse_fdw
    FOREIGN DATA WRAPPER jdbc_pxf_fdw
    OPTIONS (config 'clickhouse');

CREATE USER MAPPING FOR CURRENT_USER SERVER clickhouse_fdw;

CREATE FOREIGN TABLE ch_events_fdw (
    id         text,
    event_type text,
    event_time timestamp,
    value      float8
)
SERVER clickhouse_fdw
OPTIONS (resource '<database>.<table>');

SELECT * FROM ch_events_fdw;
```
...
```sql
-- in WarehousePG
INSERT INTO ch_events_fdw VALUES ('evt-1', 'purchase', now(), 99.99);
```
...
```sql
EXPLAIN SELECT * FROM ch_events;
__OUTPUT__
                        QUERY PLAN
-----------------------------------------------------------
 Gather Motion 4:1  (slice1; segments: 4)
   ->  Foreign Scan on ch_events
