...
Open a `clickhouse-client` session on any node, then pass the connection details inline with each query:
...
Use `INSERT INTO TABLE FUNCTION` to write rows to WarehousePG:
