...
| `edb-clickhouse-common-static` | Compiled ClickHouse binary, required by the server and client packages. `dnf` installs and updates it automatically as a dependency, so you don't need to list it explicitly. | All server and client nodes |
...
1. Install the packages:
   ...
   `dnf` pulls in `edb-clickhouse-common-static` automatically as a dependency.
...
1. Verify the EDB build by checking the version string:
   ...
   The output includes the EDB release version, for example `26.3.22.7+1edb1`.
...
On a cluster, repeat the following steps one node at a time, keeping at least one replica per shard on its current version until the upgrade completes. See the [upstream ClickHouse upgrade guidance](https://clickhouse.com/docs/operations/update) for version-skipping limits and other considerations for large clusters.

1. Refresh the EDB repository metadata, so `dnf` sees the new version instead of what it last cached:

   ```bash
   sudo dnf makecache
   ```

2. Download and install the updated packages:
   ...
   `dnf` pulls in a matching update to `edb-clickhouse-common-static` automatically, the same way it does at install time.

   On a dedicated Keeper node, update the Keeper package instead:

   ```bash
   sudo dnf update -y edb-clickhouse-keeper
   ```
