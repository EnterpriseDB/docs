...
| `edb-clickhouse-keeper` | Standalone Keeper binary for dedicated coordination nodes. Only needed for multi-node deployments that run Keeper on separate nodes. Requires `edb-clickhouse-common-static`. | Dedicated Keeper nodes only |
...
1. Install the role-appropriate packages on each node.
   ...
   sudo dnf install -y \
     edb-clickhouse-common-static \
     edb-clickhouse-keeper
   ```

   `edb-clickhouse-keeper` requires `edb-clickhouse-common-static` for the ClickHouse binary it symlinks to. Install both together.
...
1. Query the local table on any server node. Each node returns only the rows on its shard:

   ```sql
   SELECT * FROM events;
   __OUTPUT__
   ┌─event_id─┬─user_id─┬─event_type─┬──────────────created_at─┐
   │        2 │     102 │ purchase   │ 2024-01-01 10:00:00      │
   │        4 │     104 │ login      │ 2024-01-01 10:00:00      │
   └──────────┴─────────┴────────────┴─────────────────────────┘
   ```
...
1. Query `events_dist` from any node to confirm all rows are returned across shards:
   ...
   __OUTPUT__
   ┌─event_id─┬─user_id─┬─event_type─┬──────────────created_at─┐
   │        1 │     101 │ login      │ 2024-01-01 10:00:00      │
   │        2 │     102 │ purchase   │ 2024-01-01 10:00:00      │
   │        3 │     103 │ logout     │ 2024-01-01 10:00:00      │
   │        4 │     104 │ login      │ 2024-01-01 10:00:00      │
   └──────────┴─────────┴────────────┴─────────────────────────┘
   ```
