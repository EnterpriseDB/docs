...
echo "$EDB_SUBSCRIPTION_TOKEN" | docker login docker.enterprisedb.com \
  --username clickhouse \
  --password-stdin
docker pull docker.enterprisedb.com/clickhouse/edb-clickhouse-server:26.3.13.31-1edb1
```
...
1. Start the server:
   ...
   docker run -d \
     ...
     -p 8123:8123 \
     -p 9000:9000 \
     docker.enterprisedb.com/clickhouse/edb-clickhouse-server:26.3.13.31-1edb1
   ```
...
3. Connect with the ClickHouse client. The default password for the `default` user is `admin123`:

   ```bash
   docker exec -it clickhouse-server clickhouse-client --password admin123
   ```
...
2. Connect to `server-01`. The default password for the `default` user is `admin123`:

   ```bash
   docker exec -it server-01 clickhouse-client --password admin123
   ```
...
Check [Release notes](../release_notes) for the ClickHouse server version you're upgrading to before you start.

On a Compose-based cluster, upgrade one service at a time, keeping at least one replica per shard on its current version until the upgrade completes. See the [upstream ClickHouse upgrade guidance](https://clickhouse.com/docs/operations/update) for version-skipping limits and other considerations for large clusters.

1. Pull the new image tag:

   ```bash
   docker pull docker.enterprisedb.com/clickhouse/edb-clickhouse-server:<new-version>-1edb1
   ```

   Replace `<new-version>` with the target version from [Release notes](../release_notes).

2. For a single-node deployment, recreate the container with the new image, keeping the same volume mounts and ports as the original `docker run` command:
   ...
   docker run -d \
     ...
     -p 9000:9000 \
     -v /local/path/clickhouse-data:/var/lib/clickhouse \
     docker.enterprisedb.com/clickhouse/edb-clickhouse-server:<new-version>-1edb1
   ```

   With a persistent volume mounted at `/var/lib/clickhouse`, data survives the container recreation.

3. For a Compose-based cluster, in the `docker-compose.yml` you created under [Creating the Compose file](#creating-the-compose-file), edit one service's `image` line to the new tag. For example, for `server-01`:

   ```yaml
   server-01:
     image: docker.enterprisedb.com/clickhouse/edb-clickhouse-server:<new-version>-1edb1
   ```

   Then recreate just that container:

   ```bash
   docker compose up -d --no-deps server-01
   ```

   `--no-deps` recreates only `server-01`, leaving the other, still-healthy containers running. Confirm it rejoins and reports the new version, then repeat for each remaining service in turn.

4. Verify the running version. For a single-node deployment:

   ```bash
   docker exec -it clickhouse-server clickhouse-client --password admin123 --query "SELECT version();"
   ```

   Or, for a Compose-based cluster, against the container you just upgraded:

   ```bash
   docker exec -it server-01 clickhouse-client --password admin123 --query "SELECT version();"
   ```
