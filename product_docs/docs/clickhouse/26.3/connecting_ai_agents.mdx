...
Restart `edb-clickhouse-mcp` after changing any of these variables, or any of the diagnostic tool settings in [Tuning the diagnostic tools](#tuning-the-diagnostic-tools), since it only reads them at startup. For the RPM, run `sudo systemctl restart edb-clickhouse-mcp`. For the container, edit the environment variables in your AI client's own configuration file instead, since the client launches the container itself each time it connects, then restart the client to apply them.
!!!
...
These steps cover the RPM, running as the packaged systemd service. Install the RPM instead if you're running the container image and need a remote server.

1. Set `CLICKHOUSE_MCP_SERVER_TRANSPORT` to `http` or `sse`, and `CLICKHOUSE_MCP_BIND_HOST=0.0.0.0`, since it otherwise defaults to `127.0.0.1` and only accepts connections from the same host, to run `edb-clickhouse-mcp` over a network transport instead of `stdio`.
