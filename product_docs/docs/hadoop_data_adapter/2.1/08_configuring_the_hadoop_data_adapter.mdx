...
| enable_join_pushdown | If true, pushes the join between two foreign tables from the same foreign server instead of fetching all the rows for both the tables and performing a join locally. This option can also be set for an individual table and if any of the tables involved in the join has set the option to false then the join is not be pushed down. The table-level value of the option takes precedence over the server-level option value. Default is true.       |
