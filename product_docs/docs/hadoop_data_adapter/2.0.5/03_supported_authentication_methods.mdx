...
Where *path\_to\_hive-site.xml\_file* specifies the complete path to the `hive‑site.xml` file.

When creating the user mapping, you must provide the name of a registered LDAP user and the corresponding password as options. For details, see [Create User Mapping](#create_user_mapping).
