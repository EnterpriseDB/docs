...
If you have an existing installation of Hadoop Foreign Data Wrapper that you installed using the EDB repository, you can use the `upgrade` command to update your repository configuration file and then upgrade to a more recent product version. To start the process, open a terminal window, assume superuser privileges, and enter the commands that apply to the operating system and package manager used for the installation:

To upgrade your repository configuration file:
...
To upgrade to the latest product version:

  |         Operating system         | Upgrade command|
  | -------------------------------- |
  | RHEL 8/9 and derivatives         |
  | RHEL 7 and derivatives, CentOS 7 |
  | SLES                             |
  | Debian and Ubuntu                |
