...
To upgrade to the latest product version, enter one of the following commands:
  ...
  | -------------------------------- | --------------------------------------------------------------------------------- |
  | RHEL 8/9 and derivatives         | `sudo <package-manager> upgrade edb-as<xx>-hdfs_fdw`                              |
  | RHEL 7 and derivatives, CentOS 7 | `sudo <package-manager> upgrade edb-as<xx>-hdfs_fdw edb-libmongoc-libs`           |
  | SLES                             | `sudo <package-manager> upgrade edb-as<xx>-hdfs_fdw`                              |
  | Debian and Ubuntu                | `sudo <package-manager> --only-upgrade install edb-as<xx>-hdfs-fdw edb-libmongoc` |
