...
Modify the configuration file, adding the `hdfs_fdw.jvmpath` parameter to the end of the configuration file and setting the value to specify the location of the Java virtual machine (`libjvm.so`). Set the value of `hdfs_fdw.classpath` to indicate the location of the Java class files used by the adapter. Use a colon (:) as a delimiter between each path. For example:
 ...
 hdfs_fdw.jvmpath='/usr/lib/jvm/java-1.17.0-openjdk-amd64/lib/server/'
 ```
...
| auth_type                 | The authentication type of the client. Specify `LDAP` or `NOSASL`. If you don't specify an `auth_type`, the data wrapper determines the value based on the user mapping: `LDAP` is used if the user mapping includes a user name and password, otherwise `NOSASL` is used. Important: If the server's authentication mode is `NOSASL`, the JDBC connection string only accepts the case-sensitive value `noSasl`. Therefore, you must specify auth_type as `'noSasl'` to ensure a valid connection.                              |
| ssl                       | Set this boolean option to true when SSL is enabled on the hive server. The default value is `false`.                                                                                                                                                                                                                                                                                                                                                                                                                |
| useSystemTrustStore       | Set this boolean option to true to use the host operating system's trust store. This is useful for environments where the necessary SSL certificates are already installed and trusted at the system level. The default is `false`.                                                                                                                                                                                                                                                                                            |
| sslTrustStore             | Use this option to specify the path to a custom certificate trust store file. It is useful when you have a specific certificate you need to trust.                                                                                                                                                                                                                                                                                                                                                                           |
| trustStorePassword        | If you specify a custom trust store file with `sslTrustStore`, use this option to provide its password. It is a required parameter when a password-protected trust store is used.                                                                                                                                                                                                                                                                                                                                          |
...
```sql
CREATE SERVER hdfs_server FOREIGN DATA WRAPPER hdfs_fdw OPTIONS (host '170.11.2.148', port '10000', client_type 'hiveserver2', auth_type 'LDAP', ssl 'true', useSystemTrustStore 'true', connect_timeout '10000', query_timeout '10000');
```

The foreign server uses the default port (10000) for the connection to the client on the Hadoop cluster. The connection uses an LDAP server with SSL and validates the server's certificate against the local system's trust store.
