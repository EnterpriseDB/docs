...
Disable GUC for LIMIT OFFSET pushdown queries at the session level: 
...
!!! Note
    You can also enable/disable LIMIT OFFSET pushdown at the server/table level using a GUC. For more information, see [CREATE SERVER](configuring/#create-server) and [CREATE FOREIGN TABLE](configuring/#create-foreign-table).
