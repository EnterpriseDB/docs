...
Query plan with disable GUC for LIMIT OFFSET pushdown queries at the session level (by default, LIMIT OFFSET pushdown is enabled):
...
Query plan with enable GUC for LIMIT OFFSET pushdown queries at the session level: 
