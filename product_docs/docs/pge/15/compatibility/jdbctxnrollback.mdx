...
multiple statements fails. The possible values are: `server`, `always`, `never`, `conservative`. 
...
- In
`autosave=always` mode, the JDBC driver first tries to use the server
...
## `autosave` test cases
Test cases for trying out various values of the `autosave` property
...
With `autosave=server`, the following query will insert values `(1), (3) and
...
in the `artifacts` directory and run the last two commands (step 2 and 3) to compile and run the test cases.
