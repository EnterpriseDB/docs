...
navigation:
 ...
 - jdbctxnroolback
---
EDB Postgres Extended Server has a number of features to make migrations from Oracle or SQL Server easier.
...
For more information, see [`transaction_rollback_scope` parameter](txnrollback) for information on setting the transaction rollback scope inside the database and [JDBC properties for rollback scope](jdbctxnrollback) for continuing past an ERROR on a JDBC batch job.
...
EDB Postgres Extended Server is designed to integrate with other migration tools with a number of PL/pgSQL compatibility features. 
