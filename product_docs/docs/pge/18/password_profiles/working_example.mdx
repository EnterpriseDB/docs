...
description: "This section provides a working example of using the password profile APIs."
---
...
- Create a user and `GRANT EXECUTE` privileges to the user for all APIs in the `password_profile` schema:
...
PASSWORD_PROFILE.PG_GET_ROLE_STATUS('my_role');

-- Lock the role's account
PASSWORD_PROFILE.PG_ROLE_ACCOUNT_LOCK('my_role', true);

-- Unlock the role's account
PASSWORD_PROFILE.PG_ROLE_ACCOUNT_LOCK('my_role', false);

-- Set the role's password to expire
PASSWORD_PROFILE.PG_ROLE_PASSWORD_EXPIRE('my_role');