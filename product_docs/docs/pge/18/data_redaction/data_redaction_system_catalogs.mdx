...
The `pg_redaction_column` system catalog stores information about the data redaction policy attached to the columns of a table.
...
!!! Note
    The described column is redacted if the redaction policy `data_redaction.pg_redaction_column.rcpolicyid` on the table is enabled and the redaction policy expression `data_redaction.pg_redaction_policy.rcfuncexpr` evaluates to `true`.
