---
navTitle: Upgrading EDB Postgres Extended Server 
title: Upgrading EDB Postgres Extended Server
description: Perform a version upgrade of EDB Postgres Extended Server. 
deepToc: true 
---
...
This worked example upgrade an EDB Postgres Extended Server 15 database, to EDB Postgres Extended Server 16. 
...
1.  Start the target server: 
    ...
    !!!note 
       You can also start the server with the logfile option enabled to print any possible error into a logfile: 
...
1.  Restart the target server: 
    ...
    ```
    /usr/lib/edb-pge/16/bin/pg_ctl -D /var/lib/edb-pge/16/upgrade-target star
    ```
