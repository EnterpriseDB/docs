...
If you installed an RPM package of EDB Postgres Extended Server (on RHEL, AlmaLinux, Rocky Linux) with `yum` or `dnf`, use `dnf` to perform a minor version upgrade of the packages.
