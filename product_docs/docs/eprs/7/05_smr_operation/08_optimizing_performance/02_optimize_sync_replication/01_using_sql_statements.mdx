...
!!!note
In multi-master replication clusters with three or more nodes, BUS mode is strictly enforced, regardless of any user-configured settings. 
!!!
...
!!!note
- The BUS (using simple Statement) mode is more optimal than BUP (using PreparedStatement) mode in common cases.

- Replication Server defaults to BUS (Simple Statement) mode for batch updates and avoids switching to BUP (PreparedStatement) mode at runtime.

- If a user intends to use BUP mode, they must explicitly configure two settings: set defaultBatchUpdateMode to BUP and set switchBatchUpdateMode = false to disable the automatic switch.
!!!
