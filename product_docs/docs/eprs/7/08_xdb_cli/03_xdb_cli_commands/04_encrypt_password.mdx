...
1.  Execute the `encrypt` command to produce a file named `pwdfile`.
    ...
    !!!note 
    If you reran the same encryption command with the same password input, the generated     encrypted password would be a different each time. 
    !!!
...
In this example we change the encryption algorith from `DES-MD5` to `AES-SHA256`.
