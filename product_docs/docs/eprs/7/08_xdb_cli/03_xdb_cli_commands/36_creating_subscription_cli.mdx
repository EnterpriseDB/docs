...
Enter the column mappings between different but compatible columns in the source and target database. Table and schema qualified column names can be used in mappings, or a regular expression can be used to implement a mapping rule on multiple columns. 
...
Enter the path of the file containing the column mappings. Multiple custom type mappings can also be provided in the file. You can use either the fully qualify column name or regression expression. Enter each mapping on a new line. 
