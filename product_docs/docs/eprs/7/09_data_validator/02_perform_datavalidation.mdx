...
!!! Note
   Data Validator doesn't support column mapping. For custom column mapping, the source and target databases can have different data types so Data Validator can't validate the data when column mapping is applied.
!!!
