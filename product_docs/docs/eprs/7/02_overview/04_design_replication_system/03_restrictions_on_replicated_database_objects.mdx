...
As part of the re-snapshot process, when Replication Server drops constraints from the table’s column that are the part of view objects using `CASCADE` option, the views might also get dropped. Here is a simplified example of the limitation:
...
In this case, since Replication Server drops the constraints using CASCADE the view is dropped too.

The database behaves in this manner because the view query doesn't work if the primary key is dropped on the table `t_place_type`. It produces this error:
...
In the [PostgreSQL](https://www.postgresql.org/docs/15/sql-select.html#SQL-GROUPBY) documentation it states, "A functional dependency exists if the grouped columns (or a subset thereof) are the primary key of the table containing the ungrouped column."