...
Since Replication Server drops the constraints using the `CASCADE`, the above error would not occur; however, the result of using this option means that the view will also be dropped.
