...
-   Materialized views can be members of snapshot-only publications from Oracle to EDB Postgres Advanced Server databases. The materialized views are replicated as tables. 
    ...
    1. We recommend that useres create the views with auto-refresh in Oracle. If the views are not auto-refresh they are marked as invalid upon updating the base table until refreshed explicitly. EDB Replication Server currently does not support subscription creation or snapshot for invalid views.
       ...
       !!! Note
           Snapshot-only replication for views is currently tested and certified only for Oracle to EDB Postgres Advanced Server databases.
