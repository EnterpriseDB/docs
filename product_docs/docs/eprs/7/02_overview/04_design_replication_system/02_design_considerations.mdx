...
-   Materialized views can be members of snapshot-only publications from Oracle to EDB Postgres Advanced Server databases. The materialized views are replicated as tables. Do the following before using snapshot-only replication for materialized views:
