...
EPRS 7.11 adds support for logical replication slot synchronization between primary and standby databases on Postgres 17.x and later database versions.
...
![Logical replication slots with publication database standby](/eprs/images/logical_rep_standby.svg)
...
1. To enable logical replication slot synchronization, add the following line into the `xdb_pubserver.conf` file before setting up a logical replication cluster.

   ```
   replicationSlotFailover= true
   ```

1. To configure primary and standby publication database servers in a physical streaming replication configuration with slot synchronization, you must configure several database level settings. [Logical Decoding Concepts](https://www.postgresql.org/docs/17/logicaldecoding-explanation.html#LOGICALDECODING-REPLICATION-SLOTS-SYNCHRONIZATION) collects the required settings to support this. Afte you configure the database to support logical slot synchronization between primary and standby server, EPRS will create replication slots with the failover flag set to true.

1. EDB recommends using virtual IP for Logical replication slot synchronisation. EPRS stores the IP address of thw primary node and, on promotion, the promoted primary must be mapped to the same IP and port. That allows EPRS to connect to the new primary using the same host details. If virtual IP or a similar IP address management tool is not used, manual steps are required after the promotion where you update the details of the new primary in EPRS control tables. You will also have to restart the service.

1. For existing setups where replication slot is already created, you must recreate the replication slot with this feature enabled by setting up the cluster again.
