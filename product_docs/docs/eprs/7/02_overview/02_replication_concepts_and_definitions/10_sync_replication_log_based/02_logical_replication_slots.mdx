...
EPRS supports logical replication slot synchronization between primary and standby databases. When enabled, the logical replication slot is replicated to the standby. If the primary fails, the standby can take over, allowing for replication to continue in failover scenarios as the slot is already there.

![Logical replication slots with publication database standby](/eprs/latest/images/logical_rep_standby_alternative_700w.svg)
