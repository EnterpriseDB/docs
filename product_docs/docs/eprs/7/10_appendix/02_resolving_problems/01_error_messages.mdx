...
1.  Modify the following parameters in the  `postgresql.conf` file of the subscription database server: 
...
1.  Reload the configuration to make the changes effective: 

    ```sql
    select pg_reload_conf();
    ```

1.  Ensure you also enable the `tcpKeepAlive=true` setting for connections to the affected subscription database. You can do this using the EPRS console or CLI: 

    - **Console:** Provide the string `tcpKeepAlive=true` in the **URL Options** field while [Adding a database](/eprs/latest/05_smr_operation/02_creating_publication/02_adding_pub_database) or Updating a databases configuration.
    
    - ** CLI:** Use the [`-urlOptions` flag](/eprs/latest/07_common_operations/11_using_ssl_connections/#enabling-keepalive-configuration-for-database-connections) to specify the `tcpKeepAlive=true` parameter when running the `addsubdb` command, for example `-urloptions "tcpKeepAlive=true"`.
...
**Resolution for EPRS versions 7.11 and earlier** 
...
To work around this issue in EPRS versions 7.11 and earlier, update the `runPubserver` script to ensure that the `/lib/jdbc` folder contents, including `orai18n.jar`, are included in the Java classpath.
...
**Resolution for EPRS versions 7.12 and later** 

Occurs when [adding an Oracle Publication Database](/eprs/latest/05_smr_operation/01_prerequisites/04_preparing_pub_database/#oracle-publication-database) and Oracle is installed using the JA16SJIS character set encoding. 

To work around this issue in EPRS versions 7.12 and later, ensure that the `orai18n.jar` file is placed in the following location: 

```
XDB_HOME/lib/jdbc
```

This ensures that the necessary character set support is available in the classpath, preventing the `ORA-17056` error.
