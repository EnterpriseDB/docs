...
Ensure you also enable the `tcpKeepAlive=true` setting for the affected subscription database. You can do this via the Replication Server console or CLI: 

- **Console:** Provide the string `tcpKeepAlive=true` in the **URL Options** field while [Adding a database](/05_smr_operation/02_creating_publication/02_adding_pub_database) or Updating a databases configuration.

- ** CLI:** Use the [`-urlOptions` flag](/07_common_operations/11_using_ssl_connections/#setting-non-default-paths-using-ssl-connection-parameters) to specify the `tcpKeepAlive=true` parameter when running the `addsubdb` command, for example `-urloptions "tcpKeepAlive=true"`.
