...
!!! Note
    The cleanup of certain processed rows in the shadow tables might be delayed beyond the next scheduled cleanup. When Oracle and SQL Server are the publication database on clusters created on versions 7.5 and earlier, the cleanup delay can be there due to uncommitted changes. These rows are eventually removed in later cleanup events. For new clusters created on version 7.6 and later, the shadow cleanup operation is accurate and the delayed cleanup issue is fixed.
