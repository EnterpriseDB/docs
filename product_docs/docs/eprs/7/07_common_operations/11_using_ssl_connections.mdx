...
There are two options available when entering the value for the common name field: the database user name and host name. When configuring the host name as the common name value, follow the specified steps as outlined below. 
    - Create root Certificate Authority (CA) private key (`root.key`) and root CA certificate (`root.crt`) files using the example below. Enter  
    - 
