...
If you generated the server certificate with `CN=hostname/ip-address`, use the `root.crt file` presets from the database data directory in the following command to make `xdb_root.crt` file:
