...
1.  Select the scheduled replication frequency, or select **Cron Expression** to write your own cron expression. The frequency choices have the following meanings:
    ...
    -   **Cron Expression**. Provides additional flexibility for specifying a schedule. See [Writing a cron expression](../10_appendix/03_miscellaneous_xdb_processing_topics/03_writing_cron_exp/#writing_cron_exp) for details.
