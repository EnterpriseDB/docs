...
If you installed Replication Server from the RPM package, you can uninstall any Replication Server component by invoking the `yum remove package_name` command as the root account and where  `package_name` is `edb-xdb-console`, `edb-xdb-publisher`, or `edb-xdb-subscriber`.
