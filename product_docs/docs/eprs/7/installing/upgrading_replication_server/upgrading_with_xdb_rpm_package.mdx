...
redirects:
- /docs/eprs/latest/10_appendix/02_upgrading_to_xdb6_2/03_upgrading_with_xdb_rpm_package
...
1.  Complete the publication server and subscription server configuration file setup.
    ...
    The final set of active configuration files must be contained in directory `/usr/edb/xdb/etc` named `xdb_pubserver.conf` and `xdb_subserver.conf`. In the `/usr/edb/xdb/etc/sysconfig directory`, make sure the Replication Server startup configuration file `xdbReplicationServer-70.config` contains the parameter settings you want to use with Replication Server 7. See [Replication Server configuration file](../../02_overview/03_replication_server_components_and_architecture/01_physical_components/#xdb_replication_conf_file) for information on this file.
