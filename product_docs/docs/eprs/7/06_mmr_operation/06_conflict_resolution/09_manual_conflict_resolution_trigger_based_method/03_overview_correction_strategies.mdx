...
Update the shadow table entries for the conflicting transactions in the primary nodes to indicate that the conflict was resolved as in Step 2 of [Manual publication table correction](#manual_publication_table_correction).
...
In the control schema of the publication database currently designated as the controller database, modify the entries in the `xdb_conflicts` table to indicate the conflict was resolved as in Step 3 of [Manual publication table correction](#manual_publication_table_correction).
