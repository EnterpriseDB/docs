...
An update/update conflict on the primary key column might not resolve consistently according to the selected resolution option. That is, the column values might differ for the same row across multiple primary nodes after the synchronization replication.
