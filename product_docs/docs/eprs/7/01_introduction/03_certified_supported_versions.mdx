...
!!!Note
All PostgreSQL and EDB Postgres Advanced Server versions available as single-node and primary/standby high-availability cluster types (on EDB Postgres® AI Cloud Services, formerly BigAnimal) are also supported for SMR configurations. See the (managed database cloud service) [documentation](/biganimal/latest) for more information about [supported cluster types](/biganimal/latest/overview/02_high_availability/). See the [database version policy documentation](/biganimal/latest/overview/05_database_version_policy/) for the versions of PostgreSQL and EDB Postgres Advanced Server available in EDB Postgres® AI Cloud Services.
