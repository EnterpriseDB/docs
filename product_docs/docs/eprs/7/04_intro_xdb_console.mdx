...
If you choose to save the login information, the server’s network location (IP address and port number), admin user name, and password are stored in a server login file in a hidden location under the home directory of the operating system account with which you opened the Replication Server console. See [Installation details](/eprs/latest/03_installation/installation_details) for the location of this file.
