...
>  `force` option removes the publication even when one or more subscriptions are defined against the publication).
...
```text
A publication named pub2 is removed with the `-force` option
