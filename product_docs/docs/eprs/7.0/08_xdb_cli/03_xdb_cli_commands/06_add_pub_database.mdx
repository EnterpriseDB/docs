...
**For MMR only:** Applies to non-PDN nodes. Comma-separated list of filter IDs identifying the filter rules from the set of available table filters to enable on the corresponding tables in the new primary node. Use the `printpubfilterslist` command to get the filter IDs for the available filter rules in the publication (see [Printing a list of filters in a publication](17_print_publications_filters_list/#print_publications_filters_list)). Do not use any white space between the comma and filter IDs.
...
**For MMR only:** Applies to non-PDN nodes. Specify this option if you want an initial snapshot replication performed when creating the primary node. Omit this option if you don't want an initial snapshot replication  performed when creating the primary node.
