...
> The `-tables` option  includes tables from the tables list to the publication. The `-alltables` option allows to include all tables of the database or only tables from the listed schemas. 
...
> **For MMR only:** Default standby conflict resolution (E, L, N, or M), if the `-defaultconflictresolution` option is omitted. If `-defaultstandbyconflictresolution`  option is omitted, then default is `M`.
