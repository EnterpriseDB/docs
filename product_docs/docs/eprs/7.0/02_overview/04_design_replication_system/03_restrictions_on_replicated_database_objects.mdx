...
- Replication Server 7.0 provides support for LOB column streaming from Oracle to PostgreSQL/EDB Postgres Advanced Server. However, LOB columns must be created to allow NULLs on the source Oracle and target PostreSQL/EDB Postgres Advanced Server database server. The previous version of Replication Server did not stream the LOB columns. Rows of tables containing LOB columns were replicated during streaming, but LOB columns contained NULL. This limitation is now removed.
...
- If JDBC LOB APIs (provided by `java.sql.Clob` or `java.sql.Blob` classes) are used, triggers will not get fired, and hence streaming for LOB columns will not work.

- Similarly, if functions/procedures provided by the `DBMS_LOB` package are used, triggers will not get fired, and hence streaming for LOB columns will not work.
...
PostgreSQL or EDB Postgres Advanced Server database tables that include the following data type cannot be replicated to the Oracle database:
...
Columns with following data types cannot be added to a filter for PostgreSQL or EDB Postgres Advanced Server database:
