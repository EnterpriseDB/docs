...
-   In a multi-master replication system, don't set non-PDN nodes tables `REPLICA IDENTITY` options to `FULL` unless transactions are expected to be targeted on those non-PDN nodes. In addition, the transactions will be filtered when they're replicated to the other primary nodes.
