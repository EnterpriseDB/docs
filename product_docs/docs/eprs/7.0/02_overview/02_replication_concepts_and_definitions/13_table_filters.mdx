...
While simple conditions with comparison and logical operators (=, >, <, LIKE, etc.) are allowed as filters for all use cases, there is one use case when a more complex condition can be used as a filter. A subquery can be used in establishing an inclusion/exclusion list of IDs, if a single column is returned from the subquery. This is only supported for trigger-based SMR use case. Here are the patterns for this type of queries:
...
For example, if we have a tables `dept` and `dept_filter_lookup` created with:
