...
The cache size corresponds to the heap size configured for the publication server by the `-Xmxnnnm` setting of the `JAVA_HEAP_SIZE` parameter in the EPRS Startup Configuration file. See [EPRS Replication Configuration File](../../03_replication_server_components_and_architecture/01_physical_components/#xdb_replication_conf_file) for information on the EPRS Startup Configuration file.
