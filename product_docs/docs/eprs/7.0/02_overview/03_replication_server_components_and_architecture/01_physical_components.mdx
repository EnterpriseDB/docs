...
-   When a publication server or subscription server is registered during the process of creating a replication system. Registration of a publication server or subscription server is done using the EPRS Replication Console or the EPRS Replication Server Command Line Interface.
...
| `admin_user`     | EPRS administrator user name (the admin user name) for registering a publication server or a subscription server on this host containing the EPRS Replication Configuration file |
...
!!! Note
    The passwords for the admin user name and the controller database user name are encrypted. Should you change either of these passwords, you must modify the corresponding password parameters in the EPRS Replication Configuration file to contain the encrypted form of the new password. See [Encrypting the Password in the xDB Replication Configuration File](../../10_appendix/04_miscellaneous_xdb_processing_topics/02_encrypt_password_inconf_file/#encrypt_password_in%20conf_file) for directions on how to generate the encrypted form of a password.
...
The EPRS Startup Configuration file contains installation and configuration information primarily used by the Java Runtime Environment (JRE) when any publication server or subscription server is started up on the host containing the file.

The content of the file is created by the Replication Server installer when you install Replication Server.

The following is an example of the content of an EPRS Startup Configuration file:
...
The following table contains a brief description of the parameters in the EPRS Startup Configuration file.

**Table 2-3: EPRS Startup Configuration File**
...
After making any modifications to the EPRS Startup Configuration file, the publication server and subscription server must be restarted.

See [Post-Installation Host Environment](../../03_installation/05_post_installation_host_environment/#post_installation_host_environment) for the file system location of the EPRS Startup Configuration file.
