...
Whenever a primary node is added to a multi-master replication system, the publication server creates database objects in the control schema of the primary node for recording metadata. For non-PDN nodes, the publication server also calls Migration Toolkit to create the publication table definitions if you choose this option when creating the primary node.
