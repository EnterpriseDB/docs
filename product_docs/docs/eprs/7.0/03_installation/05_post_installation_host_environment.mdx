...
On Linux hosts where you installed Replication Server with the graphical user interface or from the command line, you should now have a publication server daemon and a subscription server daemon running on your computer assuming you chose to install the publication server and subscription server components. If you installed the EPRS RPM package, you must start the publication server and the subscription server based upon the instructions in Section [Registering a Publication Server](../05_smr_operation/02_creating_publication/01_registering_publication_server/#registering_publication_server) for the publication server and [Registering a Subscription Server](../05_smr_operation/03_creating_subscription/01_registering_subscription_server/#registering_subscription_server) for the subscription server. On Windows systems, the publication server and subscription server run as services named `Publication Service` and `Subscription Service`.
...
!!! Note
    For Replication Server installed from an EPRS RPM package, the EPRS Replication Console is started by invoking the script `XDB_HOME/bin/runRepConsole.sh`.
...
| `xdbReplicationServer-xx.config`   | `XDB_HOME/etc/sysconfig`              | EPRS Startup Configuration file                                                       |
...
!!! Note
    `XDB_HOME` is the directory where Replication Server is installed. For Linux EPRS is installed in the `/usr/edb/xdb` directory.
...
!!! Note
    The Replication Server version number is represented by `x.x` or by `xx` (for example `6.2` or `62`).
