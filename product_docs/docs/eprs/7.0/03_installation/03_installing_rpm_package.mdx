...
If you are upgrading from a 6.2.x version of Replication Server that was installed using the Replication Server Bitrock installer, perform the following steps:

1. Stop the Replication Server Publication and Subscription services.

1. Take a backup of the following file and folder:
   - `/etc/edb-repl.conf`
   - `/opt/PostgreSQL/EnterpriseDB-xDBReplicationServer/xdata`

1. If you have modified any of the following configuration files in the `/opt/PostgreSQL/EnterpriseDB-xDBReplicationServer/etc` folder, take a backup of the modified files:
   - `encoding_map.conf`
   - `euc_jp.conf` 
   - `sysconfig` 
   - `xdb_pubserver.conf` 
...
1. Uninstall Replication Server 6.2.x using the following command:
...
6. After you have completed the installation steps, stop any running Publication and Subscription services.
