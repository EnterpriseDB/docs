...
**Step 2:** Register the subscription server. Open the EPRS Replication Console from the system’s application menu. For Replication Server installed from an EPRS RPM package, the EPRS Replication Console is started by invoking the script `XDB_HOME/bin/runRepConsole.sh`.
...
-   `User Name.` Admin user name that is used to authenticate your usage of this subscription server. This is the user name you specified on the EPRS Admin User Details screen in Step 15 of [Installing With Stack Builder or StackBuilder Plus](../../03_installation/01_installing_with_stackbuilder/#installing_with_stackbuilder).
