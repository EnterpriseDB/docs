...
!!! Note
    For Oracle 12c pluggable database: You must create and grant privileges for a local user while connected to the pluggable database to use as the publication database. Create a common user in the Oracle 12c root container `CDB$ROOT`. Grant privileges to the common user while connected to the pluggable database to  use as the publication database.

!!! Note
    For Oracle 12c non-container database: Create and grant privileges to the publication database user in the same manner as for Oracle versions prior to 12c.
