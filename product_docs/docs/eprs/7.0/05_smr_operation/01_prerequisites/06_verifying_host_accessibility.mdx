...
When you install Replication Server, the port numbers you specify for the publication server and the subscription server are stored in the EPRS Startup Configuration file as shown by the following example. See [EPRS Startup Configuration File](../../02_overview/03_replication_server_components_and_architecture/01_physical_components/#xdb_startup_conf_file) for information on the EPRS Startup Configuration file.
...
If you want to use different port numbers, modify the PUBPORT and SUBPORT entries in the EPRS Startup Configuration file and restart the publication server and subscription server.
