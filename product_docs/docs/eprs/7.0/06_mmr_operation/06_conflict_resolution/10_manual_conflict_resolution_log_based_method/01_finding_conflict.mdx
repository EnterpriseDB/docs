...
Conflicts can be found using the `Conflict History` tab as described in Section [Viewing Conflict History](../../07_view_conflict_history/#view_conflict_history). In the Conflict History tab, click the `Refresh` button to reveal all of the latest conflicts.
