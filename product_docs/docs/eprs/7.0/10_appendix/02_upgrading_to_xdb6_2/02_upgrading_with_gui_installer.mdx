...
Perform the following steps to upgrade to Replication Server 6.2 using the graphical user interface installer.
...
**Step 3:** Install Replication Server 6.2. See Chapter [Installation and Uninstallation](../../05_smr_operation/03_creating_subscription/01_registering_subscription_server/#registering_subscription_server) for instructions on installing Replication Server, but note the differences described in the following steps.
...
In the `XDB_HOME/etc` directory, a new set of configuration files for Replication Server version 6.2 are created. These files are named `xdb_pubserver.conf.new` and `xdb_subserver.conf.new`. The new configuration files contain any new configuration options added for Replication Server 6.2.
...
Merge the old and new configuration files so that the resulting, active configuration files contain any new Replication Server 6.2 configuration options as well as any non-default settings you used with Replication Server 6.1.x and wish to continue to use with Replication Server 6.2. The final set of active configuration files must be named `xdb_pubserver.conf` and `xdb_subserver.conf`.

In the `XDB_HOME/etc/sysconfig` directory, make sure the EPRS Startup Configuration file `xdbReplicationServer-62.config` contains the parameter settings you wish to use with Replication Server 6.2. See [EPRS Startup Configuration File](../../02_overview/03_replication_server_components_and_architecture/01_physical_components/#xdb_startup_conf_file) for information on the EPRS Startup Configuration file.
...
**Step 13:** You are now ready to use Replication Server 6.2 to create new replication systems and manage existing ones.
