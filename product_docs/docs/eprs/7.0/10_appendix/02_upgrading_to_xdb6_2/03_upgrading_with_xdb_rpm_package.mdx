...
If you are using Replication Server 6.1.x that was installed using the EPRS RPM package, upgrading to Replication Server 6.2 from an RPM package is accomplished as described in this section.

!!! Note
    Be sure the repository configuration file `edb.repo` for Replication Server 6.2 is set up in the `/etc/yum.repos.d` directory. See Section [Installing the EPRS RPM Package](../../03_installation/03_installing_rpm_package/#installing_rpm_package) for information.
...
If on the other hand you perform option 2, you must copy the Oracle JDBC driver to the `/usr/edb/xdb/lib/jdbc` directory after you have installed Replication Server 6.2.
...
At this point the publication server and the subscription server for Replication Server 6.2 are not running. The directories now contain the following:

-   Replication Server 6.2 is installed in directory location `/usr/ppas-xdb-6.2`.
...
-   In the /etc directory, there may be either one or two EPRS Replication Configuration files named `edb-repl.conf` and possibly `edb-repl.conf.rpmsave`. The file `edb-repl.conf` should contain the connection and authentication information for the controller database used by the EPRS 6.1.x publication server. The file `edb-repl.conf.rpmsave` contains only the new administrator user parameters admin_user and admin_password. Before starting the publication server and subscription server, be sure the controller database is up and running, and the `edb-repl.conf` file contains the controller database connection and authentication parameters.

**Step 7:** Complete the publication server and subscription server configuration file setup.

In the `/usr/ppas-xdb-6.2/etc` directory, a new set of configuration files for Replication Server version 6.2 are created. These files are named `xdb_pubserver.conf` and `xdb_subserver.conf`. The new configuration files contain any new configuration options added for Replication Server 6.2. The old configuration files used by Replication Server version 6.1.x might be found in the /usr/ppas-xdb-6.1/etc directory renamed as `xdb_pubserver.conf.rpmsave` and `xdb_subserver.conf.rpmsave`.

!!! Note
    If these files do not exist, use the ones you saved in Step 3.

Merge the old and new configuration files so that the resulting, active configuration files contain any new Replication Server 6.2 configuration options as well as any non-default settings you used with Replication Server 6.1.x and wish to continue to use with Replication Server 6.2.

The final set of active configuration files must be contained in directory `/usr/ppas-xdb-6.2/etc` named `xdb_pubserver.conf` and `xdb_subserver.conf`. In the `/usr/ppas-xdb-6.2/etc/sysconfig directory`, make sure the EPRS Startup Configuration file `xdbReplicationServer-62.config` contains the parameter settings you wish to use with Replication Server 6.2. See [EPRS Replication Configuration File](../../02_overview/03_replication_server_components_and_architecture/01_physical_components/#xdb_replication_conf_file) for information on the EPRS Startup Configuration file.
...
**Step 11:** You are now ready to use Replication Server 6.2 to create new replication systems and manage existing ones.
