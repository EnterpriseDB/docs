...
If on the other hand you perform option 2, you must copy the Oracle JDBC driver to the `/usr/edb/xdb/lib/jdbc` directory after you have installed xDB Replication Server 6.2.
...
**Step 6:** As the root account invoke the yum update command to begin the upgrade from xDB Replication Server 6.1.x to xDB Replication Server 7.0 as shown by the following:

`yum update edb-xdb*`

Be sure to include the asterisk character (\*) following edb-xdb in order to update all xDB Replication Server components.
