...
With replica identity `NOTHING` no `WAL` records are produced, so Log-based transactional replication is not possible. Change the re[;ica identity of this table and provide Primary Key and no Unique Columns.
