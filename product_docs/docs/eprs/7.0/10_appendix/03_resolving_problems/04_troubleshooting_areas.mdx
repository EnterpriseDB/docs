...
If errors are encountered regarding the Java Runtime Environment such as the Java program cannot be found or Java heap space errors, check the parameters set in the EPRS Startup Configuration file `xdbReplicationServer-xx.config`. See [EPRS Replication Configuration File](../../02_overview/03_replication_server_components_and_architecture/01_physical_components/#xdb_replication_conf_file) for information on the EPRS Startup Configuration file.

The following is an example of the content of the EPRS Startup Configuration file:
...
If you make any changes to the parameters in the EPRS Startup Configuration file, be sure to restart the publication server and subscription server after making the modifications.
