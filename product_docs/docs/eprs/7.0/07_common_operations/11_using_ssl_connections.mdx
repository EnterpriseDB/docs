...
**Step 1:** Using files `server.crt` and `server.key` located under the Postgres database server data subdirectory, create copies of these files. Move them to the host where the publication server and subscription server are running.
...
!!! Note
    Use the following commands to change the permission of the certificates in `${user.home}/.postgresql`:
