...
!!! Note
    SSL connections are not used from the EPRS Replication Console or the Replication Server Command Line Interface. The EPRS user interfaces communicate with the publication server and subscription server, which in turn connect to the publication/subscription databases or primary nodes.
...
In the case of WAL stream changeset logging, while adding a publication or a subscription database that accepts only ssl connection, EPRS validates if the database server is configured for logical replication using `libpq` connection.
