...
- If you are using PostgreSQL, on the SSL-enabled Postgres database server:
  ...
  The default location of these files is `{user.home}/.postgresql(e.g/var/lib/pgsql/.postgresql/)`. The file location can be overridden using SSL connection parameters or Postgres SSL environmental variables, see [tbd] for more information.
