...
-   Although the Postgres `ALTER TABLE` statement allows multiple actions per statement, the EPRS DDL change replication feature permits only one action per ALTER TABLE statement.
