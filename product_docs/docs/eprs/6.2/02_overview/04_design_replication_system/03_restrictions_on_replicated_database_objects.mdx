...
-    `ENUM`, `ARRAY`, composite type, range data type (such as `int4range`, `tstzrange`, `numrange`, `daterange`)
...
-   `ACLITEM`
-   `CHKPASS`
-   Geometric data types (such as `CIRCLE`, `BOX`, `LINE`, `PATH`, `POLYGON`) grouped together
-   `CIRCLE`
-   `CUBE`
-   `JSON`
-   `ROWID`
