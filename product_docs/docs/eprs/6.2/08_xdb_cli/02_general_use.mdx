...
Command names and parameter names are all case sensitive and must be given as shown in Section [Getting Help](../08_xdb_cli/03_xdb_cli_commands/#get_help).
...
If you execute the xDB Replication Server CLI with the help command, xDB Replication Server CLI will list a syntax summary of all commands. See [Getting Help](../08_xdb_cli/03_xdb_cli_commands/#get_help) for details on the help command.
...
See [Encrypting Passwords](../08_xdb_cli/03_xdb_cli_commands/#encrypt_password) for directions on generating an encrypted password using the encrypt command.
...
**For Linux only:** The environment variable, $?, contains the exit status. The following example shows the 0 exit status upon the successful execution of the `addpubdb` command contained in the `addpubdb_advsvr` parameter file described in Section [Running xDB Replication Server CLI Using a Parameter File](../08_xdb_cli/#running_xdb_cli_using_para_file).
