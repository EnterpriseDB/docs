...
> Filter ID identifying the filter rule for which the filter clause is to be changed. Use the `printpubfilterslist` command to obtain the filter IDs for the available filter rules in the publication (see [Printing a List of Filters in a Publication](../../08_xdb_cli/03_xdb_cli_commands/#print_publications_filters_list)).
...
The filter clause with filter ID 26 in publication analysts\_managers is modified.
