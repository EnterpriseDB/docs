...
See [Adding a Publication](../../05_smr_operation/02_creating_publication/#adding_publication) for additional information on creating a publication for a single-master replication system. See [Adding a Publication](../../06_mmr_operation/#add_pub_mmr) for a multi-master replication system.
...
> The filter clause to be applied to the table in the tables parameter list at the position indicated by ordinal\_tn.
...
> **For SMR only:** The filter clause to be applied to the view in the views parameter list at the position indicated by ordinal\_vn.
...
In the following example, a publication named dept\_emp is created that contains the EDB.DEPT and EDB.EMP tables of an Oracle database. The replication method is synchronization.
...
In the following example, a publication named analysts\_managers is created that contains the `edb.dept` table and employees from the `edb.emp` table who are analysts or managers. The tables are in an Advanced Server database. The replication method is snapshot-only.
