...
See [Removing Tables from a Publication](../../07_common_operations/06_managing_publication/#remove_tables_from_pub) for additional information on removing tables from a publication.
...
In the following example, table `edb.jobhist` and view `edb.salesemp` are removed from the analysts\_managers publication.
