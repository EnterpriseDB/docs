...
The xDB Replication Server is installed in directory location `/usr/edb-xdb-x.x` where `x.x` is the xDB Replication Server version number as shown by the following:
