...
| `edb-xdbpubserver.service` (Linux) | `/usr/lib/systemd/system`             | Start, stop, or restart the publication server (CentOS 7, RHEL 7, Rocky Linux 8, RHEL 8)  |
| `edb-xdbsubserver` (Linux)         | `/etc/init.d`                         | Start, stop, or restart the subscription server                                      |
| `edb-xdbsubserver.service` (Linux) | `/usr/lib/systemd/system`             | Start, stop, or restart the subscription server (CentOS 7, RHEL 7, Rocky Linux 8, RHEL 8) |
