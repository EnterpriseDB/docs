...
Set `enablePerformanceStats` option to true only if you need to conduct performance testing and analyze the replication statistics. When enabled, the publication server creates additional triggers on the publication tables in each primary node. The triggers produce transaction statistics that are recorded in the MMR\_transaction\_history table in the control schema. This option should be disabled in a production environment to avoid performance overhead.
