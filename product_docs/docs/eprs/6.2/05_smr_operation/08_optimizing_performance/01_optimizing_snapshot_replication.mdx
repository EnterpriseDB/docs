...
When the `copyViaDBLinkOra` option is set to true, the Oracle database link API, dblink\_ora, is used instead of JDBC COPY to populate Advanced Server subscription tables from an Oracle publication during snapshot replication.
