...
-   The tables named according to the convention `RRST_schema_table` from the SELECT statement on user\_tables are found only for synchronization publications. In this example, these tables are `RRST_EDB_DEPT` and `RRST_EDB_EMP`.
...
The control schema objects contained in \_edb\_replicator\_pub are shown by the following:
...
The control schema objects contained in \_edb\_replicator\_sub are shown by the following:
