...
-   `User.` The publication database user name created in Step 1 of [Preparing the Publication Database](../../05_smr_operation/01_prerequisites/#preparing_pub_database).
...
-   `URL Options (For SSL connectivity).` Enter the URL options to establish SSL connectivity to the publication database. See <span class="title-ref">Using Secure Sockets Layer (SSL) Connections &lt;using\_ssl\_connections&gt;</span> for information on using SSL connections.
