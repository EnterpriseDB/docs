...
-   `max_replication_slots`. Specifies the maximum number of replication slots. Set at minimum, to the number of SMR publication databases on this database server that will use the log-based method. In addition, if MMR primary nodes are to run on this database server with the log-based method, see [Replication Origin](../../02_overview/02_replication_concepts_and_definitions/10_sync_replication_log_based/#replication_origin) for information on the additional number of replication slots required.
