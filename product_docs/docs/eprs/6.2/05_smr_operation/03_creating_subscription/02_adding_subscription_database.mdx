...
-   `URL Options (For SSL connectivity)`. Enter the URL options to establish SSL connectivity to the subscription database. See <span class="title-ref">Using Secure Sockets Layer (SSL) Connections &lt;using_ssl_connections></span> for information on using SSL connections.
