...
When you register a subscription server in the xDB Replication Console, you may choose to save the subscription server’s network location (IP address and port number), admin user name, and encrypted password in a server login file on the computer on which you are running the xDB Replication Console. See [Saving Server Login Information](../../#saving_server_login_info) for information on saving the login information.
