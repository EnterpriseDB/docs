...
-   `Custom.` Custom conflict handling applies to update/update conflicts only. You must supply a PL/pgSQL program to resolve any conflicts that occur resulting from an update/update conflict. See [Custom Conflict Handling](../../06_mmr_operation/06_conflict_resolution/08_custom_conflict_handling/#custom_conflict_handling) for information on using custom conflict handling.
...
Once selected, the conflict resolution strategy for a given table can later be changed to a different strategy (see [Updating the Conflict Resolution Options](../../06_mmr_operation/#update_conflict_resolution_options)).
