...
On `MMRnode_b` and `MMRnode_c`, the commands to create different settings for the configuration parameter cluster.unique\_db\_id are run as well as the commands to create the sequence and the functions.
