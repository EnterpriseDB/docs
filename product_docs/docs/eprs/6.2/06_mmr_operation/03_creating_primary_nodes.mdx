...
-   `URL Options (For SSL connectivity).` Enter the URL options to establish SSL connectivity to the primary node. See <span class="title-ref">Preparing Using Secure Sockets Layer (SSL) Connections &lt;using_ssl_connections></span> for information on using SSL connections.
