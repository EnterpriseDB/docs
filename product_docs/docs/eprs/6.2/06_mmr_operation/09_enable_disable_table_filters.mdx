...
Table filters must first be defined in a set of available table filters in the publication before they can be enabled on a primary node. See Section [Adding a Publication](../06_mmr_operation/#add_pub_mmr) for information on defining table filters in a multi-master replication system.
...
**Step 1:** Make sure the publication server whose node is the parent of the primary nodes of the replication system is running and has been registered in the xDB Replication Console you are using. See Section [Registering a Publication Server](../05_smr_operation/02_creating_publication/#registering_publication_server) for directions on starting and registering a publication server.
...
A snapshot ensures that the content of the primary node tables is consistent with the updated filtering criteria. See Section [Performing Snapshot Replication](../06_mmr_operation/#perform_snapshot_replication_mmr) for information on performing a snapshot replication.
