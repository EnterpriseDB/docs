...
6.  Invoke the platform-specific command to install `PostGIS`:
    ...
    On RHEL 7, to install the latest version of PostGIS 3.2 for EDB Advanced Server version 14.0:

    ```text
    yum -y install edb-as14-postgis32
    ```

    On RHEL 8, to install the latest version of PostGIS for EDB Advanced Server version 14.0:

    ```text
    dnf -y install edb-as14-postgis32
    ```

    On RHEL 7, to install the latest version of PostGIS 3.1 for EDB Advanced Server version older than 13.0:
    ...
    On RHEL 7, to install PostGIS older versions (for example PostGIS 3.0.2) for EDB Advanced Server version 14.0:

    ```text
    yum -y install edb-as14-postgis3-3.0.2
    ```

    On RHEL 7, to install older PostGIS versions for EDB Advanced Server version older than 14.0:
