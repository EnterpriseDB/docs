---
title: "Installing PostGIS on RHEL 7 IBM Power (ppc64le)"
navTitle: "RHEL 7"
---
...
1. Enable the codeready-builder-for-rhel-7-\*-rpms repository since EPEL packages may depend on packages from it:

   ```shell
   ARCH=$( /bin/arch )
   subscription-manager repos --enable "codeready-builder-for-rhel-7-${ARCH}-rpms"
   ```
...
```shell
dnf -y install edb-as<xx>-postgis32
```
