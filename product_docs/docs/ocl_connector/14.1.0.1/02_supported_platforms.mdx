---
title: "Supported Platforms"

---

<div id="supported_platforms" class="registered_link"></div>

The EDB OCL Connector is certified with Advanced Server version 10 and later. The EDB OCL Connector native packages are supported on the following 64-bit platforms:

-   RHEL 7.x and 8.x (x86_64) 
-   Rocky Linux/AlmaLinux 8.x (x86_64)
-   CentOS 7.x (x86_64)  
-   OL Linux 7.x and 8.x
-   RHEL 7.x (ppc64le)
-   RHEL 8.x (ppc64le)
-   SLES 12.x
-   Debian 9.x and 10.x
-   Ubuntu 18.04 and 20.04 LTS

The EDB OCL Connector graphical installers are supported on the following 64-bit Windows platforms:

-   Windows Server 2019
-   Windows Server 2016
-   Windows Server 2012 R2
-   Windows 10
-   Windows 8.1

The EDB OCL Connector graphical installers are supported on the following 32-bit Windows platforms:

-   Windows 10
-   Windows 8.1
