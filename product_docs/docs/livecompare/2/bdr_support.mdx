...
LiveCompare can be used against EDB Postgres Distributed (PGD, formerly known as BDR)
nodes, as well as non-PGD nodes.

Setting `logical_replication_mode = bdr` will make the tool assume that all
databases being compared belong to the same PGD cluster. Then you can specify
node names as connections, and replication sets to filter tables.

For example, consider you are able to connect to any node in the PGD cluster.
Let's call this `Initial Connection`. By initially connection to this node,
LiveCompare is able to check PGD metadata and retrieve connection information
from all other nodes.

Now consider you want to compare 3 PGD nodes. As LiveCompare is able to connect
...
define `node_name`. LiveCompare searches in PGD metadata about the connection
...
fetching PGD metadata, it is required that LiveCompare is able to connect to
them using the same DSN from PGD view `bdr.node_summary`, field
...
PGD metadata to build the table list, considering only tables that belong to the
replication set(s) you defined in the `replication_sets` setting.

For example, you can create an `.ini` file to compare 3 PGD nodes:
...
It is also possible to tell LiveCompare to compare all active nodes in the PGD
...
the list of all PGD nodes. Additional data connections are not required;
although if set, will be appended to the list of data connections. For example,
it would be possible to compare a whole PGD cluster against a single Postgres
...
Please note that to enable pglogical metadata fetch instead of PGD, just set
`logical_replication_mode = pglogical` instead of
`logical_replication_mode = bdr`.

## PGD Witness nodes

Using replication sets in PGD, it's possible to configure specific tables to be
included in the PGD replication, and also specify which nodes should receive
data from such tables, by configuring the node to subscribe to the replication
set the table belongs to. This allows for different architectures such as PGD
Sharding and the use of PGD Witness nodes.

A PGD Witness is a regular PGD node which doesn't replicate any DML from other
nodes. The purpose of the Witness is to provide quorum in Raft Consensus voting
(for more details on the PGD Witness node, check PGD documentation). Depending
on how replication sets were configured, the Witness may or may not replicate
DDL. Which means that there are 2 types of PGD Witnesses:
...
In the first case, even if the PGD Witness is included in the comparison (either
...
In the second case, on the other hand, the table exists on the PGD Witness.
...
case for the PGD Witness, then LiveCompare will log this:
...
**IMPORTANT**: Please note that PGD 3.6.18 introduced the new pre-created
`bdr_local_only_origin` replication origin to be used for applying local-only
transactions. So if LiveCompare is connected to PGD 3.6.18, it won't create this
replication origin.
...
-   Set the current transaction datetime to be far in the past, so if there are
    any PGD conflicts with real DML being executed on the database, LiveCompare DML
    always loses the conflict.

After applying LiveCompare fix script to a PGD node, it
...
superuser. Starting from PGD 3.7.0, those functions are deprecated. LiveCompare
...
specific for PGD clusters. It will only work in PGD 3.6, 3.7, 4 and 5.
clusters.
...
conflicts that are registered in `bdr.apply_log`, in case of PGD 3.6, or in
`bdr.conflict_history`, in case of PGD 3.7, 4 and 5.
...
conflict resolution which is being done by PGD is consistent among nodes, i.e.,
after PGD resolving conflicts the cluster is in a consistent state.
...
You will get the PGD cluster consistent again.
...
in PGD 3.7+ or `bdr.apply_log` in PGD 3.6, you should run LiveCompare with an
...
the `Initial Connection`. So it should be a pglogical- and/or PGD-enabled
database.
