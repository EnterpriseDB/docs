...
LiveCompare is an analysis tool. You can use it with EDB Postgres Distributed (PGD, formerly known as BDR) nodes and non-PGD nodes.
<!-- Is this a parameter? where are we actually setting this. specifying that will be helpful -->
When you set  `logical_replication_mode = bdr`, LiveCompare  considers that all databases being compared belong to the same PGD cluster. You can specify node names as connections and replication sets to filter tables.

You can connect to any node in the PGD cluster. The first connection is referred to as initial connection. By initially connecting to a node, LiveCompare can check PGD metadata and retrieve connection information from all other nodes.

For example, suppose you want to compare three PGD nodes. As LiveCompare can connect to any node starting from the initial connection, you do not need to define  `dsn`  or any connection information for the data connections. You only need to define  `node_name`. LiveCompare searches in PGD metadata about the connection information for that node and then connects to the node.

For LiveCompare to connect to all other nodes by fetching PGD metadata, it must be able to connect to them using the same DSN from the  `bdr.node_summary` PGD view in the `interface_connstr` field. In this case, it is recommended to run LiveCompare on the same machine as the initial connection as the postgres user. If is not possible to run LiveCompare on the same machine, then define the  `dsn`  attribute in all data connections.
...
You can also compare all active nodes in the PGD cluster with LiveCompare. 
To compare all active nodes, perform the following steps:
...
replication_sets = set_name = 'bdrgroup'
```
When  `all_bdr_nodes = on`, LiveCompare uses the  `Initial Connection`  setting to fetch the list of all PGD nodes. Additional data connections are not required. If additional data connections are set, they are appended to the list of data connections. For example, you can compare a whole PGD cluster with a single Postgres connection, which is useful in migration projects:
```
[General Settings]
logical_replication_mode = bdr
max_parallel_workers = 4
...
Using replication sets in PGD, you can configure specific tables and include them in the PGD replication. You can also specify the nodes to receive data from these tables.  You must configure the node to subscribe to the replication set that the table belongs to. This setting allows for different architectures, such as PGD sharding and using PGD witness nodes.

A PGD witness node is a regular PGD node that does not replicate any DML from other nodes. The purpose of the PGD witness node is to provide quorum in Raft Consensus voting. (For details about the PGD witness node, see  [Witness nodes](https://www.enterprisedb.com/docs/pgd/latest/nodes/#witness-nodes)  in the PGD documentation.) 

There are two types of PGD witness nodes:

-   A completely empty node without any data nor tables
-   A node that replicates DDL from other nodes, so it has empty tables

Replication set configuration determines whether the PGD witness node replicates DDLs. 

In the first case, even if the PGD witness node is included in the comparison (either manually under  `[Connections]`  or using  `all_bdr_nodes = on`), because the PGD witness node does not have any tables, the following message is logged:

```
Table public.tbl does not exist on connection node1
```
In the second case, the table exists on the PGD witness node. However, it is not correct to report data missing on the PGD witness node as divergences. So, for each table, LiveCompare checks the following information on each node included in the comparison:
...
If the intersection among all three lists of replication sets is empty, which is the case for the PGD witness node, then LiveCompare logs the following message:
```
Table public.tbl is not subscribed on connection node1
```
In both cases, the comparison for that table proceeds on the nodes where the table exists, and the table is replicated according to the replication set's configuration.

## Differences in a PGD cluster

LiveCompare makes changes only to the local node. It is important that corrective changes do not get replicated to other nodes.

When  `logical_replication_mode = bdr`, LiveCompare first checks if a replication origin called  `bdr_local_only_origin`  already exists. (You can configure the name of the replication origin by adjusting the setting  `difference_fix_replication_origin`.) 

If a replication origin called  `bdr_local_only_origin`  does not exist, then LiveCompare creates it on all PGD connections.

##### Important

PGD 3.6.18 introduced the new pre-existing  `bdr_local_only_origin`  replication origin to use for applying local-only transactions. If LiveCompare is connected to PGD 3.6.18, it does not create this replication origin.

LiveCompare generates application scripts considering the following:
...
After applying a LiveCompare fix script to a PGD node, you can get the rows that were inserted or updated by LiveCompare using the following query. Replace  `mytable`  with the name of any table.
```
with lc_origin as (
    select roident
...
LiveCompare requires at least a PostgreSQL user with `bdr_superuser` privileges to fetch metadata.

All these steps involve replication origins applied only to the output script if the PostgreSQL user has `bdr_superuser` or PostgreSQL superuser privileges. Otherwise, LiveCompare generates fixes without associating any replication origin. (Transaction replication is still disabled using  `SET LOCAL bdr.xact_replication = off`.) However, we recommend using a replication origin when applying the DML scripts. Otherwise, LiveCompare has the same precedence as a regular user application regarding conflict resolution. Also, as there is no replication origin associated with the fix, you cannot use the query to list all rows fixed by LiveCompare.
...
If a PostgreSQL superuser is not being used, then LiveCompare includes only the following information in the generated DML transaction:
...
While  the `compare`  mode is used to compare all content of tables as a whole,  the `conflicts`  mode focuses on tuples or tables that are related to existing conflicts registered in  `bdr.apply_log`, in case of PGD 3.6, or in  `bdr.conflict_history`, in case of PGD 3.7, PGD 4, and PGD 5.

The `conflicts`  execution mode is expected to run much faster than  the `compare`  mode because it inspects only specific tuples from specific tables. However, it is not as complete as the  `compare`  mode for the same reason.
...
For a general use case, the automatic conflict resolution ensures cluster consistency. However, there are a few known cases where the automatic conflict resolution can result in divergent tuples among nodes. Hence, the  `conflicts`  execution mode from LiveCompare can help in checking and ensuring consistency, providing a good balance between time and result.
...
```
This conflict indicates that when the  `DELETE` command arrived from  `node2`  to  `node3`, there was no row with  `b = 3`  in the `tbl` table . However, the  `INSERT` command might have arrived from  `node1`  to  `node3`  later, which then added the row with  `b = 3`  to  `node3`. The following snippet indicates the current situation on  `node3`:
