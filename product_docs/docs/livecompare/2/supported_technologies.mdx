...
In LiveCompare there are three kinds of connections:
...
The table shows versions and details about supported technologies and the context in which they can be used in LiveCompare.
...
and which isn't possible when `pool_mode` is either `transaction` or
`statement`.
