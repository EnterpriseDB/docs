...
| Type           | Description                                                                                                                                                                            | ID |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -- |
| New feature    | New section in setting called `Comparison Key` allows users to define a custom comparison key (list of columns) per table. This can be useful for tables without primary keys or unique indexes. | LIV-5 |
...
| New feature    | When using all columns from the table as a comparison key, LiveCompare now ignores the nullable columns to avoid false positives when considering all columns. This behavior can be disabled by setting `ignore_nullable = false`. | LIV-96 |
...
| Enhancement    | LiveCompare schema can now be added to a replication-enabled (BDR, pglogical or native logical replication) database. | LIV-42 |
