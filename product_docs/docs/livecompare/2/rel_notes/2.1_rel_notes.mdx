...
| Type           | Description                                                                                                                                                                            | ID |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -- |
| New feature    | Support for Postgres-BDR 4.  | LIV-131 |
| New feature    | New setting `min_time_between_heart_beats`, which tells LiveCompare to log the comparison progress at every heart beat, by default set to 30 seconds using the `INFO` log level.       | LIV-128 |
| New feature    | New settings `comparison_cost_limit` and `comparison_cost_delay` that, when greater than 0, tell each worker to take a nap of `comparison_cost_delay` seconds (for example, `0.5`) after processing `comparison_cost_limit` number of rows. | LIV-16 | 
