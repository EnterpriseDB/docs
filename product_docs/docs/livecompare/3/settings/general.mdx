...
### `server_side_block_hash`
Optional optimization for the `block_hash` comparison algorithm. When enabled, instead of transferring every row's hash back to LiveCompare for comparison, LiveCompare asks each database server to compute and return a single aggregate hash for each row buffer. For buffers that match — the common case when databases are largely in sync — this collapses potentially thousands of per-row transfers into a single round-trip per buffer, reducing network payload. The improvement is most noticeable on multi-node deployments compared across a WAN.

Default: `off`.

Takes effect only when all of the following are true:

- `comparison_algorithm = block_hash`
- `data_fetch_mode = prepared_statements`
- The table being compared has a single-column primary key.
- LiveCompare isn't running in `conflicts` or `recheck` mode.

!!! Note
If any of these conditions aren't met for a given table, LiveCompare automatically falls back to the standard `block_hash` behavior for that table.
!!!
