...
deepToC: true
description: Available connection settings in LiveCompare 3.0
---

## Initial connection

Use the initial connection only when `logical_replication_mode` is set to
...
If `dsn` isn't set, the `host` argument defaults to the Unix socket directory (typically `/var/run/postgresql/`). If the socket isn't there, you can connect using any socket reported by `SHOW unix_socket_directories` or specify `localhost` for the host argument.
...
Similar to `Initial Connection` and `Output Connection`, a data
connection is defined in a named section. The section name is of the
form `<Name> Connection`, where `<Name>` is any single-word string starting with an
