...
If `dsn` is not set, the `host` argument defaults to the Unix socket directory (typically `/var/run/postgresql/`). If the socket is not present there, you can connect using any socket reported by `SHOW unix_socket_directories`, or specify `localhost` for the host argument.
