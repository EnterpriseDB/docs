...
Copy any `/etc/livecompare/template*.ini` to use in your project and adjust as necessary. See [Settings](settings/) for details.

```
cp /etc/livecompare/template_basic.ini my_project.ini
```

See [Settings](settings/) for a description of the various configuration options.

For example, the basic template contains:
