...
**Step 5:** Move or copy the certificate and private key files to the EDB Postgres Advanced Server data directory (for example, `/opt/edb/data`).

```shell
$ mv root.crt /opt/edb/data
$ mv server.crt /opt/edb/data
$ mv server.key /opt/edb/data
```
