...
2. **Runtime Configuration**: Select or create [ServingRuntimes](/edb-postgres-ai/current/ai-factory/model/configure-servingruntime/) optimized for the model framework (vLLM, TensorRT-LLM, custom)
3. **Resource Allocation**: Define GPU, memory, and CPU requirements based on expected workload
4. **Endpoint Configuration**: Set up internal cluster access or external API endpoints with authentication (see [Access KServe endpoints](/edb-postgres-ai/current/ai-factory/model/access-kserve-endpoints/))
