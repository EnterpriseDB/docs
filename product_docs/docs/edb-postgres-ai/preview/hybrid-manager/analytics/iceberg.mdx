...
For a general overview of Iceberg, see [Apache Iceberg® with EDB Solutions](/edb-postgres-ai/current/analytics/iceberg).
...
For definitions of analytics terms used in Hybrid Manager—such as PGFS, PGAA, Lakehouse, Catalog, AutoPartition, and Analytics Offload—see [Analytics concepts (hub)](/edb-postgres-ai/current/analytics/analytics-concepts).
...
Related: [Analytics concepts (hub)](/edb-postgres-ai/current/analytics/analytics-concepts)
...
2. [Configure an Iceberg catalog connection](/edb-postgres-ai/current/analytics/how-to-iceberg-reads-writes-with-catalog) (HM-managed or external).
3. Learn [Tiered Tables concepts](/edb-postgres-ai/current/analytics/tiered_tables) if integrating with PGD.
4. [Read Iceberg/Delta with or without a catalog](/edb-postgres-ai/current/analytics/how-to-lakehouse-read-with-without-catalog).
