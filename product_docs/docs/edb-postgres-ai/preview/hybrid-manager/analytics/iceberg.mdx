...
For a general overview of Iceberg, see [Apache Iceberg® with EDB Solutions](/pgaa/latest/iceberg).
...
For definitions of analytics terms used in Hybrid Manager—such as PGFS, PGAA, Lakehouse, Catalog, AutoPartition, and Analytics Offload—see [Analytics concepts (hub)](/pgaa/latest/analytics-concepts).
...
Related: [Analytics concepts (hub)](/pgaa/latest/analytics-concepts)
...
2. [Configure an Iceberg catalog connection](/pgaa/latest/how-to-iceberg-reads-writes-with-catalog) (HM-managed or external).
3. Learn [Tiered Tables concepts](/pgaa/latest/tiered_tables) if integrating with PGD.
4. [Read Iceberg/Delta with or without a catalog](/pgaa/latest/how-to-lakehouse-read-with-without-catalog).
