...
For foundational concepts about EDB Postgres Lakehouse architecture, see [EDB Postgres Lakehouse: An Overview](/pgaa/1.5/lakehouse).
...
For definitions of core analytics terms used in Hybrid Manager—such as PGFS, PGAA, Lakehouse Cluster, and Analytics Offload—see [Analytics concepts (hub)](/pgaa/1.5/analytics-concepts).
...
How-To: [Create a Lakehouse cluster](/pgaa/1.5/create-lakehouse-cluster.mdx)
...
3. (Optional) [Configure an Iceberg catalog connection](/pgaa/1.5/how-to-iceberg-reads-writes-with-catalog).
4. Learn [Tiered Tables concepts](/pgaa/1.5/tiered_tables) if integrating with PGD.
5. Create Lakehouse tables or external readers for Iceberg/Delta data.
6. Query your data using Postgres clients.

## Related How-Tos

- [Create a Lakehouse cluster](/pgaa/1.5/create-lakehouse-cluster.mdx)
