...
For foundational concepts about EDB Postgres Lakehouse architecture, see [EDB Postgres Lakehouse: An Overview](/edb-postgres-ai/current/analytics/lakehouse).
...
For definitions of core analytics terms used in Hybrid Manager—such as PGFS, PGAA, Lakehouse Cluster, and Analytics Offload—see [Analytics concepts (hub)](/edb-postgres-ai/current/analytics/analytics-concepts).
...
How-To: [Configure an Iceberg catalog connection](/edb-postgres-ai/current/analytics/how-to-iceberg-reads-writes-with-catalog)
How-To: [Read Iceberg/Delta with or without a catalog](/edb-postgres-ai/current/analytics/how-to-lakehouse-read-with-without-catalog)
...
How-To: [Create a Lakehouse cluster](/edb-postgres-ai/current/analytics/create-lakehouse-cluster.mdx)

## Getting started with Lakehouse Clusters in Hybrid Manager

To begin using Lakehouse Clusters with Hybrid Manager:

1. [Provision a Lakehouse cluster](/edb-postgres-ai/current/analytics/create-lakehouse-cluster.mdx).
2. [Configure PGFS storage locations](/edb-postgres-ai/current/ai-factory/pipeline/pgfs/) pointing to object storage.
3. (Optional) [Configure an Iceberg catalog connection](/edb-postgres-ai/current/analytics/how-to-iceberg-reads-writes-with-catalog).
4. Learn [Tiered Tables concepts](/edb-postgres-ai/current/analytics/tiered_tables) if integrating with PGD.
...
- [Configure an Iceberg catalog connection](/edb-postgres-ai/current/analytics/how-to-iceberg-reads-writes-with-catalog)
- [Read Iceberg/Delta with or without a catalog](/edb-postgres-ai/current/analytics/how-to-lakehouse-read-with-without-catalog)
