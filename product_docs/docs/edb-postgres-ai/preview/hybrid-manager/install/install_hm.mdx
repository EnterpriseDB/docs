...
[Is this the right chronological place in the step-by-step for this?]: #

After the bootstrap operator creates the `HybridControlPlane` resource, the operator automatically creates a `Preflight` resource to validate that all required secrets are present. If the installation appears stuck before pods begin starting, check the Preflight status:

```bash
kubectl get preflight -o yaml
```

If the all entries in `status.secretRequirementsStatus` show the `Valid` status, proceed with the next checks. 
...
The bootstrap operator also automatically creates a `Postflight` resource to monitor your installation. It verifies that your pods, databases, backups, nodes and certificates are configured and valid.

Before you move on to post-installation configurations, ensure your installation is healthy:
