...
After the `HybridControlPlane` resource is created, the operator automatically creates a `Preflight` resource to validate that all required secrets are present. If the installation appears stuck before pods begin starting, check the Preflight status:

```bash
kubectl get preflight -o yaml
```

If any entry in `status.secretRequirementsStatus` shows `SecretNotFound` or `Invalid`, the operator is blocked waiting for those secrets to be fixed. See [Understanding Preflight checks](./preflight_checks) for details on interpreting results and resolving failures.
