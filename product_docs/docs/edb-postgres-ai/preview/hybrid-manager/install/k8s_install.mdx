...
!!!note 
Required only for deployments where the `ai` installation module is enabled. This module is included by default unless it is manually excluded via the `spec.scenarios` parameters in `values.yaml`.
!!!
