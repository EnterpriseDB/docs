...
1. Set the variable for the Data Migration Service domain name (internally called `transporter`): 
   ...
   ```bash
   export DMS_DOMAIN_NAME=dms<your-hm-portal-domain-name> # Customer domain name for the EDB Postgres AI Platform Transporter RW Service
   ```
   ...
   !!! Important
       ...
       `PORTAL_DOMAIN_NAME` and `DMS_DOMAIN_NAME` must be:
