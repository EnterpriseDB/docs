...
!!!note
Required only for deployments with the `analytics` installation module enabled. This module is included by default unless it is manually excluded via the `spec.scenarios` parameter in `values.yaml`.
!!!
