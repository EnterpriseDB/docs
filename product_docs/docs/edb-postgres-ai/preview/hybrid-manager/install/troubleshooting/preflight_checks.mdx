...
* [Phase 5: Installing Hybrid Manager](./install_hm) (Started)

### Outcomes

* *Ability to diagnose and resolve secret-related blockers during Hybrid Manager installation.*

When you start the Hybrid Manager installation, the operator automatically creates a `Preflight` custom resource. This resource validates that all required Kubernetes secrets exist and are correctly structured **before** the operator proceeds to deploy the platform services.
...
If the installation appears stuck after running the Helm command, inspect the Preflight status:
...
If the Helm command completes but pods are not starting, the Preflight checks may be blocking deployment. Check the Preflight status first:
...
1. Verify which scenarios your HCP defines:

    ```bash
    kubectl get hybridcontrolplane <name> -o jsonpath='{.spec.scenarios}'
    ```

2. Confirm the requirement's scenario matches one of the HCP scenarios. Requirements with non-matching scenarios are silently skipped.
