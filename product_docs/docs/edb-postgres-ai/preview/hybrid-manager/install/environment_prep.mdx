...
[I wonder if this is no longer required if the customer doesn't have the `ai` installation module enabled?]: #
...
[Same here, is this no longer required if a certain installation module (`ai` or `analytics` not sure what catalogs are for) is disabled?]: #
...
[Is this required even if the `migrations` scenario is disabled?]: # 
