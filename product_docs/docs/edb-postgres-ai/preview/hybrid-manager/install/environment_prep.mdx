...
!!!note
Required for deployments with the `ai` installation module enabled. This module is included by default unless it is manually excluded via the `spec.scenarios` parameter in `values.yaml`.
!!!
...
* [**SOP: Enabling GenAI Builder**](/edb-postgres-ai/current/hybrid-manager/install/configuration/genai_secret/)
    * *Guidance: Scripts for generating Fernet keys and CORS JSON configurations.*

### Creating Catalog secret

!!!note
Required for deployments with the `analytics` installation module enabled. This module is included by default unless it is manually excluded via the `spec.scenarios` parameter in `values.yaml`.
!!!

The Catalog secret is required to create encrypted storage credentials.
...
* [**SOP: Enabling data catalogs**](/edb-postgres-ai/current/hybrid-manager/install/configuration/data_catalog_secret/)
   * *Guidance: Commands for creating the confounding key, namespace, and secret.*

### Customizing Migration Portal secrets

!!!note
Required for deployments with the `migration` installation module enabled. This module is included by default unless it is manually excluded via the `spec.scenarios` parameter in `values.yaml`.
!!!

Create secrets in namespaces `edb-migration-portal` and `edb-migration-copilot`.

* [**SOP: Customizing Migration Portal secrets for secure internal communication**](/edb-postgres-ai/current/hybrid-manager/install/configuration/migration_portal_secrets/)
...
* **SOP: [Bring your own private certificate authority](/edb-postgres-ai/1.3/hybrid-manager/install/configuration/cert-man/#bring-your-own-private-certificate-authority).**
