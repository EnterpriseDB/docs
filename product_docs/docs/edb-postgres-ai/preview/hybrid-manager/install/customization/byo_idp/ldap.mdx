...
After changing the authentication settings, you need to reconcile the Dex configuration and restart the Dex services for your changes to take effect. See [Applying Dex](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/managing_users/apply_dex_config/) for instructions. 

## Assign new owner and disable native user

After assigning a new organization owner in the HM console, disable the native user. See [Disabling a native user](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/managing_users/native_users/disable_native_user/). 
