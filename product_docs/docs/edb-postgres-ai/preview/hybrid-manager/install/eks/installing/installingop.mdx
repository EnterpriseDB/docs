...
The bootstrap process requires a number of values to be set in the Helm chart. These values are stored in a file called `hm-values.yaml`. You can create this file by running the `prepare-op.sh` script, which uses variables from the `default-env.sh` file. Edit the `default-env.sh` file to set the values you require.
...
Run the `prepare-op.sh` script to create the `hm-values.yaml` file.

```shell
$SHELL prepare-op.sh
```

This create a `hm-values.yaml` file with the values set in the `default-env.sh` file.
...
You now need to edit `hm-values.yaml` file and set the values required for your installation. Find the appropriate key in the `hm-values.yaml` file and set the values as follows. Replace `$<name>` entries with the value you have set the environment variables of the same name to.
