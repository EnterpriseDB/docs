...
The version of the EDB Hybrid Manager platform is set by the environment variable EDB_PLATFORM_VERSION. This should be set to the version of the platform you wish to install. For example, to install version v2026.01.0, you would set the environment variable as follows:
...
You will also need to set the environment variable `OPERATOR_VERSION` to the version of the operator you wish to install, for example `v2026.01.0`.
...
-   **[A secret for Griptape and configures DeltaLake object storage](/edb-postgres-ai/current/hybrid-manager/install/customization/genai_secret/) (optional for GenAI Builder)**
...
1.  Set `spec.globalParameters.global.dms_domain_name` to $DMS_DOMAIN_NAME.
1.  Set `spec.global.load_balancer_provider` to $LOAD_BALANCER_PROVIDER.
1.  Set `spec.global.load_balancer_mode` to $LOAD_BALANCER_MODE.
1.  Set `spec.global.node_port_domain` to $NODE_PORT_DOMAIN.
