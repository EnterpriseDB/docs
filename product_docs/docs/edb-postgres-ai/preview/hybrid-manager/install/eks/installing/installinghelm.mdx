...
The version of the EDB Software Deployment platform is set by the environment variable EDB_PLATFORM_VERSION. This should be set to the version of the platform you wish to install. For example, to install version v2026.01.0, you would set the environment variable as follows:
...
-   **[A secret for Griptape and DeltaLake object storage configuration](/edb-postgres-ai/current/hybrid-manager/install/customization/genai_secret/) (optional for GenAI Builder)**

-   **[A secret for Catalog](/edb-postgres-ai/current/hybrid-manager/install/customization/data_catalog_secret/) (optional for using Catalog)**

-   See also **[custom secrets for Migration Portal](/edb-postgres-ai/current/hybrid-manager/install/customization/migration_portal_secrets/) (optional to secure internal communications with Migration Portal)**
...
| DMS_DOMAIN_NAME              | The domain name for the Data Migration Service (internally `transporter`). See recommendation below.                                                                                         |
...
| LOCATION_NAME                | The location name for the HM agent, formerly known as Beacon agent.                                                                                                                             |
| LOAD_BALANCER_PROVIDER       | Specifies the cloud provider to use for provisioning load balancer services. Options include `aws`, `gcp`, `azure`. Leave this value empty if a self-managed (custom) load balancer is used. |
| LOAD_BALANCER_MODE           | Specifies whether the load balancer is `public`, `private` or `disabled`.                                                                                                                    |
| NODE_PORT_DOMAIN             | A domain name or hostname that resolves to the Kubernetes worker nodes via the public network. Used to facilitate external communication with internal services.                                                                                                   |

!!!note Recommendations for PORTAL_DOMAIN_NAME and DMS_DOMAIN_NAME

    For consistency and ease of management, derive your DMS_DOMAIN_NAME from your PORTAL_DOMAIN_NAME. The most common approach is to replace the `portal` subdomain with `dms`.
    ...
    | PORTAL_DOMAIN_NAME                       | DMS_DOMAIN_NAME            |
    |------------------------------------------|-----------------------------------------------|
...
1.  Set `bootstrapImageTag` to $EDB_PLATFORM_VERSION.
1.  Set `parameters.upm-istio.cookie_aeskey` to $AES_256_KEY.
1.  Set `containerRegistryURL` to $CONTAINER_REGISTRY_URI.
1.  Set `system` to $EDB_TARGET_PLATFORM.
1.  Set `parameters.global.portal_domain_name` to $PORTAL_DOMAIN_NAME.
1.  Set `parameters.global.dms_domain_name` to $DMS_DOMAIN_NAME.
1.  Set `parameters.global.load_balancer_provider` to $LOAD_BALANCER_PROVIDER.
1.  Set `parameters.global.load_balancer_mode` to $LOAD_BALANCER_MODE.
1.  Set `parameters.global.node_port_domain` to $NODE_PORT_DOMAIN.
1.  Set `parameters.upm-beacon.server_host` to $BEACON_SERVICE_DOMAIN_NAME.
1. Set `pgai.portal.authentication.staticPasswords[0].email` to $AUTHENTICATION_EMAIL.
1. Set `pgai.portal.authentication.staticPasswords[0].hash` to $AUTHENTICATION_PASSWORD_HASH.
1. Set `pgai.portal.authentication.staticPasswords[0].username` to $AUTHENTICATION_USER.
1. Set `pgai.portal.authentication.staticPasswords[0].userID` "c5998173-a605-449a-a9a5-4a9c33e26df".
1. Set `beaconAgent.location` to $LOCATION_NAME.
...
You now need to configure your DNS service or your `/etc/hosts` file to point the domains specified in `default-env.sh` to the IP address you obtained from the command above. Those domains are in `PORTAL_DOMAIN_NAME`, `DMS_DOMAIN_NAME`, and `BEACON_SERVICE_DOMAIN_NAME`. In the example above, they are set to `portal.examplecompany.com`, `dms.examplecompany.com`, and `beacon.examplecompany.com` respectively. 
...
dms         IN CNAME cluster.examplecompany.com
