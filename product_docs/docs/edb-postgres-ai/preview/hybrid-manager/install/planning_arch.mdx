...
* **Federation:** Manages secure communication and authorization across multiple HM instances in a Multi-Location topology.
  * *See:* [Configuring multiple data centers for Hybrid Manager](/edb-postgres-ai/current/hybrid-manager/install/configuration/multi-dc/)
...
To meet strict security standards and simplify the user experience, HM supports a modular installation approach. This allows you to deploy only the specific capabilities your organization requires, reducing the software footprint for security audits and hiding UI elements for features that aren't in use or authorized for your environment.
...
The **core** module is mandatory, while the others are optional. See [Installation modules](/edb-postgres-ai/preview/hybrid-manager/install/configuration/scenarios/) for more information.

### Planning your selection

When choosing which modules to install, consider the following architectural and operational factors:

**Functional footprint:** While the baseline resource usage is similar across scenarios, each module introduces specific services and endpoints. Limiting your installation to required modules simplifies security audits and reduces the "attack surface" of your production environment.

**UI and feature governance:** The HM console dynamically hides navigation links and tools (such as GenAI Builder or Data Catalog) for disabled modules. This ensures users only interact with authorized and licensed capabilities.

**Default behavior:** If the `scenarios` parameter is omitted from your configuration, the system installs all modules by default to maintain backward compatibility.
...
| **Provisioning mode** | `beaconAgent.provisioning.provider` | `azure`, `aws`, or `gcp` |
