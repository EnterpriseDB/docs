...
To optimize resource usage and simplify the user interface, you can use HM's modular installation approach. This allows you to deploy only the capabilities your organization requires, reducing the footprint of the installation and hiding UI elements for features you aren't using.

### Available modules

The following table describes the four primary scenarios. The **Core** module is mandatory, while the others are optional.
...
When choosing which modules to install, consider the following architectural impacts:

**Resource requirements:** Each additional module increases the CPU and memory requirements for your control plane nodes. If you disable heavy modules like AI or Analytics, you can often use smaller instance types.

**Default behavior:** If no scenarios are specified in your configuration, the system installs all modules by default to ensure backward compatibility.

Decide whether you want to perform a complete HM installation, or install a subset, e.g. `core` + `migrations`, or `core` + `ai`, `core` + `analytics`, etc.
