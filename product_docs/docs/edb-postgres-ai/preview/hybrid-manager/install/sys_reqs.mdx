...
| **Migration Domain** | `parameters.transporter-rw-service:domain_name` | The internal domain for the Transporter migration service. Required unless the `migration` installation module is explicitly disabled. |
| **Migration URL** | `parameters.transporter-dp-agent:rw_service_url` | The full URL for the service: `https://<Migration Domain>/transporter`. Required unless the `migration` module is explicitly disabled. |
...
| **Phase 1** | **Installation modules** | `spec.scenarios` | **Optional**. All modules are enabled by default. To customize, specify `core` plus any combination of `migration`, `ai`, or `analytics`. |
