...
| **Migration Domain** | `parameters.transporter-rw-service:domain_name` | The internal domain for the Transporter migration service. Required unless the `migration` installation module isn't enabled. |
| **Migration URL** | `parameters.transporter-dp-agent:rw_service_url` | The full URL for the service: `https://<Migration Domain>/transporter`. Required unless the `migration` installation module isn't enabled. |
...
| **Phase 1** | **Installation modules** | `spec.scenarios` | **Optional**. All modules are enabled by default. To customize, specify `core` plus a combination of `migration`, `ai`, or `analytics`. |
