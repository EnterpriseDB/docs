...
**Sizing guidelines for an HM installation with all modules (default):**
...
[how do these requirements change if `ai`, `analytics`, or `migrations` modules are diabled?]: #
...
> Optionality: Required only if utilizing GenAI/AI Factory capabilities (Installation module `ai` is enabled).
...
| **Migration Domain** | `parameters.transporter-rw-service:domain_name` | The domain name for the internal Transporter migration service. Required for default installations or when the `migrations` installation module is enabled. |
| **Migration URL** | `parameters.transporter-dp-agent:rw_service_url` | Full URL derived from the domain above: `https://<Migration Domain>/transporter`. Required for default installations or when the `migrations` installation module is enabled. |
...
* **Consistency:** In multi-location deployments, the object storage configuration must be available equally across all locations. (See [multi-dc guidance](/edb-postgres-ai/current/hybrid-manager/install/configuration/multi-dc/) for details.)
...
| **Phase 1** | **Installation modules** | `spec.scenarios` | **Optional** All modules enables by default, otherwise configure `core` and one or several of the following: `migrations`, `ai`, `analytics`. |
...
!!!note 
   For a subset of HM installation modules, add a new `spec` section, e.g.: 
