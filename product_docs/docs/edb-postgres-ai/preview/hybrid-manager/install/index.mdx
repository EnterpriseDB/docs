...
* **Documentation:** [See Hybrid Manager Installation](install_hm.mdx)
...
During installation, the operator automatically validates that all required secrets are present and correctly configured before deploying the platform services. If the installation appears stuck, Preflight check status helps identify the blocking issue.

* **Goal:** Diagnose and resolve secret-related blockers during installation.

* **Key activities:**

    * Inspect the `Preflight` resource status to identify missing or misconfigured secrets.

    * Resolve any issues and allow the operator to proceed.

* **Documentation:** [Understanding Preflight checks](preflight_checks)
...
Once Hybrid Manager reaches the `deployed` phase, the operator automatically runs health checks covering pods, databases, certificates, nodes, and backups. Use Postflight status to verify your deployment is healthy before proceeding to day-2 configuration.

* **Goal:** Verify the deployed platform is healthy.

* **Key activities:**

    * Review the `Postflight` resource status to confirm all checks pass.

    * Investigate and resolve any unhealthy checks before proceeding.

* **Documentation:** [Monitoring with Postflight checks](postflight_checks)
