...
For GCP GKE deployments, this is typically Google Container Registry (GCR) or Artifact Registry in your GCP project, though other supported container registries can also be used.
...
- `<LOCAL_REGISTRY_USER>`: Username for your destination registry (or `_json_key` for GCP)
- `<LOCAL_REGISTRY_PWD>`: Password for your destination registry (or service account JSON for GCP)
...
```bash
# Example for GCP Artifact Registry
export GCP_PROJECT_ID=my-gcp-project
export GCP_REGION=us-central1
export ARTIFACT_REGISTRY_REPO=edbhm

edbctl image sync-to-local-registry \
    --destination-registry-uri "${GCP_REGION}-docker.pkg.dev/${GCP_PROJECT_ID}/${ARTIFACT_REGISTRY_REPO}" \
    --version "${HM_RELEASE}" \
    --source-registry-username "${EDB_SOURCE_REGISTRY}" \
    --source-registry-password "${EDB_TOKEN}" \
...
# Login to destination registry (GCP example)
skopeo login ${GCP_REGION}-docker.pkg.dev --username _json_key --password-stdin < ~/gcp-service-account-key.json
```
