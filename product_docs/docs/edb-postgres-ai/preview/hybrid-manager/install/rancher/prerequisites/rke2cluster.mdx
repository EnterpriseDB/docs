...
After you've [installed the necessary system tools](/edb-postgres-ai/current/hybrid-manager/install/rancher/prerequisites/), you're almost ready to set up a Rancher RKE2 cluster for use with Hybrid Manager (HM).
...
1.  Add the Helm chart repo from EDB Repos 2.0:
    ...
    where `<your-edb-repos-token>` is your EDB Repos 2.0 repository access token.
...
Secure your endpoints (such as the HM Portal) using [TLS certificates](/edb-postgres-ai/current/hybrid-manager/install/customization/cert-man.mdx).

This requires DNS hostnames to be finalized (see previous step) so the certificates can be issued for the correct names.

[Custom certificates](/edb-postgres-ai/current/hybrid-manager/install/customization/cert-man/#set-up-a-custom-x509-certificate-for-the-hybrid-manager-portal) are strongly suggested, with auto-generated self-signed certificates as the default fallback option.

You can also set up a [custom cert-manager issuer](/edb-postgres-ai/current/hybrid-manager/install/customization/cert-man/#set-up-a-custom-cert-manager-issuer-for-the-hm-portal) for the HM Portal and even setup your own [certificate authority](/edb-postgres-ai/current/hybrid-manager/install/customization/cert-man/#bring-your-own-private-certificate-authority).

#### User identity configuration

To configure user identity, you have to choose between implementing your own IdP or using HM's native users feature.

It is strongly recommended to [set up your identity provider (IdP)](/edb-postgres-ai/current/hybrid-manager/install/customization/byo_idp/index.mdx) with HM for managing user access in production.

[Native users](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/managing_users/index.mdx) are supported by default, but managing users this way is not recommended in production.
...
1.  Ensure [edbctl is installed and configured](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/edbctl/) if it is not already.
...
1.  [Create a secret for GenAI Builder and configure Delta Lake object storage](/edb-postgres-ai/current/hybrid-manager/install/customization/genai_secret/).

2.  [Create a secret for Catalog](/edb-postgres-ai/current/hybrid-manager/install/customization/data_catalog_secret/).

### Securing Migration Portal

[Create custom secrets for Migration Portal](/edb-postgres-ai/current/hybrid-manager/install/customization/migration_portal_secrets/) if you want to secure internal communication for Migration Portal.
...
If you are planning on using Transparent Data Encryption (TDE), [configuring a Key Management Store](/edb-postgres-ai/current/hybrid-manager/install/customization/kms_for_tde/index.mdx) (KMS) is required.
