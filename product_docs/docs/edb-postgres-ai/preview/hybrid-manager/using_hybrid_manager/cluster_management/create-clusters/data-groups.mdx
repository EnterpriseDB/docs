...
**Deployment Location** — Select the location for the data group to be deployed into. The first location in the list is in the Kubernetes cluster hosting the EDB Postgres AI service. Where Hybrid Manager (HM) has been configured in a multi-HM environment, you can select different deployment locations for each data group to distribute the cluster data groups across multiple data centers.
