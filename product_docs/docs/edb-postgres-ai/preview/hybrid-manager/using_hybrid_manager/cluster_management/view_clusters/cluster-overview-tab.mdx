...
If the cluster is a distributed high-availability cluster, the overview first shows the [Data Groups](c#data-groups) section.
...
Initiate a write leader switchover within a EDB Postgres Distributed (PGD) Essential or Expanded data group to seamlessly transfer the write leader role from one node to another. This zero-data-loss operation is designed to minimize downtime, ensuring that running applications experience a smooth transition with no lost transactions.
