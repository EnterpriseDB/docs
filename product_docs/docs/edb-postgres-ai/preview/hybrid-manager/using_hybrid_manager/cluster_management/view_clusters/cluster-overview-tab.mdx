...
Initiate a write leader switchover within a EDB Postgres Distributed (PGD) Essential or Extended data group to seamlessly transfer the write leader role from one node to another. This zero-data-loss operation is designed to minimize downtime, ensuring that running applications experience a smooth transition with no lost transactions.
