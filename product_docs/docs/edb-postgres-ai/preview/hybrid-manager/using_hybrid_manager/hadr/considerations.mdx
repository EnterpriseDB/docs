...
!!! tip
    ...
    [do we mean replication of the object storage bucket? Also, we support not only AWS, but also Azure, GCP and other, so I wonder if we should generalize this item]: # 
...
[here we say to define a new backup schedule, but then in the next "danger" do not overwrite it, so this is a bit contradictory...]: #

!!! Danger
    Do not modify the default schedule, as it may be overwritten by a HM software update.

[related to my question above, maybe we mean that the existing default shouldn't be overwritten but rather complemented with an additional one?, for example "Complement the default schedule by creating an additional custom backup schedule instead."]: #

The following example shows a custom schedule to back up the needed resources each hour:

[Below we don't say where nor how to store the CRD with the custom backup schedule. Do we apply with a kubectl command? in which namespace? we should provide more input]: #
