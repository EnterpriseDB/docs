...
1.  Save the cluster manifest to a yaml file: 
...
1.  Clean the backup area that was created during the installation of the new HM instance to avoid confusion with the old backups that you want to restore:
    ...
    [this looks a bit weird to me: should it be `aws s3 rm s3://${NEW_BUCKET}/edb-internal-backups/<new-backups-random-string>/databases/<pg-cluster-name> --recursive`?]: #
    ...
    [Same here with using ${NEW_BUCKET} instead of <linked-bucket-name>]: #
