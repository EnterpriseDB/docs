...
The DR procedure is defined as the series of manual steps that you need to take to recover your HM installation and your HM-managed Postgres clusters.
...
- The same locations (`locations.beacon.enterprisedb.com` custom resource) used in the old HM instance must be available in the new one. These are an internal resource created during install and isn't available in the HM console. `managed-devspatcher` is the default value.

[still need to check whether this is required]: #
...
1.  Copy the old backups to the new bucket using your preferred tools. Here are some examples using cloud service provider CLIs to move data between buckets:
    ...
    If you have configured additional _custom_ managed locations, use the same method to copy those folders.
...
The Velero plugin helps restore the Kubernetes resources in a correct state. The Postgres cluster's resources are restored as deleted, so you can later restore data as desired.
...
1.  Select a backup from this list and export the following environment variables with the corresponding values: 
    ...
    [this is a different timestamp than the timestamp above, right? is this for the pg database clusters onstead of HM-internal services?]: #
...
1.  Restore custom managed storage locations by applying the following Velero restore:
