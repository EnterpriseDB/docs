...
The DR procedure is defined as the series of manual steps that you need to take to recover your HM installation and your HM-managed Postgres database clusters.
...
- The same locations (`locations.beacon.enterprisedb.com` custom resource) used in the old HM instance must be available in the new one. These are an internal resource created during install and aren't available in the HM console. `managed-devspatcher` is the default value.

- The container images used to build the clusters in the old HM instance are available to the new one.

## Required tools

- [Velero CLI](https://velero.io/docs/v1.17/velero-install/).
...
1.  To simplify these process, run following script with your kubeconfig pointing to your new HM installation: 
    ...
    - One called `old-cluster-configs` with the current state of the databases in the new HM installation (default configs after installation). 
    - Another one called `new-cluster-configs` where the script will perform the patches required so the HM-internal databases start using the data from the backups.

    </details> 

    <br/>

1.  Suspend the reconciliation of all HM-internal database clusters, so that you safely remove the Custom Resource Definitions (CRDs) of the database clusters without having the operator recreating them by default: 
...
1.  Apply the yaml file for all the HM-internal database clusters to be re-created:
