...
1.  Suspend the reconciliation of all HM-internal database clusters, so that you safely remove the Custom Resource Definitions (CRDs) of the database clusters without having the operator recreating them by default: 
    ...
    for CLUSTER in $(kubectl get clusters.postgresql.k8s.enterprisedb.io -A -o json | jq -rc '.items[].metadata | select((.name | test("^p-") | not) and (.name != "stats-collector-db")) | {namespace: .namespace}')
    do
      INDEX=$(kubectl get hybridcontrolplane edbpgai -o json | jq '.status.components | to_entries[] | select(.value.name=="my-component") | .key')
      kubectl patch hybridcontrolplane edbpgai --subresource=status --type=json -p "[{\"op\": \"replace\", \"path\": \"/status/components/$INDEX/suspended\", \"value\": true}]"
    done
    ```
