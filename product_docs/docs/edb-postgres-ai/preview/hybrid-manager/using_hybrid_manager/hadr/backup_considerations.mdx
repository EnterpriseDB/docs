...
The following command backs up the Kubernetes metadata for the entire HM installation every 6 hours. Note that `--snapshot-volumes=false` is used because the actual Postgres data is already protected by continuous backups in your object storage.
