...
There is a default storage location in your bucket for these backups. However, HM supports storing backup data into a custom storage location in the same bucket.
...
**Postgres databases** use continuous backups by default, so you can restore them at any point in time. They are limited only by backup lifecycle policies.

[not sure if its helpful to call out that postgres cluster can be restore at any point in time, since we later restore them based on a velero timestamp... should we remove this?]: # 

**Critical HM configuration data**, such as the definition of the Postgres database clusters, is stored as Kubernetes objects and included in the Velero backup. By default, this backup happens daily at 23:00, as defined by the default schedule `velero-backup-kube-state`.
...
[Does this cover both Internal backups (HM control plane data) AND Postgres backups (Postgres database backups or data plane data)?]: # 
