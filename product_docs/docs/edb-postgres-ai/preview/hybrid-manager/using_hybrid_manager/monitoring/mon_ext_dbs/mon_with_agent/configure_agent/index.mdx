...
|---------------------------------------------|------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `provider.onprem.schema_export_max_workers` | N/A                    | Maximum number of workers the HM agent starts in parallel to extract schema from the source database server(s). You can control CPU, memory and disk space usage by the HM agent witht this parameter. See [Performing multiple concurrent schema ingestions](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/monitoring/mon_ext_dbs/mon_with_agent/configure_agent/schema_ingestion/#performing-multiple-concurrent-schema-ingestions) for more information. The default is set to 10. |
...
-   [Perform multiple concurrent schema ingestions](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/monitoring/mon_ext_dbs/mon_with_agent/configure_agent/schema_ingestion/#performing-multiple-concurrent-schema-ingestions) ► Adjust the HM agent configuration to tune HM agent's resource utilization and support the ingestion of a large number of databases.