...
If you are running a multi-DC environment (deployed HM across multiple data centers), you must perform and additional step before performing the upgrade. If your HM is a single-DC instance, skip this section and go to [Perform the upgrade](#perform-the-upgrade) directly.

Due to schema changes in `scenarios` options for `v2026.1.0`, the operator fails to map values from `v2025.12.0` correctly. This triggers an unintended cleanup routine. To mitigate this, the scenarios stanza must be removed prior to initiating the version upgrade.
