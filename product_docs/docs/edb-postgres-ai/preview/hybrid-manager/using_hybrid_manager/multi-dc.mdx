...
- Two kubernetes clusters available: Primary and Secondary (Kubernetes contexts configured).
...
## Configure the MultiDC topology.

Each primary and secondary cluster must be aware of its own role as well the addresses of each other cluster in the MultiDC group. 
...
1. Validation checklist
    ...
    - `location_id`, `internal_backup_folder`, `metrics_storage_prefix` must differ between the primary and the secondary.
