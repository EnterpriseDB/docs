...
Deploying HM across multiple data centers gives you high availability and disaster recovery for Postgres workloads and the Hybrid Manager control plane (CP):
...
- Survive a site loss (DR): Keep a warm Secondary site ready. If the Primary DC is unavailable, promote replicas in the Secondary and restore service.
  ...
  [I think this is wrong, multi-dc is about being able to deploy a database across regions, not having "a second HM" as a backup in another location]: #

- Minimize downtime (HA): Perform maintenance or migrations on one site while workloads continue on the other.

  [Clarify to say that this could be more about workload isolation, e.g. migration workloads on one geography and AI workload in another one]: #

- Protect data (RPO): Continuous replication to a second DC reduces potential data loss compared to single-site backups only.

 [Clarify that this is the case for multi-node PG clusters]:#
...
- Connect two or more HM clusters (Primary, Secondary A, Secondary B) on the same provider/on-prem family. 
  [I guess these must be deployed to different regions?]:#
...
- Wire the Agent (Beacon) so the Primary can register the Secondary as a managed location and provision there (9445/TCP).
  [Agent/Beacon, what is this?]: #

- Prepare a Postgres topology with a primary in one DC and replicas in the other(s); perform manual failover by promoting replicas.

### Current limitations

- Up to three sites (Primary and Secondaries).

- Manual failover: Promote replicas in one of the Secondaries if the Primary is down.

- Same cloud/on-prem family: Cross-CSP multi-DC is not supported.

### Architecture at a glance

- Control plane: Two or three Kubernetes clusters, each with an instance of HM, configured as a hub and spokes; Primary “manages” the Secondaries as a Location through Beacon.
...
- Shared object storage: The standard HM installation requires each individual Kubernetes cluster (HM installation) to have dedicated object storage. In the case of a multi-DC topology, this object storage is shared between all clusters.

### Collect the required information

1. Prepare two (or three) copies of the HM installation configuration file (`values.yaml`) named `primary.yaml`, `secondaryA.yaml`, `secondaryB.yaml`.

1. Domain names:

   Each HM must be configured with a dedicated domain name, set in the installation configuration file (`primary.yaml`, `secondaryA.yaml`, `secondaryB.yaml`) as `portal_domain_name`. This parameter is used by both Primary and Secondaries.

   Create these domain names for the Primary and Secondary clusters (Secondary A and B, if you have 3 data centers), and record this information.
...
All HM installations must have an identical Kubernetes secret named `edb-object-storage` in the default namespace on the Primary and Secondary cluster (Secondary A and B, if you have 3 data centers).
...
1. `location_id` should be set to a human readable location identifier. These values must differ between the clusters. Any human readable name is sufficient.

1. `internal_backup_folder` must match the regex `^[0-9a-z]{12}$`, and must be unique between the Primary and the Secondary locations (Secondary A and B, if you have 3 data centers).

1. `metrics_storage_prefix` must be unique between Primary and Secondary locations (Secondary A and B, if you have 3 data centers).

1. `logs_storage_prefix` must be unique between primary and secondary locations (Secondary A and B, if you have 3 data centers).

## Configure the multi-DC topology

Each Primary and Secondary cluster (Secondary A and B, if you have 3 data centers) must be aware of its own role as well the addresses of the other clusters in the multi-DC group. 
The following configuration stanza describes the topology and must be present in the `primary.yaml`, `secondaryA.yaml`, and if required, `secondaryB.yaml` values files.
...
Fill in the missing domainName parameters using the `portal_domain_name` parameter within the `primary.yaml`, `secondaryA.yaml` and `secondaryB.yaml` files.

Add this stanza to the bottom of `primary.yaml`, and configure the `role` as `primary`.

Similarly, add this stanza to the bottom of the `secondaryA.yaml` and `secondaryB.yaml` files and configure the role as `secondary`.
