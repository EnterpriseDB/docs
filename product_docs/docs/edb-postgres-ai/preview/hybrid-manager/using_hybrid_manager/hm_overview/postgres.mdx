...
-   Health Scores — Scorecards for index, statistics, configuration, and security health of database instances. 
...
-   TDS — Total database size in gibabytes.
...
- Use the **Filter** control to display clusters that match the following properties. Each property has a submenu for choosing the specific type or values related to the property.
  ...
  -   Cluster Type
  -   Engine
  -   Health Score (Index, Statistic, Configuration, Security)
  -   Include Deleted Clusters
  -   Location
  -   Management
      —   Hybrid Manager - Clusters created and managed within the Hybrid Manager.
      -   Cloud Hosted Databases - External clusters managed by a cloud service provider.
      -   External CNP Clusters - External clusters running open-source or EDB versions of CloudNativePG (for example, EDB Postgres AI for CloudNativePG Global Cluster or EDB Postgres AI for CloudNativePG Cluster).
      -   Self Managed — External clusters running on a server, VM, etc. This excludes external EDB Postgres Distributed clusters.
  -   Postgres Version
  -   Project Name
  -   Status
...
To see how often the data on the table is refreshed, refer to the values provided for **Estate page > Postgres table** in [Metrics latency](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/monitoring/monitoring_console/metrics_latency/).
