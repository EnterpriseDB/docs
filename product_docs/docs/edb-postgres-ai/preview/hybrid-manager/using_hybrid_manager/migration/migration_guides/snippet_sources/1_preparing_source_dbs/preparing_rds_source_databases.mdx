...
-  Optionally, `max_wal_size` set to the recommended setting of 8 GB or higher for [improved migration performance](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/specs/#wal-configuration)
...
3.  [Modify parameters in a DB cluster parameter group](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_WorkingWithParamGroups.Modifying.html) using either the AWS Management Console, AWS CLI, or RDS API.
    ...
    - Aurora-postgresql can not modify `max_wal_size` as it is managed by AWS. Only normal postgresql can set `max_wal_size` parameter in the parameter group.
