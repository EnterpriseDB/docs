...
Specifies the URL of the Data Migration Service that the EDB DMS agent uses to perform migrations. The `RW_SERVICE_HOST` leverages the DMS (transporter) service for migrations and derives from `TRANSPORTER_RW_SERVICE_DOMAIN_NAME`, a domain name established by HM administrators or installers for migration component communication.
