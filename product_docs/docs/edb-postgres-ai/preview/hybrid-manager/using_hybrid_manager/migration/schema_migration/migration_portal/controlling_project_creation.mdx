...
The maximum size of Migration Portal projects is 300 MB. If the schema ingested is larger than that, the Migration Portal will require you to [create Migration Portal projects manually](hm_schema_migration/#creating-a-migration-portal-project-manually). However, there are scenarios where setting a different maximum size for automatically created projects is preferred:

- Project size control
- Cost and resource management
- Workflow control

To alter the default maximum size of Migration Portal projects created automatically, you must edit the `beacon_server` ConfigMap in your HM Kubernetes cluster and restart the server deployment. The `beacon_server` is the service that controls Migration Portal project configurations.
...
To disable automatic Migration Portal project creation, you currently have to apply a workaround where you configure the maximum size of the automatic project creation to such a low number that HM fails and you are prompted to create projects manually. See [Known Issues]() for more information.
