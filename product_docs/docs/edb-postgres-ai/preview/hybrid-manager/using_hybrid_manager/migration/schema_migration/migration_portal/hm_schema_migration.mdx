...
If you registered an Oracle database with Hybrid Manager via the HM agent, a Migration Portal project is **created automatically** for all the database schemas and dependencies, unless the size of the ingested DDL exceeds the default limit of 300 GB, a [limit customized by yourself](controlling_project_creation/#customizing-the-maximum-size-of-projects) or if you [disabled Migration Portal project autocreation](controlling_project_creation/#disabling-automatic-migration-portal-project-creation). 
...
If you see a warning in the HM console because the Migration Portal project creation failed, it means the size of the schemas is too large. In this case, you will have to create Migration Portal projects manually, while ensuring the size of each project doesn't exceed the default or established limit.

You can also choose to create projects manually for projects if you disabled the automatic Migration Portal.
