...
There is a known issue affecting the internal `beacon-server` service where setting the `disable_auto_create_mp_projects: true` parameter in the ConfigMap doesn't prevent the automatic creation of Migration Portal projects.
...
The automatic creation of Migration Portal projects fails (with error message `failed to gestate`) if the `resource_id` specified for the registered Oracle database contains characters that are invalid.
...
**Workaround:** To ensure successful automatic creation of Migration Portal projects, the `resource_id` value specified in the HM Agent configuration for Oracle databases must adhere to the rules for unquoted Oracle identifiers (for example, using only letters, numbers, and underscores). Ensure that the `resource_id` doesn't contain hyphens or other special characters.