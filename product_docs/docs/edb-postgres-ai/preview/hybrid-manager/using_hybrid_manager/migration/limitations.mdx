...
!!!tip 
Resolved in HM version 2026.01. See [Disabling the automatic creation of Migration Portal projects](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/schema_migration/migration_portal/controlling_project_creation/#disabling-the-automatic-creation-of-migration-portal-projects) if you have version 2026.01 or later.
!!!

There is a known issue affecting the internal `beacon-server` service where setting the `autocreate_mp_projects: false` parameter in the ConfigMap doesn't prevent the automatic creation of Migration Portal projects.
...
Further improvements are planned in subsequent release to remove this limitation.
