...
### SSL connection error (x25519) when migrating schema to EPAS 18

[I need to link this issue to a Jira item for monthly tracking]: #

**Description:** When migrating a schema to an HM-managed EDB Postgres Advanced Server (EPAS) database cluster version 18 using the Migration Portal online migration option, the connection may fail with an SSL error (`Failed to connect: Failed to initialize pool: SSL error: Cannot decode named group: x25519`). This is typically due to the FIPS-enforced environment enabled by default on HM-managed clusters.
This occurs because PostgreSQL 18 defaults to the x25519 SSL group, which is currently not supported by certain components used by the Migration Portal application. This issue specifically affects the **Online Migration** option where the destination database is a cluster provisioned and managed using HM.

**Workaround:** To resolve the connection failure, you must manually change the supported SSL groups on the destination database cluster to a group compatible with all Migration Portal components (such as prime256v1). For example:
