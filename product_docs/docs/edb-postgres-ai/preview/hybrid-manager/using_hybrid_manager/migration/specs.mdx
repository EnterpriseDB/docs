...
HM and DMS are designed to handle multiple concurrent migrations. However, when performing bulk migrations (a high volume of concurrent or large migrations), increasing the number of Kafka brokers can significantly improve data migration performance.
...
1.  Locate the `broker_number` field in the ConfigMap and increase the value to your desired number of Kafka brokers (for exmaple, `broker_number: 3`).
