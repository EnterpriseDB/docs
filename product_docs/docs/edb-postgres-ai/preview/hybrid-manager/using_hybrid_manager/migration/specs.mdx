...
HM and DMS are designed to handle multiple concurrent migrations. However, when performing bulk migrations (i.e., high volumes of concurrent or large migrations), increasing the number of Kafka brokers can significantly enhance data migration performance.

!!!Tip
You will notice that the default DMS configuration is insufficient for bulk migration if you observe high CPU and memory utilization on the Kafka broker(s) during a data migration. Use [Grafana](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/monitoring/monitoring_grafana/) to check the resource usage for the Kafka pods in the `transporter-kafka-operator` namespace.
!!!

### Procedure to increase kafka broker count
...
1.  Before making any changes, check if a Kafka cluster has already been provisioned. This is the case if you have previously run a data migration.

    ```shell
    kubectl get kafka -n transporter-kafka-operator
    ```

1.  Remove existing migrations and kafka cluster. If the command output lists a Kafka resource, you **must** first delete all migrations to ensure a clean state.

    !!!warning
    The Kafka cluster is generally deployed to support all data migration activities across the entire HM instance. Removing the Kafka resource will halt all active data migrations, regardless of the project. Ensure you coordinate this change with other users.
    !!!

    1.  Navigate to the HM console. 
    
    1.  Delete all active and completed migrations in your project under **Migrate > Migrations**.

    <details><summary>For testing environments</summary> 

    For testing or development environments, you can also remove the kafka cluster using `kubectl`. However, this is only recommended in non-production environments, as it deletes all resources other users may be utilizing: 
...
1.  Locate the `broker_number` field in the ConfigMap and increase the value to your desired number of Kafka brokers (for example, `broker_number: 3`).

1.  Save the ConfigMap. The new configuration will be reloaded and applied automatically within approximately 10 seconds.

1.  You can start new data migrations, which will utilize the increased broker capacity.