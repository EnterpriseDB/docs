...
-   **Snapshot type** &mdash; The replication mode configured for each migration: 
    - **Snapshot** &mdash; The migration runs a snapshot only. The migration runs a snapshot only. Any subsequent changes to the source aren't replicated to the destination.
