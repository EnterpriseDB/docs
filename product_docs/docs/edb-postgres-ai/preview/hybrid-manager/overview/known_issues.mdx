...
### IdP configuration fails after upgrade to version 2026.02

[UPM-78482, backlog]: #

**Description:** In some cases, upgrading from version 2026.01 to 2026.02 can cause your Identity Provider (IdP) configuration to display the **Expiring Soon** status in the HM console. This is caused by a race condition during the migration process. While this issue doesn't occur in every environment, it prevents users from successfully authenticating via their IdP provider if triggered.
...
Execute the following SQL statement on the internal `upm-api` database **before** attempting to re-setup or modify the IdP integration in the HM console:
...
**Description:** The `accm-server` component currently has a hard dependency on a Kubernetes secret named `langflow-secret` (or `upm-langflow` in some configurations). This secret is required even in installation scenarios where AI features and Langflow aren't being used. If the secret doesn't exist, the `accm-server` pod will fail to start with the error `CreateContainerConfigError`.

**Workaround:** To allow the `accm-server` to start in environments where Langflow isn't installed, you must manually create a placeholder secret in the relevant namespace (typically `upm-beacon`).

You can create an empty secret using the following kubectl command:

```shell
kubectl create secret generic langflow-secret -n upm-beacon --from-literal=placeholder=data
```
