...
### Okta configuration fails after upgrade to version 2026.02

[UPM-78482, backlog]: #

**Description:** In some cases, upgrading from version 2026.01 to 2026.02 can cause the Okta Identity Provider (IdP) configuration to show the **Expiring Soon** status. This is caused by intermittent resource contention during the migration process. While this issue doesn't occur in every environment, it prevents users from successfully authenticating via Okta if triggered.

**Workaround:** To resolve the abnormal status and restore the configuration, a cluster administrator must manually update the internal application database. Execute the following SQL statement on the internal `upm-api` database:
