...
For most use cases, we recommend that you use the aidb [preparers](/edb-postgres-ai/current/ai-factory/pipeline/preparers/usage) to perform bulk preprocessing preparation steps on your data.
...
- `overlap_length` (optional): The amount of content to overlap between consecutive chunks (unit depends on the `strategy` used). This helps preserve context across chunk boundaries by duplicating a portion of the content from the end of one chunk to the beginning of the next chunk. Defaults to 0 (no overlap).
...
!!! Tip
This operation transforms the shape of the data, automatically unnesting collections by introducing a `part_id` column. See the [unnesting concept](/edb-postgres-ai/current/ai-factory/pipeline/preparers/concepts#unnesting) for more details.
!!!
