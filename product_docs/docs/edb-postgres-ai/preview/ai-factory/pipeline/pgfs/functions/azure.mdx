---
title: "Pipelines PGFS with Azure Blob Storage"
navTitle: "Azure Blob Storage"
description: "PGFS options and credentials with Azure Blob Storage."
---


## Overview: Azure Blob Storage

The general syntax for using Azure Blob Storage is:
...
PGFS supports the following prefixes and formats to indicate an Azure Blob Storage bucket:
...
| `container_name` | The bucket name.                      |

### Settings for the `credentials` argument in JSON format

Speficy either `account_key` for a shared access key credential, or `client-id`, `client-secret`, and `tenant-id` for client credentials.
