...
* `url` &mdash; The URL of the model to use. This value is optional and can be used to specify a custom model URL.
  * If `openai_embeddings` (or `embeddings`) is the `model`, `url` defaults to `https://api.openai.com/v1/embeddings`.
  * If `nim_embeddings` is the `model`, `url` defaults to `https://integrate.api.nvidia.com/v1/embeddings`.
