...
This reference documentation for EDB Postgres AI - AI Accelerator Pipelines PGFS includes information on the functions and views available in the [pgfs](/edb-postgres-ai/current/ai-factory/pipeline/pgfs) extension. These functions give AIDB access to S3-compatible file systems, Google Cloud Storage buckets, and local file systems.
