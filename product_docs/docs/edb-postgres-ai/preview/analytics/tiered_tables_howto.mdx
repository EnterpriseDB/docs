...
Configure your PGD cluster to point to object storage for analytics and to an external catalog service to write metadata to if you are using one. 


<TabContainer syncKey="intro">
<Tab title="HM console">

When you deploy your PGD cluster, HM automatically provisions a storage location and a write catalog location within your configured object storage at provisioning time.
...
1. Create a storage location using the PGFS extension. See [Creating a storage location](/edb-postgres-ai/current/ai-factory/pipeline/pgfs/functions/#creating-a-storage-location) for details.
