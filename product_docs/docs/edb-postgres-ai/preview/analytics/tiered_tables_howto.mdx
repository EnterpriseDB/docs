...
Configure your PGD cluster to manage your data lifecycle by moving Postgres data to object storage using three different available methods. You may configure PGD data tiering using the Hybrid Manager Console or manually from your PGD cluster command line.

## Prerequisites

- Active Hybrid Manager instance version 1.3.
- Provisioned PGD cluster version 6.0 or later with PGFS extension and PGAA extension version 1.4.0.
...
- Iceberg catalog if working with Iceberg tables.
- Lakehouse Cluster (recommended) for querying offloaded data.

## Initial configuration

Configure your PGD cluster to point to object storage for analytics and to an external catalog service to write metadata to if you are using one. Enable statistics for the node group disks in order to view the impact of tiered storage.

<TabContainer syncKey="intro">
<Tab title="Hybrid Manager Console">

Enable statistics for the node group disks:

1. Find your PGD cluster in the Hybrid Manager Console and select **Quick Actions**.
...
1. Enable statistics for the node group: 
...
1. If performing catalog-managed offloads, configure a connection to an external catalog:
...
1. Set the catalog as the analytics write destination:

      ```sql
      SELECT bdr.alter_node_group_option('default_group', 'analytics_write_catalog', 'catalog-name');
      ```
...
1. Enter the following information:
    ...
    - **Minimum Advance Partitions**: establishes the minimum number of partitions this table has.
    - **Maximum Advance Partitions**: limits the number of partitions to create if the number of partitions falls below the value of **Minimum Advance Partitions**.
    - **Enable real-time analytics replication** triggers repplication for data newer than **Analytics Offload Period** ("hot" data).
...
-- PGD AutoPartition will automatically manage partitions
-- - Create new partitions based on partition_increment
-- - Drop old partitions based on data_retention_period
-- - If enable_replication is set, new partitions will have real-time analytics replication enabled
-- - Move partitions to analytical storage based on analytics_offload_period
-- - All operations happen asynchronously in the background
...
```sql
CALL pgaa.compact_table ('public.transactional_table');
```
...
Attach as many external catalogs as desired to access table data that has been offloaded to object storage by different PGD clusters.

<TabContainer syncKey="compaction">
<Tab title="Hybrid Manager Console">

1. Select your cluster and navigate to **Catalogs**, then select **Add Catalog**.
1. Provide the name of the exsiting catalog and select **Add Managed Catalog**.
1. Find the newly added catalog under **Catalogs**, select **Actions**, **Import**, and confirm.
...
1. You may restore these tables to your local Postgres storage by selecting the **Offload** option.
