...
1. Create a table using the `PGAA` access method. Specify the storage location created in the previous step, the path to either a single Parquet file or a directory containing multiple Parquet files, and the format (currently Parquet). For example:

  ```sql
  CREATE TABLE public.supplier () USING PGAA
      WITH (pgaa.storage_location = 'biganimal-sample-data', pgaa.path = 'tpch_sf_10/customer', pgaa.format = 'parquet');
  ```
