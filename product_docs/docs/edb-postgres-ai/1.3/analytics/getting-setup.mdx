...
> **Note**
> Instructions for adding the snapshot repository to Hybrid Manager and pre-building snapshot operand images will be added here. *(TODO)*
...
* Continue to [Read-only analytics without a catalog: Delta Lake](./read-only-without-catalog.mdx).
* Or, try [Read-only analytics without a catalog: Iceberg](./read-only-analytics-iceberg.mdx).
