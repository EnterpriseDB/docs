...
Review these terms to understand and leverage Hybrid Manager (HM) capabilities.

## Hybrid Manager

Hybrid Manager is the name given to EDB's solution for deployment, management, monitoring and automation of Postgres and Lakehouse databases and sovereign AI systems. 
...
A key feature of the Hybrid Manager is the ability to provision and manage databases on Kubernetes. This capability is referred to as a Private Database-as-a-Service (DBaaS). 
...
The Engineered System is Hybrid Manager in a box, a ready-to-go physical server that you can slot into a rack and deploy as a containerized appliance. This deployment method offers a flexible and portable solution for organizations seeking to streamline their database operations in a hybrid or multi-cloud world.

## PG AI Console

The PG AI Console is a web-based graphical interface that provides users with a centralized point of interaction for the Hybrid Manager. From this dashboard, you will be able to deploy and manage databases, connect and monitor external databases, perform data migrations, manage user permissions, deploy AI workloads and more.
...
Within the scope of the Hybrid Manager, a Kubernetes cluster represents the foundational infrastructure upon which the Hybrid Manager software itself is deployed as a self-contained instance. Hybrid Manager leverages the capabilities of this underlying Kubernetes cluster to manage Postgres database deployments, with [CloudNativePG](https://www.enterprisedb.com/docs/supported-open-source/cloud_native_pg/) as its tool for orchestrating Postgres on Kubernetes.
...
This is a subtype of a Kubernetes cluster that you can also use to deploy the Hybrid Manager.

!!!note

    This functionality will be added in a later version.

Typically, all nodes of a Kubernetes cluster reside in a single Availability Zone. For Hybrid Manager deployments with high availability (HA), advanced high availability (AHA) or distributed high availability (DHA) enabled, the platform automatically handles primary node failures by initiating a seamless takeover by a standby node.
...
Database clusters can either be created within the Hybrid Manager or created externally but then connected to the Hybrid Manager for observability. See [Database cluster types in the Hybrid Manager](hm_terminology.mdx#database-cluster-types-in-hybrid-manager) for an overview of all cluster types you can create or connect to the Hybrid Manager.
...
## Database cluster types in Hybrid Manager

The Hybrid Manager instance is a centralized pane of glass that can host, manage and monitor several database cluster categories. Some database clusters are created and completely maintained within the Hybrid Manager, while others are created externally but connected to Hybrid Manager to provide monitoring and migration capabilities.

![Cluster Types in Hybrid Manager](../images/cluster_types_HM.svg)
...
HM-managed clusters are database servers you created with Hybrid Manager. They are deployed in the same Kubernetes cluster as the Hybrid Manager, using its provisioning capabilities. In the **Estate** page, HM-managed clusters have the **Hybrid Manager** label under the **Management** column.

**Examples**: Single Node, Primary/Standby High Availability (HA), Advanced High Availability (AHA), or Distributed High Availability (PGD) database instances with PostgreSQL, EDB Postgres Advanced Server, or EDB Postgres Extended Server that were deployed using Hybrid Manager.
...
**Lakehouse Analytics - Single Node:** You can also create [Lakehouse Analytics](../using_hybrid_manager/hm_overview/lakehouse/) clusters with the Hybrid Manager. PG AI Lakehouse Analytics clusters provide an environment for querying and analyzing data stored in open lakehouse formats (like Delta Lake) within object storage. Leveraging Hybrid Manager's unified control plane, users can easily deploy and manage these specialized clusters optimized for fast analytical queries. 

Hybrid Manager simplifies the deployment and operation of these analytics engines, accelerating data-driven decision-making.
...
Self-managed clusters are external Postgres, or Oracle clusters (created outside of Hybrid Manager). They are deployed using other provisioning capabilities than the Hybrid Manager (e.g. manually), and are often deployed on a different machine than Hybrid Manager. 

In the **Estate** page, Self-managed clusters have the **Self-Managed** label under the **Management** column.

You can connect self-managed clusters to Hybrid Manager for observability with the [Agent](monitoring/mon_ext_dbs/mon_with_agent) and perform migrations with the [Data Migration Service](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/).
...
!!!note
    ...
    Self-managed Oracle databases that you connect to Hybrid Manager don't appear under **Estate** > **Postgres**. Switch to **Estate** > **Migrations** to see information.

    Self-managed Postgres clusters that you connect to the Hybrid Manager appear in both **Estate** > **Postgres** and **Estate** > **Migrations**.

#### Cloud Service Provider-managed cluster

Cloud Service Provider-managed clusters or CSP-managed clusters are external clusters (created outside of Hybrid Manager) that host one or more occurrences of a Postgres instance. They are deployed using Cloud Service Provider (CSP) provisioning capabilities and their major operational tasks are handled by the CSP (backups, upgrades, failover, etc). However, architecture, developer, and application-specific activities are handled by you.

In the **Estate** page, Cloud Hosted Database clusters have the **Cloud Hosted Databases** label under the **Management** column.

You can [connect CSP-managed clusters to Hybrid Manager for observability](monitoring/mon_ext_dbs/mon_ext_csp) and perform migrations with the [Data Migration Service](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/).

**Examples**: Postgres on AWS RDS.

#### External CNPG cluster

External CNPG database clusters are external database clusters created on a Kubernetes environment outside of Hybrid Manager with CloudNativePG. They are deployed using the provisioning capabilities of open-source CloudNativePG, EDB CloudNativePG Cluster or EDB CloudNativePG Global Cluster. 
...
The term "data node" is used to refer to a Kubernetes worker node specifically designated to host Postgres database instances managed by Hybrid Manager.
