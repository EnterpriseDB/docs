...
Loglevel for the whole Quarkus application. Setting this to DEBUG might produce logs that are too verbose. It is recommended to use `QUARKUS_LOG_CATEGORY__COM_ENTERPRISEDB__LEVEL` instead

### QUARKUS_LOG_CATEGORY__COM_ENTERPRISEDB__LEVEL

Loglevel for the classes in the com.enterprisedb package. Setting this to DEBUG will only affect the classes relevant to the logic of the agent.
