...
The EDB Data Migration Service runs within the same Kubernetes environment where your HM is deployed (on-premises, AWS, Red Hat OpenShift, and Google Cloud). This means that most of the workloads required for migration operations are managed and optimized by your Kubernetes cluster.

Generally, you don't need to extensively fine-tune resources, as your Kubernetes environment meeting the general requirements for running HM should ensure a smooth migration. However, for large datasets, optimizing specific components can significantly accelerate the process. 
...
The HM and DMS are designed to handle resource usage automatically. For datasets smaller than 100 GB, they're configured to perform fast migrations without further tuning. However, for datasets larger than 100 GB, a few targeted alterations can lead to significantly better performance and quicker migrations.
...
-   Separate WAL and data volumes: Aim for storage volumes that can handle a minimum of 2500-3000 IOPS and at least 200-250 MB/s of throughput for both your data and write-ahead log (WAL) volumes. We highly recommend sing separate volumes for WAL and data for optimal performance.
...
-   Adjust WAL and data volume allocation by [editing](../../cluster_management/manage-clusters/edit-clusters/#editing-a-cluster) your cluster's storage.

-   By default, HM-managed clusters use separate volumes for data and WAL volumes, but if you cleared the **Use a separate storage volume for Write-Ahead Logs** option during cluster creation, you can't enable this setting after creation.
...
The destination database cluster needds at least 4 CPU cores. A lower core count will negatively impact migration performance.
