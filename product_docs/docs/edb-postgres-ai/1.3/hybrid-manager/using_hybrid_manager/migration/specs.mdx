...
Adjust WAL configurations by [editing](/edb-postgres-ai/hybrid-manager/using_hybrid_manager/cluster_management/manage-clusters/edit-clusters/#editing-a-cluster) your cluster's **DB Configuration**. Search for the `max_wal_size` parameter, provide the new size in MB and save to apply the configuration. 
