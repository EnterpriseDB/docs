...
### Renaming schema, tables and columns during a migration

When you create a migration with the HM console, the Data Migration Service allows you to map your source schema, table and column names to different destination schemas, tables and columns. This is particularly useful for restructuring your database during a migration, enabling you to align table names with new naming conventions or improve clarity.

#### Prerequisites 

The DMS requires that all destination objects already exist in the destination database before the data migration begins. 

In the scope of schema migration, for example, using a utility like Migration Toolkit of pg_dump/pg_restore, the schemas you imported into the destination will have the same naming as in the source database.

If you plan to rename an object during mapping (e.g., source table `T1` to destination table `T2`), you must ensure the destination object (`T2`) has been manually created or renamed in the destination database prior to initiating the data migration. The structure and data types of the renamed destination object must match the source object exactly.
...
1.  To rename schemas:
    ...
    1. Locate the table you wish to rename in the list (listed in `<schema>.<table>` format).
    1. Select the pen icon next to the schema name.
...
1.  To rename columns: 
    ...
    1. Expand one of the tables from the list (listed in `<schema>.<table>` format) with the dropdown arrow. 
    1. Locate the column you wish to rename in the **Source Column** list. 
