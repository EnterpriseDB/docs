...
**Tables**: The **Tables** tab displays a list of all tables selected for the migration. For each table, you can view the source and destination table names, schemas, overall migration status, and the count of columns selected. Each row is expandable, allowing you to view the detailed column-level mapping from the source to the destination.
