---
title: Running the EDB Postgres AI agent as a service for Oracle databases
navTitle: Configure agent to run as a service for Oracle databases
description: Learn how to run the EDB Postgres AI agent as a service when setting up monitoring for Oracle databases with HM.
---

For continuous metric streaming and to ensure your database remains visible in your EDB Postgres AI estate, configure the agent to run as a service. Running as a service allows it to start on system startup and restart after any errors.
