...
Now that you have configured the source database, validate your Oracle configuration. For this, you may use the configuration validation script packaged with the DMS Agent.
...
1.  Run the script with the necessary parameters. In this example:
    ...
    ```bash
    ODB_SYSDBA_USERNAME=sys ODB_SYSDBA_PASSWORD=password ODB_HOST=localhost ODB_PORT=5432 ORACLE_SID=postgres ODB_DBZ_USERNAME=debezium ODB_DBZ_PASSWORD=dbz ./oracleConfigValidation.sh "${arr[@]}"
    ```
