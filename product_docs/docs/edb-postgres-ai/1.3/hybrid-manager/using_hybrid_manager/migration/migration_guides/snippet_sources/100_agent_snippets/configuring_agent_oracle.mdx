...
The DSN is the connection string the HM agent uses to connect to your Oracle source database. Set the `databases.dsn` parameter using an environment variable `$DSN` (recommended) or direct URL format.
...
If you plan to connect to more than one database with then HM agent, you can specify multiple such connection strings, for example `$DSN1`, `$DSN2`. 
...
1.  Copy and paste the following template into the new file. This template is designed to enable the HM console to generate a migration assessment, as well as ingest all schemas for assessment and import.
    ...
    In this template, `$BEACON_AGENT_ACCESS_KEY` and `$DSN` are configured to fall back on environment variables you configured previusly. Replace all `< >` placeholders in the template following the [Parameter reference](/edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/monitoring/mon_ext_dbs/mon_with_agent/configure_agent/#parameter-reference).
...
- If your administrator requires some guidance on how to obtain the values for the root certificate and `beacon_server`, see [Administrative tasks](/edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/monitoring/mon_ext_dbs/mon_with_agent/admin_tasks/).
