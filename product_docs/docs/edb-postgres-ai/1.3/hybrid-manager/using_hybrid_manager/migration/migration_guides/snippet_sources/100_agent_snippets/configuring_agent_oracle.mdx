...
description: Configure the PGAI agent to connect to your Oracle database.
---
...
[Create a machine user and access key with the `estate ingester` role](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migration_guides/oracle_to_hm_managed/agent_creating_machine_user/) assigned to it. 
...
The PGAI agent requires specific configuration values that are set during the HM installation. Only an administrator or installer can access these. Contact the administrator or installer of your HM instance to request the following values:
...
The DSN is the connection string the PGAI agent uses to connect to your Oracle source database. Set the `databases.dsn` parameter using an environment variable `$DSN` or direct URL format.
...
If you plan to connect to more than one database with the PGAI agent, you can specify multiple such connection strings. For example, if you're using environment variables for the `database.dsn` settings, specify `$DSN1`, `$DSN2`.
...
1.  Create an PGAI agent configuration directory in your home directory:
    ...
    The PGAI agent looks for its configuration file starting with `/etc/beacon`, then `${HOME}/.beacon`. As a final fallback, it searches the directory from which it's executed.
    !!!
...
After configuring the PGAI agent, you can run it or configure it to run as a service.

## Recommendations for setting environment variables

Although you can set all parameters directly in the config file, we recommend handling sensitive data, such as `$BEACON_AGENT_ACCESS_KEY` or `$DSN`, as environment variables managed through a secrets manager. This strategy prevents sensitive credentials from being stored in plaintext configuration files. If you use environment variables, make sure they're available to the PGAI agent while it's running.
