...
-   You [created a machine user and access key with the `estate ingester` role](/edb-postgres-ai/hybrid-manager/using_hybrid_manager/migration/migration_guides/oracle_to_hm_managed/agent_creating_machine_user/) assigned to it.
...
1.  Request the administrator or installer of your HM instance to provide the host name they set for the HM Agent in the Helm chart configuration file used for installation. For this, they'll have to look up the value they set for `parameters.upm-beacon.server_host` (or `BEACON_SERVICE_DOMAIN_NAME`) in the `values.yaml` file. 
    
    Export it as an environment variable for this terminal session, so the HM Agent can access the correct service endpoint:
...
1.  You need the project ID for the project you are using for this migration. From the HM console, navigate to your project. In the URL displayed in the browser, look for the ID with the `prj_` prefix. For example, if your browser displays this URL:
    ...
    Export it as an environment variable for this terminal session, so the HM Agent can access the project you are using for this migration:
