...
1.  Configure `max_wal_senders` appropriately.
...
1.  Configure `max_replication_slots` appropriately.
...
1.  Configure `max_wal_size` for adequate WAL LSN lifetime.
...
1.  Run the script with the necessary parameters. DBZ_USER here refers to the user for cdc (the value of MIGRATION_ROLE above); DBZ_PASSWORD here refers to the password you set for the user of the cdc (the value of MIGRATION_ROLE_PASSWORD above) For example:
