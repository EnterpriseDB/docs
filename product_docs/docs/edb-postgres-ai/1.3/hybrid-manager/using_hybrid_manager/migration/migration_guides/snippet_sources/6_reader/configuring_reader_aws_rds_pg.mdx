---
title: "Configuring and running the EDB DMS agent in reader mode for AWS RDS/Aurora PostgreSQL sources"
navTitle: "Configuring DMS agent for AWS RDS/Aurora PostgreSQL"
deepToC: true
...
## Configuring the EDB DMS agent for migrations from AWS RDS for PostgreSQL or Aurora PostgreSQL

Set the variables in the script according to your environment and uncomment the edited lines. See [parameters](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/dms_agent_reference/) for more guidance. The following shows the unedited script:
...
# 'writer' mode is used to write changes to a destination database
...
# The resource_id is configured by the customer in the configuration file of the PGAI agent
# need to make sure that the resource_id is set to the same value as in the PGAI agent
...
!!!note
    ...
    Set the `DBCONFIG_DATABASES_0__HOSTNAME` value to point to the RDS endpoint you can find under **Connectivity & security** in the AWS Console. A redacted example is `foo_database.cb7asw1xxxxx.eu-west-0.rds.amazonaws.com`.
