...
To perform logical replication, the EDB DMS Reader requires `rds.logical_replication` set to `1`, `wal_level` set to `logical`, and, optionally, `max_wal_size` set to `8 GB`.
...
5.  Verify that the settings have been set correctly by connecting to the database and checking whether `wal_level` is now set to `logical` and the size of `max_wal_size`. 
