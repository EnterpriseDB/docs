...
-   **HM-managed destinations**: For migrations to HM-managed destinations, you must ensure `edb_admin` is the owner of the destination database, schema, and table in Hybrid Manager (HM) environments 1.1 and earlier. Otherwise, the migration will fail. This limitation is fixed in HM versions 1.2 and later.
