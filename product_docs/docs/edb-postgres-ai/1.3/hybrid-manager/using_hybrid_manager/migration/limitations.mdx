...
- **Performance planning (external destinations)**: While performance generally remains strong with the [recommended specifications](specs), if you run a large volume of migrations (five or more) to the same destination database, you should consider allocating additional CPU, memory, and volume IOPS/throughput to the destination to prevent resource contention, specially if the destination is an external database. 

- **Resource management**: Each migration requires its own instance of the EDB DMS Reader and, in the case of external destinations, of the EDB DMS Writer, even if the migration has the same destination as another migration.
