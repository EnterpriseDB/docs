...
- **Resource management**: Each migration requires its own instance of the EDB DMS Agent in reader mode and, in the case of external destinations, of the EDB DMS Agent in writer mode, even if the migration has the same destination as another migration.
