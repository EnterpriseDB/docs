...
description: Learn about migration assessments that are available to you through the HM Console.
navigation:
...
One of the main features of the Hybrid Manager is that it facilitates planning and performing database migrations.

After you have registered an external Oracle or Postgres database with the Agent (for example, [registered an Oracle database with the Agent](/edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/migration/migration_guides/oracle_to_hm_managed/agent_configuring_oracle/)), the **Migrations** tab that is accesible through the **Estate** page displays a list of external databases you have registered among with a migration assessment that you can use for planning.

This is your first stop in the HM Console when migrating external databases to the Hybrid Manager.
...
**Complexity:** This value is generated only for Oracle schemas and provides the level of complexity to migrate the database to the Hybrid Manager. Possible values are Very Easy, Easy, Medium, Hard, and Very Hard.
...
**Migration LoE:** This value is generated only for Oracle schemas and represents the level of effort, in 8-hour working days, required to convert the schema and migrate data to the Hybrid Manager. The estimation corresponds to the effort applied by members of EDB’s professional services who specialize in Oracle to Postgres migrations. This value considers database and schema-level assessments that include database features, schema complexity, data volume and other factors to estimate the number of days.
...
**Management:** Shows whether the database is managed by yourself (self-managed), by a cloud service provider or by the Hybrid Manager.
