...
When you need to automate or script Postgres cluster creation (as opposed to doing everything by hand in the web UI), you can do so using bash scripts and the Hybrid Manager API.

Automating Postgres cluster creation makes it easier to make reproducible builds. The scripts and config files that are used to create Postgres clusters can be checked into source control for documentation/tracking purposes.
...
1. In the upper-left corner, select **Projects** and navigate to your project.
  (We need to have your project "in context" before assigning permissions
   to the machine user you just created, because permissions are per-project.
   Navigating to your project correctly sets this context.)
