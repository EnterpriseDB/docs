...
Now that you have created a cluster via the API, here is how to get your new cluster via the API. Usefully, getting clusters in JSON format like this is useful for seeing what the JSON fields are when creating clusters, reducing looking up every single field when creating clusters.
