...
You can create a database cluster using bash scripts and the Hybrid Manager API.
...
Create a file named `env.sh` which includes the access key you created on a previous step, your project ID, and your Hybrid Manager (HM) console host:
...
Create a shell script named `02-create-psr-cluster.sh` that uses `create-psr-cluster.json` to create a cluster:
...
Run the script to output the content of the cluster into a JSON file:
...
1. At the bottom-right of the HM Console, select **API**. 
