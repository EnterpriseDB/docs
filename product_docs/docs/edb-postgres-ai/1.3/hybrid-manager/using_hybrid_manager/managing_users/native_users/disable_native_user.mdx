...
1.  To disable native user login, unset the `staticPasswords` configuration by setting `portal.authentication.staticPasswords` to `[]`:
    ...
    portal:
      authentication:
        staticPasswords: []
    ```
1.  [Apply the DEX configuration](/edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/managing_users/apply_dex_config/) again to populate the changes.
