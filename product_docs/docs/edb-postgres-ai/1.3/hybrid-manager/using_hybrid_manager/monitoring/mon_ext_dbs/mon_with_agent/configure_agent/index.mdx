...
2.  Ensure that following environment variables are available in your configuration.
    ...
    -   Set the `DSN` environment variable to the Data Source Name (DSN) of the database you want to monitor. Use the following format:
        ...
        When connecting to a Postgres database, you must include `?sslmode=disable` to ensure the connection succeeds. For example, to connect as user `postgres` authenticated with password `password` to a database `postgres` on port 5432 you would specify:
        ...
        !!!note
        The `?sslmode=disable` parameter instructs the Agent to use an unencrypted connection. This bypasses the default `require` or `prefer` database requirements for SSL certificates, enabling a connection with Hybrid Manager.
        !!!
