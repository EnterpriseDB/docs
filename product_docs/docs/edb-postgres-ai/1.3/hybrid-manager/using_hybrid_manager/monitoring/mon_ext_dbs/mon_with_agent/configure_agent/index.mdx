...
-   You need the project ID for the project where you want to monitor the database. To get the ID, navigate to your project and fetch the ID from the URL displayed in the browser.
...
1.  Ensure that following environment variables are available in your configuration.
    ...
    - Set `BEACON_AGENT_ACCESS_KEY` to the access key you got from the console when you [created a machine user](../create_machine_user.mdx).
...
1.  Edit the placeholders denoted by `<>` with appropriate values.
    See [Parameter reference](../configure_agent.mdx#parameter-reference) for explanations of each parameter and how to set the values.

## Test the agent locally

To perform an initial test of the agent, you can configure it to send the data to your terminal (standard output). (Usually it's sent to HM.) This test helps you quickly verify whether the agent can successfully collect data from the source database and lets you preview the gathered information.
...
| `host.metrics.disabled`              | N/A             | Set to `true` to disable collecting host metrics from the host on which the agent is located.                                                                                                                                                    |
