...
1.  Set the `DSN` environment variable to the Data Source Name (DSN) of the database you want to monitor. Use the following format:
    ...
    To connect as user `postgres` authenticated with password `password` to a database `postgres` on port `5432` you would specify:
    ...
    <Tab title="Special characters in passwords">

    The Agent only supports ASCII characters in user passwords. If your Postgres or Oracle user's password contains special, non-ASCII characters, use the URL-encoded version of the password in the DSN string (or password environment variable). For example, if your password is `pa$$word`, you must encode the `$$` characters as `%24%24`:
