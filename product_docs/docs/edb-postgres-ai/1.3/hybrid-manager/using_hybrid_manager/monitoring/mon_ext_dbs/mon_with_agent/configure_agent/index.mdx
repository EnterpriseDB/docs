...
navTitle: Configure Beacon Agent
description: Learn how to configure Beacon Agent for monitoring with Hybrid Manager.
---

Configure Beacon Agent (deployed as `beacon-agent`) to allow it to connect to Hybrid Manager (HM) and fetch monitoring data from the source database.

!!!info
    You can use the agent for different purposes. If you don't want to use your agent for monitoring, but to assess your Oracle database for migration, leave this page and see [Migrating Oracle databases](/edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/migration/migration_guides/oracle_to_hm_managed/).
...
1.  Ensure that following environment variables are available in your configuration.
    These environment variables must be permanently available as they're required each time the agent starts.

    !!!info
       It's possible to set these values directly in the config files. However, because they contain secrets, we recommend handling them as environment variables managed through a secrets manager.

    -   Set `BEACON_AGENT_ACCESS_KEY` to the access key you obtained from the HM console when you [created a machine user](../create_machine_user.mdx).
    ...
    - Set the `DSN` environment variable to the data source name (DSN) of the database you want to monitor following this format:
        ...
        ```shell
        export DSN="postgresql://postgres:pa%24%24word@localhost:5432/postgres?sslmode=disable"
        ```
        !!!
...
1.  Create a configuration directory in your home directory:
    ...
    !!! Note
        If this location isn't convenient, you also use either:
...
1.  Edit the placeholders denoted by `<>` with appropriate values.
    See [Parameter reference](../configure_agent.mdx#parameter-reference) for explanations of each parameter and how to set the appropriate value.

## Test the agent locally

To perform an initial test of the agent, you can configure it to send the data to your terminal (standard output) instead (Usually it's sent to HM.) This test helps you quickly verify whether the agent can successfully collect data from the source database and lets you preview the gathered information.
...
You can validate whether the agent can connect to HM using the specified credentials by making a test connection to HM and printing a summary of your configuration.
...
!!! note
    ...
    If the **Connectivity** check fails with a message like `failed to verify certificate x509...` this likely means that the system on which you're running the agent doesn't trust the certificate used to sign the server certificate.
    This is likely a symptom of an issue with root certificate distribution. Raise this issue with the team that manages your infrastructure.
    ...
    After setting these parameters, rerun `beacon-agent --config` to check whether the issue is resolved.

    If the the **Connectivity** check fails with a message like `connection reset by peer` this generally means the agent can't reach the specified `<beacon-server>` because the URL is incorrect or there's a firewall preventing connection.
...
You can edit these settings in the agent's configuration file.

| YAML agent settings                  | Placeholder            | Guidelines                                                                                                                                                                                                      |
|--------------------------------------|------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `agent.access_key`                   | N/A                    | Access key you obtained from the HM console when you [created a machine user](../create_machine_user.mdx).                                                                                                      |
...
| `agent.project_id`                   | `<project-id>`         | To obtain your project’s ID, go to the HM console, select **Projects**, and select your project. The ID is the identifier starting with “prj\_” in the URL of that page.                                        |
...
| ------------------------------------ | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `databases.resource_id`              | `<resource-id>` | Assign an identifier to your database to link your database to an identifier. Surround with double quotes. Must be unique in the project.                                                                                                       |
...
| `databases.poll_interval`            | N/A             | Specifies how often HM polls the database for basic metadata. Provide the value in minutes (m) or seconds (s).                                                                                                                   |
| `databases.tags`                     | N/A             | Assign tags to label the migration of that specific database. Enclose in double quotes and introduce each tag with a dash.                                                                                                                               |
| `databases.disable_host_association` | N/A             | Set to `true` to prevent this database from being associated with the host on which the agent is running, for example, if the agent is running on a different server.                                                                                        |
| `host.resource_id`                   | `<host-id>`     | Assign an identifier to the host on which the agent is running. By default this is populated by the hostname detected during setup. This setting links your host machine to an identifier. Enclose the value in double quotes. Must be unique in the project. |
...
|----------------------------------|------------------------|----------------------------------------------------------------------------------------------------------------------------------------|
| `databases.schema.enabled`       | N/A                    | Set to `true` to enable schema ingestion for the database. Set to `false` to disable it.                                               |
| `databases.schema.poll_interval` | N/A                    | Defines how frequently the Agent checks for schema changes. Provide a value in seconds or minutes (for example, 15 s or 1m).          |
| `databases.schema.filter`        | N/A                    | Use this block to specify which schemas to include or exclude from ingestion. It contains the `mode` and `names` parameters.           |
| `databases.schema.filter.mode`   | `include` or `exclude` | Specifies the filtering behavior. Use `include` to only ingest listed schemas, or `exclude` to ingest all schemas except those listed. |
| `databases.schema.filter.names`  | `<schema_name>`        | A list of strings representing the names of the schemas to be filtered. This list works in conjunction with the mode setting.          |
