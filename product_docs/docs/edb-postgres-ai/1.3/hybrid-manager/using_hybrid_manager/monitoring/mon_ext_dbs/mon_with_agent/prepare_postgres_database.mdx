...
When configuring the agent, you must first connect to the self-managed database you want to monitor and create a user that the agent will use to connect to the database. This can be any Postgres user with the `pg_monitor` role assigned.

For this example, create a new user `agent_user`:
...
However, as a matter of security best practice, we advise you run the agent using a [dedicated OS user](agent_os_user/).
