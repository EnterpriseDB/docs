...
Enable log collecting by setting `general.metrics.push.include_logs` to `true`. Disabled it at the agent level by setting this value to `false`.
...
If both options of these options are disabled, the **Query Diagnostics** tab becomes disabled.
