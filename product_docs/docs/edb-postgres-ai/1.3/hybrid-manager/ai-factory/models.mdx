...
The [Model Library](/edb-postgres-ai/current/ai-factory/model/library/) serves as your centralized governance system for AI model images. Operating within Hybrid Manager's Asset Library infrastructure, it provides a curated view of validated models ready for production deployment.
...
Models in the library power all AI Factory capabilities including [Gen AI](/edb-postgres-ai/current/ai-factory/gen-ai/) assistants, [Knowledge Base](/edb-postgres-ai/current/ai-factory/pipeline/knowledge_base/) pipelines, and custom inference applications. Only models validated through the library's governance framework can reach production environments.

### Model Serving

[Model Serving](/edb-postgres-ai/current/ai-factory/model/serving/) transforms approved models into scalable inference endpoints using KServe within your Kubernetes clusters. This infrastructure provides production-grade model deployment with automatic scaling, health management, and resource optimization.

Key serving capabilities include:

-   [**InferenceService**](/edb-postgres-ai/current/ai-factory/model/create-inferenceservice/) resources that define deployed model endpoints
-   [**ServingRuntime**](/edb-postgres-ai/current/ai-factory/model/configure-servingruntime/) configurations optimized for different model frameworks
-   GPU allocation and scheduling for high-performance inference (see [Setup GPU](/edb-postgres-ai/current/ai-factory/model/setup-gpu/) and [Update GPU resources](/edb-postgres-ai/current/ai-factory/model/update-gpu-resources/))
...
1.  **Runtime Configuration**: Select or create [ServingRuntimes](/edb-postgres-ai/current/ai-factory/model/configure-servingruntime/) optimized for the model framework (vLLM, TensorRT-LLM, custom)
1.  **Resource Allocation**: Define GPU, memory, and CPU requirements based on expected workload
1.  **Endpoint Configuration**: Set up internal cluster access or external API endpoints with authentication (see [Access KServe endpoints](/edb-postgres-ai/current/ai-factory/model/access-kserve-endpoints/))
...
1.  **Configure Repository Rules** to connect trusted model sources
1.  **Deploy Your First Model** using the guided deployment workflow
1.  **Monitor Performance** through integrated dashboards
...
-   [Monitoring InferenceServices](/edb-postgres-ai/current/ai-factory/model/monitor-inferenceservice/)
