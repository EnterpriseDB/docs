...
The `edbctl` CLI can create the NGC API key secret and image pull secret as part of the `ai` scenario. When prompted, provide your NGC API key:

```bash
edbctl setup create-install-secrets --version <version> --scenario ai
```

For non-interactive environments (CI/CD), set the NGC API key as an environment variable and use the `--non-interactive` flag:
