...
See [Private Registry Integration](/edb-postgres-ai/current/ai-factory/model/integrate-private-registry/) for custom NIM deployment procedures.
...
Consult [Prerequisites Guide](prerequisites/) for comprehensive specifications.
...
1.  Label GPU nodes with `nvidia.com/gpu=true`
1.  Apply GPU taints for dedicated scheduling
1.  Verify CUDA compatibility for NIM requirements

Detailed instructions available in [GPU Setup Documentation](/edb-postgres-ai/current/ai-factory/model/setup-gpu/).
...
1.  Download and cache model profiles
1.  Upload profiles to object storage
1.  Configure Model Library for private registry access

Complete procedures documented in [Air-Gap Configuration](prerequisites/#air-gapped-deployments).
...
1.  Select NVIDIA NIM model from catalog
1.  Configure resources (GPU allocation, memory, replicas)
1.  Deploy InferenceService to project namespace
1.  Access through generated endpoints

Step-by-step guide: [Create InferenceService](/edb-postgres-ai/current/ai-factory/model/create-inferenceservice/).
...
-   [Gen AI Hub](/edb-postgres-ai/current/ai-factory/gen-ai/) - Comprehensive reference
-   [Concepts](gen-ai/concepts/) - Component overview
-   [Architecture](gen-ai/architecture/) - System design
...
1.  Process content through NIM embedding models
1.  Store vectors in PostgreSQL with pgvector
1.  Configure retrieval strategies for search

Implementation guide: [Knowledge Base Creation](gen-ai/create-knowledge-base/).
...
Reference: [Model Observability](/edb-postgres-ai/current/ai-factory/model/observability/).
...
1.  **Testing**: Execute performance and accuracy tests
1.  **Deployment**: Implement canary or blue-green strategy
1.  **Monitoring**: Track metrics during transition
1.  **Decision**: Complete rollout or rollback based on metrics
...
1.  **Image pull failures**: Check NGC credentials and network connectivity
1.  **Profile cache missing**: Ensure profiles available in air-gapped setups
1.  **Insufficient memory**: Validate memory allocation for model size
...
1.  **Document chunking**: Adjust chunk size and overlap parameters
1.  **Search parameters**: Tune top-k and similarity thresholds
1.  **Index completeness**: Confirm all documents processed successfully
...
-   [Knowledge Base Management](gen-ai/knowledge-bases/)

**Troubleshooting Resources**

-   [Model Verification](/edb-postgres-ai/current/ai-factory/model/verify-models/)
-

For issues not addressed here, contact EDB support or consult the [AI Factory Hub](/edb-postgres-ai/current/ai-factory/).
