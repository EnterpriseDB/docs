...
Before you can use **GenAI Builders** in Hybrid Manager, you need to enable **AI Factory**.
This involves two main steps:

1. Setting up a **Fernet key** for encryption (if not already created).
2. Connecting GenAI Builders to an **S3-compatible bucket** for its datalake.
...
This creates a Kubernetes secret in the `upm-griptape` namespace for GenAI Builders to use.

---

## 2. Create an S3-compatible bucket

GenAI Builders requires an **S3 bucket** (the datalake) to store structures, tools, and indexed data.
...
If you plan to use the **GenAI Builders web UI**, configure CORS rules on the bucket so the UI can access it.
...
## 3. Connect the bucket to GenAI Builders
...
* GenAI Builders can store and retrieve content in your datalake.
