...
If you already created the Griptape secret during Hybrid Manager setup (via `edbctl setup create-install-secrets` or the `-install-secrets.sh` scripts), you can skip this step.

### Using edbctl (recommended)

The `edbctl` CLI creates the Fernet key secret automatically as part of the `ai` scenario:
...
If you prefer to create the secret manually (requires `kubectl` configured for your HM cluster):
