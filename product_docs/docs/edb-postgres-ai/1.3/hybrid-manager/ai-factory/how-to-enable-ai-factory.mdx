...
1. Connecting GenAI Builder to an **S3-compatible bucket** for its datalake.

## 1. Generate a Fernet key (if not already created)

AI Factory uses a Fernet key to encrypt sensitive data.
Normally you create the secret required for GenAI Builder as you install the Hybrid Manager.

[add command to check if secret exists, maybe something like `kubectl get secret ai-factory-fernet-key -n <namespace>`]: # 

If the secret doesn't exist, create one.

### Using edbctl (recommended)

You can create a Fernet key secret using the `edbctl` CLI if you have enabled the `ai` scenario for your installation (`ai` is enabled by default if not specified otherwise explicitly). 
