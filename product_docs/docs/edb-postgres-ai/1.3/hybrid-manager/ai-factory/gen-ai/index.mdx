...
redirects:
  - /edb-postgres-ai/innovation-release/ai-factory/gen-ai
  - /edb-postgres-ai/1.3/hybrid-manager/ai-factory/gen-ai
  - /edb-postgres-ai/latest/hybrid-manager/ai-factory/gen-ai
  - /purl/ai-factory/gen-ai/agent-studio
  - /edb_postgres-ai/latest/ai-factory/gen-ai
navigation:
  - "#Getting started"
  - getting-started
  - concepts
...
1.  **Processing Pipeline**: Transforms content into searchable formats with appropriate metadata and access control annotations
1.  **Retrieval Interface**: Provides unified search across all integrated content with relevance ranking and source attribution
1.  **Application Layer**: Delivers knowledge access through conversational interfaces and API integrations
...
1.  **Retrieval Configuration**: Implement search strategies optimized for application requirements
1.  **Generation Integration**: Connect language models for response generation based on retrieved context
1.  **Response Orchestration**: Coordinate retrieval and generation workflows with appropriate caching and optimization
...
1.  **Processing Pipeline Design**: Configure content transformation workflows including cleaning, chunking, and metadata extraction (see [configure Data Lake](configure-datalake))
1.  **Indexing Strategy**: Implement embedding generation and vector storage optimization
1.  **Retrieval Testing**: Validate search quality through systematic evaluation procedures
...
1.  **Basic Retrieval**: Implement simple search functionality with performance validation
1.  **Assistant Prototype**: Develop conversational interface for pilot knowledge domain
1.  **Production Deployment**: Scale successful prototypes with comprehensive operational controls
