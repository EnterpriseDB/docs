...
-   RAG for analytics: Help users explore data with pgvector‑backed retrieval.
    -   Build steps: prepare embeddings with [Pipelines](/edb-postgres-ai/current/ai-factory/pipeline/) or [Data Lake](configure-datalake) + KB → [Retriever](retrievers) → [Assistant](create-assistant)
    -   Tune: chunking/metadata, similarity thresholds, and rerank modules.

## Configuration Checklist

-   Model/driver selection appropriate to task (see SDK [drivers](reference/sdk/drivers/))
...
Assistants function as the primary interface between users and the Gen AI Builder ecosystem, coordinating multiple components to deliver coherent AI-powered interactions. They integrate with other AI Factory components including [Model Serving](/edb-postgres-ai/current/ai-factory/model/) for inference capabilities, [AI Accelerator Pipelines](/edb-postgres-ai/current/ai-factory/pipeline/) for data processing, and [Vector Engine](/edb-postgres-ai/current/ai-factory/vector-engine/) for semantic search operations.
...
1.  **Knowledge Retrieval**: Relevant information extraction from connected knowledge bases
1.  **Behavioral Application**: Ruleset guidelines applied to response generation parameters
1.  **Tool Execution**: External actions performed when appropriate and authorized
1.  **Response Generation**: Language model inference with retrieved context and behavioral constraints
1.  **Output Formatting**: Response packaging with citations and source attribution
...
-   **[Model Serving](/edb-postgres-ai/current/ai-factory/model/)**: Provides governed language model inference
-   **[AI Accelerator Pipelines](/edb-postgres-ai/current/ai-factory/pipeline/)**: Enables knowledge base content processing
-   **[Vector Engine](/edb-postgres-ai/current/ai-factory/vector-engine/)**: Delivers semantic search infrastructure

### Development Resources

**Implementation Guides**

-   [Create Assistant](create-assistant/): Step‑by‑step assistant configuration
-   [Knowledge Base Integration](knowledge-bases/): Comprehensive knowledge management
-   [Tool Development](tools/): External system integration patterns
...
-   Drivers: [Assistant/Prompt](reference/sdk/drivers/), [Conversation Memory](reference/sdk/drivers/conversation-memory-drivers.mdx)
-   Structures: [Agents](reference/sdk/structures/), [Tasks](reference/sdk/structures/tasks.mdx), [Workflows](reference/sdk/structures/workflows.mdx)
...
-   How do I choose models/drivers?
    -   Start with general models; route task‑specific prompts to cheaper/faster drivers; for private endpoints, integrate via [Model Serving](/edb-postgres-ai/current/ai-factory/model/).
