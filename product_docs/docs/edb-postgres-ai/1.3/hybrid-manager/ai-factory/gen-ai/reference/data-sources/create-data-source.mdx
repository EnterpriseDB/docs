...
originalFilePath: >-
  https://github.com/EnterpriseDB/pgai-ai-builder/blob/main/docs/griptape-cloud/data-sources/create-data-source.md
editTarget: originalFilePath

redirects:
  - /edb-postgres-ai/latest/ai-factory/gen-ai/reference/data-sources/create-data-source
---
...
You can connect a [Bucket](../data-lakes#buckets) and a list of [Asset Paths](../data-lakes#asset-paths) as a Data Source. Supported file types include PDF, CSV, Markdown, and most text-based file types.

### Custom Data Sources via Structures (Experimental)

You can specify a [Structure](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/reference/structures/create-structure/) to run as a Data Source as long as your Structure returns a [`TextArtifact` or `ListArtifact` from the Gen AI Builder SDK](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/reference/sdk/data/artifacts/). You can use this as a way to build custom Data Sources.
...
When creating any Data Source, you can optionally specify a [Structure](../structures/create-structure.mdx) to run as a transform step of your data ingetstion before loading into the vector store. Ensure the Structure you select to run as a transform is configured to take in a `ListArtifact` as its first positional argument and returns either a `TextArtifact` or `ListArtifact`.
