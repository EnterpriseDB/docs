...
**Workaround**: To remediate this issue, you must trigger an on-demand Barman backup or schedule a Barman backup at least once per week for clusters you previously created using `VolumeSnapshot`. This action will invoke the necessary retention policies to clear accumulated WAL files from your object storage.
...
### Kafka NodePort access not supported in RKE

**Description**: The primary method for exposing Kafka (which is required for data migrations) within Hybrid Manager is via Load Balancers. In RKE environments without an enabled Load Balancer, the Kafka component cannot be exposed, which means that data migration workflows will not function, as there is no fallback mechanism to support exposing Kafka using NodePort mode.

**Workaround**: None. This issue is currently targeted for implementation to ensure Kafka can be accessed even when standard Kubernetes Load Balancers are unavailable. To perform data migrations in RKE environments, you currently must have Load Balancers configured.
