...
- `values.yaml` is critical—manage it as a single source of truth with strict version control (Git or similar). Never pass it informally; always use a collaborative, versioned platform.
...
- For lists (like `resourceAnnotations`), Helm replaces the entire list—always specify the full structure for each item.
...
There is a single mandatory staic `User-0` for install purposes.  Additional static users can be created at the time of provisioning but no UI is provided to manage these as it's not an intended secure pattern.  Here at the minimum you should set the password for this required static user.
