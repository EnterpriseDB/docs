...
description: Execute the Helm installation to deploy the Hybrid Manager and verify system readiness.
deepToC: true
---
...
* [Phase 3: Deploy Kubernetes Cluster](../k8s_deploy) (Running and validated)
* [Phase 4: Environment Preparation](../prepare_environment) (Completed and validated)

**Outcome:** A fully deployed Hybrid Manager (HM) platform, accessible via the web portal.

**Next phase:** [Initial Configuration](../initial_config)
...
* 2. ** `bootstrapImageName`**: Must match the bootstrap image path defined in [Environment Prep](../prepare_environment).
* 3. ** `bootstrapImageTag`**: Must match the target version you are installing.

### Domain & network parameters

* 4. **`parameters.global.portal_domain_name`**: The FQDN for the Hybrid Manager UI (e.g., `portal.my-company.com`).
* 5. **`parameters.transporter-rw-service.domain_name`**: The FQDN for the migration service.
* 6. **`parameters.transporter-dp-agent.rw_service_url`**: The full URL for the data plane agent (e.g., `https://<migration-domain>/transporter`).
* 7. **`parameters.upm-beacon.server_host`**: The hostname the beacon agent will use to connect back to the control plane.
* 8. **`beaconAgent.provisioning.nodePortDomain`**: If using NodePort instead of LoadBalancer, ensure this domain is set.

### Security & Authentication

* 9. **`pgai.portal.authentication.staticPasswords`**: You must have the `email`, `hash`, `username`, and `userID` set for the initial admin user ("User-0").
* 10. **`parameters.upm-istio-gateway.cookie_aeskey`**: Ensure this 16/32-byte key is set for session encryption.
* 11. **`transporter-data-operator.fips_enabled`**: Set this to **`false`**. (FIPS support is not yet universal for all components).
...
!!! Note
**Troubleshooting:** If pods are stuck in `ImagePullBackOff`, verify your **Image Pull Secret** from [Phase 4](./environment_prep). 
...
**[Proceed to Phase 6: Initial Configuration](../initial_config) →**