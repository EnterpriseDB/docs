...
**Outcome:** A validated high-level inventory of compute, storage, and external services ready for configuration and a number of inputs for the HM Helm chart `values.yaml` used in the next Phase ([Preparing your environment](environment_prep))

**Next Step:** [Preparing the Environment](environment_prep)
...
> **Optionality:** Alternative to [Load Balancer controller](#21-load-balancer-controller).

If a Load Balancer is not available, you must use **NodePort**.
...
You configure the connections to them in **[Preparing the environment](environment_prep)** (Phase 3).
...
You must prepare the following values and artifacts for the configuration phase (Phase 3):
...
> **Optionality:** Alternative to [External IdP](#1-external-idp-recommended).
...
Here is an example of a production oriented `values.yaml` for installation which picks from above "required for optimal experience".  
...
> **Next Step:** You now have a complete inventory. 

Proceed to **[Phase 3: Deploying the Kubernetes cluster](k8s_install)** to provision and deploy your Kubernetes clsuter.
