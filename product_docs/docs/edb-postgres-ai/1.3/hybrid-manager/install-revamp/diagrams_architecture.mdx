...
The accompanying questions cover a very broad set of considerations extending beyond just the database layer.  
...
### Kubernetes: supported platforms and broad considerations
...
- The Kubernetes cluster must be dedicated to HM at this time; sharing with other workloads is not supported.
...
The minimum install for HM colocates the HM control plane on the Kubernetes control nodes, although the pattern is slightly different in case of deployment on a CSP.  This mimimum architecture is fully functional for using HM to centralize a view of your Postgres and Oracle Estate using the Agent and facilitates Database Migration capabilities as well.  GenAI is also present in this deployment although it's capabilities will be limited due to the lack of HM operated Postgres instances.
...
* **Kubernetes Cluster:** The complete Kubernetes environment—including all control plane and worker nodes—hosting the entire platform.  
...
These are the prerequisite services and infrastructure that must be available in your environment **before** you can deploy the HM. They are dependencies that the platform relies on but does not provide.
...
* **Object Storage:** S3 compatible object storage for unstructured data, Postgres backups, parquet files and HM recover and mulit-location capabilities.
...
#### HM core microservices

These are the core microservices that make up the HM Control Plane, running inside the Kubernetes cluster (within the "EDB Hybrid Manager" boundary).

* **GenAI**: Provides AI/ML capabilities. See [GenAI in HM](https://www.enterprisedb.com/docs/edb-postgres-ai/latest/ai-factory/gen-ai/)
* **Postgres Lifecycle Operations**: Manages deployment, scaling, and updates. See [Using HM](https://www.enterprisedb.com/docs/edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/cluster_management/)  
...
* **Federation**: Manages authentication and authorization across multiple HM instances.

---

### HM Data Plane (Postgres lifecycle orchestration)

Sitting aside the HM Control Plane is the Data Plane, which is comprised primarily of Postgres centered capabilities extended into AI and Analytics functionality.
...
This view shows a fully capable HM deployment, including resources like GPU acceleration.

![HM fully featured](../images/HCP_reference_architectures-full-featured.svg)

---

### HM multi-location (hub and spoke)

The multi-location capability is primarily a DBaaS offering following a hub and spoke model.
1.  As a DBaaS offering, many additional capabilities are not available on the secondary HM.
...
parameters:
  upm-beacon:
    beacon_location_id: <Deployment_Location_Name> # a simple string which will be a hint in the UI to identify this location
beaconAgent:
    provisioning:
