...
* **EDB Hybrid Manager:** The logical boundary for the core HM components. This is typically implemented as a dedicated Kubernetes namespace (e.g., edb-platform-control-plane).  
...
* **Container Registry:** The EDB registry that hosts the HM container images.  
* **Domain Name Service:** A functional DNS capability for resolving internal and external hostnames.  
* **Identity Provider:** An external service (like Okta, Keycloak, or Azure AD) for user authentication via OIDC. See [Security and access management](https://www.enterprisedb.com/docs/edb-postgres-ai/latest/hybrid-manager/install/customization/byo_idp/).  
* **Local Network:** The underlying network infrastructure allowing communication to and from the cluster.  
* **Key Management Service (KMS):** An external service to manage and store encryption keys. See [KMS for TDE](https://www.enterprisedb.com/docs/edb-postgres-ai/latest/hybrid-manager/install/customization/kms_for_tde/).  
* **Block Storage:** Storage solutions (like S3, EBS, or on-prem equivalents) required for persistent data, exposed to K8s via a StorageClass.
* **Object Storage:** Storage solutions (like S3, EBS, or on-prem equivalents) required for persistent data, exposed to K8s via a StorageClass.
...
* **Migration Assistant**: Assists with database migrations. See [Database migration](https://www.enterprisedb.com/docs/edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/migration/)  
...
* **Spiffe/Spire**: Provides workload identity. See [Security and access management](https://www.enterprisedb.com/docs/edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/#security-and-access-management)
