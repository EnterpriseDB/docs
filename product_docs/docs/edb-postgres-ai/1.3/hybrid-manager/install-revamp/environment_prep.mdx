...
**Outcome:** A fully staged environment ready for installation. This includes a validated management workstation, synchronized artifacts, a complete inventory of infrastructure and application secrets (covering storage, registry access, and encryption keys), and a finalized `values.yaml` configuration that defines your specific architectural requirements—from core administrative settings to advanced features like GenAI and multi-location logic.

**Next Step:** [Cluster deployment and installation](k8s_install)

---

Before deploying the cluster, you must bridge the gap between your architectural plan and the technical environment. 
This involves more than just setting up a laptop; you must stage artifacts, secure the environment with keys and certificates, and translate your specific requirements into a deployable configuration file.
...
8.  **Cluster configuration:** Assembling all the relevant above inputs into the final Helm chart `values.yaml` file.
...
You must generate and edit the HM configuration Helm chart file (`values.yaml`). 
This file defines network settings, node pools, and feature flags.

Below is a template configuration with all of the keys you should have determined up to this point:
...
Before proceeding to deployment, validate the environment.
...
Once your management workstation is ready, images are synced, secrets are staged, and the YAML is validated, you are ready to deploy your Kubernetes cluster.

**[Proceed to Phase 4: Cluster Deployment & Installation →]**