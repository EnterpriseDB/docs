...
* [Phase 2: Gathering your system requirements](./system_reqs) (Completed)
* [Phase 3: Deploying you Kubernetes cluster](./k8s_install) (Completed & Running)

**Outcome:** A Kubernetes cluster configured with necessary secrets (TLS, Auth, Storage), synchronized container images, any advanced features configured, and a finalized `values.yaml` configuration file ready for Hybrid Manager (HM) installation.

**Next phase:** [Install HM](./install_hm)

---

Now that your Kubernetes cluster is running, you must prepare it for the HM application. 
...
1.  **Artifact management:** Mirroring container images to your local registry and configuring the agent for image discovery.

1.  **Infrastructure secrets:** Creating Kubernetes secrets for **Image Pull** credentials and **Object Storage** access.

1.  **TLS & encryption:** Implementing your chosen TLS strategy (Cert-Manager, Custom CA, Custom cert, vs self-signed) and enabling **KMS** for Transparent Data Encryption (TDE).

1.  **Application secrets:** Staging Fernet keys for the **GenAI** and **Catalogs** stacks, and configuring secrets for the **Migration Portal**.

1.  **Advanced feature Prep:** Defining **Multi-location** logic (SPIRE federation and object storage sync), constructing complex YAML configurations for **Identity Providers**, creating namespaces and secrets for **GenAI** (including CORS policies), and configuring secrets for **Catalogs**.

1.  **Environment settings:** Configuring the internal backup folder and securing the mandatory **User-0** admin account.

1.  **Final configuration prep:** Assembling all the relevant above inputs into the final Helm chart `values.yaml` file.
...
!!! Note
For this option, you do not need to pass a secret to the HM installer. 
...
* **Use Case:** You want HM to create an Issuer for you, using your corporate CA to sign certificates.
...
> **Optionality:** Required for optimal experience using Migration Portal.
