...
* *any advanced features configured, and a finalized and validated `values.yaml` configuration file ready for Hybrid Manager (HM) installation.*
...
1.  **Configuring advanced features:** 
    ...
    [Defining **Multi-location** logic (SPIRE federation and object storage sync)](#configuring-multi-location-architecture) and [constructing complex YAML configurations for your **Identity Provider**](#configuring-identity-providers-idp).

1.  [**Completing environment preparation:**](/edb-postgres-ai/latest/hybrid-manager/install/environment_prep/#completing-environmental-preparation)

    Configuring the internal backup folder and securing the mandatory **User-0** admin account.

1.  [**Finalizing the cluster configuration (`values.yaml`):**](#finalizing-the-cluster-configuration-valuesyaml) 
