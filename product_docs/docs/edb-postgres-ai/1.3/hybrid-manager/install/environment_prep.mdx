...
-   [**Creating other necessary secrets**](#creating-other-necessary-secrets)
    ...
    -   Creating **GenAI Builder** secret - requires Fernet secret
    -   Creating **Catalog** secret - requires Fernet secret
    -   Customizing secrets for the **Migration Portal
...
- One of the secrets you will create require an NGC API key. Create one following the [NVIDIA NGC documentation](https://docs.nvidia.com/ngc/gpu-cloud/ngc-user-guide/index.html#ngc-api-keys) to enable model image pulls.

[In 1.3.x instances the secrets MUST exist regardless of whethet the installation ises GenAI or not. But what about the NGC API key? How would the customer create a secret without the key? Or can they leave the secret "empty" when prompter in th command below?]: #

### Using edbctl to create secrets

Use `edbctl` to create the required additional secrets. It knows which secrets you need, based on your HM version. 
...
- For manual installations, run this command and follow the interactive prompts:
  ...
  After running the command, the terminal prompts for any information required from you (like the NGC API key) 

- For automated pipelines (CI/CD), use the `--non-interactive` flag to suppress all confirmation prompts and allow the command to complete without human intervention. You will need to make the NGC API key you create above available as a variable.

  ```bash
  export EDB_AI_NVIDIA_NGC=<your-ngc-api-key>
