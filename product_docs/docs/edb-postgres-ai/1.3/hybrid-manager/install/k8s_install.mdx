...
* **Sizing:** Current recommendations require **Nvidia B200 GPUs** (or equivalent supported hardware). Mandatory for deployments where the ai module is active (default); not required if the ai module is disabled.

[check whether this is only required when `ai` is enabled]: #
