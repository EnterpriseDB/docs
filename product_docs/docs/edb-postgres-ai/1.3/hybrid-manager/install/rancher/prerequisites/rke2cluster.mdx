...
After you've [installed the necessary system tools](product_docs/edb-postgres-ai/1.3/hybrid-manager/install/rancher/prerequisites/), you're almost ready to set up a Rancher RKE2 cluster for use with Hybrid Manager (HM).
...
If you need to change these defaults, update your Helm chart `values.yaml`:
...
-   Ensure your Helm chart `values.yaml` is configured with the following parameters after deciding on a root domain name for HM:
...
You can also set up a [custom cert-manager issuer](product_docs/edb-postgres-ai/1.3/hybrid-manager/install/customization/cert-man/#set-up-a-custom-cert-manager-issuer-for-the-hm-portal) for the HM Portal and even setup your own [certificate authority](product_docs/edb-postgres-ai/1.3/hybrid-manager/install/customization/cert-man/#bring-your-own-private-certificate-authority).

#### User identity configuration

To configure user identity, you have to choose between implementing your own IDP or using HM's native users feature.

It is strongly recommended to [set up your identity provider](product_docs/docs/edb-postgres-ai/1.3/hybrid-manager/install/customization/byo_idp/index.mdx) (IDP) with HM for managing user access in production.
...
You need to user's email, hash, userID, userID, and username and set these values in the Helm chart `values.yaml`:
...
1. Ensure [edbctl is installed and configured](product_docs/edb-postgres-ai/hybrid-manager/using_hybrid_manager/edbctl/) if it is not already.
...
1. Sync the EDB PGAI Operator image to the destination registry:
  ...
  edbctl operator sync-to-local-registry \
      --destination-registry-uri "<LOCAL_REGISTRY_URI>" \
      --version "${EDBPGAI_RELEASE}" \
      --source-registry-username "${LOCAL_SOURCE_REGISTRY}" \
      --source-registry-password "${CS_EDB_TOKEN}" \
      --destination-registry-username "<LOCAL_REGISTRY_USER>" \
      --destination-registry-password "<LOCAL_REGISTRY_PWD>"
  ```
...
This process connects to your customer-managed local container registry (see previous step) that must be [Docker HTTP V2 compliant](https://docs.docker.com/reference/api/registry/latest/).
...
The options 
See the documentation for your lcoal internal container registry (such as [quay](https://quay.io/)) for on-premises scenario (see previous step) to configure the required registry permissions for the container registry so that HM can access it.

If using a CSP-based local registry for your images (AWS ECR, Google Cloud's AR), use the following examples as a guide:
...
In the Helm chart, `values.yaml`, set the field `bootstrapImageTag:upm-istio-gateway:cooke_aeskey` to the AES-256 encryption key you generated in the previous step.
...
With your Block Storage strategy outlined and storage classes collated to your the HM CP and Postgres data node workloads (see previous step), you are ready to implement Block Storage by installing the appropriate CSI driver.

#### Cluster prerequisites

Ensure your cluster is configured to handle dynamic volume provisioning and data protection.

#### CSI driver installation

The chosen Storage Class dictates the required Container Storage Interface (CSI) Driver and the CSI driver sets the Storage Classes for the system.

Verify the following two types of drivers are installed and properly configured in your cluster:

-   **Persistent storage driver**: Install the CSI driver that is compatible with your underlying storage backend (currently, [TopoVLM](https://github.com/topolvm/topolvm) is supported for on-prem local storage for example, but you might use a storage device like PortWorx that includes a CSI driver). This enables the creation of Persistent Volumes (PVs).
...
Your CSI driver and Snapshot controller are now installed and configured, with the appropriate Storage Classes implemented.

### Update the Helm chart

With the values of your HM CP storage class implemented, you need to set these values in the Helm chart `values.yaml`.

For the HM CP storage class for internal platform services, set the following value in `parameters:global`:

```yaml
storage_class: <the-storage-class-implemented-for-internal_services>
```
