...
You can create secrets for Migration Portal using the `edbctl` CLI if you have enabled the `migration` scenario for your installation. The HM installation instructions already include steps on how to use `edbctl` CLI to [create other necessary secrets](/edb-postgres-ai/current/hybrid-manager/install/environment_prep/#creating-other-necessary-secrets).
