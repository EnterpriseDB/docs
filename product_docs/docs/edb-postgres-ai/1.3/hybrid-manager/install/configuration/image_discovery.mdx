...
If you are using a standard OCI registry (like Harbor, Quay, or Artifactory), ensure the username/password you provided when creating your [**Image Pull Secret*](/edb-postgres-ai/latest/hybrid-manager/install/environment_prep/#create-image-pull-secret)* has `read` and `list` permissions on the target repositories.
