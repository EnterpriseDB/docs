...
1.  **Enable Discovery:** Set `beaconAgent.provisioning.imageDiscovery` to `true`.
2.  **Set Registry URL:** Set `beaconAgent.provisioning.imagesetDiscoveryContainerRegistryURL` to the URI of your synced private registry.
3.  **TLS Verification (Optional):** If your registry uses self-signed certificates, set `imagesetDiscoveryAllowInsecureRegistry` to `true`.
