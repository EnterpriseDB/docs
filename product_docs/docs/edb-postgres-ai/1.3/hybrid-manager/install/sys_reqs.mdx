...
* a number of requirements validated for configuration in Phase 4 [Preparing your environment](environment_prep) and a number of inital inputs defined for the HM Helm chart `values.yaml` to start this file now, continuing adding to it from Phase 1 if already started, or to use to populate the `values.yaml` at the end of Phase 4 ([Preparing your environment](environment_prep)).
