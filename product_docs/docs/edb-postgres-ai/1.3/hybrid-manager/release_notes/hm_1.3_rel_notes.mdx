...
- **Platform stability & flexibility**: Validated BYOA to Hybrid Manager transitions, expanded deployment support to SuSE Rancher, and secured crucial backup & recovery flows.
...
**Backup frequency**: We've introduced the ability to customize the backup frequency for the Hybrid Manager. Customers can now define the Velero backup schedule directly through the Helm values file, allowing them to precisely control their Recovery Point Objective (RPO) and meet specific operational requirements.
...
**Upgrade certification**: To ensure a seamless upgrade experience, we have implemented extensive automated testing to validate upgrade paths between all supported minor and patch releases (e.g., v1.2.x to v1.3.0 and v1.2.2 to v1.2.3). This foundational work significantly reduces the risk of regressions, enhances platform stability during transitions, and clarifies the upgrade support policy for all users.

**Transtition from BYOA**: We have validated and are now supporting a non-disruptive, self-service transition path for existing Bring Your Own Account (BYOA) customers to migrate to Hybrid Manager.

**Backup support for RHOS**: Hybrid Manager now also supports volume snapshot-based backup and recovery operations for clusters deployed on Red Hat OpenShift (RHOS) and Engineered System. This provides a faster, platform-native B&R experience for these critical environments.
