...
For details on vector distance operators `<->` (L2), `<#>` (inner product), and `<=>` (cosine), see the [Vector Engine](/edb-postgres-ai/current/ai-factory/vector-engine/) documentation.
