...
> Note: This capability was previously part of AIDB and has moved into a separate extension/product called PGPU.
...
- Supported NVIDIA GPUs architectures: 8.9, 9.0, 10.0, 10.3, 12.0, 12.1 (this includes L40S, L40, L20, L4, L2, H200, H100, GH200, B200, B100, GB20x, GB10 as well as other models and compatible consumer graphics cards).
- NVIDIA GPU driver version 525 or newer installed and functional.
- NVIDIA CUDA runtime 12.9 or newer installed (minimal requirements are the libcublas and libcudart libraries).
- The VectorChord extension version 1.0.0 or newer available on the system (for the `vchordq` index type). This package is available in EDB repositories.
...
When the function completes, the table has a VectorChord `vchordq` index ready for use. The centroids that PGPU computed to support building the index will be stored in a table `public.test_10k_vecs_centroids`. Run `\d public.test_10k_vecs;` in `psql` to inspect the table and the index settings.
...
- Debugging: PGPU will log debug information on postgres debug levels 1 and 2: `SET client_min_messages TO debug2;`
