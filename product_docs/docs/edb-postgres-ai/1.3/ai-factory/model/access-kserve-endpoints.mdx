...
```shell
curl -X POST \
  'https://portal-hcp-pm.edbhcp.com/inferenceservices/m-po7d0fta/v1/chat/completions' \
  -H 'accept: application/json' \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer a" \
  -d '{
...
- Internal URL (cluster‑local):
  - From SQL (AIDB): run `SELECT * FROM aidb.list_hcp_models();` to see internal service URLs for known models within the database context.
