...
-   The `method` determines how the PDF is parsed:
    -   `Structured` (Default) — Algorithmic text extraction. Structural PDF parsing that relies on the PDF specification can struggle with PDFs that are not spec-compliant, especially if they include complex or proprietary encodings and different fonts.
