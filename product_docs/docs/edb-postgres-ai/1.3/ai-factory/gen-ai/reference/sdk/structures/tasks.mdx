...
title: Tasks
originalFilePath: >-
  https://github.com/EnterpriseDB/pgai-ai-builder/blob/main/docs/griptape-framework/structures/tasks.md
...
A [Task](../reference/tasks) is a purpose-built abstraction for the Large Language Model (LLM). Gen AI Builder offers various types of Tasks, each suitable for specific use cases.

## Context

Tasks that take input have a field [input](../reference/tasks/base_text_input_task.mdx#griptape.tasks.base_text_input_task.BaseTextInputTask.input) which lets you define the Task objective.
Within the [input](../reference/tasks/base_text_input_task.mdx#griptape.tasks.base_text_input_task.BaseTextInputTask.input), you can access the following [context](../reference/structures/structure.mdx#griptape.structures.structure.Structure.context) variables:

-   `args`: an array of arguments passed to the `.run()` method.
-   `structure`: the structure that the task belongs to.
-   user defined context variables

Additional [context](../reference/structures/structure.mdx#griptape.structures.structure.Structure.context) variables may be added based on the Structure running the task.
...
All Tasks implement [RunnableMixin](../reference/mixins/runnable_mixin.mdx) which provides `on_before_run` and `on_after_run` hooks for the Task lifecycle.
...
For general-purpose interaction with LLMs, use the [PromptTask](../reference/tasks/prompt_task.mdx):
...
You can disable this behavior by setting [reflect_on_tool_use](../reference/tasks/prompt_task.mdx#griptape.tasks.prompt_task.PromptTask.reflect_on_tool_use) to `False`.
...
!!! warning
    ...
    `ToolTask` is deprecated and will be removed in a future version. Use [Prompt Task](tasks.mdx#prompt-task) with [reflect_on_tool_use](tasks.mdx#reflect-on-tool-use) set to `False` instead.

Another way to use [Gen AI Builder Tools](../tools), is with a [Tool Task](../reference/tasks/tool_task.mdx).
...
To extract information from text, use an [ExtractionTask](../reference/tasks/extraction_task.mdx).
This Task takes an [Extraction Engine](../engines/extraction-engines.mdx), and a set of arguments specific to the Engine.
...
To summarize a text, use the [TextSummaryTask](../reference/tasks/text_summary_task.mdx).
This Task takes an [Summarization Engine](../engines/summary-engines.mdx), and a set of arguments to the engine.
...
To query text, use the [RagTask](../reference/tasks/rag_task.mdx).
This task takes a [RAG Engine](../engines), and a set of arguments specific to the engine.
...
To execute an arbitrary Python function, use the [CodeExecutionTask](../reference/tasks/code_execution_task.mdx).
...
By default, a [Workflow](workflows.mdx) will only run a Task when all the Tasks it depends on have finished.

You can make use of [BranchTask](../reference/tasks/branch_task.mdx) in order to tell the Workflow not to run all dependent tasks, but instead to pick and choose one or more paths to go down.

The `BranchTask`'s [on_run](../reference/tasks/branch_task.mdx#griptape.tasks.branch_task.BranchTask.on_run) function can return one of three things:

1.  An `InfoArtifact` containing the `id` of the Task to run next.
2.  A `ListArtifact` of `InfoArtifact`s containing the `id`s of the Tasks to run next.
...
To generate an image, use one of the following [Image Generation Tasks](../reference/tasks). All Image Generation Tasks accept an [Image Generation Driver](../drivers/image-generation-drivers.mdx).

All successful Image Generation Tasks will always output an [Image Artifact](../data/artifacts.mdx#image). Each task can be configured to additionally write the generated image to disk by providing either the `output_file` or `output_dir` field. The `output_file` field supports file names in the current directory (`my_image.png`), relative directory prefixes (`images/my_image.png`), or absolute paths (`/usr/var/my_image.png`). By setting `output_dir`, the task will generate a file name and place the image in the requested directory.

### Prompt Image Generation Task

The [Prompt Image Generation Task](../reference/tasks/prompt_image_generation_task.mdx) generates an image from a text prompt.
...
The [Variation Image Generation Task](../reference/tasks/variation_image_generation_task.mdx) generates an image using an input image and a text prompt. The input image is used as a basis for generating a new image as requested by the text prompt.
...
The [Inpainting Image Generation Task](../reference/tasks/inpainting_image_generation_task.mdx) generates an image using an input image, a mask image, and a text prompt. The input image will be modified within the bounds of the mask image as requested by the text prompt.
...
The [Outpainting Image Generation Task](../reference/tasks/outpainting_image_generation_task.mdx) generates an image using an input image, a mask image, and a text prompt. The input image will be modified outside the bounds of a mask image as requested by the text prompt.
...
The [Structure Run Task](../reference/tasks/structure_run_task.mdx) runs another Structure with a given input.
...
The [Assistant Task](../reference/tasks/assistant_task.mdx) enables Structures to interact with various "assistant" services using [Assistant Drivers](../reference/drivers/assistant).
...
This Task enables Structures to synthesize speech from text using [Text to Speech Drivers](../reference/drivers/text_to_speech).
...
This Task enables Structures to transcribe speech from text using [Audio Transcription Drivers](../reference/drivers/audio_transcription).
