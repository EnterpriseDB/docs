...
directoryDefaults:
  displayBanner: |
    ...
    New functionality built around LangFlow is now available as a replacement. <a href="https://www.enterprisedb.com/docs/edb-postgres-ai/latest/hybrid-manager/ai-factory/langflow/">More details&hellip;</a>
...
Gen AI Builder provides comprehensive tooling for developing intelligent applications that leverage organizational knowledge through [retrieval‑augmented generation](/edb-postgres-ai/current/ai-factory/concepts/terminology/#retrieval-augmented-generation-rag) patterns. The platform operates within controlled infrastructure environments, ensuring complete organizational sovereignty over data, models, and inference operations.

New here? Start with [Getting started](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/getting-started/), review [Concepts](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/concepts/) and the [Architecture](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/architecture/), then use the [How‑to guides](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/how-to/) or jump into the [Quickstart UI](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/quickstart-customer-service-agent-ui/).

## Platform Architecture

Gen AI Builder integrates multiple components to deliver end-to-end AI application development capabilities within the EDB Postgres AI ecosystem. The platform emphasizes data sovereignty by maintaining all operations within organizational boundaries while providing enterprise-grade governance and auditability. Explore the [assistants](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/assistants/), [knowledge bases](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/knowledge-bases/), [retrievers](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/retrievers/), [rulesets](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/rulesets/), [structures](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/structures/), [tools](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/tools/), [threads](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/threads/), [data sources](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/data-sources/), [data lake](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/data-lake/), and [libraries](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/libraries/) manuals.

### Core Infrastructure Integration

**Hybrid Manager Foundation**
Gen AI Builder operates within [Hybrid Manager](/edb-postgres-ai/current/hybrid-manager/)'s Kubernetes orchestration layer, leveraging distributed computing capabilities while maintaining centralized governance. This architecture ensures consistent operational procedures across development and production environments.

**Data Estate Integration**
[Knowledge bases](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/knowledge-bases/) connect directly to Postgres databases and object storage systems (see [Data Lake](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/data-lake/)) within organizational infrastructure, eliminating external data dependencies while supporting diverse [data source](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/data-sources/) types.

**Model Serving Integration**
Applications access language models and embedding models through the [AI Factory Models](/edb-postgres-ai/current/ai-factory/model/) infrastructure, ensuring consistent governance policies across all AI capabilities.

## Architectural Components

### Knowledge Base Framework

[Knowledge bases](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/knowledge-bases/) serve as the foundation for retrieval‑augmented generation applications by combining structured database content with unstructured document repositories. The framework provides semantic search capabilities through vector embeddings (see [Vector Engine](/edb-postgres-ai/current/ai-factory/vector-engine/)) while maintaining complete data lineage.
...
[Retrieval](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/retrievers/) components orchestrate search operations across diverse data sources, providing unified interfaces for knowledge discovery while maintaining source attribution and access controls.
...
Intelligent [assistants](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/assistants/) combine knowledge retrieval with language model capabilities to provide conversational interfaces for organizational knowledge. The framework supports complex reasoning workflows while maintaining audit trails for all interactions (see [threads](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/threads/)).

**Assistant Capabilities**

-   Multi-turn conversational interfaces with context preservation
-   [Tool](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/tools/) integration for external system interactions
...
1.  **Data Integration Layer**: Connects multiple organizational [data sources](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/data-sources/) including databases, document repositories, and knowledge management systems
...
1.  **Knowledge Base Construction**: Establish comprehensive document indexing with semantic embeddings (see [create a Knowledge Base](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/create-knowledge-base/) and [manage a Knowledge Base](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/manage-knowledge-base/))
...
1.  **Source Assessment**: Evaluate [data sources](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/data-sources/) for content quality, update frequency, and access requirements
2.  **Processing Pipeline Design**: Configure content transformation workflows including cleaning, chunking, and metadata extraction (see [configure Data Lake](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/configure-datalake/))
...
Assistant development requires careful orchestration of retrieval, reasoning, and response generation capabilities to deliver reliable user experiences (see [create an Assistant](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/create-assistant/)).

**Configuration Areas**

-   **Behavior Definition**: Establish assistant personality, capabilities, and operational boundaries
-   **Knowledge Integration**: Configure retrieval strategies and knowledge source prioritization (see [retrievers](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/retrievers/))
-   **Tool Integration**: Connect external systems and APIs for expanded functionality (see [tools](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/tools/))
...
Applications expose functionality through standardized APIs that support diverse client applications while maintaining consistent security and operational characteristics (see [SDK reference](reference/sdk/)).
...
-   **[AI Factory Models](/edb-postgres-ai/current/ai-factory/model/)**: Governed access to language models and embedding models
-   **[AI Accelerator Pipelines](/edb-postgres-ai/current/ai-factory/pipeline/)**: Automated content processing and knowledge base updates
-   **[Vector Engine](/edb-postgres-ai/current/ai-factory/vector-engine/)**: High‑performance semantic search within Postgres infrastructure
-   **[Hybrid Manager](/edb-postgres-ai/current/hybrid-manager/)**: Orchestration and resource management
...
-   [Getting Started Guide](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/getting-started/) - Platform overview and initial setup procedures
-   [How-to Documentation](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/how-to/) - Practical implementation guidance for common scenarios

**Component Documentation**

-   [Knowledge Base Management](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/libraries/) - Comprehensive knowledge base development guidance
-   [Assistant Development](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/assistants/) - Intelligent assistant creation and configuration
-   [API Reference](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/reference/) - Detailed technical documentation for development teams

**Advanced Topics**

-   [Data Lake Integration](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/data-lake/) - Large-scale data processing and management
-   [SDK Documentation](/edb-postgres-ai/current/hybrid-manager/ai-factory/gen-ai/reference/) - Programmatic interfaces for custom development
