...
This limitation applies only to Oracle to EPAS migrations with a migration scope of `schema_only` or `schema_and_data`. It doesn't affect `data_only` migrations, where schema objects are created by the customer before migration runs and DMS doesn't generate or translate any PL/SQL bodies.
...
[??? ticket that tracks looking for a resolution]: #

When migrating Postgres to Postgres, the EDB Postgres AI agent extracts all extensions installed in `pg_extension` on the source database and applies them to the destination as part of schema migration. The agent extracts whatever is installed on the source with no allowlist or denylist. The following limitations apply.

- **Per-database scope**: The agent extracts extensions on a per-database basis. Extensions installed in sibling databases in the same cluster aren't included. To migrate extensions from multiple databases, run one migration per database.

- **Version pinning**: Each extension must be available on the destination at the same version as installed on the source. If the version isn't found, schema migration fails.
...
- **Extensions requiring superuser**: Extensions that require superuser to install fail silently at apply time on managed destinations (such as Amazon RDS, Aurora, or EDB Cloud) that don't grant superuser. The migration assessment flags these extensions as `RequiresSuperuser`, but this doesn't prevent the migration from proceeding.

- **Non-relocatable extensions**: Extensions that aren't relocatable can't be moved to a different schema on the destination.

- **Trusted flag reporting on Postgres 10 to 12**: The `trusted` flag for available extension versions is always reported as `false` on Postgres 10 to 12, regardless of the actual upstream value. This reporting behavior has no functional impact on migration.
