...
You can register an external Oracle or Postgres database — PostgreSQL, EDB Enterprise Postgres, or EDB Enterprise Postgres (Oracle Compatible) — with the EDB Postgres AI agent (for example, [register an Oracle database with the agent](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migration_guides/oracle_to_hm_managed/configuring_beacon_agent/)). The **Migrations** tab that you can access from the **Estate** page then lists the external databases you registered along with a migration assessment that you can use for planning.

You can see the same table scoped to a single project on that project's [**Databases**](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migrate/databases/) page, under the **Migrate** section of the left navigation.
...
**Resource ID:** Resource ID or `databases.resource_id` you assigned to a database in the `beacon_agent.yaml` configuration. See the EDB Postgres AI agent's [parameter reference](../../monitoring/mon_ext_dbs/mon_with_agent/configure_agent.mdx#parameter-reference) for more information.
...
**# of Schemas:** Number of schemas in the assessed database that the DSN user has access to. Contains at least one table on which the DSN user has `SELECT` privileges. The DSN user is the source database user you specified while configuring the EDB Postgres AI agent.
...
**Project Name:** Project that was used for configuring the EDB Postgres AI agent. It's the project you assign to an agent in `agent.project_id`. See the EDB Postgres AI agent [parameter reference](../../monitoring/mon_ext_dbs/mon_with_agent/configure_agent.mdx#parameter-reference) for more information.

**Tags:** Labels assigned to the migration of a specific database in `databases.tags`. See the EDB Postgres AI agent [parameter reference](../../monitoring/mon_ext_dbs/mon_with_agent/configure_agent.mdx#parameter-reference) for more information.
...
From the **Estate** page, the **Migration** tab displays a table-based analysis of all databases that you can migrate with HM after registering it with the EDB Postgres AI agent. To customize how assessment results are displayed or filter databases, you can:
...
-   See [Data security and sovereignty](../../monitoring/mon_ext_dbs/mon_with_agent/data_sovereignity.mdx) to understand how your data is stored by the EDB Postgres AI agent.
