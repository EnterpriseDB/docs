...
- **# of Tables:** Number of tables in the schema.
- **# of Objects:** Number of PL/SQL objects (packages, procedures, functions, and triggers) that contain references to Oracle built-in packages or subprograms.
...
To export the assessment results in `.json` formatting, select **Download Report**. For Postgres databases, the downloaded report also includes additional fields not shown in the HM console:

- `dbzUnsupportedTypes`: Data types used in table columns that Debezium doesn't support. Tables with these types can't be migrated using the Data Migration Service (DMS) streaming path.
