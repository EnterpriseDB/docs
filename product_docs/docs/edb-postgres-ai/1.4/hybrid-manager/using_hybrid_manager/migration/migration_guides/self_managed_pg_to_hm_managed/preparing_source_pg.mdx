...
!!!note
If you run `beacon-agent` directly on the database server itself — uncommon, since most environments use a separate [auxiliary machine](auxiliary_machine) — and rely on Postgres's default local peer authentication, the connecting OS user must match a database role. The default service account is `postgres` for community PostgreSQL and EDB Postgres Extended Server, and `enterprisedb` for EDB Postgres Advanced Server (EPAS).
!!!
