...
1.  Run the script with the necessary parameters. For example:
    ...
    !!!note
    Set `ORACLE_SERVICENAME` to the PDB service name.
    !!!
    ...
    ```shell
    ODB_SYSDBA_USERNAME=sys ODB_SYSDBA_PASSWORD=password ODB_HOST=localhost ODB_PORT=1521 ORACLE_SERVICENAME=ORCL ODB_DBZ_USERNAME=MIG_USER2 ODB_DBZ_PASSWORD=migrator_password LOG_MINER_TBS_NAME=LOGMINER_TBS ./oracleConfigValidation.sh "${arr[@]}"
    ```

    !!!note
    Set `ORACLE_SERVICENAME` to the SID or service name.
    !!!
