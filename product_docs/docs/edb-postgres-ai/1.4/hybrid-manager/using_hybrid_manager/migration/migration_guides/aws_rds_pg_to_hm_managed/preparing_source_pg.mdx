...
Before running schema migration, confirm that every extension installed on the source is available on the destination at the same version. Each extension must be available on the destination at the same version as installed on the source. If the version isn't found, schema migration fails.
