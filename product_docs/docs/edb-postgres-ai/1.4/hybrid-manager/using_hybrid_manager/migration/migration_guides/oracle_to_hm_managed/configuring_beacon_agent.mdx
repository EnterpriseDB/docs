...
If you'd rather not embed a password in the DSN at all, the agent also supports Oracle Wallet for Oracle connections. See [Authenticating to source databases](/tools/agent/authentication/) for setup steps and other password-less options.
!!!
...
1.  Ensure the Oracle Instant Client environment variables are set in the same shell session where you'll run the agent:
...
-  For password-less authentication options such as Oracle Wallet, see [Authenticating to source databases](/tools/agent/authentication/).
