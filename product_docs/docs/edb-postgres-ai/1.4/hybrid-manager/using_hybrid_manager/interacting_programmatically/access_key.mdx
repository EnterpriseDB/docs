...
1.  Enter a name for the machine user.
1.  Assign the roles the machine user needs.
1.  Select **Create**.
...
1.  Create a new access key for the user or machine user, following the steps in [Creating a machine user](#creating-a-machine-user) or [Creating your personal access key](#creating-your-personal-access-key), while the existing key is still active.
1.  Update all consumers (applications, scripts, and integrations) to use the new access key.
1.  Confirm the new access key is working as expected. See [Using the access key](using_access_key.mdx).
1.  Delete the old access key. See [Deleting a key](#deleting-a-key).
