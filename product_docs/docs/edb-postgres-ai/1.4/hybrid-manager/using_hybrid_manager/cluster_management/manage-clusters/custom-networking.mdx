...
Custom networking is available for single-node, high-availability, and distributed high-availability clusters (including witness groups), as well as analytics clusters. You can set it independently for a cluster's primary connection and for its [read-only connection](../create-clusters/cluster-settings.mdx#connections-read-only).
