...
description: Learn about creating data groups when creating a Hybrid Manager cluster.
---

The **Data Groups** tab is available when you create or edit a distributed high-availability cluster. Use it to create and manage *data groups* for distributed high-availability clusters. A data group is typically a regionally close group of data nodes that are configured to replicate data within the group and respond to application queries. A distributed high-availability cluster can be made of one or more data groups that also replicate to each other, enabling geodistributed clusters.

## Creating data groups

1.  With **Distributed High Availability** selected on the **Cluster Info** tab, select the **Data Groups** tab.
...
You can select the recommended three-data-node configuration or a two-data-node configuration. For more information on the node architectures see, [Distribution high availability](../../../overview/database_types/distributed_highavailability.mdx).

**Deployment Location** — Select the location for the data group to be deployed into. The first location in the list is in the Kubernetes cluster hosting the EDB Postgres AI service. Where Hybrid Manager (HM) has been configured in a multi-DC environment, you can select different deployment locations for each data and witness group to distribute the database cluster across multiple data centers.
...
For **Distributed High Availability** clusters, you can enable a read-only connection per data group to route read-only operations to nodes that aren't currently designated as the write leader, reducing load on the write leader and improving overall performance.
...
Once enabled, the read-only connection string for each data group is available on the **Connect** tab of the cluster details.
...
For a distributed high-availability cluster spanning multiple locations, each data group is created independently and can safely use a different customer-managed key per location. See [TDE across data centers](/edb-postgres-ai/current/hybrid-manager/install/configuration/multi_dc/tde/) for details on how per-group keys are applied and how this differs from replica clusters.
...
-   To duplicate a group, select **Duplicate**. Then edit the new group to differentiate it from the original. In multi-DC environments, you can select a different deployment location for the duplicated data group.
