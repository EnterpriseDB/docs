---
title: Configuring the EDB Postgres AI agent
navTitle: Configure the EDB Postgres AI agent
description: Learn how to configure the agent for monitoring with EDB Postgres AI.

---

Configure the EDB Postgres AI agent (deployed as `beacon-agent`) to allow it to connect to HM and fetch monitoring data from the source database.
...
-  If you plan to use schema ingestion as part of a migration workflow, `pg_dump` must be installed on the machine running the agent. See [Configuring your auxiliary machine](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migration_guides/self_managed_pg_to_hm_managed/auxiliary_machine/#configuring-the-environment) for installation instructions and compatibility rules.

## Obtain EDB Postgres AI connection parameters
...
The DSN is the connection string the EDB Postgres AI agent uses to connect to your source database. Set the `clusters.nodes.dsn` parameter using an environment variable `$DSN` (recommended) or direct URL format.
...
If you'd rather not embed a password in the DSN at all, the agent also supports Oracle Wallet, Postgres pgpass files, and Postgres client certificates. See [Authenticating to source databases](/tools/agent/authentication/) for setup steps.
!!!
...
To perform an initial test of the agent, you can configure it to send the data—usually sent to EDB Postgres AI—to your terminal (standard output) instead. This helps you quickly verify whether the agent can successfully collect data from the source database and lets you preview the gathered information.
...
|---------------------------------------|----------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `agent.access_key`                    | `$BEACON_AGENT_ACCESS_KEY` | Access key you obtained from the HM console when you [created a machine user with the `estate ingester` role](../create_machine_user.mdx). After creating the key, you can store it as the environment variable `$BEACON_AGENT_ACCESS_KEY` for usage with the EDB Postgres AI agent.                                                                            |
...
| `agent.plaintext`                     | `false`                    | Disables TLS when set to `true`. Defaults to `false`. In almost all cases, this should remain `false` or unset. Only set to `true` for local testing or when connecting to an endpoint that does not use TLS.                                                                                                                      |
| `agent.feature_flag_interval`         | `10m`                      | How often the agent syncs feature flags with the server. Defaults to `10m`. In almost all cases, the default is preferred.                                                                                                                                                                                                        |
...
| `provider.onprem.check_databases_DSN` | N/A                        | Enables verifying all DSN connections and reports any errors to the terminal by indicating the source of the error with the resource ID during runtime. The check is performed when the EDB Postgres AI agent runs without any [CLI subcommands/options](/tools/agent/agent_ref/), or when it runs with the `--config` subcommand. The default is set to false. |
| `dispatcher.location_id`              | `<location_id>`            | A unique identifier used by the EDB Postgres AI for this agent/location. Consider setting this to the same value as `provider.onprem.host.resource_id` for convenience.                                                                                                                                                                             |

These settings are relevant to the database configuration. All `clusters.*` parameters are nested under `provider.onprem` in the YAML file.

| YAML database settings                    | Placeholder             | Guidelines                                                                                                                                                                                                                                                                                                                                                                      |
|-------------------------------------------|-------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `clusters.resource_id`                    | `<cluster_resource_id>` | Assign a unique identifier to your database cluster. Encase in double quotes. <br /> You must begin identifiers with an alphanumeric character (`A-Z, a-z, 0-9`); in between you may also include hyphens (`-`), underscores (`_`) and periods (`.`). You must not use any other characters. <br /> Example: `An_I.D.-1`                                              |
| `clusters.name`                           | `"<instance_name>"`     | Assign a unique name to your database cluster.                                                                                                                                                                                                                                                                                                                                  |
| `clusters.manager`                        | `"other"`               | Collects statistics for cluster manager. Possible values are `"patroni"`, `"repmgr"`, `"efm"`, `"pgd"`, `"other"`.                                                                                                                                                                                                                |
| `clusters.pgd_cluster.bdr_database`       | `<bdr_database_name>`   | Required when `clusters.manager` is set to `"pgd"`. Specifies the name of the BDR database in the PGD cluster.                                                                                                                                                                                                                                                                 |
| `clusters.nodes.resource_id`              | `<node_resource_id>`    | Assign a unique identifier to your database node. Single-node instances require a single entry here. Multi-node instances require an entry per node. Encase in double quotes. Reuse the same id when configuring the DMS agent for migrations. This consistency is essential for the DMS to correctly correlate the resource across both the EDB Postgres AI agent and DMS agent components. <br /> You must begin identifiers with an alphanumeric character (`A-Z, a-z, 0-9`); in between you may also include hyphens (`-`), underscores (`_`) and periods (`.`). You must not use any other characters. <br /> Example: `An_I.D.-1` |
...
These settings are relevant to host data. All `host.*` parameters are nested under `provider.onprem` in the YAML file.
...
-   [Monitoring EDB Failover Manager Cluster](efm_cluster/) ► Learn how to use the same agent config file to monitor the EDB Failover Manager (EFM) Cluster.
-   [Enabling usage reporting in EDB Postgres AI](usage_reporting/) ► Learn how to complement monitoring efforts by providing EDB with basic insights into product usage.
...
-   [Use the agent to assess your Oracle database for migration](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migration_guides/oracle_to_hm_managed/) ► Use the EDB Postgres AI agent to asses Oracle databases and extract schema for conversion with the Migration Portal. 
-   [Authenticating to source databases](/tools/agent/authentication/) ► Learn about password-less authentication options such as Oracle Wallet, pgpass, and Postgres client certificates.
-   [Perform multiple concurrent schema ingestions](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/monitoring/mon_ext_dbs/mon_with_agent/configure_agent/schema_ingestion/#performing-multiple-concurrent-schema-ingestions) ► Adjust the EDB Postgres AI agent configuration to tune EDB Postgres AI agent's resource utilization and support the ingestion of a large number of databases.