...
!!!note Postgres schema extraction prerequisite
When you enable schema ingestion for a Postgres database as part of a migration workflow, `pg_dump` must be installed on the machine running the agent and meet compatibility requirements. See [Configuring your auxiliary machine](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migration_guides/self_managed_pg_to_hm_managed/auxiliary_machine/#configuring-the-environment) for installation instructions, compatibility rules, and credential details.
!!!
