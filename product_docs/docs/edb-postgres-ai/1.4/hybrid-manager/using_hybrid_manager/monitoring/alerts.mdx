...
[DOCS-4068-1]: # (SME-CONFIRM: "Estate-level alerts" view below is from a partially-verified eng doc — confirm this UI path exists before merging.)
...
[DOCS-4068-8]: # (SME-CONFIRM: What does "Fixed" mean for the Threshold value on WAL Keeps Growing, WAL Running Out of Disk Space, and Backup Failures? Confirm so we can clarify the wording.)

[DOCS-4068-3]: # (SME-CONFIRM: Severity/Threshold values below are from a partially-verified eng doc — confirm before merging. "CPU Utilization" and "Disk Usage" are left unconfirmed: the eng doc lists similar alerts, "CPU Usage" and "Storage Used" at High/90%, but it's unclear if they're the same.)

### Host-level alerts

| Alert | Default severity | Threshold |
|-------|-------------------|-----------|
| CPU Utilization | Pending SME confirmation | Pending SME confirmation |
| Disk IOPS | High | 10,000 count |
| Disk Usage | Pending SME confirmation | Pending SME confirmation |
| Memory Usage | High | 90% |
| OOM Killed PG Pod | High | 10 minutes |

[DOCS-4068-4]: # (SME-CONFIRM: The third bullet below — host-level alerts limited to HM-managed clusters — is from a partially-verified eng doc, more specific than the old "not available for external clusters" line. Confirm before merging.)
...
[DOCS-4068-9]: # (SME-CONFIRM: The second and third bullets above say the same thing, and "technical limitations" reads oddly. Depends on DOCS-4068-4 — confirm so we can remove the redundant bullet and reword.)
...
[DOCS-4068-6]: # (SME-CONFIRM: "Custom alert rules" below is from a partially-verified eng doc — confirm before merging.)
...
[DOCS-4068-11]: # (SME-CONFIRM: Should we add a one-line description to each example below \(PagerDuty, Slack, SMTP\)? Confirm the routing behavior for each.)
