...
### Real alert rule examples

The following examples show actual PromQL expressions from the deployed PrometheusRule resources.

**Postgres connection saturation** — Fires when a cluster reaches 90% of its configured `max_connections`:

```promql
# Alert: PostgresNinetyPercentConnectionCount
# Severity: error | For: 15m
...
**Pod OOM killed** — Detects non-Postgres pods that were OOM killed and restarted within 10 minutes:

```promql
# Alert: PodOOMKilled
# Severity: error | For: 10m
...
**Node filesystem running low** — Fires when a filesystem drops below 5% available space (warning) or 3% (critical):
...
**CloudNativePG cluster not ready** — Fires when a database cluster remains in a not-ready state for over 1 hour:

```promql
# Alert: CNPClusterNotReady
...
You can extend HM's built-in alerting with your own metric and log alert rules:
...
Both alert types feed into the same Alertmanager routing pipeline described in [Alertmanager configuration](#alertmanager-configuration-advanced), so any receiver you configure (PagerDuty, Slack, and email) handles built-in and custom alerts the same way.
...
## Alertmanager configuration (advanced)
