...
HM ships a default set of platform alert rules grouped into the categories below, plus recording rules that pre-compute frequently used metrics. HM 1.4 LTS extends this inventory with additional rule groups covering the metrics-pipeline agent introduced in the LTS line.
...
Both alert types feed into the same Alertmanager routing pipeline described in [Alertmanager configuration](#alertmanager-configuration-advanced), so any receiver you configure (PagerDuty, Slack, email) handles built-in and custom alerts the same way.
