...
| ------------------------------------------ | ---------- | ---------------------------- | --------------------------- | ------------------------ |
| Estate page ► <br/> Postgres table         | 15s        | 15s                          | 15s                         | 15s                      |
| Cluster view ► <br/> Monitoring tab        | 15s        | 15s                          | 15s                         | 15s                      |
| Cluster view ► <br/> Query Diagnostics tab | 5m         | 5m                           | na                          | 5m                       |
| Cluster view ► <br/> Recommendations tab   | 10m        | 10m                          | na                          | 10m                      |
| Cluster view ► <br/> Logs tab              | &lt;1s     | &lt;1s                       | na                          | &lt;1s                   |

The Monitoring and Query Diagnostics tabs use separate data pipelines, which produces different update frequencies. Query Diagnostics relies on the agent to collect statistics and send them in ingestion batches to HM's central data store. The Monitoring tab sources data from a Thanos time-series store &mdash; for HM-managed clusters, metrics are scraped directly, while for external or self-managed CNPG clusters, the agent is responsible for metric collection.

The scraping intervals (distinct from the refresh intervals above) for self-managed clusters can be configured under `provider.onprem.clusters.nodes.metrics.stats` in the [EDB Postgres AI agent configuration YAML](../mon_ext_dbs/mon_with_agent/configure_agent.mdx#prepare-a-configuration-file). The default values are recommended for most cases — you typically only need to enable this section without overriding the defaults. For CSP-managed clusters, backend refresh intervals aren't customer-configurable at this release.
