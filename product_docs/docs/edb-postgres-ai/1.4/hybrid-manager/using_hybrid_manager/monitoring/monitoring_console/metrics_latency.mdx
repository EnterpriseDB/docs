...
Configure the scraping intervals (distinct from the refresh intervals above) for self-managed clusters under `provider.onprem.clusters.nodes.metrics.stats` in the [EDB Postgres AI agent configuration YAML](../mon_ext_dbs/mon_with_agent/configure_agent.mdx#prepare-a-configuration-file). We recommend using the default values for most cases — you typically only need to enable this section without overriding the defaults. For CSP-managed clusters, backend refresh intervals aren't customer-configurable.
