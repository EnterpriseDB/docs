...
Fix: check with your HM administrator that the AI scenario was selected at install or upgrade time. See [HM installation scenarios](../../install/install_guide/installing/install_hm/#example-hybridcontrolplane-manifest).
...
**Fix 1:** the administrator must register a chat-completion model and apply the `chatbot-gen-content` function. See [Configuring the model](configure/#configuring-the-model).
...
**Fix 1:** try a different model from the model selector if more than one is registered. Otherwise, check the model server's status in **AI Factory > Model Serving** ([Monitor InferenceService](/edb-postgres-ai/1.3/ai-factory/model/monitor-inferenceservice)).
...
**Fix 4:** remove `chatbot-default-thinking-level` from the model's **Functions** field to fall back to the model's default, or switch to a model that documents support for per-request thinking-level configuration. See [Configuring model functions](configure/#configuring-model-functions).

## Chat Agent doesn't answer my question

Problem: Chat Agent says it can only help with HM topics.

Cause: Chat Agent answers only HM-related questions. Off-topic questions — general knowledge, math, creative writing, and other products — are out of scope.
...
If the issue persists, capture these details before contacting [EDB Support](https://support.enterprisedb.com):
