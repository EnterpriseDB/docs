...
When a user's role assignment changes, the new permissions take effect on the API within about 10 seconds, without requiring the user to re-authenticate. In the console, the user sees the updated permissions the next time they navigate to a new page or reload the current one.
