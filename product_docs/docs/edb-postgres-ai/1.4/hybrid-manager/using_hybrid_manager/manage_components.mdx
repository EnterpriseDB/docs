...
HM is made up of individual components — each a distinct service deployed to its own Kubernetes namespace (for example, `upm-istio`, `upm-marketplace`). In most cases these components install and run automatically. Occasionally, you may need to interact with a specific component directly to skip one that conflicts with your environment, to recover one that has entered an error state, or to exclude one that your deployment doesn't require. Use `hm` commands from [`edbctl`](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/edbctl/), the Hybrid Manager CLI, to suspend, resume, or retry a component at runtime. Use the `disabledComponents` field in the `HybridControlPlane` CR to prevent a component from installing.
