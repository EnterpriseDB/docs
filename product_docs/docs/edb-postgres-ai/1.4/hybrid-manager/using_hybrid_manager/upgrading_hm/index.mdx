...
When upgrading a multi-DC deployment, upgrade the primary location first, then upgrade each secondary (data-only) location to the same version immediately after. For the full procedure, see [Upgrading a multi-DC deployment](/edb-postgres-ai/current/hybrid-manager/install/configuration/multi_dc/multi-dc/#upgrading-a-multi-dc-deployment).
