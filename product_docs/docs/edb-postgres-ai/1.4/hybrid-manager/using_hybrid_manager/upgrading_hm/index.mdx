...
- If your spec lists only `core` (without `ai`) and you use the OSS Library/marketplace apps, you must explicitly add `marketplace` to your `spec.scenarios` list. Do this **before** upgrading to HM 1.4.2 or later.

!!!important
If you don't add `marketplace` to your scenario list, kapp-controller and the marketplace apps are removed as no longer part of your selected scenarios. Removing an app also removes the data it holds, so add `marketplace` before you upgrade rather than after.
!!!

To keep the marketplace, add it to `spec.scenarios` in your `HybridControlPlane` CR before you start the upgrade:
...
If you omit `spec.scenarios` entirely, all scenarios — including `marketplace` — are installed, and no action is required.
