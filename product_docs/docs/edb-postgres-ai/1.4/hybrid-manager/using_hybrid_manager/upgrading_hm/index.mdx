...
Add `dbaas` at the same time if your spec lists only `core`. The `dbaas` scenario has existed since HM 1.4.1, but as an empty placeholder. HM 1.4.2 is the first release in which it carries components — `upm-api-gucs` and `pg4k-additional-controllers` — so a `core`-only installation loses them on upgrade.

In HM 1.4.2, if you omit `spec.scenarios` entirely, all scenarios — including `marketplace` — are installed, and no action is required. Don't rely on this default — list your scenarios explicitly.
