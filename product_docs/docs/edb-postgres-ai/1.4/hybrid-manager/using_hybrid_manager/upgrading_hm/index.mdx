...
Add `dbaas` at the same time if your spec lists only `core`. `dbaas` isn't new, but its contents are: in HM 1.4.1, `upm-api-gucs` and `pg4k-additional-controllers` belonged to `core`, and in 1.4.2 they move to `dbaas`. A 1.4.1 installation that pins `spec.scenarios` to `core` is running both components today, so after the move they fall outside every scenario it selects and the upgrade removes them. Nothing pulls `dbaas` in for you — it depends only on `core`, so there's no equivalent of the `ai` → `marketplace` dependency.
