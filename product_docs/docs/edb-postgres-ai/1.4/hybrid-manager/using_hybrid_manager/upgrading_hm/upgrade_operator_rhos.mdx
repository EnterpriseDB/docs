...
On Red Hat OpenShift (RHOS), the operator is installed and upgraded through OperatorHub (OLM). This page describes how to move an operator that was installed from the `innovation` channel (`2026.x.x`) or the `lts-v1.3` channel (`1.3.x`) onto the `stable` channel. You land on the latest `2.x` release the channel offers, so the exact version depends on when you do it — check the [operator release notes](/hm-operator/latest/rel_notes/) for what that is. Although a `2.x` version is a lower number than the `2026.x.x` Innovation versions, selecting the `stable` channel correctly presents it as the upgrade.
