...
-   **Restore HM across three locations**: For the highest level of resilience, HM supports a three-location architecture: Location 1 (primary) and Location 2 (synchronous standby) provide zero data loss through synchronous replication, while Location 3 receives asynchronous replication and can take over if both primary locations are lost. For EDB Postgres Distributed (PGD) deployments, Location 3 also hosts a PGD Witness Group to maintain Raft consensus quorum during a full-location failure.
...
3.  **Database re-provisioning** — Log in to the restored HM console. Databases will appear as **Deleted** or **Missing**. Use the **Restore** function in the HM console to spin up new Postgres database clusters that point to the original data in object storage.
    ...
    While Velero restores the cluster definition from the time of the last backup, the re-provisioning process uses Barman's continuous WAL (Write Ahead Log) archiving to perform a Point-in-Time Recovery. This allows you to recover data that is much more recent than your last Velero snapshot.
    !!!
...
Failover during a region switch can take up to an hour due to Azure Blob Storage's geo-replication lag. Factor this into your RTO planning if you use Azure for multi-location DR.
!!!
