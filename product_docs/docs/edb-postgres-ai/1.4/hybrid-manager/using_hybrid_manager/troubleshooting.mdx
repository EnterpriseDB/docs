...
| na         | TDE across data centers | Physical-join PGD data group stuck with `FATAL: key unwrap command produced no output` | The joining pod's identity cannot decrypt under the *source* group's KEK during the physical join. | Grant the target location's workload identity decrypt access on the source group's KEK for the duration of the join. |
...
accompanied by a KMS access error from the KMS provider:

- Azure Key Vault: `403 Forbidden` / `Forbidden` (message names the vault key).
- AWS KMS: `AccessDeniedException`.
- Google Cloud KMS: `PermissionDenied`.
- HashiCorp Vault: `permission denied` on the Transit engine key.

The KMS error names the **source** location's key (for example, `region-key1`), not the joining group's own configured key (for example, `region-key2`).

#### Resolution

During a PGD physical join the target's manager reads the DEK out of the copied `pg_encryption/key.bin` **using the KEK named in that file's header** — which is the source group's KEK — and then re-wraps the DEK under the target group's own configured KEK before the postmaster starts. The target's workload identity therefore needs KMS decrypt access on **two** keys for the duration of the join:

- The **source group's KEK** (to unwrap the copied `key.bin` once).
- The **target group's own configured KEK** (to write the re-wrapped `key.bin`).

Confirm the following before retrying the join:

1. The target location's workload identity has decrypt permission on the source group's KEK in the same KMS.
2. The KMS is reachable from the target location's network (no vault firewall / VPC endpoint gap).
3. The source group's KEK identifier (version, ARN, resource name) has not been deleted or disabled since the source group was provisioned.

Once the join completes and the target group is running, decrypt access on the source group's KEK is no longer required by the target and can be revoked from the target's identity if your security policy calls for it.
...
1. On the replica pod, read the `key.bin` header to confirm which KEK is actually in use:
  ...
  The output is a JSON header that includes `kvKey`, `kvKeyVer` (or the AWS/GCP/Vault equivalent). If it names the primary's KEK, the replica is bootstrapped under the primary's KEK, and the replica's own configured KEK is not in effect.
...
Pod restart alone is not a reliable diagnostic here. If the replica's identity retains vault-level access to the primary's KEK, restart succeeds silently under the primary's KEK regardless of what you configured on the replica.
...
The customer key referenced by the replica part of the request is not registered in the replica's location in HM. Customer keys in HM are stored per-location — one row per (project, location, key ID).
