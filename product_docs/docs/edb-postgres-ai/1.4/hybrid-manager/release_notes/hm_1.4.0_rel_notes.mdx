...
-   **Selective installations**: Customize your HM deployment by enabling only the capabilities you need using various "scenarios"— Observability & DBaaS, Migrations, Analytics, or AI — at install time or later, reducing resource usage and simplifying component management.
...
-   **Unified schema migration**: Migrate database schemas from Oracle, Postgres, and EPAS sources to Postgres and EPAS destinations directly through HM, with Oracle SQL compatibility assessment and API-driven automation via AI agents.
...
-   **Agent Factory enhancements**: Visual AI Pipeline Designer, multistep composable pipelines, hybrid search, the `summarize_text_aggregate()` SQL function, and expanded Langflow components including EDB Airman MCP Server, EDB Embedded Models, and EDB Vector Index.

-   **Langflow integration**: Langflow is natively integrated into the HM Launchpad. You can build AI-powered agents and workflows within HM, deploy flows as managed workloads, and use a Langflow template for end-to-end database migrations from Oracle, Postgres, and EPAS sources to Postgres and EPAS destinations.
...
| Type | Description |
| ---- | ----------- |
| Feature | Added support for HM deployments on Microsoft Azure Kubernetes Service (AKS), with full support for AI Pipelines and the Data Migration Service. |
...
| Feature | Added nodeSelector and tolerations support in edbctl for PSR cluster create and edit operations, enabling dedicated and isolated node pools for Postgres workloads. |
...
| Enhancement | You can now change network configuration for Witness groups through the UI. |
...
| Enhancement | Added support for configuring kapp-controller TLS settings in the HCP operator Helm chart. You can now set a custom CA certificate trust or disable TLS verification for private container registries. |
...
| Feature | Added a Langflow template for AI-powered, end-to-end database migrations from Oracle, Postgres, and EPAS sources to Postgres and EPAS destinations, including provisioning HM-managed clusters, migrating schemas, and performing snapshot plus streaming data migration. Oracle sources require schema conversion via Migration Portal before running the migration flow. |
