...
!!!Warning PostgreSQL cluster rolling restart on upgrade to HM 1.4.0
Upgrading to HM 1.4.0 triggers a rolling restart of all managed PostgreSQL clusters due to a bundled `barman-cloud-plugin` sidecar image bump from v0.11.0 to v0.13.0. Applications will see a brief connection interruption during this period, so plan your HM upgrade inside your maintenance window.
!!!
...
| --- | --- | --- |
| Hybrid Manager (HM) Operator | `edb-hcp-operator` | 2.0.0 |
| Hybrid Manager operand images | `operandSet` | 1.1.7 (timestamp 2606080821) |
| [EDB Postgres AI agent](/tools/agent/) | `beacon-agent` | 1.392.15 |
| EDB Postgres AI agent - Helm chart | `beacon-agent` (Helm chart) | 1.367.0 |
| [DMS agent](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/cdcagent_compatibility/) | `cdcagent` | 1.13.12 |
| [Hybrid Manager CLI](/edbctl/1/) | `edbctl` | 1.7.0 |
| [AIDB](/aidb/7/) | AIDB | 7.3.2 |
| [Postgres Analytics Accelerator](/pgaa/1.9/) | PGAA | 1.9.1 |
| Postgres File System | PGFS | 3.0.0 |
| [Distributed High Availability](/pgd/6/) | PGD | 6.4.0 |
...
-   **[Selective installations](/edb-postgres-ai/current/hybrid-manager/install/install_guide/planning_arch/#choosing-an-installation-scenario)**: Customize your HM deployment by enabling only the `scenarios` you need — `core`, `dbaas`, `migration`, `ai`, and `analytics` — at install time or later, reducing resource usage and simplifying component management.
...
-   **[Custom load balancer configuration](/edb-postgres-ai/current/hybrid-manager/install/install_guide/sys_reqs/#51-load-balancer-controller)**: HM supports custom load balancer annotations for use cases such as MetalLB address pool assignment and disabling Azure Private Link on Azure private clusters.
...
-   **[Unified schema migration](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migration_guides/self_managed_pg_to_hm_managed/migrating_schema/)**: Migrate database schemas from Oracle and Postgres sources to Postgres destinations directly through HM, with Oracle SQL compatibility assessment and API-driven automation via AI agents.
...
-   **[Enhanced migration risk assessment](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migrations/)**: Granular assessments for source databases covering replication slot health, WAL generation rates, and risky table configurations (primary keys, `REPLICA IDENTITY`, `TOAST`, and unsupported data types for the Data Migration Service).

-   **[Advanced observability](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/cluster_management/view_clusters/health_reports/)**: Daily cluster health reports, schema change detection, new query detection, a metrics comparison tool, and enhanced monitoring dashboards.
...
| Deprecation | Deprecated the `bootstrap` and `edbpgai-bootstrap` Helm chart installation methods. **Action required:** migrate to the unified HM Helm chart. See [Migrating from bootstrap to the HM operator](/edb-postgres-ai/current/hybrid-manager/install/migrate_bootstrap_to_operator/). |
