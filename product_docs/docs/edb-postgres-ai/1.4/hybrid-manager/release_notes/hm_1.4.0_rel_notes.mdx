...
| Change | Automated the intermediate steps required when upgrading Hybrid Manager to a new version. Previously, upgrades required manually executing a series of Kubernetes operations such as patching deployments, annotating resources, and applying CRDs. These steps are now handled automatically once the user initiates an upgrade by specifying the target version. |
