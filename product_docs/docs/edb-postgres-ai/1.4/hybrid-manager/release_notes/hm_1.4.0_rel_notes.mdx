...
-   **Selective installations**: Customize your HM deployment by enabling only the scenarios you need — Observability & DBaaS, Migrations, Analytics, or AI — at install time or later, reducing resource usage and simplifying component management.
...
| Type | Description |
| ---- | ----------- |
| Platform support | Added support for HM deployments on Microsoft Azure Kubernetes Service (AKS). |
...
| Feature | HM now supports selective installations. Customize your deployment by enabling only the scenarios you need (Observability & DBaaS, Migrations, Analytics, AI) at install time or later, reducing resource usage and simplifying component management. |
...
| Feature | Added the ability to automatically scale up CPU and/or memory resources for HM-managed Postgres clusters when usage thresholds are exceeded, via the **Automations** engine. |
...
| Enhancement | Simplified the multi-location (multi-dc) deployment procedure. Multi-dc topology is now primarily controlled via standard HM Configuration files |
...
| Enhancement | HM now collects and displays Postgres cluster status metrics from self-managed EDB Failover Manager clusters. |
...
| Enhancement | HM Postgres cluster APIs now provide more visibility when provisioning fails, enabling self-service diagnostics and faster time to resolution. |
| Enhancement | Improved field handling in the **Settings** > **Identity Providers** management page. |
| Enhancement | Added advanced compression to Postgres backup capabilities through Barman Cloud integration. WAL compression is now enabled by default, using Snappy compression for basebackups and zstd for WAL files. |
...
| Enhancement | Added support for node selectors and tolerations in the Postgres cluster provisioning and patch API for advanced workload isolation. |
...
| Platform support | Added support for Kubernetes 1.34, 1.35, 1.36 and OpenShift 4.21. || Platform support | Added support for running HM components on ARM-based architectures, enabling deployments on modern ARM platforms and cloud instances. |
...
| Change | Decoupled the Hybrid Manager operator from HM releases to enable independent versioning and deployment cycles. |
...
| Change | The edbctl CLI now includes `hm status`, `hm component suspend`, `hm component resume`, and `hm component retry` commands. |
...
| Enhancement | Enhanced preflight checks to validate object storage connectivity before deployment begins, preventing failures caused by incorrect object storage secrets. |
| Enhancement | Added support for configuring all PGD GUCs through Hybrid Manager, including previously unavailable settings like `bdr.ddl_replication`. |
