...
1.  One of the secrets you create requires an NGC API key. Create one following the [NVIDIA NGC documentation](https://docs.nvidia.com/ngc/gpu-cloud/ngc-user-guide/index.html#ngc-api-keys) to enable model image pulls.
...
1.  You can create a Fernet key secret and NGC key secrets using the `edbctl` CLI.
    ...
    - If you're running the installation via a CI/CD pipeline, you must suppress interactive prompts. The method for achieving this depends on your `edbctl` version:
...
1.  If you plan to pull models from HuggingFace, create a `hf-secret` containing your HuggingFace access token:
    ...
    [DOCS-3788, Confirm with engineer: is HuggingFace access token support confirmed for HM 2026.4 (IR) / 1.4 (LTS) GA, or was that version pulled from edbctl source and still unconfirmed?]: #

1.  The Hybrid Manager (HM) administrator must keep the Fernet key safe and back it up. The secret name and namespace depend on the version of HM you're running.
...
1.  Update the bucket's settings with the cross-origin resource sharing (CORS) configuration: 
    ...
    Use your provider’s management console or CLI to configure CORS with Hybrid Manager.
