...
!!!important
**Service principal role assignments.** The service principal (`client_id`/`client_secret`) needs
...
* **`Storage Account Contributor`** — control-plane access. The logging stack (Loki, via its
  Thanos object-store client) calls the Azure blob **Get Account Information** API at startup, which
  requires `Microsoft.Storage/storageAccounts/blobServices/getInfo/action`. That action is **not**
  included in `Storage Blob Data Contributor` (or any `Storage Blob Data *` role).
