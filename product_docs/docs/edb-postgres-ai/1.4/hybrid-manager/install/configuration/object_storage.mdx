...
* **`Storage Account Contributor`** — control-plane access. The logging stack (Loki, via its
  ...
  requires `Microsoft.Storage/storageAccounts/blobServices/getInfo/action`. `Storage Blob Data Contributor`
  (and any `Storage Blob Data *` role) doesn't include that action.
