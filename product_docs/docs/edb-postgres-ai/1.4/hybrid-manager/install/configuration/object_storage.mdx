...
To check or update your HM backup retention period, see [Configuring backups to support disaster recovery](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/hadr/backup_considerations/).
!!!
...
Grant the service account bucket-scoped `roles/storage.admin` on the target bucket. Hybrid Manager validates the secret with a bucket-metadata read (`storage.buckets.get`), which `roles/storage.objectAdmin` does not include — a key with only `objectAdmin` fails validation even though object access itself works. For least privilege, combine `roles/storage.objectAdmin` with `roles/storage.legacyBucketReader`. Allow 30–60 seconds for an IAM binding to propagate before retrying.
!!!
