...
The Beacon server on the primary and the Beacon agents on the secondaries must run the same Hybrid Manager version to communicate reliably and provide full feature functionality. In a multi-DC deployment, this means every location must run the same HM version. When you upgrade, upgrade the primary location first and then upgrade each secondary (data-only) location immediately after, so that all locations return to the same version. See [Upgrading a multi-DC deployment](multi-dc/#upgrading-a-multi-dc-deployment).
!!!
