...
- **Cross-cloud replicas aren't supported for TDE.** A multi-DC HM deployment already requires the same provider or on-premises family across all locations. TDE reinforces this: your KMS must be reachable from every location that hosts a replica.

## Distributed high-availability (PGD) clusters across data centers

A distributed high-availability cluster is composed of independent [data groups](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/cluster_management/create-clusters/data-groups/). Each data group is a separate Postgres cluster that participates in the cluster's logical replication. Every data group ends up encrypted under its own configured KEK once it's running, regardless of whether it's the seed group or a group added later:
