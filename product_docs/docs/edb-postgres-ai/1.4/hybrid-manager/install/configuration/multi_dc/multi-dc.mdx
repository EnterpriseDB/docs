...
The Beacon server (on the primary) and the Beacon agents (on the secondaries) must run the same Hybrid Manager version to communicate reliably and provide full feature functionality. For this reason, all locations in a multi-DC deployment must run the same HM version.

!!!important
Keep all connected locations on the same HM version. Mismatched versions between the primary and a secondary are only expected transiently, during the upgrade window itself. Don't leave a deployment with locations on different versions.
!!!
...
1. Upgrade each secondary (data-only) location to the same version immediately after, one at a time, using the same upgrade guide.
