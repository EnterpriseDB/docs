...
Using your configuration files (`primary.yaml` and `secondary.yaml`, or `primary.yaml`, `secondaryA.yaml` and `secondaryB.yaml` for a three-location setup), install the Hybrid Manager on each cluster. Follow [Phase 5: Installing Hybrid Manager](../../install_guide/installing/install_hm) for the full installation procedure.
