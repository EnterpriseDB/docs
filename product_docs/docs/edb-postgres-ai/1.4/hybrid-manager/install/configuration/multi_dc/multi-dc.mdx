...
1. Prepare two (or three) copies of your HM installation configuration file. Name them `primary.yaml`, `secondary.yaml` if you are deploying in two locations, or `primary.yaml`, `secondaryA.yaml` and `secondaryB.yaml` if you are deploying in three locations. These files will be either `values.yaml` files (Bootstrap) or `HybridControlPlane` CR manifests (Operator), depending on your installation method.
...
| Parameter | Operator CR path | Bootstrap values.yaml path | Requirement | Why it matters |
|-----------|-----------------|---------------------------|-------------|----------------|
| `beacon_location_id` | `spec.componentsParameters.upm-beacon.beacon_location_id` | `parameters.upm-beacon.beacon_location_id` | Must be unique per location. | Human-readable location identifier. Identifies the specific location in the HM console and API. |
| `internal_backup_folder` | `spec.globalParameters.internal_backup_folder` | `parameters.global.internal_backup_folder` | Must be unique per location AND match the format: `^[0-9a-z]{12}$`. | Separates database backups so that a restore doesn't pull the wrong data. |
| `metrics_storage_prefix` | `spec.globalParameters.metrics_storage_prefix` | `parameters.global.metrics_storage_prefix` | Must be unique per location. | Ensures observability data from each site is stored in its own directory. |
| `logs_storage_prefix` | `spec.componentsParameters.upm-loki.logs_storage_prefix` | `parameters.upm-loki.logs_storage_prefix` | Must be unique per location. | Prevents logs from different locations from overwriting each other in shared storage. |
...
Add the `clusterGroups` stanza to each `values.yaml`:
...
Using your configuration files (`primary.yaml` and `secondary.yaml`, or `primary.yaml`, `secondaryA.yaml` and `secondaryB.yaml` for a three-location setup), install the Hybrid Manager on each cluster. Follow [Phase 5: Installing Hybrid Manager](../../install_guide/install_hm) for the full installation procedure using your chosen method (Operator or Bootstrap).
