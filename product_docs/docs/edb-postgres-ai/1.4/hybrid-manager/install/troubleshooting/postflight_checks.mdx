...
* [Phase 5: Installing Hybrid Manager](../install_guide/installing/install_hm) &mdash; You have completed and verified the installation of HM. The deployment is running and you want to either check its state or troubleshoot failed checks.
...
#### Loki pods crash-loop on Azure with `403 AuthorizationPermissionMismatch`

If the failing pods are in the `edb-observability` namespace (`loki-write`, `loki-read`, or
`loki-backend`) and their logs show `unable to get Azure storage account info … 403 …
AuthorizationPermissionMismatch`, the object-storage service principal is missing the control-plane
role needed for the blob **Get Account Information** call. Grant **`Storage Account Contributor`** on
the storage account in addition to `Storage Blob Data Contributor` — see
[Configuring object storage](../configuration/object_storage/#azure-blob-storage).
