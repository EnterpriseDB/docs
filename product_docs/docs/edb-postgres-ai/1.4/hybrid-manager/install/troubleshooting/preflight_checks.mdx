...
-   [Phase 5: Installing Hybrid Manager](../install_guide/installing/install_hm/#install-hybrid-manager) — You have run the installation command and the process seems stuck.
...
After you apply your `HybridControlPlane` manifest, the operator:

1.  Reconciles the `HybridControlPlane` resource.
...
If the installation appears stuck after running the [installation Helm command](/edb-postgres-ai/current/hybrid-manager/install/install_guide/installing/install_hm/#install-hybrid-manager), inspect the preflight status:
...
Each secret requirement is associated with an **installation scenario**: `core`, `migration`, `ai`, or `analytics`. The operator uses the scenarios configured in `spec.scenarios` on the `HybridControlPlane` to filter which requirements are checked.
...
If the secret does not exist, create it following the documentation for [Preparing your environment](/edb-postgres-ai/current/hybrid-manager/install/install_guide/environment_prep/). Ensure the name and namespace match exactly what the preflight expects.
...
2.  Confirm the established scenario matches the scenarios established by the CTO / Architect / Lead Engineer during the [Planning architecture](/edb-postgres-ai/current/hybrid-manager/install/install_guide/planning_arch/#choosing-an-installation-scenario) phase. If the output of the command doesn't match the expected scenarios, it means that the installation was not carried out as planned, and one or several of the scenarios were skipped during the installation.
