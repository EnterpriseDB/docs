...
-   *A finalized and validated configuration ready for Hybrid Manager (HM) installation—either a `HybridControlPlane` Custom Resource manifest if using the recommended Operator method, or a `values.yaml` file if using the legacy "Bootstrap" method*
