...
* *A finalized and validated configuration ready for Hybrid Manager (HM) installation&mdash;either a `HybridControlPlane` Custom Resource manifest if using the recommended Operator method, or a `values.yaml` file if using the legacy "Bootstrap" method*
...
!!! Note Using the in-cluster registry?
If you chose the [in-cluster container registry](../configuration/in_cluster_registry) in Phase 2, follow that SOP for the operator namespace, the upstream pull secret, the operator install, and the image sync. Then return here for the remaining preparation steps (object storage, TLS, advanced features, and the rest of the HCP configuration).
!!!
...
[**SOP: Implementing TLS certificates**](/edb-postgres-ai/latest/hybrid-manager/install/configuration/cert-man/) describes the implementation options.
...
1. Assign a unique folder name (e.g., using a UUID generator) and provide it in your HM configuration under `parameters.global.internal_backup_folder`. 
...
1. **Operator method (HybridControlPlane CR) - Recommended** - Kubernetes Custom Resource used with the `edb-hcp-operator` chart
1. **Bootstrap method (values.yaml) - Deprecated** - Helm values file used with the `edbpgai-bootstrap` chart

!!! Warning
The **Bootstrap method** (`values.yaml` with `edbpgai-bootstrap` chart) is deprecated and will be removed in a future release. New installations should use the **Operator method** with the `HybridControlPlane` Custom Resource. See [Phase 5: Installing Hybrid Manager](./install_hm) for details.
!!!

Both operator and bootstrap method use the same configuration values, just in different formats. 
The secrets and preparation steps are identical for both approaches.
...
The Operator method is the recommended approach for all new installations. 
It provides better version management and Kubernetes-native resource handling.
