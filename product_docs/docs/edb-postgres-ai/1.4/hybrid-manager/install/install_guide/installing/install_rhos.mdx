...
redirects:
- /edb-postgres-ai/latest/hybrid-manager/install/install_guide/install_rhos/
deepToC: true
---
...
* *A fully deployed Hybrid Manager (HM) platform on Red Hat OpenShift, accessible via the HM console.*
...
On OpenShift, the global pull secret must include the EDB container registry endpoint and token so all cluster nodes can pull EDB images. Run [`edbctl`](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/edbctl/), the Hybrid Manager CLI, to update it:
...
- **`spec.globalParameters.portal_domain_name`**: Your HM console FQDN
