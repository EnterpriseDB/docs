...
#### Alternative: in-cluster container registry

If you're deploying into a fully air-gapped cluster and don't want to maintain an external private registry, you can use the registry embedded in the operator itself. In that case the relevant fields differ:
...
The in-cluster registry has its own install and image-sync flow (operator chart values, port-forward, NodePort exposure) that you must complete before applying the `HybridControlPlane` CR.

!!! Warning Security
The in-cluster registry serves plain HTTP without authentication and exposes itself to nodes through a NodePort. Use it only when every cluster node is on a private subnet with no public IP.
!!!

See [Using the in-cluster container registry](../configuration/in_cluster_registry) for the full procedure.
