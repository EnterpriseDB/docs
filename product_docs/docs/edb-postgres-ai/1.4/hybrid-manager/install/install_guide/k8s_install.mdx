...
2.  **Install [`edbctl`](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/edbctl/) (the Hybrid Manager CLI):**
    Follow the [edbctl installation instructions](/edbctl/latest/install/installing/) for your operating system.

    EDB distributes `edbctl` as a standalone package with its own release cadence. See the [compatibility matrix](/edbctl/latest/compatibility_matrix/) for the `edbctl` version to pair with this Hybrid Manager release.
...
4.  **Install platform CLIs (Environment dependent):**
    ...
    * **OpenShift:** [Install oc CLI](https://docs.redhat.com/en/documentation/openshift_container_platform/latest/html/cli_tools/openshift-cli-oc)
...
1.  **Verify Internet/Registry access**
    ...
    ```bash
    curl -I https://docker.enterprisedb.com/v2/
    ```
...
| **Red Hat OpenShift Service on AWS (ROSA) | [**Get started with ROSA**](https://docs.aws.amazon.com/rosa/latest/userguide/getting-started.html) | n/a
...
!!!important
Set the control-plane pool's **per-node pod limit to at least 60** (`maxPods`). The control-plane
stack — including the HM system databases — places ~150 pods on this pool, which exceeds the
default on some platforms (**AKS Azure CNI defaults to `maxPods=30`**) and leaves control-plane
pods unschedulable even when CPU and memory are sufficient. On AKS this is set at node-pool
creation only (`az aks nodepool add … --max-pods 110`). See
[Control plane nodes](./sys_reqs/#31-control-plane-nodes).
!!!
...
!!!note 
Required only for deployments where the `ai` installation scenario is enabled. This scenario is included by default unless it is manually excluded via the `scenarios` configuration parameter.
!!!
...
* **Pods per node:** Verify the control-plane nodes allow **≥ 60 pods** each (`kubectl get nodes -o jsonpath='{range .items[*]}{.metadata.name}{"\t"}{.status.allocatable.pods}{"\n"}{end}'`). On AKS Azure CNI the default of 30 is too low and must be raised at node-pool creation.
...
* **Volume snapshots:** Confirm a `VolumeSnapshotClass` exists for backups (`kubectl get volumesnapshotclass`). Some managed distributions — notably GKE — provide the CSI snapshot controller but no `VolumeSnapshotClass` object, so you must create one. See [Volume snapshots](./sys_reqs/#61-volume-snapshots).
...
Next sync images to your local registry, stage your secrets, configure TLS, set up any relevant advanced features, and finalize your HM configuration.

**[Proceed to Phase 4: Preparing your environment](./environment_prep) →**
