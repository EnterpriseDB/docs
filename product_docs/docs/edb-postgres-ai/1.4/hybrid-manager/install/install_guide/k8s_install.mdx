...
2.  **Install [`edbctl`](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/edbctl/) (the Hybrid Manager CLI):**
    ...
    `edbctl` is distributed as a standalone package with its own release cadence. See the [compatibility matrix](/edbctl/latest/compatibility_matrix/) for the `edbctl` version to pair with this Hybrid Manager release.
