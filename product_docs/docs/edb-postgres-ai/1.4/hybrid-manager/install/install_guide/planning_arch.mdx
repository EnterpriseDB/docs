...
| **Scenarios**                         | core, migration, ai, marketplace                                                                                                        | dbaas, analytics, ai                                                                                         |
...
-   **EDB Postgres® AI for CloudNativePG™ Global Cluster** — streaming replication across Kubernetes clusters and failure domains using CloudNativePG's distributed topology. See the [EDB Postgres AI for CloudNativePG Global Cluster documentation](https://www.enterprisedb.com/docs/postgres_distributed_for_kubernetes/latest/).
...
**Default behavior:** If the `scenarios` parameter is omitted, the operator applies a default set, and this value depends on your operator version. Operator 2.1 installs `core`, `dbaas`, `ai`, `analytics`, `migration`, and `marketplace`. Operator 2.2 or later installs only `core`, so the set of scenarios is always an explicit choice. List the scenarios you want explicitly.

**Scenario dependencies:** The `ai` scenario depends on `marketplace`. If you select `ai`, `marketplace` is installed with it, whether or not you list it explicitly.
...
| **Installation scenarios** | `spec.scenarios` (YAML list)                              | `core`, `migration`, `ai`, `analytics`, `dbaas`, `marketplace` |
...
spec:
  ...
  scenarios: # list scenarios explicitly rather than relying on the operator default
    ...
    - marketplace # remove if not needed; installed with ai as a dependency
