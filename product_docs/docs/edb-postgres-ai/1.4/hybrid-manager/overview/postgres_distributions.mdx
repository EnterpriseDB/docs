...
- **Oracle compatibility mode** — Run Oracle PL/SQL and use Oracle-compatible data types (`VARCHAR2`, `NUMBER`, `DATE`). Use Oracle SQL syntax including hierarchical queries (`CONNECT BY`), bulk operations (`FORALL`, `BULK COLLECT`), and packages. AI-driven recommendations help assess and resolve schema incompatibilities during migration.
...
- **PGD** — Multi-master replication for active-active high-availability deployments across multiple Kubernetes clusters or data centers. Enables Distributed HA (DHA) cluster types in HM.
