...
### Backup schedule accepts invalid 5-element cron expressions
...
**Description:** The HM console accepts both 5-element and 6-element cron expressions for cluster backup schedules. The underlying system (CloudNativePG) requires the 6-element format, which includes a seconds field. Entering a 5-element expression produces unexpected behavior — for example, a schedule intended to run once daily may instead run every hour.
...
!!!tip
Resolved in HM versions 1.4.3 and later.
!!!
