...
!!!tip
This issue was resolved for Apache Superset in HM versions 1.4 and later. pgAdmin4 continues to have this issue.
!!!
...
**Description:** On Google Kubernetes Engine (GKE) clusters running on ARM64 (`arm64`/`Tau T2A`) node pools, the default storage class is `hyperdisk-balanced`. Google Cloud enforces a hard minimum volume size of **4Gi** on `hyperdisk-balanced` volumes.

If you deploy an OSS Library app (for example, HammerDB, pgBadger, or pgAdmin 4) and request a persistent volume smaller than 4Gi, provisioning fails at the storage layer. The following happens:
...
This issue only affects GKE clusters that use ARM64 node pools with the `hyperdisk-balanced` storage class. AMD64 (`x86_64`) node pools and other cloud providers aren't affected.
...
- For **HammerDB**, set the `size` parameter to `4Gi` or larger (default is `5Gi`, which already meets the minimum).
- For **pgBadger**, set the persistence size to `4Gi` or larger (default is `1Gi`, which is below the minimum and must be increased).
- For any other OSS Library app that provisions a persistent volume, set its storage-size field to `4Gi` or larger.
...
**Description:** On Google Kubernetes Engine (GKE) clusters that use ARM64 (`arm64`/`Tau T2A`) node pools, the HammerDB OSS Library app can't be scheduled onto ARM64 worker nodes. GKE ARM64 nodes are tainted (typically with `kubernetes.io/arch=arm64:NoSchedule`) so that only pods that explicitly tolerate ARM64 architecture are scheduled onto them. The HammerDB pod spec shipped with the OSS Library package currently doesn't include this toleration.
...
This behavior is specific to GKE ARM64 clusters. Other OSS Library apps and other architectures (AMD64/`x86_64`) aren't affected. All other supported HM environments (GKE AMD64, AKS, EKS, RKE, OpenShift, self-managed Kubernetes) work as expected.

**Workaround:** There's no supported workaround at this time. Don't deploy the HammerDB OSS Library app on GKE ARM64 clusters until this issue is resolved. If you need to run HammerDB benchmarks against a PostgreSQL cluster hosted on GKE ARM64, deploy the HammerDB app onto a project that runs on an AMD64 node pool and point it at the target PostgreSQL cluster using its connection string. EDB is tracking this issue as [UPM-118869] and will deliver the fix in a future release.
...
### Updated Langflow flow changes aren't immediately reflected in HM after republishing

[AID-2059]: #

**Description:** When you edit a previously published Langflow flow in the HM Langflow editor and republish it to the Asset Library, the updated version doesn't immediately appear in HM. There is a delay of up to 5 minutes before the changes propagate and become visible.
