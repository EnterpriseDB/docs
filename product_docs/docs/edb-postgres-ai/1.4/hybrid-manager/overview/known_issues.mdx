...
**Description:** On Google Kubernetes Engine (GKE) clusters running on `arm64` node pools, the default storage class is `hyperdisk-balanced`. Google Cloud enforces a hard minimum volume size of **4Gi** on `hyperdisk-balanced` volumes.
