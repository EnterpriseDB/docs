...
**Description:** In Red Hat OpenShift (RHOS) environments, deploying the pgAdmin4 Asset Library application fails due to OpenShift Security Context Constraint (SCC) violations. The upstream image for pgAdmin4 runs as a specific user ID (`5050`) that falls outside the allowed UID ranges enforced by OpenShift's restricted SCC policies. This is an upstream image compatibility issue.

**Workaround:** Don't deploy pgAdmin4 in RHOS environments until this issue is resolved upstream.
