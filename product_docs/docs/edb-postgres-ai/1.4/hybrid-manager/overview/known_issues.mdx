...
### PostgreSQL cluster rolling restart during upgrade to 1.4.0

[UPM-118428]: #

**Description:** Upgrading to HM 1.4.0 triggers a rolling restart of all managed PostgreSQL clusters. This is caused by a bundled `barman-cloud-plugin` sidecar image bump from v0.11.0 to v0.13.0, which changes the PostgreSQL pod specification. The CNP operator detects the change and rolls each cluster's pods one at a time, followed by a primary switchover per cluster. Applications will see a brief connection interruption during each primary switchover. The restart is not currently gated on a per-cluster maintenance window.

**Workaround:** Plan the HM 1.4.0 upgrade inside your maintenance window so the resulting PostgreSQL rolling restart aligns with it. A future HM release will gate behind the customer-configured maintenance window so HM upgrades no longer restart PostgreSQL clusters as a side effect.
