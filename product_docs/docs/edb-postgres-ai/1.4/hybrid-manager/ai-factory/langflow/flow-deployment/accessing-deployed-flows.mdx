...
A [deployed flow](deploying-flows) exposes two endpoints, an Agent Studio runtime API and an MCP server, both routed through the Hybrid Manager (HM) gateway. Clients authenticate to the gateway with an HM machine-user access key.

## Finding the endpoints

1. In the HM console, go to **Estate > Flows** (`/estate/flows`).

1. Wait until the **Status** column shows **Ready**. Earlier statuses such as **Provisioning** or **Starting** mean the runtime is not yet accepting requests.

1. Select the row to open the detail sidebar.

1. Scroll to the **Endpoints** section and copy the URL for the endpoint you need.
...
Logs and metrics from a deployed flow go to HM's observability stack and are surfaced through the standard HM Grafana dashboards. See [Observability](../../observability) for Prometheus alert rules and Grafana dashboard setup for Agent Studio pods and model cluster performance.
