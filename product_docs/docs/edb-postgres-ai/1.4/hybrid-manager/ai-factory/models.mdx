...
Models in the library power all Agent Factory capabilities including Agent Studio flows, Pipeline Designer knowledge bases, and custom inference applications. Only models validated through the library's governance framework can reach production environments.
...
1.  **Runtime Configuration**: Select or create [ServingRuntimes](/edb-postgres-ai/1.3/ai-factory/model/configure-servingruntime/) optimized for the model framework (vLLM, TensorRT-LLM, custom)
1.  **Resource Allocation**: Define GPU, memory, and CPU requirements based on expected workload
1.  **Endpoint Configuration**: Set up internal cluster access or external API endpoints with authentication (see [Access KServe endpoints](/edb-postgres-ai/1.3/ai-factory/model/access-kserve-endpoints/))
...
-   [Using models via OpenAI-compatible SQL calls](/edb-postgres-ai/1.3/ai-factory/pipeline/models/using-with/openai-api-compatibility/)
-   [Using NVIDIA NIM in AIDB](/edb-postgres-ai/1.3/ai-factory/pipeline/models/using-with/using-nvidia-nim/)
