...
-   **Self-managed clusters.** Customer-managed Postgres servers registered with HM through the EDB Postgres AI agent (`beacon-agent`): single-instance servers, external CloudNativePG (CNPG)-managed clusters, and external CNPG-managed Postgres Distributed (PGD) clusters. Baremetal (non-CNPG) PGD clusters aren't supported. Self-managed clusters are not provisioned by HM and do not have network connectivity to the primary location by default. Without that connectivity, only AIDB-native and built-in models are available. See [How models reach pipeline steps](#how-models-reach-pipeline-steps) for details.
