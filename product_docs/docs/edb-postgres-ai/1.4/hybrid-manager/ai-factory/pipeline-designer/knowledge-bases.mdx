...
1.  Create a new pipeline following the [pipeline creation wizard](creating-pipelines).

1.  Add a **KnowledgeBase** step as the final step.

1.  In the **Knowledge Base Table Name** field, search for and select the existing knowledge base you want to append to.

1.  Select the **Data Format** (Text or Image). The model, distance operator, and index type are inherited from the existing knowledge base.
...
!!! Warning
    ...
    Deleting a knowledge base is irreversible. All vector data is permanently removed. If the knowledge base is used by [Agent Studio](/edb-postgres-ai/current/hybrid-manager/ai-factory/langflow/) flows or other consumers, those integrations will break.

## Using knowledge bases with Agent Studio

Knowledge bases created through Pipeline Designer are accessible from the [Agent Studio-based Gen AI builder](/edb-postgres-ai/current/hybrid-manager/ai-factory/langflow/). The Agent Studio KB component connects to AIDB knowledge bases using the same [`aidb.retrieve_key()` and `aidb.retrieve_text()`](/aidb/latest/knowledge-bases) functions that power the Pipeline Designer query tool.

To use a Pipeline Designer knowledge base in Agent Studio:

1.  Create and populate a knowledge base through Pipeline Designer as described above.

1.  In Agent Studio, add a KB component to your flow.
...
!!! Note "Agent Studio database identity"

    Agent Studio components that connect to Postgres use the database credentials configured within the flow, not a system-managed role. The Postgres role used by an Agent Studio flow determines which knowledge bases and tables it can access through standard Postgres privilege rules. If the configured role does not have `SELECT` access to the knowledge base's underlying vector table, queries will fail with a permission error. Granting the Agent Studio role inheritance from `visual_pipeline_user` (for example, `GRANT visual_pipeline_user TO langflow_role`) provides access to all VPU-owned objects.
