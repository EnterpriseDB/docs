...
On AIDB earlier than 7.3.2, `visual_pipeline_user` must own the trigger handler functions directly, rather than acquiring ownership through role inheritance. Without direct ownership, the AIDB trigger handler functions (`aidb.pipeline_background_trigger_handler` and `aidb.pipeline_live_trigger_handler`) cause Live and Background processing modes to fail on PGD clusters. This requirement applies identically to HM-managed and external CNPG-managed PGD clusters.
