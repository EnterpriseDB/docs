...
The sections below cover intentional design boundaries of the current Pipeline Designer implementation and known issues with available workarounds. For release-specific details, see the [Hybrid Manager release notes](/edb-postgres-ai/current/hybrid-manager/release_notes/).
...
### Baremetal PGD clusters not supported

Self-managed cluster support requires the CloudNativePG (CNPG) operator. Baremetal Postgres Distributed (PGD) clusters — that is, PGD clusters not managed by CNPG — aren't eligible as pipeline targets, even when registered with Hybrid Manager (HM) through the EDB Postgres AI agent. External CNPG-managed clusters (single-instance or PGD) are supported. {/* TODO(DOCS-4178): confirm with engineering whether self-managed PSR pipeline creation via CNPG through the UI (UPM-104762, cancelled) is also still unsupported, and finalize this section's scope with Kamlesh Digarse before merging. */}

## Multi-step pipelines

### ChunkText overlap setting not available

The `overlap_length` parameter for the ChunkText step (which controls how much text overlaps between consecutive chunks) is supported by AI Database (AIDB) but not yet configurable through Pipeline Designer. If you need chunk overlap, create the pipeline through SQL directly. See the [ChunkText step reference](/aidb/latest/data-pipelines/pipeline-steps/#chunktext) for parameter details.
...
**Resolution**: For pipelines on clusters without primary-location connectivity, use AIDB-native models registered via `aidb.create_model()` (which contact the provider directly from the Postgres process) or built-in AIDB models that run locally. See [Executing pipelines: How models reach pipeline steps](executing-pipelines#how-models-reach-pipeline-steps) for the complete model availability breakdown.
...
On AIDB earlier than 7.3.2, `visual_pipeline_user` must own the trigger handler functions directly, rather than acquiring ownership through role inheritance. Without direct ownership, the AIDB trigger handler functions (`aidb.pipeline_background_trigger_handler` and `aidb.pipeline_live_trigger_handler`) cause Live and Background processing modes to fail on PGD clusters.
...
On AIDB earlier than 7.3.2, the `bdr_setup()` function doesn't include knowledge base tables in BDR replication. Without these tables, knowledge base registry entries and knowledge base–pipeline associations created on the write leader aren't replicated to other PGD nodes. This limitation can cause inconsistencies after a write leader switchover, where the new leader has no record of knowledge bases or their pipeline bindings.
...
This bug is resolved in AIDB 7.3.2.
