...
| LOCATION_NAME                      | The location name for the Agent, formerly known as PGAI agent.                                                                           |
...
2.  Set `spec.imageRegistry` to $CONTAINER_REGISTRY_URI.
3.  Set `spec.globalParameters.portal_domain_name` to $PORTAL_DOMAIN_NAME.
4.  Set `spec.componentsParameters.transporter-rw-service.domain_name` to $TRANSPORTER_RW_SERVICE_DOMAIN_NAME.
5.  Set `parameters.transporter-dp-agent.rw_service_url` to `https://` followed by $TRANSPORTER_RW_SERVICE_DOMAIN_NAME followed by `/transporter`. Following the first of the previously provided examples, this would be `https://dms.examplecompany.com/transporter`.
6.  Set `spec.componentsParameters.upm-beacon.server_host` to $BEACON_SERVICE_DOMAIN_NAME.
7.  Set `spec.pgai.portal.authentication.staticPasswords[0].email` to $AUTHENTICATION_EMAIL.
8.  Set `spec.pgai.portal.authentication.staticPasswords[0].hash` to $AUTHENTICATION_PASSWORD_HASH.
9.  Set `spec.pgai.portal.authentication.staticPasswords[0].username` to $AUTHENTICATION_USER.
10. Set `spec.pgai.portal.authentication.staticPasswords[0].userID` "c5998173-a605-449a-a9a5-4a9c33e26df".
11. Set `spec.beaconAgent.location` to $LOCATION_NAME
12. Set `spec.globalParameters.storage_class` to $STORAGE_CLASS.
13. Set `spec.componentsParameters.transporter-data-operator.fips_enabled` to $TRANSPORTER_FIPS_ENABLED. (Ensure the boolean value is quoted as it is handled as a string.)
