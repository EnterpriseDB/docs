...
The template provided in [Configuring the Agent](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/monitoring/mon_ext_dbs/mon_with_agent/configure_agent/#prepare-a-configuration-file) enables all monitoring features. You can fine-tune which features are enabled.
