...
To leverage the full monitoring and optimization features of Hybrid Manager (HM), you must enable extensions that add instrumentation to Postgres without which the **Query Diagnostics** and **Recommendations** tabs, as well as some metrics, will not be available.
