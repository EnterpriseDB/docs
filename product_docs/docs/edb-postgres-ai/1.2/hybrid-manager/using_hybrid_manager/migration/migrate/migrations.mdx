...
Select **Create Migration** to create migrations. See [Creating migration](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migration_guides/self_managed_pg_to_self_managed_pg/creating_migration/) and [Migration options in the Console](/) for details.
...
If you selected the **Snapshot + Streaming** option when [creating a migration](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/terminology/#snapshot--streaming-migration), you can use **Mark as Completed** to stop the streaming procedure.
