...
Use the collaborative [project-scoped workspace](/edb-postgres-ai/1.2/hybrid-manager/using_hybrid_manager/migration/schema_migration/migration_portal/access_mp/#project-scoped-workspace-vs-users-workspace) of the Migration Portal to convert and migrate schemas. The Hybrid Manager provides an integration that lets you use the HM Console to create a Migration Portal project where you can easily add source database schemas pre-loaded by the [Agent](/edb-postgres-ai/1.2/hybrid-manager/using_hybrid_manager/migration/migration_guides/oracle_to_hm_managed/) while you are performing a database migration.
...
-   **Optional:** The administrator of Hybrid Manager has [enabled and configured the AI Copilot with the AI/LLM of your organization](/edb-postgres-ai/1.2/hybrid-manager/using_hybrid_manager/enabling_migration_copilot/) to facilitate resolving schema incompatibilities in the Migration Portal.
