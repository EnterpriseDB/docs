...
For continuous metric streaming and to ensure your database remains visible in your Hybrid Manager (HM) estate, configure the PGAI agent (formerly called HM or beacon agent) to run as a service. This allows the PGAI agent to start automatically on system startup and restart after any errors.
...
This example configures PGAI agent to run on an RHEL 9 machine. However, we provide operating system aids so you know when to replace the values.
...
1.  Your agent is now running as a service. Check on the logs:
