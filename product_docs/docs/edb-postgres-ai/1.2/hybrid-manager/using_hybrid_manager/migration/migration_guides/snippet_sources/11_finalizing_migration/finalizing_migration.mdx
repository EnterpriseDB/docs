...
If you selected the [**Snapshot + Streaming**](/edb-postgres-ai/hybrid-manager/using_hybrid_manager/migration/terminology/#snapshot--streaming-migration) option when creating a migration, the EDB DMS Reader will continue to stream any data updates performed on the source database to the destination database until you mark the migration as completed (for example, to perform a cutover).

If you selected the [**Snapshot**](/edb-postgres-ai/hybrid-manager/using_hybrid_manager/migration/terminology/#snapshot-migration) option, you can skip this page, as the DMS will automatically mark the migration as completed after it finishes copying the entire snapshot to the destination.

## Stop the streaming process

For [**Snapshot + Streaming**](/edb-postgres-ai/hybrid-manager/using_hybrid_manager/migration/terminology/#snapshot--streaming-migration) migrations only. 
