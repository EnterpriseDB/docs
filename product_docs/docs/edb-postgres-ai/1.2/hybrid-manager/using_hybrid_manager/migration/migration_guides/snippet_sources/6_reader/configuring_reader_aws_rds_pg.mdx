---
title: "Configuring and running the EDB DMS Reader for AWS RDS/Aurora PostgreSQL sources"
navTitle: "Configuring Reader for AWS RDS/Aurora PostgreSQL"
deepToC: true
...
2.  [Install the EDB DMS Reader](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/installing/).

3.  Open the EDB DMS Reader located in `/opt/cdcreader/run-cdcreader.sh` and ensure you have write permissions.

## Configuring the EDB DMS Reader for migrations from AWS RDS for PostgreSQL or Aurora PostgreSQL

Set the variables according to your environment and uncomment the edited lines. See [parameters](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/reader_reference/) for further guidance. The script is reproduced below.
