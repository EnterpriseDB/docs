---
navTitle: Installing reader
title: Installing EDB Data Migration Service Reader on Linux
# This topic is generated from templates. If you have feedback on it, instead of
# editing the page and creating a pull request, please enter a GitHub issue and
# the documentation team will update the templates accordingly.
# Leaf template: products/edb-data-migration-service-reader/index.njk

redirects:
  - /advocacy_docs/edb-postgres-ai/migration-etl/data-migration-service/getting_started/installing/index
  - /advocacy_docs/edb-postgres-ai/hybrid-manager/using_hybrid_manager/migration/installing/index
indexCards: none

navigation:
  - linux_x86_64
---

!!! Note
Install and configure EDB DMS Reader with Hybrid Manager versions 1.2 and earlier. For Hybrid Manager 1.3 and later versions, use EDB DMS Agent.
!!!

Select a link to access the applicable installation instructions:

## Linux [x86-64 (amd64)](linux_x86_64)

### Red Hat Enterprise Linux (RHEL) and derivatives

- [RHEL 9](linux_x86_64/edb-dms-reader_rhel_9), [RHEL 8](linux_x86_64/edb-dms-reader_rhel_8)

- [Oracle Linux (OL) 9](linux_x86_64/edb-dms-reader_rhel_9), [Oracle Linux (OL) 8](linux_x86_64/edb-dms-reader_rhel_8)

- [Rocky Linux 9](linux_x86_64/edb-dms-reader_other_linux_9), [Rocky Linux 8](linux_x86_64/edb-dms-reader_other_linux_8)

- [AlmaLinux 9](linux_x86_64/edb-dms-reader_other_linux_9), [AlmaLinux 8](linux_x86_64/edb-dms-reader_other_linux_8)

### SUSE Linux Enterprise (SLES)

- [SLES 15](linux_x86_64/edb-dms-reader_sles_15)

### Debian and derivatives

- [Ubuntu 24.04](linux_x86_64/edb-dms-reader_ubuntu_24), [Ubuntu 22.04](linux_x86_64/edb-dms-reader_ubuntu_22)

- [Debian 12](linux_x86_64/edb-dms-reader_debian_12), [Debian 11](linux_x86_64/edb-dms-reader_debian_11)

## Linux [AArch64 (ARM64)](linux_arm64)

### Debian and derivatives

- [Debian 12](linux_arm64/edb-dms-reader_debian_12)
