...
Use the collaborative [project-scoped workspace](access_mp.mdx#project-scoped-workspace-vs-users-workspace) of the Migration Portal to convert and migrate schemas in the context of a database migration. The Hybrid Manager provides an integration that lets you use the HM Console to create a Migration Portal project where you can easily add source database schemas pre-loaded by the [Agent](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migration_guides/oracle_to_hm_managed/) while you are performing a database migration.
...
3.  Select the schemas you want to migrate, but disable **Constraints** from the list of objects underneath the database. Select **Next**.
    ...
    !!!note
        ...
        Review [Schema integrity and performance considerations](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migration_guides/self_managed_pg_to_self_managed_pg/handling_schema/) to learn more about why constraints must be disabled and to review other considerations.
