...
3.  Review the upgrade path and confirm by selecting the **Continue** button.

4.  The dialogue window now shows the specific package changes to be expected with the upgrade.
    Review these and select **Upgrade Cluster** if everything looks as correct.

5.  The upgrade is applied automatically. 
    You can then use the [PGD CLI](/pgd/latest/overview/quickstart/using-cli/#viewing-cluster-status) to ensure the data group becomes healthy. The upgrade is successfull when all checks have the Status `Ok`. 

6.  After the first data group is successfully upgraded and healthy, proceed to upgrade the remaining data and witness groups one by one until the entire cluster is on the new version.
