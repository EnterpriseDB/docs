...
- **Tiered** — a partitioned table managed by [PGD AutoPartition](/pgd/latest/partitioning/), where cold partitions are Iceberg. Hot partitions are HTAP if real-time analytics replication is enabled, or Heap if not. See [Converting to a tiered table](#converting-to-a-tiered-table).
