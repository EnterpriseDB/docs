...
- **No-catalog offload**: The metadata is stored alongside the data files and managed directly by the object storage file system. PGD uses the [PGFS extension](/aidb/7/external-data-sources) to connect directly to the object storage.
