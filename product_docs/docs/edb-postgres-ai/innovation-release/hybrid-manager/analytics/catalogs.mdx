...
- **Enable Sync** — Continuously synchronizes table metadata from the catalog into your cluster, keeping it automatically up to date. The sync status changes to **Enabled**. Use **Enable Sync** when tables are actively changing and you want queries to always reflect the latest state.

## Querying analytical data across clusters

Attach a catalog from another cluster within the same project to query its data from your cluster using PGAA. For example, tier data from an AHA cluster and query it from a DHA cluster, or vice versa.
