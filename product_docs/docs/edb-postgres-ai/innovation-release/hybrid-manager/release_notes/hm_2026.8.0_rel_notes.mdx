...
| Type | Description |
| ---- | ----------- |
| Enhancement | Improved the cluster logs experience, making it easier to filter logs by both stream attributes (pod, container, service) and structured log fields (logger, severity, and other structured data), so you can more easily locate Postgres logs across CNPG, Kubernetes, and self-managed clusters. |
...
| Enhancement | Added support for several Oracle Database 21c syntax elements during Postgres Migration Portal schema assessment: multilevel indexes are now correctly detected, `ANY_VALUE` and `WINDOW` clause syntax is now parsed, and blockchain tables and `SQL MACRO` are now flagged as unsupported object types. |
