...
1. Update the version and — if your SKU permits — add the `dbaas` scenario in a single patch command:
...
The `dbaas` scenario is new in 2026.4 and is separate from `core`. It enables database cluster management — including cluster provisioning, backups, storage locations, and templates.
