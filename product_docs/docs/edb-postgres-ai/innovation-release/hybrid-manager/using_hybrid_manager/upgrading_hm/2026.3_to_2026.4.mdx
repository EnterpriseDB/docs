...
-   Required tools:
    ...
    -   `kubectl`
    -   [`edbctl`](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/edbctl/), the Hybrid Manager CLI, version `1.4.0` or later (or a build from `main`)
...
```shell
edbctl hybrid-manager create-install-secrets --version v2026.4.0
```
