...
Several [installation scenarios](/edb-postgres-ai/current/hybrid-manager/install/install_guide/planning_arch/#available-scenarios) change across Innovation Releases 2026.8 and 2026.9. Check your `HybridControlPlane` manifest against the ones that apply to your upgrade path.
...
- If your spec lists scenarios explicitly, add `dbaas` before you upgrade:
...
- If you omit `spec.scenarios` entirely, the set that applies depends both on your operator version and on what is already stored on the resource. Operator 2.1 fills an empty list with `core`, `dbaas`, `ai`, `analytics`, `migration`, and `marketplace`; operator 2.2 or later fills it with `core` alone. Rather than reasoning about the default, read the stored list back:
