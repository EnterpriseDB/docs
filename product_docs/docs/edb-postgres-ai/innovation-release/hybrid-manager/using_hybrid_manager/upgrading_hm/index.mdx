...
Two [installation scenarios](/edb-postgres-ai/current/hybrid-manager/install/install_guide/planning_arch/#available-scenarios) change from Innovation Release 2026.8 onwards. Check your `HybridControlPlane` manifest against both before you upgrade.

### The marketplace scenario

`marketplace` (the kapp package manager and the OSS Library apps it deploys, such as Superset, Airflow, and pgAdmin) is controlled by its own installation scenario, separated from `core`.
...
- If you omit `spec.scenarios` entirely, the set that applies depends both on your operator version and on what is already stored on the resource. Operator 2.1 fills an empty list with `core`, `dbaas`, `ai`, `analytics`, `migration`, and `marketplace`; operator 2.2 or later fills it with `core` alone. Rather than reasoning about the default, read the stored list back with `kubectl get hybridcontrolplane <name> -o jsonpath='{.spec.scenarios}'` and add `dbaas` explicitly if it isn't there.
