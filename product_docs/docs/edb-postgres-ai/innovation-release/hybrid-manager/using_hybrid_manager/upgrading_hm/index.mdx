...
From HM 1.4.2 and Innovation Release 2026.8 onwards, `marketplace` — the kapp package manager and the OSS Library apps it deploys (Superset, Airflow, pgAdmin, and others) — is controlled by its own [installation scenario](/edb-postgres-ai/current/hybrid-manager/install/install_guide/planning_arch/#available-scenarios), separated from `core`.
...
- If your spec lists only `core` (without `ai`) and you use the OSS Library/Marketplace apps, you must explicitly add `marketplace` to your `spec.scenarios` list **before** upgrading to HM 1.4.2 / IR 2026.8 or later.
