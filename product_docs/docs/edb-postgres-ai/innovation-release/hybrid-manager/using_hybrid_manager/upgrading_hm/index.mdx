...
From Innovation Release 2026.8 onwards, `marketplace` (the kapp package manager and the OSS Library apps it deploys, such as Superset, Airflow, and pgAdmin) is controlled by its own [installation scenario](/edb-postgres-ai/current/hybrid-manager/install/install_guide/planning_arch/#available-scenarios), separated from `core`.
...
- If your spec lists only `core` (without `ai`) and you use the OSS Library/marketplace apps, you must explicitly add `marketplace` to your `spec.scenarios` list. Do this **before** upgrading to Innovation Release 2026.8 or later.

!!!important
If you don't add `marketplace` to your scenario list, kapp-controller and the marketplace apps are removed as no longer part of your selected scenarios.
!!!

#### The pgd scenario

From Innovation Release 2026.9 onwards, `pgd` (which allows Distributed High Availability, EDB Postgres Distributed, clusters to be created and managed) is controlled by its own [installation scenario](/edb-postgres-ai/current/hybrid-manager/install/install_guide/planning_arch/#available-scenarios), separated from `dbaas`.

- If your spec do not lists `pgd` and you use Distributed High Availability clusters, you must explicitly add `pgd` to your `spec.scenarios` list, as `pgd` is not a dependency for any other scenario. Do this **before** upgrading to Innovation Release 2026.9 or later.
