...
HM releases and the operator are versioned independently — the 1.4.x → 2026.7 upgrade needed no operator change. 2026.8 is different if you use the app marketplace: the new `marketplace` scenario requires operator version `2.1` or later, and earlier operator versions reject a manifest that lists it.
