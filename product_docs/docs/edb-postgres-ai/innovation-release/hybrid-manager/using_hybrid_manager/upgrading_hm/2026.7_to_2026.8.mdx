...
Unlike the 1.4.x → 2026.7 upgrade, this release requires an operator upgrade. HM 2026.8 requires operator version `2.1`, which adds the `marketplace` installation scenario and gives the `dbaas` scenario its first components.

Complete the steps in the order given:

1. Upgrade the operator to `2.1`.
1. Review and update your installation scenarios.
1. Update `spec.version` and trigger the upgrade.

!!!important
The order matters. Operator versions earlier than `2.1` don't include `marketplace` in the `spec.scenarios` enum and reject a manifest that lists it. Add scenarios only after the operator is on `2.1`, and before you bump `spec.version`.
!!!
...
!!!note
`<OPERATOR_VERSION>` refers to the operator chart version, which follows its own `2.x` versioning scheme and is separate from the HM version (`2026.8`). HM 2026.8 requires operator version `2.1`.
!!!
...
!!!important
If you don't add `marketplace` to your scenario list, HM removes kapp-controller and the marketplace apps, because they're no longer part of your selected scenarios.
!!!

#### The dbaas scenario

`dbaas` has been present in the scenario list since 2026.7, but it carried no components until this release. In 2026.8 it delivers `upm-api-gucs` and `pg4k-additional-controllers`.

If your `spec.scenarios` list includes only `core` and you need those components, add `dbaas` to the list.
