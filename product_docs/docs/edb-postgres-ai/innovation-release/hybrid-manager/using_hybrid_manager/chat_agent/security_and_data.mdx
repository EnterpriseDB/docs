...
This guarantee applies only when the configured model is **Large** or **Extra Large** tier. Those tiers ask structured follow-up questions for inputs like passwords. At lower tiers, Chat Agent uses free-form prompts. Any secret you type into chat then passes through the model with the rest of your prompt. See [Skills available at each tier](./enable#skills-available-at-each-tier).

## Audit logging

Every action Chat Agent performs is recorded in the HM activity log, alongside HM console and API activity. See [Activity Log](../hm_overview/using-portal#portal-related-options) for review and download.
