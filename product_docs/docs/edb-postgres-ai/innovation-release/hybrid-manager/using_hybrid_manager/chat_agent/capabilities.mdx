...
Get table-level schema design recommendations based on your table structure and running query workload. Chat Agent reads the table's DDL and the top queries running against it (last 6 hours by default, configurable in the question), then applies a Postgres and EDB best-practices catalog. Recommendations cover three areas: table access methods, partitioning, and normalization. With no workload in the window, recommendations fall back to schema-only analysis from the table definition.

Index recommendations are handled separately — see [Recommendations and advisories](#recommendations-and-advisories).

For full details on how recommendations are generated, what each category covers, and known limitations, see [Schema design recommendations](schema_design_recommendations).
