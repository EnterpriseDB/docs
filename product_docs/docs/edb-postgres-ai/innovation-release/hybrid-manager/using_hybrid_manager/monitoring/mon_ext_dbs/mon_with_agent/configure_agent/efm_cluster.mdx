...
To monitor an EDB Failover Manager, deploy one beacon agent per efm node (excluding the witness node). 
...
Since we need to deploy one beacon agent per efm node, the nodes list will contain only one entry. 
