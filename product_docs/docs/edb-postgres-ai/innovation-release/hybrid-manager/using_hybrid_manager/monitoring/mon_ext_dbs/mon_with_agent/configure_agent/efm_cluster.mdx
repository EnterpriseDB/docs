...
To monitor an EDB Failover Manager, deploy one PGAI agent per efm node (excluding the witness node). 
PGAI agent leverages the `efm` cli and run the `efm cluster-status-json <cluster_name>` command to collect the monitoring info of the failover manager.
...
Since we need to deploy one PGAI agent per efm node, the nodes list will contain only one entry. 
