...
When you log in to HM with unread notifications, a persistent banner appears at the top of the console. If you have multiple unread notifications, banners stack — one per notification. Each banner mirrors a corresponding entry in your inbox and dismisses individually.
...
For automation task events, each user can be configured as one of two roles:

-   **Approver** &mdash; Receives the notification and can approve or reject the pending task. The notification includes a direct link to the task detail page in Task Manager.
-   **Receiver** &mdash; Receives an informational notification only. Receivers can't take action on the task from the notification.

This lets you notify the full team about automation activity while restricting who can act on it. The ability to approve a task is still subject to HM's RBAC system — only users with the appropriate project role can approve tasks regardless of their notification role.
