...
1. The operator will detect the changes and begin initializing the telemetry services.

   !!! important
   If metrics are not appearing after configuration, telemetry is automatically disabled if:
   
   - The company code lookup returns `NULL`: The system cannot verify the value of `company_code` against the global registry.
...
- Telemetry will fail to initialize if the `company_code` lookup returns `NULL` or if it is missing from `values.yaml`.
!!!
