...
If you are using the Hybrid Manager, you can enable reporting as part of your HM server's configuration. See [Enabling HM server usage reporting](#enabling-hm-server-usage-reporting).

If you have a standalone deployment of a database server (PostgreSQL, EPAS, PGE) and you don't wish to use HM, see [Enabling usage reporting for standalone databases](/../../tools/agent/stand_alone).
...
1. Add the `enableEDBUsageReporting` and `hybridManagerInstallation` fields to the `beaconServer` section:
...
1. Save your changes. The operator reconciles the updated spec automatically; once `HybridControlPlane` is in a healthy state, HM server will begin initializing the telemetry services.
...
1. Enable server metrics collection by setting `enableEDBUsageReporting` to `true` and `hybridManagerInstallation` to `<install-name>`:
