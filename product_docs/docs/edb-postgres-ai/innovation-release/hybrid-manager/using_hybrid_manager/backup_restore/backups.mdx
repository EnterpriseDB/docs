...
Backup compression reduces the size of backup data using a compression algorithm before storing it to object storage. HM compresses Postgres backups by default using algorithms from the Barman Cloud plugin. Compression is applied separately to base backups and WAL archiving:

- Base backups use Snappy compression by default.
- WAL archiving uses zstd compression by default.

Compression can improve backup performance and reduces storage consumption. It's enabled automatically for all new clusters — no additional configuration is required to activate it.

You can change the compression algorithm for each backup type independently in the [Backup and security settings](../cluster_management/create-clusters/additional-settings#backup-compression) tab when creating or editing a cluster.

## HM backups

HM backups include metadata needed to restore the HM system. They don't include backups of user databases. HM backups use Velero to back up to object storage. The default backup schedule is hourly. You can [adjust the schedule](../hadr/backup_considerations/#changing-the-default-backup-schedule).
...
The method used to compress backup data. The supported algorithms differ between base backups and WAL archiving:

- Base backups: none, Snappy, gzip, bzip2, lz4
- WAL archiving: none, Snappy, gzip, bzip2, lz4, xz, zstd

Performance characteristics vary by environment and workload.
