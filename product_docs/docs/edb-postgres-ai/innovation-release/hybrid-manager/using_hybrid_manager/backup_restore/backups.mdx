...
HM compresses Postgres backups by default using algorithms from the Barman Cloud plugin. Compression is applied separately to base backups and WAL archiving:

- Base backups use Snappy compression by default.
- WAL archiving uses zstd compression by default.

Compression improves backup performance and reduces storage consumption. It's enabled automatically for all new clusters — no additional configuration is required to activate it.

You can change the compression algorithm for each backup type independently in the [Backup and security settings](../cluster_management/create-clusters/additional-settings) tab when creating or editing a cluster.
...
To configure backups of the HM system, see [HA/DR](../hadr/hadr_recovery).
...
The process of reducing the size of backup data using a compression algorithm before storing it in object storage. HM supports compression for both base backups and WAL archiving via the Barman Cloud plugin. Compression reduces storage consumption and can improve backup performance. See [Backup compression](#backup-compression-1) for configuration details.

### Compression algorithm

The method used to compress backup data. HM supports the following algorithms for base backups and WAL archiving: none, Snappy, gzip, bzip2, lz4, and zstd. The supported algorithm list differs between base backups and WAL archiving. Performance characteristics vary by environment and workload.
