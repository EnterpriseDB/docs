...
- **Genuinely different key material** (the common case): the joining group's own configured KEK simply can't decrypt data wrapped by the source group's KEK — for example, `KeyNotFound` / `BadParameter` on Azure Key Vault, `InvalidCiphertextException` on AWS KMS, or an equivalent decrypt-failure from Google Cloud KMS / HashiCorp Vault.
