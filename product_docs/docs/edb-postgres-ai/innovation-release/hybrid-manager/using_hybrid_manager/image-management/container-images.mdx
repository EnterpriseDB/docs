...
description: Learn how to use container images for Hybrid Manager clusters.
---

The **Container Images** section of the asset library is where you can view and manage the container images available for use in your Hybrid Manager (HM) clusters.
...
This tag format is specific to the official HM images and may differ for other images. The image name is used to identify the type of image, such as a particular flavor of Postgres. The image tag is used to identify the specific version of the image.
...
-   **Tags**: HM tags associated with this image repository. If no tags are associated with the image repository, **Add Tag** appears.
-   **Updated**: The number of days since the image repository was last updated.

!!! Warning "HM tags"
    HM tags aren't asset/image tags. HM tags are created in HM to identify and organize HM resources. You create and manage them using the **Tags** menu item in your user menu in the top-right corner of the HM console. HM tags aren't related to the image tags used to identify the version of an image.
...
Use the search and filter bar to search for clusters. Use the **Search** field to enter a search term that's part of the name or description of the cluster. Use the **Filter** field to filter the clusters by their attributes (cluster type, cluster architecture, Postgres flavor, and Postgres version). Use the **Sort** field to display clusters to sort by name, description, or the date the cluster was created.
...
-   **Platform Architecture**: The platform architectures for which the image tag is available, either X86 or AMD64.
