...
Check the [API documentation](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/using_the_api/) for details on the required fields and JSON structure for cluster operations.
!!!
...
-   AHA clusters use the same declarative list model and the same `patch-cluster.sh` script. Use `"aha"` in place of `"pgd"` as the top-level key in the request body. Unlike PGD, each entry in an AHA `groups` list must include both a `groupId` and a `dataGroup` configuration block — supplying a `groupId` alone is not sufficient.
!!!
