...
For Indexes, the scores are computed by considering only primary nodes in the case of PSR and write leaders in the case of PGD. Furthermore, the score is related to the database in the PG cluster that is performing the worst, not an average over all databases created in such a cluster). 
