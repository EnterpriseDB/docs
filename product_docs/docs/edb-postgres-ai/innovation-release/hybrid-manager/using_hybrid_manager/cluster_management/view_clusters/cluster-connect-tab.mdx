...
If you enabled a read-only connection when creating or editing your cluster, the **Connect** tab also displays a read-only connection string. Use this connection string to route read-only operations to standby replicas (for HA clusters) or to nodes that aren't currently designated as the write leader (for AHA and DHA clusters), reducing load on the primary instance.
...
To enable or configure a read-only connection, see [Cluster setup](../create-clusters/cluster-settings.mdx#connections-read-only) for HA clusters, or [Data groups](../create-clusters/data-groups.mdx#connections-read-only) for AHA and DHA clusters.
