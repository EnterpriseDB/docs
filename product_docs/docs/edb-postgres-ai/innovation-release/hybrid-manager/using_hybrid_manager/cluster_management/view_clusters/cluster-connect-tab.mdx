...
If you enabled a read-only connection when creating or editing your cluster, the **Connect** tab also displays a read-only connection string. Use this connection string to route read-only operations to standby replicas (for HA clusters) or to non-primary nodes (for AHA and DHA clusters), reducing the workload on the primary instance.
