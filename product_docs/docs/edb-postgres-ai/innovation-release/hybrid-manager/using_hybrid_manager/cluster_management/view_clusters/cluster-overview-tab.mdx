...
When Hybrid Manager (HM) detects a schema change in a monitored database, it automatically inserts an event entry in the activity log. You can use these entries to track when schema changes occurred and correlate them with changes in cluster performance.
