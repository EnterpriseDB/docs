...
More details are available in [Switching the write leader](/pgd/latest/reference/cli/command_ref/group/set-leader/).
...
When Hybrid Manager (HM) detects a schema change in an HM-managed database, it automatically inserts an event entry in the activity log. You can use these entries to track when schema changes occurred and correlate them with changes in cluster performance.

!!! Note
The Cluster Activity Log isn't available for self-managed clusters. For self-managed clusters, schema change events appear as annotations on the monitoring charts in the [**Monitoring**](cluster-monitoring-tab) tab.
!!!
