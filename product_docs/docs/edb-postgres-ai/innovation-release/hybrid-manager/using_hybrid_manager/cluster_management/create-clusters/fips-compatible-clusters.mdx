...
1.  Start to create a cluster from scratch. See [Creating a database cluster](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/cluster_management/create-clusters/).

1.  Configure the [Cluster info](cluster-info.mdx) and [Cluster setup](cluster-settings.mdx) tabs as needed.

1.  On the **DB Configuration** tab, find the `ssl_groups` parameter. For more information about this tab, see [Database configuration parameters](db-configuration.mdx).

1.  Change the value of `ssl_groups` from the default `X25519:prime256v1` to `prime256v1`.

1.  Finish configuring the remaining tabs and select **Create Cluster**.
