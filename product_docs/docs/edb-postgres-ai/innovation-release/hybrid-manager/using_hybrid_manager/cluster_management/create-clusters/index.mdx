...
Select the tab you want to work on, or step through the tabs in order using **Next**. If you select **Distributed High Availability**, instead of the **DB Configuration** and **Additional Settings** tabs, you see the [**Data Groups**](data-groups.mdx) tab.
...
-   [**Replica Clusters**](replica-clusters.mdx) (for single-node and high-availability clusters)
...
To provision a cluster that uses only FIPS-approved cryptography, see [Provisioning FIPS-compatible clusters](fips-compatible-clusters.mdx).
