...
For **Advanced High Availability** and **Distributed High Availability** clusters, you can enable a read-only connection per data group to route read-only operations to non-primary nodes, reducing the workload on the write leader and improving overall performance.

!!!note
The read-only connection option requires a data group with at least two nodes. It isn’t available for Advanced High Availability data groups configured with a single node.
!!!

**Enable Read-Only Connection** — Select the toggle to enable a dedicated read-only endpoint for this data group.
