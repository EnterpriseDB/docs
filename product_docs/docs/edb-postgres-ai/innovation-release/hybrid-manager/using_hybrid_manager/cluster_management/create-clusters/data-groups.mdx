...
For **Advanced High Availability** and **Distributed High Availability** clusters, you can enable a read-only connection per data group to route read-only operations to nodes that aren't currently designated as the write leader, reducing load on the write leader and improving overall performance.
