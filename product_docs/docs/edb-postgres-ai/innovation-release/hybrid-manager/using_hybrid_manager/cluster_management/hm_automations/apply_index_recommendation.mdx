...
You configure this automation per replica or data group using the dropdown in the Automations section. For HA clusters, configure it on each replica separately — logical replication doesn't sync DDL changes, so indexes won't propagate automatically. For AHA and DHA clusters, you only need to configure it on one data group — the automation applies the index to the write leader, and PGD replicates the DDL change to all other data groups automatically.
