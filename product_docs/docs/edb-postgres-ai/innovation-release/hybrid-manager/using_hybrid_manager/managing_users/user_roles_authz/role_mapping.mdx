...
!!! important
Users from an IdP with group attribute mapping enabled must have at least one group membership in the IdP. Users without any group memberships cannot log in to HM when group mapping is configured for their IdP.
!!!
...
When you update group memberships in your IdP (for example, adding a user to a new group), currently logged-in users do not automatically receive the updated group information. HM reads group attributes from the authentication token, which is only refreshed when users log in.

To force users to re-authenticate and receive updated group attributes:

1. In the HM console, go to **Settings** > **Identity Providers**.

2. Select the IdP whose users need to be re-authenticated.

3. Select **Invalidate Sessions**.
