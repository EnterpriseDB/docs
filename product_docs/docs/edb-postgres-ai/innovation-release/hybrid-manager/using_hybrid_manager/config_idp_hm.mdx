...
!!! important
When you begin adding a SAML identity provider in HM, the system generates unique **Assertion Consumer Service (ACS) URL** and **Audience URI** values. These values change each time you start a new configuration. You must complete both the IdP configuration and the HM configuration in a single session without navigating away from the page.
!!!
...
4. Set the **Name ID Format** based on your requirements. See [Name ID formats](#name-id-formats) for details.
...
7. Copy the **SSO URL** provided by your IdP.
...
5. Under **Name ID Format**, select the format that matches your IdP configuration.
...
Select the Name ID format that matches your IdP configuration:
