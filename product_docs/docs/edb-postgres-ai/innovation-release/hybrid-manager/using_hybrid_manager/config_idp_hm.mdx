...
5. (Optional) If your OIDC provider's **Issuer URL** uses HTTPS with a certificate issued by an internal or private certificate authority (CA) that HM doesn't trust by default, enable **Use Custom Root CA** and upload the CA certificate.
   ...
   - **Custom Root CA File** is a text file containing the CA certificate used to verify the OIDC provider's TLS certificate. It can contain a single CA certificate or a bundle of certificates — multiple `-----BEGIN CERTIFICATE-----` / `-----END CERTIFICATE-----` blocks concatenated together, which is useful when the OIDC provider presents a chain that includes intermediate CAs. Upload only the CA or CA chain that signed the server certificate. Don't upload the server certificate or any private keys.
