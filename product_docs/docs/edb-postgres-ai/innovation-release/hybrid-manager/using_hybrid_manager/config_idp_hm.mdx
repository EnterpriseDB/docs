...
1. Enter the **ACS URL** from HM in the **Single Sign-On URL** field.
...
2. Share the **Quick Sign-in URL** with your users:
   - In the HM console, go to **Settings** then **Identity Providers**.
...
Before you can remove an IdP, you must first delete all users who logged in through that IdP.

1. In the HM console, go to **User Management**.

2. Identify and delete all users associated with the IdP you want to remove.

3. Go to **Settings** then **Identity Providers**.
...
!!! note
The delete option is disabled for an IdP that still has users associated with it. You must remove all users from that IdP before you can delete it.
!!!
