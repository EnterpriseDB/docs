...
2. Enter the **Assertion Consumer Service URL** from HM in the **Single Sign-On URL** field.
...
5. (Optional) If your OIDC provider's **Issuer URL** uses HTTPS with a certificate issued by an internal or private certificate authority (CA) that HM doesn't trust by default, enable **Use Custom Root CA** and upload the CA certificate.
   Where:
   - **Use Custom Root CA** enables uploading a custom CA certificate. Use it when the **Issuer URL** is HTTPS and HM must trust an internal or self-signed CA to connect to the OIDC provider.
   - **Custom Root CA File** is a text file containing the CA certificate used to verify the LDAP server's TLS certificate. It can contain a single CA certificate or a bundle of certificates — multiple `-----BEGIN CERTIFICATE-----` / `-----END CERTIFICATE-----` blocks concatenated together, which is useful when the LDAP server presents a chain that includes intermediate CAs. Upload only the CA or CA chain that signed the server certificate. Don't upload the server certificate or any private keys.
