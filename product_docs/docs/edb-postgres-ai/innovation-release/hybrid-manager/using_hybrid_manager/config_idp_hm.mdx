...
1. Enter the LDAP server connection settings:
   ...
   - **Custom Root CA File** is a text file containing the CA certificate used to verify the LDAP server's TLS certificate. The file can contain a single CA certificate or a bundle — multiple `-----BEGIN CERTIFICATE-----` / `-----END CERTIFICATE-----` blocks concatenated, which is useful when the LDAP server presents a chain with intermediate CAs. Upload only the CA (or CA chain) that signed the server certificate — do not upload the server certificate or any private keys.
