...
Migration Portal manages schema migration projects for both Oracle and Postgres sources. For Oracle-to-Postgres migrations, it automates the assessment of schemas, flags incompatibilities, and provides an integrated AI Copilot to help you quickly identify and resolve complex schema issues, streamlining the conversion process. For Postgres sources, where no conversion is needed, it gives you visibility into the schema and lets you create or delete a project directly from the HM console. Originally a web tool hosted by EDB, Migration Portal has been integrated into HM. This integration deploys the service in your environment, supporting the sovereignty of your deployment.
...
- **Streaming only** — Skips the initial bulk copy and streams only the changes that occur on the source after the migration starts. Useful when the destination already has the historical data loaded, such as in fallback replication scenarios.
