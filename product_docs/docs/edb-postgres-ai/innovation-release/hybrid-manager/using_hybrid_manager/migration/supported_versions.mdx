...
Schema migration (automated schema handling via the **Scope** field when creating a migration) is supported for Postgres sources. As with Oracle, the EDB Postgres AI agent (`beacon-agent`) is required to register the source database and extract its schema before the migration begins.
...
Postgres destinations require a database role or user that can manage replications. For details, see [Preparing Postgres destination databases](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migration_guides/self_managed_pg_to_self_managed_pg/preparing_destination_selfpg_db/).
