...
-   **Streaming migrations:** They skip the snapshot phase entirely and only stream changes that occur after the migration starts. Because there's no initial bulk copy, performance characteristics match the streaming portion of a snapshot + streaming migration — driven by sustained TPS rather than data volume or disk I/O.
