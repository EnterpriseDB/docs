...
The EDB Data Migration Service (DMS) is included with Hybrid Manager (HM). You can leverage it to migrate your data from existing databases (Oracle, self-managed Postgres, and so on) to an HM-managed database cluster. EDB DMS supports three migration methods — snapshot, snapshot + streaming, and streaming only — allowing you to migrate your data with minimal-to-no downtime.
...
-   **Streaming-only migrations:** They skip the snapshot phase entirely and only stream changes that occur after the migration starts. Because there's no initial bulk copy, performance characteristics match the streaming portion of a snapshot + streaming migration — driven by sustained TPS rather than data volume or disk I/O.
