...
Specifies the URL of the DMS that the EDB DMS agent uses to perform migrations. Obtain this value from the HM console Sources or Destinations migration pages.
...
User-defined identifier for resource management. If you previously registered this database with the PGAI agent for monitoring and schema ingestion, use the same `resource_id` here. This consistency is essential for the DMS to correctly correlate the resource across both the PGAI agent and DMS agent components.
