...
User-defined identifier for resource management. If you previously registered this database with the EDB Postgres AI agent for monitoring and schema ingestion, use the same `resource_id` here. This consistency is essential for the DMS to correctly correlate the resource across both the EDB Postgres AI agent and DMS agent components.
