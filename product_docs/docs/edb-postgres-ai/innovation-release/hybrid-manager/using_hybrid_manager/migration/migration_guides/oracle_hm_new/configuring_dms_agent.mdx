...
1.  Set the required variables. Optional parameters are commented out by default — uncomment and configure any that apply to your environment.
    ...
    # For CDB/PDB: use the PDB service name (for example, ORCLPDB1)
    # For non-CDB: use the SID or service name (for example, ORCL)
    export DBCONFIG_DATABASES_0__CATALOG=<pdb_or_sid>
    ...
    #    * If `excludeList` is present -> only tables matching at least one include pattern are considered
...
Once running, the agent registers your Oracle database as a source in HM. Continue to the next section to verify it appears correctly.
