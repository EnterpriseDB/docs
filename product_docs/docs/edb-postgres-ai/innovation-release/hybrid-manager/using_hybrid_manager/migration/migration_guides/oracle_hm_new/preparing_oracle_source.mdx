...
<details><summary>Why should I increase redo log space?</summary>
...
1.  Create a new user for migrations: 
    ...
    ```sql
    CREATE USER <MIGRATION_USER> IDENTIFIED BY <MIGRATION_USER_PASSWORD> DEFAULT TABLESPACE LOGMINER_TBS QUOTA UNLIMITED ON LOGMINER_TBS;
    ```
...
1.  Exit sql, and perform a quick connection test with your migration user: 
    ...
    ```sql
    sqlplus <C##_MIGRATION_USER>@//<HOST>:<HOST>/<PDB_NAME>
    ```
    ...
    ```sql
    sqlplus <MIGRATION_USER>/<MIGRATION_USER_PASSWORD>@//<HOST>:<HOST>/<SID_OR_SERVICE>
    ```
...
## Validate your database and user configurations
