...
Configure the EDB Postgres AI agent (deployed as `beacon-agent`) to establish a secure connection with HM and retrieve necessary schema and migration data from your Oracle source database.
...
-   To support the ingestion of a large number of databases, see [Performing multiple concurrent schema ingestions](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/monitoring/mon_ext_dbs/mon_with_agent/configure_agent/schema_ingestion/#performing-multiple-concurrent-schema-ingestions)
