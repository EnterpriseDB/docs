...
However, `beacon-agent` must be able to find a `pg_dump`/`pg_dumpall` binary matching your source database's version. See [Configuring the environment](auxiliary_machine#configuring-the-environment) for the version rules and how to point the agent at the right binaries with `pg_bin_path`.
