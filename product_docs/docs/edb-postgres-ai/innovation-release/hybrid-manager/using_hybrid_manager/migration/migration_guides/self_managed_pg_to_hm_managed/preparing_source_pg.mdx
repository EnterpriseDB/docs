...
1.  Grant `CREATE` on the source database so the DMS agent can create publications:
    ...
    You also need a role on the *destination* matching each role that owns a large object on the source — see [Large object (LO) migration](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/terminology/#large-object-lo-migration) for details.
    !!!
