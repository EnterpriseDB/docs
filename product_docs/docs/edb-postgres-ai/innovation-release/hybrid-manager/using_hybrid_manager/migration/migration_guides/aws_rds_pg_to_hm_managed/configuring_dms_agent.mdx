...
description: Create migration credentials, configure the EDB DMS agent in reader mode for your AWS RDS or Aurora PostgreSQL source, and verify the source appears in HM.
deepToC: true
# Adapted from migration_guides/self_managed_pg_to_hm_managed/configuring_dms_agent.mdx
# with AWS RDS/Aurora-specific differences (prerequisites, RDS endpoint hostname, endpoint note).
# Edit both files if making shared structural changes.
redirects:
  - /edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/migration/migration_guides/aws_rds_pg_to_hm_managed/creating_project_credentials/
  - /edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/migration/migration_guides/aws_rds_pg_to_hm_managed/configuring_reader/
  - /edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/migration/migration_guides/aws_rds_pg_to_hm_managed/viewing_source/
---

The EDB DMS agent connects to your AWS RDS or Aurora PostgreSQL source and works with the Data Migration Service (DMS) to perform data migrations. Before running it, create migration credentials in HM and configure the agent with your source database details.
...
1.  Set the required variables. Optional parameters are commented out by default — uncomment and configure any that apply to your environment.
    ...
    #export DBCONFIG_DATABASES_1__HOSTNAME=<RDS_endpoint>
    ...
    # completes. See the parameter reference for the related LO_SNAPSHOT_*
    # tuning parameters.
    ...
    !!!note
    Set `DBCONFIG_DATABASES_0__HOSTNAME` to the RDS endpoint found under **Connectivity & security** in the AWS Console. A redacted example is `foo_database.cb7asw1xxxxx.eu-west-0.rds.amazonaws.com`.
    !!!
...
The script exports all required environment variables and starts the agent process. Once running, the agent registers your AWS RDS or Aurora PostgreSQL database as a source in HM. Continue to the next section to verify it appears correctly.

## Verifying the source in HM

Once the agent is running, verify that your database appears as a source in HM:
