...
If you plan to use the **Schema and data** or **Schema only** migration scope, the agent must finish ingesting your source schema before a Migration Portal project is created for the database. To confirm the project exists, go to **Estate > Migrations** in the HM console, select the database in the **Database Name** column, and check that a project appears in the **MP Project** column. If no project appears, see [Migrating Postgres schemas](migrating_schema) for how to create one manually. Do this before [creating the migration](migrating_data). The **Schema and data** and **Schema only** scopes will fail without it.
!!!
