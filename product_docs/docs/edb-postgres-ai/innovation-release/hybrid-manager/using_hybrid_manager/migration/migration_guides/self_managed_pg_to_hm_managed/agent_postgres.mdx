...
- **Reserved schema names**: The agent reserves the schema names `extensions` and `globals` for internal use during DDL extraction. If your source database contains schemas with either of these names, rename them before running the agent. See [Reserved schema names for Postgres DDL extraction](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/limitations/#reserved-schema-names-for-postgres-ddl-extraction).
