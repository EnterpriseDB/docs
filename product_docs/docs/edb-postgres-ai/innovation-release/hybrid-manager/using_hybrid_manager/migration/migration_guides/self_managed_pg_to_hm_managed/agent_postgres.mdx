      ...
      onprem:
        databases:
          - resource_id: "<database_resource_id>"
            ...
            # Set to false if the agent is running on the same host as the source database.
            schema:
              enabled: true
              poll_interval: 15s
            tags:
...
- **Automatic schema DDL extraction** that you can use for schema migrations with the built-in Migration Portal. Once ingestion is complete, the Migration Portal automatically creates a project per database, which you can use to convert incompatible schemas and migrate them to the destination database.
