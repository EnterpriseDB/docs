...
- **Machine user and access key**: Created in [Preparing your Postgres source database](preparing_source_pg#creating-a-machine-user-in-hybrid-manager). Store the access key in an environment variable:
...
-   **Beacon server hostname**: The internal endpoint for the `beacon_server`.
    ...
    When configuring the HM installation, administrators must specify an internal endpoint for the `beacon_server` service. Ask the administrator or installer of your HM instance to provide the hostname (URL) they set for the `beacon_server` service in the Helm chart configuration file used for installation. For this, they'll look up the value they set for `parameters.upm-beacon.server_host` (or `BEACON_SERVICE_DOMAIN_NAME`) in the `values.yaml` file. You'll need this value later to configure the EDB Postgres AI agent.
...
Once you have the HM connection parameters, verify that your auxiliary machine can reach both the Postgres source and Hybrid Manager. The following ports must be open:
...
1.  Copy and paste the following template into the new file.
    ...
    agent:
      ...
      providers:
        - "onprem"
      root_ca_path: ""
      # In most HM deployments, set this to the path of the ca_public.crt
      ...
      onprem:
        databases:
          - resource_id: "<database_resource_id>"
            # Controls how this database appears in HM Estate.
            dsn: $DSN
            schema:
              enabled: true
              poll_interval: 15s
