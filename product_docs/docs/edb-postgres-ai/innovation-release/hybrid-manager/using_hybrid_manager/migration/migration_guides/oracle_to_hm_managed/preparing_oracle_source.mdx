...
Before you perform a migration of an Oracle source using Hybrid Manager (HM), you must complete several database configuration steps and create a dedicated migration user. These steps ensure that all migration components, including the EDB Postgres AI agent (`beacon-agent`) and the DMS agent (`cdcagent`), can establish a stable connection, discover your schemas, and perform schema and data extraction operations.
...
### (Optional) Ensuring adequate redo log space for better performance
...
1.  Verify the migration user can see your source tables from the CDB root. If this returns no rows, the grants were not applied with `CONTAINER=ALL` — regrant all privileges as shown above.
