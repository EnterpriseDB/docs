...
For migrations with the **Schema and Data** scope, until the migration completes, the destination has the correct large object OIDs but not the content yet. Logical decoding can't capture large object writes (`lo_write`, `lo_truncate`, and similar operations don't produce DML WAL), so DMS copies large object content during the **Schema Finalization** stage instead — alongside foreign key validation, for migrations that have one. See [Large object (LO) migration](terminology/#large-object-lo-migration) for how large object migration works.
