...
### SQL Assessment file-level statistics not populated after assessment completes

[MIG-8296]: #

**Description:** When a file is uploaded for SQL Assessment within an application, the assessment runs automatically. After completion, expanding the file entry using the **+** button next to the file name should display file-level statistics. However, due to a current limitation, these statistics are not available in the backend and are not populated in the UI. The following fields remain empty: **# of Features in Query**, **Compatible Queries**, **Incompatible Queries**, and **Complexity**.

**Workaround:** None. Some aggregate statistics are still available at the application level under the **SQL Assessment** tab.
...
**Workaround:** Before upgrading to 2026.4, manually delete the `ragchew-ai-vendor-secrets` secret in the `edb-migration-copilot` namespace. The operator will recreate it with the correct type during the upgrade:
