...
**Description:** Postgres-specific schema object type support was added to `upm-beacon` and the Migration Portal for DMS schema migrations. However, the DMS components (`transporter-rw-service` and `transporter-agent`) weren't extended to request these types at schema migration time, causing them to be omitted from DMS-managed schema migrations.

**Workaround:** Migrate the schema manually and use DMS in data-only migration mode. If you need to perform a combined schema and data migration, manually migrate any missing Postgres-specific schema objects before starting the DMS migration.
