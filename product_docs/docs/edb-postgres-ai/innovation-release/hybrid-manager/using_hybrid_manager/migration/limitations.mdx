...
[limitation, ask Fabian whether this is a permanent limitation or if we plan to solve this]: #

The EDB Postgres AI agent reserves two schema names internally during Postgres DDL extraction:

- `extensions` — used to store `CREATE EXTENSION` statements
- `globals` — used to store global objects such as roles and permissions

The agent rejects any source database that contains user schemas with either of these names. If your Postgres source database has a schema named `extensions` or `globals`, rename it before registering the database with the agent.
