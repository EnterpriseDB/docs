...
Starting in HM 2026.9, EDB DMS supports migrating Postgres large objects (`pg_largeobject`) for Postgres sources. Because logical decoding can't capture large object writes (`lo_write`, `lo_truncate`, and similar operations don't produce DML WAL), large object content isn't part of the regular snapshot or streaming phases. Instead, when you enable the large object snapshot (`LO_SNAPSHOT_ENABLED=true`), DMS copies large object content in a separate bulk operation that runs during the post-cutover **Schema Finalization** stage, alongside foreign key validation. Until that stage completes, the destination has the correct large object OIDs but not yet the content.

This applies to **Snapshot + Streaming** migrations from Postgres sources. See [Postgres large object migration parameters](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/dms_agent_reference/#postgres-large-object-migration-parameters) for configuration options.
