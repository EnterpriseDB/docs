...
When viewing the list of schemas for a source EDB Postgres Advanced Server database that you registered with the PGAI agent, you may observe additional schemas that don't directly correspond to schemas you explicitly created. 
...
**Workaround:** To ensure successful automatic creation of Migration Portal projects, the `resource_id` value specified in the PGAI agent configuration for Oracle databases must conform to the following naming rules:
