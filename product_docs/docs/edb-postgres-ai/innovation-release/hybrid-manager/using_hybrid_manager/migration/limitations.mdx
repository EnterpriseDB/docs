...
Starting in HM 2026.9, EDB DMS supports migrating Postgres large objects (`pg_largeobject`) for Postgres sources, for the **Data only** and **Schema and data** migration scopes. The **Schema only** scope doesn't apply, since it migrates database structure without any table data.
