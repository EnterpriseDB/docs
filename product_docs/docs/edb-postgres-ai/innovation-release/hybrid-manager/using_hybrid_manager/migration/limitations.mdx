...
[ET-4363, partially resolved, other ticket pending, waiting on Zahid]: #
...
**Description:** When uploading a SQL file for an Application Migration assessment, the HM console doesn't associate a database resource ID with the uploaded file. Because no resource ID is provided, `upm-beacon` sends the assessment request to the Migration Portal with the source database version set to `UNKNOWN`. The Migration Portal then defaults to `Oracle_19c` as the source database type when processing the assessment. As a result, assessments may not accurately reflect the compatibility rules for your actual source database version. Additionally, at least one project must exist in the Migration Portal for the assessment to complete without errors.

**Workaround:** Ensure at least one project exists in the Migration Portal before initiating an Application Migration assessment via SQL file upload. A fix that associates database resource IDs with uploaded SQL files is being tracked for a future release.
