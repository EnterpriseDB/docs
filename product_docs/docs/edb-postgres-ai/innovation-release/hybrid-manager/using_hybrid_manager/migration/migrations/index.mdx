...
**Resource ID:** Resource ID or `databases.resource_id` you assigned to a database in the `beacon_agent.yaml` configuration. See the PGAI agent's [parameter reference](../../monitoring/mon_ext_dbs/mon_with_agent/configure_agent.mdx#parameter-reference) for more information.
...
**# of Schemas:** Number of schemas in the assessed database that the DSN user has access to. Contains at least one table on which the DSN user has `SELECT` privileges. The DSN user is the source database user you specified while configuring the PGAI agent.
...
**Project Name:** Project that was used for configuring the PGAI agent. It's the project you assign to an agent in `agent.project_id`. See the PGAI agent [parameter reference](../../monitoring/mon_ext_dbs/mon_with_agent/configure_agent.mdx#parameter-reference) for more information.

**Tags:** Labels assigned to the migration of a specific database in `databases.tags`. See the PGAI agent [parameter reference](../../monitoring/mon_ext_dbs/mon_with_agent/configure_agent.mdx#parameter-reference) for more information.
...
From the **Estate** page, the **Migration** tab displays a table-based analysis of all databases that you can migrate with HM after registering it with the PGAI agent. To customize how assessment results are displayed or filter databases, you can:
...
-   See [Data security and sovereignty](../../monitoring/mon_ext_dbs/mon_with_agent/data_sovereignity.mdx) to understand how your data is stored by the PGAI agent.
