...
When you select a database from the **Migrations** tab of the **Estate** page in Hybrid Manager (HM) — or from the project-scoped [**Databases**](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/migrate/databases/) page under **Migrate** — a detailed view opens. Use this view to plan and execute migrations from external databases. The **Schemas** and **Assessment** tabs display different information depending on whether your source database is Postgres or Oracle.
...
- **MP Project:** Link to the Migration Portal project associated with this schema. Migration Portal projects for Postgres databases are created automatically by the EDB Postgres AI agent. To create a project manually — for example, after deleting one or if automatic creation failed — select **+ New MP Project** above the table and provide the required details. HM creates the project in the background using the Migration Portal [project-scoped workspace](../../migration/schema_migration/migration_portal/access_mp.mdx#project-scoped-workspace-vs-users-workspace).
...
**Migration Portal project table:** Lists the Migration Portal project associated with the schema, including the project name, target database version, creation date, and available actions. You can delete a project from the **Actions** column.
...
- **Snapshot Type:** The type of snapshot used. Possible values are **Snapshot**, **Snapshot + Streaming**, and **Streaming**.
