...
5.  Configure another object storage by following the same steps you used during the [installation](/edb-postgres-ai/current/hybrid-manager/install/install_guide/environment_prep/#create-object-storage-secret).
6.  Create the secret for the object storage in your Kubernetes cluster. The secret body — including the `stringData:` block with the provider-specific credential fields — is identical to the one documented in the [install guide](/edb-postgres-ai/current/hybrid-manager/install/install_guide/environment_prep/#create-object-storage-secret). The only difference is the secret **name**, which must be `edb-object-storage-<normalized-project-id>`.
    ...
    metadata:
      ...
      namespace: default
    stringData:
      # Use the same provider-specific fields as the default edb-object-storage
      # secret documented in the install guide (AWS IAM, AWS static credentials,
      # Azure, GCP, or other S3-compatible storage).
      ...
    ```
