...
Use your portal access key. You must have sufficient permissions. For information on how to create an access key, see [Access key](access_key).

## Available tool categories

The MCP server exposes tools across these functional areas:

- Cluster management — Create, view, scale, update, and delete Postgres clusters (PG, PGE, and EPAS) in Single Node, High Availability, and Distributed High Availability topologies, manage cluster templates, backups, and maintenance windows.
...
- Agent Factory — Configure inference services, models, model server clusters, knowledge bases, pipelines, and Agent Studio-based AI flows.
...
!!!note Accessing Postgres cluster databases via MCP
The HM MCP server manages Hybrid Manager operations. It does not provide direct MCP access to the Postgres databases running inside the clusters it manages. To interact with those databases using MCP — for example, to run queries, tune indexes, or analyze query plans — use [Pg Airman MCP](https://github.com/EnterpriseDB/pg-airman-mcp).
!!!

!!!important Tool limits with LLM clients
