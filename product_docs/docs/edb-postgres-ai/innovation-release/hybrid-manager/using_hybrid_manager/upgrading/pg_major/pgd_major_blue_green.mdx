...
To ensure all data is being synchronized as expected, you can use the PGD CLI to [view data group state](/pgd/latest/reference/cli/command_ref/group/show/) or perform [deeper monitoring](/pgd/latest/lifecycle/monitoring/sql/#monitoring-replication-peers) on the replication peers.
...
5.  On the **Data Groups** tab, select **Save**. 
...
To manage this, you can modify the replication configuration of your cluster by [enabling or disabling the replication subscriptions](/pgd/latest/reference/tables-views-functions/nodes-management-interfaces/#bdralter_subscription_disable). 
