...
The `velero-backup-kube-state` schedule is rendered by the `upm-velero-config` component from its `schedule` parameter. The value is a standard cron expression and defaults to `0 23 * * *` (daily at 23:00).

Starting in HM 2026.9, the Velero configuration (the backup storage location, the volume snapshot location, and the backup schedule) moves out of the Velero installation into the `upm-velero-config` component.

To change the schedule, set `schedule` under `spec.componentsParameters.upm-velero-config` in your `HybridControlPlane` custom resource. For example, to back up every six hours:
...
!!!warning Upgrading to 2026.9 with a customized schedule
In HM versions before 2026.9, if you set `schedule` under `spec.componentsParameters.upm-velero`, move that value to `upm-velero-config` as part of the upgrade. Starting in HM 2026.9, Velero no longer reads a value left under `upm-velero`, and the schedule reverts to the default of `0 23 * * *`.
!!!
