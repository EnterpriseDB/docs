...
In HM 2026.9, the Velero configuration (the backup storage location, the volume snapshot location, and this schedule) was separated from the Velero installation itself into the `upm-velero-config` component.
...
If you set `schedule` under `spec.componentsParameters.upm-velero` before 2026.9, move that value to `upm-velero-config` as part of the upgrade. From 2026.9, a value left under `upm-velero` is no longer read, and the schedule reverts to the `0 23 * * *` default.
!!!
