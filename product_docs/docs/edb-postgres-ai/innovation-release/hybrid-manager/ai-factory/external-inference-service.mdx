...
The list displays each service's name, model, current status, and (when the status is not Ready) a short **Status Message** explaining the cause.
...
| Pending | The endpoint has not yet been verified by the background health check. Typically seen during the first few minutes after a Hybrid Manager restart, while the cache warms up. The Status Message reads `Remote connection check is pending — upstream has not yet been verified`. |
| Unknown | The underlying Kubernetes resources have not reported a status yet — usually a transient state right after creation. |
...
| Self-hosted / vLLM | Your internal service URL, e.g. `http://vllm-svc.inference:8000` | `OPENAI_V1` | The model name your vLLM server is serving | API key **not** validated at registration when vLLM runs unauthenticated. |
...
For providers not listed here, see the per-field reference below.
...
**Do not include `/v1`** — consumer applications append `/v1` (or `/v1beta` for Gemini) themselves. Including `/v1` here causes duplicated paths such as `/v1/v1/chat/completions`, which returns a 404. A trailing slash is tolerated and stripped automatically.

!!!Tip
Self-hosted vLLM servers return `404` on the per-model endpoint `/v1/models/{name}`, which would normally fail the registration probe. Hybrid Manager handles this by falling back to `GET /v1/models` and matching the model name in the list, so vLLM-served models register successfully.
!!!
...
A common case where this toggle is required is **registering another Hybrid Manager's external inference endpoint** (for example, HM2 federating through HM1). In that flow HM1's ingress typically presents a self-signed certificate, so both the registration probe and runtime forwarding fail with TLS errors unless this toggle is on. HM-to-HM federation works only when both ends use `OPENAI_V1` as the API Protocol Version.
...
!!!Note
For some `OPENAI_V1` providers — such as NVIDIA NIM, HuggingFace, OpenRouter, and self-hosted vLLM running without authentication — the models endpoint does not require an API key. A connectivity probe is still performed, but a wrong API key may still return HTTP 200. Key validity is not guaranteed at registration time for these providers.
!!!
...
!!!Note
The API key is never displayed in the detail view. To replace it, use the Edit form — the new value supersedes the stored secret.
!!!
...
| Status Message contains... | Likely cause | What to do |
|----------------------------|--------------|------------|
...
| `endpoint not found — check the base URL and model name` | Typo in **Model Base URL** or wrong **Model Name** | Verify both values. Make sure Base URL does not end with `/v1`. |
...
| `Remote connection check is pending — upstream has not yet been verified` | Background health check has not run yet (typically after a Hybrid Manager restart) | Wait up to 5 minutes for the next refresh tick. |
...
| API Protocol Version | No in practice — delete and re-register |
| Allow Insecure Connection | No — delete and re-register |

!!!Note
**API Protocol Version is effectively locked after registration.** Each protocol probes a different endpoint path with a different authentication header, and the connectivity probe that ran at registration only passed because the original protocol matched the **Model Base URL** and **Model Name**. Since those two fields cannot be changed, any attempt to switch to a different protocol fails the connectivity probe and the update is rejected. To use a different protocol, deregister the service and register a new one.
!!!
