...
description: How to manage remote model endpoints (OpenAI, Google Gemini, Anthropic Claude, NVIDIA NIM, or any OpenAI-compatible API) in Hybrid Manager so that Agent Studio flows, Pipeline Designer pipelines, and AIDB can proxy calls to them.
deepToC: true
---
...
| Hugging Face | `https://router.huggingface.co` | `OPENAI_V1` | `openai/gpt-oss-120b` | Use the router subdomain (`router.huggingface.co`), not the main site (`huggingface.co`). API key not validated at registration. |
