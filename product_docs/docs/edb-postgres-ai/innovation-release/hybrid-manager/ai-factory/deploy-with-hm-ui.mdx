---
title: Deploy Agent Factory with Hybrid Manager UI
navTitle: Deploy with HM UI
description: Step-by-step guide for deploying NVIDIA NIM model clusters and configuring AI flows through the Hybrid Manager web interface
---

## Overview

This guide provides deployment procedures for Agent Factory components through the Hybrid Manager (HM) web interface. 
...
Verify GPU nodes meet NIM requirements — see [GPU recommendations](gpu-recommended).
...
1. Navigate to **Agent Factory** → **Model Library**.
...
1. Navigate to **Agent Factory** → **API Token Management**
...
Gen AI application building in Hybrid Manager is done through **Agent Studio**, accessible via **Launchpad** in the HM console. Agent Studio provides a visual flow editor with EDB components that connect directly to your HM-managed models, knowledge bases, and Postgres clusters.

For a step-by-step walkthrough of building your first Gen AI application, see [Agent Studio quickstart](./langflow/quickstart).

For a full reference of EDB components, flow deployment, and sharing flows between instances, see [Agent Studio](./langflow/).
...
## High availability and isolation for Agent Studio flows

High availability, project-level isolation, stable routing, and access control for an Agent Studio flow are properties of a *deployed* flow, not of the Agent Studio flow builder. The flow builder is a single shared instance: flows you run there are not isolated per project and you can't scale them independently. To run a flow as an isolated, highly available, independently scalable service with its own endpoints, publish and deploy it. See [Flow deployment](./langflow/flow-deployment), which covers replicas, endpoints, access levels, and credential handling for deployed flows.

For the model server clusters that Agent Studio flows depend on, adjust the number of running instances in **Estate** → **InferenceServices** → select the inference service → **Edit** → **Inference Service Instances**.
