...
This guide provides deployment procedures for AI Factory components through the Hybrid Manager (HM) web interface. 
The deployment process covers GPU infrastructure setup, NVIDIA NIM model deployment, and Gen AI application configuration.
...
1. Create authentication secrets via HM UI:
...
1. **Update Model URLs**: Configure HM to reference private registry locations
1. **Cache Profiles**: Download and store model profiles in object storage
1. **Configure Storage Path**: Reference cached profiles during model deployment
...
1. Navigate to **AI Factory** → **Model Library**
...
1. Select target model from available options
1. Review model requirements and documentation links
...
1. Monitor deployment status in Model Server Clusters view
...
1. Provide token reference name
1. Store generated token securely
...
For a step-by-step walkthrough of building your first Gen AI application, see [Langflow quickstart](./langflow/quickstart).

For a full reference of EDB components, flow deployment, and promotion to production, see [Langflow](./langflow/).
...
## High availability for Gen AI applications

For production workloads, configure Langflow and its dependent components to run with high availability. This section covers replica configuration, load balancer and ingress setup, and namespace-level workload isolation between development and production environments.

### Configure replica counts

Langflow in Hybrid Manager runs as a Kubernetes deployment. Increasing the replica count ensures the application stays available if a pod restarts or a node becomes unavailable.

To adjust replica counts for the Langflow deployment:

1. Navigate to **AI Factory** → **Langflow** in the HM console.
1. Select **Settings** → **Scaling**.
1. Set the desired replica count. We recommend a minimum of 2 replicas for production environments.
1. Apply the changes. HM performs a rolling update, so existing connections aren't interrupted.

For model server clusters that Langflow flows depend on, set replica counts in **AI Factory** → **Model Server Clusters** → select the cluster → **Edit** → adjust **Server Instances**. See [Step 3](#step-3-deploy-model-server-cluster) for configuration details.

### Set up load balancing and ingress

Hybrid Manager uses Istio as its service mesh. When multiple Langflow replicas are running, Istio automatically load-balances traffic across them.

To expose Langflow to external consumers through a stable ingress endpoint:

1. In the HM console, navigate to **Project Settings** → **Networking**.
1. Verify that an ingress gateway is configured for the project. If one isn't configured, work with your platform team to set one up — see [Prerequisites](./prerequisites) for ingress requirements.
1. Once configured, Langflow flows are accessible at the project's external ingress URL.

For model server endpoints, the load balancer ingress URI is shown in **AI Factory** → **Model Server Clusters** → select the cluster → **Details**.

!!!Note
Istio manages internal load balancing between pods automatically. External ingress configuration depends on your cluster's networking setup and may require coordination with your platform or infrastructure team.
!!!

### Isolate dev and prod workloads using namespaces

Hybrid Manager maps each project to a dedicated Kubernetes namespace. Keeping development and production Langflow instances in separate HM projects ensures that:

- Development changes and experiments don't affect production traffic.
- Secrets, access keys, and Global Variables are scoped independently to each environment.
- Resource quotas and network policies can be applied per environment.

The recommended setup uses one HM project per environment:

| Environment | HM project name (example) | Kubernetes namespace  |
|-------------|---------------------------|-----------------------|
| Development | `ai-factory-dev`          | `m-<dev-project-id>`  |
| Production  | `ai-factory-prod`         | `m-<prod-project-id>` |

To create a separate production project:

1. In the HM console, navigate to **Projects** → **Create Project**.
1. Give the project a distinct name (for example, `ai-factory-prod`).
1. Configure the production project with its own machine users, secrets, and model clusters.
1. Deploy Langflow in the production project following the steps in this guide.

To promote a Langflow flow from development to the production project, see [Promote a Langflow application to production](./langflow/promoting-to-production).
