...
The maximum length is 46 characters (AIDB reserves up to 17 characters of the PostgreSQL identifier limit for internal suffixes).
...
![Step type selector showing the available pipeline step types](images/02-step-type-selector.png)
...
During pipeline creation, select one of three AIDB auto-processing modes: **On Demand** (default, manual execution), **Live** (real-time, trigger-based), or **Background** (asynchronous batch processing). You can change the mode after creation without recreating the pipeline.
...
![Processing type selector showing On Demand, Live, and Background mode options](images/04-processing-type-selector.png)
