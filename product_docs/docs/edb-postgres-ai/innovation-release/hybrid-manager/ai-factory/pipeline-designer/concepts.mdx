...
During pipeline creation, select one of three AIDB auto-processing modes: **Disabled** (default, manual execution), **Live** (real-time, trigger-based), or **Background** (asynchronous batch processing). You can change the mode after creation without recreating the pipeline.
...
![Processing type selector showing Disabled, Live, and Background mode options](images/04-processing-type-selector.png)
