...
!!! Warning
    Deleting a knowledge base is irreversible. All vector data is permanently removed. If the knowledge base is used by [Langflow](/edb-postgres-ai/innovation-release/hybrid-manager/ai-factory/langflow) flows or other consumers, those integrations will break.

## Using knowledge bases with Langflow

Knowledge bases created through Pipeline Designer are accessible from the [Langflow-based Gen AI builder](/edb-postgres-ai/innovation-release/hybrid-manager/ai-factory/langflow). The Langflow KB component connects to AIDB knowledge bases using the same [`aidb.retrieve_key()` and `aidb.retrieve_text()`](/aidb/latest/knowledge-bases) functions that power the Pipeline Designer query tool.
