...
Self-managed clusters must meet additional prerequisites. They need PostgreSQL 16 or later, the AIDB extension installed and configured, and the EDB Postgres AI agent connected to the HM control plane.
...
Paths 1 and 2 require network connectivity to the primary location's model-serving endpoints, which isn't present by default on secondary locations or self-managed clusters. Path 3 works anywhere because the PostgreSQL process contacts the provider endpoint directly.
