...
-   **Self-managed clusters.** Customer-managed PostgreSQL instances registered with HM through the EDB Postgres AI agent (`beacon-agent`). Self-managed clusters are not provisioned by HM and do not have network connectivity to the primary location by default. Without that connectivity, only AIDB-native and built-in models are available. See [How models reach pipeline steps](#how-models-reach-pipeline-steps) for details.
