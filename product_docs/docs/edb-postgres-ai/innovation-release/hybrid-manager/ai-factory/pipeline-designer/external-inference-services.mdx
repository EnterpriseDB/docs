...
When you register an external inference service through the Hybrid Manager UI (AI Factory > Model Serving), HM deploys a managed proxy on the primary location that forwards inference requests to the external provider's API endpoint. HM manages the provider's API credentials; they are not stored in the database.
