...
Most of the VPU infrastructure is set up automatically. When you create your first pipeline through Pipeline Designer, beacon-agent installs the AIDB extension (`CREATE EXTENSION aidb CASCADE`), creates the `visual_pipeline_user` role, grants it LOGIN (on PostgreSQL versions before 17), and adds it to the `aidb_users` role. You do not need to run any of these steps manually on HM-managed clusters.

One manual step is required before you create your first pipeline. VPU must be able to create pipeline destination tables and knowledge base vector tables in the `public` schema. Grant the required privilege by connecting to the database as an administrator and running:
...
!!! Note "Self-managed clusters"
    On self-managed clusters, beacon-agent performs the same automatic setup, but the operator must ensure that beacon-agent's database role has sufficient privileges to execute the setup SQL. See the [AIDB installation documentation](/aidb/latest/install/configure) for self-managed configuration requirements.
!!!
