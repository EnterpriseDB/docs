...
3.  **Install processing triggers.** Live and Background processing modes attach triggers to the source table so that new or updated rows are processed automatically. VPU needs `TRIGGER` on the source table for these modes. (On Demand mode does not use triggers and does not require this grant.)

The subsections below show the exact `GRANT` statements for each topology.

### Tables in the public schema

On PostgreSQL 15 and later, `CREATE` on the `public` schema is no longer granted to `PUBLIC` by default. VPU therefore needs an explicit `CREATE ON SCHEMA public` grant regardless of whether the cluster is managed or self-managed. The `aidb_users` role grants access to AIDB extension objects but does not cover `SELECT` on user-created tables, schema-level `CREATE`, or table-level `TRIGGER`.
