...
The constraints and limitations below are intentional boundaries of the current Pipeline Designer implementation, not transient bugs.
...
Self-managed cluster support is limited to single-instance PostgreSQL servers. External PGD (Postgres Distributed) clusters and Cloud Native PostgreSQL (CNP) operator-managed clusters aren't eligible as pipeline targets, even if registered with HM through the EDB Postgres AI agent.
...
**Recommendation**: Reduce polling frequency by increasing the `background_sync_interval` for pipelines where near-real-time processing isn't required. See the [AIDB background worker documentation](/aidb/latest/data-pipelines/orchestration/background-workers) for configuration details.
...
Note that a KnowledgeBase step alone is rarely sufficient for useful results. Raw text data typically needs at least a ChunkText step before the KnowledgeBase step to split content into appropriately sized segments for embedding. A pipeline with only a KnowledgeBase step would attempt to embed entire source rows as single vectors, which can exceed model token limits or produce poor retrieval quality.
...
On AIDB 7.3.0 (shipping with HM 2026.4), the `bdr_setup()` function doesn't include knowledge base tables in BDR replication. Without these, knowledge base registry entries and knowledge base–pipeline associations created on the write leader aren't replicated to other PGD nodes. This can cause inconsistencies after a write leader switchover, where the new leader has no record of knowledge bases or their pipeline bindings.
