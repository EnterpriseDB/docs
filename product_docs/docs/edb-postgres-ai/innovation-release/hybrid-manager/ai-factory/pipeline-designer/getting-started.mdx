...
Per-table grants are configured in [Per-source-table setup](#per-source-table-setup). For On Demand mode, this is a `SELECT` grant. For Live and Background modes, VPU must be granted the source table's owner role.
...
-   On **Postgres 16 and earlier**, VPU requires `LOGIN` privileges because background worker authentication uses role-based login:
    ...
    GRANT visual_pipeline_user TO edb_beacon;
    ```
...
See [Configuring VPU permissions](vpu-and-permissions#configuring-vpu-permissions) for the required grants (a `SELECT` grant for On Demand mode, or the source table's owner role for Live and Background modes) and topology-specific instructions (public schema, custom schemas, and PGD clusters).
...
Grant VPU access to the source table. This example uses Background processing, so VPU must be granted the source table's owner role:

```sql
GRANT <source_table_owner_role> TO visual_pipeline_user;
```

Replace `<source_table_owner_role>` with the role that owns `source_table_st`. If you instead use On Demand mode, grant only `SELECT ON source_table_st TO visual_pipeline_user`. This example covers the public schema. For custom schemas or PGD clusters, see [Configuring VPU permissions](vpu-and-permissions#configuring-vpu-permissions).
...
Replace `<source_table_owner_role>` with the role that owns `products`. If you instead use On Demand mode, grant only `SELECT ON products TO visual_pipeline_user`. This covers the public schema. For custom schemas or PGD clusters, see [Configuring VPU permissions](vpu-and-permissions#configuring-vpu-permissions).
...
The knowledge base is now available for use by downstream applications such as [Agent Studio](/edb-postgres-ai/current/hybrid-manager/ai-factory/langflow/) agents. See [Using knowledge bases with Agent Studio](knowledge-bases#using-knowledge-bases-with-agent-studio) for integration details.
