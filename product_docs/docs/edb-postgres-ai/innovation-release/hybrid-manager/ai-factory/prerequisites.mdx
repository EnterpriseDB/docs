...
You can create an NGC API key secret and image pull secret with the `edbctl` CLI for environments with the `ai` scenario enabled. 
...
- For manual installations, run this command and follow the interactive prompts:
  ...
  The terminal prompts you to enter the NGC API key created in advance.

- For automated pipelines (CI/CD), use the `--non-interactive` flag to suppress all confirmation prompts and allow the command to complete without human intervention. You will need to make the NGC API key available as a variable.
  ...
  This creates both the `nvidia-nim-secrets` and `ngc-cred` secrets in the `default` namespace with the appropriate replication annotations.

#### Manual alternative

You can also create the secret manually, but this requires `kubectl` access to your HM cluster:
