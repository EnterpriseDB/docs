...
description: Agent Studio component that performs semantic similarity search, optionally combined with column filtering, against an AIDB knowledge base in a Hybrid Manager cluster.
deepToC: true
---
...
-   The knowledge base doesn't exist yet. Create one through [Pipeline Designer](/edb-postgres-ai/current/hybrid-manager/ai-factory/pipeline-designer/) or by calling `aidb.create_table_knowledge_base()` against the cluster.
...
-   An HM cluster with at least one AIDB knowledge base. Create one through [Pipeline Designer](/edb-postgres-ai/current/hybrid-manager/ai-factory/pipeline-designer/) or directly via `aidb.create_table_knowledge_base()`. The knowledge base's source can be a table (filtering supported) or a volume (semantic-only).

-   An HM machine-user access key and the database user/password saved in upstream Langflow as [Global Variables](https://docs.langflow.org/configuration-global-variables).
