...
description: Agent Studio component that calls an inference model (LLM or other) hosted on a Hybrid Manager model server cluster.
deepToC: true
---
...
-   An HM model server cluster with the target model deployed. See [Models](/edb-postgres-ai/current/hybrid-manager/ai-factory/models) for how to create one. A [GPU node](/edb-postgres-ai/current/hybrid-manager/ai-factory/gpu-recommended/) is typically required.

-   An HM machine-user access key saved in upstream Langflow as a [Global Variable](https://docs.langflow.org/configuration-global-variables) (default name `HM_API_KEY`).
