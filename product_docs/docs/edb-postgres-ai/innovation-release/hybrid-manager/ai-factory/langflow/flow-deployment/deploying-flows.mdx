...
Deploying turns a [published bundle](publishing-bundles) into a running Agent Studio runtime hosted in a Hybrid Manager (HM) project. Each deployment is an independent workload with its own parameter values, resource configuration, and endpoints.
...
| **Runtime Configuration** | Yes      | Settings that apply to the Agent Studio runtime itself, separate from your flow's parameters (for example, service-level configuration).                  |
...
Every credential parameter in the bundle (any parameter whose Agent Studio input type is `SecretStrInput`) is listed here. Each must be set before the deployment can proceed.
...
Enter the production value for each credential field — the field name matches the Agent Studio Global Variable name used in the flow (for example, `HM_DB_PASSWORD` or `HM_API_KEY`). The **Next** button remains disabled until every required field has a value.
...
| Resource         | Purpose                                                                                                                                                                                                                                                               |
|------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `Deployment`     | Runs the Agent Studio runtime container. Replicated per the configured replica count.                                                                                                                                                                                     |
