...
Flows in the Langflow editor run in an environment shared with all other users of the flow builder and are only available to the user who created that flow. Compared to that, a deployed flow has:
...
| **Runtime**        | The Langflow runtime container that hosts a deployed flow. Exposes an HTTP API and an MCP server, both fronted by the HM gateway.                                                                                                                                                                                                                                                                                                                                                     |

## Required roles

Permissions for the publish/deploy lifecycle are defined in [Roles](/edb-postgres-ai/innovation-release/hybrid-manager/using_hybrid_manager/managing_users/user_roles_authz/authz):
...
| Deploy a published flow                       | A role with `CREATE` on `DEPLOYED_FLOWS` in the target project    |
