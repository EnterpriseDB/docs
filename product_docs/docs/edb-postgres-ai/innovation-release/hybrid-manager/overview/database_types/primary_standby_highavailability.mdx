...
Primary/standby high availability minimizes downtime in cases of failures. Primary/standby high-availability clusters—one *primary* and one or two *standby replicas*—are configured automatically. Standby replicas stay up to date through physical streaming replication. 

If read-only workloads are enabled, standby replicas serve the read-only workloads. In a two-node cluster, the single standby replica serves read-only workloads. In a three-node cluster, both standby replicas serve read-only workloads. The connections are made to the two standby replicas randomly and on a per-connection basis. You can enable this functionality by configuring a [read-only connection](../../using_hybrid_manager/cluster_management/create-clusters/cluster-settings.mdx#connections-read-only) for your cluster, which provides a dedicated read-only endpoint alongside the standard read/write connection string.
