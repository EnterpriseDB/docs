...
**Workaround:** Disable SELinux on all RKE2 cluster nodes. Set `SELINUX=disabled` in `/etc/selinux/config` and reboot the nodes. After reboot, verify with `getenforce` that the output is `Disabled`. The SPIRE agent pods will recover automatically, though pods already in CrashLoopBackOff may take several minutes due to the backoff timer.
