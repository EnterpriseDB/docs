...
**Workaround:** Don't create more than one PGD data group with private access in the same AKS region. For additional data groups in the same region, use public access instead.

### Default network access type isn't preserved in cluster templates on RKE

[UPM-90375]: #

**Description:** On RKE environments, the default `NetworkAccessTypeUnknown` value for the network access type field isn't correctly saved when creating or using cluster templates. The value is overridden to `Private`, which means clusters provisioned from templates on RKE may not reflect the intended network access configuration.

**Workaround:** None. A fix is being tracked for a future release.
...
**Description:** When HM is configured with a custom `portal_port` value other than 443, the **Launch** button in the **Asset Library** generates app URLs using port 443 instead of the configured port. This causes the launched app to be unreachable in environments where the HM portal isn't exposed on the standard HTTPS port.

**Workaround:** Manually replace port 443 with your configured `portal_port` in the app URL to access the app.

### Asset Library app CPU and memory metrics include failed and completed pod states

[UPM-91745]: #

**Description:** The CPU and memory usage metrics displayed for Asset Library apps incorrectly include pods in `Failed` or `Completed` states, such as pods spawned by jobs. This causes the displayed resource usage figures to be inflated or inaccurate.
...
**Workaround:** Manually patch the Kubernetes deployment to increase the memory resource limits for the `upm-ai-model-server-beaco` pod.
