...
### `accm-server` fails to start due to missing Agent Studio secret
...
### False positive backup alerts for Distributed High Availability (PGD) clusters
...
-   Alertmanager: Create a silence rule for these two alerts targeting your Distributed High Availability (PGD) environment.
...
!!!tip
Resolved in HM versions 2026.7 and later.
!!!
...
### Major version upgrade fails for Distributed High Availability (PGD) clusters with witness groups
...
### Distributed High Availability (PGD) witness groups appear in database selection lists
...
### EDB Knowledge Base component returns a 403 error when dragged into Agent Studio
...
**Description:** When dragging the EDB Knowledge Base component into Agent Studio, an error appears in the bottom left corner: `API call to .../projects/None/clusters returned 403`. This occurs because the project ID is missing from the API call at the time the component is loaded.
...
**Description:** When configuring the EDB Knowledge Base component in Agent Studio with a global variable for the database user field, the connection attempt fails. Global variable values aren't resolved correctly for this field during the initial connection setup.
...
**Description:** When creating a flow via the Agent Studio UI, the secret or credential field of any components must use a global variable. Pasting a password directly will cause errors when deploying the flow via HM Flow Hosting. This is a default behavior across all components from Langflow v0.19.
