...
### EPAS not supported as Metabase backend database

**Description:** Metabase's Liquibase component includes DBMS detection logic that is incompatible with EDB Postgres Advanced Server (EPAS). Using EPAS as the backend database causes failures during configuration.

**Workaround:** Use a community PostgreSQL database as the backend database when configuring the Metabase application.
[UPM-92303]: #
...
### Read-only connections not supported on Azure
...
**Description:** Read-only connections aren't supported on Azure Kubernetes Service (AKS) deployments. The option is unavailable when creating or editing clusters on AKS.

**Workaround:** None. Use a non-Azure deployment to take advantage of read-only connections.

### Read-only connections require a minimum of two instances

[limitation]: #

**Description:** Enabling a read-only connection requires the cluster to have at least two data nodes. The option is disabled for single-node clusters and for Advanced High Availability (AHA) data groups configured with a single node, as there are no standby or non-primary nodes available to serve read-only traffic.

**Workaround:** Ensure your cluster is configured with at least two nodes before enabling a read-only connection. For HA clusters, this means selecting a two-node or three-node configuration. For AHA data groups, select a two-node or three-node configuration.
