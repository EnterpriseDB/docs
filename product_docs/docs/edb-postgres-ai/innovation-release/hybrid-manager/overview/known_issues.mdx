...
**Description:** When bootstrapping a cluster via full recovery, the recovery pod can enter an infinite failure loop. After WAL replay completes and archive recovery finishes successfully, the post-recovery shutdown (`pg_ctl stop -m fast`) fails to return within the 60-second timeout. The operator then proceeds to restart the instance, but encounters a stale `postmaster.pid` file and fails with `error starting PostgreSQL instance`. A new recovery pod is scheduled and the same failure sequence repeats indefinitely with no self-healing.
