...
**Description:** Azure enforces a hard limit of eight private link services per Standard Load Balancer (`PrivateLinkServicesPerLoadBalancerLimitReached`). Because each PGD data group with private access requires five Load Balancers (one per node, plus the group and proxy services), an AKS-based HM deployment can only host one PGD data group with private access per region.

**Workaround:** Don't create more than one PGD data group with private access in the same AKS region. For additional data groups requiring private access, deploy an additional HM instance in the same region.
...
**Workaround:** None. This issue exists in the upstream CloudNativePG codebase and a fix is being tracked for a future release.
...
### Table offloading fails with a 500 timeout error

[UPM-77149]: #

**Description:** When attempting to offload a table to object storage, the API may return a 500 error with the message `error offloading table: callback error: runner error: timeout: context deadline exceeded`. This occurs when the operation takes longer than the allowed timeout period, typically due to slow write performance or high latency from the cloud provider's object storage.

**Workaround:** This issue is generally caused by external infrastructure bottlenecks rather than the HM deployment itself. To mitigate this, troubleshoot any performance bottlenecks or latency issues within your cloud provider's object storage service.

Once the storage performance is restored, retry the table offload operation from the HM console.
...
### Asset Library fails to install during upgrade due to missing image manifest

[UPM-91085]: #

**Description:** In some environments, upgrading to HM 2026.4 may produce a `MANIFEST_UNKNOWN: Requested image not found` error when the `upm-kapp-marketplace` component attempts to sync the `kapp-marketplace/free` image. The error appears during deployment but resolves itself without any impact.

**Workaround:** No action needed. The error is transient and the deployment completes successfully.
...
**Workaround:** Avoid including Langflow MCP server components in flows intended for deployment via the HM Flow Hosting API. If MCP server functionality is required, use the full Langflow interactive environment instead.

### `PGVector` component not supported in HM Flow Hosting

[AID-1244]: #

**Description:** Flows that include the `PGVector` component fail when executed via the HM Flow Hosting API with the error: `Error building Component PGVector: No module named 'pgvector'`. The HM Flow Hosting runtime (`lfx-runtime`) is a lightweight environment that doesn't include the `pgvector` Python dependency required by this component.

**Workaround:** Don't use the `PGVector` component in flows intended for deployment via the HM Flow Hosting API. If `PGVector` functionality is required, use the full Langflow interactive environment instead.

### Langflow EDB Knowledge Base component fails to connect when using global variables

[AID-1245]: #

**Description:** When configuring the EDB Knowledge Base component in Langflow with global variables for the database username and password fields, the connection attempt fails with the error `FATAL: password authentication failed for user "HM_DB_USER" connection to server`. Global variable values aren't resolved correctly during the initial connection setup.

**Workaround:** Enter the database credentials as raw values directly in the component fields first. Once the connection is established and the Knowledge Base dropdown populates, replace the raw values with your global variables and save the flow.

### Flow Hosting deployment fails intermittently with `FlowValidationError`

[AID-1277]: #

**Description:** When deploying a flow via HM Flow Hosting, the `lfx-runtime` pod may intermittently enter a `CrashLoopBackOff` state with a `FlowValidationError` indicating that `load_from_db` isn't set to `true` for secret inputs. This issue is intermittent and doesn't occur on every deployment attempt.

**Workaround:** Retry the deployment. The issue is intermittent and subsequent attempts typically succeed.
