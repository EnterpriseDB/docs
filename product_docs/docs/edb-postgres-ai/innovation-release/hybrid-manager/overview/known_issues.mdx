...
### Marketplace apps pgAdmin4 and Superset fail to deploy on RHOS

[UPM-88301]: #

**Description:** In Red Hat OpenShift (RHOS) environments, deploying the pgAdmin4 and Superset marketplace applications fails due to OpenShift Security Context Constraint (SCC) violations. The upstream images for these apps run as specific user IDs (for example, `5050` for pgAdmin4 and `0` for Superset's init container) that fall outside the allowed UID ranges enforced by OpenShift's restricted SCC policies. This is an upstream image compatibility issue.

**Workaround:** Don't deploy pgAdmin4 or Superset in RHOS environments until this issue is resolved upstream.
...
### Read-only workload incorrectly allowed for single-node AHA clusters

[UPM-88040]: #

**Description:** For Advanced High Availability (AHA) clusters with only one data node, the HM console incorrectly allows users to enable a read-only workload. The resulting connection string doesn't work and returns a runtime error: `no nodes available`. A single-node AHA cluster cannot serve read-only workloads.

**Workaround:** Don't enable read-only workloads for AHA clusters configured with only one data node.
