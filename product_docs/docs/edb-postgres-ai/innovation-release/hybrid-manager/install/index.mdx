...
description: Plan for, prepare for, install, and configure Hybrid Manager.
navigation:
- planning_arch
- sys_reqs
- k8s_install
...
* **Key Activities:**
    ...
    * Prepare to populate critical configuration entries for Hybrid Manager (either `values.yaml` for Bootstrap method or `HybridControlPlane` CR for Operator method).
...
* **Documentation:**
    * **[Operator installation (Recommended)](install_hm_operator.mdx)** - Uses `edb-hcp-operator` helm chart with `HybridControlPlane` Custom Resource
    * **[Bootstrap installation (Deprecated)](install_hm.mdx)** - Uses `edbpgai-bootstrap` helm chart with `values.yaml`
    * **[Migrating from Bootstrap to Operator](migrate_bootstrap_to_operator.mdx)** - Guide for existing Bootstrap users

!!! Warning
The **Bootstrap installation method** is deprecated and will be removed in a future release. **All new installations should use the [Operator method](install_hm_operator.mdx)**, which is the recommended and future-supported approach.

**Existing Bootstrap users:** If you want to migrate to the Operator method, follow the [Migration guide](migrate_bootstrap_to_operator.mdx). Note that you cannot install the operator on top of an existing bootstrap installation without first removing the bootstrap deployment.
!!!

!!! Note
**Installation method:** Both installation methods currently result in the same deployed platform. However, the Operator method provides better version management, Kubernetes-native resource handling, and is the strategic direction for Hybrid Manager installations.
!!!
...
1. [Configuring IdPs](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/config_idp_hm)
...
>>>>>>> dd10697fe5 (docs: address comments)
