...
* *A number of requirements validated as ready for use in Phase 4 [Preparing your environment](environment_prep) and initial inputs for the HM configuration — either a `values.yaml` file (Bootstrap method) or a `HybridControlPlane` Custom Resource manifest (Operator method, recommended).*
...
| **Bootstrap Tag** | `bootstrapImageTag` | HM/Platform version tag (for example, `"v2026.4.0"`). |
...
You now have a complete inventory and deployment strategy, as well as initial inputs for your HM configuration — either a `values.yaml` (Bootstrap method) or `HybridControlPlane` CR (Operator method). Continue building your configuration as you go, or use these inputs to assemble the file at the end of [Phase 4: Preparing your environment](environment_prep).
