...
!!! Note
The tables in this guide reference `values.yaml` parameter names. If you are using the **Operator method (recommended)**, see the [Configuration mapping reference](../configuration/migrate_bootstrap_to_operator#configuration-mapping-reference) for how these map to `HybridControlPlane` CR fields.
!!!
...
| :--- | :--- | :--- |
| **System flavor** | `system` | `eks`, `gke`, `aks`, `rke2`, or `rhos` |
| **OpenShift** | `beaconAgent.provisioning.openshift` | Set to `true` **only** if using Red Hat OpenShift. |
| **OpenShift ingress domain** | `parameters.global.default_ingress_domain` | Required only for `system: "rhos"` (query via `oc get ingresses.config/cluster -o jsonpath={.spec.domain}`). |
| **OpenShift console domain** | `parameters.upm-istio-gateway.openshift_console_domain_name` | Required only for OpenShift; used for console linking and UX. |
...
| :--- | :--- | :--- |
| **Enable LB** | `parameters.global.load_balancer_mode` | `"public", "private" or "disabled"` |
| **Provider** | `parameters.global.load_balancer_provider` | `aws`, `azure`, or `gcp` |
...
| Parameter | YAML Key | Value / Format |
| :--- | :--- | :--- |
| **Disable LB** | `parameters.global.load_balancer_mode` | `"disabled"` |
| **Domain** | `parameters.global.node_port_domain` | Postgres DNS domain (e.g., `pg.myorg.com`). |
...
| **Migration domain** | `parameters.global.dms_domain_name` | The internal domain for the Transporter migration service. Required unless the `migration` installation scenario isn't enabled. |
| **NodePort base** | `parameters.global.node_port_domain` | **(Scenario B Only)** The base domain for Postgres access if using NodePort. |
...
| :--- | :--- | :--- |
| **A. Cert Manager** | `parameters.global.portal_certificate_issuer_kind` | **Required.** The Resource Kind (`Issuer` or `ClusterIssuer`). |
| **A. Cert Manager** | `parameters.global.portal_certificate_issuer_name` | **Required.** The Resource Name (e.g., `letsencrypt-prod`). |
...
Collect these values for your HM configuration in the next phase (Phase 3). The table below uses `values.yaml` key names for the Bootstrap method. If you are using the **Operator method (recommended)**, see the [Configuration mapping reference](../configuration/migrate_bootstrap_to_operator#configuration-mapping-reference) for CR equivalents.

| Parameter | `values.yaml` Key | Value / Format |
| :--- | :--- | :--- |
| **Registry URL** | `containerRegistryURL` | `<Registry Domain>/pgai-platform` |
| **Pull Secret** | `imagePullSecrets` | Array of Kubernetes image pull secret names (for example, `["edb-cred"]`). |
...
| **TLS Validation** | `beaconAgent.provisioning.imagesetDiscoveryAllowInsecureRegistry` | Set to `true` only if intentionally allowing insecure (no/invalid TLS) registry discovery. |

!!! Note Bootstrap-specific values
The Bootstrap method additionally requires `bootstrapImageName` (e.g., `<Registry Domain>/pgai-platform/edbpgai-bootstrap/bootstrap-<K8s_Flavor>`) and `bootstrapImageTag` (HM version tag, e.g., `"v2026.4.0"`). These are not required for the Operator method, which uses `spec.imageRegistry` and `spec.version` instead.
!!!
...
| Parameter | YAML Key | Value / Format |
| :--- | :--- | :--- |
| **TDE Providers** | `beaconAgent.transparentDataEncryptionMethods` | List of enabled methods (e.g., `passphrase`, `aws_kms`, `azure_kms`, `gcp_kms`, `hashicorp_vault`). |
...
| Source | Requirement | CR Key | `values.yaml` Key |  Condition |
| :--- | :--- | :--- | :--- |
| **Phase 1** | **K8s Flavor** | `system` | **Required** (e.g., `azure`, `eks`, `gke`, `rhos`) |
...
| **Phase 1** | **Installation scenarios** | `spec.scenarios` | **Optional**. All scenarios are enabled by default. To customize, specify `core` plus a combination of `migration`, `ai`, or `analytics`. |

### Networking & ingress

| Source | Requirement | `values.yaml` Key | Condition |
| :--- | :--- | :--- | :--- |
...
| Strategy | `values.yaml` Key | Description |
| :--- | :--- | :--- |
