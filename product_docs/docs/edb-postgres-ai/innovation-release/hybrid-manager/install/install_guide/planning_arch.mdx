...
|---|---|---|
| **Role** | Orchestration, configuration, observability | Postgres cluster hosting and replication |
...
Use reference architectures A-D below as reference models to decide which topology matches your "[Target state](#architectural-discovery)."
...
* **Telemetry:** Collects metrics and logs. This service requires outbound network access to report health status.
  * *See:* [Monitoring with Hybrid Manager](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/monitoring/)
* **Database Migration Assistant:** Facilitates the movement of data from external sources into the platform.
  * *See:* [Migrating databases with Hybrid Manager](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/migration/)
...
* **Federation:** Manages secure communication and authorization across multiple HM instances in a Multi-Location topology.
  * *See:* [Configuring multiple data centers for Hybrid Manager](/edb-postgres-ai/current/hybrid-manager/install/configuration/multi_dc/)
...
3.  Connectivity is established using load-balanced endpoints&mdash;not a network mesh service, like Submariner for example.
...
![CP star topology](../../images/HCP_control-planes.png)
...
Traffic follows a strict directional pattern. **Control flows outward** from the Primary to each Secondary — configuration, orchestration commands, and lifecycle operations all originate at the hub. **Logs flow inward**, returning from each Secondary back to the Primary. Secondaries have no lateral visibility: they cannot control one another, and they cannot self-govern. All authority is centralized in the Primary.
...
![DP optional mesh topology](../../images/HCP_data-planes.png)
 
The Data Plane topology is **optional mesh**: any two Data Plane locations may be left entirely isolated, or they may be directly interconnected via VPC/VNet peering for Postgres-level replication. There is no hub, no mandatory center, and no required connectivity between locations.
...
Data Planes that are not peered remain fully independent — separate security domains, separate failure domains, separate upgrade timelines. Peering is additive: it introduces a replication path without changing the isolation characteristics of either plane for any other purpose.
...
!!!note
Scenarios are intended for advanced production deployment planning. For pilots, proof of concepts (PoCs), and initial evaluations, EDB recommends a full installation to maintain maximum optionality and ensure all integrated capabilities are available for testing. If this is the case, omit the `scenarios` configuration parameter, as this enables all available scenarios by default.
!!!
...
!!! Important
Currently, modifying installation scenarios after deployment isn't supported. Ensure you install all required capabilities, as you cannot add or remove them once HM is deployed.
!!!
...
There is an example for each installation type.
...
metadata:
  name: edbpgai
spec:
  flavour: <Kubernetes_Flavor> # e.g., rhos, rke2, aks, eks, gke
...
```yaml
system: <Kubernetes_Flavor> # e.g., rhos, rke2, aks, eks, gke
