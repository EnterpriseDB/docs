...
From 2026.9 onwards, the Distributed High Availability Database clusters management capabilities are controlled by a dedicated `pgd` scenario, separated from the `dbaas` scenario. If your scenarios list do not includes `pgd`, existing installations upgrading from an earlier release *must* explicitly add `pgd` to their scenarios list to retain the Distributed High Availability clusters. Add it once the operator is on 2.1 or later, and before you bump `spec.version` — earlier operator versions don't recognize `pgd` and reject the change.

If you don't add `pgd` to your scenario list, Distributed High Availability clusters you deployed are removed from your environment, along with the data clusters hold. For details, see [Scenario changes on upgrade](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/upgrading_hm/#scenario-changes-on-upgrade).
!!!
