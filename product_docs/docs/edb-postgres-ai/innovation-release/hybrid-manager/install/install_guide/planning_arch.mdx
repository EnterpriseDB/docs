...
!!!important
Add pgd to every location that hosts, or will host, DHA clusters — including secondary locations. If you don't add pgd to a location's scenario list, its existing Distributed High Availability clusters will be deleted (and new ones will not be allowed to be created).
!!!
...
**Default behavior:** If the `scenarios` parameter is omitted, the operator applies a default set, and this depends on your operator version. Operator 2.1 installs `core`, `dbaas`, `ai`, `analytics`, `migration`, and `marketplace`. Operator 2.2 or later installs only `core`, so the set of scenarios is always an explicit choice. List the scenarios you want explicitly.
