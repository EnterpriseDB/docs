...
**Default behavior:** If the `scenarios` parameter is omitted, the operator applies a default set — on operator 2.1 that's `core`, `dbaas`, `ai`, `analytics`, `migration`, and `marketplace`. This depends on your operator version, so list the scenarios you want explicitly.
