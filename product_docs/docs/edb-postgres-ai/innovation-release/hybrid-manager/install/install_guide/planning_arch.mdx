...
HM and Kubernetes have a 1:1 relationship: you can't run more than one HM deployment on the same Kubernetes cluster.

A dedicated Kubernetes cluster isn't required, but it's recommended, especially while you're gaining initial experience with HM. HM can coexist with other Kubernetes workloads. Istio is the most common point of conflict, and while conflicts are manageable, they require more integration effort.
...
-   **SUSE Rancher RKE2** (Rancher Kubernetes Engine)
...
| **Scenarios**                         | core, migration, ai, marketplace                                                                                                        | dbaas, pgd, analytics, ai                                                                                    |
...
-   **GenAI:** Provides the AI/ML capabilities. If enabled, this component dictates the need for GPU-enabled worker nodes in your **[system requirements](./sys_reqs)**.
    -   *See:* [GenAI in HM](/edb-postgres-ai/1.3/hybrid-manager/ai-factory/gen-ai/)
...
-   **CloudNativePG distributed** — streaming replication across Kubernetes clusters and failure domains using CloudNativePG's distributed topology. See the [CloudNativePG distributed documentation](https://cloudnative-pg.io/docs/1.30/replica_cluster).
...
Scenarios are intended for advanced production deployment planning. For pilots, proof of concepts (PoCs), and initial evaluations, we recommend a full installation to maintain maximum optionality and ensure all integrated capabilities are available for testing. Omitting the `scenarios` configuration parameter applies the operator's default set instead, and that set depends on your operator version: operator 2.1 installs the full set, while operator 2.2 or later installs only `core`. Rather than relying on it, list the scenarios you want explicitly so your configuration retains the same capabilities in later releases.
!!!
...
| `ai`          | Tools for building and serving generative AI.             | Sovereign AI, model serving (kserve), Agent Studio, and GenAI builders.                                |
| `dbaas`       | Database clusters management.                             | Postgres cluster management, including provisioning, scaling, and updates.                             |
| `pgd`         | Distributed High Availability Database clusters management.| Distributed High Availability Postgres cluster management, including provisioning, scaling, and updates. |
| `marketplace` | The app marketplace and the apps it deploys.              | kapp-controller and the [OSS Library](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/hm_overview/oss_library_apps/) apps, such as Apache Superset, Apache Airflow, Metabase, pgAdmin 4, pgBadger, and HammerDB. |
...
From 2026.8 onwards, the app marketplace capabilities are controlled by a dedicated `marketplace` scenario, separated from the `core` scenario. If your scenarios list includes `ai`, `marketplace` is added for you as a dependency. Otherwise, existing installations upgrading from an earlier release *must* explicitly add `marketplace` to their scenarios list to retain the app marketplace. Add it once the operator is on 2.1 or later, and before you bump `spec.version` — earlier operator versions don't recognize `marketplace` and reject the change.

If you don't add `marketplace` to your scenario list, kapp-controller and any OSS Library apps you deployed are removed from your environment, along with the data those apps hold. For details, see [Scenario changes on upgrade](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/upgrading_hm/#scenario-changes-on-upgrade).
!!!

!!!Note

From 2026.9 onwards, Distributed High Availability (DHA) cluster management is controlled by a dedicated `pgd` scenario, separated from the `dbaas` scenario. No other scenario adds `pgd` automatically, so existing installations upgrading from an earlier release must explicitly add `pgd` to their scenarios list to retain their DHA clusters. Add it once the operator is on 2.2 or later, and before you bump `spec.version`. Earlier operator versions don't recognize `pgd` and reject the change.

If you don't add `pgd` to your scenario list, Distributed High Availability clusters you deployed are removed from your environment, along with the data those clusters hold. For details, see [Scenario changes on upgrade](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/upgrading_hm/#scenario-changes-on-upgrade).
!!!
...
**Default behavior:** If the `scenarios` parameter is omitted, the operator applies a default set, and that set depends on your operator version. Operator 2.1 installs `core`, `dbaas`, `ai`, `analytics`, `migration`, and `marketplace`. Operator 2.2 or later — which 2026.9 requires — installs only `core`, so the set of scenarios is always an explicit choice. List the scenarios you want explicitly.

**Scenario dependencies:**
- The `ai` scenario depends on `marketplace`. If you select `ai`, `marketplace` is installed with it, whether or not you list it explicitly.
- The `pgd` scenario depends on `dbaas`. If you select `pgd`, `dbaas` is installed with it, whether or not you list it explicitly.
...
| **Installation scenarios** | `spec.scenarios` (YAML list)                              | `core`, `migration`, `ai`, `analytics`, `dbaas`, `pgd`, `marketplace` |
...
spec:
  ...
  version: <Version>
  registry:
    url: <Container_Registry_Domain>/pgai-platform
  scenarios: # list scenarios explicitly rather than relying on the operator default
    - core
    - migration   # remove if not needed
    - ai          # remove if not needed
    - analytics   # remove if not needed
    - dbaas       # remove if not needed
    - pgd         # required for Distributed High Availability (DHA) clusters
    - marketplace # remove if not needed
