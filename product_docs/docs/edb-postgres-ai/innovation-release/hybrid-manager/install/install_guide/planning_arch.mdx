...
From 2026.9 onwards, Distributed High Availability (PGD) cluster management is controlled by a dedicated `pgd` scenario, separated from the `dbaas` scenario. No other scenario adds `pgd` automatically, so existing installations upgrading from an earlier release must explicitly add `pgd` to their scenarios list to retain their Distributed High Availability clusters. Add it once the operator is on 2.2 or later, and before you bump `spec.version`. Earlier operator versions don't recognize `pgd` and reject the change.
