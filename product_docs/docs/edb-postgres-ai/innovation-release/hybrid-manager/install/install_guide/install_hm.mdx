...
**`spec.scenarios`**: Optional. Ensure you have listed the installation scenarios you require (options are `core`, `migration`, `ai`, `dbaas`, `analytics` and `klio`). However, if you don't have this option in your `values.yaml`, all scenarios except `klio` are installed by default.

!!! NOTE
`klio` is currently in preview and not yet stable enough for production use. It is not included in the default installation and must be explicitly added to
the scenarios list to enable it.
!!!
