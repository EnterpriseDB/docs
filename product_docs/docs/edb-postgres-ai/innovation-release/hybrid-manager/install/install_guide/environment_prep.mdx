...
* *A finalized and validated configuration ready for Hybrid Manager (HM) installation&mdash;either a `values.yaml` file if using the legacy "Bootstrap" method or a `HybridControlPlane` Custom Resource manifest if using the recommended Operator method*
...
1. Assign a unique folder name (e.g., using a UUID generator) and provide it in your `values.yaml` under `parameters.global.internal_backup_folder`. 
...
The following component-specific parameters should be added to your `values.yaml` under `parameters`if using the operator method:
...
!!! Warning
The **Bootstrap method** (`values.yaml` with `edbpgai-bootstrap` chart) is deprecated and is to be removed in a future release. New installations should use the **Operator method** with the `HybridControlPlane` Custom Resource. See [Phase 5: Installing Hybrid Manager](./install_hm) for details.
!!!
