...
=======
-   *Initial inputs for the HM configuration — either a `values.yaml` file (Bootstrap method) or a `HybridControlPlane` Custom Resource manifest (Operator method, recommended).*
...
The **SRE/Admin** builds on these inputs, either recording them or continuing/beginning the configuration file in [Phase 2: Gathering your system requirements](/edb-postgres-ai/current/hybrid-manager/install/sys_reqs/) and/or uses these specs to build the configuration file in **[Phase 4: Preparing your Environment](environment_prep)**.

!!! Note
Hybrid Manager supports two installation methods. The information you gather in Phases 1–2 is the same regardless of which method you choose — only the final configuration format differs:

- **Operator method (recommended):** Uses a `HybridControlPlane` Custom Resource manifest with the `edb-hcp-operator` helm chart. See [Installing with Operator](./install_hm_operator) for the full field mapping.
- **Bootstrap method (deprecated):** Uses a `values.yaml` file with the `edbpgai-bootstrap` helm chart.

The tables below reference `values.yaml` parameter names. If you are using the Operator method, see the [Configuration mapping reference](./install_hm_operator#configuration-mapping-reference) for how these map to `HybridControlPlane` CR fields.
!!!
...
| :--- | :--- | :--- |
| **Kubernetes Platform** | `system` | `aks`, `eks`, `gke`, `rhos` |
| **Target location** | `parameters.upm-beacon.beacon_location_id` | `aws-us-east-1` |
| **OpenShift mode** | `beaconAgent.provisioning.openshift` | `true` (only for RHOS/ROSA) |

!!!note Version tag formatting
When executing the installation, note that the Helm `--version` flag uses the raw numeric format or `<helm-version>` (e.g., `2026.3.0`), whereas the `bootstrapImageTag` requires an `<image-tag-version>` with a `v` prefix (e.g., `v2026.3.0`).
!!!
=======
| Architecture decision   | Config parameter (`values.yaml`)           | HybridControlPlane CR field | Example value               |
| :---------------------- | :----------------------------------------- | :-------------------------- | :-------------------------- |
| **Kubernetes Platform** | `system`                                   | `spec.flavour`              | `aks`, `eks`, `gke`, `rhos` |
| **Target location**     | `parameters.upm-beacon.beacon_location_id` | `spec.componentsParameters.upm-beacon.beacon_location_id` | `aws-us-east-1` |
| **Provisioning mode**   | `beaconAgent.provisioning.provider`        | `spec.beaconAgent.provisioning.provider` | `azure`, `aws`, or `gcp` |
>>>>>>> ec912a7f10 (docs: address comments)

### Impact on configuration file

Here is how your decisions map to the final configuration file structure. Examples are shown for both installation methods.
