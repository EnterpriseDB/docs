...
!!!note 
Required only for deployments where the `ai` installation scenario is enabled. This scenario is included by default unless it is manually excluded via the `spec.scenarios` parameters in `values.yaml`.
!!!
