---
title: Enabling API audit logging
...
Before enabling API audit logging, ensure you have:

- `kubectl` access to your cluster
- Sufficient permissions to edit the HybridControlPlane custom resource (Operator installation) or the `values.yaml` file (Bootstrap installation)
...
- **Operator installation** — You installed HM using the HybridControlPlane custom resource and `kubectl`.
- **Bootstrap installation** — You installed HM using Helm and a `values.yaml` file.

### Operator installation

Edit your HybridControlPlane custom resource to add or update the `enable_api_audit_logging` setting under `spec.componentsParameters.upm-istio-gateway`:
...
To disable API audit logging, set `enable_api_audit` to `"false"` and reapply using the same steps for your installation type.
...
Use the following [LogQL](https://grafana.com/docs/loki/latest/query/) query to filter audit entries:
...
If you don't have a log aggregation tool available, you can stream logs directly from the ingress gateway pod using `kubectl`:
