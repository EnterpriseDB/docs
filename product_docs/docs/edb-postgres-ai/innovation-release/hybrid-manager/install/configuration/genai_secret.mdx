...
The `edbctl` CLI can create this secret automatically as part of the `ai` scenario. The Fernet key is generated for you:

```shell
edbctl setup create-install-secrets --version <version> --scenario ai
```

For non-interactive environments (CI/CD), add the `--non-interactive` flag. The Fernet key is always auto-generated:
