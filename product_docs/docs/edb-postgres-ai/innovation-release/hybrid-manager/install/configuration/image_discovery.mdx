...
If you are using a standard OCI registry (like Harbor, Quay, or Artifactory), ensure the username or password you provided when creating your [**Image Pull Secret**](/edb-postgres-ai/current/hybrid-manager/install/environment_prep/#create-image-pull-secret) has `read` and `list` permissions on the target repositories.

Refer to your registry's documentation (For example: [Quay.io Permissions](https://quay.io/)) to configure the service account.
