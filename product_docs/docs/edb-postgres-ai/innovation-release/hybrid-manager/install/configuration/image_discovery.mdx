...
This option specifies the container registry provider for the registry specified using `imagesetDiscoveryContainerRegistryURL`. It defaults to `oci`.
...
If you are using a standard OCI registry (like Harbor, Quay, or Artifactory), ensure the username/password you provided when creating your [**Image Pull Secret*](/edb-postgres-ai/current/hybrid-manager/install/environment_prep/#create-image-pull-secret)* has `read` and `list` permissions on the target repositories.
...
In your `values.yaml`, confirm the `image_discovery_credentials_secret_name`. 
