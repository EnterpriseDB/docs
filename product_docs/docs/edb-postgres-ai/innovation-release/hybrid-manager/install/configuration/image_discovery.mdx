...
-   `spec.registry.url` and `spec.registry.imagePullSecrets` for pulling the HM images
-   `spec.registry.tls` for the trust used to reach the registry
-   `spec.registry.discovery` for image discovery itself
