...
!!!important
This excludes the recovery of the HM installation itself, which you must [back up and restore separately](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/hadr/hadr_recovery/).
!!!
...
- Minimize downtime (RTO) &mdash; Leverage multiple, geographically distributed, write endpoints (with DHA Postgres clusters) or failover quickly (with single-node or HA Postgres clusters) to rapidly restore services during maintenance or unplanned site outages.
...
!!!important Keep all locations on the same HM version
The Beacon server on the primary and the Beacon agents on the secondaries must run the same HM version to communicate reliably and provide full feature functionality. In a multi-DC deployment, every location must therefore run the same HM version. When you upgrade, upgrade the primary location first and then upgrade each data-only secondary location immediately after, so that all locations return to the same version. See [Upgrading a multi-DC deployment](/edb-postgres-ai/current/hybrid-manager/install/configuration/multi_dc/multi-dc/#upgrading-a-multi-dc-deployment).
!!!
...
-   **Three HMs for PGD:** For optimal PGD deployments (Distributed High Availability Postgres clusters), especially with two data groups and a witness group, we recommend a three-location topology. This configuration ensures quorum for proper distributed consensus.
...
- You must configure [automated backup schedules](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/hadr/backup_considerations/#backup-readiness) for each location individually, settings don't sync automatically between sites.

- Your organization knows how to [restore a database using the CLI](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/hadr/hadr_recovery/) if the HM console is down due to a primary site's outage.
