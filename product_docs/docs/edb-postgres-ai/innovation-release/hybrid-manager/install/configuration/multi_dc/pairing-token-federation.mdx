...
Both methods are fully supported, and a deployment uses one or the other. You choose at install time by deciding whether the locations share a bucket.
...
- A version of `edbctl` that provides the `edbctl federation` commands.

<!-- TODO(UPM-111313): replace the line above with a concrete minimum version, for example
     "edbctl 1.9.0 or later", once UPM-129829 un-hides the federation command group and that
     build ships. The commands are present in released 1.8.0 and 1.8.1 but are hidden, so they
     don't appear in `edbctl --help` and can't be referred to as publicly available yet. -->
