...
-   [**`edbctl` CLI tool**](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/edbctl/) installed and configured.

-   A valid [**EDB Repos 2.0 Token**](https://www.enterprisedb.com/docs/repos/getting_started/).
...
Edit your `HybridControlPlane` CR to set `spec.registry.url` to your private registry URI:
...
spec:
  registry:
    url: "<your_local_container_registry_uri>"
    # ... your other registry settings
  # ... your other spec fields
```

!!! Note
`spec.registry` gathers all of the container registry settings of a `HybridControlPlane` in one place and requires HM operator 2.2.0 or later. The older `spec.imageRegistry` field is deprecated: it's still honored when `spec.registry.url` is unset, but it will be removed in a future API version. See [Configuring image discovery](./image_discovery#deprecated-registry-fields) for the full list of fields it replaces.
!!!
...
**Return to [Phase 4: Preparing your Environment](../install_guide/environment_prep) or [Phase 5: Installing Hybrid Manager](../install_guide/install_hm).**
