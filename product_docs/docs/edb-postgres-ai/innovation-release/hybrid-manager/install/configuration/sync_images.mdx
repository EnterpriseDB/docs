...
### Sync Operator image (deprecated)

!!! Note Deprecated
The `edbctl operator sync-to-local-registry` command is **deprecated**. The [Sync platform images](#sync-platform-images) step above already mirrors the operator manager image, bundle, and helm chart, so a separate operator-sync step is no longer required. The command remains available for backward compatibility with existing automation and will be removed in a future release.
!!!

If you have existing automation that calls the deprecated command, it continues to work in this release:
...
```yaml
containerRegistryURL: "<your_local_container_registry_uri>"
```
