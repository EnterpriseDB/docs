...
1. Select the configuration for your provider from the sections below and fill in the required values.
...
This section applies to the [S3-compatible storage](#other-s3-compatible-storage) configuration. If your endpoint uses a certificate signed by a private or self-signed certificate authority (CA), generate the CA bundle value here and add it to the `aws_ca_bundle` or `aws_ca_bundle_base64` field in your secret. Two fields are supported — use one or the other, not both:
...
This section applies to the [GCP configuration](#gcp-object-storage). The `credentials_json_base64` field requires your GCP service account JSON key file encoded as a base64 string. Use the following commands to generate that value before adding it to the secret.
...
By default, all projects share the object storage configured during installation. You can configure dedicated object storage for individual projects at any time after installation, before creating any Postgres workloads in that project.
