...
You can create secrets for Migration Portal using the `edbctl` CLI if you have enabled the `migration` scenario for your installation (`migration` is enabled by default if not specified otherwise explicitly). Secure passwords are generated for each service account.
...
This creates the namespaces, secrets, and generates secure random passwords for all five service accounts described in [Understanding Migration Portal secrets](#understanding-migration-portal-secrets).
