...
!!!note
Required only for deployments with the `migration` installation scenario enabled. This scenario is included by default unless it is manually excluded via the `scenarios` parameter in `values.yaml`.
!!!
