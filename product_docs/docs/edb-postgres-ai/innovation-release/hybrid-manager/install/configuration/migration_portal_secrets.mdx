...
deepToC: true
redirects:
- /edb-postgres-ai/latest/hybrid-manager/install/customization/mirgration_portal_secrets/
---

By default, Hybrid Manager (HM) includes pre-configured secrets to facilitate a quick, out-of-the-box installation. However, these default secrets are not intended for production use. For any live environment, you must override these defaults with custom secrets to ensure your installation remains secure and unique.
...
!!!note
Required only for deployments with the `migration` installation scenario enabled. This scenario is included by default unless it is manually excluded via the `spec.scenarios` parameter in `values.yaml`.
!!!
...
You can create secrets for Migration Portal using the `edbctl` CLI if you have enabled the `migration` scenario for your installation. The HM installation instructions already include instructions on how to use `edbctl` CLI to [customize your Migration Portal secrets](/edb-postgres-ai/current/hybrid-manager/install/environment_prep/#customizing-migration-portal-secrets).
