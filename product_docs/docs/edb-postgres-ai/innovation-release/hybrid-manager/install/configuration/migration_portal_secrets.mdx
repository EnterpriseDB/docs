...
The `edbctl` CLI can create all Migration Portal secrets automatically as part of the `migration` scenario. Secure passwords are generated for each service account:

```shell
edbctl setup create-install-secrets --version <version> --scenario migration
```

For non-interactive environments (CI/CD), add the `--non-interactive` flag. All passwords are auto-generated:
