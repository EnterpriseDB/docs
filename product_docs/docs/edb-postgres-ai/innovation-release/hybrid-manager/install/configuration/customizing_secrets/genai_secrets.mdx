...
!!!note
Required for deployments with the `ai` installation scenario enabled. This scenario is included by default unless it is manually excluded via the scenarios configuration (`spec.scenarios` in the `HybridControlPlane` CR or `scenarios` in `values.yaml`).
!!!
