---
title: Customizing Lakehouse/Catalog secrets
navTitle: Customizing Lakehouse/Catalog secrets
description: Learn how to customize Lakehouse/Catalog secrets in your Hybrid Manager installation.
deepToC: true
---
...
!!!note
Required for deployments with the `analytics` installation scenario enabled. This scenario is included by default unless it is manually excluded via the scenarios configuration (`spec.scenarios` in the `HybridControlPlane` CR or `scenarios` in `values.yaml`).
!!!
