...
**Passphrase mode** uses a passphrase you supply to encrypt each cluster's TDE key. This passphrase is the key-encryption key (KEK) for TDE — it's unrelated to any database user's SQL password, and confusing the two is a common source of misconfiguration. Passphrase mode is quick to set up and suitable for development and testing, but isn't appropriate for production:
...
!!!note
The `passphrase` option is included for convenience in development and testing environments only. For production installs, pin `transparentDataEncryptionMethods` to KMS providers only (`aws_kms`, `azure_kms`, `gcp_kms`, `hashicorp_vault`) and omit `passphrase` from the list entirely. This closes the door at install time, so passphrase mode can't be selected downstream from any template, YAML, or CLI invocation.
!!!
