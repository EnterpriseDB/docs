...
1.  On the Console, go to the project under which you'll create TDE-enabled clusters.
    Each cluster is assigned to a specific key at creation time. You can register multiple keys in a project — see [Deciding on a key-per-cluster or key-per-project strategy](/edb-postgres-ai/current/hybrid-manager/install/configuration/kms_for_tde/#deciding-on-a-key-per-cluster-or-key-per-project-strategy) for guidance on choosing one key per project versus one key per cluster.
...
- Enable [soft-delete and purge protection](https://learn.microsoft.com/en-us/azure/key-vault/general/soft-delete-overview) on the key vault so a key can't be permanently destroyed immediately by mistake.
- For disaster recovery across regions, plan for [key vault backup and restore](https://learn.microsoft.com/en-us/azure/key-vault/general/backup) or a replicated vault in your DR region.
