...
A wildcard binding means every cluster in every namespace can reach every key, which defeats the per-cluster key isolation described in [Deciding on a key-per-cluster or key-per-project strategy](/edb-postgres-ai/current/hybrid-manager/install/configuration/kms_for_tde/#deciding-on-a-key-per-cluster-or-key-per-project-strategy).
