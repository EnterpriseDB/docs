...
!!!important
This walkthrough installs Vault inside the same Kubernetes cluster as Hybrid Manager and uses a wildcard policy (`transit/*`) and a wildcard service-account binding (`p-*`). This is a **bring-up example** to get you to a working TDE-enabled cluster quickly — it isn't a production reference architecture. Before using Vault-backed TDE in production, see [Production hardening](#production-hardening).
!!!
...
**Least-privilege policy.** Replace the wildcard policy from step 5 with per-key paths scoped to the `update` capability only:
...
**Scoped role bindings.** Replace the wildcard `bound_service_account_namespaces="p-*"` from step 9 with the specific namespace of the cluster that key is meant to serve:
...
A wildcard binding means every cluster in every namespace can reach every key, which defeats the per-cluster key isolation described in [Key-to-cluster model](index.mdx#key-to-cluster-model).
