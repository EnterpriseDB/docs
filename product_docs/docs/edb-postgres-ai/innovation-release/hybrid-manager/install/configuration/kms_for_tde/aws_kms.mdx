---
title: Adding an AWS KMS key
description: Learn about using AWS Key Management Service (KMS) with Hybrid Manager.
redirects:
- /edb-postgres-ai/latest/hybrid-manager/install/customization/kms_for_tde/aws_kms/
---
...
Ensure you have set the parameter for AWS KMS in the `HybridControlPlane` CR of the Hybrid Manager. See [Adding KMS support](adding_kms_support.mdx) to enable it.  
...
For production deployments, harden your AWS KMS configuration before relying on it for TDE-enabled clusters:

- Scope the key policy to the specific IAM role or user used by each cluster's workload identity, rather than a shared principal used by every cluster.
- Enable [key deletion protection](https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html) (the mandatory waiting period) so a key can't be destroyed immediately by mistake.
- For disaster recovery across regions, consider [multi-Region keys](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html) so a replica key is available if you need to restore in another region.
