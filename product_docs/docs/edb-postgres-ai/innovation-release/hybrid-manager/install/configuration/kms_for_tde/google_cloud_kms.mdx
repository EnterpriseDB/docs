---
title: Adding a Google Cloud KMS key
description: Use Google Cloud Key Management (KMS) with Hybrid Manager.
redirects:
- /edb-postgres-ai/latest/hybrid-manager/install/customization/kms_for_tde/google_cloud_kms
---
...
Ensure you have set the parameter for Google Cloud KMS in the `HybridControlPlane` CR of the Hybrid Manager. See [Adding KMS support](adding_kms_support.mdx) to enable it.  
...
For production deployments, harden your Google Cloud KMS configuration before relying on it for TDE-enabled clusters:

- Grant the Cloud KMS CryptoKey Encrypter/Decrypter role to the specific service account used by each cluster's workload identity, not a shared account used across clusters.
- Review the key's [scheduled destruction duration](https://cloud.google.com/kms/docs/destroy-restore) so accidental deletion isn't immediate, and set up a [key rotation schedule](https://cloud.google.com/kms/docs/key-rotation).
- For disaster recovery across regions, review [multi-region or dual-region key rings](https://cloud.google.com/kms/docs/locations) so a key is available if you need to restore in another region.
