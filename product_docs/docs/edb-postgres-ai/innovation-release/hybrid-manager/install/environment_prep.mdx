...
* **Creating other necessary secrets**
  ...
  * [Creating **GenAI Builder** secret](#creating-genai-builder-secrets) - requires Fernet secret and NGC key secrets.
  * [Creating **Catalog** secret](#creating-catalog-secret) - requires confounding key secret.
  * [Customizing secrets for the **Migration Portal**](#creating-catalog-secret) - requires secrets for service accounts.
...
Your object storage must be dedicated entirely to the HM. 
...
- If using the `scenarios` parameter: These preparatory steps are mandatory only if your selected `scenarios` require them. If a required secret is missing for a chosen scenario, the installation will fail.

- If NOT using the scenarios parameter: By default, all scenarios are enabled. In this case, all secrets are required to successfully complete the installation.
...
1.  You can create a Fernet key secret and NGC key secrets using the `edbctl` CLI.
    ...
    - For automated pipelines (CI/CD), use the `--non-interactive` flag to suppress all confirmation prompts and allow the command to complete without human intervention. You will need to make the NGC API key you create above available as a variable.
      ...
      edbctl setup create-install-secrets --version <version> --scenario ai --non-interactive
      ```
...
1.  The Hybrid Manager administrator must keep the Fernet key safe and back it up.

    Fetch the key: 

    ```shell
    kubectl get secret -n default fernet-secret -o yaml
    ```

    [is default the correct namespace?]: #
...
1.  You can create a confounding key secret with the `edbctl` CLI for environments with the `analytics` scenario enabled. 
    ...
    - For automated pipelines (CI/CD), use the `--non-interactive` flag to suppress all confirmation prompts and allow the command to complete without human intervention:
...
1.  The Hybrid Manager administrator must keep the confounding key safe and back it up. 
    ...
    [was this also moved to `default` namespace?]: #
...
- For automated pipelines (CI/CD), use the `--non-interactive` flag to suppress all confirmation prompts and allow the command to complete without human intervention. All passwords are auto-generated:
  
  ```shell
  edbctl setup create-install-secrets --version <version> --scenario migration --non-interactive
  ```
...
* **SOP: [Set up a custom cert-manager issuer for the HM Portal](/edb-postgres-ai/current/hybrid-manager/install/configuration/cert-man/#set-up-a-custom-cert-manager-issuer-for-the-hm-portal)**
...
* **SOP: [Bring your own private certificate authority](/edb-postgres-ai/current/hybrid-manager/install/configuration/cert-man/#bring-your-own-private-certificate-authority).**
...
* **SOP: [Set up a custom x.509 certificate for the Hybrid Manager Portal](/edb-postgres-ai/current/hybrid-manager/install/configuration/cert-man/#set-up-a-custom-x509-certificate-for-the-hybrid-manager-portal)**
