...
* *A finalized and validated configuration ready for Hybrid Manager (HM) installation - either a `values.yaml` file (Bootstrap method) or a `HybridControlPlane` Custom Resource manifest (Operator method)*
...
For more CLI options, see [edbctl setup](/edbctl/latest/using/edbctl_setup/). To customize your component's secrets, see [Customizing secrets](/edb-postgres-ai/current/hybrid-manager/install/configuration/customizing_secrets/).
...
[Implementing TLS certificates](/edb-postgres-ai/latest/hybrid-manager/install/configuration/cert-man/) describes several implementation options.
...
>>>>>>> f8257e6f22 (docs(UPM-73992): add operator installation)

You now construct (or update if you have previously created) the final configuration for your HM installation.

This configuration references your **system requirements** (Storage, Network, etc), **environment settings**, and the **feature configurations** you just prepared.
...
2. **Bootstrap method (values.yaml) - Deprecated** - Helm values file used with the `edbpgai-bootstrap` chart

!!! Warning Bootstrap Method Deprecation
...
!!! Note
Both methods use the same configuration values, just in different formats. The secrets and preparation steps are identical for both approaches.
!!!

### Bootstrap method: values.yaml template

Below is a template configuration with all of the keys you should have determined for a successful HM install using the **Bootstrap method**:
...
### Operator method: HybridControlPlane Custom Resource (Recommended)

If you're using the **Operator method (recommended)**, you'll create a `HybridControlPlane` Custom Resource instead of a `values.yaml` file.

The configuration values are the same, but organized in a Kubernetes Custom Resource format. See the [Install with Operator](./install_hm_operator#example-hybridcontrolplane-manifest) guide for a complete HybridControlPlane manifest template and field mapping reference.

!!! Success Recommended Approach
The Operator method is the recommended approach for all new installations. It provides better version management, Kubernetes-native resource handling, and is the future direction for Hybrid Manager deployments.
!!!

!!! Note
The secrets you created in this phase (Image Pull, Object Storage, TLS, GenAI, Catalog, Migration Portal) are used by both installation methods. The only difference is how you specify the HM platform configuration.
!!!
