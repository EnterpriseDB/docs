...
**Supported distributions:**
  * Cloud service provider platforms:
    ...
    * Amazon EKS
    * Google GKE
    * Red Hat OpenShift (RHOS) on AWS (IR 2026.2 and later IR versions)
...
| **Migration Domain** | `parameters.transporter-rw-service:domain_name` | The internal domain for the Transporter migration service. Required unless the `migration` installation scenario isn't enabled. |
| **Migration URL** | `parameters.transporter-dp-agent:rw_service_url` | The full URL for the service: `https://<Migration Domain>/transporter`. Required unless the `migration` installation scenario isn't enabled. |
