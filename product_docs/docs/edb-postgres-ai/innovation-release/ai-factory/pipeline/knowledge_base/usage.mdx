...
The `server_name` comes from calling PGFS functions to create a storage location [pgfs.create_storage_location](/pg_extensions/pgfs/usage). The `path` is the path to the data in the storage location. The last argument indicates the data format; `Text` or `Image`.
