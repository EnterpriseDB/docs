...
See [volumes](/edb-postgres-ai/current/ai-factory/pipeline/volumes) for more detail.

Here, we create a [PGFS storage location](/edb-postgres-ai/current/ai-factory/pipeline/pgfs) for a public S3 bucket:
```sql
...
The [aidb.create_volume](/edb-postgres-ai/current/ai-factory/pipeline/reference/knowledge_bases#aidbcreate_volume) function is used to create a volume.
...
A view is available that lists all the knowledge_bases. [aidb.knowledge_bases](/edb-postgres-ai/current/ai-factory/pipeline/reference/knowledge_bases#aidbknowledge_bases) also includes some of the knowledge bases configuration:
