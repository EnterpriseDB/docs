...
The next parameter is the configuration. Use the `aidb.embeddings_config()` helper function to set the model and url:
...
The next parameter to `embeddings_config()` is the important one, overriding the endpoint that the aidb model will use. In this example:
