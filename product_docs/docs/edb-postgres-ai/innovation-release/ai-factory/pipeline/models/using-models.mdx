...
------------------------+------------------------------------------------------------------------------------------------------+----------------
 ...
 openrouter_chat        | Chat/completions via OpenRouter, providing access to 200+ models.                                    |
 openrouter_embeddings  | Embeddings via OpenRouter, providing access to multiple embedding models.                            |
...
You can pass options to the model when creating it. For example, you can specify the model configuration using the appropriate configuration helper function:
...
This command creates a model named `my_model` that uses the `bert_local` model provider and the `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2` model from HuggingFace. The `revision` option specifies the version of the model to use. Configuration helper functions like `aidb.bert_config()` simplify creating the JSONB configuration object with proper parameter types and defaults.
...
This command creates an embeddings model with the name `my_embeddings_model`. You can then use this model in your Pipelines functions to generate embeddings for text data.
