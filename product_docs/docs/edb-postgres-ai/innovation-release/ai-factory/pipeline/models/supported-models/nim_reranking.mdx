...
SELECT aidb.create_model(
    ...
    aidb.nim_reranking_config(
        ...
        url => 'http://nim-nv-rerankqa-llama-l-1xgpu-g6-predictor.default.svc.cluster.local/v1/ranking'
    )
);
```
...
SELECT aidb.create_model(
    ...
    jsonb_build_object(
        ...
        'tls_config', jsonb_build_object('insecure_skip_verify', true)
    )
);
```
...
SELECT aidb.create_model(
    ...
    jsonb_build_object(
        ...
        'tls_config', jsonb_build_object('ca_path', '/etc/aidb/myCA.pem')
    )
);
```
