...
You can pass an optional `inference_config` parameter to control model behavior at runtime. Use the `aidb.inference_config()` helper to create the configuration:
...
For batch inferencing with custom parameters:
...
See [inference_config reference](/edb-postgres-ai/current/ai-factory/pipeline/reference/models/#aidbinference_config) for all available parameters.
