...
You can pass an optional `inference_config` parameter to control model behavior at runtime. Use the `aidb.inference_config()` helper function to define your configuration.

Specify parameters like `system_prompt` or `temperature` to guide the model's output:

```sql
SELECT * FROM aidb.decode_text(
    'llama',
...
For larger workloads, apply custom parameters to a batch of inputs simultaneously using `aidb.decode_text_batch()`:
...
SELECT * FROM aidb.decode_text_batch(
    ...
    ARRAY[
        ...
        'translate to german: goodbye world'
    ],
    aidb.inference_config(
        temperature => 0.3,
        max_tokens => 50,
...
For a complete list of supported parameters, see [aidb.inference_config reference](/edb-postgres-ai/current/ai-factory/pipeline/reference/models/#aidbinference_config).
