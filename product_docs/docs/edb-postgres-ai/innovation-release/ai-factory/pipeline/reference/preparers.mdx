---
title: "Pipelines Preparers (deprecated) reference"
navTitle: "Preparers (deprecated)"
...
| reduction_factor | INTEGER | NULL     | Used with `'reduce'` strategy to control how aggressively text is reduced in each iteration (default is `3`)                                                   |
| inference_config | JSONB   | NULL     | Optional runtime inference settings (use [`aidb.inference_config()`](/edb-postgres-ai/current/ai-factory/pipeline/reference/models/#aidbinference_config)) |
...
-- Use with aggregate function and inference configuration
SELECT
    category,
    aidb.summarize_text_aggregate(
        content,
        aidb.summarize_text_config(
            'llama',
            NULL,
            'combine into one concise summary',
            NULL,
            NULL,
            aidb.inference_config(
