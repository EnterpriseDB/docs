...
Configures a VectorChord HNSW index for the semantic knowledge base.
...
| ------------------ | ---------------- | ------- | ----------------------- |
| `vector_data_type` | TEXT             | NULL    | Vector storage type     |
| `m`                | INTEGER          | NULL    | Max connections per node|
| `ef_construction`  | INTEGER          | NULL    | Build-time search depth |
| `max_connections`  | INTEGER          | NULL    | Maximum connections     |
| `ml`               | DOUBLE PRECISION | NULL    | Level multiplier        |

#### Example

```sql
aidb.vector_index_chord_hnsw_config(m => 16, ef_construction => 64)
```

### `aidb.vector_index_chord_vchordq_config`

Configures a VectorChord Vchordq index for the semantic knowledge base.
...
| --------------------- | ------- | ------- | ----------------------------- |
| `vector_data_type`    | TEXT    | NULL    | Vector storage type           |
| `lists`               | TEXT    | NULL    | Number of lists configuration |
| `spherical_centroids` | BOOLEAN | NULL    | Use spherical centroids       |

#### Example

```sql
aidb.vector_index_chord_vchordq_config(lists => '100', spherical_centroids => true)
```

### `aidb.vector_index_hsphere_optimized_config`

Configures an HSphere optimized index for the semantic knowledge base.
...
| `precision_val`    | DOUBLE PRECISION | Required | Precision value     |
| `vector_data_type` | TEXT             | NULL     | Vector storage type |

#### Example

```sql
aidb.vector_index_hsphere_optimized_config(clusters => 100, precision_val => 0.95)
```
