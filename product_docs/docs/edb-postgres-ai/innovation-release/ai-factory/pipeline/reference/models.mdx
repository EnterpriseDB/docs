...
| `repeat_penalty` | REAL             | NULL    | Penalty for repeating tokens (1.0 = no penalty, higher = more penalty). Not supported by NIM and OpenAI Completions.                                           |
| `repeat_last_n`  | INTEGER          | NULL    | Number of most recent tokens to consider for repeat penalty. Not supported by NIM and OpenAI Completions.                                                      |
| `thinking`       | BOOLEAN          | NULL    | Whether to show `<think>...</think>` tags in the response. `true` keeps thinking tags (shows reasoning), `false` strips them, `NULL` keeps tags (default).     |
