...
VectorChord is a high-performance PostgreSQL extension for dense vector search. It extends `pgvector` with index types that are faster and more memory-efficient at scale, making it well-suited for knowledge bases containing millions or billions of embeddings.
...
VectorChord provides two index types, both configurable through the Pipelines config helpers.
...
- [Vector index config helpers](/edb-postgres-ai/innovation-release/ai-factory/data-pipelines/reference#vector-index-config-helpers) — full parameter reference for all index config functions
