...
- **Lifecycle bound to the pipeline.** The error log table is created with the pipeline and dropped when the pipeline is deleted. Pipelines created before 7.2.0 get their table during the upgrade. Writes happen inside the failing step's transaction, so a successful step never leaves orphan log rows and a failure never loses its record.
