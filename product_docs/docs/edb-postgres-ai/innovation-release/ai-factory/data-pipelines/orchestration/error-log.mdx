...
New in 7.5.0. Use [`aidb.requeue_pipeline_errors`](../reference#aidbrequeue_pipeline_errors) to mark failed source records dirty by error log ID. The call deletes the targeted record-level rows and marks their sources dirty on the pipeline's state surface; the next normal run does the re-processing.
...
- `skipped_pipeline_level`: the entry has a NULL `source_id` (see below). Left in place.
...
Fix the underlying issue (model availability, config, deleted model) and re-run the pipeline. The NULL row clears automatically once the step next executes without any errors. To dismiss it without re-running, use `aidb.clear_error_logs`.

#### Disabled-mode volume caveat

On a Disabled-mode pipeline with a volume source, `aidb.requeue_pipeline_errors` sets the `retry_requested_at` marker on the marked files but cannot schedule a drain, so it emits a `NOTICE` pointing at `aidb.run_pipeline`. Manual `aidb.run_pipeline` re-reads the whole volume rather than only the marked files, which pays a full-volume scan on large volumes.
...
--     This marks the source dirty; it does not re-process on its own.
