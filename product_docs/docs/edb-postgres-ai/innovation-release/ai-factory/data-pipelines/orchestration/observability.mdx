...
Look up the view [`aidb.knowledge_base_stats`](../../pipeline/reference/knowledge_bases.mdx/#aidbknowledge_base_stats), or use its short alias `aidb.kbstat`. The view shows all configured knowledge base pipelines,
...
Per-record failure details, including which step failed, the operation, the underlying error message, and retry state, live in the [Error log](./error-log/).