...
Re-runs the pipeline for the source records named by the specified error log entries and returns per-row outcomes. A pipeline-scoped advisory lock prevents two concurrent retries on the same pipeline, but it does not block `aidb.run_pipeline` or the background worker.

#### Parameters

| Parameter       | Type     | Default  | Description                                                                                                  |
| --------------- | -------- | -------- | ------------------------------------------------------------------------------------------------------------ |
| `pipeline_name` | TEXT     | Required | Name of the pipeline to retry. (Parameter names omit the `p_` prefix used by the other error log functions.) |
| `error_ids`     | BIGINT[] | Required | Error log entry IDs to retry. Duplicates are deduplicated; unknown IDs raise an exception.                   |

Raises if the pipeline does not exist, if any supplied `error_id` is missing from the error log, if the source is empty, or if another retry is already in progress for the pipeline.
...
| `status`        | text   | One of `resolved`, `failed`, or `skipped`.                                                                   |
| `error_message` | text   | NULL when `resolved`; new error text when `failed`; advisory message when `skipped` (pipeline-level errors). |
...
Settable per session, per role, per database, or in `postgresql.conf`.
