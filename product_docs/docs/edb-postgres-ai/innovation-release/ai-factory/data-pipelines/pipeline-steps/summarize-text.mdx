...
- `chunk_config` (optional): Configuration for chunking the input text before summarization. Accepts the same options as the [chunk_text operation](chunk-text.mdx). This is useful when the input text is too large to fit in the model's context window.
