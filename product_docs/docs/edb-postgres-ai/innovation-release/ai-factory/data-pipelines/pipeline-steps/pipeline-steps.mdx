...
| `max_length`     | `INTEGER` | `NULL`   | Maximum chunk size. Chunks will not exceed this. If omitted, `desired_length` is a strict upper limit. |
...
| `reduction_factor` | `INTEGER` | `NULL`   | Used with `'reduce'` strategy. Controls how aggressively text is reduced per iteration (default 3). |
...
| `topk`              | `INTEGER`                  | `NULL`   | Number of results to return on retrieval.                                       |
...
To link multiple pipelines to the same knowledge base, use `aidb.knowledge_base_config_from_kb(data_format)` instead. This inherits the model and distance operator settings from the existing knowledge base.
