...
If HM is connected, update default model image URLs to point to your registry so future clusters pull from it. You can find default models under the **Models** tab in the Asset Library. You can update the model image URL directly from the UI.
...
!!! important
    ...
    For Azure Blob Storage, you must upload a single tar archive file rather than multiple files. This prevents data loss caused by a known limitation in the KServe storage initializer when the file depth exceeds 5.
...
1. Choose a default model. 

  For example: llama‑3.2‑nemoretriever‑300m‑embed‑v1.
1. Set **Model Profiles Path on Object Storage** to the path you uploaded in Phase 2. 

  For example: `/model-cache/nim-nvidia-llama-3.2-nemoretriever-300m-embed-v1`
1. Select your compute (GPU) and size.
1. Select **Create**.
...
- You don't need to create secrets or apply any YAML to use the cache in HM.
