...
- `networking` &mdash; Network access type for the cluster (required). Valid values: `private`, `public`, or `custom`. Use `custom` to supply your own load balancer service annotations instead of HM's default annotations, for example to select a MetalLB IP address pool or disable Azure Private Link Service. See [Custom networking annotations](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/cluster_management/manage-clusters/custom-networking/) for requirements and examples.
