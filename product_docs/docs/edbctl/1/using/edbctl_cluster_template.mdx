...
- [edbctl cluster-template create](#edbctl-cluster-template-create) &mdash; Used to define and register a new database cluster template.

- [edbctl cluster-template list](#edbctl-cluster-template-list) &mdash; Used to view all available cluster templates within your project or estate.
...
```
  ...
  -d, --description string        A brief summary of the template's purpose or configuration.
...
#  enabled: true
#  configuration:
#    networking: private # private, public, or custom
#    allowedIpRanges:
#    - cidrBlock: "10.0.0.0/8"
#      description: "Internal network"
#    serviceAnnotations:
#    - key: "service.beta.kubernetes.io/load-balancer-type"
#      value: "internal"
...
- **AHA** &mdash; An advanced high-availability cluster with expanded resource limits.

**Key configuration parameters**

- `clusterType` &mdash; Specifies the type of cluster - `single`, `ha`, `pgd`, `analytics`, or `aha` (required).
- `instanceSize` &mdash; CPU Cores and Memory in Gi (required). Limits vary by cluster type: 
      - For Single, HA, PGD, and Analytics clusters, CPU is 0.1–15 cores and memory is 0.1–10 Gi. 
      - For AHA clusters, CPU is 0.1–47.51 cores and memory is 0.1–178.51 Gi.
- `storage` &mdash; Database storage size and `storageClass` like `ssd-csi` (required).
- `networking` &mdash; Network access type for the cluster (required). Valid values: `private`, `public`, or `custom`. Use `custom` to supply your own load balancer service annotations instead of HM's default annotations, for example to select a MetalLB IP address pool or disable Azure Private Link Service. See [Custom networking annotations](/edb-postgres-ai/latest/hybrid-manager/using_hybrid_manager/cluster_management/manage-clusters/custom-networking/) for requirements and examples.
- `serviceAnnotations` &mdash; Optional list of Kubernetes service annotation key/value pairs. Only valid when `networking` is `custom`. The API replaces the entire annotation set on every update, so always include every annotation you want to keep.
- `backupSchedule` &mdash; A backup schedule in 6-field cron format (UTC): `sec min hour dom month dow` (required). This differs from the standard 5-field cron used by most schedulers.
- `imageId` &mdash; The image UUID for the database cluster.
- `postgresqlConfiguration` &mdash; Key-value pairs for `postgresql.conf`.
- `pgExtensions` &mdash; Optional list of PostgreSQL extensions to enable or disable on the cluster (for example, `postgis`, `pgvector`). Each extension ID must be available in the selected image.
...
-y, --config-yaml	        Prints the template’s configuration in a raw YAML format (used with `--id`).
