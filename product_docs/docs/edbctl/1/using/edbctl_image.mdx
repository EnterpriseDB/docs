...
!!!note Two different credential models
`edbctl image` commands authenticate one of two ways:

| Command                                                              | Auth model                                                                       | Requires                                                                                                  |
|----------------------------------------------------------------------|----------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------|
| `list-image-tags`                                                    | HM access-key credential (`-c/--credential`, `context_credential`)               | A provisioned, running HM instance. Import a credential first with `edbctl credential import-access-key`. |
| `sync-to-local-registry`, `export-to-archive`, `import-from-archive` | Direct registry credentials (`--source-registry-*` / `--destination-registry-*`) | No HM instance or stored credential — usable before HM is installed.                                      |
| `cleanup-registry`                                                   | Direct Cloudsmith credentials, plus a live HM registry                           | A running HM instance; not a pre-install/bootstrap command.                                               |
