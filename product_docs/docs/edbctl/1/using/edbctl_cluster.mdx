...
tags: # Optional tags for the cluster with name and color(valid name or hex color code)
  - color: blue
    tagName: tag-1
networking: public # Network access type for the cluster. Valid values: private, public, custom
...
storage:
  ...
  walStorage:   # Optional field. WAL Storage size in Gi, a separate storage volume for Write-Ahead Logs (Recommended for high write workloads)
    sizeGi: 1 # Size must be between 0 and Unlimited (0-Unlimited Gi)
