...
The `edbctl operator` command provides tools for managing the Postgres AI Operator, including syncing operator images. 

## Commands

- [edbctl operator sync-to-local-registry](#edbctl-operator-sync-to-local-registry) &mdash; Sync operator images from EDB repository to a local registry.
