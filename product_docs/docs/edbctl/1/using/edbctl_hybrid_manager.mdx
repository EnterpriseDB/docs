...
!!!note
   The `resume-component`, `retry-component`, and `suspend-component` commands prompt for confirmation by default. Use `--yes` (`-y`) to skip the prompt for CI or scripted use. For guidance on when and why to use these commands, see [Managing HM components](/edb-postgres-ai/current/hybrid-manager/using_hybrid_manager/manage_components/).
!!!
