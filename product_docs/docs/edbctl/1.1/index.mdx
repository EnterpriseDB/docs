...
After installing the package, configure an [access key](/edb-postgres-ai/1.3/hybrid-manager/using_hybrid_manager/using_the_api/access_key/) using the `credential` command. These credentials allow you to access the supported HM resources and manage them using the CLI. The CLI supports various commands for managing projects, clusters, backups, credentials, and more.
