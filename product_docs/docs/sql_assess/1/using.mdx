...
- Location for output file &mdash; Provide the path where you want the EDB Query Extractor to place the SQL statements extracted from the `V$SQL` table.
...
!!! Note
    The path specified by `inputfilepath` is meant to contain either Oracle trace files or the .out files generated by the EDB Query Extractor script.
