...
!!! Note
    The current host may not be the host of the PEM backend database.
...
-   `port` specifies the port used by the backend PostgreSQL database; by default, the PostgreSQL database uses port `5432`.
-   `cidr_address_range` specifies the address range that will be added to the `pg_hba.conf` file of the PEM server's backend database to allow connections from the agents that will be monitored by the server. You may wish to specify a network range (for example, 192.168.2.0/24) to provide server access to agents that reside on the same network.
