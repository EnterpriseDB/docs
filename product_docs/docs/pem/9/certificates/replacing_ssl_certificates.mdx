...
1. Change the permissions of the `root.crl` file:
...
1. Move the `server.key` to the data directory of the backend server and change the permissions:
...
1. Regenerate each PEM agent's self-signed SSL certificates. For more information see,[Regenerating agent SSL ceritificates](/pem/latest/certificates/regenerating_agent_certificates).
