...
If the PEM backend database server certificates are near expiring, you should plan to regenerate the certificates and key files.
...
1. Stop all running PEM agents, first on the server host and then on any monitored host.
   ...
   - On Windows: 
     Use the Services applet to stop the PEM agent. The PEM agent service is named Postgres Enterprise Manager Agent. Select the service name in the Services dialog box and select **Stop the service**.
...
1. Use the openssl req command to create the CSR:
   ...
   Where `-subj` is provided as per your requirements. You define `CN` as hostname/domain name of the PEM server host. 
...
1. Use the openssl x509 command to sign the CSR and generate a server certificate. Move the `server.crt` to the data directory of the backend database server:
   ...
   Where `-req` option indicates the input is a CSR, and the `-CA` and `-CAkey` options specify the root certificate and private key to use for signing the CSR.
...
1. Regenerate each PEM agent's self-signed SSL certificates. For more information see, [Regenerating agent SSL certificates](/pem/latest/certificates/regenerating_agent_certificates).
