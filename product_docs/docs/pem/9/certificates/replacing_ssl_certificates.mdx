---
title: "Regenerating server self-signed SSL certificates"
---

You can regenerate the self-signed SSL certificates on an existing PEM installation, if needed. If you upgrade your PEM server to a new version, you need to regenerate the server and agent certificates. If you are [upgrading PEM](/pem/latest/upgrading/upgrading_pem_installation/upgrading_pem_installation_linux_rpm/#upgrading-a-pem-agent-native-package-installation), first upgrade the PEM server and then the PEM agent on all PEM installations before replacing the SSL certificates.
...
3.  Use the `openssl_rsa_generate_key()` function to generate the `ca_key.key` file:
    ...
    ```shell
    /usr/edb/as<x>/bin/psql -U enterprisedb -d pem --no-psqlrc -t -A -c "SELECT public.openssl_rsa_generate_key(1024)" > /var/lib/edb/as<x>/data/ca_key.key
    ```
...
4.  `cat` the contents to the variable `CA_KEY` for use when generating the `ca_certificate.crt` file. Change the permissions on the `ca_key.key` file:
    ...
    ```shell
    CA_KEY=$(cat /var/lib/edb/as<x>/data/ca_key.key)
...
6.  Use the variable to execute the query, placing the content in the `ca_certificate.crt` file.
    ...
    ```shell
    /usr/edb/as<x>/bin/psql -U enterprisedb -d pem --no-psqlrc -t -A -f /tmp/_random.$$ > /var/lib/edb/as<x>/data/ca_certificate.crt
    ```
...
12. Use the `openssl_rsa_generate_key()` function to generate the `server.key` file:
    ...
    ```shell
    /usr/edb/as<x>/bin/psql -U enterprisedb -d pem --no-psqlrc -t -A -c "SELECT public.openssl_rsa_generate_key(1024)" >> /var/lib/edb/as<x>/data/server.key
    ```
...
13.  `cat` the contents to the variable `SSL_KEY` for use when generating the `server.crt` file and change the permissions on the `server.key` file:
    ...
    ```shell
    SSL_KEY=$(cat /var/lib/edb/as<x>/data/server.key)
...
14. Use the `SSL_KEY` to generate the server certificate. Save the certificate in the `server.crt` file. For simplicity, first place the SQL query into a temporary file with a unique name:
    ...
    '/var/lib/edb/as<x>/data/ca_certificate.crt',
    '/var/lib/edb/as<x>/data/ca_key.key')" > /tmp/_random.$$

    /usr/edb/as<x>/bin/psql -U enterprisedb -d pem --no-psqlrc -t -A -f /tmp/_random.$$ >> /var/lib/edb/as<x>/data/server.crt
    ```
...
15. Change the permissions on the `server.crt` file, and delete the temporary file:
    ...
    rm -f /tmp/_random.$$
    ```
15. [Regenerate PEM agent self-signed SSL certificates](/pem/latest/certficates/regenerating-agent-certificates).
