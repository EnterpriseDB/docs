...
1. Stop all running PEM agents, first on the server host and then on any monitored host.
   ...
   - On Windows: 
     You can use the Services applet to stop the PEM agent. The PEM agent service is named Postgres Enterprise Manager Agent. Select the service name in the Services dialog box and select **Stop the service**.
