...
1.  On the primary server (172.17.0.2), update the PEM packages: 
    ...
    This ensures the latest version of the script required for the upgrade is available.

1.  Execute the upgrade script. It upgrades PEM to the latest available version, restarts the backend database, the web server and the PEM agent: 
...
1.  On the tertiary server (172.17.0.4), update the PEM packages:

    ```
    dnf update edb-pem
    ```

1.  Copy the configuration files, certificate and key from the primary server to the tertiary server: 
