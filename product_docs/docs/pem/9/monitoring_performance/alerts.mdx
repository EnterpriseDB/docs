...
| PGD Group Raft Consensus                                                    | PGD group raft consensus not working                                                                                                            |                                   |
| PGD Group Raft Leader ID not matching                                       | PGD group raft leader id not matching                                                                                                           |                                   |
| PGD Group versions check                                                    | PGD/pglogical version mismatched in PGD group                                                                                                   |                                   |
...
You can view the scheduled alert blackout details from `event_history` table in `pem` schema once the schedule is executed. For more information, see [monitoring event history](../monitoring_event_history.mdx).