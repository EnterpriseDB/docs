...
Slack provides a way to post messages via `Incoming Webhooks`. To send an alert from PEM to Slack, users must create an `Incoming Webook`. The `Incoming Webhook` gives a unique URL to which the pemAgent can send a JSON payload with the message text and other options to Slack webhooks.
