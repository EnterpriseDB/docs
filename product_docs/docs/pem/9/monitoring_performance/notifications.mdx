...
Under HTTP headers, add a header with the key `Content-Type` and the value `application/json`.
...
Save the webhook and continue using PEM as normal. All alerts will now be sent to your Slack channel!
