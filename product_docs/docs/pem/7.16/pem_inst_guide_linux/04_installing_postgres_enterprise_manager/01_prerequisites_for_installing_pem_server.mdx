...
1.  Install a backend database.
    ...
    When installing a PEM server on a Linux host, you must first install a backend database cluster that will hold the `pem` database. The PEM server’s backend database may be installed via package for Linux. The backend database must be one of the following versions:
