...
`pg_upgrade` supports a transfer of data between servers of the same type. For example, you can use `pg_upgrade` to move data from a PostgreSQL 9.6 backend database to a PostgreSQL 10 backend database, but not to an Advanced Server 10 backend database. If you wish to migrate to a different type of backend database (i.e from a PostgreSQL server to Advanced Server), see <span class="title-ref">Moving the Postgres Enterprise Manager Server &lt;moving_pem_server></span>.
