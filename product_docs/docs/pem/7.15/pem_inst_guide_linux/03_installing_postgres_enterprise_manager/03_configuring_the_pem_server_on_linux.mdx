...
> **Note:** If the web server (PEM-HTTPD) and the backing database (PEM Server) reside on separate hosts, configure the database server first (option 3), and then web services (option 2). The script will exit if the backing database is not configured before web services.
