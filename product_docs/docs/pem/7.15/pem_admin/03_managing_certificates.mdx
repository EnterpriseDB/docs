...
>     `CA_KEY` for use when generating the `ca_certificate.crt` file and modify the privileges on the `ca_key.key` file:
>
>     ```text
...
7.  Use the `openssl_rsa_generate_key()` function to generate the `server.key` file:
    ...
    ```text
    /usr/pgsql-x/bin/psql -U postgres -d pem --no-psqlrc -t -A -c
    "SELECT public.openssl_rsa_generate_key(1024)" >> /var/lib/pgsql/x/data/server.key
    ```
