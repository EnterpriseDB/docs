...
| enable_smtp             | When set to true, for multiple PEM Agents and PEM backend database server is 9.4 or lesser then it may send duplicate emails(also with PEM Agent 7.14 or higher). And also when set to true for multiple Agents with atleast one PEM Agent be 7.14 or higher then it may still send duplicate emails.    | true for PEM server host; false for all others.                                                               |
| enable_snmp             | When set to true, for multiple PEM Agents and PEM backend database server is 9.4 or lesser then it may send duplicate SNMP traps(also with PEM Agent 7.14 or higher). And also when set to true for multiple Agents with atleast one PEM Agent be 7.14 or higher then it may still send duplicate traps. | true for PEM server host; false for all others.                                                               |
