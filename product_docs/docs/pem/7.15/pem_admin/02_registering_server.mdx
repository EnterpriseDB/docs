...
-   Check the box next to `Save password?` to instruct PEM to store passwords in encrypted format in PEM backend database for later reuse. Password is stored per user per server basis, and hence - won't be shared with other team members. PEM will use the saved password to connect the database server next time. To remove a saved password, disconnect the database server first, and then use 'Clear Saved Password' menu item from the Object/Context menu.
