...
-   [Prerequisites for monitoring Barman](#prerequisites-for-monitoring-barman)

You must add a Barman server to the PEM console whose backup you want to manage with Barman. See

-   [Configuring a Barman Server](#configuring-a-barman-server)

After you configure the Barman server, you will be able to view the details of the backups in the dashboard. See

-   [Viewing the Barman Server Details on a PEM Dashboard](#viewing-the-barman-server-details-on-a-pem-dashboard)
...
- Install the [pg-backup-api](/supported-open-source/barman/pg-backup-api/) tool on Barman host.
