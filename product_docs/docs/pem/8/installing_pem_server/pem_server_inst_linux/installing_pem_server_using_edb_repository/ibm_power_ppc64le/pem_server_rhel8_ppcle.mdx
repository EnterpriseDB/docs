---
title: "Installing the PEM server on RHEL 8 ppc64le"
navTitle: "RHEL 8"
---
...
1. Set up the EDB repository:
   ...
   This command creates the `/etc/yum.repos.d/edb.repo` configuration file.

1. Replace `'USERNAME:PASSWORD'` with your username and password available from your
...
!!! Note
    The operating system user pem is created while installing the PEM Server. The PEM server web application is a WSGI application that runs under Apache HTTPD. The pem application data and the session is saved to this user's home directory.
