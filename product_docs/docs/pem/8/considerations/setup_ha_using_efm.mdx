...
This document walks you through setting up the Failover Manager for Postgres Enterprise Manager (PEM) server with new installation and not with existing installation.

PEM helps database administrators, system architects, and performance analysts to administer, monitor, and tune Postgres database servers.
