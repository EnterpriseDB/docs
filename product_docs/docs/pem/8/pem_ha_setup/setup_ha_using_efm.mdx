...
2.  Configure the PEM Server on the primary and one or more standbys independently. For more detail, see "Configuring the PEM Server" section in the [PEM Installation Guides](https://www.enterprisedb.com/docs/pem/latest/).
...
1. Configure the following in the `postgresql.conf` file:
   ...
   !!! Note
    The configuration parameters may be different for different versions of the database server. You can email Support at [techsupport@enterprisedb.com](mailto:techsupport@enterprisedb.com) for help in setting up these parameters.

1. Add the following entry in the host-based authentication (`/var/lib/edb/as13/data/pg_hba.conf`) file to allow replication user to connect from all the standbys:
    ...
    !!! Note
         You can provide an IP address with a more appropriate `cidr` range.

1. Modify the host-based authentication (`/var/lib/edb/as13/data/pg_hba.conf`) file for the `pem_user` role to connect to all databases using the `scram-sha-256` authentication method:
...
1. Remove the data directory of the PEM backend database server on all the standby nodes:
...
1. Take the backup of the primary node on each of the standby nodes using `pg_basebackup`:
   ...
   The backup command creates the `postgresql.auto.conf` and the `standby.signal` files on the standby nodes.

1. Edit the following parameter in `postgresql.con` file on all the standby nodes:

    ``` text
        hot_standby = on
    ```

1. Start the PEM backend database server (EPAS 13) on each of the standby nodes.
...
1. Copy the PEM agent certificates and keys from `/root/.pem` from the primary node to the standby nodes at same location. Set the following permissions to the PEM Agent Certificate file:
...
1. Prepare the primary pode to support EFM:

   1. Create a database user `efm` to connect to the database servers.

   1. Grant the execute privileges on the functions related to WAL logs and the monitoring privileges to the user.
...
1. Create the `efm.properties` file on all the nodes using the sample file (`/etc/edb/efm-4.1/efm.properties.in`), and grant read access to all the users:
...
1. Encrypt the `efm` user's password using efm utility:
...
1. Set the value of `is.witness` configuration parameter on the witness node to `true`:
...
1. Check the EFM cluster status from any node:
