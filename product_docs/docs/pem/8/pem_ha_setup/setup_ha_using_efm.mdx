---
title: "Set Up High Availability Using EFM"
---
...
EDB Postgres Failover Manager (EFM) is a high-availability module from EDB that enables a Postgres primary node to automatically failover to a standby node in the event of a software or hardware failure on the primary.
...
# Initial product installation and configuration prerequisites

1. Install the following on primary and one or more standbys
    ...
    You can install all the above products as per the instructions given on [EDB Repo Website](https://repos.enterprisedb.com). You need to replace the `USERNAME:PASSWORD` with your username and password in the given instructions to access the EDB repositories.
...
2.  Configure the PEM Server

    Configure the PEM Server on all the primary and one or more standbys independently. For more detail, see Configuring the PEM Server section in the [PEM Installation Guides](https://www.enterprisedb.com/docs/pem/latest/).

3.  Firewall Settings

    Add the following ports in the firewall on all the primary and standbys to allow the access:
...
# Setting up the primary node for streaming replication.

-   Create the replication role using the following command
...
-   Configure the following in `postgresql.conf` file:
...
!!! Note
    The configuration parameters may be different for different versions of database server. You can contact `support@enterprisedb` for any help in setting up these parameters.

-   Add the following entry in host based authentication (/var/lib/edb/as13/data/pg_hba.conf) file to allow replication user to connect from all the standbys:
...
-   Modify the host based authentication (/var/lib/edb/as13/data/pg_hba.conf) file for `pem_user` role to connect to all databases using `scram-sha-256` authentication method.
...
# Setting up the standby nodes for streaming replication.

-   Stop the service of PEM backend database server (EPAS 13) and the pemagent service on all the standby nodes
...
-   Remove the data directory of the PEM backend database server on all the standby nodes
...
-   Create `.pgpass` file in the home directory of `enterprisedb` user on all the standby nodes
...
-   Take the backup of Primary Node on each of the standby nodes using pg_basebackup
...
The backup command creates `postgresql.auto.conf` and the `standby.signal` file on the standby nodes.

-   Edit the following parameter in `postgresql.con` file on all the standby nodes:

    ``` text
        hot_standby = on
    ```

-   Start the PEM backend database server (EPAS 13) on each of the standby nodes.
...
-   Copy the PEM Agent Certificates and Keys from `/root/.pem` from the primary node to the standby nodes at same location. Set the following permissions to the PEM Agent Certificate file:
...
# Setting up EFM to manage failover on all hosts
...
-   Create `efm.properties` file on all the nodes using the sample file (`/etc/edb/efm-4.1/efm.properties.in`), and grant read access to all the users
...
-   Encrypt the `efm` user's password using efm utility
...
-   Set the value of `is.witness` configuration parameter on the witness node to `true`.
...
The above status confirms that EFM is set up successfully and managing the failover for the PEM Server.

The monitored database servers should register their PEM Agents using the VIP address. Also, the PEM Web Client shall be accessed using a VIP address.

In case of failover, any of the standby gets promoted as the primary node. PEM Agents automatically connect to the new primary node. All the user sessions are lost, and they will have to log in again. You can replace the failed primary node with a new standby using the above steps.
