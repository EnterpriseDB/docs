...
<figure><img src="../images/preferences_bart_servers_nodes.png" class="align-center" alt="Preferences dialog - BART Servers Nodes section" /><figcaption aria-hidden="true"><em>Preferences dialog - BART Servers Nodes section</em></figcaption></figure>
...
<figure><img src="../images/preferences_browser_display.png" class="align-center" alt="Preferences dialog - Browser Display options" /><figcaption aria-hidden="true"><em>Preferences dialog - Browser Display options</em></figcaption></figure>
...
-   When the `Confirm before closing properties with unsaved changes` switch is set to `True`, PEM will warn you before closing the properties dialog of an object if there are any unsaved changes. On user confirmation, the properties dialog will close.
-   When the `Confirm on close or refresh` switch is set to `True`, PEM will attempt to catch browser close or refresh events and prompt before allowing them to continue.
...
<figure><img src="../images/preferences_browser_keyboard_shortcuts.png" class="align-center" alt="Preferences dialog - Browser keyboard Shortcuts section" /><figcaption aria-hidden="true"><em>Preferences dialog - Browser keyboard Shortcuts section</em></figcaption></figure>
...
<figure><img src="../images/preferences_browser_nodes.png" class="align-center" alt="Preferences dialog - Browser Nodes section" /><figcaption aria-hidden="true"><em>Preferences dialog - Browser Nodes section</em></figcaption></figure>
...
<figure><img src="../images/preferences_browser_properties.png" class="align-center" alt="Preferences dialog - Browser Properties section" /><figcaption aria-hidden="true"><em>Preferences dialog - Browser Properties section</em></figcaption></figure>

-   Include a value in the `Count rows if estimated less than` field to perform a `SELECT count()` if the estimated number of rows in a table (as read from the table statistics) is below the specified limit. After performing the `SELECT count()`, PEM will display the row count. The default is 2000.
...
<figure><img src="../images/preferences_dashboard_display.png" class="align-center" alt="Preferences dialog - Dashboard Display options" /><figcaption aria-hidden="true"><em>Preferences dialog - Dashboard Display options</em></figcaption></figure>
...
<figure><img src="../images/preferences_dashboard_graphs.png" class="align-center" alt="Preferences dialog - Dashboard Graph options" /><figcaption aria-hidden="true"><em>Preferences dialog - Dashboard Graph options</em></figcaption></figure>
...
<figure><img src="../images/preferences_debugger_keyboard_shortcuts.png" class="align-center" alt="Preferences dialog - Debugger Keyboard Shortcuts section" /><figcaption aria-hidden="true"><em>Preferences dialog - Debugger Keyboard Shortcuts section</em></figcaption></figure>

**The Miscellaneous Node**

Expand the `Miscellaneous` node to specify miscellaneous display preferences.

<figure><img src="../images/preferences_misc_user_language.png" class="align-center" alt="Preferences dialog - User Language section" /><figcaption aria-hidden="true"><em>Preferences dialog - User Language section</em></figcaption></figure>

-   Use the `User language` drop-down listbox to select the display language for the PEM web interface.

<figure><img src="../images/preferences_misc_themes.png" class="align-center" alt="Preferences dialog - Themes section" /><figcaption aria-hidden="true"><em>Preferences dialog - Themes section</em></figcaption></figure>
...
<figure><img src="../images/preferences_paths_binary.png" class="align-center" alt="Preferences dialog - Binary path section" /><figcaption aria-hidden="true"><em>Preferences dialog - Binary path section</em></figcaption></figure>
...
-   Use the `EDB Advanced Server Binary Path` field to specify the location of the EDB Postgres Advanced Server utility programs. If this path is not set, PEM will attempt to find the utilities in standard locations used by EnterpriseDB.
-   Use the `Greenplum Database Binary Path` field to specify the location of the Greenplum database utility programs. If this path is not set, PEM will attempt to find the utilities in standard locations used by Greenplum.
-   Use the `PostgreSQL Binary Path` field to specify the location of the PostgreSQL utility programs. If this path is not set, PEM will attempt to find the utilities in standard locations used by PostgreSQL.

<figure><img src="../images/preferences_paths_help.png" class="align-center" alt="Preferences dialog - Paths Help section" /><figcaption aria-hidden="true"><em>Preferences dialog - Paths Help section</em></figcaption></figure>
...
<figure><img src="../images/preferences_performance_diagnostic_display.png" class="align-center" alt="Preferences dialog - Performance Diagnostic Display option" /><figcaption aria-hidden="true"><em>Preferences dialog - Performance Diagnostic Display option</em></figcaption></figure>
...
<figure><img src="../images/preferences_sql_auto_completion.png" class="align-center" alt="Preferences dialog - Query Tool Auto completion option" /><figcaption aria-hidden="true"><em>Preferences dialog - Query Tool Auto completion option</em></figcaption></figure>
...
<figure><img src="../images/preferences_sql_csv_output.png" class="align-center" alt="Preferences dialog - Query Tool CSV Output option" /><figcaption aria-hidden="true"><em>Preferences dialog - Query Tool CSV Output option</em></figcaption></figure>
...
<figure><img src="../images/preferences_sql_display.png" class="align-center" alt="Preferences dialog - Query Tool Display options" /><figcaption aria-hidden="true"><em>Preferences dialog - Query Tool Display options</em></figcaption></figure>
...
<figure><img src="../images/preferences_sql_editor.png" class="align-center" alt="Preferences dialog - Sqleditor Editor settings" /><figcaption aria-hidden="true"><em>Preferences dialog - Sqleditor Editor settings</em></figcaption></figure>
...
<figure><img src="../images/preferences_sql_explain.png" class="align-center" alt="Preferences dialog - Query Tool Explain options" /><figcaption aria-hidden="true"><em>Preferences dialog - Query Tool Explain options</em></figcaption></figure>
...
<figure><img src="../images/preferences_sql_keyboard_shortcuts.png" class="align-center" alt="Preferences dialog - Query Tool Keyboard Shortcuts section" /><figcaption aria-hidden="true"><em>Preferences dialog - Query Tool Keyboard Shortcuts section</em></figcaption></figure>

Use the fields on the `Options` panel to manage Query Tool preferences.

<figure><img src="../images/preferences_sql_options.png" class="align-center" alt="Preferences dialog - Query Tool Options section" /><figcaption aria-hidden="true"><em>Preferences dialog - Query Tool Options section</em></figcaption></figure>
...
<figure><img src="../images/preferences_sql_results_grid.png" class="align-center" alt="Preferences dialog - SQL Results Grid section" /><figcaption aria-hidden="true"><em>Preferences dialog - SQL Results Grid section</em></figcaption></figure>

Use the fields on the `Results grid` panel to specify your formatting preferences for copied data.

-   When the `Resize by data?` switch is set to *True*, then data output columns will auto fit to max data width. If it is set to *False* then size is set based on data type or column name.
...
<figure><img src="../images/preferences_sql_profiler_display.png" class="align-center" alt="Preferences dialog - SQL Profiler Display section" /><figcaption aria-hidden="true"><em>Preferences dialog - SQL Profiler Display section</em></figcaption></figure>
...
<figure><img src="../images/preferences_sql_profiler_keyboard_shortcuts.png" class="align-center" alt="Preferences dialog - Sql Profiler Keyboard Shortcuts section" /><figcaption aria-hidden="true"><em>Preferences dialog - Sql Profiler Keyboard Shortcuts section</em></figcaption></figure>

**The Scheduled Tasks Node**

Use fields on the `Options` panel to specify Scheduled Tasks preferences.

<figure><img src="../images/preferences_scheduled_tasks_options.png" class="align-center" alt="Preferences dialog - Scheduled Tasks Options section" /><figcaption aria-hidden="true"><em>Preferences dialog - Scheduled Tasks Options section</em></figcaption></figure>
...
<figure><img src="../images/preferences_schema_diff.png" class="align-center" alt="Preferences dialog - Schema Diff Display section" /><figcaption aria-hidden="true"><em>Preferences dialog - Schema Diff Display section</em></figcaption></figure>
...
<figure><img src="../images/preferences_storage_options.png" class="align-center" alt="Preferences dialog - Storage Options section" /><figcaption aria-hidden="true"><em>Preferences dialog - Storage Options section</em></figcaption></figure>
