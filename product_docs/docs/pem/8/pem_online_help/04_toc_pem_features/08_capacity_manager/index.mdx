...
When defining the boundaries of a Capacity Manager report, specify the starting date and time, and an end boundary. The end boundary can be a point in time or a threshold boundary (when the data meets a specified criteria). If the sample contains more data points than the number of points specified by the [cm_data_points_per_report](../02_pem_server_config/01_pem_config_options/#pem_config_options) configuration parameter, Capacity Manager applies the aggregation method to calculate a reduced number of graph points for the report.
