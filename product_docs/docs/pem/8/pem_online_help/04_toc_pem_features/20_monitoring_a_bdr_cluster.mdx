---
title: "Monitoring a BDR Group"
---

<div id="monitoring_a_bdr_cluster" class="registered_link"></div>

BDR (Bi-Directional Replication) is a PostgreSQL extension that provides a multi-master replicaton between the Postgres clusters. Before you monitor BDR through PEM console, you must ensure that your BDR nodes are up and running. For more information about BDR please visit the [EDB Doc Website](https://www.enterprisedb.com/docs/bdr/latest/).
...
After saving the server definition, the BDR Node will be included in the list of servers under the PEM Server Directory in the PEM client object browser tree. You can monitor the `BDR Nodes` from the [BDR dashboards](01_dashboards/03_bdr_dashboard/#bdr_dashboard).

To include monitoring information on the BDR dashboards, you must enable the relative probes for each BDR Group. You can see the complete list of BDR Probes [here](12_pem_manage_probes/03_pem_probe_config/01_pem_probes/#bdr_probes).
