...
You can configure and edit your Barman server in the following ways:
...
- Use the `Name` field to specify a user-friendly name for the server. The name specified will identify the server in the Browser tree.
...
To edit your Barman server, select your Barman server from the browser tree, right click and select `Properties` option.
...
- Use the fields on Information tab to view the detailed information about your Barman server. This tab gets populated whenever the Barman related probes are executed. You cannot modify any of the fields on Information tab.
