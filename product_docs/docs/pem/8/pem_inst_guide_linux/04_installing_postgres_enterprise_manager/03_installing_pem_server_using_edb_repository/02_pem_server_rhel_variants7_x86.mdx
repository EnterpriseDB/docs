...
On a CentOS or RHEL or Rocky Linux or AlmaLinux system, you can use the `yum` package manager to install a PEM Server; the installation tool you use will be dependent on the version of the host operating system. Before installing the server, you must ensure that your system contains the required prerequisite software listed below.
