...
| MFA_SUPPORTED_METHODS | Set to `email` to use the email authentication method (send a one-time code by email) or `email` to use the TOTP-based application authentication method. |
