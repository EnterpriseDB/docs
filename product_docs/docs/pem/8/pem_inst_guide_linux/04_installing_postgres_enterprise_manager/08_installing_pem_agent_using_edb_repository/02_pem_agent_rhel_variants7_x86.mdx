...
On a CentOS or RHEL or OL system, you can use the `yum` package manager to install a PEM Agent; the installation tool you use will be dependent on the version of the host operating system. Before installing the agent, you must ensure that your system contains the required prerequisite software listed below.
...
After installing PEM Agent using `yum` or `dnf`, you need to register the PEM Agent. For detailed information see [Registering a PEM Agent](09_registering_a_pem_agent/#registering-a-pem-agent).
