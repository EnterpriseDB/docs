...
1.  Create an `EDB Cloud` on Azure. For more details visit [EDB Cloud Documentation](https://www.enterprisedb.com/docs/edbcloud/latest/getting_started/).
 
1.  Register your `EDB Cloud Cluster` on your PEM Server:

    After creating the `EDB Cloud` on Azure, use `Create - Server` dialog to add your `EDB Cloud Cluster` to the PEM Server. Using this dialog you can describe a new server connection, bind the server to a PEM Agent, and display the server to the PEM browser tree control.
