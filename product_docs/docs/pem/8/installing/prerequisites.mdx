...
After completing the prerequisites, [install](/pem/8/installing/linux_x86_64/) and [configure](/pem/8/installing/configuring_the_pem_server_on_linux.mdx) PEM. If you're using two servers, install and configure PEM on both servers.

To install a PEM server on Linux, perform this preliminary configuration:
    
1.  Install a [supported Postgres instance](/pem/8/#postgres-compatibility) for PEM to use as a backend database. 
...
4. If you're using a firewall, allow access to port 8443 on the server where the PEM web application will be located:
     ...
     For RHEL/Rocky Linux/AlmaLinux/SLES:
