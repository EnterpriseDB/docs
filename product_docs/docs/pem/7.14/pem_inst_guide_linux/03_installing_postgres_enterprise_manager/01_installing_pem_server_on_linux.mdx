...
The `pg_hba.conf` file on the backing database must be configured to use `trust` authentication for connections. For information about modifying the pg_hba.conf file, see the [PostgreSQL core documentation](https://www.postgresql.org/docs/current/static/auth-pg-hba-conf.html).
