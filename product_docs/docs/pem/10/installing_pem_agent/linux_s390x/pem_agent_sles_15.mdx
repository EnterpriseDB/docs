---
navTitle: SLES 15
title: Installing Postgres Enterprise Manager agent on SLES 15 s390x
# This topic is generated from templates. If you have feedback on it, instead of
# editing the page and creating a pull request, please enter a GitHub issue and
# the documentation team will update the templates accordingly.

redirects:
  - /pem/10/installing_pem_agent/installing_on_linux/ibm_z_s390x/pem_agent_sles15_s390x
---

!!! Note

    Postgres Enterprise Manager 8.3 and later is supported on SLES.

## Prerequisites

Before you begin the installation process:

- Set up the EDB repository. For more information, see [getting started with repositories](https://www.enterprisedb.com/docs/repos/getting_started/).

- Activate the required SUSE module:

  ```shell
  # You can use SLES 15 SP3 for PEM 8.3 and later:
  sudo SUSEConnect -p PackageHub/15.3/s390x

  # You can use SLES 15 SP4 for PEM 8.6 and later:
  sudo SUSEConnect -p PackageHub/15.4/s390x

  ```

- Refresh the metadata:
  ```shell
  sudo zypper refresh
  ```

## Install the package

```shell
sudo zypper -n install edb-pem-agent
```

After installing PEM agent, you need to register the PEM agent. For detailed information see [Registering an agent](../../registering_agent/).
