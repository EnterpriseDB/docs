...
-   172.16.161.203 - EFM Backend Witness Node ??? remove witness for this version ???
...
1.  Open the following ports on the firewall of all backend servers:
...
1. Manually assign the VIP to the primary. For example:
    ...
    ```
2. Run the PEM configuration script, specifying the VIP as the host and option 2 (Database):  ??? check ???
...
Copy the following file from the primary to all standbys.
