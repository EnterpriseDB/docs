...
processes to provision EFM clusters. However, if you plan to use
...
If you are using your own SSL certificates, follow these steps to
...
Grant connect permission on `template1` to this common agent user.
...
will deploy on the three backend hosts and three frontend hosts.
Therefore you must generate six certificate key pairs for `agent1`
through `agent6` and place one pair on each host.
...
1.  Configure the PEM web application. Specify the VIP as the PEM server address.
