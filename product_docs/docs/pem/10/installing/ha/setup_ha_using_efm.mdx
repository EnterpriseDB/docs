...
1.  Install the following on the primary and all standbys:
    ...
    A minimal set of commands to complete this step is provided below. Refer to the installation instructions in the product documentation for more detail.
    ...
    # Start and enable the EPAS service
    sudo systemctl start edb-as-17
    sudo systemctl enable edb-as-17

    # Create a superuser role for PEM
    sudo su - enterprisedb -c "psql edb -c \"create role pemserver login superuser password 'your-password-here';\""

    # Edit the pg_hba.conf file
    HBA_FILE="/var/lib/edb/as17/data/pg_hba.conf"
    sudo sed -i "1i host    all             pemserver       172.16.161.1/24            scram-sha-256" "$HBA_FILE"

    sudo systemctl restart edb-as-17
    ```

2.  Configure the PEM backend on the primary server as well as on all the standby servers with an initial configuration of type 1 (web services and database): ??? Maybe change this to just install database on primary ???

    a. On each host, starting with the replicas, manually assign the VIP to the host. For example:

    ```shell
    sudo /usr/edb/efm-5.0/bin/efm_address add4 eth0 172.16.161.245/32
    ```
    b. Then, configure the PEM server, specifying the VIP as the host

    ```shell
    /usr/edb/pem/bin/configure-pem-server.sh -t 1 -ho 172.16.161.245
    ```
    c. Before proceeding to the next server, release the VIP. Omit this step on the primary.

    ```shell
    sudo /usr/edb/efm-5.0/bin/efm_address del eth0 172.16.161.245/32
    ```
...
4.  Modify the host-based authentication (`/var/lib/edb/as17/data/pg_hba.conf`) file for the pem_user role to connect to all databases using the scram-sha-256 authentication method:
    ??? is the modification here to allow the user to connect to all databases? or just to allow it to connect from the web application servers? given that they are local I guess not the latter?
    ??? Isn't this already configured by the script, not sure what we're really changing here...
...
7.  Copy the following files from the primary node to the standby nodes at the same location, overwriting any existing files. Set the permissions on the files:
    ...
    -   `/etc/httpd/conf.d/edb-pem.conf`  ??? change for NGINX ???
...
9. Disable and stop HTTPD and PEM agent services if they're running on all replica nodes:  ??? no longer needed ???
...
2.  Create the scripts on each node to start/stop the PEM agent:  ??? Not needed anymore ???
...
3.  Create a `sudoers` file (`/etc/sudoers.d/efm-pem`) on each node to allow the efm user to start/stop the pemagent: ??? Not needed anymore ???
