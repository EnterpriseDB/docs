...
Uses the pg_basebackup utility to create replicas of the PEM backend database server on the standby servers. 
...
4.  Take a backup of the primary node on each of the standby nodes using pg_basebackup:
    ...
    The `backup` command creates the `postgresql.auto.conf` and `standby.signal` files on the standby nodes. The `postgresql.auto.conf` file has the following content: ??? does this matter ???
