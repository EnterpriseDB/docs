...
- SSL is configured such the the `cert` auth method can be used for connections from PEM agents to the PEM backend database
...
If you are supplying your own SSL certificates, follow these steps to
...
Grant it connect permission on `template1`.
...
PEM will automatically create numbered agent users for the agents we
will deploy on the three hosts.
Therefore you must generate three certificate key pairs for `agent1`
through `agent3` and place one pair on each host.
...
1.  Run the PEM configuration script, specifying option 3 (Database) and the VIP as the host.
    You must specify the `--replication-solution efm` and `--replication-role primary` options to ensure HA mode is used.
    It is also important to specify `--skip-db-certificate` to tell the script not to attempt to set up SSL itself,
    and `--use-existing-sslutils` to tell it that it should still use sslutils to generate the agent certificates.
    ...
    You must also supply the `--skip-db-certificate` option to prevent PEM trying to configure SSL itself.
