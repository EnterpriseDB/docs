...
1.  Install the following packages:
    ...
    -   [PEM Server](/pem/latest/installing/) and libpq5
    ...
    dnf -y install edb-as17-server edb-pem edb-as17-server-sslutils edb-efm50 libpq5
    ```
...
1.  Run the following commands to create links to the copied files:

    ```shell
    cp /usr/edb/pem/web/pem.wsgi.in /usr/edb/pem/web/pem.wsgi
    
    ln -sf /usr/edb/pem/web/pem.wsgi /usr/edb/pem/web/wsgi.py
    if [ ! -f "/usr/edb/pem/web/venv/lib64" ]; then
        ln -fs /usr/edb/pem/web/venv/lib /usr/edb/pem/web/venv/lib64
    fi
    
    ln -sf $(which python3) /usr/edb/pem/web/venv/bin/python3
    ```
...
## Correct the agent-server binding host on the primary

If you are using PEM 10.2 or 10.1, login to the PEM web application, right click the primary server in Object Explorer and navigate to `Properties > PEM Agent`.
