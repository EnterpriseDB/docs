...
Please see [High Availability Patterns for PEM Deployment](pem/latest/considerations/ha_pem.mdx) to understand other options.

!!! Note "Witness nodes vs standby nodes"
    In this example we configured a primary, two standbys, and a witness node.
    In reality, you only need a witness node if you have an even number of data nodes.
    If you are configuring three data nodes (primary and two standbys), you may omit the steps pertaining to the witness node.
    If you are configuring two data nodes, include the steps pertaining to the witness node.
...
1.  **On the standbys only**, install the NGINX web server and add the `pem` user to the `nginx` group.
    This is not required on the primary because it occurs automatically during PEM configuration.
...
3. Optionally, to synchronize PEM web application user preferences between instances, [configure central storage of user preferences](user_settings.md).
   This will be replicated to the standbys in the next step.
