...
1.  Run the PEM configuration script, specifying option 3 (Database) and the IPs of all three backend hosts as `pemsuper` user.
    Provide the loopback IP address rather than the external IP address for the primary. 
    You must specify the `--replication-solution patroni` and `--replication-role primary` options to ensure HA mode is used.
...
1.  Run the PEM configuration script, specifying option 3 (Database) and the IPs of all three backend hosts as `pemsuper` user.
    Provide the loopback IP address rather than the external IP address for the local host.
    You must specify the `--replication-solution patroni` and `--replication-role standby` options to ensure HA mode is used.
