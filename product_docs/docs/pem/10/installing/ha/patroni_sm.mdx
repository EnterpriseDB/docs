...
-   172.16.161.111 - etcd1
-   172.16.161.112 - etcd2
-   172.16.161.113 - etcd3
...
1.  Install etcd from the PGDG repository
    ...
    ```
2.  Replace the contents `/etc/etcd/etcd.conf` with the following, substituting the appropriate values for `<node-ip>` and `node-name`.
    For example, `172.16.161.311` and `etc1`.
    ...
    ETCD_INITIAL_CLUSTER="etcd1=http://172.16.161.111:2380,etcd2=http://172.16.161.112:2380,etcd3=http://172.16.161.113:2380"
...
1.  Install the following packages:
    ...
    -   [PostgreSQL](/postgresql/latest/installing/) (backend database for PEM Server)
    ...
    -   [Patroni](/patroni/latest/installing/)

    ```shell
    dnf -qy module disable postgresql
    dnf -y install postgresql17-server postgresql17-contrib edb-pem sslutils_17
    ```
...
1.  Run the PEM configuration script, specifying option 3 (Database) and the IPs of all three backend hosts.
    Provide the loopback IP rather than the external IP for the primary.

    ```shell
    /usr/edb/pem/bin/configure-pem-server.sh -t 3 -ho '127.0.0.1,172.16.161.201,172.16.161.202' -su pemserver
    ```
...
1.  Create a `pg_hba` section under `bootstrap.dcs.postgresql` and populate it with a common set of HBA rules for all nodes.
    ...
    - host    all             patroni-sup     127.0.0.1/32            scram-sha-256
...
1.  Execute the following SQL on the `pem` database as a superuser, providing the correct server and port for each standby.
