...
-   Use the **On error** list to specify the behavior of pemAgent if it encounters an error while executing the step. Select from:
...
-   Use the **Enabled** switch to indicate for pemAgent to use the schedule (**Yes**) or to disable the schedule (**No**).
