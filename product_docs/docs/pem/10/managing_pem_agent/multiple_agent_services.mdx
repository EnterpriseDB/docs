...
You can run more than one PEM agent service on a single server. This configuration is useful in environments where multiple independent monitoring contexts are required on the same host — for example, when migrating from Oracle environments where running multiple Oracle Enterprise Manager (OEM) agents per database server is standard practice.
...
1.  Create a directory for the additional agent's configuration and certificates:
    ...
    mkdir -p /etc/edb/pem/agent2/certs
    chmod 700 /etc/edb/pem/agent2/certs
    ```
...
On Windows, the PEM agent is registered as a Windows service during installation. To run multiple agent services on the same host, install the PEM agent into separate installation directories, each in its own directory path.

1.  Download the PEM agent installer.

2.  Run the installer for the first additional agent and, when prompted, specify a unique installation directory (for example, `C:\Program Files\edb\pem\agent2`).

3.  During installation, clear the **Register now** checkbox to skip automatic registration.

4.  After installation, open a Command Prompt as an administrator and register the agent manually:

    ```shell
    set PEM_SERVER_PASSWORD=<pem_admin_password>
    "C:\Program Files\edb\pem\agent2\bin\pemworker.exe" REGISTER ^
      --pem-server <pem_server_ip> ^
      --pem-port 5432 ^
      --pem-user enterprisedb ^
      --display-name "agent-host1-2"
...
5.  Open the Windows Service Manager (`services.msc`), locate the new PEM agent service, set **Startup Type** to **Automatic**, and start the service.

Repeat these steps for each additional agent, using a distinct installation directory and display name for each one.
...
You can also use the `verify-registration` subcommand on Linux to validate each agent:
