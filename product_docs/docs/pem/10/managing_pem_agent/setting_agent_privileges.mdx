...
If the PEM agent connects to the monitored database as a non-superuser, the level of functionality will depend on what exactly permissions the agent user has.
The following sections explain what operations are impacted by Postgres user permissions and what permissions are required for normal operation.

The PEM agent needs to read data from the `pg_catalog` schema for the majority of SQL-based probes. 
In most cases, the `pg_monitor` role is sufficient, however some catalog functions require additional privileges

In addition, the agent user must be able to connect to all databases on which it needs to run probes.
If the agent cannot connect to a database, no probes will be executed on that database, meaning only data which can be collected at a server level (for example from `pg_stat_database` will be collected).

The following table lists probes which require permissions in addition to `pg_monitor`.
