...
PEM includes a set of built-in scheduled system jobs that run automatically on the PEM server. These jobs maintain the PEM database, purge stale data from log and history tables, and perform routine monitoring operations. Unlike [agent-scheduled jobs](managing_pem_agent/managing_scheduled_jobs), system jobs are managed by PEM and can't be modified or deleted by users.

To view system jobs, in the PEM web interface select **Management > Scheduled Tasks**. System jobs are listed alongside any custom agent jobs.
...
| Check CA certificate expiry | Checks the expiry of the CA certificate. |
...
| Refresh Stale Materialized Views | Checks a flag table every minute and refreshes the `pem.probe_target_view` materialized view if it's stale or hasn't been refreshed in one hour. |
...
| Update next run of Check CA certificate expiry Job | Updates the scheduled next-run time for the Check CA certificate expiry job. |
