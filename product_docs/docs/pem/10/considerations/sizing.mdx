...
!!!Note Alert threads
In larger deployments, you need to increase the total number of alert threads in the PEM agent configuration on the PEM hosts. This modification allows PEM to efficiently use more CPU time specifically for alert evaluation, preventing bottlenecks.
...
| Medium                                               | For ≤100 servers,<br/> ~5 concurrent web users;<br/> go higher if >100 k tables/indexes | —         | 6    | 16 GB  | 300 GB  | 500  | Use **pgBouncer**<br/> if heartbeat<br/> connections are enabled |
...
| XX Large                                             | For >600 servers,<br/> - recommend multiple PEM domains                                     | —         | —    | —      | —       | —      | Segment estate<br/> into multiple<br/> PEM deployments           |
