...
description: "Best practices for running PEM server upgrades and SQL remediation steps for privilege-loss errors caused by the PEM-6100 and PEM-6103 bugs."
---
...
This is a concrete walkthrough for the most common non-standard scenario. PEM was installed on the source server owned by `dbsdbadm`; the cluster has been restored on a target server where the corporate standard owner is `ddbadm`, and the goal is to upgrade PEM with `ddbadm` as the final owner.
...
3. Transfer ownership: `REASSIGN OWNED BY dbsdbadm TO ddbadm; GRANT dbsdbadm TO ddbadm;` (step 2).
4. Register defaults `FOR ROLE ddbadm` (step 3).
5. Revoke orphaned defaults `FOR ROLE dbsdbadm` (step 4).
6. Apply the schema USAGE safety net (step 5).
7. Run the RPM upgrade and `configure-pem-server.sh` as `ddbadm` (step 6).
8. Run post-upgrade verification.
9. Start services.

If step 4 is skipped, step 7 re-creates `pem.probe_target_view` and `pem.probe_schedule_view` as `ddbadm`. PostgreSQL finds no defaults for `ddbadm` in `pg_default_acl` (they're still bound to `dbsdbadm`), and the views come up with no grants. The worker log immediately shows `permission denied for view ...` and [Remediation for already-affected clusters](#remediation-for-already-affected-clusters) is required to recover.
