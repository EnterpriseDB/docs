...
title: 'EDB Postgres Enterprise Manager REST APIs v17.0: Custom Probe'
...
curl -X GET https://PEM-SERVER-IP/pem/api/v17/probe/ \
...
r = requests.get('https://PEM-SERVER-IP/pem/api/v17/probe/', headers = headers)
...
fetch('https://PEM-SERVER-IP/pem/api/v17/probe/',
{
  method: 'GET',
...
Gets all the custom/system probes. Supports optional filtering via `internal_name`, `internal_name_contains`, `display_name`, or `display_name_contains` query parameters. Only one filter can be specified at a time.
...
| internal_name          | query  | string | false    | Filter probes by exact internal_name match (case-sensitive). Mutually exclusive with other filters.       |
| internal_name_contains | query  | string | false    | Filter probes by internal_name substring match (case-insensitive). Mutually exclusive with other filters. |
| display_name           | query  | string | false    | Filter probes by exact display_name match (case-sensitive). Mutually exclusive with other filters.        |
| display_name_contains  | query  | string | false    | Filter probes by display_name substring match (case-insensitive). Mutually exclusive with other filters.  |
...
```shell
# You can also use wget
curl -X POST https://PEM-SERVER-IP/pem/api/v17/probe/ \
...
r = requests.post('https://PEM-SERVER-IP/pem/api/v17/probe/', headers = headers)
...
```shell
# You can also use wget
curl -X GET https://PEM-SERVER-IP/pem/api/v17/probe/{probe_id} \
  -H 'Accept: application/json' \
  -H 'X-Auth-Token: string'
...
r = requests.get('https://PEM-SERVER-IP/pem/api/v17/probe/{probe_id}', headers = headers)
...
fetch('https://PEM-SERVER-IP/pem/api/v17/probe/{probe_id}',
{
  method: 'GET',
...
```shell
# You can also use wget
curl -X PUT https://PEM-SERVER-IP/pem/api/v17/probe/{probe_id} \
  -H 'Content-Type: application/json' \
  -H 'X-Auth-Token: string'
...
r = requests.put('https://PEM-SERVER-IP/pem/api/v17/probe/{probe_id}', headers = headers)
...
```shell
# You can also use wget
curl -X DELETE https://PEM-SERVER-IP/pem/api/v17/probe/{probe_id} \
  -H 'X-Auth-Token: string'
...
r = requests.delete('https://PEM-SERVER-IP/pem/api/v17/probe/{probe_id}', headers = headers)
