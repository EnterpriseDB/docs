...
- `pem_assign_profile` or `pem_database_server_registration` role to assign or unassign profiles on servers or agents.
...
A user with `pem_assign_profile` or `pem_database_server_registration` role can unassign profiles from servers or agents.
...
* Users with the `pem_manage_profile` role can delete unassigned profiles or delete assigned profiles by resetting them to defaults. They cannot reassign a new profile to existing targets.
