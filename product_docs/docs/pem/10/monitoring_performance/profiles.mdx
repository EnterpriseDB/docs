...
- Select the `Delete icon` next to the profile you want to remove.
- If the profile has no active assignments, a confirmation dialog appears to finalize the deletion.
- If the profile is actively assigned to any servers or agents, a **Reassign Profile** dialog appears to prevent the loss of monitoring configurations.
- In the dialog, use the dropdown menu to select a replacement profile or choose the option to reset the targets to system default configurations.
- Select `Save` to apply the reassignment and delete the original profile.
