...
Before following the detailed instructions that install the PEM server on your specific platform, you must perform the prerequisite steps detailed in [Prerequisites for installing PEM Server](../04_installing_postgres_enterprise_manager/#prerequisites_for_installing_pem_server).
...
If you want to install PEM server on a machine that is in isolated network, you must first create PEM repository on that machine. For more information about creating PEM repository on an isolated network, see [Creating a PEM repository in an Isolated Network](../04_installing_postgres_enterprise_manager/#creating_pem_repository_in_isolated_network).

After installing PEM Server using `yum` or `dnf`, you need to configure the PEM Server. For more detailed information see [Configuring the PEM Server on Linux platforms](../04_installing_postgres_enterprise_manager/#configuring_the_pem_server_on_linux).
...
2.  Configure the EDB repository:
    ...
    -   For Debian 10:
        ...
        2.  Substitute your EnterpriseDB credentials for the `username` and `password` placeholders in the following command:
...
When the installation completes, you must configure the PEM Server. For detailed information see [Configuring the PEM Server on Linux Platforms](../04_installing_postgres_enterprise_manager/#configuring_the_pem_server_on_linux).
...
After installing PEM Server using `zypper`, you must configure the PEM Server. For detailed information see [Configuring the PEM Server on Linux Platforms](../04_installing_postgres_enterprise_manager/#configuring_the_pem_server_on_linux).
