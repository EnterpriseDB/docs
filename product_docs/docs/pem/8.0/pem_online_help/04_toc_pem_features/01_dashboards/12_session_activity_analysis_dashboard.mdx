...
-   The `IO Reads (#bytes)` column displays the number of bytes used by the session while reading the transactions; this column is not displayed if the server is remotely monitored.
-   The `IO Writes (#bytes)` column displays the number of bytes used by the session while writing the transactions; this column is not displayed if the server is remotely monitored.
