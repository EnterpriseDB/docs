...
Use parameters on the [PEM Server Configurations](../../04_toc_pem_features/02_pem_server_config/#pem_server_config) dialog to specify the auto-refresh rate for the `Session Activity Analysis` dashboard. To access the `Server Configuration` dialog, select `Server Configuration...` from the PEM client `Management` menu.
...
> -   `advisory` - a user-defined lock created by pg\_advisory\_lock() or pg\_advisory\_lock\_shared()
