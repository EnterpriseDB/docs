...
You can use the PEM Audit manager to configure, enable, and disable audit logging of EDB Postgres Advanced Server instances. The Audit manager also enables audit log collection, allowing you to view log data on the [Audit Log Dashboard](../04_toc_pem_features/01_dashboards/#audit_log_dashboard).
...
The scheduled jobs can be viewed in the [Task Viewer](../04_toc_pem_features/18_pem_task_view/#pem_task_view), and the results in the [Log Viewer](../04_toc_pem_features/18_pem_task_view/#pem_log_view) when opened from the appropriate server or agent.
