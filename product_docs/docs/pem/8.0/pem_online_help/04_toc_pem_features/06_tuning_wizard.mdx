...
Before using the Tuning Wizard, you must specify the name of the service in the `Service ID` field on the `Advanced` tab of the server's `` `Properties `` &lt;pem\_define\_connection&gt;\` dialog. PEM will use the service name when restarting the service after tuning.
