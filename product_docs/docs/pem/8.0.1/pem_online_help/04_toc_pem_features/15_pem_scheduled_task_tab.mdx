...
!!! Note
    Tasks with no `Next run` date will automatically be removed from the PEM server when the last run date is more than [probe_log_retention_time](02_pem_server_config/01_pem_config_options/#pem_config_options) days ago.
