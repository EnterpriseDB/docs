...
To connect to a server, the pg\_hba.conf file on the database server must be configured to accept connections from the host of the pgAdmin client. Modify the pg\_hba.conf file on the database server host, and add an entry in the form:
