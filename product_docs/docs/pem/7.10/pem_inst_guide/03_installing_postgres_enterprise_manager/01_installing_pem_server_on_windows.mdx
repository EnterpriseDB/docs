...
On Linux, the service script must reside in the `/etc/init.d` directory. The service script must be able to start, stop and restart the database server. Service scripts are platform-specific; you can find a sample service script for Linux in [Linux Service Script (Sample)](../07_reference_linux_service_script/#reference_linux_service_script) section. For information about customizing a Postgres service, visit:
