...
Replace `<top-level group name>` with the name of your cluster’s top-level group. If you don't know the name, it's the group with a node_group_parent_id equal to 0 in [`bdr.node_group`](/pgd/latest/reference/catalogs-visible#bdrnode_group). 
