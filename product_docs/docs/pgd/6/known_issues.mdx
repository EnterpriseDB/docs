...
-   Parallel Apply isn't currently supported in combination with Group Commit. Make sure to disable it when using Group Commit by either (a) Setting `num_writers` to 1 for the node group using [`bdr.alter_node_group_option`](/pgd/6/reference/tables-views-functions/nodes-management-interfaces/#bdralter_node_group_option) or (b) using the GUC [`bdr.writers_per_subscription`](/pgd/6/reference/tables-views-functions/pgd-settings#bdrwriters_per_subscription). See [Configuration of generic replication](/pgd/6/reference/tables-views-functions/pgd-settings#generic-replication).
...
-   The [PGD CLI](/pgd/6/reference/cli) can return stale data on the state of the cluster if it's still connecting to nodes that were previously parted from the cluster. 
    Edit the [`pgd-cli-config.yml`](/pgd/6/reference/cli/configuring_cli/#using-a-configuration-file) file, or change your [`--dsn`](/pgd/6/reference/cli/configuring_cli/#using-database-connection-strings-in-the-command-line) settings to ensure only active nodes in the cluster are listed for connection.

To modify a commit scope safely, use [`bdr.alter_commit_scope`](/pgd/6/reference/tables-views-functions/functions#bdralter_commit_scope).
...
-   EDB Postgres Advanced Server's native autopartioning is not supported in PGD. See [Restrictions on EDB Postgres Advanced Server-native automatic partitioning](autopartition/#restrictions-on-edb-postgres-advanced-server-native-automatic-partitioning) for more information.
...
-   The minimum recommended number of nodes in a group is three to provide fault
    ...
    details](overview/basic-architecture/).
...
-   [Legacy synchronous replication](commit-scopes/legacy-sync/) uses a mechanism for transaction confirmation
...
[Group Commit](commit-scopes/group-commit/) enables configurable synchronous commits over 
...
[Eager](commit-scopes/group-commit/#eager-conflict-resolution) is available through Group Commit. It avoids conflicts by eagerly aborting transactions that might clash. It's subject to the same limitations as Group Commit. 
...
[Commit At Most Once](commit-scopes/camo/) (CAMO) is a feature that aims to prevent
...
PGD was developed to [enable rolling upgrades of PGD](/pgd/6/reference/upgrades) by allowing mixed versions of PGD to operate during the upgrade process. 
...
[https://www.enterprisedb.com/docs/pgd/latest/reference/appusage/behavior/#replication-behavior](appusage/behavior/#replication-behavior)
and
