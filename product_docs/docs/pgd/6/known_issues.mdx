...
-   If the resolver for the `update_origin_change` conflict
      ...
      [Conflicts](/pgd/6/reference/conflict-management/conflicts/), `skip` isn't the default
...
-   Group Commit can't be combined with [CAMO](/pgd/6/reference/commit-scopes/camo/). 
...
-   EDB Postgres Advanced Server's native autopartioning is not supported in PGD. See [Restrictions on EDB Postgres Advanced Server-native automatic partitioning](/pgd/6/reference/autopartition/#restrictions-on-edb-postgres-advanced-server-native-automatic-partitioning) for more information.
...
-   The minimum recommended number of nodes in a group is three to provide fault
    ...
    details](/pgd/6/reference/overview/basic-architecture/).
...
These limitations are a product of the interactions between Group Commit and CAMO, and how they interact with PGD features such as the [WAL decoder](/pgd/6/reference/decoding_worker/) and  [transaction streaming](/pgd/6/reference/transaction-streaming/).
...
-   [Legacy synchronous replication](/pgd/6/reference/commit-scopes/legacy-sync) uses a mechanism for transaction confirmation
...
[Group Commit](/pgd/6/reference/commit-scopes/group-commit) enables configurable synchronous commits over 
...
[Eager](/pgd/6/reference/commit-scopes/group-commit/#eager-conflict-resolution) is available through Group Commit. It avoids conflicts by eagerly aborting transactions that might clash. It's subject to the same limitations as Group Commit. 
...
[Commit At Most Once](/pgd/6/reference/commit-scopes/camo) (CAMO) is a feature that aims to prevent
...
-   CAMO isn't currently compatible with transaction streaming.
    ...
    [Transaction streaming configuration](/pgd/6/reference/transaction-streaming#configuration).
...
-   CAMO isn't currently compatible with decoding worker.
    ...
    [Decoding worker disabling](/pgd/6/reference/decoding_worker#enabling).
