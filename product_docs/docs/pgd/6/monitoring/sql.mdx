...
something is down or disconnected. See [Replication slots](/pgd/current/node_management/replication_slots/) in Node Management.
...
serving a [logical standby](/pgd/current/nodes/logical_standby_nodes/).
...
Replication [conflicts](/pgd/current/conflict-management/conflicts/) can arise when multiple nodes make
...
PGD node replication slots are working as expected. This summary is also available on subscriber-only nodes that are operating as subscriber-only group leaders in a PGD cluster when [optimized topology](/pgd/current/nodes/subscriber_only/optimizing-so/) is enabled. For example:
