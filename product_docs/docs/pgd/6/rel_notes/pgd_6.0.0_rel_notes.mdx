...
<tr><td><details><summary>Subscriber-only nodes will not take a lock when running DDL</summary><hr/><p>Subscriber-only nodes will no longer attempt to take a lock when running DDL. The DDL will be executed locally and not replicated to other nodes.</p>
