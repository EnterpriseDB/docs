...
For details, see [Replication sets](/pgd/6/reference/repsets).
...
[Security and roles](/pgd/6/reference/security) for details.
...
replication](/pgd/6/reference/ddl) to specify how to alter a role password on only a subset of
nodes or locally.
...
notes](/pgd/6/rel_notes/) for any known incompatibilities.