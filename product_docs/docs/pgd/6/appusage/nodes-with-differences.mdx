...
For details, see [Replication sets](/pgd/current/repsets/).
...
[Security and roles](/pgd/current/security/) for details.
...
replication](/pgd/current/ddl/) to specify how to alter a role password on only a subset of
nodes or locally.
