...
In this example, we will configure the PGD Expanded cluster with EDB Postgres Extended Server 17 on a CentOS/RHEL system using an enterprise subscription that we [configured](02-configure-repository) and [installed](03-installing-database-and-pgd) in the previous steps.
...
pgd node node-1 setup "host=host-1 user=postgres port=5432 dbname=pgddb" --group-name group1 --update-pgpass
```
