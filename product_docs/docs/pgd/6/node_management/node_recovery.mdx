...
When a node starts up, each connection begins showing up in [`bdr.node_slots`](/pgd/6/reference/tables-views-functions/catalogs-visible#bdrnode_slots) with
...
(see [Origin conflict detection](/pgd/6/reference/conflict-management/conflicts)).

Administrators must monitor for node outages (see [Monitoring](/pgd/6/reference/monitoring/))
