...
-   Increase the maximum worker processes to 16 or higher by setting `max_worker_processes` to `'16'` in `postgresql.conf`.<br/><br/>
    ...
    To calculate the needed value, see [Postgres configuration/settings](/pgd/6/reference/postgres-configuration/#postgres-settings).
...
You can find instructions for joining a node to a group in the [Joining a node to a group](/pgd/6/reference/node_management/creating_and_joining) section.
