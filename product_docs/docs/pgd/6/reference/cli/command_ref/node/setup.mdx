...
When `pgd node setup` performs a physical join (copying data from the remote node when the local node is not up and running), it requires that both the source node and the joining node have exactly the same PGD version. You cannot use physical joins to join a node with a different PGD version to an existing cluster. For rolling upgrades, ensure you use logical joins instead. See [Rolling upgrade using node join](/pgd/6.1/upgrades/manual_overview/#rolling-upgrade-using-node-join) for more details.
!!!
...
The `pgd node setup` command supports this feature when initializing a new database server or joining an existing one. It exposes TDE options that are similar to the standard [initdb TDE options] (/tde/latest/initdb_tde_options), with a key difference concerning the `--data-encryption` flag.

Unlike `initdb`, the `pgd node setup` command treats `--data-encryption` as a `boolean-only` option (it doesn't accept the AES key length value). Instead, the key length must be specified separately using the dedicated `−−data−encryption−keylen` option.
...
| `-y`, `--data-encryption`                                                                                                                                                                                                                                                              | Adds transparent data encryption when initializing a database server                                                                                                                                                                                             |
| `--data-encryption-keylen <AES_KEYLEN>`                                                                                                                                                                                                                                                | The AES key length for TDE. Default is `128`. Possible values are `128` and `256`.                                                                                                                                                                               |
| `--key-wrap-command <KEY_WRAP_COMMAND>`                                                                                                                                                                                                                                                | The wrapping/encryption command to protect the data encryption key The command is customizable, but it must contain the placeholder %p. Also set with environment variable `PGDATAKEYWRAPCMD`.                                                                   |
| `--key-unwrap-command <KEY_UNWRAP_COMMAND>`                                                                                                                                                                                                                                            | The unwrapping/decryption command to access the data encryption key The command is customizable, but it must contain the placeholder %p. Also set with environment variable `PGDATAKEYUNWRAPCMD`.                                                                |
| `--no-key-wrap`                                                                                                                                                                                                                                                                        | Disable key wrapping. Not recommended for production use.                                                                                                                                                                                                        |
| `--copy-key-from <COPY_KEY_FROM>`                                                                                                                                                                                                                                                      | Copy an existing data encryption key from the provided location Normally, encryption keys are stored in `pg_encryption/key.bin`                                                                                                                                  |
