...
```shell
pgd --dsn "host=kaboom port=5444 user=enterprisedb dbname=pgddb" nodes show --versions
```
...
cluster:
  name: cluster-name
  endpoints:
  - "host=bdr-a1 port=5444 dbname=pgddb user=enterprisedb"
  - "host=bdr-b1 port=5444 dbname=pgddb user=enterprisedb"
  - "host=bdr-c1 port=5444 dbname=pgddb user=enterprisedb"
```
