...
Starting with PGD 6.0.1, when a PGD node is manually joied to a PGD group without using `bdr_init_physical`, existing roles are automatically copied to the newly joined node. This means that you no longer need to create roles manually on the new node before joining it to the group.
