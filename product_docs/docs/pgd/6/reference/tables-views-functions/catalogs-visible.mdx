...
For details, see [Logging conflicts to a table](/pgd/6/reference/conflict-management/conflicts).
...
This table tracks the status of logical replication slots that are being used with failover support. For more information on failover replication slots, see [CDC Failover support](/pgd/6/reference/cdc-failover).
...
| subscription_mode   | char                     | Mode of the subscription, see [`bdr.subscription_summary`](/pgd/6/reference/tables-views-functions/catalogs-visible/#bdrsubscription_summary) for more details
