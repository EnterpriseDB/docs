...
By default and since version 6.0, PGD does not log conflicts to the `bdr.conflict_history` table. This is because the table can grow large and cause performance issues. You can enable conflict logging by using the [bdr.alter_node_set_log_config](/pgd/latest/reference/tables-views-functions/conflict_functions/#bdralter_node_set_log_config) function. This function gives you granular control over which conflicts to log, or you can set it to log all conflicts like this:

```sql
SELECT bdr.alter_node_set_log_config(`nodename`, false, true, NULL, NULL);
```

This command needs to be run on the named node and it will enable logging of all conflicts on that particular node, and only that node. If you want to enable logging on all nodes, you need to run this command on each node in the PGD group.
