...
| `delete_recently_updated` | An incoming delete with an older commit timestamp than the most recent update of the row on the current node or when using [row version conflict detection](/pgd/6/reference/conflict-management/conflicts/03_conflict_detection/#row-version-conflict-detection). |
