...
| `enable_routing`    | `boolean`  | Where [`Connection Manager`](/pgd/latest/reference/connection-manager/) through the group write leader is enabled for given group. Valid values are `on` or `off`. Default is `on` for subgroups and off for the cluster group.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `node_group_type`   | The valid values are `NULL` or  `subscriber-only`. `NULL` (the default) is for creating a normal, general-purpose node group. `subscriber-only` is for creating [subscriber-only groups](/pgd/latest/reference/nodes/subscriber_only/) whose members receive changes only from the fully joined nodes in the cluster but that never send changes to other nodes. |
...
Removes (parts) the node from the PGD group and eventually removes the parted node’s  metadata automatically from all nodes in the cluster.

* For the local node, it will remove all the node metadata, including information about remote nodes.
* For remote nodes, it removes only the metadata for that specific node.

This operation does not remove data from the node.
...
Cancels a sync request for the specified origin and source nodes.
...
This can be invoked only from a write lead.
