...
If you are using [Optimized Topology](/pgd/6/reference/nodes/subscriber_only/optimizing-so), we recommend using [`bdr.wait_node_confirm_lsn`](/pgd/6/reference/tables-views-functions/functions#bdrwait_node_confirm_lsn) instead.
)
...
See [DDL locking details](/pgd/6/reference/ddl/ddl-locking.mdx) for information
...
This function also provides status information on subscriber-only nodes that are operating as subscriber-only group leaders in a PGD cluster when [optimized topology](/pgd/6/reference/nodes/subscriber_only/optimizing-so) is enabled.
