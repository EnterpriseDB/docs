...
* The `generation` field in the `bdr.node` table, which was previously used to differntiate between nodes is no longer used. It will remain at 0 for all nodes. 
