...
When the first PGD node group is created a default replication set is also created and all existing tables are added 
into the set. After that, all tables will have the `bdr.default_replica_identity` value configured as the 
`REPLICA IDENTITY` for the table.
