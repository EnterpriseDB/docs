...
This sequence results in an `UPDATE-UPDATE` conflict. With the [`update_if_newer`](/pgd/6/reference/tables-views-functions/conflicts/#default-conflict-resolvers) conflict resolution, the commit timestamps are compared, and the new row version is kept. Assuming the second node committed last, the result is `(1,100)`, which effectively discards the change to column a.
