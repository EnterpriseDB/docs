...
By default, all conflicts are logged to [`bdr.conflict_history`](/pgd/6/reference/tables-views-functions/catalogs-visible/#bdrconflict_history). If conflicts are possible, then table owners must monitor for them and analyze how to avoid them or make plans to handle them regularly as an application task. The [LiveCompare](/livecompare/latest) tool is also available to scan regularly for divergence.
...
For `INSERT`/`INSERT` conflicts, use [global sequences](/pgd/6/reference/sequences#pgd-global-sequences) to prevent this type of conflict.
