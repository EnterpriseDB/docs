...
If you want to avoid conflicts, you can use [Group Commit](/pgd/6/reference/commit-scopes/group-commit/) with [Eager conflict resolution](/pgd/6/reference/commit-scopes/group-commit/#eager-conflict-resolution) or conflict-free data types (CRDTs), described in [CRDT](crdt). You can also use Connection Manager to route all writes to one write-leader, eliminating the chance for inter-nodal conflicts.
