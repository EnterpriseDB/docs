...
`barman recover` handles this automatically. Manual restores must set the value before starting Postgres. To find the runtime value from a healthy peer:
...
A physical backup taken through `pg_basebackup --wal-method=stream` doesn't include logical replication slots. The same applies to backups taken through `barman backup` with `backup_method = postgres`, because Barman calls `pg_basebackup` under the hood. After a `barman recover` or a manual `pg_basebackup` restore, you must recreate the slots on each restored node before BDR replication resumes.
...
Run this on every restored node. Once the slots exist on each node, the `pgd manager` background worker stops crash-looping and the per-peer slots transition to `active = t` within a few seconds. Verify with:
...
!!! Warning
    Don't call `bdr.initialize_replication_slot()` to recreate slots after a restore. The function exists, but the slots it creates use the peer's `node_name` rather than the `node_uuid` form that BDR subscribers consume. The resulting slots are never used, and they can't be removed afterward — `pg_drop_replication_slot()` rejects them with `cannot drop replication slot as it is created automatically by BDR`.

#### Extension catalog tables that aren't in any replication set

Some extensions — for example, [PGAA](/pgaa/latest/) — register catalog tables with `set_name = NULL` in `bdr.tables`, so they are **not** logically replicated. They stay consistent at runtime through PGD's global DDL lock and the extension's per-node `object_access_hook`, not through the replication stream.

For backup, this means **each node's basebackup contains its own physical copy** of those tables. Cluster-wide consistency requires a coordinated basebackup from every node. A single-node basebackup only captures that node's view.

After a multi-node restore, compare row counts and a digest of each table's contents across nodes. For PGAA specifically, see [Backup and restore](/pgaa/latest/backup_restore/).
