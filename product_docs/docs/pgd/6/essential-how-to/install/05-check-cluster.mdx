...
On any of the installed and configured nodes, log in and run `psql` to connect to the database. If you are using EDB Postgres Advanced Server, use the `enterprisedb` user, otherwise use `postgres`. If you configured the cluster with the previous steps you can use the pgddb database and pgdadmin user:

```bash
sudo -iu postgres psql "host=host-1 port=5432 username=pgdadmin dbname=pgddb"
```
...
```
ssh admin@host-2
sudo -iu postgres psql "host=host-2 port=5432 username=pgdadmin dbname=pgddb"
```
