...
For PGD Essential, there is just one repository to install.

```bash
curl -1sSLf "https://downloads.enterprisedb.com/$EDB_SUBSCRIPTION_TOKEN/standard/setup.$EDB_REPO_TYPE.sh" | sudo -E bash
```
