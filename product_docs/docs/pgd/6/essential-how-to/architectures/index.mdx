...
!!!Note For multi-region deployments
   For multi-region deployments, geo-distributed architectures are available in [PGD Expanded](/pgd/6/expanded-how-to/architectures/). These architectures are designed for use cases that require data to be distributed across multiple regions or data centers. They provide advanced features such as conflict resolution, data distribution, and support for large-scale deployments.
   For more information on PGD Expanded, see the [Expanded how-to](/pgd/6/expanded-how-to).
