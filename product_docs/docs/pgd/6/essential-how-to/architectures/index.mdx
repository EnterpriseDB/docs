...
In the event of a complete failure in the primary location, the secondary location's database has a complete replica of the data. Applications should be configured to automatically failover to the secondary location and all writes will be directed to that node. When the primary location comes back online, it will automatically rejoin the cluster and begin replicating data from the secondary location.
