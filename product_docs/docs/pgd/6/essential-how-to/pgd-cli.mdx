...
There are [options](/pgd/6/reference/cli/using_cli) that allow you to override this to use alternative configuration files or explicitly point at a server. But, by default, PGD CLI looks for a configuration file in preset locations.
...
Also consult the [PGD CLI documentation](/pgd/6/reference/cli/) for details of other configuration options and a full command reference.
...
The [`cluster show --health`](/pgd/6/reference/cli/command_ref/cluster/show/#options) command provides a quick way to view the health of the cluster:
...
As previously seen, the [`nodes list`](/pgd/6/reference/cli/command_ref/nodes/list/) command lists the nodes in the cluster:
...
Finally, the [`groups list`](/pgd/6/reference/cli/command_ref/groups/list/) command for PGD CLI shows which groups are configured, and more:
...
You can set group options using PGD CLI, too, using the [`group set-option`](/pgd/6/reference/cli/command_ref/group/set-option/) command. 
...
You can verity the new location using the [`group get-option`](/pgd/6/reference/cli/command_ref/group/get-option/) command:
...
If you need to change write leader in a group, to enable maintenance on a host, PGD CLI offers the [`group set-leader`](/pgd/6/reference/cli/command_ref/group/set-leader/) command.
...
You can verify the write leader using the [`group show command`](/pgd/6/reference/cli/command_ref/group/show/) with the `--summary option`:
...
More details on the available commands in PGD CLI are available in the [PGD CLI command reference](/pgd/6/reference/cli/command_ref/).
