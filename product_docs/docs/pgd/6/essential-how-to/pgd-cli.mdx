...
The `pgd assess` command will not perform the below listed assessments for of a Postgres server for PGD Essential compatibility.
...
The `pgd commit-scope create`, `pgd commit-scope update`, and `pgd commit-scope drop` commands are not supported in PGD Essential and exit with below advice.
```shell
