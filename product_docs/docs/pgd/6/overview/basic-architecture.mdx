...
You can run PGD without Connection Manager if you need a custom deployment better utilizing the multi-master functionality. When running without Connection Manager, writes are distributed amongst the nodes, replicated to one another, and conflict resolution is relied upon for maintaining consistency.
