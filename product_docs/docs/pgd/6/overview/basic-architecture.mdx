...
redirects:
  ...
  - /pgd/latest/reference/overview/basic-architecture/ #generated for DOCS-3034
---
...
[Logical replication](/pgd/6/terminology/#logical-replication) in PGD is asynchronous by default, so only eventual consistency is guaranteed (within seconds usually). 
However, [commit scope](/pgd/6/reference/commit-scopes/commit-scopes) options offer immediate consistency and durability guarantees via [CAMO](/pgd/6/reference/commit-scopes/camo/), [group](/pgd/6/reference/commit-scopes/group-commit) and [synchronous](/pgd/6/reference/commit-scopes/synchronous_commit) commits.
...
PGD comprises several key architectural elements that work together to provide its distributed database solution:
  ...
  - **Monitoring tools**: To monitor performance, health, and usage with PGD, you can use its [built-in command-line interface](/pgd/6/reference/cli) (CLI), which offers several useful commands. For example: 
    - The [`pgd nodes list`](/pgd/6/reference/cli/command_ref/nodes/list/) command provides a summary of all nodes in the cluster, including their state and status. 
    - The [`pgd cluster show --health`](/pgd/6/reference/cli/command_ref/cluster/show/#options) command checks the health of the cluster, reporting on node accessibility, replication slot health, and other critical metrics. 
    - The [`pgd events show`](/pgd/6/reference/cli/command_ref/events/show/) command lists significant events like background worker errors and node membership changes, which helps in tracking the operational status and issues within the cluster. 
...
All nodes in PGD are effectively data nodes. They vary only in their purpose in the cluster. 
  ...
  - **[Data nodes](/pgd/6/reference/nodes/overview#data-nodes)**: Store and manage data, handle read and write operations, and participate in replication.

There are then three types of nodes which, although built like a data node, have a specific purpose. These are:

  - **[Subscriber-only nodes](/pgd/6/reference/nodes/subscriber_only/)**: Subscribe to changes from data nodes for read-only purposes. Used in reporting or analytics.

  - **[Witness nodes](/pgd/6/reference/nodes/witness_nodes/)**: Participate in the consensus process without storing data, aiding in achieving quorum and maintaining high availability.

  - **[Logical standby nodes](/pgd/6/reference/nodes/logical_standby_nodes/)**: Act as standby nodes that can be promoted to data nodes if needed, ensuring high availability and disaster recovery.
...
These roles can include:
  ...
  - **[Write leader](/pgd/6/terminology/#write-leader)**: Receives all write operations when applications connect through the connection manager.
...
- **[Synchronous Commit](/pgd/6/reference/commit-scopes/synchronous_commit.mdx)**: Works a lot like PostgreSQL’s synchronous_commit option in its underlying operation. Requires writing to at least one other node at COMMIT time but can be tuned to require all nodes.

- **[CAMO](/pgd/6/reference/commit-scopes/camo.mdx)** (Commit At Most Once): Works by tracking each transaction with a unique ID and using a pair of nodes to confirm the transaction's outcome, ensuring the application knows whether to retry the transaction or not.

- **[Group Commit](/pgd/6/reference/commit-scopes/group-commit.mdx)**: An experimental commit scope, the goal of which is to protect against data loss in case of single-node failures of temporary outages by requiring more than one PGD node to successfully confirm a transaction at COMMIT time.

- **[Lag Control](/pgd/6/reference/commit-scopes/lag-control.mdx)**: If replication is running outside of set limits (taking too long for another node to be replicated to), a delay is injected into the node that originally received the transaction, slowing things down until other nodes have caught up.
