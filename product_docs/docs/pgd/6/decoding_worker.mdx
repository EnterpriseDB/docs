...
disabled by default. You can use [`bdr.alter_node_group_option()`](/pgd/6/reference/tables-views-functions/nodes-management-interfaces/#bdralter_node_group_option) to enable or
...
[`bdr.lcr_cleanup_interval`](/pgd/6/reference/tables-views-functions/pgd-settings#bdrlcr_cleanup_interval), which defaults to 3 minutes. The cleanup is
disabled when [`bdr.lcr_cleanup_interval`](/pgd/6/reference/tables-views-functions/pgd-settings#bdrlcr_cleanup_interval) is 0.
...
-   [`bdr.enable_wal_decoder`](/pgd/6/reference/tables-views-functions/pgd-settings#bdrenable_wal_decoder) &mdash; When `false`, all WAL
...
-   [`bdr.receive_lcr`](/pgd/6/reference/tables-views-functions/pgd-settings#bdrreceive_lcr) &mdash; When `true` on the subscribing node, it requests WAL
...
To enable transaction streaming with the WAL decoder, set the PGD group's `bdr.streaming_mode` set to ‘default’ using [`bdr.alter_node_group_option`](/pgd/6/reference/tables-views-functions/nodes-management-interfaces#bdralter_node_group_option).
