...
When upgrading to PGD 6.3+, if you use Connection Manager HTTP endpoints and have `ssl_cert_file` and `ssl_key_file` set in `postgresql.conf` (or via the command line), you must explicitly enable HTTPS before upgrading to maintain previous behavior:
