...
In PGD 6.3.0, Connection Manager introduces stricter requirements for HTTP endpoints when SSL is configured.

If you use Connection Manager HTTP endpoints and have `ssl_cert_file` and `ssl_key_file` set in `postgresql.conf` (or via the command line), you must explicitly enable HTTPS before upgrading to maintain previous behavior:

```sql
pgd group <GROUP_NAME> set-option use_https true
```

For more details see [Connection Manager](/pgd/current/connection-manager/).
...
When creating a PGD cluster, existing [sequences](/pgd/current/sequences/) are automatically converted to distributed ones.
