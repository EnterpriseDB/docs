...
- **Per transaction:** Use `SET LOCAL` inside a transaction to override the scope for that transaction only. The session-level setting is restored when the transaction ends:
    ...
    ```sql
    BEGIN;
    SET LOCAL bdr.commit_scope = 'majority_protect';
...
Regardless of commit scope, if your application writes on one connection and immediately reads on another, the read can land on a node that hasn't yet received the replication. To guarantee that a subsequent read reflects a prior write in the same session, call `bdr.wait_slot_confirm_lsn(NULL, NULL)` after the write:

```sql
-- Insert a new transaction using an existing active account
...
-- Wait for replication to all nodes before proceeding
SELECT bdr.wait_slot_confirm_lsn(NULL, NULL);

-- This read now reflects the insert, regardless of which node serves it
SELECT balance_after
FROM transactions
WHERE account_id = (SELECT account_id FROM accounts WHERE status = 'active' LIMIT 1)
ORDER BY initiated_at DESC
LIMIT 1;
```

`bdr.wait_slot_confirm_lsn(NULL, NULL)` blocks until all PGD replication slots have confirmed the current LSN. The delay is proportional to network latency to the farthest node in the cluster. Use it selectively for operations that genuinely need it, not on every query.
