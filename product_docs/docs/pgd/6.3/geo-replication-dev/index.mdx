...
- **A failed commit can't safely be retried**: if a connection drops while a commit is in flight, the transaction outcome is unknown. It could have committed on some nodes already.
...
Schema changes in a geo-distributed cluster acquire locks across all nodes, not just locally, which makes them slower and more disruptive than in a single-location setup. Avoid mixing DDL with DML, keep DDL transactions small, and use `bdr.replicate_ddl_command()` for controlled execution.
