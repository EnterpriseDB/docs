...
PGD is implemented as a PostgreSQL extension (see [Supported Postgres database servers](/pgd/current/planning/architecture/architecture-and-performance/#supported-postgres-database-servers)). It takes advantage of PostgreSQL's expandability and flexibility to modify low-level system behavior to provide multi-master replication.
