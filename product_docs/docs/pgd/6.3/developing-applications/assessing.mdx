...
By default, outgoing logical replication to a data warehouse or event streaming pipeline is tied to a single PGD node. Logical replication slots aren't replicated across the cluster unless CDC Failover support is explicitly enabled. With CDC Failover support enabled, slots are replicated across nodes, but failover isn't fully transparent. The consumer must support reconnecting to a new node. See [CDC Failover support](/pgd/current/cdc-failover/) for details.
