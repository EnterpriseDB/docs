...
| `concurrent`          | If `true`, allows reuse of parting node name as soon as node is in `PARTING` state or above, without waiting for node to reach the `PARTED` state eventually dropped. Default is `false`. This parameter is available when the cluster is at Raft protocol version 6003 or above. |
