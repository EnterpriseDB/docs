...
[DDL locking details](https://www.google.com/search?q=/pgd/current/ddl/ddl-locking/)
...
| `kind` | The lease kind (for example, `'a'` for analytics). | 
...
| `ddl_locking` | A string that sets the <br/>[`bdr.ddl_locking`](https://www.google.com/search?q=/pgd/latest/reference/tables-views-functions/pgd-settings%23bdrddl_locking)<br/> value while replicating. Defaults to the GUC value for `bdr.ddl_locking` on the local system that's running `replicate_ddl_command`. |
...
[`bdr.ddl_replication`](https://www.google.com/search?q=./pgd-settings%23bdrddl_replication)
.
...
[bdr.wait\_for\_camo\_partner\_queue](https://www.google.com/search?q=%23bdrwait_for_camo_partner_queue)
.
...
[Optimized Topology](https://www.google.com/search?q=../../nodes/subscriber_only/optimizing-so)
 instead of 
[`bdr.wait_slot_confirm_lsn`](https://www.google.com/search?q=/pgd/6/reference/tables-views-functions/functions%23bdrwait_slot_confirm_lsn)
.
...
If you are using 
[Optimized Topology](https://www.google.com/search?q=/pgd/current/nodes/subscriber_only/optimizing-so/)
, we recommend using 
[`bdr.wait_node_confirm_lsn`](https://www.google.com/search?q=/pgd/latest/reference/tables-views-functions/functions%23bdrwait_node_confirm_lsn)
 instead.
