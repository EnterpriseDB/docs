...
While PGD 6.2.0 introduced `RENAME` and `SET SCHEMA` functionality for `galloc` sequences, the feature did not work for sequences created in earlier versions, as PGD used a global UUID identifier to track `galloc` sequences generated at the time of sequence creation, but it was missing for upgraded sequences.

PGD 6.3.0 removes that limitation as BDR manager generates the UUID for sequences if it is missing.
