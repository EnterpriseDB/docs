...
Starting with PGD 6.3, `ALTER SEQUENCE ... RENAME` and `ALTER SEQUENCE ... SET SCHEMA` are fully supported for all sequence kinds. This support is enabled by the `sequuid` (UUID) identifier in the `bdr.sequence_kind` catalog, which allows PGD to track sequences consistently across name and schema changes.

For clusters upgraded from versions prior to 6.3, the PGD automatically generates the required UUIDs for existing sequences to ensure compatibility.
