...
Starting with PGD 6.3.0, `ALTER SEQUENCE` is fully supported across all sequence types. 

PGD uses a global UUID identifier to track `galloc` sequences during `RENAME` or `SET SCHEMA` operations. While these identifiers were introduced in PGD 6.2.0, they were missing for sequences created in earlier versions, causing those operations to fail. PGD 6.3.0 removes this limitation automatically generating the required UUIDs for all upgraded sequences.
