...
The Raft majority concern and the galloc sequence exhaustion issue can be mitigated by adding a third location, possibly in cloud, containing a single witness node. The witness participates in Raft consensus but doesn't store user data. This approach provides an odd-number quorum (five voters: four data nodes and one witness), keeping consensus working even during loss of a single DC.
