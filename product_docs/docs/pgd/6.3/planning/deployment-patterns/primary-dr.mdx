...
The DR site receives asynchronous replication as a warm standby. During DR failover, applications switch to the single DR node at reduced capacity until the primary is restored.
...
- Steady state: two nodes active in the primary, one node receiving replication in the DR location (warm standby).
...
- The primary group can have consistency issues on cascading failures or recovery.
