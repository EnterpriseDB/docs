...
This procedure applies to PGD clusters using any [synchronous commit scope](/pgd/current/deep-dive/commit-scopes/). It covers:
...
- The [`bdr_superuser` role](/pgd/current/deep-dive/security/) or equivalent to run `pgd` and SQL commands on the cluster.
...
Check for resource constraints (CPU, disk I/O, memory), verify network bandwidth between nodes, and confirm that the Postgres configuration allows sufficient workers and replication slots (see [Postgres configuration](/pgd/current/deep-dive/postgres-configuration/)).
