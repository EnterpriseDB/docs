...
!!! Note 
  Enabling the `use_https` option requires `ssl_cert_file` and `ssl_key_file` to be configured in `postgresql.conf` (or via the command line), though global server SSL (`ssl = on`) is not required. If these certificates are missing, Connection Manager silently falls back to plaintext HTTP.
!!!