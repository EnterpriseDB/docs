...
In this phase, the seed node is initialized by creating a standby copy of the database on the seed node that mirrors the source node via physical replication. This standby serves as the seed node for the eventual PGD cluster. To minimize interference with the existing cluster—particularly if it is managed by Failover Manager (EFM)—this replica is added manually. 

## Preparing the source node for logical replication

The seed node must be prepared to transition to logical replication in [Converting to logical replication](3-convert-to-logical-replication). If not already enabled, you must set the `wal_level` to `logical` on the source node. Since this change requires a database restart, it is best to perform this step ahead of time.
...
1. Verify connectivity from the PGD nodes using the following command to ensure they can connect without interactive password entry:
    ...
    ```bash
    # Test connectivity with REPLICATION permissions from all PGD nodes
...
Install the same distribution and version of Postgres on the seed node to accommodate a physical backup. See [Installing EPAS](/epas/latest/installing/), [Installing PGE](/pge/latest/installing/), or [Installing PostgreSQL](/supported-open-source/postgresql/installing/) for details.
...
The seed node is now ready to receive a physical snapshot of the database. Ensure the `${PGDATA}` directory does not exist before running the transfer.
...
The transfer duration depends on the total volume of data and the network bandwidth between the source and seed nodes. For environments with limited bandwidth, you can enable compression using the `--compress-level=[0-9]` flag to optimize the transfer speed; refer to the official [pg_basebackup](https://www.postgresql.org/docs/14/app-pgbasebackup.html) documentation for detailed configuration options.
...
- Check the migration slot status. It must show the slot assigned to the seed node with a low or decreasing `lag_size`:
...
- Verify active connections. Confirm that the seed node's IP address appears in the replication statistics:
...
- Confirm the recovery mode. The node must be correctly acting as a standby:

    ```sql
    SELECT pg_is_in_recovery();             -- Must return true
    ```

- Verify the WAL receiver status. Check the health of the connection to the primary (source node):
...
- Compare receive vs. replay position. This identifies the replay lag &mdash; the amount of data received but not yet applied to the database:
...
- Monitor lag by time. Measure the time difference between the current time and the last replayed transition:
