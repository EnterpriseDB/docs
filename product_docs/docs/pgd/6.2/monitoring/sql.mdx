...
something is down or disconnected. See [Replication slots](/pgd/latest/reference/node_management/replication_slots) in Node Management.
...
serving a [logical standby](/pgd/latest/reference/nodes/logical_standby_nodes/).
...
See [Monitoring degrade events](/pgd/latest/reference/commit-scopes/degrading/#monitoring-degrade-events) for more information about monitoring degrade events.
...
Replication [conflicts](/pgd/latest/reference/conflict-management/conflicts) can arise when multiple nodes make
...
PGD node replication slots are working as expected. This summary is also available on subscriber-only nodes that are operating as subscriber-only group leaders in a PGD cluster when [optimized topology](/pgd/latest/reference/nodes/subscriber_only/optimizing-so) is enabled. For example:
