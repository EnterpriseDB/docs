...
The `pgd node setup` command for a TDE-enabled cluster is similar to the standard command, but requires additional TDE-specific options.

We specify three additional options for Transparent Data Encryption (TDE): `--data-encryption`, `--key-wrap-command`, and `--key-unwrap-command`. The description for each of these options is provided in the preceding section.

For simplicity, the values for `--key-wrap-command` and `--key-unwrap-command` use a simple passphrase; however, you can configure more secure mechanisms, such as an external Key Management Service (KMS).

The `--data-encryption-keylen` option is not explicitly set here, defaulting the key length to 128 bits. You should set this option explicitly if a key length of 256 bits is desired.
...
This command is similar to the setup used for the first node, retaining the required TDE options. The remaining arguments are consistent with the standard command used when setting up subsequent nodes in a cluster.

In this instance, only the `--key-wrap-command` and `--key-unwrap-command` options are explicitly set, and their values **must be identical** to those provided for the initial node.
