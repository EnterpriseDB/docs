...
| `PARTED_CONCURRENT` | Node parting operation was started with concurrent option. |


Only one node at a time can be in either of the states `PROMOTE` or `PROMOTING`.

The `PARTED_CONCURRENT` state is used as target state to indicate that parting was done with a `concurrent = true` option. 
...
-   `-D, --pgdata=DIRECTORY` — The data directory to use for the new node. It
...
-   `-l, --log-file=FILE` — Use FILE for logging. The default is
...
-   `-L` — Perform selective pg_basebackup when used with an
    empty/nonexistent data directory (-D option). This is a feature of
    EDB Postgres Extended Server only.
-   `-S` — Instead of dropping logical replication subscriptions, disable
    them.
