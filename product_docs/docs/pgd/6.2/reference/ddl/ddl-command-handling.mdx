...
Generally, the standard PostgreSQL `ALTER SEQUENCE` command is supported for PGD. However, specific restrictions apply when modifying global sequences which utilize the underlying galloc mechanism:

`ALTER SEQUENCE ... RENAME` and `ALTER SEQUENCE ... SET SCHEMA` are only fully supported for galloc sequences if the `sequuid` (UUID) column is present in the `bdr.sequence_kind` catalog.
