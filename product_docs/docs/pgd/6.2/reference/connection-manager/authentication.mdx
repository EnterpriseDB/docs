...
- Connection Manager does not use OpenLDAP for communication with the LDAP server. As a result, standard OpenLDAP configuration files (such as `ldap.conf` or `.ldaprc`) and environment variables are ignored.
...
- Suppress validation of the LDAP server's SSL certificate by setting `LDAPTLS_REQCERT=never`. Other values for this variable are ignored.
