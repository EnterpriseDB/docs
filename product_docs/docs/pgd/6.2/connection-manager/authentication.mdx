...
redirects:
  - /pgd/latest/reference/connection-manager/authentication/ #generated for DOCS-3034
---
...
!!! Note
For Connection Manager authentication to work correctly, set or change role passwords only while connected to a PGD-enabled database with [`bdr.role_replication`](/pgd/current/reference/tables-views-functions/pgd-settings/#bdrrole_replication) set to `on`. A password set or changed while disconnected from the PGD-enabled database, or with `bdr.role_replication` set to `off`, doesn't replicate, and Connection Manager can then fail to authenticate that role.
!!!
...
- `cert` - SSL certificate authentication. Not recommended. See [Using client certificate authentication](#using-client-certificate-authentication).
...
- Connection Manager does not use the OpenLDAP library for communication with the LDAP server. As a result, standard OpenLDAP configuration files (such as `ldap.conf` or `.ldaprc`) and environment variables are ignored.
- Connection Manager validates LDAP server certificates by default when using LDAPS or LDAP StartTLS. The necessary CA certificate for the LDAP server's SSL certificate must be in the system trust-store.
- Suppress validation of the LDAP server's SSL certificate by setting `LDAPTLS_REQCERT=never` environment variable. Other values for this variable are ignored.
