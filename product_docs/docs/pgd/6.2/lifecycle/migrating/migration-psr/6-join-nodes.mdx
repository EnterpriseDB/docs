...
1. On the source node, check active connections from standbys. It mustn't show entries for the seed node nor the additional node:
...
1. On the additional node, verify that the table list in the subscription matches your configuration you obtained on [Enabling logical replication to the seed node](3-convert-to-logical-replication/#enabling-logical-replication-to-the-seed-node):
...
Next step: [Execute application cutover](7-application-cutover).