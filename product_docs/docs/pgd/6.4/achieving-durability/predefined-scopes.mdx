...
Once applied, the scope behaves as follows:

- The write leader waits for confirmation from the second primary node before the commit returns.
- If the second primary node can't confirm within 10 seconds, the local requirement degrades to async and writes continue without blocking.
- When the second primary node is reachable again, strict behavior resumes automatically.
- The DR node always receives changes asynchronously, without delaying commits.
- On failover to the second primary node, PGD swaps node roles automatically. No scope reconfiguration is needed.
