...
A table storing captured changes from transactions that failed to apply during replication. Each row represents one DML or DDL operation from a failed transaction. Created by the apply error policy when the policy is set to `record_all_and_disable` or `record_all_and_skip`.
...
| `total_skips` | `integer` | Total number of transactions skipped by a `record_and_skip` or `record_all_and_skip` policy. |
