...
This table tracks the status of logical replication slots that are being used with failover support. For more information on failover replication slots, see [CDC Failover support](/pgd/current/cdc-failover/).
...
A table storing captured changes from transactions that failed to apply during replication. Each row represents one DML or DDL operation from a failed transaction. Created by the apply error policy when the policy is set to `record_all_and_disable` or `record_all_and_skip`. For guidance on reviewing and resolving the contents of this table, see [Handling replication apply errors](/pgd/current/apply-error-policy).
