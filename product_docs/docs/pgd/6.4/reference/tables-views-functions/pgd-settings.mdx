...
### `bdr.lag_control_max_commit_delay`, `bdr.lag_control_max_lag_size`, `bdr.lag_control_max_lag_time`

These aren't global settings. Lag Control is configured per commit scope, using the
`max_commit_delay`, `max_lag_size`, and `max_lag_time` arguments of the `LAG CONTROL`
commit scope kind. See [Lag control](/pgd/current/commit-scopes/lag-control) for how
to set them.
...
Maximum delay between checks for vacuuming Raft catalogs. Default is 5 minutes (`5min`).
Time-unit suffixes are supported.

### `bdr.raft_vacuum_full_interval`

Maximum delay between automatic `VACUUM FULL` operations on Raft catalogs. Default is
1 day (`1d`). Set to `-1` to disable. Time-unit suffixes are supported.
...
Target size of the Commit Decisions memory cache used for Quorum Commit. Default is
40 MB (`40MB`). You can set this parameter only at Postgres server start.
