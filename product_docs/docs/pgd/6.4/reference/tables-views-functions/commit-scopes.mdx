...
Commit scopes are rules that determine how transaction commits and conflicts are handled within a PGD system. You can read more about them in [Commit Scopes](/pgd/current/commit-scopes/).
...
{ GROUP COMMIT [ ( group_commit_parameter = value [, ... ] ) ] [ ABORT ON ( abort_on_parameter = value ) ] [ DEGRADE ON (degrade_on_parameter = value [, ... ] ) TO commit_scope_degrade_operation ]
  | QUORUM COMMIT [ ( commit_decision = { group | raft } ) ] ABORT ON ( timeout = value )
...
When `ALL` is used with [`GROUP COMMIT`](#group-commit), the [`commit_decision`](#group-commit-commit_decision-settings) setting must be set to `raft` to avoid reconciliation issues.
...
The confirmation level sets the point in time when a remote PGD node confirms that it reached a particular point in processing a transaction. `QUORUM COMMIT` only supports `durable` and `visible`.
...
Provides distributed transaction consistency by coordinating commit decisions across all participating nodes before any node commits.

See [Quorum Commit](../../commit-scopes/quorum-commit) for more details.

```
QUORUM COMMIT [ ( commit_decision = { group | raft } ) ] ABORT ON ( timeout = value )
```

Supported commit scope groups are `ALL`, `MAJORITY ORIGIN GROUP`, `MAJORITY CLUSTER`, and `MAJORITY FOR EACH GROUP`. `MAJORITY` with an explicit group list, `ANY n`, and `NOT` aren't supported.

Only the `durable` and `visible` confirmation levels are supported with `QUORUM COMMIT`. Using `received` or `replicated` results in an error.

### QUORUM COMMIT parameters

| Parameter         | Type | Default | Description                                                                           |
| ----------------- | ---- | ------- | ------------------------------------------------------------------------------------- |
| `commit_decision` | enum | group   | Specifies how the commit decision is made. (`group` \| `raft`). See [commit_decision settings](#commit_decision-settings). |

### ABORT ON parameters (QUORUM COMMIT)

`ABORT ON` is mandatory for Quorum Commit.

| Parameter | Type     | Default | Description                                                 |
| --------- | -------- | ------- | ----------------------------------------------------------- |
| `timeout` | interval | 0       | Time to wait for quorum before aborting. (0 means not set.) |

### commit_decision settings

The value `group` means the origin node decides to commit once it receives the required confirmations from the commit scope group.

The value `raft` means the commit decision goes through PGD's built-in Raft consensus. This is slower than `group` but may be required for certain multi-group topologies.
...
| `commit_decision`      | enum    | group     | Specifies how the COMMIT decision is made. (`group`\|`partner`\|`raft`). See [commit_decision settings](#group-commit-commit_decision-settings). |
...
When used with the `ALL` commit scope group, the [`commit_decision`](#group-commit-commit_decision-settings) must be set to `raft` to avoid reconcilation issue.

See ["Conflict resolution" in Group Commit](../../commit-scopes/group-commit/#conflict-resolution).

### GROUP COMMIT commit_decision settings
