...
Supported commit scope groups are `ALL`, `MAJORITY ORIGIN_GROUP`, `MAJORITY CLUSTER`, and `MAJORITY FOR EACH GROUP`. `MAJORITY` with an explicit group list, `ANY n`, and `NOT` aren't supported.
