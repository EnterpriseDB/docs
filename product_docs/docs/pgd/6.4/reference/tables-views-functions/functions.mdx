...
bdr.skip_failed_transaction(
    ...
    re_enable boolean DEFAULT true
)
...
| `re_enable` | When `true`, re-enables the subscription after skipping. Default is `true`. |
...
| `re_enable` | When `true` and `dry_run` is `false`, re-enables the subscription after applying. Default is `true`. |
