...
`pgd node upgrade` runs PGD-specific checks before and after the standard `pg_upgrade` steps.

!!! Note
    When you supply `--check`, the command skips every step that modifies the database, and passes `--check` through to `pg_upgrade` as well.
!!!
...
| Step                                            | With `--check` |
| :----------------------------------------------|:--------------:|
| Collecting pre-upgrade new cluster control data | runs           |
| Checking new cluster state is shutdown          | runs           |
| Checking PGD versions                           | runs           |
| Starting old cluster (if shutdown)              | skipped        |
| Connecting to old cluster                       | skipped        |
| Checking if bdr schema exists                   | skipped        |
| Turning DDL replication off                     | skipped        |
| Terminating connections to database             | skipped        |
| Waiting for all slots to be flushed             | skipped        |
| Disconnecting from old cluster                  | skipped        |
| Stopping old cluster                            | skipped        |
| Starting old cluster with PGD disabled          | skipped        |
| Connecting to old cluster                       | skipped        |
| Collecting replication origins                  | skipped        |
| Collecting replication slots                    | skipped        |
| Disconnecting from old cluster                  | skipped        |
| Stopping old cluster                            | skipped        |

### pg_upgrade steps

Standard `pg_upgrade` steps are performed, with `--check` passed through if supplied.

### PGD post-upgrade steps

| Step                                                    | With `--check` |
| :-------------------------------------------------------|:--------------:|
| Collecting old cluster control data                     | skipped        |
| Collecting new cluster control data                     | skipped        |
| Advancing LSN of new cluster                             | skipped        |
| Starting new cluster with PGD disabled                   | skipped        |
| Connecting to new cluster                                 | skipped        |
| Creating replication origin, repeated for each origin    | skipped        |
| Advancing replication origin, repeated for each origin   | skipped        |
| Creating replication slot, repeated for each slot         | skipped        |
| Stopping new cluster                                      | skipped        |
