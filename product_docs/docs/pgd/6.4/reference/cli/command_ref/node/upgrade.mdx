...
Starting from PGD 6.4, `pgd node setup` advances the `NextOID` counter before creating BDR objects in single-user mode, preventing those objects from receiving system-range OIDs (< 16384) that would cause upgrade failures.

Before performing the upgrade, the command also checks whether BDR objects (database, extension, schema) were created with system-range OIDs. If detected, the command fails immediately with a clear error identifying the affected objects. If the command fails with a system-range OID error, see [Resolving system-range OID errors](#resolving-system-range-oid-errors) for steps to recover.
