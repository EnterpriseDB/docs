...
Quorum Commit uses eager conflict resolution, so the application encounters serialization errors at commit time more often than with other scopes. Wrap transactions in a retry loop to handle them:
