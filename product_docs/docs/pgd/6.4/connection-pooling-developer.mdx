...
`DISCARD ALL` runs when the backend connection is returned to the pool at the end of each transaction, deallocating all prepared statements on that connection. The next transaction can run on a different backend where those statements don't exist.
