---
title: Creating a commit scope
description: How to create a Quorum Commit scope, including syntax, topology-specific configurations, commit decision options, and timeout guidance.
deepToC: true
---
...
- **`commit_decision`:** Optional. Controls how the commit decision is reached, and affects how [automatic reconciliation](how-quorum-commit-works#automatic-reconciliation) behaves after a node failure. When using `ALL` as the commit scope group, set `commit_decision` to `raft`.
