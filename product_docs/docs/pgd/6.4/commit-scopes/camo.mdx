...
See the CAMO section of [Limitations](/pgd/current/overview/known_issues/#camo).
...
must have at least the [bdr_application](../advanced-functionality/security/pgd-predefined-roles/#bdr_application) 
role assigned to them.
!!!

The function [`bdr.is_camo_partner_connected()`](/pgd/latest/reference/tables-views-functions/functions#bdris_camo_partner_connected) allows checking the connection status of a CAMO partner node.
...
You pass this function the node_id and transaction_id of the transaction you want to check
on. You can use this function only on a node that's part of a CAMO pair.
...
Consider the effect of connection pools and proxies when designing a CAMO cluster. A proxy might freely distribute transactions to all nodes in the commit group, that is, to both nodes of a CAMO pair.
...
CAMO limitations are covered in [Known Issues and Limitations](/pgd/current/overview/known_issues/#camo).
