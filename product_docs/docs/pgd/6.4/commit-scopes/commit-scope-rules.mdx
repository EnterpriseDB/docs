...
A typical commit scope rule, such as `MAJORITY ORIGIN_GROUP QUORUM COMMIT ABORT ON (timeout = 10s)`, can be broken down into its components. `MAJORITY ORIGIN_GROUP` is the commit scope group, specifying which nodes need to respond and confirm they processed the transaction. In this example, a majority of nodes in the origin node's group must confirm.
