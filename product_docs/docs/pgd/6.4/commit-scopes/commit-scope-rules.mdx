...
No confirmation level is specified, which means that the default is used. You can think of the rule in full as:

```
MAJORITY ORIGIN_GROUP ON visible QUORUM COMMIT ABORT ON (timeout = 10s)
```

The `visible` setting means nodes can confirm once all the transaction's changes are flushed to disk and are visible to other transactions.
...
Currently, there are five commit scope kinds. The following is a summary, with links to more details.
