...
These limitations are a product of the interactions between Quorum Commit, Group Commit, and CAMO, and how they interact with PGD features such as the [WAL decoder](/pgd/current/decoding_worker/) and  [transaction streaming](/pgd/current/transaction-streaming/).
...
-   CAMO isn't currently compatible with transaction streaming.
    ...
    [Transaction streaming configuration](/pgd/current/transaction-streaming/#configuration).
...
-   CAMO isn't currently compatible with decoding worker.
    ...
    [Decoding worker disabling](/pgd/current/decoding_worker/#enabling).
