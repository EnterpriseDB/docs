...
- `output_plugin_libraries` &mdash; Must include `bdr` to allow it as a trusted output plugin for
  logical decoding. Available for community Postgres 18.6 and later. Check the current value, then append
  `bdr` to it and reload the configuration, for example:

  ```sql
  SHOW output_plugin_libraries;
  ALTER SYSTEM SET output_plugin_libraries = '<current_value>, bdr';
