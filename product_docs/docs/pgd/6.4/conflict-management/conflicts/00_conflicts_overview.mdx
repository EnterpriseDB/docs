...
deepToC: true
redirects:
  - /pgd/latest/conflict-management/conflicts/00_conflicts_overview/ #generated for DOCS-1247-PGD-6.0-Docs
  - /pgd/latest/reference/conflict-management/conflicts/00_conflicts_overview/ #generated for DOCS-3034
---
...
Conflict handling is configurable, as described in [Conflict resolution](04_conflict_resolution). PGD can detect conflicts and handle them differently for each table using conflict triggers, described in [Stream triggers](../../advanced-functionality/stream-triggers).

Column-level conflict detection and resolution is available with PGD, as described in [CLCD](../column-level-conflicts).

By default, all conflicts are logged to [`bdr.conflict_history`](/pgd/latest/reference/tables-views-functions/catalogs-visible/#bdrconflict_history). If conflicts are possible, then table owners must monitor for them and analyze how to avoid them or make plans to handle them regularly as an application task. The [LiveCompare](/livecompare/latest) tool is also available to scan regularly for divergence.
...
For `INSERT`/`INSERT` conflicts, use [global sequences](/pgd/current/advanced-functionality/sequences/#pgd-global-sequences) to prevent this type of conflict.
