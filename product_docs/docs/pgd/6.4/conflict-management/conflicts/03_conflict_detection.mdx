...
deepToC: true
redirects:
  - /pgd/latest/conflict-management/conflicts/03_conflict_detection/ #generated for DOCS-1247-PGD-6.0-Docs
  - /pgd/latest/reference/conflict-management/conflicts/03_conflict_detection/ #generated for DOCS-3034
---
...
!!! Note
  ...
  If a local node's clock lags behind a remote node's commit timestamp, transactions using group commit scopes will experience increased commit latency. This delay occurs regardless of the settings for `bdr.maximum_clock_skew` or `bdr.maximum_clock_skew_action`, as the local node must wait until its local timestamp is no longer older than the remote commit timestamp. 
  
  To ensure optimal performance and minimize latency, system clocks must be strictly synchronized.
!!!
...
To change the current conflict detection strategy, use [bdr.alter_table_conflict_detection](/pgd/latest/reference/tables-views-functions/conflict_functions/#bdralter_table_conflict_detection).
