...
[logical change records (LCRs)](/pgd/current/advanced-functionality/decoding_worker/#enabling) with a
...
using the new [commit scope](/pgd/current/commit-scopes/commit-scopes/)-based settings.
...
4.  Create a dedicated node group for the two nodes and move them into
    that node group.
5.  Create a [commit scope](/pgd/current/commit-scopes/commit-scopes/) for this node
...
version), PGD's replication slots stop working unless `bdr` is added to it. EDB Postgres Extended
and EDB Postgres Advanced Server already allow `bdr` by default and aren't affected.
!!!

If you're upgrading to one of those minor versions, follow this order instead of the general steps
above, for each node:

1.  [Fence the node](#fence-the-node), then [stop Postgres](#stop-postgres), as usual.
1.  While Postgres is still stopped, add `bdr` to `output_plugin_libraries` in `postgresql.conf`:
...
1.  [Upgrade the packages](#upgrade-packages), then [start Postgres](#start-postgres), as usual.
1.  [Unfence the node](#unfence-the-node), as usual.

See [Postgres configuration](/pgd/current/postgres-configuration/#postgres-settings) for the
required setting.
...
Always check your [monitoring](/pgd/current/monitoring/) after upgrading a node to confirm
