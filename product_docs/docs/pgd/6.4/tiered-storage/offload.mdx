...
!!! Note
If you perform a point-in-time recovery (PITR) with a target time that falls during a `pgaa.convert_to_analytics()` operation, the deployment can be left in a partially recovered state. Postgres restores the heap data but can't roll back the Iceberg data files and catalog entry already written to object storage, leaving them as orphans. See [Performing a PITR restore after offloading tables](/pgaa/latest/backup_restore#performing-a-pitr-restore-after-offloading-tables) for details and cleanup steps.
!!!
