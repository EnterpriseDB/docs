...
Explicit two-phase commit doesn't work with Quorum Commit, Group Commit, or CAMO, as those commit scope kinds use two-phase commit internally. Explicit two-phase commit also doesn't work well with [Parallel Apply](/pgd/current/parallelapply/) or [transaction streaming](/pgd/current/transaction-streaming/). Disable both before using explicit two-phase commit.
