...
redirects:
  ...
  - /pgd/latest/reference/twophase/ #generated for DOCS-3034
  - /pgd/latest/twophase/ #generated for DOCS-3995-new
---

!!!Note

Two-phase commit isn't available with Quorum Commit, Group Commit, or CAMO. See [Commit scope limitations](../overview/known_issues/#general-durability-limitations).
!!!
...
Explicit two-phase commit doesn't work with Quorum Commit, Group Commit, or CAMO, as those commit scope kinds use two-phase commit internally. Explicit two-phase commit also doesn't work well with [Parallel Apply](/pgd/current/advanced-functionality/parallelapply/) or [transaction streaming](/pgd/current/advanced-functionality/transaction-streaming/). Disable both before using explicit two-phase commit.
