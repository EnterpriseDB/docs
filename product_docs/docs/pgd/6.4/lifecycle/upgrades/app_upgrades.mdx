...
[DDL replication](/pgd/current/ddl/). You can use this approach
...
See [Replicating between nodes with differences](/pgd/current/appusage/) for details.
...
environment on all the nodes, one by one. See [ALTER TABLE](/pgd/current/ddl/ddl-command-handling/#alter-table) for more details.
