...
TPAexec deploys the EDB Postgres Distributed Command Line Interface (PGD CLI) in each cluster. To install PGD CLI on a standalone server, see [Installing PGD CLI](installing_cli).
