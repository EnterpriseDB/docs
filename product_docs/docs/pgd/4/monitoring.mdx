...
Also, the table [`bdr.node_catchup_info`](/pgd/latest/bdr/catalogs) will give information
...
-   [`bdr.node_slots`](/pgd/latest/bdr/catalogs) for monitoring outgoing replication
-   [`bdr.subscription_summary`](/pgd/latest/bdr/catalogs) for monitoring incoming replication
...
something is down or disconnected. See [`Replication Slots created by BDR`](/pgd/latest/bdr/nodes).

### Monitoring Outgoing Replication

There is an additional view used for monitoring of outgoing replication activity:

-   [`bdr.node_replication_rates`](/pgd/latest/bdr/catalogs) for monitoring outgoing replication
