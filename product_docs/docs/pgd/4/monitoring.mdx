...
Also, the table [`bdr.node_catchup_info`](/pgd/latest/overview/bdr/catalogs) will give information
...
-   [`bdr.node_slots`](/pgd/latest/overview/bdr/catalogs) for monitoring outgoing replication
-   [`bdr.subscription_summary`](/pgd/latest/overview/bdr/catalogs) for monitoring incoming replication
...
something is down or disconnected. See [`Replication Slots created by BDR`](/pgd/latest/overview/bdr/nodes).

### Monitoring Outgoing Replication

There is an additional view used for monitoring of outgoing replication activity:

-   [`bdr.node_replication_rates`](/pgd/latest/overview/bdr/catalogs) for monitoring outgoing replication
...
If the [Decoding Worker](/pgd/latest/overview/bdr/nodes#decoding-worker) is enabled, information about the
current LCR (`Logical Change Record`) file for each WAL sender can be monitored
via the function [bdr.wal_sender_stats](/pgd/latest/overview/bdr/functions#bdrwal_sender_stats),
e.g.:
...
serving a [logical standby](/pgd/latest/overview/bdr/nodes#logical-standby-nodes).

Additionally, information about the Decoding Worker can be monitored via the function
[bdr.get_decoding_worker_stat](/pgd/latest/overview/bdr/functions#bdrget_decoding_worker_stat), e.g.:
...
view](/pgd/latest/overview/bdr/catalogs#bdrglobal_locks).  This view shows the type of the lock; for
...
Replication [conflicts](/pgd/latest/overview/bdr/conflicts) can arise when multiple nodes make
...
and monitor the EDB Postgres Distributed cluster.  By centralizing this information, external
