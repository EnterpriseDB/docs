...
Read about [Conflicts](/pgd/latest/overview/bdr/conflicts/) to understand
...
-   If the resolver for the `update_origin_change` conflict
    ...
    [Conflicts](/pgd/latest/overview/bdr/conflicts/) chapter, `skip` is not the default
...
-   Group Commit cannot be combined with [CAMO](/pgd/latest/overview/bdr/camo/) or [Eager All Node
    replication](/pgd/latest/overview/bdr/eager/). Eager Replication currently only works by using the
...
-   Parallel apply is not currently supported in combination with Group
    ...
    [`bdr.alter_node_group_config`](/pgd/latest/overview/bdr/nodes#bdralter_node_group_config)) or
    via the GUC `bdr.writers_per_subscription` (see
    [Configuration of Generic Replication](/pgd/latest/overview/bdr/configuration#generic-replication)).
