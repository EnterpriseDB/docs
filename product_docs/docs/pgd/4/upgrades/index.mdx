...
An upgrade process can take more time when caution is required to reduce
business risk. Do not run with mixed versions of the software for any longer
than is necessary to complete the upgrade. The longer you run with mixed
versions, the more likely you are to encounter issues, the more difficult it is
to diagnose and resolve them, and you will not be able to use new features until
all nodes are upgraded. We recommend upgrading in off peak hours for your
business, and over a short period of time. For maximum caution, we suggest
performing the upgrades in separate a test environment first.  
