...
[DDL Replication](../overview/bdr/ddl/), which
...
See [Replicating between nodes with differences](../overview/bdr/appusage/) for details.
...
environment on all the nodes, one by one. See [ALTER TABLE](../overview/bdr/ddl/#alter-table) for more details.
 section.
