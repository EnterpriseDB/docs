...
Always On architectures reflect EDB’s Trusted Postgres archtictures that encapsulate practices and help you to achieve the highest possible service availability in multiple configurations. These configurations range from single-location architectures to complex distributed systems that protect from hardware failures and data center failures. The architectures leverage EDB Postgres Distributed’s multi-master capability and its ability to achieve 99.999% availability, even during maintenance operations.
...
-  [Bronze](bronze): Single location with 2 data nodes, a witness node, and a local backup
-  [Silver](silver): Single location with 3 data nodes and second location with an offsite backup
-  [Gold](gold): Two locations with 2 data nodes each and a third location with a witness
-  [Platinum](platinum): Two locations with 3 data nodes each (2 masters plus additional redundant hardware in a hot standby mode)
...
|-----------------------------|------------------|------------------|----------------|----------------------|
| Minimum Locatiions Needed   | 1   | 2   | 3     | 2    |
