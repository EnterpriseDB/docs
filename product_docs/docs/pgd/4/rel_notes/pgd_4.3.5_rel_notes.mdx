...
| BDR       | 4.3.5   | Enhancement | BDR now executes each autopartition task in its own transaction.                                            |           |
| BDR       | 4.3.5   | Enhancement | BDR now uses `DETACH CONCURRENTLY` to drop partitions.                                                      | RT101407/35476  |
| BDR       | 4.3.5   | Enhancement | BDR now grants use of `pg_file_settings` and `pg_show_all_file_settings()` to bdr_superuser.              |           |
| BDR       | 4.3.5   | Enhancement | BDR now stops the initial replication connection for `bdr_init_physical` and starts it only when needed.     | RT102828/35305  |
