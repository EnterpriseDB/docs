...
HARP provides [HTTP(S) health check APIs](./#health-check). Add the below settings only if health check APIs are required. Specify all elements under a section named `http` with these multiple
supplementary entries:
