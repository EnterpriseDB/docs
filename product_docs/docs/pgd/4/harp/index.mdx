...
Liveness check either return `true` with HTTP status code `200 (OK)` or an error but never `false`. This is because the HTTP server listening for request is automatically stopped if the corresponding HARP service fails to start or exits. This API endpoint is available for `harp-manager` and `harp-proxy` services.
