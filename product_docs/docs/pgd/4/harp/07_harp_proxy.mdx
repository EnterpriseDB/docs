...
Routing decisions are stored in a DCS (Distributed Consensus Store). Harp Manager supports two DCS stores, the BDR Postgres database and etcd. The etcd option is capable of managing a very high volume of the simple transactions used to track leadership leases and lease renewals. It is also lightweight, leaving more server capacity to the core database.
...
* **Partition of an AZ containing the write leader**
  * Leader lease expires
  * New leader is elected by the remaining nodes
  * Synchronous replication is reconfigured


* **Partition of a AZ not containing the write leader**
  * The monitor function detects the loss of synchronous commit consensus (on a 1 second loop)
  * Synchronous replication is reconfigured
...
* **Partition of all AZ**s
  ...
  * Synchronous replication consensus is lost, replication is blocked
...
* **Recovery**
  ...
  * Synchronous replication will be reconfigured once the recovering node catches update
