...
-   `bdr.track_subscription_apply` &mdash; Track apply statistics for
    each subscription with [`bdr.stat_subscription`](/pgd/4/bdr/catalogs.mdx#bdrstat_subscription).
    The default value is `on`.
-   `bdr.track_relation_apply` &mdash; Track apply statistics for each
    relation with [`bdr.stat_relation`](/pgd/4/bdr/catalogs.mdx#bdrstat_relation).
    The default value is `off`.
-   `bdr.track_apply_lock_timing` &mdash; Track lock timing when tracking
    statistics for relations with [`bdr.stat_relation`](/pgd/4/bdr/catalogs.mdx#bdrstat_relation).
    The default value is `off`.
