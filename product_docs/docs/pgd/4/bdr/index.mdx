...
BDR should only be used to replicate a single databases in a PostgreSQL instance. While it will
not block you from installing it to multiple databases and adding groups in each of them, some
features don't work when BDR is used in multiple databases in a single instance.
