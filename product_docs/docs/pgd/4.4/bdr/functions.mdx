...
This function retrieves the textual representation of the BDR edition
currently in use.
...
Both parameters are required (neither has a default), but you can pass
`NULL` explicitly for either one to get the behaviors described above.

#### Parameters

-   `slotname` &mdash; Name of replication slot, or `NULL` for all BDR slots.
-   `target` &mdash; LSN to wait for, or `NULL` to use the current write LSN on the
    local node.
...
```sql
bdr.wait_for_apply_queue(node_name TEXT, target_lsn pg_lsn)
```

#### Parameters

-   `node_name` &mdash; The name of the peer node from which incoming
...
Modifies the kind of an existing BDR node. The current kind of a node
can be checked in the `node_kind` column of [`bdr.node`](catalogs#bdrnode).

#### Synopsis

```sql
bdr.alter_node_kind(node_name text, node_kind text)
```

#### Parameters

-   `node_name` &mdash; Name of the node to modify.
-   `node_kind` &mdash; New kind to assign to the node.
...
This function returns a record with fields `status` and `message`,
as explained in [Monitoring replication slots](/pgd/4.4/monitoring/#monitoring-replication-slots).
...
For further details, see [Monitoring WAL senders using LCR](/pgd/4.4/monitoring/#monitoring-wal-senders-using-lcr).
...
If [lag control](lag-control#configuration) is enabled, this function
returns a history of recent lag samples recorded for each peer node,
complementing the current-state snapshot returned by `bdr.lag_control()`.
...
- `peer_node_id` &mdash; OID of the peer node this sample was recorded for.

- `ts` &mdash; Timestamp at which the sample was recorded.

- `send_ptr`, `write_ptr`, `flush_ptr`, `apply_ptr` &mdash; LSNs reported by
  the peer for the send, write, flush, and apply positions at the time of
  the sample.

- `apply_rate` &mdash; Apply rate calculated for the peer at the time of
  the sample.
...
Looks up a node group and reports BDR and pglogical node information for its
members. Used internally during node join to protect against concurrent
catalog changes.
