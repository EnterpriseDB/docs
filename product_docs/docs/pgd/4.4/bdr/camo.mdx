...
1.  Create the EDB Postgres Distributed cluster where nodes `node1` and `node2` are part of the
`mygroup` node group.
...
- `require_camo_partner` &mdash; Defaults to true and enables configuration
  ...
  Replication.
- `queried_by_origin` &mdash; Advanced/internal use. Only the origin node
  should set this to `true`, and only when it's uncertain about a
  transaction that is still prepared but not yet finished. It forces
  the CAMO partner to resolve an otherwise-unknown or
  provisional-abort decision to `abort`, since the partner can't
  safely report `committed` before the transaction actually commits.
  Defaults to `false`.
