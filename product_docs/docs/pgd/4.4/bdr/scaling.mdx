...
-   `partition_initial_lowerbound` &mdash; If the table has no partition, then the
    ...
    bound is created.
-   `partition_min_upperbound` &mdash; Minimum upper bound to use when creating new
    partitions.
...
```sql
bdr.autopartition_get_last_completed_workitem(local boolean DEFAULT false);
```
...
-   `local` &mdash; When `true`, return the last workitem completed on the
    local node instead of on all nodes.
...
-   `workid` &mdash; The key of the AutoPartition worker.
...
is used, the function returns `unknown`. `In-progress` is the typical status.

### Task manager leader

AutoPartition work is coordinated by a task manager leader, elected among
the eligible nodes in the cluster. Use `bdr.taskmgr_set_leader()` to
manually request that a specific node become the new task manager leader,
analogous to `bdr.raft_leadership_transfer()` for the Raft leader.
...
-   `node` &mdash; Name of the node to promote to task manager leader. The
    node must be an eligible candidate.
-   `wait_for_completion` &mdash; If `true`, wait until the given node becomes
    the new task manager leader before returning.

#### Notes

This function requires all nodes in the cluster to be running a consensus
protocol version that supports task manager leader transfer; otherwise it
raises an error suggesting you upgrade all nodes.
