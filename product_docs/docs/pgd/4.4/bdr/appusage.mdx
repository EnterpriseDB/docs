...
!!! Warning REPLICA IDENTITY FULL required for tables with TOAST-able columns
    ...
    safely under concurrent updates from multiple nodes. BDR 4 does not set
    ...
    When an `UPDATE` doesn't change a TOAST column, the column is sent in the
    replication stream as an *unchanged* marker rather than as full data.
    Under `REPLICA IDENTITY FULL`, PostgreSQL flattens the entire pre-update
    row (including its TOAST data) into WAL, so the receiver always has an
    authoritative source for unchanged columns. Under `REPLICA IDENTITY
    DEFAULT` (or `USING INDEX`), the receiver fills unchanged TOAST columns
    from its local copy of the row, which may be stale if a concurrent
    update on another node modified a different column of the same row —
...
See [Release notes](/pgd/4.4/rel_notes/) for any known incompatibilities.
