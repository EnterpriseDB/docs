...
### Creating a commit scope rule

The function `bdr.create_commit_scope` creates a rule for the given
...
!!! Note
    `bdr.add_commit_scope` is a deprecated alias for this function and
    will be removed in a future release. Use `bdr.create_commit_scope`
    instead.
...
### Dropping a commit scope rule

You can use `bdr.drop_commit_scope` to drop a single rule in
...
!!! Note
    `bdr.remove_commit_scope` is a deprecated alias for this function
    and will be removed in a future release. Use `bdr.drop_commit_scope`
    instead.
