...
-   `detector_args` &mdash; Only valid when `method` is `row_version`. A JSON
    object that can set `atttype` to the name of the data type to use for
    the row-version tracking column (defaults to `integer`). Passing this
    for any other method raises an error.
