...
Also, the table [`bdr.node_catchup_info`](/pgd/4.4/bdr/catalogs/#bdrnode_catchup_info) will give information
...
-   [`bdr.node_slots`](/pgd/4.4/bdr/catalogs/#bdrnode_slots) for monitoring outgoing replication
-   [`bdr.subscription_summary`](/pgd/4.4/bdr/catalogs/#bdrsubscription_summary) for monitoring incoming replication
...
something is down or disconnected. See [`Replication Slots created by BDR`](/pgd/4.4/bdr/nodes/#replication-slots-created-by-bdr).

### Monitoring Outgoing Replication

There is an additional view used for monitoring of outgoing replication activity:

-   [`bdr.node_replication_rates`](/pgd/4.4/bdr/catalogs/#bdrnode_replication_rates) for monitoring outgoing replication
...
If the [Decoding Worker](/pgd/4.4/bdr/nodes#decoding-worker) is enabled, information about the
current LCR (`Logical Change Record`) file for each WAL sender can be monitored
via the function [bdr.wal_sender_stats](/pgd/4.4/bdr/functions#bdrwal_sender_stats),
e.g.:
...
serving a [logical standby](/pgd/4.4/bdr/nodes#logical-standby-nodes).

Additionally, information about the Decoding Worker can be monitored via the function
[bdr.get_decoding_worker_stat](/pgd/4.4/bdr/functions#bdrget_decoding_worker_stat), e.g.:
...
view](/pgd/4.4/bdr/catalogs#bdrglobal_locks).  This view shows the type of the lock; for
...
Replication [conflicts](/pgd/4.4/bdr/conflicts) can arise when multiple nodes make
