...
| `--layout`                   | Required. Specify one of the four supported architectures: bronze, silver, gold, and platinum. See [Choosing your architecture](/pgd/4.4/architectures) for more information. |
...
Specify `--enable-camo` to set the pair of BDR primary instances in each region to be each other's Commit At Most Once (CAMO) partners. See [Commit At Most Once (CAMO)](/pgd/4.4/bdr/camo/) for more information.
