...
redirects:
  ...
  - /pgd/latest/ddl/ddl-command-handling/ #generated for DOCS-3995-new
---
...
| ALTER MATERIALIZED VIEW          | Y                                             | [Y](#matview_commands)                     | DML                                              |
...
| CREATE MATERIALIZED VIEW         | Y                                             | [Y](#matview_commands)                     | DML                                              |
...
| DROP MATERIALIZED VIEW           | Y                                             | [Y](#matview_commands)                     | DML                                              |
...
| REFRESH MATERIALIZED VIEW        | Y                                             | [Y](#matview_commands)                     | DML                                              |
| REINDEX                          | Y                                             | Y                                          | DDL                                                |
| RELEASE                          | Y                                             | N                                          | N                                                |
| RESET                            | Y                                             | N                                          | N                                                |
...
Starting with PGD 6.3.0, `ALTER SEQUENCE` is fully supported. 

While PGD 6.2.0 introduced `RENAME` and `SET SCHEMA` functionality for `galloc` sequences, the feature did not work for sequences created in PGD versions earlier than 6.2.0 following an upgrade to 6.2.0. The reason for the failure was that PGD 6.2.0 introduced a global UUID identifier to track `galloc` sequences, which was generated at the time of sequence creation, but it was missing for sequences created on an earlier PGD version.

PGD 6.3.0 removes that limitation as BDR manager generates the UUID for sequences if it is missing, enabling the feature for sequences created in previous PGD versions.
...
!!!Note
PGD doesn't execute `PASSWORD_VERIFY_FUNCTION` on witness servers, since database objects aren't replicated to these servers. The origin node already executed the function and verified the password, so a duplicate check on the witness is redundant.

To run `PASSWORD_VERIFY_FUNCTION` on a witness server anyway, create the function manually there, then run `CREATE PROFILE` or `ALTER PROFILE` directly on the witness server.
!!!
...
<div id='matview_commands'></div>

### MATERIALIZED VIEW

`CREATE MATERIALIZED VIEW` populates data on the upstream node only; the data is then replicated. `REFRESH MATERIALIZED VIEW` also runs on the upstream node only; the result is replicated to downstream nodes.

Materialized view replication is available only on BDRPG, EDB Advanced Server (EDBAS), or 2QPG. A community Postgres build of PGD treats all four commands as unreplicated.
