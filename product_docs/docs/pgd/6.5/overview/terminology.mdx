...
Rules for managing how transactions are committed between the nodes and groups of a PGD cluster. Used to configure [synchronous replication](#synchronous-replication), [Quorum Commit](#quorum-commit), [Group Commit](#group-commit), [CAMO](#camo-or-commit-at-most-once), [Eager](#eager), [Lag control](#lag-control), and other PGD features.
