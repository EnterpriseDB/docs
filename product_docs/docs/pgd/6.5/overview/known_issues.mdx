...
-   If the resolver for the `update_origin_change` conflict
      ...
      [Conflicts](/pgd/current/conflict-management/conflicts/), `skip` isn't the default
...
-   Group Commit can't be combined with [CAMO](/pgd/current/commit-scopes/camo/). 
...
-   Community Postgres 14.24, 15.19, 16.15, 17.11, and 18.6 add the `output_plugin_libraries` parameter, which
    restricts which libraries can be used as logical decoding output plugins and defaults to
    `pgoutput, test_decoding`. A node upgraded to one of these minor versions without adding `bdr`
    to `output_plugin_libraries` fails to create or use PGD replication slots, logging an error such
    as `library "bdr" may not be used as an output plugin`. EDB Postgres Extended and EDB Postgres
    Advanced Server already include `bdr` in this parameter by default starting with the matching
    minor version, so no manual step is needed there. See [Postgres
    configuration](/pgd/current/postgres-configuration/#postgres-settings) for the required setting
    and instructions, and [PGD
    compatibility](/pgd/current/overview/compatibility/#pgd-compatibility-with-postgres-versions)
    for the minimum EDB Postgres Extended and EDB Postgres Advanced Server versions PGD 6.5.0
    requires.

-   In PGD 6.3.0, user login attributes aren't replicated to peer nodes after a `CREATE USER` statement. Users created on one node can log in on the originating node but are blocked on all other nodes because the login attribute is missing from the replicated role definition. This issue is resolved in PGD 6.3.1.
...
-   Renaming a table under [tiered storage](/pgd/latest/tiered-storage/), whether it's a tiered table, a table replicating to analytics, or an offloaded table, or renaming the schema that contains it, isn't supported. PGD tracks these tables by name in `bdr.analytics_table`, so renaming interrupts synchronization with the object storage copy and can leave stale or duplicate data behind. Don't rename these tables or their schemas.
...
-   The minimum recommended number of nodes in a group is three to provide fault
    ...
    details](/pgd/current/planning/deployment-patterns/).
...
These limitations are a product of the interactions between Quorum Commit, Group Commit, and CAMO, and how they interact with PGD features such as the [WAL decoder](/pgd/current/advanced-functionality/decoding_worker/) and  [transaction streaming](/pgd/current/advanced-functionality/transaction-streaming/).
...
-   [Legacy synchronous replication](/pgd/current/commit-scopes/legacy-sync/) uses a mechanism for transaction confirmation
    different from the one used by CAMO, Eager, Group Commit, and Quorum Commit. The two aren't
    compatible, so don't use them together. Whenever you use Quorum Commit, Group Commit, CAMO,
...
-   Postgres two-phase commit (2PC) transactions (that is, [`PREPARE
    ...
    can't be used with CAMO, Group Commit, Quorum Commit, or Eager because those
    features use two-phase commit underneath.

#### Group Commit

[Group Commit](/pgd/current/commit-scopes/group-commit/) enables configurable synchronous commits over 
...
[Eager conflict resolution](/pgd/current/commit-scopes/quorum-commit/how-quorum-commit-works#eager-conflict-resolution) is available through Group Commit and is always active in Quorum Commit. It avoids conflicts by eagerly aborting transactions that might clash. When used through Group Commit, it's subject to the same limitations as Group Commit.
...
[Quorum Commit](/pgd/current/commit-scopes/quorum-commit/) is the commit scope kind for distributed transaction consistency. If you use this feature, take the following limitations into account:

-   All nodes in the cluster must be running PGD 6.4.0 or later. A cluster with any node on an earlier version can't use Quorum Commit, including clusters undergoing a rolling upgrade.

-   `MAJORITY` with an explicit group list, `ANY n`, and `NOT` aren't supported. Supported groups are `ALL`, `MAJORITY ORIGIN GROUP`, `MAJORITY CLUSTER`, and `MAJORITY FOR EACH GROUP`.

-   During a rolling upgrade from a version earlier than PGD 6.5, keep [`bdr.default_streaming_mode`](/pgd/current/reference/tables-views-functions/pgd-settings/#bdrdefault_streaming_mode) set to `file` or `off` until every node is on PGD 6.5 or later. See [Compatibility](/pgd/current/commit-scopes/quorum-commit/using-quorum-commit/#compatibility) for details.

-   Explicit two-phase commit (2PC) isn't supported by Quorum Commit, as Quorum Commit already uses 2PC internally.

-   `DEGRADE TO` isn't supported.

#### CAMO

[Commit At Most Once](/pgd/current/commit-scopes/camo/) (CAMO) is a feature that aims to prevent
...
-   CAMO isn't currently compatible with transaction streaming.
    ...
    [Transaction streaming configuration](/pgd/current/advanced-functionality/transaction-streaming/#configuration).
...
-   CAMO isn't currently compatible with decoding worker.
    ...
    [Decoding worker disabling](/pgd/current/advanced-functionality/decoding_worker/#enabling).
...
PGD was developed to [enable rolling upgrades of PGD](/pgd/current/upgrades/) by allowing mixed versions of PGD to operate during the upgrade process. 
...
- The [`password_profile`](/pge/latest/password_profiles/) extension in
  [EDB Postgres Extended Server](/pge/latest/) stores password profiles in
  system catalogs that PGD can't automatically replicate to other nodes. To
  ensure correct replication of password profiles, execute the extension's
  functions using
  [`bdr.run_on_all_nodes()`](/pgd/current/reference/tables-views-functions/functions/#bdrrun_on_all_nodes).
