...
!!! Note
This 18.6.0 restriction doesn't affect running PGD on community Postgres, and EDB Postgres Advanced
Server already allows `bdr` by default regardless of version. If you're upgrading to a community
Postgres minor version that introduces the new `output_plugin_libraries` parameter (14.24, 15.19,
16.15, 17.11, or 18.6, depending on your major version), add `bdr` to it and reload the
configuration, or PGD's replication slots stop working. See [Postgres
