...
redirects:
- /pgd/latest/tiered-storage/offload/
...
!!! Warning
Don't rename an offloaded table, or the schema that contains it. PGD tracks these tables by name in `bdr.analytics_table`, so renaming interrupts synchronization with the object storage copy and can leave stale or duplicate data behind.
!!!
