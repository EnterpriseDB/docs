...
redirects:
- /pgd/latest/tiered-storage/replicate/
- /pgaa/latest/pgd/replicate
- /pgaa/1.8/pgd/replicate
- /pgaa/1.3/pgd/replicate
---

Use Postgres Analytics Accelerator (PGAA) within a Postgres Distributed (PGD) cluster to synchronize local heap tables with an analytical data lake by converting them to Hybrid Transactional and Analytics Processing (HTAP) tables.
...
!!! Note
When using Barman to back up a PGD cluster with HTAP tables, a point-in-time recovery (PITR) with a target time that falls during a `pgaa.enable_analytics_replication()` operation can leave the deployment in a partially recovered state. See [Known limitation: PITR restore after enabling analytics replication](/pgaa/latest/backup_restore#known-limitation-pitr-restore-after-enabling-analytics-replication) for details and cleanup steps.
!!!

!!! Warning
Don't rename a table you've enabled analytics replication on, or the schema that contains it. PGD tracks the table by name, so renaming interrupts synchronization with the object storage copy and can leave stale or duplicate data behind.
!!!
