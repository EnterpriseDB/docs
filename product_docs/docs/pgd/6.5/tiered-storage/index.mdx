...
!!! Warning
Don't rename an HTAP or PGAA table, or the schema that contains it, no matter which tiering method created it. PGD tracks these tables by name in `bdr.analytics_table`, so renaming interrupts synchronization with the object storage copy and can leave stale or duplicate data behind.
!!!
