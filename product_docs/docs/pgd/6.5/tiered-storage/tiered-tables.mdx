...
redirects:
- /pgd/latest/tiered-storage/tiered-tables/
- /pgaa/latest/pgd/tiered-tables
- /pgaa/1.8/pgd/tiered-tables
- /pgaa/1.3/pgd/tiered-tables
---
...
-   **Hot tier:** Recent data partitions stay in local heap partitions, or become Hybrid Transactional and Analytics Processing (HTAP) tables if replication is enabled. For HTAP, PGAA uses Iceberg Merge-on-Read (MoR) to capture updates and deletes as lightweight "delete files." This keeps the cloud tier synchronized without the overhead of rewriting large data blocks.

-   **Cold tier:** Once partitions cross the age threshold defined by `analytics_offload_period`, they are automatically converted to PGAA tables in the data lake. Local storage is reclaimed, but the data remains transparently queryable through the same table interface.
...
Tiered tables create a unified view that performs a `UNION` of all partitions, giving you a single query interface across the full data lifecycle. You can enable the `bdr.prefer_analytics_engine` parameter to route queries on the entire tiered table to the Seafowl engine:

```sql
SET bdr.prefer_analytics_engine = TRUE;
```

If `enable_replication` is `true`, this setting enables DirectScan for the entire partitioned table, allowing Seafowl to execute queries across both offloaded cold data and replicated hot data in a single pass.

!!! Note
    When using storage location-based replication, union views over partitioned tables are only supported by the Seafowl engine and are not accessible via Spark. When using a catalog service, union views are accessible via both Seafowl and Spark.
!!!
...
!!! Warning
Don't rename a tiered table, its partitions, or the schema that contains them. PGD tracks these tables by name in `bdr.analytics_table`, so renaming interrupts synchronization with the object storage copy and can leave stale or duplicate data behind.
!!!
...
See [Automating table partitioning](/pgd/current/partitioning/) for details.
...
To change the current setting for `enable_replication`, use the `pgaa.enable_analytics_replication()` or `pgaa.disable_analytics_replication()` functions. When called on the parent table, these functions propagate the change to all existing and future child partitions:

```sql
CALL pgaa.enable_analytics_replication('my-transactional-table'::regclass);
```

You can also enable or disable replication on individual partitions directly. However, you cannot disable replication on a child partition if it is already enabled on the parent table, and you cannot enable it on a child partition if the parent already has it enabled.
