...
deepToC: true
redirects:
  - /pgd/latest/essential-how-to/install/02-configure-repositories/
  - /pgd/latest/expanded-how-to/install/02-configure-repositories/
  - /pgd/latest/installing/02-configure-repositories/
  - /pgd/latest/lifecycle/deploying/deploy-manual/02-configure-repositories/
---
...
In this example, we will configure the repositories on a CentOS/RHEL system that will allow us to install EDB Postgres Advanced Server 17 with PGD using an enterprise subscription.
...
```bash
# There are two repositories to install.
curl -1sSLf " https://downloads.enterprisedb.com/$EDB_SUBSCRIPTION_TOKEN/$EDB_SUBSCRIPTION_PLAN/setup.$EDB_REPO_TYPE.sh" | sudo -E bash
curl -1sSLf " https://downloads.enterprisedb.com/$EDB_SUBSCRIPTION_TOKEN/postgres_distributed/setup.$EDB_REPO_TYPE.sh" | sudo -E bash
```
