...
redirects:
  ...
  - /pgd/latest/installing/04-configuring-cluster/
  - /pgd/latest/lifecycle/deploying/deploy-manual/04-configuring-cluster/
---
...
You also need to choose a name for the database that PGD replicates. It's referred to as `<db-name>` in the examples.

Use the same database name on every node in the cluster. Different names per node can cause problems with cluster-wide operations that assume a consistent name, including how the [Connection Manager](connections) routes a client's connection to whichever node is the current write leader, using the database name from that connection.
...
```bash
pgd node <first-node-name> setup --dsn "host=<first-host> user=<db-user> port=<db-port> dbname=<db-name>" --group-name <group-name>
```
...
In this example, we will configure the PGD cluster with EDB Postgres Extended Server 17 on a CentOS/RHEL system that we [configured](02-configure-repositories) and [installed](03-installing-database-and-pgd) in the previous steps.
