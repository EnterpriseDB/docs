...
redirects:
  - /pgd/latest/geo-replication-dev/using-commit-scopes/
---
...
Quorum Commit uses eager conflict resolution, so the application encounters serialization errors at commit time more often than with other scopes. For setup, configuration, and application patterns including retry loop examples, see [Using Quorum Commit](/pgd/current/commit-scopes/quorum-commit/using-quorum-commit/).
...
!!!Important
Quorum Commit doesn't guarantee consistency when transactions using it execute concurrently on the same tables as transactions that don't. If the node group default is a Quorum Commit scope, avoid overriding it to a weaker scope for transactions that touch the same tables.
!!!
