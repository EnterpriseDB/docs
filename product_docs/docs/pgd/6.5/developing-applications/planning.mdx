...
redirects:
  - /pgd/latest/developing-applications/planning/
---
...
PGD supports partitioned tables transparently. Changes to any partition are replicated downstream automatically. For a table that grows continuously, such as an event log or audit trail, plan for [AutoPartition](/pgd/current/partitioning/) to automate the creation and dropping of partitions using low-conflict locking. Configuring AutoPartition itself is a DBA task, but identifying which tables need it and choosing a suitable partition key is a schema design decision.
...
- Use `BYTEA` columns for binary data. They replicate fully, including the underlying `TOAST` storage, up to 1 GB. If your application requires the stream-oriented large object interface, PGD also supports [replicated large objects](/pgd/current/advanced-functionality/large-objects/).
