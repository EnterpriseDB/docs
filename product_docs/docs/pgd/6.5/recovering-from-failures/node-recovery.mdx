...
As part of a physical join, `pgd node setup` takes its own base backup of the remote node using `pg_basebackup`. By default, the command then waits indefinitely for the joining node to replay WAL and promote, and a retry resumes from where a previous attempt left off rather than redoing the base backup. Set `--timeout` (or the `PGCTLTIMEOUT` environment variable) if you want to bound this wait instead of waiting indefinitely.

If you need more control over a large database join, for example to monitor the base backup and the join as separate operations, or to protect the WAL with an explicit permanent replication slot, separate the process into two steps instead. Take the base backup yourself first, then run `pgd node setup` against the already-backed-up data directory.
...
1. Run `pgd node setup`, pointing `--pgdata` at the directory you backed up in step 1. The command detects that the directory already contains a valid base backup of the source node and skips taking its own, going straight to the join. It waits indefinitely by default for WAL replay and promotion to finish, and a retry resumes rather than redoing the backup, so set `--timeout` or `PGCTLTIMEOUT` only if you want to bound the wait:
