...
!!!Note
For a large database, where replaying WAL from the base backup can take a long time, see [PLACEHOLDER: link to KB article] for a procedure that takes the base backup separately first and protects the WAL with a permanent replication slot.
!!!
