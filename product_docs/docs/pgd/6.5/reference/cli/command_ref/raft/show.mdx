...
Requires the `bdr_monitor` role or higher. See [User roles](/pgd/current/reference/cli/command_ref/#user-roles).
...
The tabular output displays a subset of available fields; `leader_id`, `voted_for_id`, and `node_id` are included only in the full field set. Use `--output json` or `--output markdown` for the full field set.
