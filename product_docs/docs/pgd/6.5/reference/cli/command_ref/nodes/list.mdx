...
Requires the `bdr_superuser` role for the default output. With `--versions`, `bdr_monitor` is sufficient. See [User roles](/pgd/current/reference/cli/command_ref/#user-roles).
