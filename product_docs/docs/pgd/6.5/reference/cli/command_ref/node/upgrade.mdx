...
|       | --check              |                     |                    | Run in dry-run mode, validating that the upgrade would succeed without performing it. Requires the new data directory to already be initialized with `--prepare`. Can't be used with `--prepare`. |
>>>>>>> 1a471aeba9 (PGD CLI changes for 6.5)
