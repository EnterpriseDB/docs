...
On a large database, replaying the write-ahead log generated during the base backup can take longer than `pg_ctl`'s default startup timeout, causing the physical join to fail and forcing you to redo the entire backup. See [Recovering large databases in two steps](/pgd/current/recovering-from-failures/node-recovery/#recovering-large-databases-in-two-steps) for a procedure that takes the base backup separately first, to avoid this.
