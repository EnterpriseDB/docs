...
| group_dsn      | An alternative DSN used for connections between nodes within the same node group (for example, a faster local network path within a data center). Falls back to `route_dsn` for connections outside the group. Added in PGD 6.4.  | string  |
