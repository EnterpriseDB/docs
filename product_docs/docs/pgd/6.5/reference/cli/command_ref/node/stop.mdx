...
Runs `pg_ctl stop` as an OS-level operation and doesn't check any PGD or Postgres role. Run the command as the OS user that owns the node's data directory. See [User roles](/pgd/current/reference/cli/command_ref/#user-roles).
