...
Requires the `bdr_read_all_stats` role or higher. See [User roles](/pgd/current/reference/cli/command_ref/#user-roles).
...
And `<OPTION>` is the name of a specific group option you want to get. If you don't specify an option, the command lists all group options. The following options are available:
...
| analytics_write_catalog    | The catalog used for writing analytics data. Requires Raft protocol version 6000 and group Raft and routing enabled. |
