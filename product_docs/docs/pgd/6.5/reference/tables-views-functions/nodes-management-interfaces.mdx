...
| `server_max_prepared_statements` | `integer` | Maximum number of prepared statements Connection Manager caches on a pooled backend before forcing a `DEALLOCATE ALL` when the connection is released back to the pool under the `fast` `server_reset_mode`. Only consulted in `transaction` pool mode. Default is `-1` (no limit). |
