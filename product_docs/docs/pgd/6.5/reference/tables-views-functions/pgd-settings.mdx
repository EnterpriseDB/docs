...
Number of worker threads for Connection Manager's multi-threaded engine (default is `-1`). A value of `-1` sizes the thread count automatically from the number of CPU cores available to the node. Valid values are `-1` to `16`. Changes take effect on the next Connection Manager restart. This parameter can be set only by the `bdr_superuser` or superuser roles.
