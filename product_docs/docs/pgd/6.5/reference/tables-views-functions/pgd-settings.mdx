...
### `bdr.watermark_wait_timeout`

For Quorum Commit, how long the apply process waits for a replicated delete or truncate to catch up with the origin before aborting and retrying the transaction. A value of `0` retries the transaction immediately whenever the node is behind. Default is 1 second.
