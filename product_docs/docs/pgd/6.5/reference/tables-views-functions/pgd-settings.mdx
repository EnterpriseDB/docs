...
Includes Postgres statistics families, the `postgres_exporter` equivalents, in the [PGD Monitor Prometheus endpoint](/pgd/current/lifecycle/monitoring/pgd-monitor/#using-prometheus) (default is `on`). A per-node parameter. Set it to `off` on a node if `postgres_exporter` already runs there and you don't want duplicate series, and reload the configuration. No restart is required.
