...
deepToC: true
redirects:
  - /pgd/latest/reference/pgd-settings/ #generated for DOCS-1247-PGD-6.0-Docs
---
...
Sets the default [sequence kind](../../advanced-functionality/sequences).
...
intervene. See [DDL replication](../../advanced-functionality/ddl) for details.
...
See [Role manipulation statements](/pgd/current/advanced-functionality/ddl/ddl-role-manipulation/)
for details.
...
| `leader` | Use leader-based global DML locking.                                                                                 |
| `auto`   | Selects `leader` if possible, otherwise `all`. (Default)                                                             |
...
A `LOG`-level log message is emitted to the PostgreSQL server logs whenever
`bdr.ddl_locking` is set to `off`. Additionally, a `WARNING` message is
written whenever any global locking steps are skipped due to this setting.
Some statements result in two `WARNING` messages: one for skipping
...
Sets the TRUNCATE command's locking behavior (default is `on`/`true`).
When `on`/`true`, TRUNCATE obeys the `bdr.ddl_locking` setting.
...
Sets the maximum allowed duration of any wait for a global lock (default is 1 minute). 
...
| Value | Description                                        |
| ----- | -------------------------------------------------- |
| `on`  | Use global locking for all table locks.  (Default) |
| `off` | Don't use global locking for table locks.          |
...
Sets the output format of [CRDT data types](/pgd/current/conflict-management/crdt/).
...
### `bdr.override_large_objects`

When set to `on`, transparently redirects PostgreSQL's [built-in large object functions](https://www.postgresql.org/docs/current/lo-funcs.html#LO-FUNCS) (`lo_create`, `lo_open`, `lo_read`, and so on) to their `bdr.lo_*` equivalents, so existing applications use replicated large objects without code changes.
...
The default is `off`. For more details, see [Replicated large objects](/pgd/current/advanced-functionality/large-objects/).

## Commit scope

### `bdr.commit_scope`

Sets the current (or default) [commit scope](/pgd/current/commit-scopes/commit-scopes/) (default
is an empty string).
...
[Transaction streaming](../../advanced-functionality/transaction-streaming).
...
value is `debug3`. If you want to see detailed PGD debug output, set
`bdr.debug_level = 'log'`.
...
Tracks apply statistics for each subscription with [`bdr.stat_subscription`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrstat_subscription) (default is `on`).

### `bdr.track_relation_apply`

Tracks apply statistics for each relation with [`bdr.stat_relation`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrstat_relation) (default is `off`).

### `bdr.track_apply_lock_timing`

Tracks lock timing when tracking statistics for relations with [`bdr.stat_relation`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrstat_relation) (default is `off`).

### `bdr.monitor_enabled`

Enables the [PGD Monitor](/pgd/current/lifecycle/monitoring/pgd-monitor) HTTP server, serving the web UI, REST API, and Prometheus endpoint (default is `off`). A per-node parameter, not a node group option. Set it on each node that should serve PGD Monitor, and reload the configuration. No restart is required.

### `bdr.monitor_prometheus_pg_metrics`

Includes Postgres statistics families, the `postgres_exporter` equivalents, in the [PGD Monitor Prometheus endpoint](/pgd/current/lifecycle/monitoring/pgd-monitor/prometheus) (default is `on`). A per-node parameter. Set it to `off` on a node if `postgres_exporter` already runs there and you don't want duplicate series, and reload the configuration. No restart is required.

### `bdr.monitor_ssl_cert_file`

SSL certificate for [PGD Monitor](/pgd/current/lifecycle/monitoring/pgd-monitor)'s HTTP server, overriding Postgres' `ssl_cert_file` (default is unset, falling back to Postgres' `ssl_cert_file`). A per-node parameter. Set it on each node individually, and reload the configuration.

### `bdr.monitor_ssl_key_file`

SSL private key for [PGD Monitor](/pgd/current/lifecycle/monitoring/pgd-monitor)'s HTTP server, overriding Postgres' `ssl_key_file` (default is unset, falling back to Postgres' `ssl_key_file`). A per-node parameter. Set it on each node individually, and reload the configuration.

### `bdr.monitor_ssl_ca_file`

SSL CA certificate for [PGD Monitor](/pgd/current/lifecycle/monitoring/pgd-monitor)'s HTTP server, overriding Postgres' `ssl_ca_file` (default is unset, falling back to Postgres' `ssl_ca_file`). A per-node parameter. Set it on each node individually, and reload the configuration.

## Decoding worker

### `bdr.enable_wal_decoder`

Enables logical change record (LCR) sending on a single node with a [decoding worker](../../advanced-functionality/decoding_worker/) (default is false). When set to true, a decoding worker process starts, and WAL senders send the LCRs it produces. If set back to false, any WAL senders using LCR are restarted and use the WAL directly.
...
Logical change record (LCR) file cleanup interval (default is 3 minutes). When the [decoding worker](../../advanced-functionality/decoding_worker/) is enabled, the decoding worker stores LCR files as a buffer. These files are periodically cleaned, and this setting controls the interval between any two consecutive cleanups. Setting it to zero disables cleanup.
...
### `bdr.conn_mgr_server_max_lifetime`

Maximum time, in seconds, that Connection Manager keeps a pooled backend server connection open before closing it (default is 3600, that is, 1 hour). Connection Manager closes unused pooled server connections that have been open longer than this value. A value of 0 disables the limit. This parameter can be set only by the `bdr_superuser` or superuser roles.

### `bdr.conn_mgr_worker_threads`

Number of worker threads for Connection Manager's multi-threaded engine (default is `-1`). A value of `-1` sizes the thread count automatically as the number of CPU cores available to the node divided by 4, clamped between 2 and 8. If the core count can't be determined, it falls back to 2. Valid values are `-1` to `16`. Changes take effect on the next Connection Manager restart. This parameter can be set only by the `bdr_superuser` or superuser roles.
...
### `bdr.prefer_analytics_engine`

Determines whether the system should route queries to the Postgres Analytics Accelerator (PGAA) engine (Seafowl or Spark), even when a local copy of the data exists in the Postgres heap. The default is `false`.
...
### `bdr.raft_vacuum_interval`

The interval between routine vacuum operations on the Raft catalog tables.
The minimum value is `60s` and the maximum value is `24h`, and it must be
set to a lower value than `bdr.raft_vacuum_full_interval`. This setting
cannot be disabled.

### `bdr.raft_vacuum_full_interval`

The interval between vacuum full operations on the Raft catalog tables.
The minimum value is `1h`, and it must be set to a higher value than
`bdr.raft_vacuum_interval`. `-1` disables this setting entirely.

### `bdr.raft_catalog_read_consistency`

Sets the consistency level for reads from catalogs that are backed by Raft consensus.
Accepted values are `local`, `group_leader`, `leader`, and `consistent`. Default is
`local`.
...
When enabled, PGD automatically synchronizes data from the node furthest ahead when a node goes down, and automatically reconciles unresolved prepared transactions left by a failed origin node. Default (from 6.0.1) is on.

### `bdr.allow_bdr_schema_changes`

Permits manual changes to database objects in the `bdr` schema. Defaults to off. Change this parameter only when explicitly instructed to do so.

### `bdr.taskmgr_nap_time`

Maximum sleep time for the task manager worker between checks for new tasks. Default is
290 seconds (`290s`). Time-unit suffixes are supported.

### `bdr.cds_memory_cache_size`

For Quorum Commit and CAMO, the target size, in bytes, of the shared memory PGD reserves for caching commit decisions. Requires a restart to take effect.

### `bdr.commit_decision_min_age`

For Quorum Commit and CAMO, how long PGD retains a commit decision after the transaction commits or aborts. Default is 15 minutes.

### `bdr.prev_leader_wait_timeout`

For Quorum Commit and CAMO, the maximum duration to wait for the previous write leader's changes before voting a Quorum Commit transaction down. A value of `0` turns off the timeout. Keep this well below the commit scope's `ABORT ON` timeout. The wait happens inside a transaction the origin is waiting on, so a longer value guarantees the origin aborts first while this node holds the transaction open. Default is 5 seconds.

### `bdr.slot_checkpoint_interval`

Interval for saving slot checkpoint data, which lets the group slot advance without
decoding large amounts of WAL. Lower values give more granular checkpoints but increase
I/O. Default is 1 minute (`1min`). Time-unit suffixes are supported.

### `bdr.ddl_queue_retention_period`

Regularly deletes queued DDL rows in `bdr.queue` beyond this age threshold. Default is
30 days (`30d`).

### `bdr.temp_directory`

Directory used to store temporary dump files during a local restore, such as when a node
joins the cluster. If not set, PGD uses the `TMPDIR` environment variable, falling back
to `/tmp` if that isn't set either.

### `bdr.shuffle_scheduled_tasks`

Toggles between scheduling and running background tasks in scan order or in random
order. Default is `on`. You can set this parameter only at Postgres server start.