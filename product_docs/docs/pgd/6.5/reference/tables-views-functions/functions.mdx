...
| --------------------- | -------- | ------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `writeleader_timeout` | interval | `'10s'` | Timeout for each write leadership transfer. The write leader is auto-selected and retried until this timeout is reached. |
...
In this portion of log, you have the information you need: the\_target\_lsn: **0/300AC18** the\_subscription: **bdr\_regression\_bdrgroup\_node1\_node2**
...
This function acquires a global DML locks on a given table. See 
[DDL locking details](/pgd/current/advanced-functionality/ddl/ddl-locking/)
 for information about global DML lock.
...
| Parameter | Description |
| --- | --- |
| `node_group` | Name of the group where the lease override is applied. |
| `node` | Name of the node that will hold the lease. |
| `duration` | Duration of the lease in seconds. Providing a value of `0` or less cancels the current lease. |
| `kind` | The lease kind (for example, `analytics`). | 
| `local_only` | When `false` (default), the override is sent over consensus to all nodes. When `true`, the override only affects the local node. Use `true` only if consensus is lost. | 
...
`bdr.replicate_ddl_command()` always replicates the command and is unaffected by the setting of 
...
Resets statistics for the specified category. Running this function clears the value of the column `stats_reset` in the [bdr.stat_reconcile_commit_scope](catalogs-visible#bdrstat_reconcile_commit_scope) view.
...
| Parameter | Description |
| --- | --- |
| stats_tupe | The category of statistics to reset. Valid values are `commit_scope` and `reconcile_commit_scope`. If `NULL`, the function defaults to `commit_scope`. |
...
For convenience, PGD provides a variant of this function for CAMO and the CAMO partner node. See 
[bdr.wait\_for\_camo\_partner\_queue](#bdrwait_for_camo_partner_queue)
.
...
Upon being called, the function waits for a node to pass a certain LSN. If no LSN is supplied, the current wal\_flush\_lsn (using the `pg_current_wal_flush_lsn()` function) position is used on the local node. Supplying a node name parameter tells the function to wait for that node to pass the LSN. If no node name is supplied (by passing NULL), the function waits until all the nodes pass the LSN.

We recommend using this function if you are using 
[Optimized topology](../../nodes/subscriber_only/optimizing-so)
 instead of 
[`bdr.wait_slot_confirm_lsn`](/pgd/current/reference/tables-views-functions/functions#bdrwait_slot_confirm_lsn)
.
...
If a node is currently down, isn't updating, or simply can't be connected to, the wait will continue indefinitely. To avoid this condition, set the statement\_timeout to the maximum amount of time you are prepared to wait.
...
| `target_lsn` | LSN to wait for. If NULL, uses the current wal\_flush\_lsn on the local node. |
...
If you are using 
[Optimized topology](/pgd/current/nodes/subscriber_only/optimizing-so/)
, we recommend using 
[`bdr.wait_node_confirm_lsn`](/pgd/latest/reference/tables-views-functions/functions#bdrwait_node_confirm_lsn)
 instead.
...
as explained in [Monitoring](/pgd/current/lifecycle/monitoring/sql/#monitoring-pgd-versions).
...
This function returns a record with fields `status` and `message`,
as explained in [Monitoring](/pgd/current/lifecycle/monitoring/sql/#monitoring-raft-consensus).
...
This function also provides status information on subscriber-only nodes that are operating as subscriber-only group leaders in a PGD cluster when [optimized topology](/pgd/current/nodes/subscriber_only/optimizing-so/) is enabled.
...
Further explaination is available in [Monitoring replication slots](/pgd/current/lifecycle/monitoring/sql/#monitoring-replication-slots).

### `bdr.ts_metrics`

This function lists every registered timeseries metric definition. PGD Monitor
uses this registry to give the same metric names and descriptions across SQL,
its REST API, its web UI, and its Prometheus `/metrics` endpoint.
...
| -------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `metric_id`    | Internal numeric identifier for the metric.                                                                            |
| `metric_name`  | The metric's name. The same name is used in the REST API, the web UI, and the Prometheus `/metrics` endpoint.         |
| `kind`         | The metric's kind, `gauge` or `histogram`.                                                                             |
| `value_type`   | The type used to store sampled values, `int8` or `float8`.                                                             |
| `display_kind` | How the PGD Monitor web UI charts summarize the metric when zoomed out to a coarser time range (mean, mean with a minimum/maximum band, minimum, or maximum). |
| `unit`         | The metric's unit, for example bytes, percent, or seconds.                                                             |
| `description`  | A description of the metric.                                                                                           |

#### Notes

Executable by roles with `bdr_superuser`, `bdr_read_all_stats`, or `bdr_monitor`
membership.

See [Monitoring metrics reference](/pgd/current/reference/tables-views-functions/monitoring-metrics)
for the full list of registered metrics, and
[PGD Monitor](/pgd/current/lifecycle/monitoring/pgd-monitor) for the interfaces
that expose them.

### `bdr.ts_latest`

This function reads the most recently sampled value for a named timeseries
metric.
...
| Parameter     | Description                                                            |
| ------------- | ----------------------------------------------------------------------- |
| `metric_name` | The metric to read, matching a `metric_name` value from `bdr.ts_metrics()`. |
...
| `value_int`   | The sampled value, when the metric's `value_type` is `int8`.   |
| `value_float` | The sampled value, when the metric's `value_type` is `float8`. |
...
This function reads recent errors captured by the timeseries error ring
buffer, which backs the PGD Monitor web UI's Error Log page.
...
| Parameter     | Description                                          |
| ------------- | ------------------------------------------------------ |
| `max_entries` | Maximum number of error entries to return. Defaults to 50. |
...
| `sqlerrcode` | The error's SQLSTATE code.               |
| `message`    | The error message.                       |
| `detail`     | Additional detail for the error, if any. |
...
This function reads historical timeseries samples for a metric at a given
downsampling frequency level, retrieved over IPC from the monitor background
worker. It backs the timeseries charts in the PGD Monitor web UI.
...
| ------------- | ---------------------------------------------------------------------------------- |
| `metric_name` | The metric to read, matching a `metric_name` value from `bdr.ts_metrics()`.        |
| `level`       | The downsampling frequency level to read, from `0` (finest) to `4` (coarsest). Defaults to `0`. |
...
| ----------- | -------------------------------------------------------------- |
| `ts`        | Timestamp of the sample.                                   |
| `value`     | The sampled value, or the mean value for a downsampled window. |
| `min`       | The minimum value in the sample window, for a downsampled window. |
| `max`       | The maximum value in the sample window, for a downsampled window. |
...
For a histogram-kind metric, such as `pgd_commit_latency_milliseconds`, this
function reads percentile summaries (p50, p90, p99) at a given downsampling
frequency level.
...
| Parameter     | Description                                                          |
| ------------- | ------------------------------------------------------------------------ |
| `metric_name` | The histogram metric to read, matching a `metric_name` value from `bdr.ts_metrics()`. |
| `level`       | The downsampling frequency level to read, from `3` to `4`. Defaults to `3`. |

#### Output columns

| Column name      | Description                                    |
| ---------------- | --------------------------------------------------- |
| `ts`              | Timestamp of the sample window.                     |
| `p50`             | 50th percentile value in the window.                |
| `p90`             | 90th percentile value in the window.                |
| `p99`             | 99th percentile value in the window.                |
| `count`           | Number of observations in the window.               |
| `sum`             | Sum of observations in the window.                  |
| `overflow_count`  | Observations that exceeded the histogram's largest bucket boundary. |
...
For a histogram-kind metric, this function returns per-bucket observation
counts aggregated across all samples at a given downsampling frequency level.
...
| Column name      | Description                                    |
| ------------------ | --------------------------------------------------- |
| `bucket_upper_ms` | Upper boundary of the bucket, in milliseconds. |
...
For a histogram-kind metric, this function returns cumulative-since-start
bucket counters, one row per bucket boundary ending with `+Inf`. This is the
SQL equivalent of the `_bucket`, `_sum`, and `_count` series that Prometheus
histograms expose.
...
| Column name          | Description                                                  |
| ----------------------- | ----------------------------------------------------------------- |
| `bucket_le_ms`         | Bucket boundary, in milliseconds (the bucket covers values less than or equal to this boundary). |
| `cumulative_count`     | Cumulative number of observations at or below the boundary.        |
| `sum_ms`               | Sum of all observations, in milliseconds.                          |
| `total_count`          | Total number of observations across all buckets.                   |

#### Notes

Executable by roles with `bdr_superuser`, `bdr_read_all_stats`, or `bdr_monitor`
membership. Buckets are cumulative monotone counters, matching the convention
`histogram_quantile()` and `rate()` expect in PromQL.

### `bdr.wal_sender_stats`

If the [decoding worker](../../advanced-functionality/decoding_worker/) is enabled, this
...
If the [decoding worker](../../advanced-functionality/decoding_worker/) is enabled, this function
...
For details, see [Monitoring WAL senders using LCR](/pgd/current/lifecycle/monitoring/sql/#monitoring-wal-senders-using-lcr).
...
| `leader_name`      | text     | `NULL`       | Name of node that will become write leader. If NULL, the system automatically selects the most appropriate candidate based on write leader progress.                                                  |
| `transfer_method`  | text     | `'strict'` | Type of the transfer. It can be `'fast'` or the default, `'strict'`, which checks the maximum lag.                                                                                                   |
| `transfer_timeout` | interval | `'10s'`    | Timeout of the leadership transfer. Default is 10 seconds. When using auto-selection (`leader_name` is NULL), the system retries selecting the furthest-progressed node until the timeout is reached. |
...
- Route-fenced nodes (nodes with `route_fence` enabled via [`bdr.alter_node_option`](/pgd/current/reference/tables-views-functions/nodes-management-interfaces/#bdralter_node_option)) aren't considered candidates for leadership transfer.
...
These functions let you review, resolve, and analyze replication failures recorded by the apply error policy. See the `apply_error_policy`, `apply_error_max_retries`, `apply_error_max_skips`, and `apply_error_max_record_size` options in [`bdr.alter_node_group_option`](/pgd/current/reference/tables-views-functions/nodes-management-interfaces#bdralter_node_group_option) for configuration details. For a step-by-step guide to configuring a policy and resolving failures, see [Handling replication apply errors](/pgd/current/recovering-from-failures/apply-error-policy).
...
| Parameter | Description |
| --- | --- |
| `change_id` | Primary key of the row in `bdr.pending_failed_changes` to skip. |
...
| `remote_xid` | Transaction ID on the origin node. |
| `remote_commit_lsn` | Commit LSN of the failed transaction on the origin node. |
| `failed_txn_id` | UUID of the failed transaction from `bdr.pending_failed_changes`. Use as a convenient alternative to specifying `subscription_name`, `remote_xid`, and `remote_commit_lsn`. |
| `dry_run` | When `true`, returns the count of changes that would be skipped without modifying anything. Default is `false`. |
| `re_enable` | When `true`, re-enables the subscription after skipping. Default is `false`. |
...
| Parameter | Description |
| --- | --- |
| `change_id` | Primary key of the row in `bdr.pending_failed_changes` to apply. |
...
| `remote_commit_lsn` | Commit LSN of the failed transaction on the origin node. |
| `failed_txn_id` | UUID of the failed transaction from `bdr.pending_failed_changes`. Use as a convenient alternative to specifying `subscription_name`, `remote_xid`, and `remote_commit_lsn`. |
| `dry_run` | When `true` (the default), shows what would happen without actually applying changes (rolls back at the end). Pass `false` to execute. |
| `re_enable` | When `true` and `dry_run` is `false`, re-enables the subscription after applying. Default is `false`. |
...
| --- | --- | --- |
| `change_num` | `integer` | Order of the change within the transaction. |
| `change_id` | `bigint` | Primary key of the row in `bdr.pending_failed_changes`. |
| `operation` | `text` | Operation type: `INSERT`, `UPDATE`, `DELETE`, `SQL`, `DDL`, or `TRUNCATE`. |
| `schema_name` | `text` | Schema of the target table. |
| `table_name` | `text` | Name of the target table. |
| `status` | `text` | Result of the apply attempt. In dry-run mode, values are `would_apply`, `would_skip`, or `would_error`. When executed, values are `apply`, `skip`, or `error`. |
| `message` | `text` | Error message if the change failed, or `NULL` on success. |
...
| Parameter | Description |
| --- | --- |
| `change_id` | Primary key of the row in `bdr.pending_failed_changes` to modify. |
| `new_values` | JSONB object containing the updated column values to use when the change is retried. |
...
| Parameter | Description |
| --- | --- |
| `operation` | Operation type. Valid values are `INSERT`, `UPDATE`, `DELETE`, `SQL`, `DDL`, and `TRUNCATE`. |
| `schema_name` | Schema of the target table. |
| `table_name` | Name of the target table. |
| `key_columns` | JSONB object containing the primary key column values. |
| `old_values` | JSONB object containing the before-image column values, used for `UPDATE` and `DELETE` operations. |
| `new_values` | JSONB object containing the after-image column values for `INSERT` and `UPDATE` operations, or the SQL text for `DDL` and `SQL` operations. |
...
| `remote_commit_lsn` | Commit LSN of the failed transaction on the origin node. |
| `failed_txn_id` | UUID of the failed transaction from `bdr.pending_failed_changes`. Use as a convenient alternative to specifying `subscription_name`, `remote_xid`, and `remote_commit_lsn`. |
| `resolution` | Resolution action to apply. Valid values are `skipped`, `applied`, and `discarded`. Default is `skipped`. |
| `re_enable` | When `true`, re-enables the subscription after resolving. Default is `false`. |
| `dry_run` | When `true` (the default), returns a detailed preview of what the operation would do without making any changes. Pass `false` to execute. |
...
| Parameter | Description |
| --- | --- |
| `change_id` | Primary key of the row in `bdr.pending_failed_changes` to analyze. |
| `verify_state` | When `true` (the default), queries the local database to verify current state before assessing safety. Pass `false` for a faster static analysis based on error code and operation type only. |
| `include_transaction_analysis` | When `true` (the default), examines all other changes in the same transaction to detect dependencies and generate warnings. |
...
| Column | Type | Description |
| --- | --- | --- |
| `skip_safety` | `text` | Safety classification: `safe`, `caution`, or `risky`. |
| `recommendation` | `text` | Human-readable recommendation. |
| `explanation` | `text` | Detailed explanation including error context and any state verification results. |
| `data_impact` | `text` | Data impact level: `none`, `possible_divergence`, or `certain_divergence`. |
| `suggested_action` | `text` | Suggested action: `skip`, `apply_modified`, or `manual_review`. |
| `dependency_warnings` | `text[]` | Array of warnings about related changes in the transaction that will also be affected, or `NULL` if none. |

#### Safety classifications

| Safety | Description |
| --- | --- |
| `safe` | Skipping the change is unlikely to cause data divergence between nodes. |
| `caution` | Minor potential issues exist. Review the change before skipping. |
| `risky` | Skipping the change will likely cause data divergence between nodes. |
...
| Column | Type | Description |
| --- | --- | --- |
| `change_seq` | `integer` | Order of the change within the transaction. |
| `operation` | `text` | Operation type: `INSERT`, `UPDATE`, `DELETE`, `SQL`, `DDL`, or `TRUNCATE`. |
| `qualified_table` | `text` | Schema-qualified table name. |
| `skip_safety` | `text` | Safety classification: `safe`, `caution`, or `risky`. |
| `recommendation` | `text` | Human-readable recommendation for the change. |
| `has_dependents` | `boolean` | Whether other changes in the transaction may depend on the outcome of this change. |
