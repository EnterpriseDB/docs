...
bdr.alter_table_conflict_detection(relation regclass,
                                   ...
                                   detector_args jsonb DEFAULT NULL)
```
...
-   `detector_args` — Extra options for the conflict detection method, as a JSON object. Only the `row_version` method accepts an argument: `atttype`, which sets the data type of the row-version tracking column to `smallint`, `integer`, or `bigint`. The default is `integer`.

The recognized methods for conflict detection are:

-   `row_origin` — Origin of the previous change made on the tuple (see [Origin conflict detection](/pgd/current/conflict-management/conflicts/03_conflict_detection/#origin-conflict-detection)). This is the only method supported that doesn't require an extra column in the table.
-   `row_version` — Row version column (see [Row version conflict detection](/pgd/current/conflict-management/conflicts/03_conflict_detection/#row-version-conflict-detection)).
-   `column_commit_timestamp` — Per-column commit timestamps (described in [CLCD](/pgd/current/conflict-management/column-level-conflicts/)).
-   `column_modify_timestamp` — Per-column modification timestamp (described in [CLCD](/pgd/current/conflict-management/column-level-conflicts/)).
...
This function uses the same replication mechanism as `DDL` statements. This means the replication is affected by the [ddl filters](../../advanced-functionality/repsets/#ddl-replication-filtering) configuration.
...
-   `conflict_type` — Conflict type for which to apply the setting (see [List of conflict types](conflicts/#list-of-conflict-types)).
-   `conflict_resolver` — Resolver to use for the given conflict type (see [List of conflict resolvers](conflicts/#list-of-conflict-resolvers)).
