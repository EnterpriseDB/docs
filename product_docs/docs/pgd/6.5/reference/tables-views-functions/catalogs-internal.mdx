...
| ap_minimum_advance_partitions   | integer                  |             |
...
| ap_analytics_offload_period     | interval                 |             |
| ap_drop_after_retention_period  | boolean                  |             |
...
protocol - leader, follower, or candidate to leader); see [Monitoring Raft consensus](/pgd/current/lifecycle/monitoring/sql#monitoring-raft-consensus)
* Whenever a worker has errored out (see [bdr.workers](/pgd/latest/reference/tables-views-functions/catalogs-visible/#bdrworkers)
and [Monitoring PGD replication workers](/pgd/current/lifecycle/monitoring/sql#monitoring-pgd-replication-workers))
...
### `bdr.leader_tenure_backlog`

An internal catalog table that tracks outstanding write-leader catchup debt on the local node, used with [Quorum Commit](/pgd/current/commit-scopes/quorum-commit/). This is a local-only catalog, meaning it isn't replicated, and the values apply only to the local node.

Each row represents one leader tenure the local node has heard about and hasn't yet caught up on. Before applying a Quorum Commit transaction written by a leader at or after `from_lsn`, the local node must have applied everything the previous leader committed at or below `prev_lsn`. Rows are keyed by `leader_node_id` and `leader_version`, since several tenures can be outstanding at once, including two for the same node when it's elected, interrupted, and re-elected.

#### `bdr.leader_tenure_backlog` columns

| Name           | Type   | Description                                                          |
| -------------- | ------ | --------------------------------------------------------------------- |
| leader_node_id | oid    | OID of the write leader for this tenure.                              |
| leader_version | bigint | Version of the write leader tenure.                                   |
| from_lsn       | pg_lsn | LSN in the leader's replication stream where this tenure begins.      |
| prev_node_id   | oid    | OID of the previous write leader.                                     |
| prev_lsn       | pg_lsn | LSN up to which the local node must have applied the previous leader's changes before applying transactions from this tenure. |

### `bdr.local_leader_change`

This is a local cache of the recent portion of leader change history. It has the same fields as [`bdr.leader`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrleader), except that it is an ordered set of (node_group_id, leader_kind, generation) instead of a map tracking merely the current version.
...
| node_group_dsn          | text     | Preferred connection string for nodes in the same subgroup. When set, nodes within the same subgroup connect using this DSN instead of `node_dsn`. Defaults to empty. Set using [`bdr.alter_node_option`](/pgd/current/reference/tables-views-functions/nodes-management-interfaces#bdralter_node_option) with key `group_dsn`. |
...
An internal state table storing the current state of 2 phase commit reconciliation requests. The view [bdr.reconcile_2pc_requests_summary](#bdrreconcile_2pc_requests_summary) provides a human-readable representation of this table.
...
A view providing a human-readable summary of the reconciliation requests stored in [bdr.reconcile_2pc_requests](#bdrreconcile_2pc_requests).
