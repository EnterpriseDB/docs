...
apply to them, if that's currently active. See [DDL replication](../../advanced-functionality/ddl).
!!!
...
function calls. See [DDL filters](../../advanced-functionality/repsets#ddl-replication-filtering) for how to set
...
that the replication is affected by the [DDL filters](../../advanced-functionality/repsets#ddl-replication-filtering)
configuration.
...
This function uses the same replication mechanism as `DDL` statements. This means
the replication is affected by the [DDL filters](../../advanced-functionality/repsets#ddl-replication-filtering)
configuration.
...
```sql
bdr.drop_replication_set(set_name name, ifexists boolean DEFAULT false)
```

### Parameters

-   `set_name` &mdash; Name of an existing replication set.
-   `ifexists` &mdash; When `true`, doesn't raise an error if the replication set doesn't exist.
...
bdr.alter_node_replication_sets(node_name name,
                                ...
                                pub_repsets text[] DEFAULT NULL,
                                sub_repsets text[] DEFAULT NULL)
```

### Parameters

-   `node_name` &mdash; The node to modify. Currently must be a local node.
-   `set_names` &mdash; Replication sets applied to both the published and subscribed
    lists. Omit or pass `NULL` to leave the common list unchanged.
-   `pub_repsets` &mdash; Additional replication sets added to the node's published
    list only, on top of `set_names`. Omit or pass `NULL` for none.
-   `sub_repsets` &mdash; Additional replication sets added to the node's subscribed
    list only, on top of `set_names`. Omit or pass `NULL` for none.

You must give at least one of `set_names`, `pub_repsets`, or `sub_repsets` a
non-`NULL` value. Calling the function without specifying any of them raises
an error.

The node's final published replication sets are the union of `set_names` and
`pub_repsets`. The node's final subscribed replication sets are the union of
`set_names` and `sub_repsets`. Names appearing in more than one array aren't
...
versions of PGD.) This is currently true even if the [DDL filters](../../advanced-functionality/repsets#ddl-replication-filtering)
