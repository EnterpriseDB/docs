...
deepToC: true
redirects:
  - /pgd/latest/reference/functions-internal/ #generated for DOCS-1247-PGD-6.0-Docs
---
...
This function is used by the upgrade scripts to transfer commit decisions
saved in bdr.commit_decisions catalog to the shared memory hash table.
...
Use [`bdr.part_node`](/pgd/latest/reference/tables-views-functions/nodes-management-interfaces#bdrpart_node) to remove a node from a PGD group. That function sets the node to `PARTED` state and enables reuse of the node name.
...
```sql
bdr.drop_node(node_name text, force boolean DEFAULT false)
```
...
### `bdr.reconcile_2pc_status_name`

Converts a numerical reconciliation status code into its corresponding textual representation. This function is primarily used to populate the [bdr.reconcile_2pc_requests_summary](catalogs-internal#bdrreconcile_2pc_requests_summary) view.

#### Synopsis

```sql
bdr.reconcile_2pc_status_name(reconcile_status oid) RETURNS text
```
...
| Parameter | Description                           |
|-----------|---------------------------------------|
| `reconcile_status` | The OID code representing the specific reconciliation status to be converted. |
...
Returns the minimum value needed for `max_replication_slots` so a node join can complete without exhausting available slots. The estimate reserves headroom for the temporary catch-up subscription that briefly coexists with the steady-state subscriptions during a join. On Postgres versions earlier than 18, where `max_active_replication_origins` is aliased to `max_replication_slots`, the returned value also covers the origin requirement. PGD-CLI uses this function internally, but you can also run it directly when sizing a cluster.

### `bdr.get_min_required_active_replication_origins`

Returns the minimum value needed for `max_active_replication_origins`, the configuration parameter introduced in Postgres 18. On earlier Postgres versions, where `max_active_replication_origins` is aliased to `max_replication_slots`, this value is informational only. PGD-CLI uses this function internally, but you can also run it directly when sizing a cluster.

### `bdr.get_min_required_worker_processes`

Internal function intended for use by PGD-CLI.

### `bdr.stat_last_reset_time`

Internal function used to retrieve the most recent reset timestamp for a specific statistics category. For standard monitoring, each statistic that supports [bdr.stat_reset](functions#bdrstat_reset) includes a `stats_reset` column in the [bdr.stat_commit_scope](catalogs-visible#bdrstat_commit_scope) view, which provides this information in a more accessible format for user consumption.

### `bdr.stat_get_activity`

Internal function underlying view `bdr.stat_activity`. Do not use directly. Use the [`bdr.stat_activity`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrstat_activity) view instead.

### `bdr.stat_get_commit_scope`

Internal function to fetch commit scope statistics. For standard monitoring and troubleshooting, use the [bdr.stat_commit_scope](catalogs-visible#bdrstat_commit_scope) view, which provides the same information in a more accessible format. 

### `bdr.stat_get_commit_scope_proc_wait`

Internal function used to retrieve the wait state statistics for processes associated with specific commit scopes. For standard monitoring and troubleshooting, use the [bdr.stat_commit_scope](catalogs-visible#bdrstat_commit_scope) view, which provides a more accessible representation of this data.

### `bdr.stat_get_reconcile_commit_scope`

Internal function used to retrieve reconciliation statistics for commit scopes. For standard monitoring and troubleshooting, use the [bdr.stat_reconcile_commit_scope](catalogs-visible#bdrstat_reconcile_commit_scope) view, which provides a more accessible and user-friendly representation of this data.

### `bdr.worker_role_id_name`

Internal helper function used when generating view `bdr.worker_tasks`. Do not use directly. Use the [`bdr.worker_tasks`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrworker_tasks) view instead.

### `bdr.lag_history`

Internal function used when generating view `bdr.node_replication_rates`. Do not use directly. Use the [`bdr.node_replication_rates`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrnode_replication_rates) view instead.

### `bdr.get_raft_instance_by_nodegroup`

Internal function used when generating view `bdr.group_raft_details`. Do not use directly. Use the [`bdr.group_raft_details`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrgroup_raft_details) view instead.

### `bdr.monitor_camo_on_all_nodes`

Internal function used when generating view `bdr.group_camo_details`. Do not use directly. Use the [`bdr.group_camo_details`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrgroup_camo_details) view instead.
...
Internal function used when generating view `bdr.group_replslots_details`. Do not use directly. Use the [`bdr.group_replslots_details`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrgroup_replslots_details) view instead.

### `bdr.monitor_subscription_details_on_all_nodes`

Internal function used when generating view `bdr.group_subscription_summary`. Do not use directly. Use the [`bdr.group_subscription_summary`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrgroup_subscription_summary) view instead.

### `bdr.monitor_version_details_on_all_nodes`

Internal function used when generating view `bdr.group_versions_details`. Do not use directly. Use the [`bdr.group_versions_details`](/pgd/latest/reference/tables-views-functions/catalogs-visible#bdrgroup_versions_details) view instead.
