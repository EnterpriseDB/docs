...
To perform maintenance on a node in a cluster running Quorum Commit transactions, for example before taking it offline, transfer leadership away from the node without naming a specific replacement. Quorum Commit always routes writes through the best available write leader, and assigning a specific new leader can conflict with that selection. Use [`bdr.transfer_all_leaderships`](/pgd/current/reference/tables-views-functions/functions/#bdrtransfer_all_leaderships) to drain all write and Raft leaderships from the node instead:

```sql
SELECT bdr.transfer_all_leaderships();
```

The CLI commands [`pgd group set-leader`](/pgd/current/reference/cli/command_ref/group/set-leader) and [`pgd raft set-leader`](/pgd/current/reference/cli/command_ref/raft/set-leader) require naming a specific replacement leader, so don't use them to move leadership away from a node running Quorum Commit.
...
From PGD 6.5, Quorum Commit is compatible with [transaction streaming](/pgd/current/advanced-functionality/transaction-streaming/) using any value of [`bdr.default_streaming_mode`](/pgd/current/reference/tables-views-functions/pgd-settings/#bdrdefault_streaming_mode) (`off`, `file`, `writer`, and `auto`). PGD detects Quorum Commit transactions at the origin and streams them to file regardless of the cluster's `default_streaming_mode` setting, so no special configuration is required.

During a rolling upgrade, keep `bdr.default_streaming_mode` set to `file` or `off` until every node is on PGD 6.5 or later. Nodes on earlier versions don't emit the hint Quorum Commit relies on, so PGD logs a warning at `create_commit_scope` time while any peer is on an earlier version.
