...
From PGD 6.5, Quorum Commit is compatible with [transaction streaming](/pgd/current/transaction-streaming/) using any value of [`bdr.default_streaming_mode`](/pgd/current/reference/tables-views-functions/pgd-settings/#bdrdefault_streaming_mode) (`off`, `file`, `writer`, and `auto`). PGD detects Quorum Commit transactions at the origin and streams them to file regardless of the cluster's `default_streaming_mode` setting, so no special configuration is required.
