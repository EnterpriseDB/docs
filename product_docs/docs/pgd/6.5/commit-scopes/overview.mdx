...
redirects:
  ...
  - /pgd/latest/reference/commit-scopes/overview/ #generated for DOCS-3034
---
...
-   [Quorum Commit](quorum-commit): Quorum Commit internally uses two-phase commit to guarantee that a transaction commits or rolls back in the entire cluster, effectively serializing conflicting transactions across the cluster.

-   [CAMO](camo): This kind of commit scope works with the client to verify that a transaction
    was committed before retrying, preventing duplicate commits in failover scenarios.
...
Synchronous Commit and CAMO each support [degrading commit scope rules](degrading), for even further control of durability.
