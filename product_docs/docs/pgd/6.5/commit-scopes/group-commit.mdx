---
title: Group Commit (legacy)
navTitle: Group Commit (legacy)
redirects:
  - /pgd/latest/bdr/group-commit/
  - /pgd/latest/commit-scopes/group-commit/ #generated for DOCS-1247-PGD-6.0-Docs
  - /pgd/latest/reference/commit-scopes/group-commit/ #generated for DOCS-3034
deepToC: true
---
...
!!! Warning Deprecated
Group Commit was offered as an experimental feature intended for preview and evaluation purposes. It had known limitations and challenges that made it unsuitable for production environments. It has been superseded by [Quorum Commit](quorum-commit), which addresses these limitations, and will be removed in a future major version.
!!!
...
commit](/pgd/current/overview/terminology/#two-phase-commit-2pc) underneath. Therefore, configure
`max_prepared_transactions` high enough to handle all such transactions
originating per node.

## Limitations

See the Group Commit section of [Known Issues and Limitations](/pgd/current/overview/known_issues/#group-commit).
...
See [Conflicts](/pgd/current/conflict-management/conflicts/) for details about how the asynchronous
...
See also [Limitations](/pgd/current/overview/known_issues/#general-durability-limitations). 
...
When `commit_decision` is `raft`, then, Raft acts as the coordinator, and these
...
the write lead in the cluster performs the role of coordinator. This is because 
