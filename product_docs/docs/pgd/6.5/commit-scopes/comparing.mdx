...
redirects:
  ...
  - /pgd/latest/reference/commit-scopes/comparing/ #generated for DOCS-3034
---
...
!!!Note
[Quorum Commit](quorum-commit/) supports only `ON DURABLE` and `ON VISIBLE`. `ON VISIBLE` is the default.
!!!
...
When used with the Quorum Commit commit scope kind, this also means the changes
are visible.
