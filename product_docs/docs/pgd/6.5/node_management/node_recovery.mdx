...
redirects:
  ...
  - /pgd/latest/reference/node_management/node_recovery/ #generated for DOCS-3034
---
...
(see [Origin conflict detection](/pgd/current/conflict-management/conflicts/)).

Administrators must monitor for node outages (see [Monitoring](/pgd/current/lifecycle/monitoring/))
...
When a node is down for an extended period or lost permanently, the recommended strategy is to part the failing node and rejoin a new one in its place. The new node can use the same name as the original.
