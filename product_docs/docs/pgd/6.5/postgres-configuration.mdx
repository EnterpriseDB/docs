...
redirects:
  ...
  - /pgd/latest/reference/postgres-configuration/ #generated for DOCS-3034
---
...
- `shared_preload_libraries` &mdash; Must include `bdr` to enable the extension. Most other
  extensions can appear before or after the `bdr` entry in the comma-separated list. Don't include
  `pglogical` in this list.
- `output_plugin_libraries` &mdash; Must include `bdr` to allow it as a trusted output plugin for
  logical decoding. Introduced in community Postgres 14.24, 15.19, 16.15, 17.11, and 18.6. Check the
  current value with `SHOW output_plugin_libraries;`, then edit `postgresql.conf` directly to
  append `bdr` to it, for example:

  ```
  output_plugin_libraries = 'pgoutput, test_decoding, bdr'
  ```

  Don't use `ALTER SYSTEM SET` for this, since it fails on Postgres versions that don't have the
  parameter yet. Reload the configuration once you've made the change:
  ...
  for the minimum EDB Postgres Extended and EDB Postgres Advanced Server versions PGD 6.5.0 requires.
...
-   `max_active_replication_origins` &mdash; Three needed for every peer node. Available for PostgreSQL 18 and later.
...
When the [decoding worker](advanced-functionality/decoding_worker) is enabled, this
...
While certain PostgreSQL configuration parameters are useful for standalone instances, they can negatively impact PGD cluster stability, background workers, and replication processes.

The following parameters should not be enabled globally in `postgresql.conf`, nor should they be included as options in PGD node data source names (DSNs):

- `idle_session_timeout`
- `transaction_timeout`

Set these parameters on a per-session basis instead to make sure that they do not interfere with system-level operations.