...
redirects:
  ...
  - /pgd/latest/reference/conflict-management/crdt/05_crdt-reset/ #generated for DOCS-3034
---
...
Operation-based CRDT types can be reset reliably only using a commit scope that performs Eager Replication, such as [Quorum Commit](/pgd/current/commit-scopes/quorum-commit/), since this avoids multiple concurrent resets. You can also use a Quorum Commit scope to set either kind of CRDT to a specific value.
