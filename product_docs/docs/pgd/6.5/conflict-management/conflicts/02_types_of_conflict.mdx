...
!!! Note
    ...
    From PGD 6.5, PGD rejects `VACUUM FULL` or `CLUSTER` run directly on a `TOAST` table by default. Running either command on just the `TOAST` table without also running it on the main table removes metadata needed for the extra logging to work, leaving the protection against these concurrency issues absent for a short period. To run these commands directly on a `TOAST` table anyway, enable [`bdr.permit_unsafe_commands`](/pgd/current/reference/tables-views-functions/pgd-settings/#bdrpermit_unsafe_commands).
