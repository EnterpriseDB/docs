...
redirects:
  ...
  - /pgd/latest/reference/conflict-management/conflicts/02_types_of_conflict/ #generated for DOCS-3034
---
...
You can effectively eliminate this type of conflict by using [global sequences](/pgd/current/advanced-functionality/sequences/#pgd-global-sequences).
...
You can also define a different behavior using a [conflict trigger](/pgd/current/advanced-functionality/stream-triggers/#conflict-triggers).
...
To see how these updates behave, create a basic table with a primary key and insert an initial row:
...
Updating a primary key column is technically possible. This command succeeds because the new value does not conflict with any existing data:
...
Confirm the changes to the table by selecting all rows. Notice that the primary key values have successfully shifted:
```sql
SELECT * FROM pktest;
 pk | val
----+-----
...
Select all rows from the table and obvserve a standard constraint violation error:
...
Even without PGD, you must design your application to prevent these runtime errors, as Postgres strictly enforces primary key uniqueness.

Distributed environments like PGD increase the complexity of primary key updates. When multiple nodes attempt to modify keys simultaneously, the results depend on whether the changes overlap or conflict.

If two nodes update different rows to different new primary key values, PGD can replicate the changes successfully:

```sql
--node1:
UPDATE pktest SET pk=pk+1 WHERE pk = 2;

After replication, both nodes reflect the incremented values, maintaining a consistent state across the cluster:

--node2:
UPDATE pktest SET pk=pk+1 WHERE pk = 4;
```
...
Executing these next two changes concurrently causes a divergent error, since both changes are accepted. Both nodes accept their own local change, but applying the changes on the other node results in `update_missing` conflicts.

```sql
--node1: 
UPDATE pktest SET pk=1 WHERE pk = 3;
--node2: 
UPDATE pktest SET pk=2 WHERE pk = 3;
```
...
```sql
--node1: 
UPDATE pktest SET pk=6, val=8 WHERE pk = 5;
--node2: 
UPDATE pktest SET pk=6, val=9 WHERE pk = 5;
```
...
changes, make those changes using [Quorum Commit](/pgd/current/commit-scopes/quorum-commit/), which uses eager conflict resolution to prevent concurrent conflicting commits before any node commits.
...
Another alternative is to use [eager conflict resolution](/pgd/current/commit-scopes/quorum-commit/how-quorum-commit-works#eager-conflict-resolution) to prevent these conflicts. Eager conflict resolution is always active with [Quorum Commit](/pgd/current/commit-scopes/quorum-commit/) and aborts conflicting transactions before either commits.
...
!!! Note
    ...
    From PGD 6.5, PGD rejects `VACUUM FULL` or `CLUSTER` run directly on a `TOAST` table by default. Run these commands on the owning table instead, which rewrites its `TOAST` table as part of the same operation and keeps the extra logging metadata intact. Running either command directly on just the `TOAST` table removes that metadata, leaving the protection against these concurrency issues absent for a short period. To run these commands directly on a `TOAST` table anyway, enable [`bdr.permit_unsafe_commands`](/pgd/current/reference/tables-views-functions/pgd-settings/#bdrpermit_unsafe_commands).
