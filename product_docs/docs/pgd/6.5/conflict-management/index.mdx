...
If you want to avoid conflicts, use [Quorum Commit](/pgd/current/commit-scopes/quorum-commit/) or conflict-free data types (CRDTs), described in [CRDT](crdt). You can also use Connection Manager to route all writes to one write-leader, eliminating the chance for inter-nodal conflicts.
