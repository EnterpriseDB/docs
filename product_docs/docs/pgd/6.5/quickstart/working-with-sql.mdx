...
The comand gives you a shell inside the `host-1` container where you can run PGD and SQL commands against the PGD cluster.
...
1. As the a cluster has no users apart from the `postgres` superuser and only one replicated database (`pgddb`), connect to the cluster using the following command:
...
1. Verify which node you are connected to in the cluster:
...
1. Verify you are connected to the `node-1` node in the cluster:
...
Once that you are connected to the cluster, start working with SQL commands. Create tables, insert data, and run queries like you would in a regular PostgreSQL database.
