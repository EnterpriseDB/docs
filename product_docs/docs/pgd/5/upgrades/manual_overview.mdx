...
rolling upgrade, WAL decoder on a higher version of PGD node produces LCRs ([logical change records](../node_management/decoding_worker/#enabling))
...
The following approach is recommended for upgrading two BDR nodes that
