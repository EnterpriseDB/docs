...
To see if the target node is the write-leader, run the `pgd show-groups` command:

```bash
sudo -u postgres pgd show-nodes
```

in our example, our target node kaboom, is the current write-leader:
...
### If the target node is the write-leader, perform a planned switchover

Therefore, to keep the cluster available throughout the upgrade (i.e. a *rolling upgrade*), we need to perform a planned switchover to make another node the write-leader while we upgrade the target node, kaboom. In this case, we switchover the write-leader from kaboom to kaftan:

```bash
sudo -u postgres pgd switchover --group-name dc1_subgroup --node-name kaftan
```

Go ahead and run `pgd show-groups` again to make sure kaftan is now the write leader:

```bash
sudo -u postgres pgd show-groups
```

Then you should see that kaftan is now the write-leader:
...
It is now safe to stop Postgres on kaboom.
...
!!! Note
When running the first or second command to stop the old Postgres instance, you may receive the following error:
...
```
Next, navigate to the `/home/upgrade` and then run:
...
To complete upgrading the cluster for this worked example, repeat the previous steps on the other cluster nodes: kaftan and kaolin. After completing the upgrade on all nodes, while connected to one of the nodes, you can once again check your versions in the PGD database using:
...
Since we used TPA to build our cluster in this worked example, after all the nodes have been upgraded, we still need to [reconcile](https://www.enterprisedb.com/docs/tpa/latest/reference/reconciling-local-changes/) the upgraded version of Postgres with TPA to use TPA to manage the cluster in the future.
...
```
Then we can redeploy the cluster:

```shell
tpaexec deploy democluster
```

This final step should apply the configuration changes to the cluster managed by TPA. Upon successful deployment, reconciliation of the new version of Postgres with TPA and the upgrade procedure is complete.
