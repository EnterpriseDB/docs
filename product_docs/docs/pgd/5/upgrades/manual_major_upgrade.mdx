...
!!! Note
Ensure that your databases and configuration files are backed up before starting the upgrade process do help prevent data loss.
!!!
...
However, first, we need a directory for the files created by `bdr_pg_upgrade`, which we can give ownership of to the user `posgres`. 
...
Since we used TPA to build our cluster in this worked example, after all the nodes have been upgraded, we still need to reconcile the new version of Postgres with TPA to use TPA to manage the cluster in the future.

To do this, return to the command line of the server where your TPA cluster directory resides. In this worked example, our TPA cluster directory is `/home/ubuntu/democluster` on the instance where we originally deployed the cluster using TPA.
...
This should apply the configuration changes to the cluster managed by TPA. Upon successful deployment, reconciliation of the new version of Postgres with TPA and the upgrade procedure is complete.
