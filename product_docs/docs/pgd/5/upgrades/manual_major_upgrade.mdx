...
A three-node cluster (kaftan, kaolin, and kaboom) are currently running Postgres 15 on a Linux OS. We want to upgrade a single node to Postgres 16 (and then upgrade our whole cluster). In this walkthrough the target node we will upgrade will be bdr3. We will be using `bdr_pg_upgrade` to do this.
...
Use SSH to connect to the target node, in this case kaboom, and then run:
