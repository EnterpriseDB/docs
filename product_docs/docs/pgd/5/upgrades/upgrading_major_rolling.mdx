...
* **Execute the Postgres major version upgrade**
  * Execute the upgrade process by running the `pgd node upgrade` command without the `--check` option.
...
After the switchover is successful, it's safe to stop Postgres on the target node. 
Of course, if kaftan is still the write leader when you come to upgrading it, you'll need to perform another planned switchover at that time.
