...
## Performing a Postgres major version rolling upgrade on PGD with TPA

This document outlines the procedure for performing a major version upgrade of a Postgres cluster using Postgres Distributed (PGD) as installed by Trusted Postgres Architect (TPA). 
...
Before beginning the upgrade process, we want to check our versions

- Confirm the current major Postgres version.
  - View versions from PGD by logging in and running: `SELECT * FROM bdr.group_versions_details;`
- Plan the upgrade order of the nodes.
  - Check which node is the write leader by running `sudo -u postgres pgd show-groups` and plan it to be the last node to upgrade after a planned switchover to an already upgraded node.

### Perform the upgrade on each node.

To perform the rolling upgrade of the Postgres major version, carry out the following steps on each node:

- Perform a planned switchover if the target node is the current write leader.
- Stop Postgres.
...
### Reconciling the upgrade with TPA

- Change the config.yml of the TPA-deployed cluster to the new version.
...
We will look at these steps in more detail below, specifically for upgrading the write leader node from Postgres 15 to Postgres 16. 
