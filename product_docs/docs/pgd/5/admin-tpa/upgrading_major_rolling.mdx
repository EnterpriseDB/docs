...
To prepare for the upgrade, identify which subgroups and nodes you are trying to upgrade and note an initial upgrade order.
...
The `pgd show-nodes` command shows you all the nodes in your PGD cluster and the subgroup to which each node belongs. Then we want to know which node is the write leader in each subgroup by running:

```bash
sudo -u postgres pgd show-groups
```

This command outputs a list of the different groups/subgroups running in your cluster and the write leader of each group. Write leaders need to be switched over to another node in their subgroup before they can be upgraded to maintain operational continuity, so you should upgrade them lastly when upgrading a subgroup of nodes to keep the number of planned switchovers to a minimum.

Even though you have verified which node is the current write leader for planning purposes, note that the write leader of a subgroup could change to another node at any moment for operational reasons before you upgrade that node. Therefore, you still need to verify that a node is not the write leader just before upgrading that node.

You now should have enough information to determine your upgrade order, one subgroup at a time, aiming to upgrade the identified write leader node last in each subgroup. 
...
* **Verify that the target node is not the write leader**
  * Check whether the target node is the write leader for the group you are upgrading using `sudo -u postgres pgd show-groups`.
...
* **Verify both Postgres instances are inactive**
  ...
  * Utilize the `sudo systemctl status postgres` command to verify that Postgres has been stopped.
  * If it is not stopped, run `systemctl stop postgres` and reverify that it has been stopped.
...
* **Verify Upgrade Feasibility**
  ...
  * You need to run this check from an upgrade directory with ownership given to user `postgres`, such as `/home/upgrade/`, so that the upgrade log files created by the `bdr_pg_upgrade` can be stored.
...
* **Execute the Postgres major version upgrade**
  ...
  * It's essential to monitor the command output for any errors or warnings that may require attention. 
...
* **Clean up post-upgrade**
  * Run `vacuumdb` with the `ANALYZE` option immediately after the upgrade but before introducing a heavy production load. 
    * This minimizes the immediate performance impact, preparing the database for more accurate testing.
...
Next, we look in detail at upgrading the Postgres major version from 15 to 16 on a PGD 5 cluster deployed with TPA.
...
In this example, we start with kaboom.
...
SSH into kaboom to confirm the major version of Postgres is expected by running the following:

```bash
sudo -u postgres pgd show-version
```

The output should be similar to this for your cluster:
...
The cluster must be available throughout the process (that is, a *rolling* upgrade). There must always be an available write leader to maintain continuous cluster availability. So, if the target node is the current write leader, you must [perform a planned switchover](#performing-a-planned-switchover) of the [write leader](../terminology/#write-leader) node before upgrading it so that a write leader is always available. 

While SSH'd into kaboom, see which node is the current write leader of the group you are upgrading using the `pgd show-groups` command:
...
So we must perform a planned switchover of the write leader of `dc1_subgroup` to another node in the cluster.
...
After the switchover is successful, it is safe to stop Postgres on the target node. Of course, if kaftan is still the write leader when we come to upgrading it, we will need to perform another planned switchover at that time.

### Stop Postgres on the target node

While SSH'd into the target node (in this case, kaboom), stop Postgres on the node by running:
...
Although you should have stopped [previously stopped the Postgres service on the target node](#stop-postgres-on-the-target-node), kaboom, to be sure it's stopped, verify by running the `systemctl status postgres` command:
...
You are now finished upgrading the target node.

### Next steps

After completing the upgrade on kaboom, run the same steps on kaolin and kaftan.

If you followed along with this example and kaftan is the write leader, you must [perform a planned switchover](#perform-a-planned-switchover) to another, already upgraded, node before running the upgrade steps on kaftan to ensure availability.
