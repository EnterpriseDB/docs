...
You can perform a major version upgrade of Postgres on a Postgres Distributed (PGD) cluster deployed by Trusted Postgres Architect (TPA).
...
You can do run the vacuum now. On the target node, run:
