...
This document outlines the procedure for performing a major version upgrade of Postgres on a Postgres Distributed (PGD) cluster deployed by Trusted Postgres Architect (TPA).
...
Upgrading your Postgres cluster to a newer major version is recommended to access improved features, performance enhancements, and security updates. This guide provides general step-by-step instructions and a worked example to help facilitate a smooth and controlled upgrade process.

### Plan the upgrade

The cluster must be available throughout the process (i.e., a *rolling* upgrade). To achieve continuous cluster availability, you must [perform a planned switchover](#performing-a-planned-switchover) of the [write leader](https://www.enterprisedb.com/docs/pgd/latest/terminology/#write-leader) node before upgrading it so that there is always a write leader available. 

You can see which node is the write leader by connecting to the PGD cluster (e.g. using SSH) and running the `pgd show-groups` command:

```bash
sudo -u postgres pgd show-groups
```

To keep the number of planned [switchovers](https://www.enterprisedb.com/docs/pgd/latest/cli/command_ref/pgd_switchover/) to a minimum, you should upgrade the current write leader last in your upgrade order, so you only have to perform [one planned switchover](#performing-a-planned-switchover) during the upgrade process.
...
Next, carry out the following steps on each node in order:

- Perform a [planned switchover](#plan-the-upgrade) if the target node is the current write leader.
  - `sudo -u postgres pgd switchover --group-name <group_name> --node-name <new_write_leader_node_name>`
- Confirm the current major Postgres version.
  - View versions from PGD by logging in and running: `SELECT * FROM bdr.group_versions_details;`. Ensure the node you are about to upgrade has the expected major version running.
- Stop Postgres.
...
- Rename the PGDATA directories to switch the old version's out and the new version's in.
  - Move `/opt/postgres/data` to `/opt/postgres/dataold` and move `/opt/postgres/datanew` to `/opt/postgres/data`.
- Double-check that both Postgres instances are not running.
...
- Update the Postgres service configuration to the new version and refresh the system's service manager to apply the changes.
...
- Test the upgrade directly against the newly upgraded node.
  - Run your test suite against while still SSH'd into the newly upgraded node.
...
We will look at these steps in more detail below, specifically for upgrading a [shadow node](https://www.enterprisedb.com/docs/pgd/latest/terminology/#write-leader) from Postgres 15 to Postgres 16. 

## Worked example

For this worked example, we start with a [TPA-managed PGD cluster deployed using the AWS Quickstart](https://www.enterprisedb.com/docs/pgd/latest/quickstart/quick_start_aws/). The cluster has three nodes: kaboom, kaolin, and kaftan, all running Postges 15.

In this scenario, the current write leader is kaftan, and we will upgrade the first of two shadow nodes, kaboom.
...
This command halts the server on the current node, in this case, kaboom. Your cluster will continue running using the other two nodes.
...
After kaolin, only kaftan needs to be upgraded. However, because kaftan is our write leader, we should perform a planned switchover before running the upgrade steps on the node to ensure availability.

#### Performing a planned switchover

When you are ready to upgrade kaftan, SSH into kaftan and switchover the write leader from kaftan to an already upgraded shadow node, for example kaboom, with the following command:
...
It is now safe to upgrade kaftan using the same steps you used on the shadow nodes.
...
After all the nodes have been upgraded, we still need to [reconcile](https://www.enterprisedb.com/docs/tpa/latest/reference/reconciling-local-changes/) the upgraded version of Postgres with TPA so we can continue to use TPA to manage the cluster in the future.

To do this, return to the command line where your TPA cluster directory resides. In this worked example, our TPA cluster directory is `/home/ubuntu/democluster` on the instance where we originally deployed the cluster using TPA.
