...
To keep the number of planned [switchovers](https://www.enterprisedb.com/docs/pgd/latest/cli/command_ref/pgd_switchover/) to a minimum, put the current write leader last in your upgrade order. This allows you to only perform [one planned switchover](#performing-a-planned-switchover) during the upgrade process.
...
Perform the following steps on each node in order:

* **Verify that the target node is not the write leader**
  * If the target node is the current write leader for the group/subgroup you are upgrading, perform a [planned switchover](#plan-the-upgrade) to another node.
...
* **Confirm the current Postgres version**
  * View versions from PGD by logging in with `psql` and running `SELECT version();`
  * Ensure that the expected major version is running.


* **Stop Postgres on the target node**
  * Stop the Postgres service on the current node by running `sudo systemctl stop postgresql`
  * The target node is no longer actively participating as a node in the cluster.


* **Install PGD and utilities**
  * Install PGD and its utilities compatible with the Postgres version you are upgrading to.
...
* **Migrate configuration to the new Postgres version**
  * Locate the following configuration files in your current PostgreSQL data directory:
    ...
    * postgresql.auto.conf 
      * Contains settings automatically set by PostgreSQL, such as those modified by the ALTER SYSTEM command.
    * pg_hba.conf
...
* **Verify both Postgres instances are inactive**
  ...
  * Utilize the pg_ctl command to safely stop any running instances associated with both the old and new PostgreSQL data directories. Execute the following commands for each directory:
  * Confirm that both commands execute successfully, ensuring that each PostgreSQL instance is fully stopped before moving forward with the upgrade process.

!!! Note
When running the pg_ctl command to stop either of the Postgres instances, you may receive the following error:

```bash
pg_ctl: PID file "/opt/postgres/data/postmaster.pid" does not exist
Is server running?
```
This error is expected when both instances of Postgres are already shut down. Therefore, it doesn't prevent the upgrade from progressing and does not mean that the command ran unsuccessfully.
!!!
...
* **Execute the Postgres major version upgrade**
  * Execute the actual upgrade process by running the `bdr_pg_upgrade` command without the `--check` option.
  * This step updates your PostgreSQL installation to the new major version, incorporating all the improvements and changes to the updated version.
  * It's essential to monitor the output of the command for any errors or warnings that may require attention. 
  * The upgrade process may take some time, depending on the size of your database and the complexity of your setup.
...
Then refresh the system's service manager to apply these changes:
...
With the changes now applied to the postgres.service file by the service manager, proceed to restart the PostgreSQL service:
...
After completing the upgrade, verify that your PostgreSQL instance is now running the new version. This step ensures that the upgrade process was successful and that your system is now utilizing the latest features and improvements of PostgreSQL.

While logged into the Postgres instance using psql, run the following command to verify that the instance has been upgraded:

```sql
SELECT version();
```

The command's output will display the PostgreSQL version information. Verify that this matches the expected new version to confirm that your upgrade was successful.

#### Initial vacuum

Consider running `vacuumdb` with the `ANALYZE` option immediately after the upgrade but before introducing a heavy production load. This minimizes the immediate performance impact, preparing the database for more accurate testing.

#### Test the upgrade directly against the newly upgraded node
  
Run your [prepared test suite](#prepare-a-test-suite) against the upgraded database. This includes: 
 - pointing your test suite's database connection settings to the upgraded node. 
 - running the test suite against the upgraded database and paying close attention to the results, noting any failures or performance regressions.
 - analyzing the results for any test failures or performance degradations.
 - addressing any identified issues promptly by addressing application code, tuning database configurations, or rolling back the upgrade if necessary.

#### Clean up post-upgrade

After successfully upgrading and testing your PostgreSQL instance, it's important to perform clean-up tasks to ensure optimal performance and reclaim unnecessary disk space. 

First, run the `vacuumdb` utility again with the `ANALYZE` option to clean up anything post-testing.  

Lastly, remove the old version's data directory, `/opt/postgres/dataold`.
...
Although you should have stopped [previously stopped the Postgres service on the target node](#stop-postgres-on-the-target-node), kaboom, to be sure it's stopped, run the `stop` command for both instances of Postgres (PG15 and PG16) on
kaboom:
