...
* **Verify both Postgres instances are inactive**
  ...
  * Utilize the `pg_ctl` command to safely stop any running instances associated with both the old and new PostgreSQL data directories.
...
* **Validate the new Postgres version**
  * Verify that your PostgreSQL instance is now running the new version by logging in with `psql` and running `SELECT version();`


* **Clean up post-upgrade**
  * Consider running `vacuumdb` with the `ANALYZE` option immediately after the upgrade but before introducing a heavy production load. 
...
TPA needs to continue to manage the deployment effectively after the upgrade. Therefore, it's necessary to reconcile the upgraded nodes with TPA. 
...
Next, we look in detail at the steps for upgrading the Postgres major version from 15 to 16 on a PGD 5 cluster deployed with TPA.
...
This example first upgrades kaboom.
...
SSH into kaboom to confirm the major version of Postgres is as expected. First, connect using `psql`:

```bash
sudo -u postgres psql
```

When connected, use the following query:
...
While SSH'd into kaboom, see which node is the current write leader using the `pgd show-groups` command:

```bash
sudo -u postgres pgd show-groups
```

In this case, you can see that kaboom is the current write leader:
...
So we must perform a planned switchover to another node in the cluster.
...
After this is successful, it is safe to stop Postgres on the target node. Of course, if kaftan is still the write leader when we come to upgrading it, we will need to perform another planned switchover at that time.
...
However, if you followed along with this example and kaftan is the write leader still, you must perform a planned switchover before running the upgrade steps on that node to ensure availability.
