...
Perform the following steps on each node while connected via SSH:
...
SSH into kaboom to confirm the major version of Postgres is as expected by running:
...
This output shows that all the nodes were successfully upgraded to the new Postgres version 16.
