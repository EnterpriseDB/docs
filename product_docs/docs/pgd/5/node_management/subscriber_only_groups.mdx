...
number of `subscriber-only` nodes only receive changes but never
send any changes to any other node in the cluster. This topology avoids
connection explosion due to a large number of nodes. But it still provides
...
subgroup of the `subscriber-only` type, and they can have
