...
Logical standby nodes are nodes which are held in a state of continual recovery,
constantly updating until they are required. This is similar to how Postgres physical standbys operate while useing logical replication for better performance. [`bdr.join_node_group`](/pgd/latest/reference/nodes-management-interfaces#bdrjoin_node_group) has the `pause_in_standby`
