...
As a general rule, don't perform maintenance operations on a cluster's write leader. 
...
This usually means that you must not connect to a proxy to perform maintenance.
