...
![Multi-Region 3 Nodes Configuration](images/Multi-Region-3Nodes.png)
...
But, we are not done, because `opinion` is still a member of the `sere` replication set. When a tbale is a member of multiple replication sets, it is replicated within each. This doesn't impact performance though as each row in only replicated once on each target node. We don't want `opinion` replicated across all nodes, so we need to remove it from the top-level group's replication set:
...
```sql
INSERT INTO attendee (email) VALUES ('fb@gmail.com');
...
You will see the same result on each of the `region_b` data nodes. Run the query on `region_a` nodes and you will not see this particulare entry.
