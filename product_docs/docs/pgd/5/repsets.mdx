...
This example assumes you have a cluster of six data nodes, `data-a1` to `data-a3` and `data-b1` to `data-b3` in two locations. The two locations they are members of are represented as `region_a` and `region_b` groups. 
...
This is the standard Always-on Multi-location configuration discussed in [Choosing your architecture](planning/architectures).

### Application requirements

This example works with an application that records the opinions of people who attended performances of musical works. There's a table for attendees, a table for the works, and an opinion table. The opinion table records the works each attendees saw, where and when they saw it, and how they scored the work. Because of data regulation, the example assumes that opinion data must stay only in the region where the opinion was recorded. 
...
You want the `opinion` table to be replicated only within `region_a` and, separately, only within `region_b`. To do that, you add the table to the replica sets of each region:
...
Finally, note that the `attendee` table is shared identically across all nodes. On any node, run the query:
