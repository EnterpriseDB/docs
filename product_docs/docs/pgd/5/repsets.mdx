...
You can create replication sets using [`create_replication_set()`](reference/repsets_management#bdrcreate_replication_set),
...
[]`alter_node_replication_sets`](reference/repsets-management/#bdralter_node_replication_sets).
...
The current version of PGD doesn't automatically check for or enforce
this condition. When adding a table to a replication set, the database administrator must make sure
...
You can control membership using [bdr.replication_set_add_table](reference/repsets-membership#bdrreplication_set_add_table) and [bdr.replication_set_remove_table](reference/repsets-membership#bdrreplication_set_remove_table).
...
By default, the replication of all supported DDL happens by way of the default PGD
group replication set. This is achieved with a DDL filter with
the same name as the PGD group. This filter is added to the default PGD
group replication set when the PGD group is created.
...
](reference/repsets-ddr-replication-filtering#bdr-replication-set-add-ddl-filter) and [bdr.replication_set_remove_ddl_filter
](reference/repsets-ddr-replication-filtering#bdr-replication-ser-remove-ddl-filter)  to manipulate DDL filters. 
