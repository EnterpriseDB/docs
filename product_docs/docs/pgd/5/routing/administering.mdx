...
In `strict` mode, the maximum lag is checked before switching the write leader. This is the default.
In `fast` mode, the write leader is switched without checking the maximum lag.
