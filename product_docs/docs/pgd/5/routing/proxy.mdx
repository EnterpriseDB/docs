...
The PostgreSQL C client library (libpq) allows specifying multiple host names in single connection strings for simple failover. This is also supported by the client libraries (drivers) in other programming languages. This works well for failing over across PGD Proxy instances that are down or inaccessible, however if the PGD Proxy instance is accessible but does not have access to write leader or write leader for given instance does not exist (i.e. because there is not write leader for given PGD group), the connection will simply fail and not other hosts in the multi-host connection string will be tried. This is consistent with behavior of PostgreSQL client library with other prosixes like HAProxy or pgbouncer.
