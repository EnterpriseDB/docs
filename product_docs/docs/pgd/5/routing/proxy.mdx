...
The main purpose of this option is to allow user to configure the client connections behaviour when consensus is not present in the cluster. However, one should be aware that when set to non-zero value, writes through proxy may end up on 2 different write leaders during some cases like network partition.

**For example:** Consider a 3 node cluster with 2 proxies. A network partition occurs and the current `write leader` node and a proxy node fall in minority partition while the majority partition elects new `write leader` and has another proxy attached to it. In such a scenario both the proxies might be writing to two different nodes.

Therefore, one should be careful and choose a suitable value depending upon the nature of their workloads.
