...
If enabled proxy exposes following two HTTP(s) health check APIs that can be used to validate that the proxy can successfully route traffic to the current write leader. These APIs are HTTP(s) `GET` requests and do not require a request body.
```
GET /health/is-live
GET /health/is-ready
```
...
On the execution of this API, proxy executes server check query through the `proxy.endpoint`. The query is executed on the current write leader if it is available. If the query returns successfully then API returns `true` with HTTP status code `200 (OK)` to HTTP client else `false` with HTTP status code `500 (Internal Server Error)`. It will return `false` in the following scenarios.
...
It is important to know that once started proxy always remains in available state even if these probes return false because the proxy can recover itself automatically (self heal) once the BDR side or connectivity issues are resolved.

!!! Note
    Proxy will execute the SQL query only when the readiness endpoint is requested and not on periodic basis.
...
The `proxy.http.probes.timeout` is used by readiness check. Readiness probe will return false if proxy does not return the response from backend Postgres within this time period. This value should be set based on failover time which varies from 1s to 25s and retry attempts to avoid false positives.
