...
The main purpose of this option is to allow users to configure the write behaviour when the Raft leader is lost. When the Raft leader is not present in the cluster it is not always guaranteed that the current write leader seen by the proxy is the correct one. In some cases, like network partition, example given below, it is possible that two write leaders may be seen by two different proxies attached to the same group increasing the chances of write conflicts. If this is not the desired behavior then as mentioned above grace period can be set to 0s which configures the proxy to stop routing and close existing open connections immediately when it detects the Raft leader is lost.
