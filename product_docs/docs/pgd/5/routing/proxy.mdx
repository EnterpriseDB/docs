...
PGD Proxy provides the following HTTP(s) health check API endpoints. These APIs are `GET` requests and do not require a request body.
```
GET /health/is-live
GET /health/is-ready
```
...
On receiving this request, proxy checks if it can successfully route traffic through the `proxy.endpoint`. If the check returns successfully then API returns `true` with HTTP status code `200 (OK)` else `false` with HTTP status code `500 (Internal Server Error)`. It will return `false` in the following scenarios.
...
It is important to know that once started proxy always remains in available state even if this check return false because the proxy can recover itself automatically (self heal) once the BDR cluster or connectivity is restored.
...
By default the proxy health check is disabled. It can be enabled by adding the config `proxy.http.enable: true` in the `pgd-proxy-config.yml` file as shown below.
```
  proxy:
	  name: proxy-a1
...
Default HTTP server address is `:8080` with timeout `10s`. Below is a sample config to set the custom values of host, port and other configs.
```
  proxy:
	  name: proxy-a1
	  endpoint: "host=proxy-a1 port=6432 dbname=bdrdb user=pgdproxy "
...
The `proxy.endpoint` parameter is used by readiness API endpoint which try to connect to the proxy server using this endpoint. Host and port given in the endpoint should be of proxy (e.g. host=proxy-a1 port=6432). Please note this is different from cluster endpoints where host and port are of the backend BDR server. User and database could be the same as per setup though. Proxy user (`pgdproxy` in above example) should have the database login access and its password set up in `~/.pgpass` file. If cluster is setup through TPAExec, it creates the `pgdproxy` user as an OS user as well as database user with `bdr_superuser` role and have it password set in the `pgpass` file.

To enable HTTPS set the config parameter `proxy.http.secure: true`. If enabled, the `cert_file` and `key_file` config parameters become mandatory and expect server certificate and key file paths respectively.

The `proxy.http.probes.timeout` is used by readiness API endpoint. Readiness probe will return `false` if proxy does not receive the response from backend Postgres within this time period. This value should be set based on failover time and retry attempts to avoid false positives.
