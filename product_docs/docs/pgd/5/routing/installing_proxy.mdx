...
PGD Proxy provides [HTTP(S) health check APIs](proxy#proxy-health-check). If the health checks are required, you can enable them by setting the configuration parameters below to the pgd-proxy configuration file. By default it is disabled.
...
It can be enabled by adding the config `cluster.proxy.http.enable: true`. When enabled, the HTTP server will be present on the default port, `8080`, with a ten second `timeout` and no HTTPS support.
