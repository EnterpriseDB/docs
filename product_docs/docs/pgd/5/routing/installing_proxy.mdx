...
There are two ways to install and configure PGD Proxy to manage Postgres Distributed cluster. The easiest way to install and configure PGD Proxy is to use the EDB Trusted Postgres Architect (TPA) utility
for cluster deployment and management.


### Installing through TPA

If PGD cluster is being deployed through TPA then it installs and configures PGD Proxy automatically as per recommended architecture. If you wish to install PGD Proxy on any other node in PGD cluster then, you simply need to attach the `pgd-proxy` role to that instance in TPA configuration file and set the `bdr_child_group` parameter as shown below before deploying. See [Trusted Postgres Architect](/product_docs/docs/pgd/latest/tpa) for more information.
```yaml
- Name: proxy-a1
  location: a
  node: 4
