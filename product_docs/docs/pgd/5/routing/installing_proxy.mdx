...
PGD Proxy provides [HTTP(S) health check APIs](proxy#proxy-health-check). Add the below configuration parameters only if health check APIs are required. By default it is disabled.
...
It can be enabled by adding the config `cluster.proxy.http.enable: true`. Default HTTP server address is `:8080`, timeout is `10s` and secure is `false`. To enable HTTPS set the config parameter `cluster.proxy.http.secure: true`. If it is set to `true`, the `cert_file` and `key_file` config parameters are mandatory. The `cluster.proxy.endpoint` parameter is only required for readiness API. It could be same as BDR node `dsn` with exception of host and port where host could be `localhost` or proxy hostname and port will be `listen_port` proxy option. User can add additional connection string parameters in this endpoint, like `sslmode`, `sslrootcert` or change DB user, if required.
