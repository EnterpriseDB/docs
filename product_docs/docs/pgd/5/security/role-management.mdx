...
setting is turned on. However, if these commands are executed in other databases
in the same PostgreSQL instance, then they aren't replicated, even if those
users have rights on the PGD database.
