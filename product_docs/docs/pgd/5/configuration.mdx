...
-   `wal_sender_timeout` and `wal_receiver_timeout` &mdash; Determines how
    ...
    reconnected. See [CAMO failure scenarios](/pgd/latest/durability/camo/#failure-scenarios) for
    details.
...
parameters. See [Durability and performance options](/pgd/latest/durability) for details and
limitations.
...
-   `bdr.default_conflict_detection` &mdash; Sets the default conflict detection method
    ...
    [bdr.alter_table_conflict_detection()](/pgd/latest/consistency/conflicts#bdralter_table_conflict_detection).
...
-   `bdr.crdt_raw_value` &mdash; Sets the output format of [CRDT data types](/pgd/latest/consistency/crdt).
