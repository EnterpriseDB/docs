...
-   If parallel apply is on but all writers are already busy handling streamed
    ...
    [bdr.writers](../monitoring#monitoring-bdr-writers) to check PGD
    writer status.
...
You can monitor the use of transaction streaming using the [bdr.stat_subscription](reference/catalogs-visible#bdrstat_subscription)
