...
Finally, `--keyring-backend legacy` tells that the keyring backend is the legacy backend. Secrets are stored with an older keyring backend, as the version of Ubuntu this example is based on doesn't support the newer keyring backend.
...
You can now [deploy](/tpa/latest/tpaexec-deploy/) the distributed cluster. The `tpaexec` command runs its `provision` command to prepare the Docker containers for the deployment:
