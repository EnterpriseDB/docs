...
This example is based on the quick started configuration and, for speed
...
In a production environment, it is recommended that you create a separate
bastion server to run the failover experiment from and to create an appropriate
Postgres user to log into the cluster. 
!!!
