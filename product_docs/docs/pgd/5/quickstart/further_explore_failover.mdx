...
With xpanes imstalled, you can now create an SSH session with all four servers by running:
...
If you want to be double sure, exit the psql console and run:

```
psql -h kaftan bdrdb
```

Run `\dt` and `\d ping` and you will see the same results.
...
```
while true; do psql -h kaboom,kaftan,kaolin -p 6432 bdrdb -c "INSERT INTO ping(node, timestamp) select node_name, current_timestamp from bdr.local_node_summary;"; done
```
...
In the lower left pane, set the leader back to kaolin.
...
```
pgd switchover --group-name dc1_subgroup --name-name kaolin
```
