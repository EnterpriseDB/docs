...
Or you can type Control-b followed by q and the number of the pane you want to immediately focus on. We'll show both ways in this article.

Move to the bottom left pane by typing Control-b cursor-down Control-b cursor-right (or Control-b q 3).
...
The `select` command displays the conflict history for the cluster. The `\watch 1` command is a psql command which re-runs the preceding command every second.
...
Move to the top left pane with Control-b cursor-up Control-b cursor-left (or Control-b q 0). This is the "kaboom" node. It's here that you want to start a transaction and insert a row:
...
Now move to the top right pane with Control-b cursor-right (or Control-b q 1). This is the "kaftan" node. You'll want to do the same here, start a transaction and insert into the same row with different data.
...
* Control-b cursor-left (or Control-b q 0) then `commit;` 
* Control-b cursor-right (or Control-b q 1) then `commit;`
...
Move to the top left pane with Control-b cursor-left (or Control-b q 0). This is the "kaboom" node. It's here that you want to start a transaction and update a row:
...
Now move to the top right pane with Control-b cursor-right (or Control-b q 1). This is the "kaftan" node. You'll want to do the same here, start a transaction and update the same row with different data.
...
Now there is an additional row in the conflict history showing an `update_origin_change` conflict had occurred and that the resolution was the `apply_remote`. This means that the remote change was applied, updating the record. This is called an UPDATE/UPDATE conflict and is explained in more detail in [UPDATE/UPDATE Conflicts](../consistency/conflicts/#updateupdate-conflicts). 
