...
When one database node has an update to a row from an application and another node has a different update to the same row, a conflict occurs between the two nodes. Resolving that conflict is core to how Postgres Distributed maintains consistency. To see how it works, you need to open a command line view of all the servers. You can then create and monitor conflicts to better understand them.
