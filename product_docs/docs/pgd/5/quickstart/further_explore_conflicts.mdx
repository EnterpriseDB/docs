...
A multi-master architecture like PGD will see conflicts happen. PGD is built to handle them. It's important to remember the best way to avoid conflicts is to not have them in the first place. Use PGD's always-on architecture with proxies that ensure your applications write to the same server in the cluster. 

At it's simplest, a conflict is when one database node has an update from an application to a row and another node has a different update to the same row. That's what is called a row level conflict. Conflicts aren't errors and effectively resolving them is core to how Postgres Distributed maintains consistency. 

When there are conflicts though, it is useful to know what PGD does to resolve them, how you can control that resolution and how you can find out they are happening. 

To see how it works, you need to open a command line view of all the servers. 

## Your quick started configuration

This exploration assumes that you created your PGD cluster using the [quick start for Docker](quick_start_docker) or the [quick start for AWS](quick_start_aws).

At the end of both those quick starts, you'll have a cluster with four nodes and these roles:
