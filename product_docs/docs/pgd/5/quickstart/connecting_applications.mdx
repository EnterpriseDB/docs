...
```
    postgresql://<ipaddressofnode1>:6432,<ipaddressofnode2>:6432,<ipaddressofnode3>:6432/bdrdb
```
...
```
psql postgresql://192.168.9.10:6432,192.168.10.10:6432,192.168.11.10:6432/bdrdb
```

This format of URL will assume the user is the user you are currently logged in as, and prompt for that users database password - add `-U enterprisedb` to the command and it'll connect as enterprisedb (and the .pgpass file will take care of the password).

You can also embed the username and password in the created URL. If we are using the enterprisedb user and the password for the enterprisedb user is `notasecret` then the URL 
would look like:

```
psql postgresql://enterprisedb:notasecret@192.168.9.10:6432,enterprisedb:notasecret@192.168.10.10:6432,enterprisedb:notasecret@192.168.11.10:6432/bdrdb
```
