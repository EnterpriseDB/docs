...
!!! Note Other deployment options
    * If you prefer to have a fully managed EDB Postgres Distributed experience, PGD is now available as an option on EDB Postgres® AI Cloud Services (formerly BigAnimal), EDB's cloud platform for Postgres. Read more about [distributed high-availability clusters](/biganimal/latest/overview/02_high_availability/distributed_highavailability/).
    * If you prefer to deploy PGD on Kubernetes, you can use the EDB PGD Operator for Kubernetes. Read more about [EDB PGD Operator for Kubernetes](/postgres_distributed_for_kubernetes/latest/quickstart).
...
* Linux hosts &mdash; Deploys and hosts the cluster on Linux servers that you have already provisioned with an operating system and SSH connectivity. These can be actual physical servers or virtual machines, deployed on-premises or in the cloud. 
