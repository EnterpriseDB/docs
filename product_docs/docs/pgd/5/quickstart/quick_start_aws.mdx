...
TPA defaults to t3.micro instances on AWS - this is both sufficient for this demonstration and also suitable for use with an [AWS free tier](https://aws.amazon.com/free/) account.
...
You will see that you are logged in as `admin` on `kaboom`.
