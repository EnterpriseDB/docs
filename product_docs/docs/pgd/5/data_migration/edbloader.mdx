...
With PGD it's possible to run the direct load path method to each node. The can be performed on one node at a time or in parallel to all nodes, depending on the use case. When doing this, it's important to ensure there are no other writes happening to the table concurrently as this may result in inconsistencies.
