...
Read about [Conflicts](/pgd/latest/consistency/conflicts/) to understand
...
-   If the resolver for the `update_origin_change` conflict
    ...
    [Conflicts](/pgd/latest/consistency/conflicts/) chapter, `skip` is not the default
...
-   Group Commit cannot be combined with [CAMO](/pgd/latest/durability/camo/) or [Eager All Node
    replication](/pgd/latest/consistency/eager/). Eager Replication currently only works by using the
...
-   Parallel apply is not currently supported in combination with Group
    ...
    [`bdr.alter_node_group_config`](/pgd/latest/nodes#bdralter_node_group_config)) or
    via the GUC `bdr.writers_per_subscription` (see
    [Configuration of Generic Replication](/pgd/latest/configuration#generic-replication)).
