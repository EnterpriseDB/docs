...
-   The [PGD CLI](cli) can return stale data on the state of the cluster if it is still connecting to nodes that have previously been parted from the cluster. Edit the [pgd-cli-config.yml](cli/installing_cli/#specifying-database-connection-strings) file to ensure only active nodes in the cluster are listed for connection.
