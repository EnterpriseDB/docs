...
-   `ADD CONSTRAINT ... EXCLUDE` &mdash; Exclusion constraints aren't supported.
    Exclusion constraints are a way to ensure that no two rows in a table
    have overlapping ranges. This is useful in a synchronous system where
    you can ensure that the constraint is enforced on all nodes at the same time.
    In an asynchronous system, you can't ensure that the constraint is enforced
    on all nodes at the same time. If you insert a row on one node that violates
    the exclusion constraint, you can't be sure that the constraint will be
    enforced on all nodes before you insert another row that also violates the
    constraint. This can lead to data inconsistency and changes that can't be replayed.
