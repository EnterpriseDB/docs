...
| ---------- | ---------- | --------------- | ------------------------------ |
| host-one   | node-one   | 192.168.254.166 | host=host-one dbname=bdrdb     |
| host-two   | node-two   | 192.168.254.247 | host=host-two dbname=bdrdb     |
| host-three | node-three | 192.167.254.135 | host=host-three dbname=bdrdb   |
...
```
select bdr.create_node('node-one','host=host-one dbname=bdrdb');
```
...
```
select bdr.create_node('node-two','host=host-two dbname=bdrdb');
```
...
```
select bdr.join_node_group('host=host-one dbname=bdrdb','dc1');
```
...
```
select bdr.create_node('node-three','host=host-three dbname=bdrdb');
```
