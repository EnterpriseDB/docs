...
The
[`bdr.autopartition()`](/pgd/latest/reference/autopartition#bdrautopartition)
function doesn't lock the actual table. It changes the definition of when and
how new partition maintenance actions take place.
...
(Autopartion currently only supports this when used with 2nd Quadrant Postgres 11).
...
locally is useful when the partitioned table isn't a replicated table, in which
...
evaluated by SQL, which is evaluated every time a check is performed. For
example, for a partitioned table on column type `date`, if
`partition_autocreate_expression` is specified as
`DATE_TRUNC('day',CURRENT_DATE)`, `partition_increment` is specified as `1 Day`
and `minimum_advance_partitions` is specified as `2`, then new partitions are
...
Partitions are dropped at the same time as new partitions are added, to minimize
locking. If this value isn't set, you must drop the partitions manually.
...
guarantee that the partitions also exists on the remote nodes.
...
function to find the partition for the given partition key value. If partition
