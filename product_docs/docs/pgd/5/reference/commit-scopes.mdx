...
#### GROUP COMMIT parameters
 ...
 `conflict_resolution` | enum | Specifies how to handle conflicts. (`async`|`eager`). See [conflict_resolution settings](#conflict_resolution_settings)
  `commit_decision` | enum | Specifies how the COMMIT decision is made. (`group`|`partner`|`raft`). See [commit_decision settings](#commit_decision_settings)
