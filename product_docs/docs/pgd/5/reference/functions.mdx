...
Function to run DDL on specific replicasets.

```postgresql
bdr.replicate_ddl_command( ddl_command text, repl_sets []text, ddl_locking bool);
```

#### Synopsis

This function runs DDL on all the replication sets listed in the array of replication sets. It defaults to locking the DDL when replicating. 

#### Parameters

-   `ddl_command` &mdash; A DDL command to be run.
-   `repl_sets` &mdash; An array of replication sets where this DDL command should be replicated.
-   `ddl_locking` &mdash; Apply DDL locking. Defaults to on/true.
