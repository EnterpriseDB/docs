...
leadership transfer should happen. If not specified, it defaults to NULL which
is interpreted as the top-level group in the cluster. If the `node_group_name` is
specified, the function will only transfer leadership within the specified node
group.
...
- `commit_scope_id` &mdash; OID of the commit scope (see [`bdr.commit_scopes`](catalogs-visible.mdx#bdrcommit_scopes))
...
When using `bdr.add_commit_scope` if a new commit scope is added which has the
