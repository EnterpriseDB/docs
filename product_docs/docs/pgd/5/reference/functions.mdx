...
- `ddl_locking` &mdash; A string sets [`bdr.ddl_locking`](/pgd/latest/reference/pgd-settings#bdrddl_locking) while replicating. 
- `execute_locally` &mdash; A boolean which determines wether the DDL command will be executed locally. Defaults to true.

#### Notes

The only required parameter of this function is ddlcommand.
