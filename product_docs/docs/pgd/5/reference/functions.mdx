...
The `node_group` is optional and can be used to specify the node group where the
leadership transfer should happen. If not specified, it defaults to NULL which
is interpreted as the top-level group in the cluster. If the `node_group` is
