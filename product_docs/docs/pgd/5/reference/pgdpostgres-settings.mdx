---
navTitle: PGD/Postgres settings
title: PGD/Postgres settings
redirects:
  - ../bdr/configuration
...
You can also set PGD-specific configuration settings.
...
-   `bdr.default_replica_identity` &mdash; Sets the default value for `REPLICA IDENTITY`
    ...
    PGD can't replicate `UPDATE` and `DELETE` operations on tables without a `PRIMARY KEY`
...
-   `bdr.ddl_replication` &mdash; Automatically replicate DDL across nodes (default is
    ...
    message is written whenever replication of captured DDL commands or PGD
...
-   `bdr.predictive_checks` &mdash; Log level for predictive checks (currently used only
    ...
    timeouts. In global lock terms, PGD checks that there are enough nodes
...
-   `bdr.writers_per_subscription` &mdash; Default number of writers per
    subscription (in PGD, you can also change this with
...
-   `bdr.accept_connections` &mdash; Option to enable or disable connections to PGD.
    Defaults to `on`.
...
failover-candidate streaming physical replicas for this PGD node
...
You can use the view [`bdr.worker_tasks`](catalogs-visible#bdrworker_tasks) to inspect this state so the administrator can see any backoff
...
-   `bdr.debug_level` &mdash; Defines the log level that PGD uses to write
    its debug messages. The default value is `debug2`. If you want to see
    detailed PGD debug output, set `bdr.debug_level = 'log'`.

-   `bdr.trace_level` &mdash; Similar to the above, this defines the log level
    to use for PGD trace messages. Enabling tracing on all nodes of a
...
-   `bdr.raft_group_max_connections` &mdash; The maximum number of connections
     across all PGD groups for a Postgres server. These connections carry
...
-   `bdr.backwards_compatibility` &mdash; Specifies the version to be
    ...
    Defaults to the current PGD version. Since this changes from release
