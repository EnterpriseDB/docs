...
- `consensus_grace_period` &mdash; Duration for which proxy continues to route even upon loss of a Raft leader. If set to 0s, proxy stops routing immediately. Default is generally '6' (seconds) for local proxies and '12' (seconds) for global proxies though may be varied automatically depending on `raft_response_timeout`, `raft_global_election_timeout` and `raft_group_election_timeout`.
