...
If `bdr.default_replica_identity` is `default` and is a `UNIQUE`
constraint is on the table, it isn't automatically picked up as `REPLICA IDENTITY`.
...
Defaults to `off`. When enabled PGD treats every lock as a global DML lock. This makes the entire cluster wait for the lock to be released. This setting is useful for debugging and testing purposes.
