...
| nprovisional_waits   |                          | Number of update/delete operations on same tuples by concurrent apply transactions. These are provisional waits which may be converted to tuple waits to avoid deadlocks while maintaining sequential consistency |
| ntuple_waits         |                          | Number of tuple waits |
| ncommit_waits        |                          | Number of fully applied transactions that had to wait before being applied |
