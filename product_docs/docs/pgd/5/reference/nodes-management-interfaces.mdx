...
- `route_priority` &mdash; Relative routing priority of the node against other nodes in the same node group. Default is '100'.
...
- `route_writes` &mdash; Whether writes can be routed to this node, that is, whether the node can become write leader. Default is 't' (true).
- `route_reads` &mdash; Boolean: Whether read-only connections can be routed to this node. Currently reserved for future use. Default is 't' (true).
- `route_dsn` &mdash; The dsn that the proxy will use to connect to this node.
