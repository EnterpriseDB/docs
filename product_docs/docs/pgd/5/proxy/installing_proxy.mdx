...
If PGD cluster is being deployed through TPAExec then it installs and configures PGD Proxy automatically as per recommended architecture. If you wish to install PGD Proxy on any other node in PGD cluster then, you simply need to attach the `pgd-proxy` role to that instance in TPAexec's configuration file and set the `bdr_child_group` parameter as shown below before deploying. See [TPAexec](/product_docs/docs/pgd/5/deployments/tpaexec) for more information.
```yaml
- Name: proxy-a1
  location: a
  node: 4
