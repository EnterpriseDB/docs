...
This feature has been enhanced in PGD 5.2 (and later) and is recommended for more scenarios, especially with commit scopes. 
!!!
...
Where there are intersections, Parallel Apply will execute the transaction until it finds a conflicting row. Only then will it put the transaction on hold until the intersecting transactions have committed the conflicted rows. It will then continue applying row changes from the transaction.
...
From PGD 5.2, Parallel Apply works with Group Commit, CAMO and eager replication. 

### Monitoring Parallel Apply

To support Parallel Apply's deadlock mitigation, PGD 5.2 adds columns to [`bdr.stat_subscription`](/pgd/latest/reference/catalogs-visible#bdrstat_subscription). The new columns are `nprovisional_waits`, `ntuple_waits` and `ncommmit_waits`. The `nprovisional_waits` value reflects the number of operations on the same tuples being performed by concurrent transaction applications. These are provisional waits which are not actually waiting yet but could. 

If they are affected by the deadlock mitigation measures, they are converted to `ntuple_waits` and will be waiting until they can be safely applied. Fully applied transactions which waited before being applied are counted in `ncommit_waits`. 
