...
The metric collection is enabled automatically when the configuration option
`bdr.metrics_otel_http_url` is set to a non-empty URL.
...
The metrics and tracing endpoints can be HTTP or HTTPS. It's possible
