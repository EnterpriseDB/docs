...
A PGD Group consists of multiple servers, often referred to as nodes. All of the
...
internal metrics and also PGD metadata catalogs that store the configuration
...
Each node has one PGD group slot which should never have a connection to it
...
Note that because PGD is a mesh network, to get full view of lag in the
