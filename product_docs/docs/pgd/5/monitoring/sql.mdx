...
A PGD group consists of multiple servers, often referred to as nodes. You need to monitor all of the
...
When a node is parted, it's possible that some nodes in the cluster didn't receive
...
You use two main views or monitoring of replication activity:
...
You can use an additional view for monitoring of outgoing replication activity:
...
Because PGD is a mesh network, to get full view of lag in the
...
is `NULL`, as is `client_addr` and other fields that make sense only
...
!!! Note
    ...
    force retention of more WAL than is desirable but is otherwise harmless.
    If you're concerned, look for very long running transactions and forgotten
    prepared transactions.

### Monitoring incoming replication

You can monitor incoming replication (also called subscription) by querying
the `bdr.subscription_summary` view. This query shows the list of known subscriptions
to other nodes in the EDB Postgres Distributed cluster and the state of the replication worker, for example:
...
This is the case if the cecoding worker isn't enabled or the WAL sender is
...
it's important to note the time of the error and not just the existence of one.
...
the commit position isn't yet known. This can happen for a streaming
transaction or when the writer isn't applying any transaction at the
moment.
...
Monitor the PGD system to identify conflicts.
Where possible, make application changes to eliminate the conflicts or make
them less frequent.
...
For relations only, these views provide these statistics:
...
For subscriptions only, these views provide these statistics:
...
10 rows, and did two DDL commands inside five transactions.
...
PGD versions across the nodes in the same cluster. This capabiity is useful for
upgrading.

The view `bdr.group_versions_details` uses the function
`bdr.run_on_all_nodes()` to retrieve Postgres and PGD versions from all nodes
...
that the cluster doesn't run different PGD versions for too long.
...
bdrdb=# SELECT * FROM bdr.monitor_group_versions();
 status |                message
--------+-----------------------------------------
 OK     | All nodes are running same PGD versions
```
...
-   PGD data changes replication might still be working correctly.
...
-   PGD group replication slot doesn't advance LSN, thus keeps WAL files on
    disk.
...
Raft consensus is working correctly if all below
conditions are met:
...
seconds on all nodes. From PGD 3.6.11 onwards, setting
...
bdrdb=# SELECT * FROM bdr.monitor_local_replslots();
 status |                    message
--------+-------------------------------------------------
 OK     | All PGD replication slots are working correctly
```

One of the following status summaries is returned:

-   `UNKNOWN`: `This node is not part of any PGD group`
-   `OK`: `All PGD replication slots are working correctly`
-   `OK`: `This node is part of a subscriber-only group`
-   `CRITICAL`: `There is at least 1 PGD replication slot which is inactive`
-   `CRITICAL`: `There is at least 1 PGD replication slot which is missing`
