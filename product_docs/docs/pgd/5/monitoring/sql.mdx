...
A BDR Group consists of multiple servers, often referred to as nodes. You need to monitor all of the
...
Each node has one BDR group slot that must never have a connection to it
...
Because BDR is a mesh network, to get full view of lag in the
...
-  Writes for tables that are not part of a
...
-   The `query` column will be blank in PGD workers, except
    when a writer process is executing DDL

The `bdr.workers` view shows PGD worker specific details, that are not
...
because most errors are transient in their nature and PGD workers will retry
...
PGD honours commit ordering by following the same commit order as
...
The PGD system should be monitored to ensure that conflicts are identified
...
PGD versions across the nodes in the same cluster. This is useful for
upgrading.
...
that the cluster does not run different PGD versions for too long.
...
-   PGD data changes replication may still be working correctly
...
-   PGD group replication slot does not advance LSN, thus keeps WAL files on
    disk
...
PGD node replication slots are working as expected, e.g.:
...
PGD can be used with standard PostgreSQL synchronous replication, while
