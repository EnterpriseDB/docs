...
[Connection management](/pgd/latest/routing) leverages consensus-driven quorum to determine
...
[PGD Proxy](/pgd/latest/routing/proxy) is the tool for application connection management
...
and is deployed as a standard Postgres extension named BDR. See the [Compatibility matrix](/pgd/latest/#compatibility-matrix)
...
replicated in multi-node setting. [Application usage](/pgd/latest/appusage)
...
provides row-version conflict detection, as described in [Conflict detection](/pgd/latest/consistency/conflicts).
...
[Architectural details](/pgd/latest/architectures/#architecture-details).
