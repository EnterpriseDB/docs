...
### Connection manaagement

[PGD Proxy](../proxy) is connection management tool for a EDB Postgres Distributed cluster.
...
or two groups in two separate locations. Each group provides high availability and IS.  When two
...
and is deployed as a
standard Postgres extension named BDR. See the [Compatibility matrix](/pgd/latest/#compatibility-matrix)
