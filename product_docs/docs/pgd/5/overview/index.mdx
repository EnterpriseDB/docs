...
You don't have to write to all the masters all of the time. A frequent configuration directly writes mostly to just one master called the [write leader](../terminology/#write-leader).
...
Nodes can run different release levels, negotiating the required protocols to communicate. As a result, [EDB Postgres Distributed clusters can use rolling upgrades, even for major versions of database software](https://www.enterprisedb.com/docs/pgd/latest/upgrades/upgrading_major_rolling/).
...
Since writes are mainly to only one node, the possibility of contention between nodes is reduced to almost zero. As a result, performance impact is significantly reduced.
...
In summary, adding more master nodes to a PGD group doesn't result in a significant write
