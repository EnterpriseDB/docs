...
You can also run PGD without PGD proxy if you need a custom deployment better utilizing the multi-master functionality. When running without PGD Proxy, writes will be distributed amongst the nodes, replicated to one another, and conflict resolution is relied upon for maintaining consistency.
