...
!!! Note "Get started with PGD quickly"
    [Quick Start for PGD 5 on Docker](/tpa/latest/quick_start_docker) &mdash; Configure, provision and deploy a PGD 5 Always-On cluster on Docker.
    
    Suitable for evaluation and testing with a free trial account.
