...
* `apply_delay`, enabling the creation of a delayed read-only [replica](https://www.enterprisedb.com/docs/pgd/5/node_management/subscriber_only/) for additional options for disaster recovery and to mitigate the impact of human error, such as accidental DROP table statements.
