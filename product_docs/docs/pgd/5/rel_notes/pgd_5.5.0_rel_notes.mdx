...
| BDR       | 5.5.0   | BDR now does not save changes received by the writer to a temp file.                                                           |                |
| BDR       | 5.5.0   | BDR now logs completion of an extension upgrade.                                                                               |                |
| BDR       | 5.5.0   | Added restrictions for group commit options.                                                                                   |                |
| BDR       | 5.5.0   | BDR now executes each autopartition task in its own transaction.                                                               | RT101407/35476 |
| BDR       | 5.5.0   | BDR now uses DETACH CONCURRENTLY to drop partitions.                                                                           | RT101407/35476 |
| BDR       | 5.5.0   | BDR now disallows node group creation on a node bad state.                                                                     |                |
...
| BDR       | 5.5.0   | BDR now exposes heatbeat timings in Raft `get_raft_status()`.                                                                  |                |
| BDR       | 5.5.0   | BDR now exposes follower info in Raft get_raft_status() on Raft leader nodes.                                                  |                |
| BDR       | 5.5.0   | BDR now exposes the number of changes processed by writer in a large transaction.                                              |                |
