...
We recommend that all users of PGD 5 upgrade to PGD 5.3. See [PGD upgrades](../upgrades) for details.
!!!
...
| BDR       | 5.3.0   | Only consider CAMO transactions to be asynchronous when the CAMO setup was degraded to local mode.<sup>1</sup>  | RT78928 |
...
| Utilities | 1.2.0   | Removed the pre-upgrade step which sets the CONNECTION LIMIT to 0 at database level in `bdr_pg_upgrade` | |


1: This solves the split brain problem when deciding fate of transactions that happened during failover.
