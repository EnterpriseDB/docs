...
-  The ability to reliably reset the values.
-  A somewhat self-healing nature in the presence of lost changes, which doesn't happen in a cluster that operates properly.
-  The ability to receive changes from other than source nodes.
