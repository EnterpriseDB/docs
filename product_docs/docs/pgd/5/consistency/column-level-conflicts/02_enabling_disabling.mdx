...
### DDL locking and managing timestamps
...
### Example of enabling column-level conflict resolution using bdr.alter_table_conflict_detection

The [bdr.alter_table_conflict_detection](../../reference/conflict_functions/#bdralter_table_conflict_detection) function takes a table name and column name as its arguments. The column will be added to the table as a column_modify_timestamp column. The function also adds two triggers (BEFORE INSERT and BEFORE UPDATE) that are responsible for maintaining timestamps in the new column before each change.
