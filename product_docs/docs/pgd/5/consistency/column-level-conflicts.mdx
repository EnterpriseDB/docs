...
You can also use the actual commit timestamp, although this feature is considered experimental. Use of the commit timestamp can only be specified via the deprecated function `bdr.column_timestamps_enable`. Set the last parameter to `true` when enabling column-level conflict resolution:
