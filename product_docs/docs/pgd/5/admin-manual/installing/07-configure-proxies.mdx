...
* Create a `.pgpass` file for that user that allows it to log into the cluster as pgdproxy.
...
You can use ghe [`bdr.proxy_config_summary`](/pgd/latest/reference/catalogs-internal#bdrproxy_config_summary) view to check that the proxies were created:
...
The proxy configuration fileis slightly different for each host.
...
The `name` property is a name created with `bdr.create_proxy` earlier, and it is different on each host.
