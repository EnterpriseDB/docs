...
* Create a configuration file.
  * This is a YAML file that specifies the cluster and endpoints for the PGD CLI application to use.
...
For more details about these commands, see the worked example below.
...
For example, yuo can use the `show-nodes` command to list the nodes in yur cluster and their status:
...
The location is descriptive metadata, and so far you haven't set it. You can use PGD CLI to remedy that.
...
It takes a `--group-name` flag with the group the node exists in and a `--node-name` flag with the name of the node to swicth to.
You can then run:
