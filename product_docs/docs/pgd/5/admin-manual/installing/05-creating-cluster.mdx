...
To create the cluster, select and log into one of the host Postgres server's `bdrdb` database. 
...
* **Create the first node.**  
  ...
  * Create a subgroup.
    Create a subgroup as a child of the top-level group with `bdr.create_node_group`, giving it two parameters: the name of the sub-group and the name of the parent (and top-level) group.
...
You call the [`bdr.create_node`](/pgd/latest/reference/nodes-management-interfaces#bdrcreate_node) function to create this node, passing it the node name and a connection string that other nodes can use to connect to it.
