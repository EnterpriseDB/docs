...
* **Create the replicated database.**
  * Log into the server's default database (`edb` for EDB Postgres Advanced Server, `postgres` for PGE and community Postgres).
...
If you're installing PGD with EDB Postgres Extended Server or community Postgres, the steps are similar, but details such as package names and paths are different. These differences are summarized in [Installing PGD for EDB Postgres Extended Server](#installing-pgd-for-edb-postgres-extended-server) and [Installing PGD for community Postgresql](#installing-pgd-for-community-postgresql).

## Worked example

### Install the packages

The first step is to install the packages. Each Postgres packagehas a `edb-bdr5-<postgresversion>` package to go with it. 
For example, if you're installing EDB Postgres Advanced Server (epas) version 16, you would install `edb-bdr5-epas16`. 
...
For EDB Postgres Advanced Server, yu can find this script in `/usr/edb/as16/bin` as `edb-as-16-setup`. 
...
You will need this password when for [Connecting the cluster](05-connecting-cluster).
