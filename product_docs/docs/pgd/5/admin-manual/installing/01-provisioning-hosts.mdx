...
This example has provisioned three hosts with Red Hat Enterprise Linux 9:
...
For the example cluster, we have also edited `/etc/hosts` to use those private IP addresses:
