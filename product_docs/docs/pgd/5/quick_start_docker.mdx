...
!!! SeeAlso
    ...
    - [AWS platform](/tpa/latest/platform-docker/) in the Trusted Postgres Architect documentation
...
You will now see that you are logged in as `root` on `kaboom`
```console
[root@kaboom ~]# 
```
...
```console
[root@kaboom ~]# su -iu enterprisedb
enterprisedb@kaboom:~ $
```

You can now run the `psql` command to access the `bdrdb` database. Enter `psql bdrdb`:
...
----- -----        ---------------- -----------
kapok dc1_subgroup [0.0.0.0]        6432
kaput dc1_subgroup [0.0.0.0]        6432
```
...
```
enterprisedb@kaboom:~ $ psql -h kapok,kaput -p 6432 bdrdb
psql (15.2.0, server 15.2.0)
