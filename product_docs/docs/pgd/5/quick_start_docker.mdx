...
```shell
tpaexec configure myedbdpcluster --architecture PGD-Always-ON --platform docker --epas --location-names dc1 --data-nodes-per-location 3 --no-git
```
...
```shell
tpaexec deprovision myedbdpcluster
```
