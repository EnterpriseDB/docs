...
We created TPA to make installing the installation and management of various Postgres configurations easily repeatable. TPA orchestrates the creation and deployment of Postgres. In this quick start, we install TPA first. If you already have TPA installed, you can skip those steps. TPA can be used, and re-used, to deploy various configurations of Postgres clusters

PGD is an multi-master replicating implemenation of Postgres designed for high performance and availability. The installation of PGD is orchestrated by TPA. We will get TPA to generate a configuration file for a PGD demonstration cluster. This cluster uses local Docker containers to host the cluster's nodes; three replicating database nodes, two connection proxies and one backup node. We can than use TPA to provision and deploy the required configuration and software to each node.
...
To complete this example, you will need free storage and Docker installed. 
...
All the software needed for this example is available from the Postgres Distributed package repository. We will download and run a script to configure the Postgres Distributed repository. This repository also contains the TPA packages.
...
We can also do a quick hands-on test with psql. For convenience, we'll use the barman instance to launch psql and connect to a proxy instance. As we saw previously in config.yml, TPA has assigned the name `karakul` to the barman instance, and `kapok` to the first proxy.
