...
!!! Note PGD Proxy only enabled in active locations
    While PGD Proxy notes will be configured wihtout it, the flag `--active-locations` causes proxy routing (and subgroup RAFT) to be enabled for the location(s) specified in it. Here we have only one location specified by `--location-names`, so 
    we pass that to `--active-locations` as well. 
