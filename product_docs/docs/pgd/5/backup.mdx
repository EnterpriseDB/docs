...
they were local sequences. This is intentional, to allow a PGD
...
A physical backup of a PGD node can be performed with the same
...
-   Backups will make a copy of just one node. In the simplest case,
    ...
    one node to capture all data. However, the goal of PGD will not be
...
PGD allows for changes from multiple masters, all recorded within the
...
to the original PostgreSQL PITR behavior that is designed around the assumption
of changes arriving from a single master in COMMIT order.

!!! Note
    This feature is only available with EDB Postgres Extended.
    Barman does not create a `multi_recovery.conf` file.
...
**restoring** the physical backup of a PGD node.
...
In that case, PGD capabilities can be restored based on a physical backup
...
-   If you still have some PGD nodes live and running, fence off the host you
    restored the PGD node to, so it cannot connect to any surviving PGD nodes.
...
#### Cleanup PGD Metadata

The cleaning of leftover PGD metadata is achieved as follows:
...
PGD creates one replication origin for each remote master node, to
...
...and those created by PGD are easily recognized by their name, as in
