...
Please note that in some cases when the raft is not working properly or the
group raft leader is not present, this command may show stale or incorrect
write leader for that group.
