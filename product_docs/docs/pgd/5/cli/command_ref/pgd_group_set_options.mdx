---
title: group set options
navTitle: set options
description: Sets group options such as enable_raft, enable_proxy_routing, and location
deepToC: true
---
...
Use `pgd group list -o json` to view option values for each group.
...
```text
  ...
  $ pgd group set options --group-name bdrgroup --option enable_proxy_routing=true,route_writer_max_lag=1000000
  group options updated successfully


  Example 2 (multiple --option flags are allowed)

  $ pgd group set options --group-name bdrgroup --option enable_proxy_routing=true --option route_writer_max_lag=1000000
  group options updated successfully


  Example 3 (use double quote if option value has spaces or special characters)

  $ pgd group set options --group-name bdrgroup --option "location = mumbai" --option "route_writer_max_lag = 1000000"
  group options updated successfully
