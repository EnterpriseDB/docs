...
Trusted Postgres Architect installs and configures PGD CLI on each PGD node, by default. If you wish to install PGD CLI on any non-PGD instance in the cluster, you simply attach the pgdcli role to that instance in TPA's configuration file before deploying. See [Trusted Postgres Architect](/pgd/latest/tpa) for more information.
