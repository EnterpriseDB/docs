...
By default, Trusted Postgres Architect (TPA) installs and configures PGD CLI on each PGD node. If you want to install PGD CLI on any non-PGD instance in the cluster, attach the pgdcli role to that instance in TPA's configuration file before deploying. See [Trusted Postgres Architect](/tpa/latest/) for more information.
...
When the TPA configures the PGD CLI, it creates a configuration file that allows PCD CLI to connect automatically when run. With a manual installation to a standalone EDB PGD cluster, you need to either create the configuration file or provide a connection string on the command line.
