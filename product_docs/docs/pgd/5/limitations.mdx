...
Consult the [Durability limitations](durability/limitations) section for a full
and current listing.

## Mixed PGD versions

PGD has been developed to [enable rolling upgrades of PGD](upgrades) by allowing mixed versions of PGD to operate during the upgrade process. 
We expect users to only run mixed versions during upgrades and, once an upgrade has been started, that they complete that upgrade.
We do not support running mixed versions of PGD other than while there is an upgrade taking place.
