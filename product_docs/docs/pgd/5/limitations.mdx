...
[Group Commit](durability/group-commit) is a feature that enable configurable consensus synchronous commit across a node group or PGD cluster. If you use this feature, take these limitations into account when planning:
...
- Changing the CAMO partners in a CAMO pair isn't possible; you can only add or remove a pair.
