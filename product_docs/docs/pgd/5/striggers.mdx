...
Stream triggers are a PGD feature which requires permission. Any user wishing to use triggers, 
...
stream trigger into a table that is crash-safe and specific to that
