...
BDR should only be used to replicate a single database in a Postgres instance.
While it will not block you from installing it to multiple databases and adding
groups in each of them, only asynchronous replication is know to work when BDR
is used in multiple databases in a single instance. The support for multiple
databases is considered deprecated and in the future BDR will enforce single
