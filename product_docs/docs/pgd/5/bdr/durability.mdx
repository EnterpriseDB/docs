...
!!! WARNING
    ...
    a node specified in `synchronous_standby_names` and thus not assure the
    data is really flushed to disk.

!!! WARNING
    While using multiple BDR enabled databases (we advise against it)
    the lsn sync confirmation may come from a diferent database on a
    node that is in our synchronous_standby_names and thus not assure
    that the data is really synced on the database that we really want.
