...
| `--location-names` | Required. Specifies the number and name of the locations PGD will be deployed to. |
...
Specify `--enable-camo` to set the pair of BDR primary instances in each region to be each other's Commit At Most Once (CAMO) partners. See [Commit At Most Once (CAMO)](/pgd/latest/durability/camo/) for more information.
