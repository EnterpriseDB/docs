...
redirects:
  - ../deployments/tpaexec/using_tpaexec
---
...
| `--location-names` | Optional. Specifies the number and name of the locations PGD will be deployed to. |
| `--active-locations` | list of active (write) locations. All locations specified must also be in the list provided to `--location-names`. |
| `--data-nodes-per-location` | Optional. How many PGD nodes per location should be deployed. Defaults to 3; must be at least 2. |
| `--witness-node-per-location` | Optional. Should there be witness node in every location. Defaults to false. If true, `--data-nodes-per-location` must be an even number. |
| `--witness-only-location` | Optional. Name of a witness-only location. Defaults to no witness-only location. The location specified (which must also be in `--location-names` and cannot be in `--active-locations`) will ONLY host a witness node. That is, no data nodes will be deployed to it. This option is only valid when `--location-names` specifies an odd number of locations to deploy to. |
| `--cohost-proxies` | Optional. If true, PGD will cohost proxies on PGD data nodes. Defaults to false. |
| `--enable-subgroup-raft` | Enables subgroup RAFT for all eligible node groups, i.e., for the groups in all non-witness locations. This is required to enable proxy routing. |
