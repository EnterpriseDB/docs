...
a Linux OS. We want to upgrade a single node to Postgres 15 (and then upgrade our whole cluster). In this walkthrough the target node we will upgrade will be bdr3. We will be using `bdr_pg_upgrade` to do this.
...
### 3- Install the new Postgres version and bdr_pg_upgrade

Establish the name of the package for your new Postgres version. For example, if you are upgrading to Postgres 15 
...
On the target node, au postgres, copy over the postgresql.conf,
...
### 12 - Post-upgrade cleanup

We want to run a vacuum over to database at this point, as best practice. When the upgrade
