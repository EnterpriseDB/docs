...
```
For this CAMO commit scope to be legal, the number of nodes in the group must equal 2. Using ALL or ANY 2 on a group consisting of several nodes is an error because the node group expression does not resolve to a definite pair of nodes.
