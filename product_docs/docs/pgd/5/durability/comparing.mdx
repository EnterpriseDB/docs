...
The table summarizes what a client can expect from a peer node that has
replicated a transaction after receiving a COMMIT confirmation from the origin
node. For commit scopes the Mode column refers to the confirmation requirements
of the [commit scope configuration](commit-scopes#configuration).
...
| PGD Synchronous Commit | 'ON received' nodes   | no       | no      | no      |
| PGD Synchronous Commit | 'ON replicated' nodes | no       | no      | no      |
| PGD Synchronous Commit | 'ON durable' nodes    | no       | no      | no      |
| PGD Synchronous Commit | 'ON visible' nodes    | no       | no      | no      |
