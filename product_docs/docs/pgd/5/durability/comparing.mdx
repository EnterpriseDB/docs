...
**Expect**: The peer node has recieved the changes. Nothing has been updated in
...
**Expect**: The peer node has recieved the changes and applied them to the tuple
store. The changes have not been persisted to storage.

Confirmation on replication means the peer has recieved and applied the changes, but 
has yet to persist them to disk. 

**For**: A better checkpoint than recieved with less performance
**Against**: Less robust when a crash occurs.

### ON DURABLE

**Expect**: The peer node has recieved the changes, applied them to the tuple store and persisted the changes to storage. It has yet to make the changes available to other concurrent transactions.
