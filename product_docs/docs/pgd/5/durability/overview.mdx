...
-   [CAMO](camo): This kind of commit scope is a variant of Group Commit in which the client is part of the consensus.
...
-   [Synchronous Commit](synchronous-commit): This kind of commit scope allows for a behaviour where the origin node awaits a majority of nodes to confirm and behaves more like a native Postgres synchronous commit.
...
but consider using [Group Commit](group-commit) or [Synchronous_Commit](synchronous_commit).
!!!
