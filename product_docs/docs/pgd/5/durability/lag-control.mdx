...
-  RCO - Resource constraint objective acknowledges that there is finite storage
   available. In PGD, as lag increases the demands on these storage resources
   also increase.
...
To allow an organization to achieve their objectives, PGD offers Lag Control,
...
Given a configuration with two datacenters, represented as sub-groups:
...
This example shows Lag Control rules for two data centers:
...
The `max_commit_delay` is typically specified in milliseconds (1ms). Using
fractional values for sub-millisecond precision is supported.
