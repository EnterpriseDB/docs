...
regardless of whether the origin transaction committed or is still in progress. It
using the row-level resolution as configured for the given node. See
...
performance. For the details abpit how eager conflict resolution works, see
