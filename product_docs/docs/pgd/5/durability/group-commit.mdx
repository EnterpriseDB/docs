...
The `raft` decision uses PGDs built-in raft consensus for commit decisions. Use of the `raft` decision can reduce performance. It is currently only required when using GROUP COMMIT
...
where it also sends this decision via replication to other nodes which will also eventually commit on getting this message.
