...
by the origin node upon it receiving as many confirmations as required by the
...
The `raft` decision uses PGD's built-in Raft consensus for commit decisions. It can have low performance and is currently required only when using GROUP COMMIT with an ALL commit scope group.
...
decision](#commit-decisions) be set to `raft` to avoid reconciliation
issues.
