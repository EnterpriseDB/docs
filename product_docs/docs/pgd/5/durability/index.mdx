...
per-transaction behavior. It's also possible to configure the Postgres
...
[Synchronous replication](#synchronous-replication-using-pgd),
...
on different meaning depending on the variant. For PSR and 
...
| PGD Sync (3) | remote_write (2)      | yes      | no      | no      |
| PGD Sync (3) | on (2)                | yes      | yes     | yes     |
| PGD Sync (3) | remote_apply (2)      | yes      | yes     | yes     |
...
<!--Please check the above edit. It was hard to make sense of the sentence. -->
...
| Setting (GUC)                        | Group Commit | Lag Control | PSR (1) |   Postgres Sync |
|--------------------------------------|:------------:|:-----------:|:-------:|:-----------:|
...
## Synchronous replication using PGD
