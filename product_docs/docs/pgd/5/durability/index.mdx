...
In its basic configuration, EDB Postgres Distributed uses asynchronous bidirectional
logical replication. However, commit scopes can change both the default and the 
per-transaction behavior to enable synchronous logical replication and other capabilities.

While it is possible to configure Postgres's synchronous replication using
the built-in `synchronous_standby_names` GUC variable, commit scopes 
...
-   Durability: By ensuring transactions are written to multiple nodes,
    you can increases crash resilience and that allows you to recover 
    the data after a crash and restart.
...
!!! Note Legacy synchronization options
For backward compatibility, PGD still supports configuring synchronous
replication using `synchronous_standby_names`. See
