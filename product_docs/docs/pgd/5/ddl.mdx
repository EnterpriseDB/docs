...
The second kind is known as a relation DML lock (or global DML lock). This kind of lock is used when
...
explained in [Executing DDL on PGD systems](#executing-ddl-on-pgd-systems):
...
[PGD functions that behave like DDL](#pgd-functions-that-behave-like-ddl).
...
PGD replicates role manipulation statements if `bdr.role_replication` is
