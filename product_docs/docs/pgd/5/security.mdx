...
Configuring and managing PGD doesn't require superuser access, nor is that recommended.
The privileges required by PGD are split across the following default/predefined roles, named
...
are executed in the database where PGD is running and the
...
and need to be dropped manually if required. This allows PGD to be used in multiple
...
### bdr_superuser
<!-- Not sure whether these should be changed to PGD -->
