...
You should also check the release notes for [pglogical 3.7.21](/pgd/3.7/pglogical/release-notes/#pglogical-3721) for resolved issues
which affect BDR as well.


### Resolved Issues

- Fix memory leak in consensus process (BDR-3409, RT91830)
  The memory consumed by the node is merely 32 bytes but when the
...
- Fix issue where a node can be inconsistent with the group after rejoining
  If a node had been part of a sub-group, then parted and then rejoined to the
  group, it could be inconsistent with the group. The changes from some nodes
...
- Fix upgrades for nodes with CRDTs (BDR-3302)

- Fix replication for subscriber-only node (BDE-3204, RT89814)

- Fix WARNING message in bdr.raft_leadership_transfer() (BDR-3426, RT92180)

- Fixed segfault where a conflict_slot was being used (BDR-1745, RT76439, RT92180)
