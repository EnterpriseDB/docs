...
Tracks per-transaction progress used by `bdr.wait_for_xid_progress()`. A node
records the transaction status here as soon as it makes progress on it, and
also reports it to all other nodes; the same table stores the reports received
from peer nodes, with `peer_node_id`, `peer_apply_xid` and `peer_apply_lsn` then
referring to the peer.
...
| origin_node_topxid     | xid    | Top-level XID of the transaction on the origin node                 |
| origin_node_lsn        | pg_lsn | LSN on the origin node at the time progress was recorded            |
| peer_node_id           | oid    | OID of the peer node this progress report is about                  |
| peer_apply_xid         | xid    | XID applied on the peer node                                        |
| peer_apply_lsn         | pg_lsn | LSN applied on the peer node                                        |
...
| is_decoder_slot     | boolean     | True if the slot is the [decoding worker](nodes#decoding-worker)'s slot                                                                 |
...
nodes in the EDB Postgres Distributed cluster. If this is a logical standby node, then only
...
| ap_wq_depends_on   | Oid\[] | Oids of the nodes on which the work item depends                                                                         |
...
| ap_wq_work_kind    | char   | The work kind can be either 'c' (Create Partition), 'm' (Migrate Partition), 'd' (Drop Partition), 'a' (Alter Partition) |
| ap_wq_work_sql     | text   | SQL query for the work item                                                                                              |
| ap_wq_depends_on   | Oid\[] | Always NULL                                                                                                              |
...
Stores the autopartitioning rule configured for each relation via `bdr.autopartition()`.
One row per relation that has autopartitioning enabled.

#### `bdr.autopartition_rules` Columns

| Column                          | Type        | Description                                                              |
| -------------------------------- | ----------- | ------------------------------------------------------------------------ |
| ap_partition_relid                | oid         | Oid of the relation being autopartitioned                                |
| ap_partition_relname              | name        | Name of the relation being autopartitioned                               |
| ap_partition_schemaname           | name        | Schema of the relation being autopartitioned                             |
| ap_partition_increment_kind       | char        | Internal kind marker for the partition increment value                   |
| ap_secondary_tablespace           | oid         | Tablespace Oid for new partitions, if configured                         |
| ap_minimum_advance_partitions     | integer     | Minimum number of partitions to keep pre-created ahead of current data   |
| ap_maximum_advance_partitions     | integer     | Maximum number of partitions to keep pre-created ahead of current data   |
| ap_is_autoscaled                  | boolean     | Whether the partition increment is automatically scaled                  |
| ap_latest_partitions              | integer     | Number of most recent partitions to keep                                 |
| ap_enabled                        | boolean     | Whether autopartitioning is currently enabled for this relation          |
| ap_migrate_after_period           | interval    | Age after which partitions are migrated, if configured                  |
| ap_data_retention_period          | interval    | Age after which partitions are dropped, if configured                    |
| ap_last_triggered                 | timestamptz | When the autopartition worker last processed this rule                   |
| ap_partition_increment_value      | text        | The configured partition increment                                      |
| ap_partition_autocreate_expr      | text        | Expression used to determine when to create the next partition          |
| ap_partition_initial_lowerbound   | text        | Lower bound used for the first partition                                 |
| ap_partition_last_upperbound      | text        | Upper bound of the most recently created partition                       |
| ap_is_local                       | boolean     | Whether this rule is managed locally rather than cluster-wide            |

### `bdr.autopartition_partitions`

Tracks the lifecycle of each partition created by the autopartitioning worker for
a given parent relation.

#### `bdr.autopartition_partitions` Columns

| Column               | Type        | Description                                             |
| --------------------- | ----------- | -------------------------------------------------------- |
| ap_parent_relid        | oid         | Oid of the autopartitioned parent relation                |
| ap_part_relname        | name        | Name of the partition                                     |
...
Lists the local Raft consensus instance for each top-level node group. There is
currently always exactly one Raft instance per top-level group.

#### `bdr.raft_instances` Columns

| Name          | Type | Description                              |
| -------------- | ---- | ------------------------------------------ |
| instance_id     | int  | Identifier of the Raft instance (always 1)  |
| node_group_id   | oid  | OID of the top-level node group             |
