...
-   `bdr.accept_connections` - Whether this node accepts BDR connections
    (default "on"). Settable only by a superuser.
...
-   `bdr.raft_election_timeout` - Election timeout for the Raft consensus
    mechanism (default 6 seconds).
-   `bdr.raft_vacuum_interval` - Maximum delay between checks for vacuum of
    Raft catalogs (default 5 minutes).
-   `bdr.raft_vacuum_full_interval` - Maximum delay between automatic vacuum
    full of Raft catalogs (default 1 day).
...
-   `bdr.raft_group_max_connections` - The name used internally by
     `bdr.group_max_connections` (see above) starting with BDR 3.7.13.
     Same meaning, default (100), minimum (10), and maximum (1000) as
     `bdr.group_max_connections`; setting either one sets the same
     underlying value. May only be set at Postgres server start.
-   `bdr.lcr_cleanup_interval` - The minimum time interval, in seconds,
     between two cleanup attempts for LCR (logical change record) files used
     by the [decoding worker](nodes#decoding-worker). Default 3 minutes.
     A value of zero turns off LCR cleanup.
