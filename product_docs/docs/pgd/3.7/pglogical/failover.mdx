...
-   Provider2 has:
    ...
    -   `postgresql.conf`:
        ...
        -   [`pglogical.synchronize_failover_slot_names`](configuration/#pglogicalsynchronize_failover_slot_names)
            can be modified to specify which slots should be synchronized (default is all pglogical/bdr slots)
-   Provider1 has:
    -   `postgresql.conf`:
        -   [`pglogical.standby_slot_names`](configuration/#pglogicalstandby_slot_names)
...
slots specifyed in the [`pglogical.synchronize_failover_slot_names`](configuration/#pglogicalsynchronize_failover_slot_names)
