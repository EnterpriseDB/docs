...
HARP provides [HTTP(S) health check APIs](02_overview#health-check). Add the below settings only if health check APIs are required. Specify all elements under a section named `http` with these multiple
supplementary entries:

- **`enable`**: Set `true` to enable HTTP(S) health check APIs. Defaults to  `false`.

- **`host`**: Host name of HTTP(S) API server. Typically same as hostname of HARP service as it's running within the HARP service. Defaults to  `0.0.0.0`.
...
- **`cert_file`**: HTTP server SSL certificate file. Mandatory if `secure` set to `true`.

- **`key_file`**: HTTP server SSL key file. Mandatory if `secure` set to `true`.
