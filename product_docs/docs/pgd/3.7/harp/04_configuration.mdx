...
HARP provides [HTTP(S) health check APIs](02_overview#health-check). Add the following settings only if health check APIs are required. Specify all elements under a section named `http` with these multiple
supplementary entries:
