...
HARP exposes the following liveness and readiness health check endpoints. These are HTTP endpoints and if enabled HARP will start a HTTP server within the respective service. Both the endpoints are HTTP `GET` requests and do not require a request body. Below are the endpoints for the both the health checks.
```
/health/is-live
/health/is-ready
```
...
Liveness endpoint returns `true` with HTTP status code `200 (OK)` if the service is up and a connection refused error if the service is down but will never return `false`. This is due to the fact that the HTTP server listening for probes request is stopped if the corresponding HARP service is down. This endpoint is exposed for `harp-manager` and `harp-proxy` services.

#### Readiness

Readiness endpoint is exposed for `harp-proxy` only. It returns `true` with HTTP status code `200 (OK)` if the proxy is up and is able to successfully query the backend Postgres through itself i.e using the host and port of proxy node exposed for client applications. It would return `false` with HTTP status code `500 (Internal Server Error)` in the following scenarios.
* The HARP Proxy cannot establish a TCP connection to the DCS
* The DCS reports that there is no current write leader available
...
To enable secure communication over the HTTP endpoints set the config parameter `http.secure` to `true` and provide cert and key file paths in config parameters `cert_file` and `key_file` respectively.

The `http.probes.timeout` is used by readiness check. This check will return `false` if proxy doesn't return the response from backend Postgres within this time period. This value should be set based on failover time which varies from 1s to 25s and retry attempts to avoid false positives.
