...
The `http.probes.timeout` is used by readiness check. This check will return `false` if proxy does not receive the response from backend Postgres within this time period. This value should be set based on failover time and retry attempts to avoid false positives.
