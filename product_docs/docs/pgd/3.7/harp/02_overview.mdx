...
HARP exposes liveness probe for both `harp-manager` and `harp-proxy` services and a readiness probe for `harp-proxy` service only. These are HTTP probes and will start a HTTP server within the respective service if enabled.


#### Liveness probes

Liveness probe returns `UP` with http status code `200 (OK)` if the service is up and running and a connection refused error if the service is down but will never return `DOWN`. This is due to the fact that the http server listening for probes request is stopped if the `harp-manager` and `harp-proxy` services are not up. Below are the endpoints for the liveness probe for the respective services.
```
/probes/manager/is_live
/probes/proxy/is_live
```

#### Readiness probe

Readiness probes is available for `harp-proxy` only. It returns `UP` with http status code `200 (OK)` if the proxy is up and is able to successfully execute the sql query (`select 1`) through itself i.e using the hostname and port of proxy node exposed for client applications. It would return `DOWN` with http status code `500 (Internal Server Error)` in the following scenarios.
...
Having that said, it is important to know that once started proxy always remains in ready state even if this probe return `DOWN` because the proxy can recover itself automatically (self heal) once problematic conditions are resolved on BDR or DCS side. If `harp-proxy` service is down then, similar to liveness probe this probe too would return connection refused error. Below is the endpoint for readiness probe.
```
/probes/proxy/is_ready
```

!!! Note
    Proxy would execute the SQL query only when the readiness endpoint is
    requested even though probes are enabled.

#### Probes configuration

The configuration to enable health check probes is simple and needs to be defined in the local config file of the respective HARP services. Since this can be enabled for both `harp-manager` and `harp-proxy` service the configs are defined at root level and not under any particular service in config file. Below is a sample config.
```
probes:
  enable: true
  http_port: 8080
...
The hostname for HTTP server is the hostname of the HARP node (`harp-manager` or `harp-proxy`) itself.
