...
HARP exposes the following liveness and readiness health check APIs. These are HTTP(s) APIs and if enabled HARP will start a HTTP server within the respective service. Both the APIs are HTTP(s) `GET` requests and do not require a request body.
```
GET /health/is-live
GET /health/is-ready
```
...
!!! Note
    Proxy would query backend database only when the readiness endpoint is requested and not on periodic basis.
...
The `http.probes.timeout` is used by readiness check. This check will return `false` if proxy does not return the response from backend Postgres within this time period. This value should be set based on failover time which varies from 1s to 25s and retry attempts to avoid false positives.
