...
HARP exposes liveness probe for `harp-manager` and `harp-proxy` services and a readiness probe for `harp-proxy` service only. These are HTTP probes and HARP will start a HTTP server within the respective service if enabled. Both the probes are http `GET` requests and do not require a request body.


#### Liveness probes

Liveness probe returns `UP` with http status code `200 (OK)` if the service is up and running and a connection refused error if the service is down but will never return `DOWN`. This is due to the fact that the http server listening for probes request is stopped if the requested `harp-manager` or `harp-proxy` service is down. Below are the endpoints for the liveness probe for the respective services.
```
...
The address used for HTTP server is `:http_port`.
