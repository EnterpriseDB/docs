...
<tr><td>BDR</td><td>5.8.0</td><td><details><summary>Ensure that disables subscriptions on subscriber-only nodes are not re-enabled</summary><hr/><p>During subscription reconfiguration, if there is no change required to a subscription,
...
<tr><td>BDR</td><td>5.8.0</td><td><details><summary>Subscriber-only nodes will give an error if there is an attempt to run DDL on them</summary><hr/><p>Subscriber-only nodes will now give an error if there is an attempt to run DDL on them.
This is to prevent accidental DDL execution on subscriber-only nodes. DDL can only be executed on the node when <code>bdr.permit_unsafe_commands</code> is set to true; the executed DDL is not replicated.</p>
</details></td><td></td></tr>
