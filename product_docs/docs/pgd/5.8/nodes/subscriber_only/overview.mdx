...
Subscriber-only nodes are not intended to be used for DDL operations. You can use them for read-only queries, but you should not use them for any kind of write operation, including DDL operations. If you attempt to run DDL on a subscriber-only node, you will receive an error message. DDL can only be executed on the node when `bdr.permit_unsafe_commands` is set to true; the executed DDL is not replicated.
