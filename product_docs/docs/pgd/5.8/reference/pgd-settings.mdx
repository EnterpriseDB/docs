...
Tracks apply statistics for each subscription with [`bdr.stat_subscription`](/pgd/5.8/reference/catalogs-visible#bdrstat_subscription) (default is `on`).

### `bdr.track_relation_apply`

Tracks apply statistics for each relation with [`bdr.stat_relation`](/pgd/5.8/reference/catalogs-visible#bdrstat_relation) (default is `off`).

### `bdr.track_apply_lock_timing`

Tracks lock timing when tracking statistics for relations with [`bdr.stat_relation`](/pgd/5.8/reference/catalogs-visible#bdrstat_relation) (default is `off`).
