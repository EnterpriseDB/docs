...
| `failover_slot_scope` | `text` | PGD 5.7 and later only. Sets the scope for Logical Slot Failover support. Valid values are `global` or `local`. Default is `local`. For more information, see [CDC Failover support](/pgd/5.8/cdc-failover). |
...
the log files and the [`bdr.event_summary`](/pgd/5.8/reference/catalogs-internal#bdrevent_summary)
