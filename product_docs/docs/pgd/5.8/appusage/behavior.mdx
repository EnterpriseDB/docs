...
!!! Warning REPLICA IDENTITY FULL required for tables with TOAST-able columns
    ...
    safely under concurrent updates from multiple nodes. PGD 5 does not set
    this by default — you need to enable it explicitly on every table that
    can store TOAST'd values:
    ...
    of the same row — resulting in data divergence between nodes.

    To check the current replica identity:
