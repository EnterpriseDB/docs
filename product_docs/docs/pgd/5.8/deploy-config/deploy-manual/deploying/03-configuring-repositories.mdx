...
Perform the following operations on each host. For the purposes of this exercise, each host is a standard data node, but the procedure would be the same for other [node types](/pgd/5.8/nodes/overview), such as witness or subscriber-only nodes.
...
To install Postgres Distributed, you will need an EDB Repos 2.0 token to gain access to the EDB private software repositories. 
