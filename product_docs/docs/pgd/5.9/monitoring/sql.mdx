...
Also, the table [`bdr.node_catchup_info`](/pgd/5.8/reference/catalogs-visible/#bdrnode_catchup_info) gives information
...
-   [`bdr.stat_routing_state`](/pgd/5.8/reference/catalogs-visible/#bdrstat_routing_state) for monitoring the state of the connection routing with PGD Proxy uses to route the connections.
-   [`bdr.stat_routing_candidate_state`](/pgd/5.8/reference/catalogs-visible/#bdrstat_routing_candidate_state) for information about routing candidate nodes from the point of view of the Raft leader (the view is empty on other nodes).

## Monitoring Replication Peers

You use two main views for monitoring of replication activity:

-   [`bdr.node_slots`](/pgd/5.8/reference/catalogs-visible/#bdrnode_slots) for monitoring outgoing replication
-   [`bdr.subscription_summary`](/pgd/5.8/reference/catalogs-visible/#bdrsubscription_summary) for monitoring incoming replication
...
-   [`bdr.node_replication_rates`](/pgd/5.8/reference/catalogs-visible/#bdrnode_replication_rates) for monitoring outgoing replication

The [`bdr.node_replication_rates`](/pgd/5.8/reference/catalogs-visible/#bdrnode_replication_rates) view gives an overall picture of the outgoing
...
local node data. The other fields are also available from the [`bdr.node_slots`](/pgd/5.8/reference/catalogs-visible/#bdrnode_slots)
view.

Administrators can query [`bdr.node_slots`](/pgd/5.8/reference/catalogs-visible/#bdrnode_slots) for outgoing replication from the
...
You can further monitor subscriptions by monitoring subscription summary statistics through [`bdr.stat_subscription`](/pgd/5.8/reference/catalogs-visible/#bdrstat_subscription), and by monitoring the subscription replication receivers and subscription replication writers, using [`bdr.stat_receiver`](/pgd/5.8/reference/catalogs-visible/#bdrstat_receiver) and [`bdr.stat_writer`](/pgd/5.8/reference/catalogs-visible/#bdrstat_writer), respectively.
...
using the function [`bdr.wal_sender_stats()`](/pgd/5.8/reference/functions/#bdrwal_sender_stats). For example:
...
[`bdr.get_decoding_worker_stat()`](/pgd/5.8/reference/functions/#bdrget_decoding_worker_stat). For example:
...
-   [bdr.stat_commit_scope](/pgd/5.8/reference/catalogs-visible/#bdrstat_commit_scope) for cumulative statistics for each commit scope.

-   [bdr.stat_commit_scope_state](/pgd/5.8/reference/catalogs-visible/#bdrstat_commit_scope_state) for information about the current use of commit scopes by backend processes.
...
Global locks held on the local node are visible in the [`bdr.global_locks`](/pgd/5.8/reference/catalogs-visible/#bdrglobal_locks) view. 
...
See [Catalogs](/pgd/5.8/reference/catalogs-visible/) for details on all fields, including lock
timing information.
...
For monitoring purposes, use [`bdr.conflict_history_summary`](/pgd/5.8/reference/catalogs-visible#bdrconflict_history_summary), which doesn't
...
Two monitoring views exist: [`bdr.stat_subscription`](/pgd/5.8/reference/catalogs-visible#bdrstat_subscription) for subscription statistics
and [`bdr.stat_relation`](/pgd/5.8/reference/catalogs-visible#bdrstat_relation) for relation statistics. These views both provide:
...
For relations only, [`bdr.stat_relation`](/pgd/5.8/reference/catalogs-visible#bdrstat_relation) also includes:
...
For subscriptions only, [`bdr.stat_subscription`](/pgd/5.8/reference/catalogs-visible#bdrstat_subscription) includes:
...
[`bdr.track_subscription_apply`](/pgd/5.8/reference/pgd-settings#bdrtrack_subscription_apply) and [`bdr.track_relation_apply`](/pgd/5.8/reference/pgd-settings#bdrtrack_relation_apply),
respectively.
...
You can reset the stats counters for these views to zero using the functions [`bdr.reset_subscription_stats`](/pgd/5.8/reference/functions-internal#bdrreset_subscription_stats) and [`bdr.reset_relation_stats`](/pgd/5.8/reference/functions-internal#bdrreset_relation_stats).

PGD also monitors statistics regarding subscription replication receivers and subscription replication writers for each subscription, using [`bdr.stat_receiver`](/pgd/5.8/reference/catalogs-visible/#bdrstat_receiver) and [`bdr.stat_writer`](/pgd/5.8/reference/catalogs-visible/#bdrstat_writer), respectively.
...
The view [`bdr.group_versions_details`](/pgd/5.8/reference/catalogs-visible#bdrgroup_versions_details) uses the function
[`bdr.run_on_all_nodes()`](/pgd/5.8/reference/functions#bdrrun_on_all_nodes) to retrieve Postgres and BDR extension versions from all 
...
[`bdr.monitor_group_versions()`](/pgd/5.8/reference/functions#bdrmonitor_group_versions), which uses PGD version
...
The view [`bdr.group_raft_details`](/pgd/5.8/reference/catalogs-visible#bdrgroup_raft_details) uses the functions
[`bdr.run_on_all_nodes()`](/pgd/5.8/reference/functions#bdrrun_on_all_nodes) and [`bdr.get_raft_status()`](/pgd/5.8/reference/functions#bdrget_raft_status) to retrieve Raft
...
The described behavior is implemented in the function
[`bdr.monitor_group_raft()`](/pgd/5.8/reference/functions#bdrmonitor_group_raft), which uses Raft consensus status
information returned from the view [`bdr.group_raft_details`](/pgd/5.8/reference/catalogs-visible#bdrgroup_raft_details)
...
Two further views that can give a finer-grained look at the state of Raft consensus are [`bdr.stat_raft_state`](/pgd/5.8/reference/catalogs-visible/#bdrstat_raft_state), which provides the state of the Raft consensus on the local node, and [`bdr.stat_raft_followers_state`](/pgd/5.8/reference/catalogs-visible/#bdrstat_raft_followers_state), which provides a view when on the Raft leader (it is empty on other nodes) regarding the state of the followers of that Raft leader.
...
[`bdr.local_group_slot_name()`](/pgd/5.8/reference/functions#bdrlocal_group_slot_name).
...
The function [`bdr.monitor_local_replslots()`](/pgd/5.8/reference/functions#bdrmonitor_local_replslots) provides a summary of whether all
...
You can monitor these transactions by examining the [`bdr.stat_activity`](/pgd/5.8/reference/catalogs-visible#bdrstat_activity) catalog. The processes report different `wait_event` states as a transaction is committed. This monitoring only covers transactions in progress and doesn't provide historical timing information.
