...
The [`bdr.ddl_replication`](/pgd/5.9/reference/pgd-settings#bdrddl_replication) parameter specifies replication behavior.
...
function [`bdr.replicate_ddl_command()`](/pgd/5.9/reference/functions#bdrreplicate_ddl_command). This function can be helpful if you
...
execute it manually on each node using the [`bdr.ddl_replication`](/pgd/5.9/reference/pgd-settings#bdrddl_replication) configuration
parameter.
