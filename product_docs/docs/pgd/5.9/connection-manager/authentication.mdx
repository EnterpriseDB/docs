...
!!! Note
For Connection Manager authentication to work correctly, set or change role passwords only while connected to a PGD-enabled database with [`bdr.role_replication`](/pgd/current/reference/pgd-settings/#bdrrole_replication) set to `on`. A password set or changed while disconnected from the PGD-enabled database, or with `bdr.role_replication` set to `off`, doesn't replicate, and Connection Manager can then fail to authenticate that role.
!!!
