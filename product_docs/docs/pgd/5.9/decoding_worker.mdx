...
[`bdr.lcr_cleanup_interval`](/pgd/5.9/reference/pgd-settings#bdrlcr_cleanup_interval), which defaults to 3 minutes. The cleanup is
disabled when [`bdr.lcr_cleanup_interval`](/pgd/5.9/reference/pgd-settings#bdrlcr_cleanup_interval) is 0.
...
-   [`bdr.enable_wal_decoder`](/pgd/5.9/reference/pgd-settings#bdrenable_wal_decoder) &mdash; When `false`, all WAL
...
-   [`bdr.receive_lcr`](/pgd/5.9/reference/pgd-settings#bdrreceive_lcr) &mdash; When `true` on the subscribing node, it requests WAL
...
To enable transaction streaming with the WAL decoder, set the PGD group's `bdr.streaming_mode` set to ‘default’ using [`bdr.alter_node_group_option`](/pgd/5.9/reference/nodes-management-interfaces#bdralter_node_group_option).
