...
Node removal, also called *parting*, is done using the [`bdr.part_node()`](/pgd/5.9/reference/nodes-management-interfaces#bdrpart_node)
function. You must specify the node name (as passed during node creation)
to remove a node. You can call the [`bdr.part_node()`](/pgd/5.9/reference/nodes-management-interfaces#bdrpart_node) function from any active
...
node by using the function [`bdr.drop_node()`](/pgd/5.9/reference/functions-internal#bdrdrop_node).
