...
!!!note
On Postgres 17 and later, `pgd node upgrade` can fail with an invalid logical replication slot error when upgrading from PGD 5 to PGD 6. PGD 5.9.4 fixes the issue by hiding BDR-managed slots from `pg_upgrade`'s slot queries, allowing the upgrade to complete successfully. Upgrade to PGD 5.9.4 before running `pgd node upgrade` on Postgres 17+.
!!!
...
|       | ~~--new-socketdir~~ | ~~/var/run/postgresql~~ | ~~PGSOCKETDIRNEW~~ | ~~Directory to use for postmaster sockets in the new cluster~~ Removed in 5.8.1. |
...
|       | --key-unwrap-command |                     | PGDATAKEYUNWRAPCMD | Command to unwrap (decrypt) the data encryption key and access the files to copy. The command must be the same specified during the server initialization. |

See also [Global Options](/pgd/5.9/cli/command_ref/#global-options).
