...
Column-level conflict resolution requires the table to have `REPLICA IDENTITY FULL`. The [bdr.alter_table_conflict_detection()](/pgd/5.8/reference/conflict_functions#bdralter_table_conflict_detection) function checks that and fails with an error if this setting is missing.
