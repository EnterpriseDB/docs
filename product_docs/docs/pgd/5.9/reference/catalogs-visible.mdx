...
This table tracks the status of logical replication slots that are being used with failover support. For more information on failover replication slots, see [CDC Failover support](/pgd/5.9/cdc-failover).
...
| subscription_status        | text | Status of the subscription. Possible values are: `initializing`,`replicating`,`down`,`disabled` and `unknown` |
...
Lists the local Raft consensus instance for each top-level node group. There
is currently always exactly one Raft instance per top-level group.

#### `bdr.raft_instances` columns

| Name          | Type | Description                                 |
| ------------- | ---- | -------------------------------------------- |
| instance_id   | oid  | OID of the Raft instance                     |
...
A view showing the connection and replay status of each subscription on the
local node, joined with the origin and target node names. Useful for
checking whether replication from a given peer node is currently connected
or blocked.
...
| Name               | Type        | Description                                                             |
| ------------------ | ----------- | ------------------------------------------------------------------------ |
| node_id             | oid         | OID of the target (local) node                                           |
| node_name           | name        | Name of the target (local) node                                          |
| origin_node_id      | oid         | OID of the origin node the subscription replicates from                  |
| origin_node_name    | name        | Name of the origin node the subscription replicates from                 |
| sub_id              | oid         | OID of the subscription                                                  |
| sub_name            | name        | Name of the subscription                                                 |
| connected           | boolean     | Whether the subscription is currently connected to the origin node       |
| replication_blocked | boolean     | Whether replication appears blocked (retries at the same LSN with no commits in progress) |
| connect_time        | timestamptz | Time the current connection was established, if connected                |
| disconnect_time     | timestamptz | Time of the last disconnect, if currently disconnected                   |
| uptime              | interval    | How long the current connection has been up, if connected                |
...
| subscription_mode   | char                     | Mode of the subscription, see [`bdr.subscription_summary`](/pgd/5.9/reference/catalogs-visible/#bdrsubscription_summary) for more details
...
also reports it to all other nodes; the same table stores the reports
received from peer nodes, with `peer_node_id`, `peer_apply_xid`, and
`peer_apply_lsn` then referring to the peer.
