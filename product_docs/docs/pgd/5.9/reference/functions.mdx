...
```sql
bdr.consensus_snapshot_import(snapshot bytea, raft_instance_id integer DEFAULT NULL)
```
...
Returns status information about the current consensus (Raft) worker, as a JSON document.

#### Synopsis

```sql
bdr.get_consensus_status(node_group_name text DEFAULT NULL)
```
...
|-------------------|--------------------------------------------------------------------------------------|
| `node_group_name` | Name of the node group to report on. If NULL, reports on the top-level node group. |

### `bdr.get_raft_status`

Returns status information about the current consensus (Raft) worker, as a JSON document.
Both this function and `bdr.get_consensus_status` are backed by the same
underlying implementation, so calling `bdr.get_raft_status` without the
second argument is equivalent to calling `bdr.get_consensus_status`.

#### Synopsis

```sql
bdr.get_raft_status(node_group_name text DEFAULT NULL, include_node_info boolean DEFAULT false)
```
...
|---------------------|--------------------------------------------------------------------------------------|
| `node_group_name`   | Name of the node group to report on. If NULL, reports on the top-level node group. |
| `include_node_info` | If `true`, also includes per-node information (such as heartbeat response time and clock drift) in the returned JSON. Defaults to `false`. |
...
If you are using [Optimized Topology](../nodes/subscriber_only/optimizing-so), we recommend using [`bdr.wait_node_confirm_lsn`](/pgd/5.9/reference/functions#bdrwait_node_confirm_lsn) instead.
)
...
Both parameters are required. Pass `NULL` explicitly for either one to get the "wait for all slots" or "use the current write LSN" behavior described above — there's no default that lets you omit them.

#### Parameters

| Parameter  | Description                                                                 |
|------------|-----------------------------------------------------------------------------|
| `slotname` | Name of the replication slot to wait for. If NULL, waits for all PGD slots. |
| `target`   | LSN to wait for. If NULL, uses the current write LSN on the local node.     |
...
We recommend using this function if you are using [Optimized Topology](../nodes/subscriber_only/optimizing-so) instead of [`bdr.wait_slot_confirm_lsn`](/pgd/5.9/reference/functions#bdrwait_slot_confirm_lsn).
...
```sql
bdr.wait_node_confirm_lsn(nodename name, target pg_lsn)
```
...
| Parameter  | Description                                                                 |
|------------|-----------------------------------------------------------------------------|
| `nodename` | Name of the node to wait for. If NULL, waits for all nodes.                 |
| `target`   | LSN to wait for. If NULL, uses the current wal_flush_lsn on the local node. |

#### Notes

Requires `bdr_application` privileges to use.

Both parameters are required. Pass `NULL` explicitly for either one to get the "wait for all nodes" or "use the current wal_flush_lsn" behavior described above — there's no default that lets you omit them.
...
| `ddl_locking` | A string that sets the [`bdr.ddl_locking`](/pgd/5.9/reference/pgd-settings#bdrddl_locking) value while replicating. Defaults to the GUC value for `bdr.ddl_locking` on the local system that's running `replicate_ddl_command`. |
...
| Column name                | Description                                                                                                               |
|----------------------------|---------------------------------------------------------------------------------------------------------------------------|
| `commit_scope_id`          | OID of the commit scope (see [`bdr.commit_scopes`](/pgd/5.9/reference/catalogs-visible#bdrcommit_scopes)).             |
...
bdr.logical_transaction_status(node_id OID, xid OID,
                               ...
                               queried_by_origin boolean DEFAULT false)
```

Note that the underlying function doesn't declare SQL parameter names, so
these arguments must be passed positionally rather than by name.
...
| `queried_by_origin`    | Advanced/internal use. Only the origin node should set this to `true`, and only when it's uncertain about a transaction that is still prepared but not yet finished. It forces the CAMO partner to resolve an otherwise-unknown or provisional-abort decision to `abort`, since the partner can't safely report `committed` before the transaction actually commits. Defaults to `false`. |
...
**Deprecated**. Use [`bdr.create_commit_scope`](/pgd/5.9/reference/functions#bdrcreate_commit_scope) instead. Previously, this function was used to add a commit scope to a node group. It's now deprecated and will emit a warning until it is removed in a future release, at which point it will raise an error.
...
`bdr.create_commit_scope` replaces the deprecated [`bdr.add_commit_scope`](/pgd/5.9/reference/functions#bdradd_commit_scope) function. Unlike `add_commit_scope`, it does not silently overwrite existing commit scopes when the same name is used. Instead, an error is reported.
...
**Deprecated**. Use [`bdr.drop_commit_scope`](/pgd/5.9/reference/functions#bdrdrop_commit_scope) instead. Previously, this function was used to remove a commit scope from a node group. It's now deprecated and will emit a warning until it is removed in a future release, at which point it will raise an error.
