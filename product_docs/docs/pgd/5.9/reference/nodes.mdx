...
`bdr_init_physical` requires that both the source node and the joining node have exactly the same PGD version. You can't use this command to join a node with a different PGD version to an existing cluster. This means that you can't use `bdr_init_physical` for rolling upgrades using the node join method. For rolling upgrades, use logical join instead. See [Rolling upgrade using node join](/pgd/5.9/upgrades/manual_overview/#rolling-upgrade-using-node-join) for more details.
!!!
...
#### Preparing a data directory for large databases

For very large databases, where `pg_basebackup` takes hours, populate the data directory manually before running `bdr_init_physical` rather than letting the command take its own base backup. Doing so protects the write-ahead log (WAL) the new node needs during the gap between taking the backup and running the join, and avoids redoing the backup if the join has to be retried.
...
1. Before running `bdr_init_physical`, run this query on the source and check `wal_status`:
   ...
   `reserved` or `extended` means it's safe to run `bdr_init_physical` now. `unreserved` means the WAL is close to being removed, so run `bdr_init_physical` right away. `lost` means the WAL is already gone, and you need to redo the base backup before you can proceed.

1. Run `bdr_init_physical`, pointing `-D`/`--pgdata` at the directory you backed up in step 1. Since the directory isn't empty, the command uses it as the base for the new node instead of taking its own backup, and waits as long as needed for the node to replay WAL and promote before joining the group:

   ```shell
   bdr_init_physical -D <pgdata> --node-name <new_node_name> \
     --local-dsn "host=localhost port=<port> dbname=<dbname> user=<username>" \
     --remote-dsn "host=<source_host> port=5432 dbname=<dbname> user=<username>"
   ```

1. Once `bdr_init_physical` completes and the new node has joined the group, drop the manual slot on the source:
