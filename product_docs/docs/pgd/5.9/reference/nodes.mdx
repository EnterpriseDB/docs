...
!!!Note
For very large databases, where `pg_basebackup` takes hours, see [PLACEHOLDER: link to KB article] for a procedure that populates the data directory manually before running `bdr_init_physical` and protects the WAL with a permanent replication slot.
!!!
