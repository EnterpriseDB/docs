...
To enable CDC Failover support run the SQL command and call the [`bdr.alter_node_group_option`](/pgd/5.9/reference/nodes-management-interfaces#bdralter_node_group_option) function with the following parameters:
...
Replace `<top-level group name>` with the name of your cluster’s top-level group. If you don't know the name, it's the group with a node_group_parent_id equal to 0 in [`bdr.node_group`](/pgd/5.9/reference/catalogs-visible#bdrnode_group). 

If you do not know the name, it is the group with a node_group_parent_id equal to 0 in [`bdr.node_group`](/pgd/5.9/reference/catalogs-visible#bdrnode_group). You can also use:
...
The status of failover slots is tracked in the [`bdr.failover_replication_slots`](/pgd/5.9/reference/catalogs-visible#bdrfailover_replication_slots) table.
