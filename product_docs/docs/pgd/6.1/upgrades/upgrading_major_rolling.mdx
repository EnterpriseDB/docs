...
### Check harp-proxy leader

Start the upgrade on a node that isn't the harp-proxy leader. Check the harp-proxy leader:

```bash
a14b6xa14b4tpgd6major-d1:~ $ harpctl get leader a
```
```
...
```bash
a14b6xa14b4tpgd6major-d1:~ $ harpctl fence a14b6xa14b4tpgd6major-d1
```
```
2025-08-19T07:20:20.204Z	INFO	cmd/fence.go:42	fence node a14b6xa14b4tpgd6major-d1
```
```bash
a14b6xa14b4tpgd6major-d1:~ $ harpctl get nodes
Cluster  Name                     Location Ready Fenced Allow Routing Routing Status Role    Type Lock Duration 
-------  ----                     -------- ----- ------ ------------- -------------- ----    ---- ------------- 
bdrgroup a14b6xa14b4tpgd6major-d1 a        false true   true          N/A            primary bdr  6             
bdrgroup a14b6xa14b4tpgd6major-d2 a        true  false  true          ok             primary bdr  6
```
...
On the fenced node, enable Connection Manager by adding the guc `bdr.enable_builtin_connection_manager = true`  in postgresql.conf file. 
...
On the fenced node, start the Postgres service.
...
```bash
a14b6xa14b4tpgd6major-d1:~ $ harpctl unfence a14b6xa14b4tpgd6major-d1
```
