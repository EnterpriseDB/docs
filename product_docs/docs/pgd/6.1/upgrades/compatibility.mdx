...
There are some changes in PGD 6.1 that are not compatible with 5.x and other previous versions.
...
Moreover, `bdr.proxy_conifg_summary` and `bdr.proxy_config_view
For more details check [here](/pgd/6.1/reference/connection-manager/).
...
- The `enable_proxy_routing` `group` option is now `enable_routing`, although `bdr.alter_node_group_option` still supports the older syntax for backwards compatibility. The views only show the new name.
...
This feature is documented [here](pgd/6.1/reference/node_management/automatic_sync/).
