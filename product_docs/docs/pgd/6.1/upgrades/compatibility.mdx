...
PGD 6.1k comes with Connection Manager (CM) built in and it runs on each data node.
It is not backward compatible with 5.8.1 or other previous versions.
...
There are no more proxies as there were in 5.8.1, CM is configurable using group options.
...
- Previously deprecated configuration parameters (GUCs) now removed (this includes, for example, all the pglogical prefixed GUCs—this is not backwards compatible with PGD 5.8.1 or older versions).
