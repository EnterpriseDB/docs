...
navTitle: v5 to v6 - rolling major version upgrade
deepToC: true
redirects:
...
!!!Note
The following overview and worked example assume you're upgrading to or from 5.9.0+. 
...
```bash
enterprisedb@pgd-a1:/home/rocky $ pgd nodes list
```
```
...
Ensure that the packages for EPAS 17 and the corresponding PGD 6 packages are installed on all nodes in the cluster.
To prevent binary conflicts, you must remove the PGD 5 packages, viz., edb-pgd5-cli and edb-bdr5-epas13, before installing the PGD 6 packages. The commands below were used for the RHEL 8 platform. Use the appropriate commands for your specific platform.

```bash
dnf remove edb-pgd5-cli edb-bdr5-epas13 -y
...
```bash
enterprisedb@pgd-a1:/home/rocky $ pgd nodes list --versions
```
```
...
```bash
enterprisedb@pgd-a1:/home/rocky $ pgd group group-a show --summary
```
```
...
I am on node pgd-a1 and want to upgrade it. My current write leader is pgd-a2, so we are good to go.
...
```bash
enterprisedb@witness-a1:/home/rocky $ /usr/edb/as17/bin/pgd group group-a set-leader pgd-a1
```
```
...
```bash
enterprisedb@witness-a1:/home/rocky $ /usr/edb/as17/bin/pgd group group-a show --summary
```
```
 Group Property    | Value   
-------------------+---------
...
- postgresql.conf, postgresql.auto.conf,and pg_hba.conf 
...
For instance, I encountered the `operator_precedence_warning` GUC, which I successfully ignored in my new configuration.
!!!
...
Before running the actual upgrade, perform a dry run to check the compatibility and upgrade feasibility. Run the upgrade command with the `--check` option.
...
Run the command without the `--check` option
...
!!! Tip 
For a complete upgrade, follow any suggestions, like executing scripts or SQL commands after the upgrade is complete.
!!!

!!! Note
You can use the `--link` option for a hard link. This option works if both the data dirs are on the same filesystem.
!!!

## Post-upgrade steps

### Update server, data, and binary dir
...
```bash
enterprisedb@pgd-a1:/home/rocky $ /usr/edb/as17/bin/pgd nodes list --versions
```
```
 Node Name  | BDR Version         | Postgres Version 
------------+---------------------+------------------
...
Note that the node pgd-a1’s BDR Version has been upgraded to 6.1.0 and Postgres Version to 17.6.0.
...
```sql
enterprisedb@pgd-a1:/home/rocky $ psql "host=pgd-a1 port=6444 dbname=bdrdb user=enterprisedb " -c "select node_name from bdr.local_node_summary;"
```
```
 node_name 
-----------
...
!!! Note
If PGD Proxy is running on the same host and using the Connection Manager port (default 6444), you must stop it before starting the new server. Otherwise, the startup will fail due to a port conflict.
!!!
...
```bash
enterprisedb@pgd-a2:/home/rocky $ /usr/edb/as17/bin/pgd nodes list --versions
```
```
 Node Name  | BDR Version          | Postgres Version 
------------+----------------------+------------------
...
### Verify the Connection Manager for every data node 

```sql
enterprisedb@pgd-a2:/home/rocky $ psql "host=pgd-a2 port=6444 dbname=bdrdb user=enterprisedb " -c "select node_name from bdr.local_node_summary;"
```
```
 node_name 
-----------
...
```
```sql
enterprisedb@pgd-a2:/home/rocky $ psql "host=pgd-a2 port=6445 dbname=bdrdb user=enterprisedb " -c "select node_name from bdr.local_node_summary;"
```
```
 node_name 
-----------
