...
1. Before upgrade from PGD4.4+ to PGD6.1+, ensure the Connection Manager guc is set to *false* in postgresql.conf:
...
6. Ensure the PGD write leader is the same as the harp-proxy leader:
...
7. Fence off the non write leader node from HARP: 
...
9. Fence from PGD’s routing (`route_fence: true`) as well , so it doesn't end up becoming the leader:
...
11. Enable the Connection Manager by changing the `guc bdr.enable_builtin_connection_manager` to True in postgresql.conf file 
12. Stop the Postgres service.
13. Stop harp manager.
14. Start the Postgres service.
15. Start harp manager.
16. Unfence d1 from harp.

```bash
test-pgd6major-d1:~ $ harpctl unfence test-pgd6major-d1
```

17. Unfence d1 from PGD’s routing 
...
18. Repeat the same steps on all nodes.
19. Stop proxy and let the application switch to Connection Manager.
...
Use the following steps to move it to Connection Manager:
