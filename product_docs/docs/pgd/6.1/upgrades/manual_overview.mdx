...
## Moving from HARP to Connection Manager

A PGD 4 cluster using HARP-proxy based routing continue this routing method to all nodes until the entire cluster is upgraded to 6.1.0 or higher.
HARP-proxy based routing functions the same within a mixed version cluster.

### Switching from HARP to connection manager

HARP uses its own mechanism to elect a leader since a 4.x cluster does not have a write leader. 
There are two scenarios:
 ...
 - If Proxy runs on one or more PGD nodes, connection manager can be kept disabled when restarting PGD to 6.1.  Disabling it will prevent port interference. Once the entire cluster is upgraded to version 6.1.0 or higher, use the following steps to change to connection manager routing:
   ...
   1. Change the write leader to be same node as the leader proxy is using.
   2. Choose a PGD node that is not the write leader.
   3. Fence off with `route_fence` to *true* so it does not become the write leader.
   4. Stop the node.
   5. Enable connection manager and restart it.
   6. Unfence the node.
   7. Repeat the steps for all remaining nodes.
   8. Stop proxy and let the application switch to connection manager.
