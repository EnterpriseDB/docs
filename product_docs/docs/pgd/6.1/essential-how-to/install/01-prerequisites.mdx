...
If you want to install a learning/test environment, we recommend using the [PGD First Cluster](/pgd/current/get-started/first-cluster/).

!!!Note

If you want to install EDB Postgres Distributed (PGD) Expanded, consult the [Expanded installation guide](/pgd/current/expanded-how-to/install/01-prerequisites/).
!!!
...
In particular, the PostgreSQL TCP/IP port (5444 for EDB Postgres Advanced Server, 5432 for EDB Postgres Extended and community PostgreSQL) must be open
to all machines in the cluster. 
The PGD Connection Manager must also be accessible to all nodes in the cluster. By default, the Connection Manager uses port 6432 (or 6444 for EDB Postgres Advanced Server).
