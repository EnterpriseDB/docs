...
description: 'Known issues and limitations in EDB Postgres Distributed 6.1'
redirects: 
...
These are currently known issues in EDB Postgres Distributed 6.1.
...
-   Replication of materialized views only works with newly-created materialized views.  Materialized views that were created before 
    upgrading to 6.1.0 do not get replicated after upgrading. To prevent this issue, you can do one of the following:
...
PGD was developed to [enable rolling upgrades of PGD](upgrades/) by allowing mixed versions of PGD to operate during the upgrade process. 
