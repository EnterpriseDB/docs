...
commit](/pgd/current/terminology/#two-phase-commit-2pc) underneath. Therefore, configure
`max_prepared_transactions` high enough to handle all such transactions
originating per node.

## Limitations

See the Group Commit section of [Known Issues and Limitations](/pgd/current/known_issues/#group-commit).
...
See also [Limitations](/pgd/current/known_issues/#general-durability-limitations). 
