...
[Logical replication](/pgd/current/terminology/#logical-replication) in PGD is asynchronous by default, so only eventual consistency is guaranteed (within seconds usually). 
...
-   **[Write leader](/pgd/current/terminology/#write-leader)**: Receives all write operations when applications connect through the connection manager.
