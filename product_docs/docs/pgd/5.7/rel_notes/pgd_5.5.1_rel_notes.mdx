...
| BDR       | 5.5.1   | <details><summary>Disabled auto-triggering of node sync by default.</summary><hr/>Automatically triggered synchronization of data from a down node caused issues by failing to resume once it came back up. As a precautionary measure, the feature is now disabled by default (PGD setting [`bdr.enable_auto_sync_reconcile`](/pgd/5.7/reference/pgd-settings#bdrenable_auto_sync_reconcile)).</details>                                                                               | 11510  |
