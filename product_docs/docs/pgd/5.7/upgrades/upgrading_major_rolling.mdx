...
The only difference being that with `bdr_pg_upgrade` you can only upgrade major versions of Postgres (Community/PGE/EPAS), but with `pgd node upgrade` you can upgrade both at once.
!!!
