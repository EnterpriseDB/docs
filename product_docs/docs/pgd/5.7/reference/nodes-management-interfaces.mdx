...
We recommend avoiding using nodes to part themselves from the cluster and, instead, performing node parting operations from a node that can wait for completion. Then check the cluster status after the operation is complete.
