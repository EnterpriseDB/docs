...
* Whenever a worker has errored out (see [bdr.workers](/pgd/5.7/reference/catalogs-visible/#bdrworkers)
...
This is a local cache of the recent portion of leader change history. It has the same fields as [`bdr.leader`](/pgd/5.7/reference/catalogs-visible#bdrleader), except that it is an ordered set of (node_group_id, leader_kind, generation) instead of a map tracking merely the current version.
