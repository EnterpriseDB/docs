...
deepToC: true
redirects:
  - /pgd/5.6/install-admin/admin-manual/installing/03-configuring-repositories/ #generated for pgd deploy-config-planning reorg 
  - /pgd/5.6/admin-manual/installing/03-configuring-repositories/ #generated for pgd deploy-config-planning reorg 
---
...
Perform the following operations on each host. For the purposes of this exercise, each host is a standard data node, but the procedure would be the same for other [node types](/pgd/5.6/nodes/overview), such as witness or subscriber-only nodes.

-   Use your EDB account.
    -   Obtain your EDB repository token: [Get your token](/repos/getting_started/get_your_token/). 
...
To install both TPA and PGD, you will need an EDB Repos 2.0 token to gain access to the EDB private software repositories. 
