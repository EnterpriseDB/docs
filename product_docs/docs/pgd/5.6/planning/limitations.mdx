...
See [Durability limitations](/pgd/5.6/commit-scopes/limitations/) for a full
and current listing.

## Mixed PGD versions

PGD was developed to [enable rolling upgrades of PGD](/pgd/5.6/upgrades) by allowing mixed versions of PGD to operate during the upgrade process. 
