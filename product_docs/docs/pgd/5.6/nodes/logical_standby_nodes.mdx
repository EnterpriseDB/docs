...
constantly updating until they're required. This behavior is similar to how Postgres physical standbys operate while using logical replication for better performance. [`bdr.join_node_group`](/pgd/latest/reference/tables-views-functions/nodes-management-interfaces#bdrjoin_node_group) has the `pause_in_standby`
