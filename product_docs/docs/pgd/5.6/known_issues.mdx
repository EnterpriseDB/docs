...
- DDL run in a serializable transactions can face the error such as `ERROR: could not serialize access due to read/write dependencies among transactions`. A workaround to this issue is to run the DDL outside serializable transactions.
