...
- When using EDB Postgres Advanced Server 17 or later, the [`bfile`](/epas/latest/reference/sql_reference/02_data_types/03a_bfiles/) type isn't supported for replication. This is due to the data type being a reference to a file in the underlying file system, which isn't replicated by PGD.
