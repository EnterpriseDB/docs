...
The endpoints given in the config file are used only at startup. After that, actual endpoints are taken from the PGD catalog's `route_dsn` field in [`bdr.node_routing_config_summary`](/pgd/latest/reference/tables-views-functions/catalogs-internal#bdrnode_routing_config_summary).
