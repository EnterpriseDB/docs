...
|      | `--proxy-mode`                                                              | `default`<br/>\|`read-only`<br/>\|`any` | proxy mode. The proxy will route connections to - <br/>`default` - Write Leader<br/> `read-only` - Read Nodes<br/> `any` - both Write Leader and Read Nodes<br/> (default `default`) |
...
Attaching a new proxy called `proxy-a1` to group `g`roup_a`, with `default` mode.
