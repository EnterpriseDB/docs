...
title: events list
navTitle: list
description: Lists events such as background worker errors and node membership changes
deepToC: true
---
...
For more details on each node state, see 'pgd node list' command help
('pgd node list -h').
