...
This will create a subscriber-only group named `sogroup` and add the current node to it.

#### TPA

Using TPA, if you only need one subscriber, TPA allows you to add an SO node to the cluster and it will automatically create a single SO group, named `subscriber-only` for that node.

To do this, add a node to the TPA config.yml file with `bdr` and `subscriber_only` roles:
...
select bdr.join_node_group('subscriber_group', 'node_name');
