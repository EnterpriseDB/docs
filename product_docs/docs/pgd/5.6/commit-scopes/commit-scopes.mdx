...
CAMO, SYNCHRONOUS_COMMIT, and GROUP COMMIT each have rule degrade options.

### CAMO 

For [CAMO](pgd/latest/reference/commit-scopes/#camo), the options are simpler in that you can only degrade to asynchronous operation. 

```
ALL (left_dc) CAMO DEGRADE ON (timeout=500ms, require_write_lead=true) TO ASYNC
```

You can (and probably should) set the timeout (default `0`) to wait before switching to asynchronous operation. You can also require that the write lead be the originator of the transaction in order to switch to asynchronous mode (default is `false`).

### SYNCHRONOUS_COMMIT and GROUP COMMIT 

Both SYNCHRONOUS_COMMIT and GROUP COMMIT have the same `timeout` and `require_write_lead` parameters with the same defaults (`0` and `false` respectively).

In addition, you have options when it comes to what rule you can degrade to depending on which rule you are degrading from.

First of all, you can degrade to asynchronous operation in the way CAMO does:

```
ALL (left_dc) SYNCHRONOUS_COMMIT DEGRADE ON (timeout=20s) TO ASYNC
```

Furthermore, you can degrade to a less restrictive commit group with the same commit scope kind (again as long as the kind is either `SYNCHRONOUS_COMMIT` or `GROUP COMMIT`). For instance, you can degrade as follows:
...
To avoid this, the PGD manager process also periodically (every 5s) checks the connectivity and apply rate (the one in [bdr.node_replication_rates](/pgd/latest/reference/catalogs-visible/#bdrnode_replication_rates)) and if there are commit scopes that would degrade right now based on the current state of replication, they will be automatically degraded—such that any transaction processing uses the degraded rule from the beginning instead of waiting for timeout.
