...
You can also refer to the origin group of a transaction dynamically when creating a commit scope rule by using `ORIGIN_GROUP` syntax sugar. 
...
However, with `ORIGIN_GROUP`, just adding and using the following single-rule commit scope will have the same effect as the two individual rules we used above in `example_scope`:
...
Under this commit scope, when a transaction originates from `left_dc`, `ORIGIN_GROUP` maps to `left_dc` and `NOT ORIGIN_GROUP` maps to `right_dc`. Likewise, when a transaction originates from `right_dc`, `ORIGIN_GROUP` maps to `right_dc` and `NOT ORIGIN_GROUP` maps to `left_dc`. So by only specifying one rule, you get the effect of two.
