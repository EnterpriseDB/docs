...
You can also refer to the origin group of a transaction dynamically when creating a commit scope rule by using `ORIGIN_GROUP` syntax sugar. The `ORIGIN_GROUP` of a transaction refers to the local group from which the transaction originated. 
...
However, with `ORIGIN_GROUP`, just adding and using the following commit scope will have the same effect as the two individual rules we used above in `example_scope`:
