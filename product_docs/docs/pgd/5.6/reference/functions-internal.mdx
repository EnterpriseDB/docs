...
-   `force` &mdash; Circumvents all sanity checks and forces the removal of
      ...
      in case of emergencies related to parting.

#### Notes

This function removes metadata for a given node from the local database.

Before you run this function, part the node using `bdr.part_node()`. You can
reuse the node name after parting it and should not need to drop the node.

The node can be the local node, in which case all the node metadata is removed,
...
Unless you are performing a [disaster recovery
operation](/pgd/latest/backup/#edb-postgres-distributed-cluster-failure-or-seeding-a-new-cluster-from-a-backup),
you should not use `bdr.drop_node()` without direction from EDB technical
support.
