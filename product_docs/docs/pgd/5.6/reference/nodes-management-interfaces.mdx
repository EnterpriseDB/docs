...
| `default_commit_scope` | `text` |  All | The commit scope to use by default, initially the `local` commit scope. This option applies only to the top-level node group. You can use individual rules for different origin groups of the same commit scope. See [Origin groups](../commit-scopes/commit-scopes/#origin-groups) for more details. |
...
-   `synchronize_structure` &mdash; Specifies whether to perform database structure (schema)
