...
| `default_commit_scope` | `text` | The commit scope to use by default, initially the `local` commit scope. This option applies only to the top-level node group. You can use individual rules for different origin groups of the same commit scope. See [Origin groups](../commit-scopes/commit-scopes/#origin-groups) for more details. |
...
| `streaming_mode` | `text` | Enables/disables streaming of large transactions. See table [Streaming_mode values](#streaming_mode-values) for more details. Default is `default`. |

### `streaming_mode` values

| Value | Description |
| ----- | ----------- |
| `off` | Streaming is disabled. When set to any other value, large transactions are decoded while they're still in progress, and the changes are sent to the downstream. |
| `file` | Incoming changes of streaming transactions are stored in a file and applied only after the transaction is committed on upstream. |
| `writer` | Incoming changes are directly sent to one of the writers, if available. If parallel apply is disabled or no writer is free to handle streaming transactions, then the changes are written to a file and applied after the transaction is committed. |
| `auto` | PGD tries to intelligently pick between `file` and `writer`, depending on the transaction property and available resources. |
| `default` | Streaming matches what `default_streaming_mode` on the node is set to, unless the node is set to `auto`, when it always selects `writer`. |
...
| `node_group_type`   | The valid values are `NULL` or  `subscriber-only`. `NULL` (the default) is for creating a normal, general-purpose node group. `subscriber-only` is for creating [subscriber-only groups](../node_management/subscriber_only/) whose members receive changes only from the fully joined nodes in the cluster but that never send changes to other nodes. |
...
| `streaming_mode` | Enables/disables streaming of large transactions. See [streaming_mode values](#streaming_mode-values). Applies to top-level group only.|
| `default_commit_scope` | The commit scope to use by default, initially the `local` commit scope. This parameter applies only to the top-level node group. You can use individual rules for different origin groups of the same commit scope. See [Origin groups](../commit-scopes/commit-scopes/#origin-groups) for more details. |
