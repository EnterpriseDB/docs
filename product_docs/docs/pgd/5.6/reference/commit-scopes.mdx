...
Instead of targeting a specific group, you can also use `ORIGIN_GROUP` syntax sugar, which dynamically refers to the bottommost group from which a transaction originates. Therefore, if you have a top level group, `top_group`, and two subgroups as children, `left_dc` and `right_dc`, then adding a commit scope like:
