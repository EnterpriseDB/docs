...
originalFilePath: tpaexec-deploy.md
...
[EDB repositories](reference/edb_repositories/). If you
forget to do this, an error message will soon remind you.

## Quickstart

```shell
tpaexec deploy ~/clusters/speedy -v
```
...
```shell
tpaexec deploy ~/clusters/speedy -v -e deploy_hosts=keeper,quaver
```
