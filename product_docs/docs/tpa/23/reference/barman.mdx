...
| barman_path_prefix              | *backed up node's postgres bin dir* |
...
environment](air-gapped/) for the directory layout and the
[`tpaexec download-packages`
reference](tpaexec-download-packages/) for how to populate a
`local-repo`.
...
Refer to the section on [package version selection and upgrade](../tpaexec-upgrade/#package-version-selection)
