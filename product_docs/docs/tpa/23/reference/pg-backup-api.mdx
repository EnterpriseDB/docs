...
title: PG Backup API
originalFilePath: pg-backup-api.md
...
## Minor update for pg backup api using `tpaexec upgrade`

When trying to upgrade to a specific package version, ensure the `pg_backup_api_package_version` in
...
To select PG Backup API for upgrade, ensure the `--components` flag passed to the `tpaexec upgrade`
command contains `pg-backup-api` (or `all`)
