...
Any *additional* [parameter keywords supported by libpq](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-PARAMKEYWORDS) 
...
If pgd-proxy and/or pgd-cli are installed and [`bdr_client_dsn_attributes`](bdr/#bdr_client_dsn_attributes) includes parameters that are *unsupported* by the Go driver (such as timeout), two new variables must be included in the cluster configuration:
...
[commit scopes](https://www.enterprisedb.com/docs/pgd/latest/reference/tables-views-functions/commit-scopes/)
...
bdr-pre-group-join [hooks](../tpaexec-hooks/) during the PGD cluster
setup process.
