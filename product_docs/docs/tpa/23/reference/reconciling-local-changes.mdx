---
description: >-
  How to reconcile changes made outside of TPA with the configuration in
  `config.yml`.
title: Reconciling changes made outside of TPA
originalFilePath: reconciling-local-changes.md
...
other tools such as pg_upgrade or [pgd node upgrade](/pgd/latest/lifecycle/upgrades/inplace_upgrade/#pgd-node-upgrade-command-line).
...
1.  Made in the `conf.d/9999-override.conf` file reserved for manual
    edits;
1.  Made using `ALTER SYSTEM` SQL; or
1.  Made [natively in TPA](postgresql.conf/#postgres_conf_settings) by adding
...
When you have used `pg_upgrade`, [`pgd node upgrade`](/pgd/latest/lifecycle/upgrades/inplace_upgrade/#pgd-node-upgrade-command-line),
...
1.  Re-run `tpaexec provision` so the new versions are written into
    the inventory:
...
1.  Re-run `tpaexec deploy` so any version-conditional configuration
    is regenerated:
...
1.  Verify TPA's view of the cluster matches reality:
