...
title: Configuring Postgres Enterprise Manager (PEM)
originalFilePath: pem.md
...
-   [Configure pg_hba.conf](pg_hba_conf/)
-   [Configure postgresql.conf](postgresql_conf/)
...
4.  Update postgresql config on pem server node so it allows connections
    from the new pg-cluster. You can modify existing pg_hba_conf on pem
