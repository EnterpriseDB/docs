...
[BDR-Always-ON](architecture-BDR-Always-ON/) or
[PGD-X](architecture-PGD-X/).
...
repositories is available [here](reference/edb_repositories/) and on the page
...
a valid [EDB repository subscription](reference/edb_repositories/).
...
the EDB Postgres AI Console. See [Configuring the PGAI agent](reference/beacon-agent/) for details.
