...
[EDB Postgres Distributed](/pgd/latest) in an Always-ON configuration, intended for use in production.
...
connection routing management and will not configure any [PGD-Proxies](/pgd/latest/proxy/).
...
primary instances in each region to be each other's [CAMO](/pgd/latest/durability/camo/) partners.
