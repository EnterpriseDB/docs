...
<tr><td><details><summary>Use SLES 15 SP6 as the standard SLES for nodes</summary><hr/><p>When SLES is requested at configure-time, TPA will now install SLES 15 SP6. The docker and EC2 images are now SP6, and the systemd-sysvcompat package is installed on SLES, so that local boot-time scripts continue to work.</p></details></td><td></td></tr>
