...
| Type | Description |
| ---- |------------ |
| Enhancement | Added a new `--pgd-proxy-routing` option to the configure command. This can be set to `global` or `local`. Local routing will make every PGD-Proxy route to a write leader within its own location. Global routing will make every proxy route to a single write leader, elected amongst all available data nodes across all locations. |
| Change | Removed the `--active-locations` option from the configure command. |
