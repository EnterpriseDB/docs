...
requires credentials for EDB Repos 1.0. If you choose EDB Postgres Advanced
Server (EPAS), you also need credentials for the legacy
2ndQuadrant repos.
...
| Postgres flavor and version (e.g., `--postgresql 15`) | A valid [flavor and version specifier](tpaexec-configure/#postgres-flavor-and-version). |
...
| `--enable-efm`            | Configure Failover Manager as the cluster failover manager. | TPA selects EFM<br/>as the failover manager<br/>for EPAS and repmgr<br/>for all other flavors. |
