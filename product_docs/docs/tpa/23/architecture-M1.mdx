...
| One of: <br /> - `--failover-manager {efm, repmgr, patroni}`<br />- `--enable-efm`<br /> - `--enable-repmgr`<br />- `--enable-patroni` | Select the failover manager from [`efm`](reference/efm/), [`repmgr`](reference/repmgr/) and [`patroni`](reference/patroni/). |
