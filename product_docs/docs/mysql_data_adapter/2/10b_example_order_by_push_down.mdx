...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 Foreign Scan  (cost=15.00..25.00 rows=10 width=12)
   ...
   Relations: Aggregate on (suraj.sales_records)
   Local server startup cost: 10
   Remote query: SELECT count(`warehouse_id`), `warehouse_id` FROM `suraj`.`sales_records` WHERE ((`qty` > 80)) GROUP BY 2 ORDER BY `warehouse_id` IS NULL, `warehouse_id` ASC
(5 rows)
```