...
------------------------------------------------------------------------------------------------------------------------------------------------------------
 Foreign Scan on public.sales_records  (cost=1.00..2.00 rows=1 width=8)
   ...
   Remote query: SELECT `warehouse_id`, `qty` FROM `suraj`.`sales_records` WHERE ((`qty` > 80)) ORDER BY `warehouse_id` IS NULL, `warehouse_id` ASC LIMIT 5
(4 rows)
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 Foreign Scan on public.sales_records  (cost=1.00..2.00 rows=1 width=8)
   Output: warehouse_id, qty
   Local server startup cost: 10
   Remote query: SELECT `warehouse_id`, `qty` FROM `suraj`.`sales_records` WHERE ((`qty` > 80)) ORDER BY `warehouse_id` IS NULL, `warehouse_id` ASC LIMIT 5 OFFSET 5
(4 rows)
```
