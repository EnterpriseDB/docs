...
MySQL 2.9.0 and later provides the `mysql_fdw_pushdown.config` configuration file to restrict the pushdowns. You can define the list of functions and operators in this file that can pushdown to the remote server. This configuration file is read and stored in the in-memory hash table to check whether pushdown is possible. You can easily add or modify the list as per the requirements.
