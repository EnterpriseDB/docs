...
MySQL Foreign Data Wrapper 2.9.2 and later provides the `MYSQL_DEFAULT_FILE` option to read the connection details from the default file. You can provide the filename using this option with connection details like username, password, etc. Set the `MYSQL_DEFAULT_FILE` option for a foreign server to the default connection file name. If this option is set to a wrong filename or the file contents are not in the required format, then an error is raised.
