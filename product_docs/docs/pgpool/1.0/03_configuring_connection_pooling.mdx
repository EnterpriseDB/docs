...
> **Note:** The above command is applicable for RHEL/CentOS 7 platforms. To restart the server on RHEL/CentOS 6 platform, use the below command:
> 
...
> **Note:** When authenticating with the database server, use the user names and passwords specified in the `pool_hba.conf` file; you must also specify those user names and passwords in the database server's `pg_hba.conf` file.
