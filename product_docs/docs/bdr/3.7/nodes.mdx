...
1. Create the BDR node for the witness and join it to the BDR cluster.

1. Change the witness node replication sets to subscribe only to the `witness` replication set which was created in step 1:
   ...
   This BDR node is now only subscribed to the `witness` replication set, which replicates nothing.

1. If you are using TPAexec to deploy your BDR clusters, you can assign the roles `bdr` and `witness` in your `config.yml` file to get a BDR witness. For example:
...
1. Run the TPAexec `provision` and `deploy` commands to apply the changes.
