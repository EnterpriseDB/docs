...
* Eager replication provides conflict free replication by synchronizing across cluster nodes before committing a transaction *\[extended]* 
* Commit at most once consistency guards application transactions even in the presence of node failures *\[extended]*
...
* Single decoding worker improves performance on upstream node by doing logical decoding of WAL once instead of for each downstream node *\[extended]*
...
* Tooling to assess applications for distributed database suitability *\[extended]* 
...
!!! Important *\[extended]* Indicates feature is only available with EDB Postgres Extended at this time.
!!!
