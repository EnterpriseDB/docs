...
`bdr.min_worker_backoff_delay` milliseconds. Time-unit suffixes are supported.
...
The default is 1s. The delay is a time limit applied from launch-to-launch,
so the default value of `1s` limits of workers to at most once (re)launches
per second.
