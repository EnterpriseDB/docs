...
-   `bdr.enable_wal_decoder` - when turned `false` it stops WAL decoder, if
...
-   `enable_wal_decoder` - Enables/disables the WAL decoder process. Note that
      the WAL decoder process cannot be enabled if `streaming_mode` is already
      enabled.
