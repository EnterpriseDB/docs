...
Aee [Replication sets](repsets) for further details and examples.
...
running in SERIALIZABLE mode isno' replicated to other nodes. The
...
up to 1GB in size. Use of the PostgreSQL "large object" facility isn't
supported in BDR.
...
replicate tables other than base tables results in an error.
...
changes that involve any of the partitions is replicated downstream.
...
To ensure this doesn't occur, resolve object references clearly using
...
See the [release notes](release-notes) for any known incompatibilities.
...
Further details are available in [Replication sets](repsets).
...
This setting doesn't throw an error, just silently ignores any additional
columns.
...
create, then drop, and then later reuse the same object names.
...
Now, add a `.spec` file containing tests in the `specs/isolation` directory
...
isolationtester recognizes that a command has blocked by looking
...
to develop performance tests using the benchmarking tools. pgbench
...
can easily be modified to become BDR compliant. Users can undertake an
...
These aid in the process of deciding suitability of their application in
