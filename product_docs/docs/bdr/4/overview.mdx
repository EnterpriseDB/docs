...
The minimum recommended number of nodes in a BDR cluster is three because with two nodes the consensus stops working if one of the node stops working. For more information about the consensus mechanism used by EDB Postgres Distributed, see [Architectural details](/pgd/latest/architectures/#architecture-details).
