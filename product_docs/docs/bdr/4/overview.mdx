...
The minimum recommended number of nodes in a BDR cluster is three to ensure the consensus mechanism works properly. Consensus is a challenge in fault-tolerant distributed systems. Consensus involves multiple nodes in a cluster agreeing on values. BDR uses a [Raft](https://raft.github.io/)-based consensus mechanism. The mechanism makes progress towards a decision as long as a majority of nodes is available. For example, if you only have two nodes in your cluster, the consensus mechanism stops working if one of the nodes stops working. 
