...
recommended to upgrade to a newer PostgreSQL release.
...
!!! Warning
    At this point in time, both of these two methods still have their
    issues and need to be considered beta.  We do not recommend to
    upgrade from BDR 3.7 to BDR 4.0 just yet.  This restriction will
    be lifted in future versions of BDR 4.
...
An upgrade to 4.0 is only supported from 3.7 in beta testing mode,
using a specific minimum
maintenance release (e.g. 3.7.15). Please consult the Release Notes
...
While this chapter primarily discusses upgrades of BDR itself, it's also
possible to do major version upgrades of Postgres this way. This
can be done using the `bdr_pg_upgrade` tool which is described further
in [In-place Postgres Major Version Upgrades](upgrades) chapter.
