...
You can create a witness node by following these steps:

- 1) Create a replication set that replicates nothing. For example:
...
Note: the `witness` name for the replication set is only a suggestion.

- 2) Create the BDR node for the witness and join it to the BDR cluster

- 3) Change the witness node replication sets to subscribe only to `witness`
replication set which was created in step `1)`:

```sql
SELECT bdr.alter_node_replication_sets(
  node_name := '<YOUR_WITNESS_NODE_NAME>',
  set_names := ARRAY['witness']::TEXT[]
);
```

At this point this BDR node will only be subscribed to the `witness` replication
set, which replicates nothing.

If you are using TPAexec to deploy your BDR clusters, you can assign the roles
`bdr` and `witness` in your `config.yml` to get a BDR witness. For example:

```yaml
...
- Name: bdr-witness-node
  node: 3
  role:
  - bdr
  - witness
...
Then you need to run TPAexec `provision` and `deploy` commands to apply the
changes.
...
that act similarly to DDL also aren't replicated. See [DDL replication](#ddl-replication).
...
-   Inform the BDR cluster of the change in address by executing the
...
well. It's important to set it in a BDR cluster where there is a
...
!!! Warning
    ...
    still vital to the proper operation of the BDR cluster. If you drop it,
