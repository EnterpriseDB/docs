...
Shows all nodes in the BDR cluster and their summary like name, node id, group,
current/target state etc.
...
Note that the read-only state of a node, as shown in the 'Current State' or
in the 'Target State' columns, is indicated as 'STANDBY'.
