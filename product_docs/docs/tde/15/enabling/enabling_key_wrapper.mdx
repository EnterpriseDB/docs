...
1.  Store the passphrase in a file accessible by initdb named `pass.bin`:
    ...
    !!!important 
       This example stores the passphrase in plaintext, a method you should only use for testing or demo purposes. In production environments, don't store you passphrase. Instead, use a method where OpenSSL propmts the user to enter the passphrase as explained in [Using a passphrase](../secure_key/passphrase).
    !!!
