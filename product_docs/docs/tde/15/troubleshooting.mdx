...
To reset a corrupt encrypted WAL file, you must ensure the [pg_resetwal](https://www.postgresql.org/docs/15/app-pgresetwal.html) command can access the unwrap key and decrypt it. You can either pass the key for the unwrap command using the following option to the `pg_resetwal` command or depend on the fallback environment variable:
