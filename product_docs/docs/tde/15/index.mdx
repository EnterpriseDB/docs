...
EDB TDE uses [OpenSSL](https://openssl-library.org/) to encrypt data files with the AES encryption algorithm. By default, it uses the OpenSSL libraries available on the operating system machine that initializes the database cluster.
