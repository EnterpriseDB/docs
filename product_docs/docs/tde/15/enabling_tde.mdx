...
`-y, --data-encryption`
  ...
  Initialize the new database cluster with transparent data encryption. See [Transparent Data Encryption](/tde/latest) for more information.
...
Is this the environment variable fallback? I changed the word "option" to parameter. -->
...
## Checking for TDE in use using SQL

The presence of TDE on a server can be determined by querying the `data_encryption_version` column of the `pg_control_init` table. 

A value of 0 means TDE is not enabled. Any non-zero value reflects the version of TDE in use. Currently, when TDE is enabled, this value will be 1.
