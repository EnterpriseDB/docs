...
1. Initialize the cluster using initdb with encyrption enabled:
   ```shell
   /usr/edb/as15/bin//initdb --data-encryption -D /var/lib/edb/as15/data -y
   ```
1. Start the cluster:
   ```shell
