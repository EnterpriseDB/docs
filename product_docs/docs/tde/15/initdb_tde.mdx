...
Use the following TDE options when initializing a cluster with `initdb`.

## Enabling TDE

To create a TDE-enabled database server, you must use the `--data-encryption` flag (short form: `-y`).

## Add a mechanism to protect the data encryption key 

When creating a TDE-enabled database, TDE generates a data encryption key that is transparent to the user. An additional protection mechanism is required to wrap this key. 

To provide this mechanism, you must select a way to [secure the data encryption key](./secure_key/). You have three options:

- Provide the protection mechanism directly in the command line when initializing a server with the `--key-wrap-command=<command>` and `--key-unwrap-command=<command>` options.

- Set environment variables before creating the database, so TDE can fall back on those variable when initializing a server.

- Disable the protection of your data encryption key with `--no-key-wrap`. This leaves your key unprotected and is recommended for testing proposes only.
