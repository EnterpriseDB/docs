...
```shell
# TBD, maybe we'll need templating for this depeneding on the OS we need to support
dnf/zypper/apt-get 
```
...
1.  Create a CSR. Ensure you select the RSA option, `2048` size and then select **Create**: 
	...
	In the same pop-up window, after the certificate has been created, select **save csr** and **save private key** to download a `CSR.pem` and `key.pem` file.
