...
1.  Create a CSR. Ensure you select the RSA algorithm and set the size to 2048. Select **Create**. 
...
1.  Update your user configuration to allow certificate-based login. Under **Users**, select **Access Management and select your user.
...
1.  To create they key, execute `edb_tde_createkey.py`. To execute it correctly, you need:
    ...
    - The uid of the user that created the certificate. You can find the user uid in the Thales CipherTrust Manager. Under **Access Management**, select **Users**, and then select your user. The user uid information is a sequence of digits and letters, for example, `local|285xxxfb-xxxb-4xxx-9339-2d58xxx60ae6`.
...
1.  You can now [initialize a TDE-enabled database cluster](/tde/latest/initdb_tde_options/#option---data-encryption-or--y) with `initdb --data-encryption`. TDE falls back on the exported `PGDATAKEYWRAPCMD` and `PGDATAKEYUNWRAPCMD` commands for the wrapping and unwrapping of data encryption keys. 
