...
You can configure TDE to use an external key from Thales CipherTrust Manager to wrap the data encryption key with a key from the Thales key store. You can either use `python3` and `pykmip`, or Thales native CLI tool `ksctl` for the integration. 

-  To use `python3` and `pykmip`, see [Using](/partner_docs/ThalesCipherTrustManager/05-UsingThalesCipherTrustManager/) in the [Implementing Thales CipherTrust Manager](/partner_docs/ThalesCipherTrustManager/) documentation for instructions.

-  To use `ksctl`, see [Thales key store with ksctl](/tde/latest/secure_key/key_store/thales_ksctl).
