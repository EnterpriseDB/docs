...
!!!warning Passphrase TDE is for development use only
Passphrase mode is not appropriate for production environments.

A passphrase stored alongside the database provides encryption but not assurance: the key custodian and the data custodian are the same principal. Auditors do not accept this model for PCI, HIPAA, or FedRAMP workloads.

The passphrase *is* the key. Losing it makes data unrecoverable. Leaking it voids the encryption. There is no recovery path and no reset. Do not automate passphrase handling — doing so defeats the security model.
