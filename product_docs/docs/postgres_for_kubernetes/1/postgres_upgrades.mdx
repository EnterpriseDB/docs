...
### Upgrading a Minor Version in EDB Postgres for Kubernetes
...
EDB Postgres for Kubernetes will trigger a [rolling update of the cluster](rolling_update.md),
...
EDB Postgres for Kubernetes supports three methods for performing major upgrades:
...
EDB Postgres for Kubernetes performs an **offline in-place major upgrade** when a new operand
...
!!! Warning
    EDB Postgres for Kubernetes is not responsible for PostgreSQL extensions. You must ensure
...
If the upgrade is successful, EDB Postgres for Kubernetes:
...
cluster's configuration and delete the upgrade job, as EDB Postgres for Kubernetes cannot
...
spec:
  imageName: quay.io/enterprisedb/postgresql:16-minimal-bookworm
  instances: 3
  storage:
    size: 1Gi
```
...
metadata:
  name: cluster-example
spec:
  imageName: quay.io/enterprisedb/postgresql:17-minimal-bookworm
  instances: 3
  storage:
    size: 1Gi
```
