...
originalFilePath: 'src/postgres_upgrades.md'
---
...
### Upgrading a Minor Version in {{name.ln}}
...
{{name.ln}} will trigger a [rolling update of the cluster](rolling_update.md),
...
{{name.ln}} supports three methods for performing major upgrades:
...
{{name.ln}} performs an **offline in-place major upgrade** when a new operand
...
!!!warning
{{name.ln}} is not responsible for PostgreSQL extensions. You must ensure
...
If the upgrade is successful, {{name.ln}}:
...
cluster's configuration and delete the upgrade job, as {{name.ln}} cannot
...
```yaml
spec:
  imageName: docker.enterprisedb.com/k8s_enterprise/postgresql:16-minimal-ubi9
  plugins:
...
```yaml
spec:
  imageName: docker.enterprisedb.com/k8s_enterprise/postgresql:17-minimal-ubi9
  plugins:
    - name: plugin-barman-cloud
      enabled: true
      parameters:
...
spec:
  imageName: docker.enterprisedb.com/k8s/postgresql:16-minimal-ubi9
  instances: 3
  storage:
    size: 1Gi
```
...
```sh
kubectl cnp psql cluster-example -- -qAt -c 'SELECT version()'
```
...
metadata:
  name: cluster-example
spec:
  imageName: docker.enterprisedb.com/k8s/postgresql:17-minimal-ubi9
  instances: 3
  storage:
    size: 1Gi
```
...
```sh
kubectl cnp psql cluster-example -- app -c 'ANALYZE'
```
