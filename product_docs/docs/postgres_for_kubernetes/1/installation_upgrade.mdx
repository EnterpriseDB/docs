...
Before installing EDB Postgres for Kubernetes, you need to create a pull secret for EDB software in your namespace.

You will require a Repos 2.0 token to install. You can obtain a token by visiting the [EDB website](https://www.enterprisedb.com/repos) and signing into your account.
...
There are two operator manifests available:

- Standard: The [latest operator manifest](https://get.enterprisedb.io/pg4k/pg4k-standard-1.24.1.yaml )
    for the current standard release.
- Enterprise: The [latest operator manifest](https://get.enterprisedb.io/pg4k/pg4k-enterprise-1.24.1.yaml)
    for the current enterprise release.

You can apply your manifest with:
