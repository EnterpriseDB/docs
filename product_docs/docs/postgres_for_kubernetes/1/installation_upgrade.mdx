...
You can obtain the token by visiting your [EDB Repos account page](https://www.enterprisedb.com/accounts/progile) and signing into your account.
Your account profile page displays the token to use - by default it is obscured, click the "Show" button (an eye icon) to reveal it.

Your token entitles you to access one of two repositories: standard or enterprise. 

* Standard - Includes the operator and standard operands, including EDB Postgres Extended.
* Enterprise - Includes the operator and standard and enterprise operands, including EDB Postgres Advanced and EDB Postgres Extended.

Export the appropriate value as an environment variable EDB_SUBSCRIPTION_PLAN.

```shell
EDB_SUBSCRIPTION_PLAN=enterprise
```

then export the Repos 2.0 token to an environment variable EDB_SUBSCRIPTION_TOKEN.
...
Before installing EDB Postgres for Kubernetes, you need to create a pull secret for EDB private registries in the `postgresql-operator-system` namespace.


The pull secret needs to be saved in the namespace where the operator will reside. Create a postgresql-operator-system namespace using this command:
...
through a YAML manifest applied via `kubectl`. There are two different manifests available depending on your subscription plan:
