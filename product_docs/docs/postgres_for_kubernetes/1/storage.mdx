...
Before you deploy a PostgreSQL cluster with {{name.ln}},
...
{{name.ln}} doesn't use `StatefulSet` for managing data persistence.
...
Since {{name.ln}} supports volume snapshots for both backup and recovery,
...
## Benchmarking {{name.ln}}

Before deploying the database in production, we recommend that you benchmark
{{name.ln}} in a controlled Kubernetes environment. Follow the guidelines in
...
Encryption at rest is possible with {{name.ln}}. The operator delegates that
...
In {{name.ln}}, the volumes attached to a single PostgreSQL instance are
...
{{name.ln}} was designed to work interchangeably with all storage classes.
...
{{name.ln}} supports declarative tablespaces. You can add one or more
...
Kubernetes does not provide an API to shrink a PVC, and {{name.ln}}'
...
{{name.ln}} does not support automated volume shrinking, as it is a
...
1.  Disable the validating webhook by setting the `k8s.enterprisedb.io/validation: disabled`
...
2.  Re-enable validation by removing the `k8s.enterprisedb.io/validation` annotation (or
...
{{name.ln}} was designed to work with dynamic volume provisioning. This
...
breaks the fully declarative model on which {{name.ln}} was built.
!!!

To use a pre-provisioned volume in {{name.ln}}:
...
3.  Create the Postgres `Cluster` using, for each storage section, a coherent
    ...
    and enable {{name.ln}} to create the needed `PersistentVolumeClaim`.
...
However, {{name.ln}} integrates this resiliency directly into the Postgres
...
For {{name.ln}} usage, consider reducing the number of replicas at the block storage
...
replica. If a host becomes corrupted, you can use the [`cnp` plugin to destroy](kubectl-plugin.md#destroy)
the affected instance. {{name.ln}} will then recreate the instance on another
