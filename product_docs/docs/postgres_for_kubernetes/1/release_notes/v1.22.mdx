---
title: 'Release notes for EDB Postgres for Kubernetes 1.22'
originalFilePath: 'src/release_notes/v1.22.md'
---

History of user-visible changes in the 1.22 minor release of EDB Postgres for Kubernetes.

For a complete list of changes, please refer to the
[commits](https://github.com/EnterpriseDB/cloud-native-postgres/commits/release-1.22)
...
-   Plugin:
    -   Add the `logs pretty` command in the `cnp` plugin to read a log stream
...
-   `cnp` plugin for `kubectl`:
    ...
    -   Correctly handle the percentage of the backup progress in `cnp status`
        (#4131)
...
-   Add the `k8s.enterprisedb.io/skipWalArchiving` annotation to disable WAL archiving when
    set to `enabled` (#4055)
-   Enrich the `cnp` plugin for `kubectl` with the `publication` and
...
-   Introduce initial support for tab-completion with the `cnp` plugin for
    `kubectl` (#3875)
...
-   Enhancement of the `cnp status` plugin command, providing information about
...
-   Implicit setting of online backup with the `cnp backup` plugin command when
