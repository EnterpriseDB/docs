...
The first public release of EDB Postgres for Kubernetes is version 1.15.0. Before that,
...
**None of the versions listed here exists for EDB Postgres for Kubernetes.**
...
- Add the `switchoverDelay` option to control the time given to the former
  ...
  promoting the new primary (by default, EDB Postgres for Kubernetes waits
...
- **IMPORTANT:** If you have previously deployed the EDB Postgres for Kubernetes
  ...
  the EDB Postgres for Kubernetes documentation for additional detail on upgrading
  to 1.4.0
...
The first major stable release of EDB Postgres for Kubernetes implements `Cluster`,
