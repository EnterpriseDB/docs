...
-   Kubernetes API integration for high availability
    ...
    -   EDB CloudNativePG Cluster uses the `postgresql.k8s.enterprisedb.io/v1` API version
...
## Features unique to EDB CloudNativePG Cluster
...
You can [evaluate EDB CloudNativePG Cluster for free](evaluation.md) as part of a trial subscription.
You need a valid EDB subscription to use EDB CloudNativePG Cluster in production.

!!! Note

    Based on the [Operator Capability Levels model](operator_capability_levels.md),
    users can expect a **"Level V - Auto Pilot"** set of capabilities from the
    EDB CloudNativePG Cluster Operator.

### Long Term Support

EDB is committed to declaring a Long Term Support (LTS) version of EDB
CloudNativePG Cluster annually. 1.25 is the current LTS version. 1.18 was the
...
Because it was declared an LTS version of EDB CloudNativePG Cluster, it
...
CloudNativePG Cluster works with both PostgreSQL, EDB Postgres Extended and EDB Postgres
...
For a list of the minor releases of EDB CloudNativePG Cluster that are
...
The EDB CloudNativePG Cluster Operator container images support the multi-arch
format for the following platforms: `linux/amd64`, `linux/arm64`, `linux/ppc64le`, `linux/s390x`.

!!! Warning

    EDB CloudNativePG Cluster requires that all nodes in a Kubernetes cluster have the
...
Follow the instructions in the ["Quickstart"](quickstart.md) to test EDB CloudNativePG Cluster
