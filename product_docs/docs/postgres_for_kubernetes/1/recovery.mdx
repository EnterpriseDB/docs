...
In EDB Postgres for Kubernetes, recovery is **not performed in-place** on an existing
...
EDB Postgres for Kubernetes supports recovery via:

-   A **pluggable backup and recovery interface (CNP-I)**, enabling integration
...
!!! Important
    The object store must contain backup data produced by a EDB Postgres for Kubernetes
...
EDB Postgres for Kubernetes allows you to create a new cluster from a `VolumeSnapshot` of a
...
This setup enables EDB Postgres for Kubernetes to restore the base data from a volume
...
!!! Warning
    EDB Postgres for Kubernetes includes a safety check to prevent a cluster from overwriting
...
!!! Important
    ...
    `k8s.enterprisedb.io/skipEmptyWalArchiveCheck` annotation to `enabled` on the recovered
