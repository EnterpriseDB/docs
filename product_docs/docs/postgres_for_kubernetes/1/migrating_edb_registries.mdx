...
navTitle: Registry migration guide
description: Instructions for migrating a {{name.ln}} system from a previous tier-based registry configuration to EDB's unified container image registry
---

## What's changing

Previously, EDB published operator and operand images selectively to one of several private repositories: `k8s_standard`, `k8s_enterprise`, `k8s_standard_pgd`, `k8s_enterprise_pgd`. Your repository was determined by your subscription level *and* which operator(s) you wished to use. Additionally, public registries may have
been used for some operand images, including those provided by EDB and by the CloudNativePG project. 

Now, all operators and operands are provided via a single repository, `k8s`. If your subscription includes access to a given image, you will be able to pull it via this repository. The old repositories will remain available for the time being, but new images will be published to the unified repository. Thus, future operator *and* operand upgrades will require migrating to the unified repository.
...
| docker.enterprsedb.com/anything_else       | docker.enterprisedb.com/k8s |
| quay.io/enterprisedb                       | docker.enterprisedb.com/k8s |

## Scope of changes

- **Pull secrets** Because the old system relied on specifying the desired repository via the username when authenticating, you'll need to update the credentials storied in secrets that Kubernetes uses to pull images. 
- **Helm values** If you're using Helm charts, you'll need to update the `image.repository` and `image.imageCredentials.username` values. 
- **Manifest image names** If you're using custom manifests that specify one of the old repository paths as part of an `imageName` or `images[].image` value, you'll need to update those as well. 
...
If you're working directly with operator manifests, you'll need to update pull secrets in the relevant namespaces. 

For {{name.ln}}, the default namespace is `postgresql-operator-system`. To create or update the pull secret in this namespace, you would run:
...
!!! Tip 
    These examples overwrite existing secrets; if you have additional credentials stored (for example, to authenticate with internal repositories), you'll want to instead unpack the value and update only the credentials for `.auths["docker.enterprisedb.com"]`, leaving other servers untouched.
...
```yaml 
  imageName: docker.enterprisedb.com/k8s/edb-postgres-extended:16
```
...
spec:
  images:
    - major: 15
      image: docker.enterprisedb.com/k8s/postgresql:15.14-system-trixie
    - major: 16
      image: docker.enterprisedb.com/k8s/postgresql:16.10-system-trixie
    - major: 17
      image: docker.enterprisedb.com/k8s/postgresql:17.6-system-trixie
    - major: 18
      image: docker.enterprisedb.com/k8s/postgresql:18.0-system-trixie
```
