...
originalFilePath: 'src/resource_management.md'
---
...
{{name.ln}} allows administrators to control and manage resource usage by the pods of the cluster,
...
!!!info
When the quality of service is set to "Guaranteed", {{name.ln}} sets the
...
-   Set up database server pods on a dedicated node using nodeSelector.
    ...
    [“affinityconfiguration"](pg4k.v1.md#affinityconfiguration) resource on the API reference page.
...
`Cluster` through a normal manifest update; {{name.ln}} performs the rolling
...
{{name.ln}}-managed `Cluster`. The operator owns the pod specification and
...
HPA is also unaware of {{name.ln}}' own constraints on `spec.instances`. If
