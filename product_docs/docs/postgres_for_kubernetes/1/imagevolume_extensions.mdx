...
feature to which the {{name.ln}} project contributed.
...
The {{name.ln}} Community maintains a suite of extension container
images, including
...
[API reference for `ExtensionConfiguration`](pg4k.v1.md#extensionconfiguration).
!!!
...
limited to 59 characters, leaving room for the prefix {{name.ln}} adds when
...
Following the *"convention over configuration"* paradigm, {{name.ln}}
...
By default, {{name.ln}} automatically manages the relevant GUCs by setting:
...
official {{name.ln}} catalogs pre-configure these options to the correct
...
{{name.ln}} automatically reconciles this resource by executing
...
As anticipated earlier, {{name.ln}} offers two ways to define extension
...
like the [official ones](image_catalog.md#edb-cloudnativepg-cluster-catalogs)
...
the catalog.
While every field in an [`ExtensionConfiguration`](pg4k.v1.md#extensionconfiguration)
...
definitions, {{name.ln}} provides a "resolved" view of the configuration in
...
This triggers {{name.ln}} to execute `DROP EXTENSION` within the database.
...
effect.
{{name.ln}} does not currently trigger this restart automatically; you will
need to manually restart the cluster (e.g., using `cnp restart`) after
...
{{name.ln}} will append `/extensions/my-extension/bin` to the `PATH` environment
...
{{name.ln}} emits a warning in the instance manager logs when an extension
...
manually restart the cluster (e.g., using `kubectl cnp restart`) for the new
...
which serves as the reference for building official {{name.ln}} extension images.
