...
By default, EDB Postgres for Kubernetes automatically manages the relevant GUCs, setting:
...
Adding an extension to a database in EDB Postgres for Kubernetes involves a few steps:
...
!!! Warning
    ...
    This limitation will be addressed in a future release of EDB Postgres for Kubernetes.
...
!!! Important
    When a new extension is added to a running `Cluster`, EDB Postgres for Kubernetes will
...
Once mounted, EDB Postgres for Kubernetes will automatically configure PostgreSQL by appending:
...
EDB Postgres for Kubernetes will automatically reconcile this resource, executing the
...
Following the *"convention over configuration"* paradigm, EDB Postgres for Kubernetes allows
...
EDB Postgres for Kubernetes will configure PostgreSQL with:
...
EDB Postgres for Kubernetes will set the `LD_LIBRARY_PATH` environment variable to include
...
!!! Important
    ...
    EDB Postgres for Kubernetes does not currently trigger this restart automatically; you will need to
    manually restart the cluster (e.g., using `cnp restart`) after modifying `ld_library_path`.

## Image Specifications

A standard extension container image for EDB Postgres for Kubernetes includes two
...
discoverable and usable by PostgreSQL within EDB Postgres for Kubernetes without requiring
