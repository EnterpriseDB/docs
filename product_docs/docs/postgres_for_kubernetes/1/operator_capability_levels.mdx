...
EDB Postgres for Kubernetes manages replication slots for all replicas in the
...
EDB Postgres for Kubernetes supports declarative offline in-place major upgrades when a new
...
EDB Postgres for Kubernetes provides a pluggable interface (CNP-I) for managing
...
The EDB Postgres for Kubernetes Community officially supports the [Barman Cloud Plugin](https://cloudnative-pg.io/plugin-barman-cloud/),
which enables continuous physical backups to object stores, along with full and
Point-In-Time Recovery (PITR) capabilities.

In addition to CNP-I plugins, EDB Postgres for Kubernetes also natively supports backups
...
EDB Postgres for Kubernetes configures startup, liveness, and readiness probes for PostgreSQL
...
EDB Postgres for Kubernetes supports PostgreSQL's logical replication in a declarative manner
