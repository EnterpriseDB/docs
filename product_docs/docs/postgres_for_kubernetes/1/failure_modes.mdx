...
!!! Note
    In previous versions of EDB Postgres for Kubernetes, this page included specific failure
    ...
    underlying Kubernetes stack and is not specific to EDB Postgres for Kubernetes.

EDB Postgres for Kubernetes adheres to standard Kubernetes principles for self-healing and
high availability. We assume familiarity with core Kubernetes concepts such as
storage classes, PVCs, nodes, and Pods. For EDB Postgres for Kubernetes-specific details,
...
!!! Important
    If you are running EDB Postgres for Kubernetes in production, we strongly recommend
