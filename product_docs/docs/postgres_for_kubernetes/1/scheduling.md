...
!!! Info
    EDB Postgres for Kubernetes does not support pod templates for finer control
...
EDB Postgres for Kubernetes by default will configure the cluster's instances
...
Similarly, EDB Postgres for Kubernetes consents you to define a `nodeSelector` in the
