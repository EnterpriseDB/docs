...
## Deploying clusters with EPAS operands using their Iron Bank image

To deploy a cluster using the EPAS [operand](/postgres_for_kubernetes/latest/private_edb_registries/#operand-images) from a pulled image, you must reference the Ironbank operand image appropriately in a Custom Resource (CR) file. 
For example, to deploy a PG4K cluster using a EPAS 16.4 operand:
