...
EDB Postgres for Kubernetes can be installed and managed via:
...
!!! Warning
    ...
    have in the namespace in order to interact with EDB Postgres for Kubernetes's managed
...
!!! Warning
    ...
    interact with EDB Postgres for Kubernetes's managed custom resources - primarily the `Cluster`
...
proper `RoleBinding` objects, as it enables mapping EDB Postgres for Kubernetes's
...
!!! Seealso "OpenShift CLI"
    For more detailed and updated information, refer to the official
...
For [every CRD owned by EDB Postgres for Kubernetes's CSV](#custom-resource-definitions-crd),
...
!!! Seealso "There's more ..."
    ...
    OperatorGroup, refer to the
    [OperatorGroup: RBAC](https://olm.operatorframework.io/docs/concepts/crds/operatorgroup/#rbac) in the Operator Lifecycle Manager documentation.
...
!!! Important
    ...
    ["Cluster roles on EDB Postgres for Kubernetes CRDs"](#cluster-roles-on-cloud-native-postgresql-crds)).
