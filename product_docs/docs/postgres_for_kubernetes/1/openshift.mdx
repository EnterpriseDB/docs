...
-   `fast`: designed for users who want timely access to the latest stable
    ...
    latest **patch release** of the latest **minor release** of {{name.ln}}.
...
1.  Scroll in the `Database` section or type a keyword into the `Filter by keyword`
...
1.  Read the information about the Operator and select `Install`.

1.  The following `Operator installation` page expects you to choose:
...
![Menu Software Catalog](./images/openshift/software-catalog.png)
!!!
...
This doesn't mean that every user in the OpenShift cluster can use the {{name.ln}} Operator, deploy a `Cluster` object or even see the `Cluster` objects that
...
This doesn't mean that every user in the namespace can use the {{name.ln}} Operator, deploy a `Cluster` object or even see the `Cluster` objects that
...
1.  Inspect the operator to verify the installation modes (`MultiNamespace` in
...
1.  Create an `OperatorGroup` object in the `my-operators` namespace so that it
...
1.  Create a `Subscription` object in the `my-operators` namespace to subscribe
...
1.  Use `oc apply -f` with the above YAML file definitions for the
...
["Cluster roles on {{name.ln}} CRDs"](#cluster-roles-on-edb-postgres-for-kubernetes-crds)).
!!!
