...
!!! Important
    ...
    Please refer to ["Accessing EDB private image registries"](private_edb_registries) for further details.
...
choose?"](private_edb_registries#which-repository-to-choose)
- `@@TOKEN@@` is the repository token for your EDB account, as explained in
["How to retrieve the token"](private_edb_registries#how-to-retrieve-the-token)
