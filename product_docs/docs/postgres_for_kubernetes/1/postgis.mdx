...
Since {{name.ln}} is based on Immutable Application Containers, the only way
...
spec:
  instances: 1
  imageName: docker.enterprisedb.com/k8s/postgresql:17-postgis-ubi9
  storage:
    size: 1Gi
  postgresql:
    parameters:
...
from the [API reference](pg4k.v1.md#bootstrapinitdb), such as
