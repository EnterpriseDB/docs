...
EDB Postgres for Kubernetes implements its own custom controller to manage PostgreSQL
...
this design choice has made the implementation of EDB Postgres for Kubernetes
...
In contrast, EDB Postgres for Kubernetes leverages the configured storage class to
...
[treated specially by PostgreSQL because EDB Postgres for Kubernetes uses hot standby
replicas](https://www.postgresql.org/docs/current/hot-standby.html)).

While doing that, EDB Postgres for Kubernetes considers the PostgreSQL instance's
...
lower `max_connections`. In that case, EDB Postgres for Kubernetes will:
...
EDB Postgres for Kubernetes would instead classify the remaining PVC as unusable, and
...
The EDB Postgres for Kubernetes controller implements all these strategies so that the
