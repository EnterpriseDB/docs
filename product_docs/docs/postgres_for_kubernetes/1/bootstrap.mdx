...
-   `pg_basebackup`: create a PostgreSQL cluster by cloning an existing one of
    ...
    databases to EDB Postgres for Kubernetes, although meeting all requirements can be
...
storage that the EDB Postgres for Kubernetes operator provides for `recovery`, please refer to
...
This approach enables EDB Postgres for Kubernetes to securely establish connections with an
...
EDB Postgres for Kubernetes enables bootstrapping a cluster starting from
...
options provided by the EDB Postgres for Kubernetes operator for `recovery`, please refer
