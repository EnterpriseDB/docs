...
specification.
EDB Postgres for Kubernetes currently supports the following bootstrap methods:
...
There are two ways to achieve this result in EDB Postgres for Kubernetes:
...
spec:
  instances: 3
  imageName: quay.io/enterprisedb/postgresql:15.1
...
Future versions of EDB Postgres for Kubernetes will enable users to control
...
-   replication over different Kubernetes clusters in EDB Postgres for Kubernetes
-   *0 cutover time* migrations to EDB Postgres for Kubernetes with the `pg_basebackup`
    bootstrap method