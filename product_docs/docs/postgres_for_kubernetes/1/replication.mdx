...
PostgreSQL, we have decided to build high availability in {{name.ln}} on top
...
transaction level, supporting [RPO](before_you_start.md#postgresql-terminology)=0 clusters. Cascading
...
At the moment, {{name.ln}} natively and transparently manages physical
...
!!!note Certificates
For details on how {{name.ln}} manages certificates, please refer
...
For details on how {{name.ln}} automatically manages replication slots for the
...
In case continuous backup is configured in the cluster, {{name.ln}}
...
permitted. However, {{name.ln}} automatically populates this option with the
names of local pods, while also allowing customization to extend synchronous
replication beyond the `Cluster` resource.
This can be achieved through the
[`.spec.postgresql.synchronous` stanza](pg4k.v1.md#synchronousreplicaconfiguration).
...
This replication method is the most common setup for a {{name.ln}} cluster.
...
With this configuration, {{name.ln}} automatically sets the content of
...
By default, {{name.ln}} populates `synchronous_standby_names` with the names
...
`standbyNamesPost`. {{name.ln}} expects that you manage any standby with
...
once. However, {{name.ln}} allows you to configure startup and readiness
...
!!!warning
Prior to {{name.ln}} 1.24, only the quorum-based synchronous replication
...
{{name.ln}} supports the configuration of **quorum-based synchronous
...
In case both `minSyncReplicas` and `maxSyncReplicas` are set, {{name.ln}}
...
{{name.ln}} enables you to select which PostgreSQL instances are eligible to
...
([RPO](before_you_start.md#postgresql-terminology)).
...
{{name.ln}} provides a turn-key solution to synchronize the content of
...
{{name.ln}} fills this gap by introducing the concept of cluster-managed
...
["replicationSlots" section in the API reference](pg4k.v1.md#replicationslotsconfiguration).
...
Although {{name.ln}} doesn't support a way to declaratively define physical
...
{{name.ln}} can manage the synchronization of any user managed physical
...
{{name.ln}} can synchronize logical decoding (replication) slots across all
...
For PostgreSQL 17 and newer, {{name.ln}} transparently manages the
...
For PostgreSQL 16 and older versions, {{name.ln}} uses the
...
how to monitor a {{name.ln}} deployment.
!!!
