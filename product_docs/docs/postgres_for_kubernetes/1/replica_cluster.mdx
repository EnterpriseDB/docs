...
originalFilePath: 'src/replica_cluster.md'
---



A replica cluster is a EDB Postgres for Kubernetes `Cluster` resource designed to
...
1.  **Disaster Recovery and High Availability**: Enhance disaster recovery and,
    to some extent, high availability of a EDB Postgres for Kubernetes cluster across different
    Kubernetes clusters, typically located in different regions. In EDB Postgres for Kubernetes
...
2.  **Read-Only Workloads**: Create standalone replicas of a PostgreSQL cluster
    ...
    replicas are primarily for read-only workloads. In EDB Postgres for Kubernetes terms, this
...
EDB Postgres for Kubernetes builds on the PostgreSQL replication framework, allowing you to
...
the source cluster. EDB Postgres for Kubernetes applies any local redefinitions once the
cluster is promoted.
...
If you are planning to promote the cluster at some point, EDB Postgres for Kubernetes will
...
2.  **WAL Archive**: Use the WAL (Write-Ahead Logging) archive stored in an
    ...
    the object store, from where a CNP-I plugin like [Barman Cloud](https://cloudnative-pg.io/plugin-barman-cloud/)
...
-   **`plugin` section**:
    ...
    -   EDB Postgres for Kubernetes supports the [Barman Cloud Plugin](https://cloudnative-pg.io/plugin-barman-cloud/docs/usage/#restoring-a-cluster)
...
-   **`connectionParameters` section**:
    ...
    -   EDB Postgres for Kubernetes automatically sets the `primary_conninfo` option in the
...
-   Enables use of the WAL archive, with EDB Postgres for Kubernetes automatically setting
...
3.  **Continuous Recovery Strategy**: Define this in the `.spec.replica` stanza:
    -   **Distributed Topology**: Configure using the `primary`, `source`, and
        ...
        `externalClusters`. This allows EDB Postgres for Kubernetes to declaratively control the
...
into a EDB Postgres for Kubernetes API configuration. This configuration primarily involves:
...
In this scenario, assume you have EDB Postgres for Kubernetes installed in the Southern
...
longer possible. EDB Postgres for Kubernetes then:
...
You can obtain the `demotionToken` using the `cnp` plugin by checking the
...
After making these adjustments, EDB Postgres for Kubernetes will initiate the promotion of
the replica cluster to a primary cluster. Initially, EDB Postgres for Kubernetes will wait
...
To verify the role change, use the `cnp` plugin to check the status of the
cluster:
...
In EDB Postgres for Kubernetes, a Standalone Replica Cluster is a PostgreSQL cluster in
...
EDB Postgres for Kubernetes supports the creation of **delayed replicas** through the
[`.spec.replica.minApplyDelay` option](pg4k.v1.md#postgresql-k8s-enterprisedb-io-v1-ReplicaClusterConfiguration),
