...
1.  **Disaster Recovery and High Availability**: Enhance disaster recovery and,
    to some extent, high availability of a {{name.ln}} cluster across different
    Kubernetes clusters, typically located in different regions. In {{name.ln}}
...
2.  **Read-Only Workloads**: Create standalone replicas of a PostgreSQL cluster
    ...
    replicas are primarily for read-only workloads. In {{name.ln}} terms, this
...
{{name.ln}} builds on the PostgreSQL replication framework, allowing you to
...
the source cluster. {{name.ln}} applies any local redefinitions once the
cluster is promoted.
...
If you are planning to promote the cluster at some point, {{name.ln}} will
...
-   **`plugin` section**:
    ...
    -   {{name.ln}} supports the [Barman Cloud Plugin](https://cloudnative-pg.io/plugin-barman-cloud/docs/usage/#restoring-a-cluster)
...
-   **`connectionParameters` section**:
    ...
    -   {{name.ln}} automatically sets the `primary_conninfo` option in the
...
-   Enables use of the WAL archive, with {{name.ln}} automatically setting
...
3.  **Continuous Recovery Strategy**: Define this in the `.spec.replica` stanza:
    -   **Distributed Topology**: Configure using the `primary`, `source`, and
        ...
        `externalClusters`. This allows {{name.ln}} to declaratively control the
...
into a {{name.ln}} API configuration. This configuration primarily involves:
...
In this scenario, assume you have {{name.ln}} installed in the Southern
...
{{name.ln}} provides the functionality to demote a primary cluster to a
...
After making these adjustments, {{name.ln}} will initiate the promotion of
the replica cluster to a primary cluster. Initially, {{name.ln}} will wait
...
!!! Important
    ...
    before the introduction of the Distributed Topology strategy in {{name.ln}}
    1.24.

In {{name.ln}}, a Standalone Replica Cluster is a PostgreSQL cluster in
...
{{name.ln}} supports the creation of **delayed replicas** through the
