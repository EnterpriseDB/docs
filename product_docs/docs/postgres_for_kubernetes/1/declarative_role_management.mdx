...
{{name.ln}} provides full lifecycle management for PostgreSQL database roles.
...
Please refer to the [API reference](pg4k.v1.md#roleconfiguration)
...
```
database role is already managed by the CNP cluster
```
...
CNP does not manage Certificate Revocation Lists (CRLs). If a certificate must
...
With the `managed` stanza in the cluster spec, {{name.ln}} provides
...
1.  Create the `DatabaseRole` with the desired specification. Both methods
    share the same [`RoleConfiguration`](pg4k.v1.md#roleconfiguration)
...
Label the Secret with `k8s.enterprisedb.io/reload: "true"`, as shown above. Password
...
{{name.ln}} protects this path in two complementary ways:
...
set the annotation `k8s.enterprisedb.io/passwordPassthrough: "enabled"` on the
...
With `k8s.enterprisedb.io/passwordPassthrough: "enabled"`, the operator forwards
