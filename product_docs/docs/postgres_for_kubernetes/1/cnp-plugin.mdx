...
This plugin is based on [the CloudNativePG Plugin](https://cloudnative-pg.io/documentation/1.15.0/cnpg-plugin/), 
and can work with `oc` in an OpenShift environment.

## Features 

The EDB Postgres for Kubernetes Plugin provides the same features as the CloudNativePG plugin, plus:

- It works with `oc` in an OpenShift environment.
- It can manage 
