...
{{name.ln}} does not independently test all authentication methods
supported by `barman-cloud`. {{name.ln}}' responsibility is limited to passing
...
{{name.ln}} supports the following authentication methods for Azure Blob Storage:
