...
CloudNativePG provides [a plugin for kubectl](https://cloudnative-pg.io/documentation/1.15.0/cnpg-plugin/) to manage a cluster in Kubernetes, along with a script to install it:
...
__OUTPUT__
EnterpriseDB/kubectl-cnpg info checking GitHub for latest tag
EnterpriseDB/kubectl-cnpg info found version: 1.15.0 for v1.15.0/linux/x86_64
EnterpriseDB/kubectl-cnpg info installed /usr/local/bin/kubectl-cnpg
```

The `cnpg` command is now available in kubectl:
...
!!! Note "There's more"
    See [the CloudNativePG Plugin page](https://cloudnative-pg.io/documentation/1.15.0/cnpg-plugin/) for more commands and options.
