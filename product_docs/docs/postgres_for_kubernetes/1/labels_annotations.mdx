...
: Specifies the type of PostgreSQL user associated with the
  ...
  default users created by EDB Postgres for Kubernetes (typically `postgres` and `app`).
...
:   When set to `disabled` on a EDB Postgres for Kubernetes-managed custom resource, the
