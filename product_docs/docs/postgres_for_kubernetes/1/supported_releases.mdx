...
Supported releases of EDB Postgres for Kubernetes include releases that are in the active
...
-   [Support status of EDB Postgres for Kubernetes releases](#support-status-of-cloudnativepg-releases)
-   [What we mean by support](#what-we-mean-by-support)

## Support policy

We produce new builds of EDB Postgres for Kubernetes for each commit.
...
level of assistance from the EDB Postgres for Kubernetes community.
...
You can find available releases on the [releases page](https://github.com/EnterpriseDB/cloud-native-postgres/releases).
...
the EDB Postgres for Kubernetes maintainers think is reasonable to support and to test.

At the moment, the EDB Postgres for Kubernetes community does not support any additional
...
by the EDB Postgres for Kubernetes maintainers).
...
:   Technical assistance is offered on a best-effort basis for supported
    ...
    [EDB Postgres for Kubernetes Slack](https://cloudnativepg.slack.com/) (in the `#general` channel),
    or using [GitHub Discussions](https://github.com/EnterpriseDB/cloud-native-postgres/discussions).
...
:   We back-port important bug fixes — including security fixes - to all
    ...
    *"Does this back-port improve `EDB Postgres for Kubernetes`, bearing in mind that we really
