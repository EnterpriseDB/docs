...
EDB Postgres for Kubernetes relies on application-level replication, for a simple reason: the
...
Such an architecture enables the EDB Postgres for Kubernetes operator to control the full
...
If your Kubernetes cluster has only one availability zone, EDB Postgres for Kubernetes still
...
EDB Postgres for Kubernetes clusters suffer a failure.
...
is available for PostgreSQL at the moment with EDB Postgres for Kubernetes, as the operator
can only work within a single Kubernetes cluster.

## PostgreSQL architecture

EDB Postgres for Kubernetes supports clusters based on asynchronous and synchronous
...
EDB Postgres for Kubernetes automatically takes care of updating the above services if
...
Availability purposes EDB Postgres for Kubernetes will trigger a failover, pointing the `-rw`
...
!!! Info
    EDB Postgres for Kubernetes supports deploying PostgreSQL across multiple
...
In order to address the above concerns, EDB Postgres for Kubernetes introduces the
concept of a *PostgreSQL Replica Cluster*. Replica clusters are the
EDB Postgres for Kubernetes way to enable multi-cluster deployments in private, public,
...
EDB Postgres for Kubernetes allows you to define multiple replica clusters.
