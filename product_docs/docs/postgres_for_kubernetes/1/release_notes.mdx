...
History of user-visible changes for EDB Postgres for Kubernetes.
For a complete list of changes, please refer to the
[commits](https://github.com/EnterpriseDB/cloud-native-postgres/commits/main)
in GitHub.
...
-   Fix the `cnp report operator` command to correctly handle the case of a
...
-   Properly propagate changes in the cluster’s `inheritedMetadata` set of labels
    ...
    restart
-   Fix the `cnp` plugin to correctly parse any custom configmap and secret name
...
-   Control of hot standby sensitive parameters correctly works with EPAS
    instances now
