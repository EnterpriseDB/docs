...
EDB Postgres for Kubernetes currently supports two main approaches for physical backups:
...
strategy in EDB Postgres for Kubernetes. They are defined using the `ScheduledBackup` custom
resource.
...
EDB Postgres for Kubernetes currently supports the following backup methods for scheduled
...
To reduce this impact, **EDB Postgres for Kubernetes supports taking backups from a standby
...
When `prefer-standby` is the target (the default behavior), EDB Postgres for Kubernetes will
attempt to:
...
EDB Postgres for Kubernetes is evolving toward a **backup-agnostic architecture**, where
...
built-in capabilities and scope of EDB Postgres for Kubernetes.
