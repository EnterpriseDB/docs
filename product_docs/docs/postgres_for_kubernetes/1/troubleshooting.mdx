...
All resources created and managed by EDB Postgres for Kubernetes log to standard output in
...
those from EDB Postgres for Kubernetes, accessing logs directly from the command line
...
-   Use the [`kubectl cnp logs` command](kubectl-plugin.md#logs) for
    EDB Postgres for Kubernetes-specific logging.
...
PostgreSQL Image:   quay.io/enterprisedb/postgresql:17.2-3
...
```shell
  Image Name:    quay.io/enterprisedb/postgresql:17.2-3
```
