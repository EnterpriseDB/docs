...
1. OpenShift Local doesn't come with a StorageClass, so one has to be configured. 
...
and how to create a Grafana Dashboard to monitor EDB Postgres for Kubernetes clusters, and a
...
provide useful initial settings for observability of EDB Postgres for Kubernetes clusters.
...
At this point, a EDB Postgres for Kubernetes cluster deployed with Monitoring activated
...
``` sh
kubectl apply -f \
  https://raw.githubusercontent.com/EnterpriseDB/cloud-native-postgres/main/docs/src/samples/monitoring/prometheusrule.yaml
```
...
see the `EDB Postgres for Kubernetes` dashboard.
