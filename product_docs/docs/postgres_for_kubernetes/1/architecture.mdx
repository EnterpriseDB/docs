...
within EDB Postgres for Kubernetes' scope, as the operator can only function within a single
Kubernetes cluster.
...
EDB Postgres for Kubernetes recommends using the `node-role.kubernetes.io/postgres` label.
...
-   **Human decision:** You choose to make the other PostgreSQL cluster (or the
    ...
    primary using the API provided by EDB Postgres for Kubernetes.
