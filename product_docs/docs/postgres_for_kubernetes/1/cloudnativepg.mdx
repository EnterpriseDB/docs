...
Despite defining both a customer resource called `Cluster`, they have two
...
**IMPORTANT:** Existing EDB customers with deployments of EDB Cloud Native
Postgres might have noticed that the API group for version 1 has not changed
with the introduction of EDB Postgres for Kubernetes.
