...
deploying EDB Postgres for Kubernetes on a local Kubernetes cluster
...
You should find a series of metrics relating to EDB Postgres for Kubernetes clusters.
Please refer to the [monitoring section](monitoring.md) for more information.

![local prometheus](images/prometheus-local.png)

You can also monitor the EDB Postgres for Kubernetes operator by creating a PodMonitor to
...
EDB Postgres for Kubernetes provides a default dashboard for Grafana in the dedicated
...
You can now click on the `EDB Postgres for Kubernetes` dashboard just created:
...
automatically discover and monitor any EDB Postgres for Kubernetes clusters deployed with the
