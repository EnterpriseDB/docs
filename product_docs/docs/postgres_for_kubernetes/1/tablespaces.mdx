...
{{name.ln}} provides support for PostgreSQL tablespaces through *declarative
...
Being a part of the Kubernetes ecosystem, {{name.ln}}' declarative
...
persistent volume. {{name.ln}} takes care of generating the PVCs. It mounts
...
{{name.ln}} creates the persistent volume claims for each instance
...
If you define a tablespace with an owner that doesn't exist, {{name.ln}} can't
...
{{name.ln}} handles backup of tablespaces (and the relative
...
{{name.ln}} provides the `.spec.tablespaces[*].temporary` option to
...
Currently, you can't remove tablespaces from an existing {{name.ln}}
cluster.
