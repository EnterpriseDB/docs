...
To accommodate these requirements, EDB Postgres for Kubernetes extends the
...
EDB Postgres for Kubernetes 1.26 introduces an opt-in experimental behavior for the liveness
...
metadata:
  annotations:
    alpha.k8s.enterprisedb.io/livenessPinger: '{"enabled": true}'
```

!!! Warning
    This feature is experimental and will be introduced in a future EDB Postgres for Kubernetes
...
metadata:
  annotations:
    alpha.k8s.enterprisedb.io/livenessPinger: '{"enabled": true,"requestTimeout":1000,"connectionTimeout":1000}'
```
