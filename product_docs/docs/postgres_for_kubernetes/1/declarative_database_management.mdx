...
EDB Postgres for Kubernetes simplifies PostgreSQL database provisioning by automatically
...
For more advanced use cases, EDB Postgres for Kubernetes introduces **declarative database
...
!!! Important
    EDB Postgres for Kubernetes manages **global objects** in PostgreSQL clusters, including
...
!!! Info
    ...
    EDB Postgres for Kubernetes clusters in the same Kubernetes namespace.
...
objects in EDB Postgres for Kubernetes.
...
EDB Postgres for Kubernetes supports two methods for database deletion:
...
!!! Info
    ...
    EDB Postgres for Kubernetes provides a declarative interface for managing them. This approach
    is necessary because installing certain extensions may require superuser
    privileges, which EDB Postgres for Kubernetes recommends disabling by default. By leveraging
...
EDB Postgres for Kubernetes simplifies and automates the management of PostgreSQL extensions within the
target database.
...
!!! Info
    EDB Postgres for Kubernetes manages extensions using the following PostgreSQL’s SQL commands:
...
!!! Warning
    Before the introduction of declarative extension management, EDB Postgres for Kubernetes
...
!!! Info
    Schema management in PostgreSQL is an exception to EDB Postgres for Kubernetes' primary
    ...
    EDB Postgres for Kubernetes provides a declarative interface for schema management, primarily
    to complete the support of extensions deployment within schemas.

EDB Postgres for Kubernetes simplifies and automates the management of PostgreSQL schemas within the
target database.
...
!!! Info
    EDB Postgres for Kubernetes manages schemas using the following PostgreSQL’s SQL commands:
...
While EDB Postgres for Kubernetes adheres to PostgreSQL’s
...
-   For new databases, EDB Postgres for Kubernetes uses the `CREATE DATABASE` statement.
...
EDB Postgres for Kubernetes adheres to PostgreSQL's capabilities. For example, features like
...
EDB Postgres for Kubernetes does not overwrite manual changes to databases. Once reconciled,
