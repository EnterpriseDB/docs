...
EDB Postgres for Kubernetes' source code undergoes systematic static analysis, including
...
The EDB Postgres for Kubernetes repository has the *"Private vulnerability reporting"* option
...
!!! Important
    ...
    block the entire delivery process of EDB Postgres for Kubernetes. Every commit must pass all
...
Postgres version (including [EDB Postgres Extended](/pge/latest/) and [EDB Postgres Advanced](/epas/latest/)). During the CI/CD process, images undergo scanning with the
following tools:
...
!!! Seealso "About Container-Level Security"
    ...
    at the container level in EDB Postgres for Kubernetes, please refer to the blog article
    ["Security and Containers in EDB Postgres for Kubernetes"](https://www.enterprisedb.com/blog/security-and-containers-cloud-native-postgresql).
