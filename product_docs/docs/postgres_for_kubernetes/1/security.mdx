...
This section contains information about security for {{name.ln}},
...
Operand images are build and published regularly for each supported PostgreSQL version 
...
levels, providing **patch-level updates** for the container images distributed to EDB download sites.
!!!
...
{{name.ln}} does not require *privileged* mode for containers execution.
