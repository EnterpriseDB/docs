...
!!! Info
    EDB Postgres for Kubernetes does not use `StatefulSet`s for managing data persistence.
...
Encryption at rest is possible with EDB Postgres for Kubernetes. The operator delegates that
...
!!! Important
    EDB Postgres for Kubernetes has been designed to be storage class agnostic.
...
When set to `'true'`, EDB Postgres for Kubernetes triggers a rolling update of the
Postgres cluster.
...
EDB Postgres for Kubernetes has been designed to work with dynamic volume provisioning, which
...
!!! Important
    ...
    EDB Postgres for Kubernetes has been built.
