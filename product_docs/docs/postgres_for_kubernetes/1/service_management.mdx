...
services directly managed by EDB Postgres for Kubernetes. For more details, refer to the
...
By default, EDB Postgres for Kubernetes creates all the above services for a `Cluster`
...
Kubernetes cluster, EDB Postgres for Kubernetes offers flexibility to:
...
A common scenario arises when using EDB Postgres for Kubernetes in database-as-a-service
...
!!! Warning
    ...
    EDB Postgres for Kubernetes has no control over the service configuration, except honoring
    the selector.
