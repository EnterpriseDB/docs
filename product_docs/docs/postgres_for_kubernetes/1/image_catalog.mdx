...
spec:
  images:
    - major: 18
      image: docker.enterprisedb.com/k8s/postgresql:18.4-standard-ubi9
  componentImages:
    - key: pgbouncer
...
spec:
  images:
    - major: 18
      image: docker.enterprisedb.com/k8s/postgresql:18.4-minimal-ubi9
      extensions:
        - name: foo
          image:
...
## EDB CloudNativePG Cluster Catalogs
...
containing an `extensions` section are only compatible with {{name.ln}} 1.29
