...
{{name.ln}} supports mounting custom files inside the Postgres pods through
...
In {{name.ln}}, the `.spec.projectedVolumeTemplate` field is a
...
{{name.ln}} relies on [ephemeral volumes](https://kubernetes.io/docs/concepts/storage/ephemeral-volumes/)
...
{{name.ln}} allows you to set custom environment variables using the `env`
