...
originalFilePath: 'src/service_management.md'
---



A PostgreSQL cluster should only be accessed via standard Kubernetes network
services directly managed by {{name.ln}}. For more details, refer to the
...
{{name.ln}} defines three types of services for each `Cluster` resource:
...
By default, {{name.ln}} creates all the above services for a `Cluster`
...
!!!info Important
Default service names are reserved for {{name.ln}} usage.
!!!

While this setup covers most use cases for accessing PostgreSQL within the same
Kubernetes cluster, {{name.ln}} offers flexibility to:
...
A common scenario arises when using {{name.ln}} in database-as-a-service
...
[`managed.services.disabledDefaultServices` option](pg4k.v1.md#managedservices).

!!!info Important
The `rw` service is essential and cannot be disabled because {{name.ln}}
...
[`managed.services.additional` stanza](pg4k.v1.md#managedservice)
...
{{name.ln}} has no control over the service configuration, except honoring
the selector.
!!!
