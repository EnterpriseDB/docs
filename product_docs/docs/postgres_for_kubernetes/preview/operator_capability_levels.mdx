...
`RoleBinding`, and `Role`), and reconciles user-defined CNP resources such as
...
{{name.ln}} supports the declarative definition of `podSelectorRefs` to
...
{{name.ln}} provides declarative support for managing PostgreSQL extensions
...
{{name.ln}} provides a pluggable interface (CNPG-I) for managing
...
In addition to CNPG-I plugins, {{name.ln}} also natively supports backups
...
{{name.ln}} enables *zero-data-loss (RPO=0)* high-availability clusters by
