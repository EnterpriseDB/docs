...
In {{name.ln}}, recovery is **not performed in-place** on an existing
...
!!! Important
    The object store must contain backup data produced by a {{name.ln}}
...
{{name.ln}} allows you to create a new cluster from a `VolumeSnapshot` of a
...
This setup enables {{name.ln}} to restore the base data from a volume
...
!!! Warning
    {{name.ln}} includes a safety check to prevent a cluster from overwriting
