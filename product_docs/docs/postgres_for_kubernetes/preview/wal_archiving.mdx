...
Write-Ahead Log (WAL) archiving in EDB Postgres for Kubernetes is the process of continuously
...
EDB Postgres for Kubernetes supports WAL archiving through a **plugin-based mechanism**,
defined via the [`spec.pluginConfiguration`](pg4k.v1.md#postgresql-k8s-enterprisedb-io-v1-ClusterSpec)
...
archiving plugin maintained by the EDB Postgres for Kubernetes Community.
...
EDB Postgres for Kubernetes still supports WAL archiving natively through the
