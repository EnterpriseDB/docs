...
Before using the TDS Foreign Data Wrapper:
...
The following command installs the TDS foreign data wrapper:
...
Use the `DROP EXTENSION` command to remove an extension. Connect to the Postgres database from which you're dropping the TDS Foreign Data Wrapper, and run the command:
