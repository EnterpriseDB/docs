...
!!! Note
    You can use the `CREATE TYPE` command to define a nested table type that's available to all SPL programs in the database. See [Database Compatibility for Oracle Developers: SQL](/epas/latest/epas_compat_sql/39_create_type/) for more information about the `CREATE TYPE` command.
