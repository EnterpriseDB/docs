...
redirects:
  - ../../../../../epas_compat_ora_dev_guide/03_advanced_concepts/05_hierarchical_queries/06_retrieving_the_root_node_with_connect_by_root #generated for docs/epas/reorg-role-use-case-mode
---
...
The values of both `ename` and `empno` are affected by the `CONNECT_BY_ROOT` operator. As a result, they return the values from the root node as shown under the `top mgr/empno` column.
