...
Use the following configuration parameters to control database auditing. See [Summary of configuration parameters](../01_configuration_parameters/02_summary_of_configuration_parameters/#summary_of_configuration_parameters) to determine if a change to the configuration parameter:
...
`edb_audit_archiver_compress_size_limit`
 ...
 Specifies a file size threshold, in megabytes, after which audit logs are eligible for compression. If the parameter is set to `-1`, compression of the log file on a size basis doesn't occur. The threshold applies to multiple audit logs such that they are eligible for compression if their combined size exceeds the threshold size limit, even though the size of each audit log is smaller.
...
`edb_audit_archiver_expire_size_limit` 
 ...
 Specifies a file size threshold in megabytes, after which audit logs are eligible to expire. If the parameter is set to `-1`, the expiration of a log file on the size basis doesn't occur. The threshold applies to multiple audit logs such that they expire if their combined size exceeds the threshold size limit, even though the size of each audit log is smaller.
...
`edb_audit_statement`
 ...
 Specifies auditing of different categories of SQL statements. You can specify various combinations of the following values: `none`, `dml`, `insert`, `update`, `delete`, `truncate`, `select`, `error`, `rollback`, `ddl`, `create`, `drop`, `alter`, `grant`, `revoke`, `set`, `all`, and `{ select | update | delete | insert }@groupname`. The default is `ddl` and `error`. See [Selecting SQL statements to audit](02_selecting_sql_statements_to_audit/#selecting_sql_statements_to_audit) for information about setting this parameter.
