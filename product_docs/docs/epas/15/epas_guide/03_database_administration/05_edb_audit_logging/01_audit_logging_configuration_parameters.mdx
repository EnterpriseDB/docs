...
`edb_audit_archiver_expire_size_limit` 
 ...
 Specifies a file size threshold in megabytes, after which audit logs are eligible to expire. If the parameter is set to `-1`, the expiration of a log file on the size basis doesn't occur. The threshold applies to multiple audit logs such that they expire if their combined size exceeds the threshold size limit, even though the size of each audit log is small.
