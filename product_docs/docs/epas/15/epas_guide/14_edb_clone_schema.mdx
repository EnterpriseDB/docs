...
-   **localcopyschema_nb.** This function performs the same purpose as `localcopyschema` but as a background job, thus freeing up the terminal from which the function was initiated. This function is referred to as a *non-blocking* function. See [localcopyschema_nb](#localcopyschema_nb) for more information.
...
-   **remotecopyschema_nb.** This function performs the same purpose as `remotecopyschema` but as a background job, thus freeing up the terminal from which the function was initiated. This function is a non-blocking function. See [remotecopyschema_nb](#remotecopyschema_nb) for more information.
-   **process_status_from_log.** This function displays the status of the cloning functions. The information is obtained from a log file you specify when a cloning function is invoked. See [process_status_from_log](#process_status_from_log) for more information.
-   **remove_log_file_and_job.** This function deletes the log file created by a cloning function. You can also use this function to delete a job created by the non-blocking form of the function. See [remove_log_file_and_job](#remove_log_file_and_job) for more information.

You can clone the database objects from one schema to another:
...
Ghe following limitations apply:
...
Perform this insstallation on any database to be used as the source or target database by an EDB Clone Schema function.
...
1.  Modify the `postgresql.conf` file by adding `$libdir/parallel_clone` to the `shared_preload_libraries` configuration parameter as shown by the following example:
...
You might need to tune the system for copying a large schema as part of one transaction.
...
-   **maintenance_work_mem.** Specifies the maximum amount of memory for maintenance operations to use, such as `VACUUM`, `CREATE INDEX`, and `ALTER TABLE ADD FOREIGN KEY`.
...
Perform these steps on any database to be used as the source or target database by an EDB Clone Schema function.
...
When using one of the local cloning functions, `localcopyschema` or `localcopyschema_nb`, one of the required parameters includes a single, foreign server. This server is for identifying the database server and its database that's the source and the receiver of the cloned schema.
...
For the `localcopyschema` and `localcopyschema_nb` functions, the source and target schemas are both in the same database of the same database server. Thus, only one foreign server must be defined and specified for these functions. This foreign server is also referred to as the *local server* because this server is the one to which you must be connected when invoking the `localcopyschema` or `localcopyschema_nb` function.

The user mapping defines the connection and authentication information for the foreign server. This foreign server and user mapping must be created in the database of the local server in which the cloning occurs.
...
In this case, the mapped database user, `enterprisedb`, and the database user, `enterprisedb`, used to connect to the local `edb` database are the same, identical database user. However, that's not a requirement.

For specific use of these foreign server and user mapping examples, see the example given in [`localcopyschema`](#localcopyschema).

#### Foreign server and user mapping for remote cloning functions

For the `remotecopyschema` and `remotecopyschema_nb` functions, the source and target schemas are in different databases of either the same or different database servers. Thus, you must define and specify two foreign servers for these functions.
...
For specific use of these foreign server and user mapping examples, see the example given in [`remotecopyschema`](#remotecopyschema).
...
-   When using the remote copy functions, if the `copy_acls` parameter is to be set to `true`, then all roles that have `GRANT` privileges on objects in the source schema exist in the target database cluster. Otherwise granting privileges to those roles fails in the target schema, which causes a failure of the cloning process.
...
The `localcopyschema_nb` function copies a schema and its database objects in a local database specified in the `source_fdw` foreign server from the source schema to the specified target schema in the same database. The copy occurs in in a non-blocking manner as a job submitted to pgAgent.
...
You can use this command to swee whether pgAgent is running on the appropriate local database:
...
!!! Note
    If `true` is specified and a database object has a `TABLESPACE` clause but that tablespace doesn't exist in the target database cluster, then the cloning function fails.

`verbose_on`

Boolean value to specify whether toprint the DDLs in `log_filename` when creating objects in the target schema. If `false`, then DDLs aren't printed. If `true`, then DDLs are printed. The default value is `false`.
...
!!! Note
    If `true` is specified and a role with `GRANT` privilege doesn't exist in the target database cluster, then the cloning function fails.
...
Name of the schema from which to cone database objects.
...
The following shows use of the `process_status_from_log` function:
...
You cam specify values for either or both of the two parameters when invoking the `remove_log_file_and_job` function:
...
This example removes only the log file, given the log filename:
