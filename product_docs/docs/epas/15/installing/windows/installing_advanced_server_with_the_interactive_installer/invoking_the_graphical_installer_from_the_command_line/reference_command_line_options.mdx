...
The `--enable_acledit 1` option grants permission to the user specified by the `--serviceaccount` option to access the EDB Postgres Advanced Server binaries and `data` directory. By default, this option is disabled if `--enable_acledit 0` is specified or if the `--enable_acledit` option is omitted.
...
- Include `--unattendedmodeui none` to not display progress bars during the EDB Postgres Advanced Server installation.
