...
-   EDB Postgres Advanced Server (EPAS) installation defaults

You can embed the non-interactive EPAS installer within another application installer; during the installation process, a progress bar allows the user to view the progression of the installation.

You must have administrative privileges to install EPAS using the `--mode unattended` option. If you are using the `--mode unattended` option to install EPAS with a client, the calling client must be invoked with superuser or administrative privileges.
...
When invoking the installer, include the `--servicepassword` option to specify an operating system password for the user installing EPAS.
