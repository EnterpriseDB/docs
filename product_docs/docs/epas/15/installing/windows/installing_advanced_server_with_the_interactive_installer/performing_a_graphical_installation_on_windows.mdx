...
A graphical installation is a quick and easy way to install EDB Postgres Advanced Server 15 on a Windows system. Use the wizard's screens to specify information about your system and system usage. When you complete the screens, the installer performs an installation based on your selections.
...
!!! Note
    To install EDB Postgres Advanced Server on some versions of Windows, you might need to right-click the file icon and select **Run as Administrator** from the context menu to invoke the installer with administrator privileges.
...
Carefully review the license agreement before selecting the appropriate radio button. Select **Next**.

The Installation Directory window opens.

![The Installation Directory window](../../../images/installation_directory.png)

By default, the EDB Postgres Advanced Server installation directory is  C:\Program Files\edb\as15

You can accept the default installation location and select **Next**. Optionally, you can select the file browser icon to open the Browse For Folder dialog box to choose a different installation directory.

!!! Note
    Don't store the `data` directory of a production on an NFS file system.

![The Select Components window](../../../images/select_components.png)

The Select Components window contains a list of optional components that you can install with the EDB Postgres Advanced Server setup wizard. You can omit a module from the EDB Postgres Advanced Server installation by clearing the check box next to its name.
...
-  Select **StackBuilder Plus** to install that utility. The StackBuilder Plus utility is a graphical tool that can update installed products or download and add supporting modules (and the resulting dependencies) after your EDB Postgres Advanced Server setup and installation completes. See [Using StackBuilder Plus](using_stackbuilder_plus/) for more information about StackBuilder Plus.
...
1. Accept the default file locations, or use the file browser icon to select a different location. Select **Next**. 
...
-   If the **Locale** field is set to **[Default locale]**, EDB Postgres Advanced Server uses the system locale as the working locale. From the list, select an alternate locale for EDB Postgres Advanced Server.
-   By default, the setup wizard installs corresponding sample data for the server dialect specified by the compatibility mode (Oracle or PostgreSQL). Clear the check box next to **Install sample tables and procedures** if you don't want to install sample data.
...
1. After verifying the information on the Additional Configuration window, select **Next**. The Dynatune Dynamic Tuning: Server Utilization window opens.
    ...
    The `edb_dynatune` configuration parameter determines how EDB Postgres Advanced Server allocates system resources. 

1. Use the options on the Server Utilization window to set the initial value of the `edb_dynatune` configuration parameter:
...
1. Review the options before selecting **Next**. Select **Back** to navigate back through the screens and update any options.

1. The Ready to Install window confirms that the installer has the information it needs about your configuration preferences to install EDB Postgres Advanced Server. Select **Next**.
    ...
    Before the setup wizard completes the EDB Postgres Advanced Server installation, it offers: `Launch StackBuilder Plus at exit?`

    ![The Setup wizard offers to Launch StackBuilder Plus at exit](../../../images/setup_wizard.png)

1. Clear the **StackBuilder Plus** check box and select **Finish** to complete the EDB Postgres Advanced Server installation, or accept the default and proceed to StackBuilder Plus.
