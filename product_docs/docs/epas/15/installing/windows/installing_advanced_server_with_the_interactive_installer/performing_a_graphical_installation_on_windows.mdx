...
To invoke the wizard, you need administrator privileges. 

1. Assume administrator privileges, and double-click the `edb-as15-server-15.x.x-x-windows-x64` executable file.
...
1. After selecting the components you want to install, select **Next**. The Additional Directories window opens.
    ...
    By default, the EDB Postgres Advanced Server data files are saved to `C:\Program Files\edb\as15\data`.  
    
    The default location of the EDB Postgres Advanced Server write-ahead log (WAL) directory is `C:\Program Files\edb\as15\data\pg_wal`. EDB Postgres Advanced Server uses write-ahead logs to promote transaction safety and speed transaction processing. When you make a change to a table, the change is stored in shared memory, and a record of the change is written to the write-ahead log. When you perform a COMMIT, EDB Postgres Advanced Server writes contents of the write-ahead log to disk.
