...
The EPAS interactive installer creates an uninstaller that you can use to remove EPAS or components that reside on a Windows host. The uninstaller is created in `C:\Program Files\edb\as15`. 
...
1. Select **Next**.
    ...
    If you selected components to remove that depend on EPAS, those components are removed first. 
