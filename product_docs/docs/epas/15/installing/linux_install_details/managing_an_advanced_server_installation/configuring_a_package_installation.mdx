...
The PostgreSQL `initdb` command creates a database cluster. When installing EDB Postgres Advanced Server (EPAS) with an RPM package, the `initdb` executable is in `/usr/edb/asx.x/bin`. After installing EPAS, you must manually configure the service and invoke `initdb` to create your cluster. When invoking `initdb`, you can:
