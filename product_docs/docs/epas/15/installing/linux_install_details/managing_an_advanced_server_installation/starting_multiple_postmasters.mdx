...
The `edb-as15-server-core` RPM for version 8.x contains a unit file that starts the EDB Postgres Advanced Server instance. The file allows you to start multiple services with unique `data` directories and monitor different ports. You need root access to invoke or modify the script.
