...
You can create a local repository to act as a host for the EDB Postgres Advanced Server (EPAS) native packages if the server on which you want to install EPAS or supporting components can't directly access the EDB repository. This is a high-level listing of the steps required. Modify the process for your network.
...
After specifying the location and connection information for your local repository, you can use `yum` or `dnf` commands to install EPAS and its supporting components on the isolated servers. For example:
