...
-   If you export role data with `pg_dumpall --no-role-passwords`, the `rolpolicyexempt` attribute (`EXEMPT ACCESS POLICY` privilege) isn't preserved and defaults to `false` on restore. See [Virtual private database](/epas/15/epas_security_guide/03_virtual_private_database/) for how to identify and reapply affected roles.
