...
While minor upgrades between versions are fairly simple and require only installing new executables, past major version upgrades were both expensive and time consuming. `pg_upgrade` facilitates migration between any version of EDB Postgres Advanced Server (version 9.0 or later) and any subsequent release of EDB Postgres Advanced Server that's supported on the same platform.
...
If the upgrade involves a change in the on-disk representation of database objects or data or involves a change in the binary representation of data types, `pg_upgrade` can't perform the upgrade. To upgrade, you have to `pg_dump` the old data and then import that data to the new cluster.
