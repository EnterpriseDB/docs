...
When invoking `pg_upgrade`, you must specify the location of the old and new cluster's `PGDATA` and executable (`/bin`) directories, as well as the name of the EDB Postgres Advanced Server superuser, and the ports on which the installations are listening. A typical call to invoke `pg_upgrade` to migrate from EDB Postgres Advanced Server 14 to EDB Postgres Advanced Server 15 takes the form:
...
Use the `--old-datadir` option to specify the complete path to the `data` directory within the EDB Postgres Advanced Server 14 installation.

`--new-datadir path_to_14_data_directory`

Use the `--new-datadir` option to specify the complete path to the `data` directory within the EDB Postgres Advanced Server 15 installation.
