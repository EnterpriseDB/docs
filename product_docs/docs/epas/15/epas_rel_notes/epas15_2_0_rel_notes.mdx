...
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------|  --------------------- |
| Upstream merge | See the [PostgreSQL 15 Release Notes](https://www.postgresql.org/docs/15/release-15.html) for more information.                           |                        |
| Feature        | Transparent Data Encryption (TDE) - TDE is an optional feature supported by EDB Postgres Advanced Server with high availability. It encrypts, transparently to the user, any user data stored in the database system. User data includes the actual data stored in tables and other objects, as well as system catalog data such as the names of objects. See [TDE docs](/tde/latest) for more information. | Security |
...
| Enhancement    | EDB Postgres Advanced Server now provides the ability to obfuscate the LDAP password in `pg_hba.conf` file. The user can supply a module which will transform the ldapbindpasswd in the `pg_hba.conf` file before it is passed to the LDAP server. |
...
| Enhancement    | MERGE syntax - Adding support for WHERE clause to the UPDATE and INSERT of MERGE command for Oracle compatibility.| |
| Enhancement    | Adding HTP and HTF packages to built-in-packages for Oracle compatibility. |
...
| Enhancement    | Skip IN/OUT/IN OUT/INOUT modifiers in the USING expression. USING clause in EXECUTE IMMEDIATE does support passing parameters to embedded SPL blocks. We treat those parameters as INOUT only, and there is currently no way to specify whether that parameter is IN, OUT or INOUT. However, to ease migration from Oracle, we skip these modifiers when they appear at the beginning of the expression whenever possible. | |
| Enhancement    | Adding format_error_stack() and format_error_backtrace() functions to DBMS_UTILITY package. These functions are used in a stored procedure, function or package to return the current exception name. These functions are useful for debugging and logging purposes. | |
| Enhancement    | Adding Oracle compatible UPDATE..SET ROW syntax support. UPDATE changes the values of the specified columns in all rows that satisfy the condition. Only the columns to modify are mention in the SET clause; columns not to modify explicitly retain their previous values. SET ROW clause enables users to update a target record using a record type variable or row type objects. The condition is that the record or row used should have compatible data types with table's columns in order. |
| Enhancement    | EDB Postgres Advanced Server 15 now provides INDEX and NO_INDEX hints for the partitioned table. This optimizer hints applies to inherited index in the partitioned table. The execution plan internally expands to include the corresponding inherited child indexes and applied in later processing. | |
...
| Enhancement    | Adding TO_MULTI_BYTE() and TO_SINGLE_BYTE() functions. | |
...
| Enhancement    | Adding TO_DSINTERVAL() function. Convert a character string of CHAR, VARCHAR2, NCHAR, or NVARCHAR2 datatype to an interval datatype. | |
...
| Enhancement    | `index _advisor` is now a separate extension. | |
