...
| Enhancement    | EDB Postgres Advanced Server now lets you obfuscate the LDAP password in the `pg_hba.conf` file. You can supply a module that transforms the `ldapbindpasswd` value in the `pg_hba.conf` file before the value is passed to the LDAP server. See [Obfuscating the LDAP password](/epas/15/epas_guide/03_database_administration/01_configuration_parameters/01_setting_new_parameters/#obfuscating-the-ldap-password). | Security |
...
| Enhancement    | The NVL function now accepts the double precision data type arguments and returns the same. | |
