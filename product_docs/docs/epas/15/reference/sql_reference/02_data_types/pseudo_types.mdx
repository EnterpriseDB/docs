...
The `internal` pseudo-type is used to declare functions that are meant only to be called internally by the database system, and not by direct invocation in an SQL query. If a function has at least one internal-type argument, then it can't be called from SQL. To preserve the type safety of this restriction, it's important to follow this coding rule: don't create any function that's declared to return `internal` unless it has at least one internal argument.
