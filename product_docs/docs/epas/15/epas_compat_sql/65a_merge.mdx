...
`MERGE` allows you to select rows from one or more sources for update or insertion into a table. You specify join condition to determine whether to UPDATE or INSERT into the target table. Optionally you specify conditional UPDATE and INSERT using the WHERE clause in the MERGE statement.
...
This example shows how to use the WHERE condition for conditional UPDATE and INSERT in the MERGE statement:
