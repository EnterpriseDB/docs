...
`MERGE` provides a single SQL statement that can conditionally INSERT, UPDATE or DELETE rows, a task requiring multiple procedural language statements.
...
This example shows how to UPDATE and INSERT rows using PostgreSQL style and conditionally DELETE rows on the target table using the MERGE statement:
