...
`MERGE` allows you to select rows from one or more sources for update or insertion into a table or view. You can specify conditions to determine whether to UPDATE or INSERT into the target table or view. You can also do conditional UPDATE and INSERT using the WHERE clause in the MERGE statement.
...
`when_clause`
 ...
 At least one WHEN clause is required. Specifies conditional merge_update_clause and merge_insert_clause.
...
This example shows how to use where condition for update and delete statements, and also WHEN MATCHED THEN DELETE:
