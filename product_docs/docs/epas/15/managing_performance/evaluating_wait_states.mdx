...
redirects:
  - ../../epas_guide/13_performance_analysis_and_tuning #generated for docs/epas/reorg-role-use-case-mode
---
...
The EDB Wait States interface allows you to control when and for how long the wait events are sampled, and to extract the gathered samples (in `edb_wait_states_data`) for further analysis. By gathering this data over time, optimization opportunities can be discovered and insight gained into what resources sessions are waiting on when performance is lower than expected.

See [EDB Wait States](advocacy_docs/pg_extensions/wait_states) for more information.