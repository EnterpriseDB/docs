...
title: "UPDATE"
redirects:
  - ../../../../epas_compat_spl/04_basic_statements/07_update #generated for docs/epas/reorg-role-use-case-mode
---
...
!!! Note
   You can include the `UPDATE` command in a `FORALL` statement. A `FORALL` statement allows a single `UPDATE` command to update multiple rows from values supplied in one or more collections. See [Using the FORALL statement](../12_working_with_collections/03_using_the_forall_statement/#using_the_forall_statement) for more information.
