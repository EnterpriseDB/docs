...
Use the `ENABLE ROW MOVEMENT` with a list, range, or hash partition tables to support the migration of tables with similar syntax from oracle databases, however it do not enable the actual row movement. Also, this syntax is not supported with `ALTER TABLE` command.
