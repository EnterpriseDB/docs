...
1. On the Launch an instance page, specify the following:
   ...
   - **Application and OS Images (Amazon Machine Image)** &mdash; On the **AWS Marketplace** tab, type **EDB** in the search bar and choose the EDB Postgres Advanced Server image. The image name is in the format `EDB-AS<x>-AWS-<y>`, where: 
