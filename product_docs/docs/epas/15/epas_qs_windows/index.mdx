...
This is not a comprehensive list of the compatibility features for Oracle included when EDB Postgres Advanced Server is installed in `Compatible with Oracle` mode. For more information, see [Database compatibility for Oracle developers: built-in packages](/epas/latest/epas_compat_bip_guide/).
