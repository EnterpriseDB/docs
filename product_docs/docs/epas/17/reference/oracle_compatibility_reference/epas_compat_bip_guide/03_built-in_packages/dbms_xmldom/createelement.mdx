...
This example creates a new XML DOMDocument named `l_domdoc` and a new `l_department_element` DOMElement named `Departments`. It then converts `l_domdoc` and `l_department_element` into DOMNodes, so `l_department_element` can be appended as a child to `l_domdoc`.
