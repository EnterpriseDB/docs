...
```sql
LOADCLOBFROMFILE (<dest_lob> IN OUT NOCOPY BLOB, <src_file> IN BFILE, <amount> IN INTEGER,
    <dest_offset> IN OUT INTEGER, <src_offset> IN OUT INTEGER), <src_csid> IN NUMBER, 
...
For IN, the offset in characters in the destination CLOB (origin: 1) for the start of the load. For OUT, the new offset in characters in the destination CLOB at the end of this load, which is also where the next load should begin.
...
For IN, the language context for the current load. For OUT, the language context when the load stopped and what the next load should if loading continues from the same source.
