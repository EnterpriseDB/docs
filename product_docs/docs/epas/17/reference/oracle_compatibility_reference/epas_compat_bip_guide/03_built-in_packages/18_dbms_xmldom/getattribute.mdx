...
This example creates a new `DOMDocument` named `l_domdoc`, and a `DOMElement` named `elem` with tag name “Departments”. It adds an attribute to the `DOMElement` named "value" and appends the `DOMElement` as a child of the `DOMDocument`. Finally, it converts the `DOMDocument` to `XMLType`.
