...
If you're encountering NOTICE messages due to invalid entries in the NLS config file, the best approach is to manually remove those invalid entries. These messages typically occur when the database is trying to reference locales or collations that doesn't exist or are incorrectly specified in the configuration file.
...
| `edb_nls_cf_hash_dump(OUT nls TEXT, OUT collationid INT)` | INT         | Dumps the NLS parameter and a mapping collation HASH contents on the client session.                             |
