...
> pg_upgrade --old-datadir *path_to_9.6_data_directory* --new-datadir *path_to_10_data_directory* --user *superuser_name* --old-bindir *path_to_9.6_bin_directory* --new-bindir *path_to_10_bin_directory* --old-port *9.6_port* --new-port *10_port*
...
> pg_upgrade.exe -d "C:\\Program Files\\ PostgresPlus\\9.6AS \\data" -D "C:\\Program Files\\edb\\as10\\data" -U enterprisedb
...
If pg_upgrade reports that the new cluster is not empty, please empty the new cluster. **T**he target cluster may not contain any user-defined databases.
...
Use StackBuilder Plus to download and install the missing module as described in Chapter 4, [Using Stackbuilder Plus](004_installing_advanced_server_with_the_interactive_installer/#using-stackbuilder-plus). Then:
