...
If you select `Compatible with PostgreSQL`, EDB Postgres Advanced Server will exhibit compatibility with PostgreSQL version 14. If you choose to install in `Compatible with PostgreSQL` mode, the default EDB Postgres Advanced Server superuser name is `postgres`.
