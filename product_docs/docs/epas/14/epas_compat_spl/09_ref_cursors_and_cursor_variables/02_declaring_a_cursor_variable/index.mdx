...
Only the declaration of `SYS_REFCURSOR` and user-defined `REF CURSOR` variables are different. The remaining usage like opening the cursor, selecting into the cursor and closing the cursor is the same across both the cursor types. The rest of the examples primarily make use of the `SYS_REFCURSOR` cursors. All you need to change in the examples to make them work for user defined `REF CURSORs` is the declaration section.
