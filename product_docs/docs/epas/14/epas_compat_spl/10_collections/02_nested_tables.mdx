...
!!! Note
    You can use the `CREATE TYPE` command to define a nested table type that is available to all SPL programs in the database. See the *Database Compatibility for Oracle Developer Reference Guide* for more information about the `CREATE TYPE` command.
