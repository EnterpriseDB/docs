...
`count` is the number of elements removed from the end of the collection. EDB Postgres Advanced Server will return an error if `count` is less than `0` or greater than the number of elements in the collection.
