...
In this example, the exception section contains a `ROLLBACK` command. Even though the first two `INSERT` commands are executed successfully, the third causes an exception that results in the rollback of all the `INSERT` commands in the anonymous block.
...
This example uses both `COMMIT` and `ROLLBACK`. First, the following stored procedure is created which inserts a new employee:
...
The following anonymous block is run. The `COMMIT` command is used after all calls to the `emp_insert` procedure and the `ROLLBACK` command in the exception section.
