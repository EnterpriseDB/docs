...
An *autonomous transaction* is an independent transaction started by a calling program. A commit or rollback of SQL commands in the autonomous transaction has no effect on the commit or rollback in any transaction of the calling program. A commit or rollback in the calling program has no effect on the commit or rollback of SQL commands in the autonomous transaction.
...
The following set of examples show the use of autonomous transactions. This first set of scenarios show the default behavior when there are no autonomous transactions.
...
Thiis example shows the effect of declaring a trigger with `PRAGMA AUTONOMOUS_TRANSACTION`.
