...
The `TABLE()` function returns a `SETOF ANYELEMENT` (a set of values of any type). For example, if the argument passed to this function is an array of `dates, TABLE()` returns a `SETOF dates`. If the argument passed to this function is an array of `paths, TABLE()` returns a `SETOF paths`.
