...
You can add the `BULK COLLECT` to the `RETURNING INTO` clause of a `DELETE`, `INSERT`, or `UPDATE` command:
...
This example increases everyone’s salary by 1.5, stores the employees’ numbers, names, and new salaries in three associative arrays, and displays the contents of these arrays:
