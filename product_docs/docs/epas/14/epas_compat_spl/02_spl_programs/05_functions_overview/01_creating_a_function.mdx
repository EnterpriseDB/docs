...
If a schema name is included, then the function is created in the specified schema. Otherwise it is created in the current schema. The name of the new function must not match any existing function with the same input argument types in the same schema. However, functions of different input argument types may share a name (this is called *overloading*). (Overloading of functions is an EDB Postgres Advanced Server feature - overloading of stored, standalone functions is not compatible with Oracle databases).
...
-   `VOLATILE` indicates that the function value can change even within a single table scan, so no optimizations can be made. Any function that has side-effects must be classified volatile, even if its result is quite predictable, to prevent calls from being optimized away.
...
`[ EXTERNAL ] SECURITY DEFINER`
 ...
 `SECURITY DEFINER` specifies for the function to execute with the privileges of the user that created it; this is the default. The key word `EXTERNAL` is allowed for SQL conformance, but is optional.
