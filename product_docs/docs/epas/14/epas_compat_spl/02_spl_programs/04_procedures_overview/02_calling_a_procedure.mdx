...
!!! Note
    ...
    -   The syntax for calling a procedure is the same as in the preceding syntax diagram when executing it with the `EXEC` command in PSQL or EDB\*Plus. See the *Database Compatibility for Oracle Developer Tools and Utilities Guide* for information about the `EXEC` command.
