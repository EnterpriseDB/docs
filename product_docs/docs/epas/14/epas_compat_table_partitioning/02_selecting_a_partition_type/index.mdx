...
!!! Note
    The table does not inherit the primary key, which does not have partition keys in a column list. However, you can create primary keys that do not use the partition columns on individual partitions by using `ALTER TABLE ... ADD CONSTRAINT` for each newly added partition. The primary index enforces uniqueness on each partition and not across the entire partition hierarchy.
