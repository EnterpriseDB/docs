...
Use the `ALTER TABLE… SET SUBPARTITION TEMPLATE` command to create or replace existing range, list, or hash subpartitions for each partitioned table. For more information about creating a subpartition template, see [CREATE TABLE...PARTITION BY](../../04_partitioning_commands_compatible_with_oracle_databases/01_create_table_partition_by/#create_table_partition_by).
