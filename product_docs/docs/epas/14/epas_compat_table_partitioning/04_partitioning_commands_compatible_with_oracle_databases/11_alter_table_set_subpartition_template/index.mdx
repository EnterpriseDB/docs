...
The `ALTER TABLE… SET SUBPARTITION TEMPLATE` command can be used to create or replace existing range, list, or hash subpartitions for each partitioned table. The command creates a new subpartition template or replaces an existing subpartition template. For more information about creating a subpartition template, see [CREATE TABLE...PARTITION BY](../../04_partitioning_commands_compatible_with_oracle_databases/01_create_table_partition_by/#create_table_partition_by).
