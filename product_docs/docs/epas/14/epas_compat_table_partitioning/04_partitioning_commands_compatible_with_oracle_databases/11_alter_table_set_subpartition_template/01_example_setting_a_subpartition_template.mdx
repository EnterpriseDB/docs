...
The following example creates a table `sales` that is list-partitioned by `country` and subpartitioned using list by `date`.  
...
The `SELECT` statement shows partition `q4_2021` consisting of a subpartition with system-generated  name. 
