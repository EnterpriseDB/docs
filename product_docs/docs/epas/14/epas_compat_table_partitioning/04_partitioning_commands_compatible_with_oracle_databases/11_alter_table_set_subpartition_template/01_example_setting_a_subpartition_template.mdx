...
The `sales` table is modified with the subpartition template set to eight. Now, if you try to add a new partition `q1_2013`, a new partition is created consisting of eight subpartitions as described in the subpartition template.
...
Query the `ALL_TAB_PARTITIONS` view, a new partition `east_asia` is created consisting of one subpartition with a system-generated name assigned to them.
