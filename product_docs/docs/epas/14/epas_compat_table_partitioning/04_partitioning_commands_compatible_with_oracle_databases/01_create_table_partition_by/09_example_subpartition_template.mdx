...
The example in this section shows how to create subpartitions in a partitioned table using a subpartition template.

The following example creates a table `(sales)` list-partitioned by `country` and subpartitioned using list by `date`. The `(sales)` table uses a subpartition template and displays the subpartition and tablespace name. 
