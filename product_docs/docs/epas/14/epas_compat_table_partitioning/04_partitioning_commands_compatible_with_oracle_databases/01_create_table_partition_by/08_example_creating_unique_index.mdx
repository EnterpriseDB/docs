...
The following shows that unique index is created for `sales` table.
...
edb=# SELECT oid, relname, relhasindex from pg_class where relname='sales_europe';
  oid  |   relname    | relhasindex 
-------+--------------+-------------
 16536 | sales_europe | t
(1 row)

edb=# SELECT * from pg_index where indrelid='16536';
...
------------+----------+----------+-------------+-------------+--------------+----------------+--------------+----------------+------------+--------------+------------+-----------+----------------+
--------+--------------+----------+-----------+----------+---------
      16540 |    16536 |        1 |           1 | t           | f            | f              | t            | f              | t          | f            | t          | t         | f              |
 1      | 0            | 3124     | 0         |          | 
(1 row)
```
