...
Use the fourth form to create a subpartition template for the partitioned table:

```text
CREATE TABLE <table_name> ( <col_list> ) 
   <partition_description> 

Where partition_description is:   
...
Where partition may be one of the following:

      {<list_partition> | <range_partition> | <hash_partition> }

Where subpartition_description is:
...
example_partition_by_list example_automatic_list_partition example_partition_by_range example_interval_range_partition example_partition_by_hash example_partitions_number example_partition_by_range_subpartition_by_list example_creating_unique_index
