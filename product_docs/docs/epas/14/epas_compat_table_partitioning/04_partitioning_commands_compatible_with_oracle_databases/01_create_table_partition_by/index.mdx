...
Use the `SUBPARTITION TEMPLATE` with a list, range, or hash subpartitioning to create default subpartitions for each table partition for which you do not explicitly define subpartitions. The `subpartition_template_description` in the `SUBPARTITION BY` clause displays a list of subpartition details with subpartition name, subpartition bound, and tablespace name where a subpartition resides. For more information about setting subpartition template, see [ALTER TABLE...SET SUBPARTITION TEMPLATE](../../04_partitioning_commands_compatible_with_oracle_databases/11_alter_table_set_subpartition_template/#alter_table_set_subpartition_template).
