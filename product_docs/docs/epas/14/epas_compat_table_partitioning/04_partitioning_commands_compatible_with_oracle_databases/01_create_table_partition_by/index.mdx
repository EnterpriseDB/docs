...
**Subpartition Template Syntax** 

where subpartition template is:
...
Use the `SUBPARTITION TEMPLATE` with list, range, or hash subpartitioning to define default subpartitions for each table partition. Advanced Server will create default subpartitions in any partition for which you do not explicitly define subpartitions. For more information about setting subpartition template, see [ALTER TABLE...SET SUBPARTITION TEMPLATE](../../04_partitioning_commands_compatible_with_oracle_databases/11_alter_table_set_subpartition_template/#alter_table_set_subpartition_template).
...
`partition_strategy`
 ...
 The `partition_strategy` specifies partition either by list, range, or hash partitioning method.

`subpart_strategy`

 The `subpart_strategy` specifies subpartition either by list, range, or hash subpartitioning method.
