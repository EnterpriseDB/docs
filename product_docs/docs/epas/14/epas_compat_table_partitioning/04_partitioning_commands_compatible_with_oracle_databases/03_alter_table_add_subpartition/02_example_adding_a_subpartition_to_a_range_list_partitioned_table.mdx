...
After executing the above command, the `sales` table has two partitions, named `first_half_2012` and `second_half_2012`. The `first_half_2012 partition` has two subpartitions, named `europe` and `americas`, and the `second_half_2012` partition has one partition, named `asia`.
