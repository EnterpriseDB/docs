...
The `ALIASES` can be used with `SELECT`, `INSERT`, `UPDATE`, or `DELETE` statements to access a partition or subpartition. The following example creates a partitioned table `sales` range-partitioned by `date` and subpartitioned using list partitioning by the `country` column.
...
The following `INSERT` statement creates an alias of the `sales` table and inserts rows into partition `q1_2020` and `q2_2020` or subpartition `q1_europe` and `q1_asia`.
