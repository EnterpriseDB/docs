...
For a partitioned table, you can access the partition or subpartition using `PARTITION part_name` or `SUBPARTITION subpart_name`. The following example creates a partitioned table `sales` that is range-partitioned by `date` and subpartitioned using list partitioning by the `country` column.  
...
The following `INSERT` statement inserts rows into the `sales` table using specific `PARTITION part_name` or `SUBPARTITION subpart_name`.
...
The following `SELECT` statement selects rows from a specific partition or subpartition of a partitioned table by specifying the keyword `PARTITION` or `SUBPARTITION`.  
...
The following `DELETE` statement removes rows from the partition `q2_2020` or subpartition `q2_asia` of the `sales` table.
...
The `ALIASES` can be used with `INSERT`, `UPDATE`, or `DELETE` statements to access a partition or subpartition. The following example creates a partitioned table `sales` range-partitioned by `date` and subpartitioned using list partitioning by the `country` column.
...
The following `INSERT` statement creates an alias of the `sales` table and inserts rows into partition `q1_2020` and `q2_2020` or subpartition `q1_europe` and `q2_asia`.

```text
INSERT INTO sales PARTITION (q1_2020) AS q1_sales VALUES (10, 'q1_2020', 'FRANCE', '2020-Feb-01', '500000');
INSERT INTO sales PARTITION (q2_2020) AS q2_sales VALUES (10, 'q2_2020', 'ITALY', '2020-Apr-01', '550000');
INSERT INTO sales SUBPARTITION (q1_europe) AS sales_q1_europe VALUES (10, 'q1_europe', 'FRANCE', '2020-Feb-01', '600000');
INSERT INTO sales SUBPARTITION (q2_asia) AS sales_q2_asia VALUES (10, 'q2_asia', 'INDIA', '2020-Apr-01', '650000');
```

The following `SELECT` statement selects rows from a specific partition or subpartition of a `sales` table.  
...
edb=# SELECT tableoid::regclass, * FROM sales SUBPARTITION (q2_asia) AS sales_q2_asia ORDER BY country;
   tableoid    | dept_no | part_no | country |        date        | amount 
---------------+---------+---------+---------+--------------------+--------
 sales_q2_asia |      10 | q2_asia | INDIA   | 01-APR-20 00:00:00 | 650000
(1 row)
...
edb=# UPDATE sales SUBPARTITION (q1_europe) AS sales_q1_europe SET sales_q1_europe.amount = 5000 WHERE sales_q1_europe.amount = 600000;
UPDATE 1
```

The following `DELETE` statement removes rows from the partition `q1_2020` or subpartition `q1_europe` of the `sales` table.

```text
edb=# DELETE FROM sales PARTITION (q1_2020) AS q1_sales WHERE q1_sales.amount = 10000;
DELETE 1
