...
`PRESERVE BLANKS`
 ...
 The `PRESERVE BLANKS` option retains leading white space when the optional enclosure delimiters are not present.

 The `NO PRESERVE BLANKS` option works with the delimited and predetermined-sized data and trims only the leading white space. If the control file has a positional column specified using a `POSITION` clause with `PRESERVE BLANKS` or `NO PRESERVE BLANKS` option, then only the trailing whitespaces are trim.
