...
```text
CHAR [(<length>)] | DATE [(<length>)] | TIMESTAMP [(<length>)] [ "<datemask>" ] | INTEGER EXTERNAL [(<length>)] |
...
`PRESERVE BLANKS`
 ...
 -  Retains leading white space when the optional enclosure delimiters are not present.

 -  Leaves trailing white space intact when fields are specified with a predetermined size. When omitted, the default behavior is to trim leading and trailing white space.

 The ` [ NO ] PRESERVE BLANKS` option works with the delimiter clause, trims only the leading white space, and the trailing white space is trim in case of positional specification.

 Consider the field `--a` loaded with the following delimiter clause:

```
TERMINATED BY ',' OPTIONALLY ENCLOSED BY '"'
```

 In this case, if `PRESERVE BLANKS` is specified, then both the leading and trailing whitespace are retained. If `[ NO ] PRESERVE BLANKS` is specified, then the leading whitespace is trimmed.
 
 Now suppose the control file has a positional column specified with a `POSITION` clause; in such case, if `PRESERVE BLANKS` is specified, it includes some of the whitespaces. But if `[ NO ] PRESERVE BLANKS` is specified, then only trailing whitespaces are trimmed.
 ...
 **Note**: If the `DATE` field type is specified along with a SQL expression for the column, then `datemask` must be specified after `DATE` and before the SQL expression. See the following discussion of the `expr` parameter.
 ...
 If the `TIMESTAMP` field type is specified along with a SQL expression for the column, then `datemask` must be specified after `TIMESTAMP` and before the SQL expression. 

 When using the `TIMESTAMP` field datatype, if you specify `time_stamp timestamp "yyyymmddhh24miss"` the `datemask` is converted to the SQL expression. However, in case of `time_stamp timestamp "select to_timestamp(:time_stamp, 'yyyymmddhh24miss')"`, the EDB\*Loader cannot differentiate between datemask and the SQL expression. It treats the third field (SQL expression in the example) as datemask and prepares the SQL expression, which will not be valid. 
...
`PRESERVE BLANKS`
 ...
 The `NO PRESERVE BLANK` option works with the delimiter clause, trims only the leading white space, and the trailing white space is trim in case of positional specification.
