 ...
 **Note**: If the `DATE` field type is specified along with a SQL expression for the column, then `datemask` must be specified after `DATE` and before the SQL expression. See the following discussion of the `expr` parameter.
 ...
 When using the `TIMESTAMP` field datatype, if you specify `time_stamp timestamp "yyyymmddhh24miss"` the `datemask` is converted to the SQL expression. Where:
 
-   `time_stamp` specifies the column name 

-   `timestamp` specifies the datatype

-   `yyyymmddhh24miss` specifies the datemask

However, in case of `time_stamp timestamp "select to_timestamp(:time_stamp, 'yyyymmddhh24miss')"`, the EDB\*Loader cannot differentiate between datemask and the SQL expression. 
 
 If you want to provide an SQL expression then the simple workaround is to specify the datemask and SQL expression using `TO_CHAR` function as: 
