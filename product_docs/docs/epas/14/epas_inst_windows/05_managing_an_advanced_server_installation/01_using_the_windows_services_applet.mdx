...
-   Use the `Stop the service` option to stop the instance of EDB Postgres Advanced Server. Please note that any user (or client application) connected to the EDB Postgres Advanced Server instance is abruptly disconnected if you stop the service.
...
-   Use the `Restart the service` option to stop and then start the EDB Postgres Advanced Server. Please note that any user sessions are terminated when you stop the service. This option is useful to reset server parameters that only take effect on server start.
