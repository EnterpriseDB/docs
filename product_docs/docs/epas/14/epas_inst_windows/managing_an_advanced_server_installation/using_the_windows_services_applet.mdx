...
-   Use the `Pause the service` option to tell EDB Postgres Advanced Server to reload the server configuration parameters without disrupting user sessions for many of the configuration parameters. See [Configuring EDB Postgres Advanced Server](configuring_epas/) for more information about the parameters that can be updated with a server reload.
