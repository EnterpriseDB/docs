...
Although this option is listed when you run the installer with the `--help` option, the `--enable-components` parameter has absolutely no effect on which components are installed. All components are installed regardless of what is specified in `component_list`. In order to install only specific selected components, you must use the `--disable-components` parameter previously described in this section to list the components you do not want to install.
...
If you are installing EDB Postgres Advanced Server in unattended mode, and do not specify a value using the `--serverport` parameter, the installer uses port `5444`, or the first available port after port `5444` as the default listener port.
...
!!! Note
    If you do not include the `--serviceaccount` option when invoking the installer, the `NetworkService` account owns the database service, and the pgAgent service is owned by either `enterprisedb` or `postgres` (depending on the installation mode).
