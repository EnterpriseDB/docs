...
Use the `--prefix` parameter to specify an installation directory for Advanced Server. The installer will append a version-specific sub-directory (that is, `as14`) to the specified directory. The default installation directory is:
...
Use the `--servicename` parameter to specify the name of the Advanced Server service. The default is `edb-as-14`.
