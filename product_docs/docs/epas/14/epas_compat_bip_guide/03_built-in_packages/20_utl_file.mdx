...
| `GET_RAW(file, buffer, len)`                                                   | n/a         | Reads a `RAW` value from a file and returns the number of bytes read.                                                                                              |
...
| `PUT_RAW(file, buffer, autoflush)`                                             | `BOOLEAN`   | Accepts a `RAW` value and writes the value to a specified file.                                                                                                    |
...
The `GET_RAW` procedure reads a `RAW` value from a file and returns the number of bytes read, but not including the end-of-line terminators. An `INVALID_FILEHANDLE`, `INVALID_OPERATION`, and `READ_ERROR` exceptions are thrown when there are no more lines to read.

```text
GET_RAW(<file> FILE_TYPE, <buffer> OUT VARCHAR2, <len> INTEGER)
```
...
`buffer`
 ...
 Variable to receive `RAW` data from the file.

`len`

 The number of bytes read from a file.
...
The `PUT_RAW` procedure accepts a `RAW` value and writes the value to a specified file. An `INVALID_FILEHANDLE`, `INVALID_OPERATION`, `WRITE_ERROR`, and `VALUE_ERROR` exceptions are thrown when there are no more lines to read.

```text
PUT_RAW(<file> FILE_TYPE, <buffer> VARCHAR2, <autoflush> BOOLEAN)
```
...
`buffer`
 ...
 Variable to write `RAW` data to the file.
