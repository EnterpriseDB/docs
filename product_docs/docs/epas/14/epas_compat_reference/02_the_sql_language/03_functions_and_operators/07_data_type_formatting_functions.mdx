...
## Behavioural difference between SUBSTR, TO_CHAR, and TO_NUMBER Functions 

You can initialize the database using the `INITDBOPTS` variable to create clusters in a mode compatible with Oracle databases. The clusters created in PostgreSQL mode do not include compatibility features. To create a new cluster in PostgreSQL mode, use the `"--no-redwood-compat"` option, which offers a similar behavior to PostgreSQL, except the `FF<digit>` modifier is not supported in PostgreSQL. 

Listed are the following differences for `TO_CHAR`, `TO_NUMBER`, and `SUBSTR` functions:

`TO_CHAR`:

A `SELECT` statement that contains one of the function as shown in the following table returns the indicated result in EPAS and PostgreSQL.

| Function                            | Description                                                     | Examples                                                           | Result               |                                        
| ----------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------ | -------------------- |
| `TO_CHAR(timestamp, text)`          | Converts the timestamp to string according to a given format.   | **EPAS** <br /> `SELECT TO_CHAR(current_timestamp, 'of');` <br />  | **EPAS** `+05:30`    |
...
1. The following additional format options not present in PostgreSQL, are supported in EPAS:
tzh, tzm, of, syear, syyyy, year, rrrr, sp, spth, scc, rr, ff<digit>
2. TH modifier adds fewer padding zeros in EPAS.
3. Toggling of the 'FM' pattern that is when we specify an ‘FM’ pattern an even number of times, 
then it turns off the effect of the ‘FM’ pattern, and when we specify an odd number of times, 
then it turns on the effect of ‘FM’ pattern in PostgreSQL. In the case of EPAS, if the ‘FM’ 
pattern is specified irrespective of whether it is even or an odd number of times, it turns on the effect.
4. The upper case 'TH' pattern which displays the suffix in uppercase is not supported in EPAS. 
It always displays the suffix in lower case if the ‘TH/th’ pattern is specified.