...
The `SYS_CONTEXT` function returns the value of `parameter` associated with the context `namespace` in the current session. The signature is:
...
`parameter`
 ...
 `parameter` is a defined attribute of namespace. The following table lists the predefined attributes of `USERENV` namespace:
