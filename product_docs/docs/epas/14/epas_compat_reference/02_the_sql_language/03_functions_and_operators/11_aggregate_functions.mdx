...
The `BIT_AND_AGG` is a bitwise aggregation function that performs a bitwise `AND` operation and returns a `NUMERIC` value.

**Objective**

-  `BIT_AND_AGG` function can be used with `GROUP BY` clause. In this case, the `BIT_AND_AGG` function operates on each group and returns an bitwise aggregated numeric value. 
-  The keywords `DISTINCT` or `UNIQUE` can be used with `BIT_AND_AGG` function. In this case, the `BIT_AND_AGG` function ensure that only unique values in the `expr` are used for computation.
...
The return type is determined by the input data type of `expression`. However, if all the values of a column are `NULL`, the `BIT_AND_AGG` returns `NULL`. The following table illustrates the return type for each input type.
...
The following example returns the bitwise aggregated `comm` value for each department in the `emp` table:
...
edb=# SELECT BIT_AND_AGG(comm) FROM emp;
 bit_and_agg 
-------------
         288
(1 row)
...
The following example uses `GROUP BY` clause and `DISTINCT` keyword with `BIT_AND_AGG` in `emp` table and returns the bitwise aggregated value:

The following example applies `BIT_OR_AGG` function to the `sal` column in a `emp` table and groups the result by `deptno`: 

```text
edb=# SELECT comm,BIT_AND_AGG(DISTINCT sal) FROM emp GROUP BY comm;
  comm   | bit_and_agg 
---------+-------------
  300.00 |     1600.00
  500.00 |     1250.00
...
The `BIT_OR_AGG` is a bitwise aggregation function that performs a bitwise `OR` operation and returns a `NUMERIC` value.

**Objective**

-  `BIT_OR_AGG` function can be used with `GROUP BY` clause. In this case, the `BIT_OR_AGG` function operates on each group and returns an bitwise aggregated numeric value. 
-  The keywords `DISTINCT` or `UNIQUE` can be used with `BIT_OR_AGG` function. In this case, the `BIT_OR_AGG` function ensure that only unique values in the `expr` are used for computation.
...
The return type is determined by the input data type of `expression`. However, if all the values of a column are `NULL`, the `BIT_OR_AGG` returns `NULL`. The following table illustrates the return type for each input type.
...
The following example returns the bitwise aggregated `sal` value for each department in the `emp` table:
...
edb=# SELECT BIT_OR_AGG(sal) FROM emp;
 bit_or_agg 
------------
       4095
(1 row)
