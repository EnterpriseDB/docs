...
-  `BIT_AND_AGG` function can be used with the `GROUP BY` clause. In this case, the `BIT_AND_AGG` function operates on a group and returns a bitwise aggregated numeric value. 
...
The input data type of `expression` determines the return type. However, if all the values of a column are `NULL`, the `BIT_AND_AGG` returns `NULL`. The following table illustrates the return type for each input type.
...
The following example applies the `BIT_AND_AGG` function to the `sal` column in an `emp` table and groups the result by `comm`: 
...
-  `BIT_OR_AGG` function can be used with the `GROUP BY` clause. In this case, the `BIT_OR_AGG` function operates on a group and returns a bitwise aggregated numeric value. 
...
The input data type of `expression` determines the return type. However, if all the values of a column are `NULL`, the `BIT_OR_AGG` returns `NULL`. The following table illustrates the return type for each input type.
