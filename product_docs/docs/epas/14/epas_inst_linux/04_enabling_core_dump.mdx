...
-   To set the core limit for the services, add the following setting in `/usr/lib/systemd/system/edb-as-13.service`.
...
-   To set the core limit for the services, add the following setting in `/lib/systemd/system/edb-as@13-main.service`.
...
-   Now, the core dumps are enabled, install the `gdb` tool and the debug symbols using the following command:   
    ...
    ```text
    apt-get install gdb
    apt-get install gdb edb-as13 edb-as-contrib edb-as13-server edb-debugger ubuntu-dbgsym-keyring
    ```
