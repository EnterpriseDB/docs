...
| edb-as14-server-plpython3        | Installs the PL/Python procedural language for EDB Postgres Advanced Server. Please note that the PL/Python2 support will no longer be available from EDB Postgres Advanced Server version 14 onwards.                                                                                                                                                     |
...
-   On each isolated database server, configure `yum` or `dnf` to pull updates from the mirrored repository on your local network. For example, you might create a repository configuration file called `/etc/yum.repos.d/edb-repo` with connection information that specifies:
    ...
    ```text
    [edbas14]
    name=EnterpriseDB EDB Postgres Advanced Server 14
