...
START COMMENT: *rewrote and moved to Installing on Linux topic*
...
START COMMENT: Moved to new Troubleshooting > Working with Logs topic - need to be rewritten
...
START COMMENT:Delete - Obsolete - install instructions available in MKT and EDBPlus doc
...
START COMMENT:
This is valuable information but needs clarification. Once we have a better understanding of the content and the use cases we will determine where to put it and what updates are required.
...
END COMMENT
START COMMENT If this is still applicable move to Upgrading section
...
END COMMENT
START COMMENT: Need to check if it is appliable to Ubuntu also. Has been moved to Default Components Location topic}
...
END COMMENT
START COMMENT: Same deal as with CentOS RHEL section
...
END COMMENT
START COMMENT: moved to install template for CentOS and RHEL - Nidhi will check for Debian template
...
START COMMENT: Nidhi to check if this is still an option or default behavior - TBD if some of this information needs to move 
...
START COMMENT Needs to be evaluated - some content may go in the template and other content in a separate
