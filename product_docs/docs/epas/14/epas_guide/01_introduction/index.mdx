...
-   Advanced Server now supports `AS` clause to alias a table, view, materialized view, or a subquery to be referenced in the `INSERT` statement. For information, see the *Database Compatibility for Oracle Developer SQL Guide*.
...
-   Advanced Server has added support to accept the `NUMBER` type values in the `DBMS_SQL` function. For information, see the *Database Compatibility for Oracle Developer Built-in Package Guide*.
-   Advanced Server has added support to include default database tablespace `PG_DEFAULT` if the tablespace name is not specified. For information, see the *Database Compatibility for Oracle Developer Catalog Views Guide*. 
-   Advanced Server has added the `GET_RAW` and `PUT_RAW` APIs in the `UTL_FILE` package to read and write the `RAW` data value. For information, see the *Database Compatibility for Oracle Developer Built-in Package Guide*.
...
-   Advanced Server now supports `USERENV` and `SYS_CONTEXT` functions to retrieve information about the current session. For information, see the *Database Compatibility for Oracle Developer Reference Guide*.
-   Advanced Server now supports bitwise function `BITAND` and `BITOR` and bitwise aggregation function `BIT_AND_AGG`, and `BIT_OR_AGG`, which performs a bitwise `AND` or `OR` operation and returns a value based on the data type of input argument. For information, see the *Database Compatibility for Oracle Developer Reference Guide*.
