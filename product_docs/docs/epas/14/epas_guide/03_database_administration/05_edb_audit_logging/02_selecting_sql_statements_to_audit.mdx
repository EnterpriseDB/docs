...
The resulting audit log file contains only the connection authentication information. The setting the `edb_audit_statement` parameter to `none` does not allow auditing any of the SQL statements for `carol`, and thereby no audit logs are generated.
