...
-   `{ select | update | delete | insert }@groupname` – Results in the selective auditing of objects for specific `dml` statements including `(@)` and excluding `(-)` groups on a given table. For more information about object auditing, see [Object Auditing](09_object_auditing).
