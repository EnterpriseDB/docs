...
The `select@low_security@high_security` audits `SELECT` statements of the `low_security` and `high_security` audit groups, `insert@high_security-low_security` audits `INSERT` statements of `high_security` and exclude the insert statements of `low_security` audit group, `update-low_security@high_security` audits `UPDATE` statements of `high_security` and exclude the statements of the `low_security` audit group, `delete@low_security-high_security` audits `DELETE` statements of `low_security` and exclude the statements of `high_security` audit group for a table in the log file.
