...
`edb_audit_archiver_filename_prefix`
 ...
 Specifies the file name of an audit log file that needs to be archived; the file name must align with the `edb_audit_filename` parameter. The default value for `edb_audit_archiver_filename_prefix` will be `audit-`. The audit files with `edb_audit_archiver_filename_prefix` in the `edb_audit_directory` are eligible for compression and/or expiration.

`edb_audit_archiver_compress_time_limit`

 Specifies the time in seconds after which audit logs are eligible for compression. The possible values to set this parameter are: `0` = compression starts as soon as the log file is not a current file, `-1` = compression of file on time basis will not occur.

`edb_audit_archiver_compress_size_limit`

 Specifies a file size threshold in megabytes after which audit logs will be eligible for compression. If the parameter is set to -1, compression of the file on a size basis will not occur.

`edb_audit_archiver_compress_command`

 Specifies the command to execute the compressing of the audit log files. The default value for `edb_audit_archiver_compress_command` is `gzip %p`. The `gzip` provides a standard method of compressing files. The `%p` in the string is replaced by the path name of the file to archive.

`edb_audit_archiver_compress_suffix`

 Specifies the file name of an already compressed log file; the file name must align with `edb_audit_archiver_compress_command`. The default file name will be `.gz`. 

`edb_audit_archiver_expire_time_limit`

 Specifies the time in seconds after which audit logs are eligible for expiration. The possible values to set this parameter are: `0` = expiration starts as soon as the log file is not a current file, `-1` = expiration of file on time basis will not occur.

`edb_audit_archiver_expire_size_limit` 

 Specifies a file size threshold in megabytes after which audit logs will be eligible for expiration. If the parameter is set to -1, the expiration of a file on the size basis will not occur.
...
`edb_audit_statement`
 ...
 This configuration parameter is used to specify auditing of different categories of SQL statements. Various combinations of the following values may be specified: `none`, `dml`, `insert`, `update`, `delete`, `truncate`, `select`, `error`, `rollback`, `ddl`, `create`, `drop`, `alter`, `grant`, `revoke`, `set`, and `all`. The default is `ddl` and `error`. See [Selecting SQL Statements to Audit](02_selecting_sql_statements_to_audit/#selecting_sql_statements_to_audit) for information regarding the setting of this parameter.
