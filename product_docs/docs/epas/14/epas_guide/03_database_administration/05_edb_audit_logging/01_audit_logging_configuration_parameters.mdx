...
`edb_audit_archiver_filename_prefix`
 ...
 Specifies the file name of the audit file that needs to be archived; the file name must align with the `edb_audit_filename` parameter. The default file name will be `audit-`. The audit files with `edb_audit_archiver_filename_prefix` in the `edb_audit_directory` are eligible for compression and/or expiration.

`edb_audit_archiver_compress_time_limit`

 Specifies the time in seconds after which audit logs are eligible for compression. To enable, set this parameter to 0.
...
`edb_audit_archiver_expire_time_limit`
 ...
 Specifies the time in seconds after which audit logs are eligible for expiration. To enable, set this parameter to 0.
