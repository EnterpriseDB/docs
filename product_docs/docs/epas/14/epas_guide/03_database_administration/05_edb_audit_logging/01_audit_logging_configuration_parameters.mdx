...
`edb_audit_archiver_filename_prefix`
 ...
 Specifies the file name of the audit file that needs to be archived; the file name must align with the `edb_audit_filename`. The default file name will be `audit-`. The audit files with `edb_audit_archiver_filename_prefix` in the `edb_audit_directory` are eligible for compression and/or expritation.

`edb_audit_archiver_compress_time_limit`

 Specifies the time in seconds after which audit logs are eligible for compression. To enable this feature, set this parameter to 0.

`edb_audit_archiver_compress_size_limit`

 Specifies a file size threshold in megabytes when audit logs will be eligible for compression. If the parameter is set to -1, compression of the file on a size basis will not occur.

`edb_audit_archiver_compress_command`

 Specifies the command to execute the compressing of the audit log files. The default file name will be `gzip %p`. The `gzip` provide a standard method of compressing files. The `%p` in the string is replaced by the path name of the file to archive.
...
`edb_audit_archiver_expire_time_limit`
 ...
 Specifies the time in seconds after which audit logs are eligible for expiration. To enable this feature, set this parameter to 0.

`edb_audit_archiver_expire_size_limit` 

 Specifies a file size threshold in megabytes when audit logs will be eligible for expiration. If the parameter is set to -1, expiration of the file on a size basis will not occur.
...
`edb_audit_archiver_sort_file`
 ...
 Identifies the oldest file to be sorted alphabetically or based on `mtime`. `mtime` indicates sorting of files based on file modification time; `alphabetic` indicates sorting of files alphabetically based on file name.
