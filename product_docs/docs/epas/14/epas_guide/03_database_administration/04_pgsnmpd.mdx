...
After installing EDB Postgres Advanced Server, you must update the `LD_LIBRARY_PATH` variable. Use the command:
...
Ensure that an instance of EDB Postgres Advanced Server is up and running (`pgsnmpd` connects to this server). Open a command line and assume superuser privileges, before invoking pgsnmpd with a command that takes the following form:
...
Include connection information for your installation of EDB Postgres Advanced Server (in the form of a `libpq` connection string) after the `-C` option.
