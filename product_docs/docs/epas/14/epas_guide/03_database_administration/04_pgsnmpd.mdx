...
!!! Note
    'pgsnnmpd' is deprecated as of EDB Postgres Advanced Server v14. It will not be included in EDB Postgres Advanced Server v15 and later.
...
After installing EDB Postgres Advanced Server, you will need to update the `LD_LIBRARY_PATH` variable. Use the command:
...
Ensure that an instance of EDB Postgres Advanced Server is up and running (`pgsnmpd` will connect to this server). Open a command line and assume superuser privileges, before invoking pgsnmpd with a command that takes the following form:
