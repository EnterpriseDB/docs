...
2.  Create the `index_advisor` extension, which creates the database objects. Connect to the database as the database superuser using psql, and enter the command:
    ...
    !!! Note
        If you're using Index Advisor in earlier version of EDB Postgres Advanced Server and upgrading to latest version, then use this command to create the index_advisor extension:
