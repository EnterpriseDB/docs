---
title: "Using a Package Manager to Install Advanced Server - orig"
---

START COMMENT: *rewrote and moved to Deployment Options topic*
...
START COMMENT??: *this should go in the local repo topic, correct?
...
START COMMENT: Moved to install topic template
...
START COMMENT-WIP: Moved to new Troubleshooting > Working with Logs topic - need to be rewritten
...
START COMMENT: Moved to Installation Details
...
START COMMENT??-Nidhi confirming that this is a requirement for the legacy interactive installers and can be removed
...
START COMMENT??:Delete - Obsolete - install instructions available in MKT and EDBPlus doc - do we need to add a note about this as we are now making the instructions applicable to installing multiple products?
...
START COMMENT: Moved to install template file
...
START COMMENT: moved to templates 
...
-   On Rocky Linux/AlmaLinux 8:
    ...
    ```
END COMMENT
START COMMENT??-Nidhi to check, but probably can remove 
Did we include this in the template? if so, we probably don't need in the tools install template
...
END COMMENT
START COMMENT: included in template
## Installing on RHEL
...
This table lists the core components.
...
END COMMENT
START COMMENT-WIP: Moved to Installation Details folder but yet to link from Installing the Package section
...
END COMMENT
START COMMENT-WIP Compare with the section https://www.enterprisedb.com/docs/epas/latest/epas_upgrade_guide/05_performing_a_minor_version_update_of_an_rpm_installation/
...
START COMMENT Nidhi will review and take anything applicable for the template
...
END COMMENT
START COMMENT: moved to install template for CentOS and RHEL 
...
START COMMENT-Nidhi: Nidhi to check if this is still an option or default behavior - TBD if some of this information needs to move 
...
END COMMENT
START COMMENT-WIP: Need to find a place - Not a part of the initial config. We will try putting it in a folder called Initializing Clusters under Deployment Options. It will talk about the two modes: Postgres and Oracle.
...
END COMMENT
START COMMENT-WIP: Move under Installing on Linux
...
-   Now, the `/opt/edb` location has been labeled correctly with the context, use the following command to start the service:
    ...
    ```
END COMMENT
START COMMENT: moved to Troubleshooting
## Enabling Core Dumps
...
END COMMENT
START COMMENT-WIP: Needs new location - for now move to Managing an Advanced Server Installation
...
-   Start the second cluster with the following command:
    ...
    ```
END COMMENT
START COMMENT: Moved to local repo topic
