---
navTitle: Release notes
title: "EDB Postgres Advanced Server release notes"
navigation:
- epas14_8_0_rel_notes
- epas14_7_0_rel_notes
- epas14_6_0_rel_notes
- epas14_5_0_rel_notes
- epas14_4_0_rel_notes
- epas14_3_0_rel_notes
- 19_epas14.2.1_rel_notes
- 20_epas14_rel_notes
---

EDB Postgres Advanced Server 14 is built on open-source PostgreSQL 14, which introduces myriad enhancements that enable databases to scale up and scale out in more efficient ways. PostgreSQL 14 has significant performance improvements, which includes an improved indexing and lookup system, a level up on query planning when using extended statistics, improved performance for queries that use aggregates or partitioned tables, and providing more ways to monitor activity within a PostgreSQL database. And along with highly requested features like parallelized vacuuming, incremental sorting, PostgreSQL 14 has a multitude of other new features and capabilities.

The EDB Postgres Advanced Server (EDB Postgres Advanced Server) documentation describes the latest version of EDB Postgres Advanced Server 14 including minor releases and patches. The release notes in this section provide information on what was new in each release. 

| Version                               | Release date | Upstream merges                                                                                                  | 
| ------------------------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------- | 
| [14.8.0](epas14_8_0_rel_notes)        | 2023 May 11  | [14.8](https://www.postgresql.org/docs/14/release-14-8.html)                                                     |
| [14.7.0](epas14_7_0_rel_notes)        | 2023 Feb 10  | [14.7](https://www.postgresql.org/docs/14/release-14-7.html)                                                     |
| [14.6.0](epas14_6_0_rel_notes)        | 2022 Nov 10  | [14.6](https://www.postgresql.org/docs/14/release-14-6.html)                                                     |
| [14.5.0](epas14_5_0_rel_notes)        | 2022 Aug 11  | [14.5](https://www.postgresql.org/docs/14/release-14-5.html)                                                     |
| [14.4.0](epas14_4_0_rel_notes)        | 2022 Jun 16  | [14.4](https://www.postgresql.org/docs/14/release-14-4.html)                                                     |
| [14.3.0](epas14_3_0_rel_notes)        | 2022 May 12  | [14.3](https://www.postgresql.org/docs/14/release-14-3.html)                                                     |
| [14.2.1](19_epas14.2.1_rel_notes)     | 2022 Feb 10  | [14.2](https://www.postgresql.org/docs/14/release-14-2.html)                                                     |
| [14.1.0](20_epas14_rel_notes)         | 2021 Dec 01  | [14.0](https://www.postgresql.org/docs/14/release-14.html), [14.1](https://www.postgresql.org/docs/14/release-14-1.html)  |

## Support announcements

### Backup and Recovery Tool (BART) incompatibility

Beginning with version 14, the EDB Backup and Recovery Tool (BART) is incompatible with EDB Postgres Advanced Server. Alternatively, the capabilities delivered by BART can also be delivered by [Barman](https://www.enterprisedb.com/docs/supported-open-source/barman/) and [PgBackRest](https://www.enterprisedb.com/docs/supported-open-source/pgbackrest/).
