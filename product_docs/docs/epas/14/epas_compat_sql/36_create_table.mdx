...
!!! Note
    Advanced Server allows rowids to be created on a foreign table by specifying either `ROWIDS` or `(ROWIDS=true)` option in the `CREATE FOREIGN TABLE` syntax. Specifying the `ROWIDS` or `(ROWIDS=true)` option adds a rowid column to a foreign table, and the rowids are created implicitly like a regular table. For information about `CREATE FOREIGN TABLE`, see the PostgreSQL core documentation at: 
