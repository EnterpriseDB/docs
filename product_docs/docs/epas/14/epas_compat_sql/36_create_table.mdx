...
!!! Note
    Advanced Server allows rowids to be created on a foreign table by specifying `WITH (ROWIDS=true)` option in the `CREATE FOREIGN TABLE` syntax to create rowid columns. For information about `CREATE FOREIGN TABLE`, see the PostgreSQL core documentation at: 
