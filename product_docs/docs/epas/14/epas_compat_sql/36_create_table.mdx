...
!!! Note
    Advanced Server allows rowids to be created on a foreign table by specifying `WITH (ROWIDS=true)` option in the `CREATE FOREIGN TABLE` syntax. Specifying the `WITH (ROWIDS=true)` option adds a rowid column to a foreign table. For information about `CREATE FOREIGN TABLE`, see the PostgreSQL core documentation at: 
