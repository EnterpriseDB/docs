...
After a sequence is created, use the functions `NEXTVAL` and `CURRVAL` to operate on the sequence. These functions are documented in Sequence Manipulation Functions of *Database Compatibility for Oracle Developer Reference Guide*.
