...
The `FROM` keyword is optional if EDB Postgres Advanced Server is installed in Oracle mode. It isn't optional if EDB Postgres Advanced Server is installed in Postgres mode.
