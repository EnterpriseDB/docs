...
The `FROM` keyword is optional keyword if EDB Postgres Advanced Server is installed in Oracle mode. It isn't optional if EDB Postgres Advanced Server is installed in Postgres mode.
