...
!!! Note
    ...
    -  The `OCILobFreeTemporary` function improves Oracle memory usage and frees the contents of the temporary LOB. It is advisable to set `edb_dblink_oci.rescans=serializable` to free a temporary LOB.
...
For example, to set an environment variable for Advanced Server, you can create a file named `/etc/systemd/system/edb-as-14.service`; include `/lib/systemd/system/edb-as-14.service` within the file.
