...
-  The `NOCYCLE` keyword can be used with or without the `CONNECT_BY_ISCYCLE` pseudo-column which, shows if the current row contains a cycle. The possible values for `CONNECT_BY_ISCYCLE` pseudo-column is either 0 or 1; returns 1 when a loop is detected in the `CONNECT BY` query, otherwise returns 0.
   ...
   !!! Note
      If `NOCYCLE` clause is used and a loop is detected, then the query may run out of memory and result in a crash. It is advised to set the configuration parameter `statement_timeout` to a value other than `0` to avoid the crash. 
