...
-  At least one expression in the `CONNECT BY` clause must be qualified with the `PRIOR` operator in a hierarchical query. For example:
...
-  `PRIOR` is a unary operator that can be used to evaluate the parent row in a parent-child relationship in a hierarchical query. `PRIOR` takes the same precedence as `+` and `-` operators.
