...
If a table contains the result set in a hierarchical order, then you can select rows using the hierarchical query clause:

```text
{ CONNECT BY <condition> }
```

`CONNECT BY` specifies the relation between a parent record and its child records of the hierarchy. 

-  In a hierarchical query, one expression in the `CONNECT BY` clause must be qualified with the `PRIOR` operator. For example:
...
-  If the `CONNECT BY condition` is compound, then only one condition requires the `PRIOR` operator, although there can be multiple `PRIOR` conditions.

-  The `PRIOR` keyword can be on either side of the operator.

-  `PRIOR` is a unary operator and has the same precedence as `+` and `-` operators. It evaluates the parent row in a parent-child relation in a hierarchical query.

-  `CURRVAL` and `NEXTVAL` are not valid `PRIOR` expressions, therefore `PRIOR` expression cannot refer to a sequence.

You can further enhance the functionality of the `CONNECT BY [PRIOR]` condition by using the `CONNECT_BY_ROOT` operator. This operator not only returns immediate parent rows but also all ancestors rows in the hierarchy. For more information, see [Retrieving the Root Node with CONNECT_BY_ROOT](../05_hierarchical_queries/06_retrieving_the_root_node_with_connect_by_root/#retrieving_the_root_node_with_connect_by_root).

**Example**

The following example uses the `CONNECT BY` clause with `PRIOR` to define relationship between employees and managers. Using the `LEVEL` value, the employee names are indented to further emphasize the depth in the hierarchy of each row.
...
The following example adds `ORDER BY` clause and `AND` operator with `CONNECT BY` clause to show employee and manager hierarchy.
...
The following example uses `CONNECT BY` clause with optional `PRIOR` statement to show the level value.
