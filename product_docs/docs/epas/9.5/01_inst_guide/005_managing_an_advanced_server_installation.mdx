...
-   Use the Pause the service option to tell Advanced Server to reload the server configuration parameters. The Pause the service option is an effective way to reset the server parameters without disrupting user sessions for many of the configuration parameters. See Section [<span class="underline">6</span>](006_configuring_advanced_server/#configuring-advanced-server), *Configuring Advanced Server* for more information about the parameters that can be updated with a server reload.
