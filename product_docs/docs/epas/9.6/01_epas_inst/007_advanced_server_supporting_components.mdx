...
-   Perform the upgrade to Advanced Server 9.6 using pg_upgrade as described in [Section 8.3](#upgrading-to-advanced-server-9.6-step-by-step), *Upgrading to Advanced Server 9.6*. This upgrades all non-PostGIS databases to Advanced Server 9.6.
...
Please note: At this time, skip step 8 of [Section 8.3](#upgrading-to-advanced-server-9.6-step-by-step). Do not restore the authentication settings in the pg_hba.conf file; the server must use trust authentication when loading the new PostGIS databases from the backup files.
