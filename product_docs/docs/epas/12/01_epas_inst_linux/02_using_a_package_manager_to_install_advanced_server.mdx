...
To create a database cluster in the `PGDATA` directory that listens on the port specified by the `PGPORT` environment variable specified in the service configuration file (described in [Using a Service Configuration File on CentOS or Redhat 6.x](#using-a-service-configuration-file-on-centos-or-redhat-6x)), assume `root` privileges, and invoke the `service` script:
...
-   Use `INITDBOPTS` to specify any `initdb` option or options that you wish to apply to the new cluster. For more information, see [Specifying Cluster Options with INITDBOPTS](#specifying-cluster-options-with-initdbopts).
