...
Once the `autoprewarm` process has finished loading buffers from disk, it will periodically dump shared buffers to disk at the interval specified by the `pg_prewarm.autoprewarm_interval` parameter. See the <span class="title-ref">pg\_prewarm.autoprewarm\_interval &lt;pg\_prewarm.autoprewarm\_interval&gt;</span> for information on the `autoprewarm` background worker. Upon the next server restart, the `autoprewarm` process will prewarm shared buffers with the blocks that were last dumped to disk.
