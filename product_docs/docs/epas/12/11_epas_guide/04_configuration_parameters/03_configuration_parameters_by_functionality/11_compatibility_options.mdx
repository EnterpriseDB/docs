...
-   pg\_dump: If a table includes `OIDs` then specify `--convert-oids-to-rowids` to dump a database. Otherwise, ignore the `OIDs` to continue table creation on Advanced Server version 12 onwards.
-   pg\_upgrade: Errors out. But if a table includes `OIDs` or `ROWIDs`, then you must perform the following steps:
