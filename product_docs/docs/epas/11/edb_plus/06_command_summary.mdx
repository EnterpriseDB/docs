 ...
 **Note**: When a password is not required, EDB\*Plus does not prompt for a password such as when the `trust` authentication method is specified in the `pg_hba.conf` file. For more information about the `pg_hba.conf` file and authentication methods, see the PostgreSQL core documentation at <https://www.postgresql.org/docs/11/static/auth-pg-hba-conf.html>
>>>>>>> 90431ffacb078971e385b08a2128041d460a180f
