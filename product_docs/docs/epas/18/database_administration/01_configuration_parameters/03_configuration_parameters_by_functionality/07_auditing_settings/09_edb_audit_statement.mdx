...
**Range:** `{none | ddl | dml | insert | update | delete | truncate | select | error | create | drop | alter | grant | revoke | rollback | set | copy | all | { select | update | delete | insert }@groupname} ...`
...
-  To audit `COPY TO` and `COPY FROM` statements, include the parameter value `copy`. This option can also be combined with `edb_audit_group`, for example - `set edb_audit_statement ='copy@high_security';`
