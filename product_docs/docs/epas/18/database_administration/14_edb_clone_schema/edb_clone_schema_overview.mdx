...
-   The `edb_util.remove_log_file_and_job` function removes logs and scheduled jobs by job ID or log file name. This function doesn't work using the job ID on EDB Postgres Advanced Server 17 because, in this version, job scheduling is done by `edb_job_scheduler` and not by pgAgent. pgAgent is used for job scheduling in versions 14 and 15.