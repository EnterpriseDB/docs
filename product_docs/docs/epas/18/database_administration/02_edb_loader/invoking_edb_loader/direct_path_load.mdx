...
!!! Note
    If transparent data encryption (TDE) is enabled, a direct path load isn't supported. Using the `DIRECT=TRUE` option returns the following error:

    ```
    ERROR: direct load is not possible with data encryption enabled
    ```
