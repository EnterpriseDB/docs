...
By default, data files reside under the `/var/lib/edb/as18/data` directory. To use a data directory that resides in a nondefault location:
...
-   In the `/etc/systemd/system/edb-as-18.service` file, update the following values with the new location of the data directory:
...
By default, the data files reside under the `/var/lib/edb/as18/data` directory. To change the default data directory location depending on individual environment preferences, you must configure the SELinux policy:
...
-   Create a file `edb-as-18.service` under `/etc/systemd/system` to include the location of a new data directory:
