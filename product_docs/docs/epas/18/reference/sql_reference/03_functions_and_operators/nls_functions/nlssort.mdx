...
This example shows how NLSSORT function behavior is different in EPAS as compared to Oracle:

```sql
CREATE VIEW test_view_1 AS SELECT * FROM DB2621_tab WHERE NLSSORT(name, 'NLS_SORT = XDanish') BETWEEN NLSSORT('Z', 'NLS_SORT = XDanish') AND NLSSORT('ä', 'NLS_SORT = XDanish') ORDER BY NLSSORT(name, 'NLS_SORT = XDanish');
```

This example shows how to read data from the above created view:
