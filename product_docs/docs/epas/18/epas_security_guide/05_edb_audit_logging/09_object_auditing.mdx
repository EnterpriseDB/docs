...
## Auditing system catalog tables

Starting with EPAS version 18, superusers can now enable auditing on system catalog tables (`pg_catalog`tables). Previously, attempts to set audit parameters on these tables would result in a permission error. With this enhancement, superusers can configure auditing for system tables using the `edb_audit_group` parameter.
...
Enable the auditing of pg_class table for low_security group:
...
Execute a select statement on pg_class table:

```sql
# This statement will NOT be audited as the edb_audit_statement is set to low_security only:
edb=# select * from pg_class limit 2 ;
```

```sql
# set to pg_class table for high_security group:
...
```sql
# This statement will be audited and information will be added to log file:
edb=# select * from pg_class limit 2 ;
```