...
- **edb_audit** &mdash; Enables auditing. Set this to `xml` or `csv` to define the log format.

- **edb_audit_directory** &mdash; Defines the physical location on the server where audit files are stored.

- **edb_audit_rotation_day** &mdash; Specifies the day of the week for audit file rotation.

- **edb_audit_connect** &mdash; Set to all to log every successful connection attempt.

- **edb_audit_disconnect** &mdash; Set to all to log every disconnection.

- **edb_audit_statement** &mdash; Controls which SQL statements are captured.

## Refining audit scope

While setting edb_audit_statement in the configuration file applies to the entire cluster, you can override this behavior at more specific levels using the following hierarchy:

| Level            | Command                                        | Scope & Override Behavior                                                               |
|------------------|------------------------------------------------|-----------------------------------------------------------------------------------------|
| Database         | ALTER DATABASE dbname SET ...                  | Overrides the global configuration file for all sessions in dbname.                     |
| Role             | ALTER ROLE rolename SET ...                    | Overrides both the global config and database-level settings for rolename.              |
| Role in Database | ALTER ROLE rolename IN DATABASE dbname SET ... | The most specific setting overrides all other levels when rolename is active in dbname. |
