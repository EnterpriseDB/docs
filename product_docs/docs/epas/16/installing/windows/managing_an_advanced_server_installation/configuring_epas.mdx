...
```text
C:\Program Files\edb\as16\pgplus_env.bat
```

As the `pgplus_env.bat` script executes, it sets the following environment variables:

```text
PATH="C:\Program Files\edb\as16\bin";%PATH%
EDBHOME=C:\Program Files\edb\as16
PGDATA=C:\Program Files\edb\as16\data
PGDATABASE=edb
REM @SET PGUSER=enterprisedb
PGPORT=5444
PGLOCALEDIR=C:\Program Files\edb\as16\share\locale
```

If you used an installer created by EDB to install PostgreSQL, the `pg_env` script performs the same function:

```text
C:\Progra~1\PostgreSQL\16\pg_env.bat
```
