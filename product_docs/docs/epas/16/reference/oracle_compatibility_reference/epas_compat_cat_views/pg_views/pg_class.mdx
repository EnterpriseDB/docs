...
| --------------------- | ------- | ----------------------------------------------------------------------------------------------------- |
| relbackingtable       | "char"  | Identifies whether a table is backing relation for any other object. The default value is `d` for a normal object and `q` for queue tables. We store the relbackingtable as `q` for all the backing objects of QUEUE TABLE (backing table, index, toast table, and so forth). |
