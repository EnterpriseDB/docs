...
Including `jsonlog` as a value for the `log_destination` parameter (see [Summary of configuration parameters](../02_summary_of_configuration_parameters/)) allows you to import log files into different programs. This option generates log lines in the JSON format. 
