...
> This form adds a new constraint to a table; for details, see [CREATE TABLE](#create_table).
...
You can use `ALTER TRIGGER ...ON AUTHORIZATION` command to alter a trigger's implicit object owner, for information, see [ALTER TRIGGER](#alter_trigger).
...
Changing any part of a system catalog table is not permitted. Refer to [CREATE TABLE](#create_table) for a further description of valid parameters.
...
[CREATE TABLE](#create_table), [DROP TABLE](04_partitioning_commands_compatible_with_oracle_databases/#drop_table)
