...
A dump/restore using `pg_dumpall` or `pg_upgrade` or logical replication is required for migrating data from any previous release. See <span class="title-ref">Upgrading an Installation With pg\_upgrade &lt;upgrading\_an\_installation\_with\_pg\_upgrade&gt;</span> for information on migrating to new major releases.
