...
A dump/restore using `pg_dumpall` or `pg_upgrade` or logical replication is required for migrating data from any previous release. See <span class="title-ref">Upgrading an Installation With pg_upgrade &lt;upgrading_an_installation_with_pg_upgrade></span> for information on migrating to new major releases.
