...
-   Added support for DBMS\_SQL function/procedures (`DEFINE_COLUMN_LONG`, `COLUMN_VALUE_LONG` and `LAST_ERROR_POSITION`).
...
-   Added support for utl\_http.end\_of\_body exception. This feature declares the end\_of\_body exception into utl\_http package and throws the same from read\_line, read\_text, and read\_raw package procedures when no data is left in the response body.
