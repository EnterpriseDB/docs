...
The `SQL%FOUND` conditional expression returns `TRUE` if a row is updated, `FALSE` otherwise. See [Obtaining the Result Status](../04_basic_statements/#obtaining_the_result_status) for a discussion of `SQL%FOUND` and other similar expressions.
...
> **Note:** The `UPDATE` command can be included in a `FORALL` statement. A `FORALL` statement allows a single `UPDATE` command to update multiple rows from values supplied in one or more collections. See [Using the FORALL Statement](../12_working_with_collections/#using_the_forall_statement) for more information on the `FORALL` statement.
