...
**Note:** The `UPDATE` command can be included in a `FORALL` statement. A `FORALL` statement allows a single `UPDATE` command to update multiple rows from values supplied in one or more collections. See [Using the FORALL Statement](../12_working_with_collections/03_using_the_forall_statement/#using_the_forall_statement) for more information on the `FORALL` statement.
