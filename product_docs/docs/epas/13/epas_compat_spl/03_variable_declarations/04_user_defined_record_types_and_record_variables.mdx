...
Records can be declared based upon a table definition using the `%ROWTYPE` attribute as shown in <span class="title-ref">Using %ROWTYPE in Record Declarations &lt;using\_%\_row_type_in_record_declarations></span>. This section describes how a new record structure can be defined that is not tied to any particular table definition.
