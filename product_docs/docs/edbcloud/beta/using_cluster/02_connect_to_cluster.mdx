...
You can connect to your cluster using [`psql`](http://postgresguide.com/utilities/psql.html), the terminal-based client for Postgres, or another client. See [Recommended Settings for SSL Mode](recommended-setting-for-ssl-mode) for EDB's recommendations for secure connections.
