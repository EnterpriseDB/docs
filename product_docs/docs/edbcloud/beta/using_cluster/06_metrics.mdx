...
A variety of metrics are collected by the EDB Cloud instance and made available
...
## Understanding EDB Cloud Logs and Metrics
...
| InsightsMetrics | Metrics streams from EDB Cloud Prometheus and Azure Monitor. EDB Cloud metrics have `namespace == "prometheus"` |
...
EDB Cloud collects a wide set of metrics about postgres instances into the
...
| postgresql                     | EDB Cloud postgres cluster identifier e.g. `p-abcdef012345` |
...
In addition to postgres metrics from Cloud Native PostgreSQL, additional
metrics about Kubernetes cluster state and other details may be streamed to the
Log Workspace. Any such metrics are generally well-known metrics from widely
used tools, documented by the upstream vendor of the component.
