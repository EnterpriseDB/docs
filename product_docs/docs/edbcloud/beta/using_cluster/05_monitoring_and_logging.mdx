...
You can monitor your Postgres clusters using Postgres Enterprise Manager or by viewing the metrics and logs from Azure.

## Monitoring with Postgres Enterprise Manager

To monitor your cluster using Postgres Enterprise Manager, you need to register your cluster as a Postgres Enterprise Manager server and enable remote monitoring. See the following topics in the Postres Enterprise Manager documentation: 
- [Registering a Server](../../../pem/latest/pem_admin/02_registering_server/) for information on registering your cluster as a Postgres Enterprise Manager server.
- [Remote Monitoring Capabilities](../../../pem/latest/pem_admin/03a_pem_define_azure_instance_connection/) for details on what level of monitoring is available.
