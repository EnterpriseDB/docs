...
EDB Cloud resource virtual network, this provides higher level isolation
and security to your EDB Cloud cluster.

All other resources you created in the same virtual network can acce ss
private EDB Cloud cluster, but we strongly discourage you from
provisioning any other Azure resources in dedicated EDB Cloud resource
virtual network.

Instead, you can use the following approaches to connect to your
EDB Cloud cluster in a private network.

## Connect from Other Azure Resources

There are three different Azure connectivity solutions you can use to
connect to your cluster from Azure applications outside of
cluster VNet. They have different levels of availability and security,
among them Azure Private Endpoint is the recommended one which is
applicable to most of your use scenarios, but you can pick others
...
Let's use an example to guide you setting up the connection step by
step. In this example, you will access your EDB Cloud cluster on
subscription `development` from a VM on another subscription `test`.

### Prerequisites

 - Your EDB Cloud cluster url, you can get them from the EDB Cloud portal **Connect** tab of your cluster instance.
...
bringing the service into your VNet.

Using private endpoints to connect to your EDB Cloud PG service from
another virtual network is a recommended solution from us. This solution
keeps network isolation as much as possible, as you only grant access to
a single PG service instead of the whole EDB Cloud resource virtual
network. Vice versa, the connectivity through the private endpoint is
unidirectional, EDB Cloud machinery can't initiate any traffic to your
application virtual networks. Additionally, there is no IP address
...
which are available to be connected via private endpoint.
...
group, in the virtual network where the EDB Cloud cluster resides, e.g.
...
on this page, the client application may need to request access
...
After the Private Link Service is created, note its `alias` down. `Alias`
is the unique ID for your private service, it can be shared to any of
your service consumers offline. You can either get it from Azure portal
or by the following CLI command:
...
In next steps, connect this private endpoint to prior EDB Cloud private
...
Configure P2S for VPN gateway vpng-edbcloud, from Azure virtual network
gateway portal, "Point-to-site configuration" page.
...
After you configured the "Point-to-site configuration" in step 3, you
can download a "VPN client configuration files for P2S certificate
authentication", it is a downloadable zip file which contains all the
needed manifests you need for setting up your VPN client in different
operating systems.

IMAGE PLACEHOLDER

Follow this link, you can find the detailed [[configuration guide for
...
There are other enterprise level connectivity solutions which have
higher bandwidth and throughputs, but with higher cost and need
dedicated devices or private circuits from internet providers. Please
