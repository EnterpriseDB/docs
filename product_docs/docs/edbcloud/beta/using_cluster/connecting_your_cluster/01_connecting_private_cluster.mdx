...
connect to your cluster from Azure cloud applications. 
...
Let us use an example to guide you through the different methods of setting up the connection. 
In this example, assume that your EDB Cloud cluster is on a subscription called `development` 
...
A private endpoint uses a private IP address from your VNet, effectively
bringing an external service into your VNet. It is a standard connectivity option in most Azure 
homebrew services, such as CosmosDB, which have have built-in [private link
resources](https://docs.microsoft.com/en-us/azure/private-link/private-endpoint-overview#private-link-resource)
that you can connect via a private endpoint.

This solution provides maximum network isolation, because you only grant access to
...
unidirectional. EDB Cloud machinery can not initiate any traffic to your
application from the virtual networks. Additionally, there is no risk of a conflict in IP addresses in this solution. 
...
in your EDB Cloud cluster resource group, in the virtual network where the EDB Cloud cluster resides, for example,
...
create an [Azure Private Endpoint](https://docs.microsoft.com/en-us/azure/private-link/private-endpoint-overview) 
in your VM's virtual network. The private endpoint can expose the private PG service with a private IP of
VM's virtual network, making it directly accessible to the VM.
