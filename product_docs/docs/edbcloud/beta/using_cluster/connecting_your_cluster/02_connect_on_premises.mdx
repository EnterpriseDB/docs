...
Sometimes your applications are deployed on your on-premises
environments, or on other Cloud providers instead Azure, but they also
want to consume private EDB Cloud services from these premises via a
secure approach. In these cases, you can set up a secure tunnel
between your on premises network and Azure virtual network. There are a
...
These are the steps to connect using the Azure VPN Gateway Point-to-Site option. To walk through this example in you own environment, see the prerequisites described in [Connection Walkthrough Examples](../connecting_your_cluster/#connection-walkthrough-examples).
