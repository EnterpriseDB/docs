...
#### Step 1: Create an Azure Private Link Service tied to your EBD Cloud PG service
  ...
  1. Get the resource group details from Azure CLI or Azure portal and note down the resource group name. For example, if the EDB Cloud cluster's virtual network is `vnet-japaneast`, use the following command: 
  ...
  1. Enter the resource group name obtained in the previous step. 
...
1. In the Azure portal search bar, type *Private DNS Zones* in the search text box and press Enter.
...
1. Click the **Virtual network** link on the **Private DNS Zone** page of `brcxzr08qr7rbei1.edbcloud.io` and link the private DNS Zone to the VM's virtual network
`vnet-client`.
