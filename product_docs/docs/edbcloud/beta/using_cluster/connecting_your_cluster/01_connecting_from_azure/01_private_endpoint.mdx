...
#### Step 1: Create an Azure Private Link Service in each EDB Cloud cluster
 
 Perform this procedure for every EDB Cloud cluster you want to connect to in Azure. 
 
 In this example, you will create an [Azure Private Link Service](https://docs.microsoft.com/en-us/azure/private-link/private-link-service-overview) in your EDB Cloud cluster resource group. 
...
#### Step 2: Create an Azure Private Endpoint in each client virtual network
  ...
  Perform this procedure for every client application in Azure.

  In this example you will create an Azure Private Endpoint in the client VM's virtual network. After you have created the private endpoint, you can use its private IP address to access the PG service.  
...
11. You have now successfully built a tunnel between your client VM's virtual network and the EDB Cloud cluster network. You can access the private PG service by using the private endpoint:
...
12. From the client VM `vm-client`, access the PG service by using the private IP address.
...
1. Select the **Virtual network** link on the **Private DNS Zone** page of `brcxzr08qr7rbei1.edbcloud.io` and link the private DNS Zone to the client VM's virtual network
`vnet-client`.
