...
1. In the search box at the top of the Azure portal, enter "Virtual networks". When Virtual networks appear in the search results, select it. Don't select Virtual networks (classic), as you can't create a peering from a virtual network deployed through the classic deployment model.
...
Access the cluster with its domain name from your cluster's connection string. It is accessible from `vnet-client` after peering.
