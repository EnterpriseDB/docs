...
## Azure Private Endpoint (recommended)
 ...
 address from your Azure Virtual Network (VNet) to an external service. You only grant access to a single cluster instead of the entire EDB Cloud resource virtual network, thus ensuring maximum network isolation. Private Endpoints are the same mechanism used by first-party Azure services such as CosmosDB for private VNet connectivity. For more information, see [Azure Private Endpoint](https://docs.microsoft.com/en-us/azure/private-link/private-endpoint-overview).
