...
PostgreSQL clusters in EDB Cloud are continuously backed up through a combination of base backups and transaction log (WAL) archiving. When a new cluster is created, an initial "base" backup is taken; thereafter, every time a WAL file is closed - by default, every 5 minutes - it will be automatically uploaded to Azure Blob Storage.
