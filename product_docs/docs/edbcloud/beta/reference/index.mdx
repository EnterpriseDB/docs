...
curl --request POST \
  ...
  --data device_code=<device_code returned from your [request for a device code](#request-the-device-code)> \
  --data 'client_id=pM8PRguGtW9yVnrsvrvpaPyyeS9fVvFh'
```
...
The `refresh_token` is retrieved in when you [requested the token](#request-the-device-code)). 

The `client_id` is always the same one which is retrieved when you [queried the authentication endpoint](#query-the-authentication-endpoint).
