...
!!!note
The access token obtained at this step will be used in the next step to exchange the EDB Cloud token only.
!!!
...
The token got from this step is still the raw access token, you still need to follow the step of [Exchange for EDB Cloud token](#exchange-the-edbcloud-token-using-curl) for exchanging an EDB Cloud token.
