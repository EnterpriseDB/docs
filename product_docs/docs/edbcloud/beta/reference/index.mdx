...
You can perform all the steps using CURL, but EDB provides an optional script to simplify getting and refreshing your tokens. The script automates steps 2-4. See [Using the get-token Script](#using-the-get-token-script) for details.
...
### Use get-token to Get the Tokens
...
###  Use get-token to Refresh the Token
