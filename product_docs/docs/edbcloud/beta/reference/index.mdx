...
EDB provides an optional script to simplify getting and refreshing your tokens. Steps 2-4 are automated by the script. See [Using the get-token Script](#using-the-get-token-script) for details.
...
If you are not using the get-token script, use the following call to get a device code:
```
curl --request POST \
...
If you are not using the get-token script, use the following steps to authorize as a user:
...
If you are not using the get-token script, use the `curl --request POST` command to request a token. For example,
```
curl --request POST \
...
### Use get-token to Get your Tokens
...
###  Use get-token to Refresh your Token
 To use the script to refresh your token, use the script with the --refresh option. For example,
