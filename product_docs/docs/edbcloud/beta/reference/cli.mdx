...
Use EDB Command Line Interface (CLI) for EDB Cloud management activities such as cluster provisioning and getting cluster status from your terminal. The EDB CLI is an efficient way to integrate with EDB Cloud, it enables system administrators and developers to script and automate the EDB Cloud administrative operations.
