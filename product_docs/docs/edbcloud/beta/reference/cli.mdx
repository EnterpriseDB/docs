...
- For Linux operating systems, use the following command to get the latest version of the binary executable: 
   ```
   curl -LO "https://cli.biganimal.com/download/latest/$(uname -s)/$(uname -m)/biganimal"
   ```
...
- For Linux operating systems, including Linux running from MacOS platforms: 
  1. Download the `checksum` file with following command:
     ```
     curl -L0 "https://cli.biganimal.com/download/latest/$(uname -s)/$(uname -m)/biganimal.sha256"
     ```
...
- For Windows:
  ...
  2. Validate the binary executable file against the checksum file using `CertUtil`:
    ...
    ```
## Command reference
Use the `-h` or `--help` flags for more information on the CLI commands. You can use the flag on the `animal` command to get a listing of all the available commands (`animal -h`) or on a sub-command to get information on that particular command (for example, `animal create-cluster -h`). 
...
```
You can create multiple credentials for different BigAnimal accounts, then set one as context of your current management session. Use `show-credentials` to list all available credentials and use `set-context-credential` to set a default credential for the current context. For example:
```
...
```
You are prompted to confirm you want to create the cluster and after the cluster creation process is completed, it generates a cluster ID. 
```
$ biganimal create-cluster
........
...
You can use the `--clusterConfigFile` command to create one or more clusters with the same configuration in a non-interactive mode.
...
You can update following attributes:
- Cluster name
...
For example, to enable high availability, use the `--high-availability` option. For example,
```sh
$ biganimal update-cluster --name my-biganimal-cluster --provider azure --region eastus --high-availability true
```

To check the setting took effect, use the `show-clusters` command and view the detailed cluster information output in json format. For example, 
```sh
...
edbcloud CLI is installable on a variety of Linux platforms, macoOS and Windows. Download the executable binary from following links for your operating systems.
...
Or for the Linux operating system, your can download the latest release of edbcloud with the following command.
...
The initial running of `edbcloud` CLI will create a hidden configuration folder under your user root directory. For example, for Linux it’s “${HOME}/.edb-cli” . edbcloud persists cli configuration file in this directory, as well as credentials you retrieved for accessing edbcloud platform.

Don’t edit files in this directory directly, instead, edbcloud provides you with the following subcommand to query and update the configuration.
...
```
edbcloud get-confirm                 Get the current value for the confirmation mode
edbcloud enable-confirm              Enable confirmation when creating/deleting/updating resources
edbcloud disable-confirm             Disable confirmation before creating/deleting/updating resources
```
...
```
edbcloud get-prompt                  Get the current value for the prompt mode
edbcloud enable-prompt               Enable prompting for missing required parameters
edbcloud disable-prompt              Disable prompting for missing required parameters
```
...
```
edbcloud get-output                  Get the current output mode
edbcloud set-output                  Set the output mode, valid modes are [ json, yaml, xml, table ]
```
