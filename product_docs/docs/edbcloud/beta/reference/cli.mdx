...
## Installing the CLI 
The CLI is available for Linux, MacOS, and Windows.
...
## Before you begin
Before using the CLI to manage BigAnimal, you need to authenticate as a valid BigAnimal platform user. Use the `create-credential` command to authenticate yourself, get an access token, and store it in a local credential. For example:
...
```
You can create multiple credentials for different BigAnimal accounts, then set one as context of your current management session. Use `show-credentials` to list all available credentials and you set `set-context-credential` to set a default credential for the current context.
```sh
...
The initial running of the CLI will create a hidden configuration folder under your user root directory. For example, for Linux it’s `${HOME}/.edb-cli`. The CLI persists the configuration file in this directory as well as the credentials.
...
Check your cluster was created successfully using the status cluster command:
```
...
You can use the `--clusterConfigFile` to create one or more clusters with the same configuration in a non-interactive mode.
...
---
 ...
 volumeType:
   ...
   storageClass: managed-premium         # <string:      storage class type>
...
For example, to enable high availability, use the `--high-availability` flag. For example,
```sh
...
```
You can list all deleted clusters with the `show-deleted-clusters` command and restore them from their history backups if you need.



## Command Reference

See [Command Reference](biganimal) full list of reference documentation. 
