...
Alternatively, you can use the `-h` or `--help` flags for more information. You can use the flag on the `animal` command to get a listing of all the available commands (`animal -h`) or on a sub-command to get information on that particular command. 
