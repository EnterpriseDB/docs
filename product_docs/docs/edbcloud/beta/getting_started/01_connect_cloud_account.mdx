...
Check that your subscription allows for a sufficient number of vCPU cores and a sufficient number of Public IP Addresses. See [Azure Resource Limits](#Azure-Resource-Limits).

Create an Azure Active Directory Application client to delegate Identity and Access Management functions to Azure AD. See [Azure AD Application](#Azure-AD=Application).

Gather information needed for the subscription form. See [Required Parameters](#Required-Parameters).
...
You need access to the values of parameters described in [Required Parameters](#Required-Parameters) to fill out the form.
