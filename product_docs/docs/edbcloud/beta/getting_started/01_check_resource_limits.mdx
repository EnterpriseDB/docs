...
Your Postgres clusters deployed in the region will use separate ESv3 virtual machine cores.
The number of cores depends on Instance Type and High Availability option of the clusters your provision. You can calculate the number of ESv3 cores required for your cluster  based on the following:

* Virtual Machine Instance of type E{N}Sv3 uses {N} cores. For example, instance of type E64Sv3 uses 64 ESv3 cores.
* A cluster running on E{N}Sv3 instance with High Availability- Not Enabled, uses exactly {N} ESv3 cores.
* A cluster running on E{N}Sv3 instance with High Availability- Enabled, uses 3* {N} ESv3 cores.

As an example, if you provision the largest virtual machine E64Sv3 with High Availability-Enabled, it will require  (3*64)=192 ESv3 cores per region. EDB Cloud infrastructure will require additional 6 ESv3 and 6 DSv2 virtual machine cores per region.
