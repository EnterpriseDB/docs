...
The number of cores depends on the Instance Type and High Availability option of the clusters you provision. You can calculate the number of ESv3 cores required for your cluster based on the following:
...
As an example, if you provision the largest virtual machine E64Sv3 with High Availability- Enabled, it will require (3*64)=192 ESv3 cores per region. EDB Cloud infrastructure will require an additional 6 ESv3 and 6 DSv2 virtual machine cores per region.
