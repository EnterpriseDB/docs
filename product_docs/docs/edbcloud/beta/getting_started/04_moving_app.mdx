...
```shell
pg_dump --format custom --schema=public --blobs $PGDATABASE > chinook.dump
```

This uses a binary format (`--format custom`) to efficiently capture the schema and data into the file `chinook.dump`. 
It will capture *only* the public schema (`--schema=public`) and associated data (including large objects, `--blobs`) for 
the database specified in the `$PGDATABASE` connection string (chinook). 
...
Once again, you'll be prompted to enter your admin password for this.
...
pg_restore: from TOC entry 4766; 0 0 COMMENT SCHEMA public postgres
...
!!! Note The errors are expected
    You can safely ignore these errors: because we specified the "public" schema explicitly, 
    the dump file contained entries for recreating it - but of course as the *default* schema, 
    "public" already exists and cannot be modified (at least, not by `edb_admin`).
