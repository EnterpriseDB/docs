...
For the destination, you'll create a connection string using the information you collected above. For convenience, let's save those in another environmental variable: use the format `postgres://USERNAME:PASSWORD@HOST:PORT/DATABASE?sslmode=require`. 

```shell
export PGDEST=postgres://edb_admin:password@something.edbmp.edbcloud.io:5432/edb_admin?sslmode=require
```

!!! Warning
    Even though you're working with this information in the privacy of your own account, please be mindful of the fact that you've now stored sensitive credentials in a place where they might be retrieved later; for safety, you should change your password 
...
Before we import the schema and data that we exported in the previous step, we'll want to modify the connection string to point to the new `chinook` database. Use the same password and host you identified earlier, as we still need write access!

```shell
export PGDEST=postgres://edb_admin:password@something.edbmp.edbcloud.io:5432/chinook?sslmode=require
```

Now we can bring in the schema and data using `pg_restore`:

```shell
pg_restore --no-owner -d $PGDEST chinook.dump
```

The option `--no-owner` ensures that pg_restore does not try to assign ownership of objects to their original owner; instead they will be owned by edb_admin, and thus safe from modification unless we connect as that user.
...
one that points at the newly-created database and authenticates with the newly-created (read-access-only) user. Look up your cluster's host again, and compose a connection string in the form, `postgres://chinook:chinook@HOST:5432/chinook?sslmode=require` - then overwrite the original connection string (stored in `PGDATABASE`):

```shell
export PGDATABASE="postgres://chinook:chinook@something.edbmp.edbcloud.io:5432/chinook?sslmode=require"
```
