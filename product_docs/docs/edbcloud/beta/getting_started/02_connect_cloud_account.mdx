...
To simplify the process of creating an Azure AD Application, EDB provides the `create-spn` script for Azure API users. The script automates the manual steps described in the previous section. You can download it [here](https://github.com/EnterpriseDB/cloud-utilities/tree/main/azure/create-spn.sh).
...
|  -y *YEARS* | --years *YEARS* | [Optional] The Number of years for which the credentials will be valid. Only accept positive integer value. Default: 1 year. |
|  -h | --help | Option to display help |
...
```
./create-spn.sh --display-name "hello-s" --subscription c808xxxx-xxxx-xxxx-xxxx-xxxxxxxxb712 -y 2
