...
1. From the migration host, use the `pg_dump` command to export the source database into the target database in EDB Cloud. For example:  
...
The command generates a custom archive called `app.dump` which contains the compressed dump of the source database, `app` in the example. The duration of the command execution varies depending on several variables, including size of the database, network speed, disk speed, and CPU of both the source instance and the `pg-migration` host. You can inspect the table of content of the dump with `pg_restore -l app.dump`.

As with any other custom format dump produced with `pg_dump`, you can take advantage of the features that `pg_restore` provides you with, including selecting a subset of the import tasks by editing the table of contents and passing it to the `-L` option. For further information, please refer to the `pg_restore` documentation.
Alternatively, if you are familiar with `pg_dump`, you can also explore the possibility to run it in parallel using the `-j` option in conjunction with the directory format.
