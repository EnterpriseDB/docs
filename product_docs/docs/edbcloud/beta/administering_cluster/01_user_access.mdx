...
The available `actions` are: _create_, _read_, _update_, _delete_

The available `objects` are: _backups_, _billing_, _databases_, _events_, _permissions_, _roles_, _tickets_, _users_,  _versions_
...
!!!Note: Changing role permissions will affect every user who is assigned the same role. 
