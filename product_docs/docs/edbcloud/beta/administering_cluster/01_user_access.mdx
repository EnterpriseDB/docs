...
EDB Cloud uses Azure Active Directory to authenticate users and role based access controls to grant users access to different parts of the portal application.

## Organizations

Each subscribed customer has a unique *organization*. Each organization’s database clusters are deployed and managed in their own cloud account ensuring complete segregation of customer data.

Each EDB Cloud organization is associated with an Azure Active Directory (AD) tenant. Azure AD establishes the identity of users that belong to an organization. When an EDB Cloud account is linked with an Azure AD application during subscription, users that belong to that AD can login. After initial login and until an **owner** assigns an appropriate role, usuer will have minimum access privileges.
...
Permissions are generally represented in the format 'action:object' where 'action' represents an operation that can be performed and 'object' represents a category of EDB Clou functionality.
