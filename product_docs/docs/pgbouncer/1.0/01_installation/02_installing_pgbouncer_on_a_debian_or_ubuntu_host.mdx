...
6.  Install the DEB package:
    ...
    >
    > For example, to install the pgbouncer 1.13.0.1 package for Advanced Server 12, execute the following command:
    >
    > ``` text
    > apt-get install edb-pgbouncer113
    > ```

PgBouncer will be installed in the `/usr/edb/pgbouncer<x.x>` directory, where `<x.x>` is the PgBouncer version you have installed.
