...
The EDB JDBC driver supports GSSAPI encrypted connections for EDB Postgres Advanced Server 12 onwards. This support is available from EDB JDBC Connector release 42.2.18.1 onwards. 
...
The example assumes that SSPI authentication is configured on Windows machine. Suppose edb-jdbc.jar path is <PATH_DRIVER> and waffle libraries path is <PATH_WAFFLE>. Here is how to set CLASSPATH and run JEdb sample:

```text 
set CLASSPATH=<PATH_DRIVER>\edb-jdbc.jar;<PATH_WAFFLE>\*;
javac JEdb.java
java JEdb
```
...
When the EDB Advanced Server and JDBC client both are on Linux, JDBC driver connects with GSSAPI authentication using one the following connection strings:
...
```text 
javac JEdb.java
java -Djava.security.auth.login.config=/etc/pgjdbc.conf -cp .:edb-jdbc.jar JEdb
```
...
```text 
set CLASSPATH=C:\Program Files\edb\jdbc\edb-jdbc.jar;
