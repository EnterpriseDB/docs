...
The example assumes that SSPI authentication is configured on Windows machine. Suppose `edb-jdbc.jar18` path is `<PATH_DRIVER>` and waffle libraries path is `<PATH_WAFFLE>`. Here is how to set `CLASSPATH` and run JEdb sample:

```text 
set CLASSPATH=<PATH_DRIVER>\edb-jdbc.jar18;<PATH_WAFFLE>\*;
javac JEdb.java
java JEdb
```
...
```text 
javac JEdb.java
java -Djava.security.auth.login.config=/etc/pgjdbc.conf -cp .:edb-jdbc.jar18 JEdb
```
...
```text 
set CLASSPATH=C:\Program Files\edb\jdbc\edb-jdbc.jar18;
