...
For `gsslib=ssapi` or `gsslib=auto`, EDB JDBC uses SSPI. For `gsslib=gssapi` it uses GSSAPI authentication.
...
con = DriverManager.getConnection(databaseUrl, connectionProps);
```
Running an EDB JDBC-based app in this case is the same as described in [Using SSPI (Windows-only environment)](#using-sspi-windows-only-environment).
...
Create the `pgjdbc.conf` file with the same contents described in [Using GSSAPI (Linux-only environment)](#using-gssapi-linux-only-environment).
