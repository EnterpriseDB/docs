...
After invoking the `efm disallow-node` command, you must use the [efm allow-node command](07_using_efm_utility/#efm_allow_node) to add the node to the cluster again .
