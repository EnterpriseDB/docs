...
Until you invoke the efm disallow-node command (removing the node's address of the node from the Allowed node host list), you can use the `service efm-3.8 start` command to restart the node at a later date without first running the `efm allow-node` command again.
...
If you are using RHEL 6.x or CentOS 6.x, you should copy the `efm-3.8` service script to new file with a name that is unique for each cluster. For example:

> `# cp /etc/init.d/efm-3.8 /etc/init.d/efm-acctg`
>
> `# cp /etc/init.d/efm-3.8 /etc/init.d/efm-sales`
...
If you are using RHEL 7.x or CentOS 7.x, you should copy the `efm-3.8` unit file to new file with a name that is unique for each cluster. For example, if you have two clusters (named acctg and sales), the unit file names might be:
...
> `PIDFile=/var/run/efm-3.8/acctg.pid`
