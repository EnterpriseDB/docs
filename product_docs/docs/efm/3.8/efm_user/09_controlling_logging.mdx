...
Failover Manager writes and stores one log file per agent and one startup log per agent in `/var/log/<cluster_name>-3.8` (where `<cluster_name>` specifies the name of the cluster).
...
By default, Failover Manager log files are rotated daily, compressed, and stored for a week. You can modify the file rotation schedule by changing settings in the log rotation file (`/etc/logrotate.d/efm-3.8`). For more information about modifying the log rotation schedule, consult the logrotate man page:
