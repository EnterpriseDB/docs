...
1.  Use a version-specific command to stop the old Failover Manager cluster; for example, you can use the following command to stop a version 3.8 cluster:

    > `/usr/efm-3.8/bin/efm stop-cluster efm`

2.  Start the new [Failover manager service](08_controlling_efm_service/#controlling_efm_service) (`edb-efm-3.9`) on each node of the cluster.
...
Moving existing properties file to efm.properties_2020-27-02T05:08:06.715
...
Checking directory /etc/edb/efm-3.8
Moving existing nodes file to efm.nodes_2020-27-02T05:08:06.730
