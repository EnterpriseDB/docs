...
## Installing Failover Manager on a RedHat or CentOS Host
...
The following steps will walk you through using the EnterpriseDB apt repository to install .

## Installing on a Debian 9 or Debian 10 Host
...
## Installing on an Ubuntu 18.04 or 20.04 Host
...
1.  Create the `cluster properties file <cluster_properties>` on each node.
2.  Create the `cluster members file <cluster_members>` on each node.
...
4.  Start the agent on each node of the cluster. For more information about controlling the service, see the section `Controlling the Failover Manager Service <controlling_efm_service>`.
