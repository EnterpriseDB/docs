...
On RHEL/CentOS 7.x and RHEL/CentOS 8.x, runs as a Linux service named (by default) `edb-efm-4.1.service` that is located in `/usr/lib/systemd/system`. Each database cluster monitored by Failover Manager will run a copy of the service on each node of the replication cluster.
...
Process: 58125 ExecStart=/bin/bash -c /usr/edb/edb-efm-/bin/runefm.sh start ${CLUSTER} (code=exited, status=0/SUCCESS) Main PID: 58180 (java) CGroup: /system.slice/edb-efm-.service └─58180 /usr/lib/jvm/java-1.8.0-openjdk-1.8.0.161-0.b14.el7\_4.x86\_64/jre/bin/java -cp /usr/edb/edb-efm-/lib/EFM-.0.jar -Xmx128m...
