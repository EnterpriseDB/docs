...
After creating the `acctg.properties` and `sales.properties` files, create a service script or unit file for each cluster that points to the respective property files; this step is platform specific. If you are using RHEL/CentOS 7.x or RHEL/CentOS 8.x, see <span class="title-ref">RHEL/CentOS 7.x or RHEL/CentOS 8.x &lt;rhel_or_centos_7></span>.
