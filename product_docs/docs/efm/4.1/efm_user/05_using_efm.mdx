...
3.  Assume superuser privileges on the new node, and start the Failover Manager agent. To start the cluster on RHEL/CentOS 7.x or RHEL/CentOS 8.x, invoke the command:
...
**Please note:** : If you are using a unit file, you must manually update the file to reflect the new service name when you upgrade .
