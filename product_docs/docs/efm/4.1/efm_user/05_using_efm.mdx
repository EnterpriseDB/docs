...
**Note**: If you are running `/var/lock` or `/var/run` on `tmpfs` (Temporary File System), make sure that the systemd service file for Failover Manager has a dependency on `systemd-tmpfiles-setup.service`.
...
**Note:** : To be a useful Standby for the current node, the node must be a standby in the PostgreSQL Streaming Replication scenario.
...
**Note:** : The promotion priority may change if a node becomes isolated from the cluster, and later re-joins the cluster.
...
Until you invoke the `efm disallow-node` command (removing the node's address of the node from the Allowed node host list), you can use the `service edb-efm-4.1 start` command to restart the node at a later date without first running the `efm allow-node` command again.

<div id="stopping_efm_cluster" class="registered_link"></div>
Note that stopping an agent does not signal the cluster that the agent has failed unless the [primary.shutdown.as.failure](04_configuring_efm/01_cluster_properties/#primary_shutdown_as_failure) property is set to `true`.
...
**Note:** When you invoke the `efm stop-cluster` command, all authorized node information is lost from the Allowed node host list.
...
After creating the `acctg.properties` and `sales.properties` files, create a service script or unit file for each cluster that points to the respective property files; this step is platform specific. If you are using RHEL/CentOS 7.x or RHEL/CentOS 8.x, see [RHEL/CentOS 7.x or RHEL/CentOS 8.x](#rhel_or_centos_7).

**Note:** : If you are using a unit file, you must manually update the file to reflect the new service name when you upgrade Failover Manager.

### RHEL/CentOS 7.x or RHEL/CentOS 8.x

If you are using RHEL/CentOS 7.x or RHEL/CentOS 8.x, you should copy the `edb-efm-4.1` unit file to new file with a name that is unique for each cluster. For example, if you have two clusters (named acctg and sales), the unit file names might be:
