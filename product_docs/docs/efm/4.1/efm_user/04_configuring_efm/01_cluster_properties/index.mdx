...
Each node in a cluster has a properties file (by default, named `efm.properties`) that contains the properties of the individual node on which it resides. The installer creates a file template for the properties file named `efm.properties.in` in the `/etc/edb/efm-4.1` directory.
...
The property files are owned by `root`. The service script expects to find the files in the `/etc/edb/efm-4.1 directory`. If you move the property file to another location, you must create a symbolic link that specifies the new location.
...
> **Note:** If you set the value of `jdbc.sslmode` to `verify-ca` and you want to use Java trust store for certificate validation, you need to set the following value:
...
> **Note:** If `is.witness` is `true`, will not check the recovery state.
...
For information about using a virtual IP address, see [Using Failover Manager with Virtual IP Addresses](05_using_vip_addresses).
...
For more information about configuring syslog logging, see [Enabling syslog Log File Entries](../09_controlling_logging#enabling-syslog-log-file-entries).



<div id="loglevel" class="registered_link"></div>

Use the `jgroups.loglevel` and `efm.loglevel` parameters to specify the level of detail logged by . The default value is INFO. For more information about logging, see [Controlling Logging](../09_controlling_logging)
