...
- Install EDB Postgres Advanced Server. See [Installing EDB Postgres Advanced Server](/epas/14/epas_inst_linux).

- Ensure that the other [Prerequisites](/efm/latest/01_prerequisites/) are met.  

- Log in as superuser before running any commands for setting up the repository or installing the package.
...
- Set up the repository.
  ...
  The script available from the repository includes the install command. If you need additional information about running the install command or initial configuration, refer back to this topic.

## Install the package

```shell
dnf -y install edb-efm<4x>
```
