...
```text
# cp /etc/edb/efm-4.2/efm.properties.in /etc/edb/efm-4.2/efm.properties
```
...
The property files are owned by `root`. The Failover Manager service script expects to find the files in the `/etc/edb/efm-4.2` directory. If you move the property file to another location, you must create a symbolic link that specifies the new location.
...
| [enable.stop.cluster](#enable_stop_cluster)                   |       |       | true                                  | This value must be same for all the agents. **Available in EFM 4.2 and later.**                                                                                                                     |
| [stop.isolated.primary](#stop_isolated_primary)               | Y     |       | true                                  | This value must be same for all the agents.                                                                                                                                                               |
...
| [reconfigure.num.sync](#reconfigure_num_sync)                 | Y     |       | false                                 |               To raise num_sync, see the reconfigure_num_sync_max (added in EFM version 4.1). |                                                                                                                                                 |
| [reconfigure.num.sync.max](#reconfigure_num_sync_max)         |       |       |                                       |  **Available in EFM 4.1 and later.**                    |
...
| [priority.standbys](#priority_standbys)                       |       |       |     | **Available in EFM 4.2 and later.**   |                                                                                                                                                                |
...
| [pgpool.enable](#pgpool_enable)                               |       |       | false                                 |   **Available in EFM 4.1 and later.**                                                                                                                                                             |
| [pcp.user](#pgpool_enable)                                    |       |       |                                       | Required if pgpool.enable is set to true. **Available in EFM 4.1 and later.**                                                                                                                       |
| [pcp.host](#pgpool_enable)                                    |       |       |                                       | Required if pgpool.enable is set to true, this value must be same for all the agents. **Available in EFM 4.1 and later.**                                                                           |
| [pcp.port](#pgpool_enable)                                    |       |       |                                       | Required if pgpool.enable is set to true, this value must be same for all the agents. **Available in EFM 4.1 and later.**                                                                          |
| [pcp.pass.file](#pgpool_enable)                              |       |       |                                       | Required if pgpool.enable is set to true. **Available in EFM 4.1 and later.**                                                                                                                       |
| [pgpool.bin](#pgpool_enable)                                  |       |       |                                       | Required if pgpool.enable is set to true. **Available in EFM 4.1 and later. **                                                                                                                     |
...
| [detach.on.agent.failure](#detach_on_agent_failure)           |       |       | true                                  | Set to false if you want to keep a running primary database attached.  **Available in EFM 4.2 and later.**                                                                                   |
...
Use the `enable.stop.cluster` property to enable or disable the `stop-cluster` command. The command could be a convenience in some environments but could cause issues when unintentionally invoked. In Eager Failover mode, the command would result in stopping Postgres without failover.
...
!!! Note
    ...
    If you are using the `reconfigure.num.sync` property, ensure that the `wal_sender_timeout` in the primary database is set to at least ten seconds less than the `efm.node.timeout` value.
...
!!! Note
    ...
    If you are using the `reconfigure.sync.primary` property, ensure that the `wal_sender_timeout` in the primary database is set to at least ten seconds less than the `efm.node.timeout` value.
...
Use the `pgpool.enable` property to specify if you want to enable the Failover Manager and Pgpool integration for high availability. If you want to enable Pgpool integration in a non-sudo mode (running as the DB owner), the PCPPASS file must be owned by the DB owner operating system user and file permissions should be set to 600.  
...
Use the `detach.on.agent.failure` property to indicate if you do not want to detach a node from the load balancer in a scenario where the primary agent fails but the database is still reachable. The default value is true.
