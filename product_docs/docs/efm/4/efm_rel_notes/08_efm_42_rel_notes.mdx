...
| Bug Fix | Added validation in the agent startup to check if the parent directory of pg_wal/pg_xlog is writable. The parent directory needs to be writable so EFM can later create a backup of it. If the new validation fails, an error is logged and the agent refuses to start. |
