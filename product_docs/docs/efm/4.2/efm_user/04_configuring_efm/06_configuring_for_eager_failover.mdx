...
You can setup Eager Failover by  performing the following steps (the example uses Advanced Server version 12 and EFM version 4.2):

<div id="enabling_the_eager_failover" class="registered_link"></div>

# Enabling the Eager Failover
...
# Disabling the Eager Failover

- If you want to stop Failover Manager without stopping the database, comment out the following line:
    ```text
    BindsTo=edb-efm-4.2.service
    ```
- Run the following command to reload the configuration files:
...
- Eager Failover is suitable in the following situations:
  ...
  - When custom scripting triggered by `script.fence` would fence the old primary server (STONITH). Some of the examples would be to shut down the VM with VMWare vCenter integration, openstack integration, or lights-out management. 
...
- With every failover, a primary ends up being a failed primary, which does not automatically recover to an operational standby. Therefore, the cluster should contain multiple promotable standbys, and the total number of standbys should be at least two more than the value specified for the `minimum.standbys` property. This is a general recommendation, but it becomes more pressing when using Eager Failover.
