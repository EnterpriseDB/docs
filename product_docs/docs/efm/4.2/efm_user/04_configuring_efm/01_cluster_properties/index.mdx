...
| [enable.stop.cluster](#enable_stop_cluster)                   |       |       | false                                 | This value must be same for all the agents                                                                                                                     |
...
| [priority.standbys](#priority_standbys)                       |       |       |                                      | This value must be same for all the nodes                                                                                                                      |
...
| [detach.on.agent.failure](#detach_on_agent_failurer)          |       |       | true                                  | Set to false if you want to keep a running primary database attached.                                                                                     |
...
Use the `enable.stop.cluster` to enable or disable the stop-cluster command. The command could be a convenience in some environments, but could cause issues when unintentionally invoked. In Eager Failover mode, the impact would even be stopping Postgres on all nodes,  which is why it should be disabled.
