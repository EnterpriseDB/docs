...
Use the `enable.stop.cluster` property to enable or disable the `stop-cluster` command. The command could be a convenience in some environments but could cause issues when unintentionally invoked. In Eager Failover mode, the command would result in stopping Postgres without failover. For all nodes in Eager Failover mode, stopping EFM escalates to stopping Postgres without resulting in a failover as well.
