...
If a Primary node reboots, Failover Manager may detect the database is down on the Primary node and promote a Standby node to the role of Primary. If this happens, the Failover Manager agent on the (rebooted) Primary node will attempt to write a `recovery.conf` file to make sure Postgres does not start as a secondary primary. Therefore, the Failover Manager agent must be started before starting the database server. The agent will start in idle mode, and check to see if there is already a primary in the cluster. If there is a primary node, the agent will verify that a recovery.conf or standby.signal file exists, or create recovery.conf if needed to prevent the database from starting as a second primary.
...
!!! Note
    ...
    The promotion priority for nodes will change when a new primary is promoted.

    If the efm set-priority command has been used to change whether or not a standby is promotable, this may be reset to the value in the standby's properties file through promotion or cluster splits and rejoins.
...
Until you invoke the `efm disallow-node` command (removing the node's address of the node from the Allowed node host list), you can use the `service edb-efm-4.2 start` command to restart the node at a later date without first running the `efm allow-node` command again.
...
If you are using RHEL/CentOS 7.x or RHEL/CentOS 8.x, you should copy the `edb-efm-4.2` unit file to new file with a name that is unique for each cluster. For example, if you have two clusters (named acctg and sales), the unit file names might be:
