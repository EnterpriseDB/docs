...
For database version 9.6, the db.user needs to be a superuser if `reconfigure.num.sync` or `reconfigure.sync.primary` is enabled.

For database versions higher than version 9.6, the `pg_reload_conf()` and `pg_read_all_stats` privileges are required only if you have the `reconfigure.num.sync` or `reconfigure.sync.primary` property set to `true`.
