...
/etc/edb/efm-4.0/acctg.properties
...
```text
[witness@localhost ~]# systemctl start edb-efm-4.0
Job for edb-efm-4.0.service failed because the control process exited with error code. See "systemctl status edb-efm-4.0.service" and "journalctl -xe" for details.
```

If you receive this message when starting the Failover Manager service, please see the startup log (located in `/var/log/efm-4.0/startup-efm.log`) for more information.
