...
1.  Install on each node of the cluster. For detailed information about installing , see [Installing Failover Manager](03_installing_efm/#installing_efm).

2.  After installing , invoke the `efm upgrade-conf` utility to create the `.properties` and `.nodes` files for Failover Manager . The installer installs the upgrade utility ([efm upgrade-conf](07_using_efm_utility/#efm_upgrade_conf)) to the `/usr/edb/efm-4.0/bin directory`. To invoke the utility, assume root privileges, and invoke the command:
...
2.  Start the new [Failover manager service](08_controlling_efm_service/#controlling_efm_service) (`edb-efm-4.0`) on each node of the cluster.
...
When your updates are complete, you can use the [efm set-priority](07_using_efm_utility/#efm_set_priority) command to add the old primary to the front of the standby list, and then switchover to return the cluster to its original state.
