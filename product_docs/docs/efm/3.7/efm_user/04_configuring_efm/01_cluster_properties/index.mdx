...
> `# cp /etc/edb/efm-3.7/efm.properties.in /etc/edb/efm-3.7/efm.properties`
...
The property files are owned by `root`. The Failover Manager service script expects to find the files in the `/etc/edb/efm-3.7 directory`. If you move the property file to another location, you must create a symbolic link that specifies the new location.
...
# file will exist for a standby node.
db.recovery.dir=
```
...
# If this property is blank, the db.recovery.dir location specified by the
# db.recovery.dir property will be used.
db.config.dir=
```
