...
The `db.user` specified must have sufficient privileges to invoke selected PostgreSQL commands on behalf of Failover Manager. For more information, please see [Prerequisites](../../02_failover_manager_overview/02_prerequisites/#prerequisites).
...
Use the `application.name` property to provide the name of an application that will replace the *value* specified in an existing application.name=value pair in the primary_conninfo parameter in the recovery.conf file before restarting an old master node as a standby.
