...
The property files are owned by `root`. The Failover Manager service script expects to find the files in the `/etc/edb/efm-3.6 directory`. If you move the property file to another location, you must create a symbolic link that specifies the new location.
...
Use the `db.recovery.conf.dir` property to specify the location to which a recovery file will be written on the Master node of the cluster, and a trigger file is written on a Standby. This property required on master and standby nodes; it is not required on a dedicated witness node.
...
Use the `db.config.dir` property to specify the location of database configuration files if they are not stored in the same directory as the `recovery.conf` file. This value will be used as the location of the Postgres `data` directory when stopping, starting, or restarting the database.
...
The Postgres pg\_is\_in\_recovery() function is a boolean function that reports the recovery state of a database. The function returns true if the database is in recovery, or false if the database is not in recovery. When an agent starts, it connects to the local database and invokes the pg\_is\_in\_recovery() function. If the server responds true, the agent assumes the role of standby; if the server responds false, the agent assumes the role of master. If there is no local database, the agent will assume an idle state.
...
-   If the database is reached, a notification will be sent informing you  
    of the agent status.
...
Please note: primary\_conninfo is a space-delimited list of keyword=value pairs.
...
Use the `application.name` property to provide the name of an application that will replace the *value* specified in an existing application.name=value pair in the primary\_conninfo parameter in the recovery.conf file before restarting an old master node as a standby.
