...
Copy the `efm.properties` and `efm.nodes` files to the `/etc/edb/efm-3.6` directory on the other nodes in your sample cluster. After copying the files, change the file ownership so the files are owned by `efm:efm`. The `efm.properties` file can be the same on every node, except for the following properties:
...
On any node, start the Failover Manager agent. The agent is named `efm-3.6`; you can use your platform-specific service command to control the service. For example, on a CentOS or RHEL 7.x host use the command:
