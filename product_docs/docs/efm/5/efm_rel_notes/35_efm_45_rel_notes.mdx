...
| Bug Fix      | Failover Manager properly follows the promotable=false value of a standby that is restarted after being made promotable manually through the `efm` command.                                                                                                | 83192                        |
| Bug Fix      | When using physical replication slots, Failover Manager will now drop and recreate slots on standbys when advancing the LSN values on them if they have an `xmin` value set.                                                                               | 83910                        |
