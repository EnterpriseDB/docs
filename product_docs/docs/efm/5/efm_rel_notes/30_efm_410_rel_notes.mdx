...
| Bug Fix     | The `efm upgrade-conf` command now doesn't lose `.nodes` file information when the `-source` and destination directories are the same.                                   | 37479     |
| Bug Fix     | Fixed an issue where the `efm cluster-status` command hid connection errors if every database connection failed.                                                         | 39108     |
