...
| Enhancement | Added new 'backup.wal' property to control whether or not standby WAL is backed up during reconfiguration. [Support ticket: #1277862, #91506, #101026] |
...
| Enhancement | Added new 'check.num.sync.period' property to control how often a primary database is checked for synchronous standbys. [Support ticket: #1133047, #91506] |
| Enhancement | Removed unnecessary check of database operating system user in sudoers conf file. [Support ticket: #42594, #99957] |
| Enhancement | Removed unnecessary network status check that was added for older operating systems. |
| Enhancement | Table information in 'efm cluster-status' output will now resize to handle long host names. [Support ticket: #101878] |
...
| Bug Fix | Fixed agent shutdown code that could cause a 'systemctl stop' call to finish before the agent completes shutdown, resulting in the agent process not exiting gracefully. [Support ticket: #37270] |
| Bug Fix | Handle a case where an agent on a failed/rebooted standby node attempts to join the cluster before the previous agent from the same host has timed out. [Support ticket: #43162] |
