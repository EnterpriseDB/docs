...
/etc/edb/efm-3.10/acctg.properties
...
```text
[witness@localhost ~]# systemctl start edb-efm-3.10
Job for edb-efm-3.10.service failed because the control process exited with error code. See "systemctl status edb-efm-3.10.service" and "journalctl -xe" for details.
```

If you receive this message when starting the Failover Manager service, please see the startup log (located in `/var/log/efm-3.10/startup-efm.log`) for more information.
