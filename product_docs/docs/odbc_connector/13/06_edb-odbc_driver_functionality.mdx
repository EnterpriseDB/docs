...
-   `conn_handle` &mdash; The connection handle.

-   `info_type` &mdash; The type of information SQLGetInfo() is retrieving.

-   `info_pointer` &mdash; A pointer to a memory buffer to hold the retrieved value.

    If the `info_type` argument is `SQL_DRIVER_HDESC` or `SQL_DRIVER_HSTMT`, the `info_pointer` argument is both `Input` and `Output`.

-   `buffer_len` &mdash; The length of the allocated memory buffer pointed to by `info_pointer`. If `info_pointer` is `NULL`, `buffer_len` is ignored. If the returned value is a fixed size, `buffer_len` is ignored. `buffer_len` is used only if the requested value is returned in the form of a character string.

-   `string_length_pointer` &mdash; A pointer to an `SQLSMALLINT` value. `SQLGetInfo()` writes the size of the requested value in this integer.
...
| SQL_CONVERT_REAL: Indicates conversion support for the REAL type using the `CONVERT` function                                                                                                                                                                               | Returns `0`. The server doesn't support conversion.                                                                                                                                                                                                                                                          |
...
| SQL_CONVERT_WVARCHAR: Indicates conversion support for the WVARCHAR type using the `CONVERT` function.                                                                                                                                                                      | This info_type is valid only when using the Unicode driver. Returns`0`. The server doesn't support conversion.                                                                                                                                                                                              |
...
-   `attribute` &mdash; Identifies the attribute whose value you want to retrieve.

-   `value_pointer` &mdash; A pointer to the location in memory to receive the `attribute` value.

-   `buffer_length` &mdash; If `attribute` is defined by ODBC and `value_pointer` points to a character string or binary buffer, `buffer_length` is the length of `value_pointer`. If `value_pointer` points to a fixed-size value (such as an integer), `buffer_length` is ignored.
...
-   `string_length_pointer` &mdash; A pointer to a `SQLINTEGER` that receives the number of bytes available to return in `value_pointer`. If `value_pointer` is NULL, `string_length_pointer` isn't returned.
...
-  `attribute` &mdash; Identifies the attribute whose value you want to set.

-  `value_pointer` &mdash; A pointer to the value that the attribute assumes.

-  `string_length` &mdash; If `attribute` is defined by ODBC and `value_pointer` points to a binary buffer or character string, `string_length` is the length of `value_pointer`. If `value_pointer` points to a fixed-length value (such as an integer), `string_length` is ignored.
...
-  `env_handle` &mdash; The environment handle.

-  `attribute` &mdash; Identifies the attribute whose value you want to retrieve.

-  `value_pointer` &mdash; A pointer to the location in memory to receive the `attribute` value.

-   `buffer_length` &mdash; If the attribute is a character string, `buffer_length` is the length of `value_ptr`. If the value of the attribute isn't a character string, `buffer_length` is unused.
...
-   `env_handle` &mdash; The environment handle.
-   `attribute` &mdash; Identifies the attribute whose value you want to set.
-   `value_pointer` &mdash; A pointer to the value assigned to the attribute. The value is a NULL terminated character string or a 32-bit integer value, depending on the specified attribute.
-   `string_length` &mdash; If `value_pointer` is a pointer to a binary buffer or character string, `string_length` is the length of `value_pointer`. If the value being assigned to the attribute is a character, `string_length` is the length of that character string. If `value_pointer` is NULL, `string_length` isn't returned. If `value_pointer` is an integer, `string_length`is ignored. 
...
-   `stmt_handle` &mdash; The statement handle. 

-   `attribute` &mdash; The attribute value.

-   `value_pointer` &mdash; A pointer to the location in memory to receive the `attribute` value.

-   `buffer_length` &mdash; If the attribute is defined by ODBC, `buffer_length` is the length of `value_pointer` (if `value_pointer` points to a character string or binary buffer).  If `value_pointer` points to an integer, `buffer_length` is ignored. 
...
-   `string_length_pointer` &mdash; A pointer to an `SQLINTEGER` that receives the number of bytes required to hold the requested value. If `value_pointer` is NULL, `string_length_pointer` isn't returned.  
...
-   `stmt_handle` &mdash; The environment handle.

-   `attribute` &mdash; Identifies the statement attribute whose value you want to set.

-   `value_pointer` &mdash; A pointer to the location in memory that holds the value to assigned to the attribute. `value_pointer` can be a pointer to:
...
-   `string_length` &mdash; If `attribute` is defined by ODBC and `value_pointer` points to a binary buffer or character string, `string_length` is the length of `value_pointer`. If `value_pointer` points to an integer, `string_length` is ignored. 
...
-   `handle_type` &mdash; The handle type of the `handle` argument. `handle_type` must be one of the following:  
...
-   `handle` &mdash; The handle associated with the attribute error message.

-   `record_number` &mdash; The status record that the application is seeking information from (must be greater than or equal to 1).

-   `SQLState_pointer` &mdash; Pointer to a memory buffer that receives the `SQLState` error code from the record. 

-   `native_error_pointer` &mdash; Pointer to a buffer that receives the native error message for the data source (contained in the `SQL_DIAG_NATIVE` field).

-   `error_text_pointer` &mdash; Pointer to a memory buffer that receives the error text (contained in the `SQL_DIAG_MESSAGE_TEXT` field). 

-   `buffer_length` &mdash; The length of the `error_text` buffer.

-   `text_length_pointer` &mdash; Pointer to the buffer that receives the size (in characters) of the `error_text_pointer` field. If the number of characters in the `error_text_pointer` parameter exceeds the number available (in `buffer_length`), `error_text_pointer` is truncated.
