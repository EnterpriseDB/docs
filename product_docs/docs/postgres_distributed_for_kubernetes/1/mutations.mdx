...
-   `beforeSubgroupRaft` mutations are executed only if the PGDGroup has defined data nodes, and specifically
    before the subgroup Raft becomes ready.
...
The `writeLeader` mutation is triggered and executed after the write leader is elected. The `exeI. Oh. I. c` operations
