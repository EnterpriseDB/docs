...
in the secret `bindPassword`, then postgres try to perform a search under `baseDN` to find a 
username that matches the item specified by `searchAttribute`, if a match is found,  finally 
