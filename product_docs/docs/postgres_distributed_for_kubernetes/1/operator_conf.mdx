...
The operator for EDB Postgres® AI for CloudNativePG™ Global Cluster (PGD4K) is installed from a standard
...
| `MONITORING_QUERIES_CONFIGMAP` | Name of the ConfigMap in the operator namespace containing default monitoring queries to distribute to all PGDGroup namespaces (see [Default Monitoring Queries](monitoring.md#default-monitoring-queries))                    |
| `MONITORING_QUERIES_SECRET`    | Name of the Secret in the operator namespace containing default monitoring queries to distribute to all PGDGroup namespaces (see [Default Monitoring Queries](monitoring.md#default-monitoring-queries))                       |
| `OPERATOR_LOG_LEVEL`           | Operator log level is the log level for the operator (Defaults to empty), available options are: trace, debug, info, warn, or error. This option will override the log level defined in deployment                             |
...
The CNPG-GC (PGD4K) operator manages each PGD node using the `Cluster`
