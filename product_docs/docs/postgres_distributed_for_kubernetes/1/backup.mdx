...
EDB Postgres® AI for CloudNativePG™ Global Cluster (CNPG-GC) supports *online/hot backup* of
...
Given that CNPG-GC configures the connection with object stores by relying on
...
!!! Important
    ...
    available at `spec.backup.barmanObjectStore`. In CNPG-GC examples, the object store section is at a
...
In CNPG-GC, each PGD node is set up to archive WAL files in the object store independently.
...
Scheduled backups are the recommended way to configure your backup strategy in CNPG-GC.
...
!!! Note
    ...
    cluster to back up. This option currently isn't supported by CNPG-GC and is
...
CNPG-GC can manage the automated deletion of backup files from the backup
...
A PGD node is represented as a single-instance EDB Postgres for Kubernetes `Cluster` object.
As such, if you need to, it's possible to request an on-demand backup
of a specific PGD node by creating an EDB Postgres for Kubernetes `Backup` resource.
