...
In EDB Postgres® AI for CloudNativePG™ Global Cluster (CNPG-GC), recovery is available as a way
...
CNPG-GC also supports point-in-time recovery (PITR), which allows you to restore a PGD group up to
...
Refer to the
[PGD documentation](/pgd/5.9/node_management/) for more detail.
