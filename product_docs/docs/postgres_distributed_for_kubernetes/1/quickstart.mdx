...
You cam test an EDB Postgres Distributed (PGD) cluster on your
laptop or computer using EDB Postgres Distributed for Kubernetes (PG4K-PGD)
on a single local
...
[architecture document](architecture.md), they're implemented on top
of PG4K clusters.
