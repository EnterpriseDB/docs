...
* [Repository name](/postgres_distributed_for_kubernetes/latest/identify_images/private_registries/#which-repository-to-choose)
* [Your repository token](/postgres_distributed_for_kubernetes/latest/identify_images/private_registries/#how-to-retrieve-the-token)
...
1.  Set the environment variable to use [your personal token](/identify_images/private_registries/#how-to-retrieve-the-token). In this example, the token is not real:

    ```
    export EDB_SUBSCRIPTION_TOKEN=aaAa1bbbBBcccccC1a1111bB1C1aBbB
    ```
...
!!! Note Token 
    Before running the command, replace the `EDB_SUBSCRIPTION_TOKEN` placeholder.
