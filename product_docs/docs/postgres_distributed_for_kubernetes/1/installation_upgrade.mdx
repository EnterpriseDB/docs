...
`REPO` should be set to either `k8s_enterprise_pgd` or `k8s_standard_pgd` depending on the EDB software subscription purchased and Postgres distribution to be installed. Trial and preview user should use `k8s_enterprise_pgd`.

`TOKEN` should be set to the value displayed using the Reveal Token or Copy button after signing into the [Repos page](https://www.enterprisedb.com/repos-downloads) on the EDB website. 
