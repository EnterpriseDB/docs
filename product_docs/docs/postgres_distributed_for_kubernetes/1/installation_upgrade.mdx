...
-   Standard: The [latest standard operator manifest](https://get.enterprisedb.io/pg4k-pgd/pg4k-pgd-standard-1.1.3.yaml).
-   Enterprise: The [latest enterprise operator manifest](https://get.enterprisedb.io/pg4k-pgd/pg4k-pgd-enterprise-1.1.3.yaml).

To install the manifest for the latest version of the operator:

```sh
kubectl apply --server-side -f \
  https://get.enterprisedb.io/pg4k-pgd/pg4k-pgd-${EDB_SUBSCRIPTION_PLAN}-1.1.3.yaml
```

...where `${EDB_SUBSCRIPTION_PLAN}` is either "enterprise" or "standard".
...
spec:
  ...
  proxyInstances: 2
  witnessInstances: 1
  imageName: docker.enterprisedb.com/k8s/edb-postgres-extended-pgd:17.5-pgd581-ubi9
  pgdProxy:
    imageName: docker.enterprisedb.com/k8s/edb-pgd-proxy:5.8.1-ubi9
  imagePullSecrets:
  ...
  cnp:
    storage:
      size: 1Gi
  pgdProxy:
    imageName: docker.enterprisedb.com/k8s/edb-pgd-proxy:5.7.1-1
```
...
This command creates an EDB Postgres Extended PGD 5.8 `ImageCatalog`:
...
```shell
pgd: docker.enterprisedb.com/k8s/edb-postgres-advanced-pgd:16.9-pgd581-ubi9
