...
The following example shows how to use the EDB Postgres Extended PGD 5.8 `ImageCatalog` to
...
-   Minor version upgrades (e.g., postgreSQL from 17.4 to 17.5 and PGD from 5.7 to 5.8)
-   Major version upgrades (e.g., postgreSQL from 16.x to 17.5, no PGD major version upgrade as we only support PGD 5.x now)

!!! Note
    The PGD Operand upgrade proceeds sequentially on each node. The node upgrade process
    is managed by the PG4K operator. For detailed information, please refer to
    the [PostgreSQL Upgrades](/postgres_for_kubernetes/latest/postgres_upgrades/)
...
You plan to upgrade to PostgreSQL 17.5 plus PGD 5.8.1.

##### Step-by-Step Guide

1.  Check the current operand version we are using.
...
Observe the upgrading node, e.g., pgd-sample-3, shows "Upgrading Postgres major version".
...
During the process, the PGDGroup status will be:
