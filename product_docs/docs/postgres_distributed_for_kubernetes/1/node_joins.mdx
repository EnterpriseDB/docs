...
-   logical join:
      ...
      This method uses the [bdr.join_node_group]((https://www.enterprisedb.com/docs/pgd/latest/reference/nodes-management-interfaces/#bdrjoin_node_group)) function to integrate the new node into the PGD group.
      It is important that the joining node should not contain any schemas or data present in the PGD group.
      PGD recommends that the new database should only contain the BDR extension, as data synchronization occurs during
      the join.

-   physical join

      This method uses the [bdr_init_physical](/pgd/latest/reference/nodes/#bdr_init_physical) command to speed up the joining process. We can prepare data in advance before executing `bdr_init_physical`.

For more information about join method, refer to the documentation on [Creating and Joining PGD Groups](/pgd/latest/node_management/creating_and_joining/#creating-and-joining-pgd-groups).
...
In the following sections, we will explain how to configure the join method for the first
...
This section explains how to configure the join method for a node joining a group, particularly when
...
In the following example, the second node will use the `pg_basebackup` command to replicate data from the
first node in the `region-a` group, and then it will use `bdr_init_physical` to join the `region-a` group.

Here's an example of a physical join:
...
This section outlines how to configure the join method for group joins, specifically when creating
cross-region PGD groups and how the first node in the new PGD group joins an existing PGD group.
...
how PGD groups join with each other, and uses the `spec.pgd.discovery` section to which target to join.

Below is an example of a logical join, where `spec.pgd.groupJoinMethod=logical`. The first node
...
Below is an example of a physical join, configured with `spec.pgd.groupJoinMethod=physical`.
