...
scope of this document. However, once you've established connectivity between the OpenShift clusters you can deploy
...
`OpenShift Cluster C`. In OpenShift, an installation of the PG4K-PGD-Operator from OperatorHub includes an
...
1. Finally, you can switch context to `OpenShift Cluster C` and create the third PGD group. The YAML for the PGD
group is:
