...
-   `@@REPOSITORY@@` is the name of the repository, as explained in [Which repository to choose?](private_edb_registries.md#which-repository-to-choose).
...
-   EDB Postgres for Kubernetes operator (PG4K):
    Since release v1.0.1, PGD4K is constrained to use an LTS release of PG4K.
-   cert-manager:
    Since v1.1.1, both cert-manager Operator for Red Hat OpenShift and cert-manager operator for Community
    are supported.

For more details about the supported versions for each release, please refer to the
[supported versions](supported_versions.md) page. Please be aware that only when those dependencies are met
can the PGD4K operator be installed or upgraded.
