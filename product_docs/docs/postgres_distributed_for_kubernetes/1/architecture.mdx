...
for Kubernetes (PG4K), specifically a cluster with a single instance (that is, no
replicas).
...
write leaders. PG4K handles only the deployment and healing of data nodes.
...
-   Data nodes: as mentioned previously, a node is a database and is managed
    by PG4K, creating a cluster with a single instance.
...
See
[Choosing your architecture](/pgd/latest/architectures/) in the PGD documentation
...
PG4K-PGD can be installed in a
...
The PG4K-PGD operator can control the scheduling of pods (that is, which pods go
...
and [Scheduling](/postgres_for_kubernetes/latest/scheduling/) in the PG4K documentation
for more information.
