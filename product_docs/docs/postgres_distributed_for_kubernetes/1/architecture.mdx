...
EDB Postgres® AI for CloudNativePG™ Global Cluster (CNPG-GC) is a
...
[EDB Postgres Distributed (PGD)](/pgd/5.9/)
...
CNPG-GC offers a different way of deploying PGD clusters,
...
In CNPG-GC, as in EDB Postgres for Kubernetes,
...
## Managing PGD using CNPG-GC

The CNPG-GC operator can manage
...
separate write leader. In CNPG-GC,
...
In CNPG-GC, local routing is used by default,
...
CNPG-GC leverages Kubernetes to deploy
...
[PGD documentation](/pgd/5.9/planning/choosing_server/)
...
Such an architecture enables the CNPG-GC
...
The CNPG-GC operator can control the scheduling
