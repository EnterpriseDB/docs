...
EDB Postgres® AI for CloudNativePG™ Global Cluster (CNPG-GC) requires a Kubernetes version supported by the community. See
...
[test CNPG-GC on your laptop using a local cluster](quickstart.md) before
