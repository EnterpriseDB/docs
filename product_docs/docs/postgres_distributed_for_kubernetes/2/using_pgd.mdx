...
!!! Note
    ...
    On OpenShift environments, pods run with an arbitrary user ID rather than the default postgres user.
    You need to explicitly specify the postgres user in the PGD DSN:

    -   Use `--dsn "dbname=$APP_DATABASE user=postgres"` with each command, or
    -   Export `PGD_CLI_DSN="dbname=$APP_DATABASE user=postgres"` for the entire session.

```shell
$ pgd groups list --dsn "dbname=$APP_DATABASE"
# On OpenShift, add user=postgres:
# pgd groups list --dsn "dbname=$APP_DATABASE user=postgres"
