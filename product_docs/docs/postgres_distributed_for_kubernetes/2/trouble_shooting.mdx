...
starts the physical join, but there is probably still a chance that a physical join fails. If a physical join job fails:
...
you can try to delete the job to trigger the physical join again:
