...
EDB Postgres® AI for CloudNativePG™ Global Cluster supports *online/hot backup* of
...
Given that EDB CloudNativePG Global Cluster configures the connection with object stores by relying on
PG4K, see the [PG4K common object stores for backups](/postgres_for_kubernetes/latest/object_stores/)
documentation for more information.

!!! Important
    The PG4K documentation's Cloud Provider configuration section is
    available at `spec.backup.barmanObjectStore`. In EDB CloudNativePG Global Cluster examples, the object store section is at a
...
In EDB CloudNativePG Global Cluster, each PGD node is set up to archive WAL files in the object store independently.
...
For more information, see the [PG4K WAL archiving](/postgres_for_kubernetes/latest/backup/#wal-archive) documentation.

## Scheduled backups

Scheduled backups are the recommended way to configure your backup strategy in EDB CloudNativePG Global Cluster.
When the PGD group `spec.backup.configuration.barmanObjectStore` and `.spec.backup.schedulers[].schedule` stanzas are configured,
...
For more information, see [Backup on object stores](/postgres_for_kubernetes/latest/backup_barmanobjectstore/) in the PG4K documentation.
...
For more information, see [Backup on volume snapshots](/postgres_for_kubernetes/latest/backup_volumesnapshot/) in the PG4K documentation.
...
For a comparison of these two backup methods, see [Object stores or volume snapshots](/postgres_for_kubernetes/latest/backup/#comparing-available-backup-options-object-stores-vs-volume-snapshots) in the PG4K documentation.
...
`.spec.backup.schedulers[].backupOwnerReference` indicates the `ownerReference` to use
...
!!! Note
    The PG4K `ScheduledBackup` object contains the `cluster` option to specify the
    cluster to back up. This option currently isn't supported by EDB CloudNativePG Global Cluster and is
...
EDB CloudNativePG Global Cluster can manage the automated deletion of backup files from the backup
...
For more information, see the [PG4K retention policies](/postgres_for_kubernetes/latest/backup_barmanobjectstore/#retention-policies) in the PG4K documentation.
...
supported. For more information, see the [PG4K compression algorithms](/postgres_for_kubernetes/latest/backup_barmanobjectstore/#compression-algorithms) documentation.
...
For more information, see the PG4K documentation about [tagging of backup objects](/postgres_for_kubernetes/latest/backup_barmanobjectstore/#tagging-of-backup-objects).

## On-demand backups of a PGD node

A PGD node is represented as a single-instance PG4K `Cluster` object.
As such, if you need to, it's possible to request an on-demand backup
of a specific PGD node by creating an PG4K `Backup` resource.
To do that, see [PG4K on-demand backups](/postgres_for_kubernetes/latest/backup/#on-demand-backups) in the PG4K documentation.

!!! Hint
    You can retrieve the list of PG4K clusters that make up your PGD group
