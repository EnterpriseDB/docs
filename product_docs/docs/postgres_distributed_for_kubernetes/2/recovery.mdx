...
In EDB Postgres® AI for CloudNativePG™ Global Cluster, recovery is available as a way
...
EDB CloudNativePG Global Cluster also supports point-in-time recovery (PITR), which allows you to restore a PGD group up to
...
For more information, see [PG4K recovery - Additional considerations](/postgres_for_kubernetes/latest/recovery/#additional-considerations) in the PG4K documentation.
...
This example defines a time-based target for the recovery:
...
You can also recover a PGDGroup from a volumeSnapshot backup. The stanza
...
For more information, see [Recovery from volumeSnapshot objects](/postgres_for_kubernetes/latest/recovery/#recovery-from-volumesnapshot-objects) in the PG4K documentation.
...
For more information on all the available recovery targets, see [PG4K recovery targets](/postgres_for_kubernetes/latest/recovery/#point-in-time-recovery-pitr) in the PG4K documentation.

## Recovering and creating PGD Groups in multiple regions

To recover from a backup and create multiple PGD Groups joined across regions,
you can only restore the first group (where `spec.pgd.parentGroup.create` is set to true) from the backup.
...
Refer to the
[PGD documentation](/pgd/latest/node_management/creating_and_joining/) for more details.
