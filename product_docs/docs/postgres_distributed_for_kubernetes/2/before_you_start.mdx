...
EDB Postgres® AI for CloudNativePG™ Global Cluster requires a Kubernetes version supported by the community. See
[Supported releases](https://www.enterprisedb.com/resources/platform-compatibility#pgdk8s) for details.

## PGD terminology

For more information, see
[Terminology](/pgd/latest/overview/terminology/) in the PGD documentation.

[Connection manager](/pgd/latest/overview/terminology/#connection-manager)
: A running process built into a database instance that ensures clients connect to the appropriate nodes based on their needs.

[Data node](/pgd/latest/overview/terminology/#node)
: PGD database instances that participate in data replication.
  In EDB CloudNativePG Global Cluster, a CNP cluster with no replicas is used to represent a data node.

[Witness node](/pgd/latest/overview/terminology/#witness-nodes)
: PGD database instances that do not participate in data replication.
  In EDB CloudNativePG Global Cluster, a CNP cluster with no replicas is used to represent a witness node.

[PGD node](/pgd/latest/overview/terminology/#pgd-node)
: A database instance participating in a PGD cluster, which can be either a data node or a witness node.

[Node group](/pgd/latest/overview/terminology/#node-groups)
: Multiple data nodes and a witness node organized in one group.
  In EDB CloudNativePG Global Cluster, a custom resource named PGDGroup is used to represent a node group.

[PGD cluster](/pgd/latest/overview/terminology/#pgd-cluster)
: Multiple node groups joined together to form a redundant database system.

[RAFT](/pgd/latest/overview/terminology/#replicated-available-fault-tolerance-raft)
: PGD uses Raft within groups (top-level or local) to establish the node that's the write leader.

[Failover](/pgd/latest/overview/terminology/#failover)
...
[Switchover](/pgd/latest/overview/terminology/#switchover)
...
[Write leader](/pgd/latest/overview/terminology/#write-leader)
...
[test EDB CloudNativePG Global Cluster on your laptop using a local cluster](quickstart.md) before
