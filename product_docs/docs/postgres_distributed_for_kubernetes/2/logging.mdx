...
EDB Postgres® AI for CloudNativePG™ Global Cluster outputs logs in JSON format directly to standard output,
including PostgreSQL logs, without persisting them to storage for security reasons. This
...
As EDB CloudNativePG Global Cluster leverages the PG4K clusters
to manage nodes, logs for PG4K operator and instance are also applicable
here. Check [here](/postgres_for_kubernetes/latest/logging/)
for more information about logs for PG4K.
...
Node logs are PostgreSQL instance pod logs. You can configure the log level for the PGD node
in the PGDGroup specification using the `logLevel` option, PGDGroup will deliver the `logLevel`
changes to all the PG4K clusters underlying. Available log levels are: `error`,
`warning`, `info` (default), `debug`, and `trace`.

-   change the logLevel for data nodes in `spec.cnp.logLevel`
-   change the logLevel for witness nodes in `spec.witness.logLevel`

!!! Important
    Currently, the log level can only be set at the time the instance starts.
    Changes to the log level in the pgdgroup specification after the group has
    started will cause the nodes to restart.

## Operator Logs

There are two types of operator logs you can check:

-   Operator logs for EDB CloudNativePG Global Cluster
-   Operator logs for PG4K
...
EDB CloudNativePG Global Cluster offers seamless and native support for
...
!!! Important
    ...
    PG4K automatically manages this based on the presence of `pgaudit.*`
...
spec:
  instances: 3
  imageName: docker.enterprisedb.com/k8s/edb-postgres-advanced-pgd:17.6-pgd612-expanded-ubi9
  licenseKey: <LICENSE>
...
-   `instance-manager`: from the [PostgreSQL instance manager](/postgres_for_kubernetes/latest/instance_manager/) in each node.

With the exception of `postgres` and `edb_audit`, which follow a specific structure,
