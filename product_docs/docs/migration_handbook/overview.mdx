...
Organizations migrate their Oracle databases for a range of reasons including cost, license constraints, and the desire to move away from proprietary data centers. Legacy licenses drive cost up while constraining innovation and agility. For example, not every license is portable to every virtualization platform, every Cloud, or every operating system— without even talking about scaling up and scaling down. 
...
The replatforming approach is best suited for those seeking to reduce the high cost of the database. 

We will focus on this approach: migrating an Oracle database to the EDB Postgres Advanced Server database.

### “Lift and shift”

“Lift and shift,” also known as rehosting, moves your database to a new host platform without changing the underlying software stack. For example, suppose your existing application is:
...
|----------------------------|----------|---------|-------------------------|
| Replatforming | Easy way to reduce database license cost and eliminate platform restrictions;  Minimal changes to the application and existing integrations; take advantage of Postgres innovation | May require minor application changes and retesting | High license cost databases with longer-term strategic role; use of database logic (stored procedures, packages) or proprietary features, as EDB Postgres Advanced Server can help with this with its Oracle compatibility features |
...
It is also important to identify potential migration problems. For an Oracle-to-EDB Postgres Advanced Server migration, see the [Assess feasibility](link to section) for details.
...
Determining whether to run your applications in the cloud (and which cloud) or on-premises. 
Deciding whether to use an open source platform or a proprietary one.

### 3. Plan migration

Planning your migration involves these processes:

Prioritize applications. Which application is the most important to migrate? This can help you to zero in on the compatibilities that are critical.
Decide on the approach (replatform, lift and shift, restructure). See [Approaches to migrating your Oracle database](need link here).
...
Migrate dB functionality
Migrate data as a snapshot (copy at a particular point in time) or change data capture (CDC). For small data sets, use Migration Toolkit. If it's an Oracle migration into EDB Postgres Advanced Server, and the data set is large or if you notice slow data transfer, take advantage of one of the other data movement methods available:
...
In this phase, you’ll test the system performance and tune the new server. If you're migrating into an EDB Postgres Advanced Server database, take advantage of EDB Postgres Advanced Server's performance tuning utilities:
...
See EDB Postgres Advanced Server and Database Compatibility for Oracle Developers for information about the performance tuning tools available with EDB Postgres Advanced Server.
...
See [Analyze feasibility](analyze_feasibility) for information on compatibility between Oracle and EDB Postgres Advanced Server and for information about how to work around incompatibilities. 
...
Replication Server. EDB Postgres Replication Server provides a robust data platform that replicates from non-PostgreSQL databases to PostgreSQL in single-master mode. It can be used as an alternative to Migration Toolkit for migrating data from Oracle to Postgres and is especially useful when application downtime is a concern.
Database links and FDW. These are technologies that can also be used to help migrate data from an Oracle database.  Whereas, the Migration Toolkit and Replication Server tools can be used to automatically push data from Oracle to Postgres.  Database links and Foreign Data Wrappers (FDWs) can be used to pull data from Oracle into Postgres.  Setup of DB links and FDWs and migration of data using them is more manual in nature.
...
Postgres Enterprise Manager (PEM). PEM consists of components that provide the management and analytical functionality for your EDB Postgres Advanced Server or PostgreSQL database. PEM is a comprehensive database design and management system. PEM simplifies the creation, maintenance, and use of database objects.
Embedded Postgres SME service.
...
Replication Server. EDB Postgres Replication Server (EPRS) provides a robust data platform that replicates between PostgreSQL databases in single-master or multi-master mode, or from non-PostgreSQL databases to PostgreSQL in single-master mode.
