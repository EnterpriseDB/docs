...
`` Create an `Azure user account with sufficient privileges <#provide_access_azure>`_ to access the Ark Administrator's console. `` Create an [Azure network security group](../06_ark_installing_console/#azure_security_group) .

`` Create an `Azure storage account <#azure_storage_account>`_ . `` Launch a [VM Image](../06_ark_installing_console/#azure_launch_ark_console) that contains the Ark console.

`` Configure the `Ark console <#azure_configure_ark_console>`_ . `` Register an [Ark Administrative user](../06_ark_installing_console/#azure_register_user) .
...
-   Use the `` `Admin `` tab &lt;ark\_using\_admin\_tab&gt;\` to create the server ../images and database engines that will be used by non-administrative users.
