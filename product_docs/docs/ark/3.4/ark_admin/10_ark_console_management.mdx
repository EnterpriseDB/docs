...
1.  Use the `yum list edb-ark` command to review a list of available updates.

    `yum list edb-ark`

2.  If any updates are available, use yum to install the updates:

    `yum update package_ name`

    Where `package_name` specifies the name of the package that you wish to update.

3.  When the downloads complete, navigate into the `/usr/share/tomcat/` directory:

    `cd /usr/share/tomcat`

4.  Invoke the EDB Ark post-installation script to upgrade the console:
...
2.  Use yum to update the PEM agent and PEM server version installed on your Ark console host:

    `yum install edb-pem-python-idna.noarch`

    `yum update edb-pem edb-pem-server`

3.  When the installation completes, configure the updated PEM server:
...
<img src="images/ark_console_mgmt_modify_backup_props.png" class="align-center" alt="Console management" />
...
If you wish to manually save backups, you can use the Postgres [pg\_dump](http://www.enterprisedb.com/docs/en/9.6/pg/app-pgdump.html) or [pg\_dumpall](http://www.enterprisedb.com/docs/en/9.6/pg/app-pg-dumpall.html) command to archive the console database. Then, you can then use the [pg\_restore](http://www.enterprisedb.com/docs/en/9.6/pg/app-pgrestore.html) command to restore the console database if necessary.

**Recovering the Console with a Backup Script**

The backup script provided with the Ark console uses pg\_dump to create a plain-text SQL script file that contains the commands required to rebuild the console database to the state in which the backup was taken. After using ssh to connect to the host of the console, you can use the following command to invoke the psql command line tool and restore the console:
