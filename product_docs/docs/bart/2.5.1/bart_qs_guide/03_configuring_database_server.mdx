...
1.  First, enable the usage of public key authentication:
    ...
    - In the SSH server daemon configuration file `/etc/ssh/sshd_config`, ensure that the following parameter is set to `yes` and is not commented out:

         `PubkeyAuthentication yes`

    - Reload the configuration file using the `service sshd reload`, `service sshd stop`, `service sshd start`, or `service sshd restart` command.
