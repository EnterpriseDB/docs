...
-   You can take full backups using the transaction log streaming method (xlog\_method = stream in the BART configuration file) `--with-pg_basebackup` and the `BACKUP` subcommand in either tar (`-F t`) or plain text (`-F p`) backup file format.
