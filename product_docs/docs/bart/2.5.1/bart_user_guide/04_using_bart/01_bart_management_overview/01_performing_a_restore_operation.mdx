...
> **Note:** If the backup is restored to a different location than where the original database cluster resided, operations dependent upon the database cluster location may fail if supporting service scripts are not updated to reflect the location where the backup has been restored. For information about the use and modification of service scripts, see the [EDB Postgres Advanced Server Installation Guide](https://www.enterprisedb.com/edb-docs).
