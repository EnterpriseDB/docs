...
For more details about incremental backup, see [Block-Level Incremental Backup](../../03_overview/01_block-level_incremental_backup/#block-level_incremental_backup).
...
-   `--parent { backup_id | backup_name }`
    ...
    > </div>
    >
    > An incremental backup cannot be taken on a standby database server. See [Section - Block-Level Incremental Backup](../../03_overview/01_block-level_incremental_backup/#block-level_incremental_backup) for additional information on incremental backups.
    >
    > </div>
