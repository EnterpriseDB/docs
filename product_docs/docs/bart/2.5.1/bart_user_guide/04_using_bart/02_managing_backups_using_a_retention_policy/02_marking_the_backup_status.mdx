...
After setting the `retention_policy` parameter and running the `MANAGE` subcommand to mark the backups according to that `retention_policy` setting if you change the `retention_policy` parameter, the current, marked status of the backups are probably inconsistent with the new `retention_policy` setting.

To modify the backup status to be consistent with the new `retention_policy` setting, you need to run the `MANAGE` subcommand with:
...
> **Note:** You can review the current marking (the status) of your backups with the `SHOW-BACKUPS` subcommand.
