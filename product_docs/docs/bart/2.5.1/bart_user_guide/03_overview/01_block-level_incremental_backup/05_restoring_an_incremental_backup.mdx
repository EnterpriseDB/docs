...
To restore an incremental backup onto a remote host where BART has not been installed, follow the restore process steps outlined in [Restoring an Incremental Backup on a BART Host](#restoring-an-incremental-backup-on-a-bart-host) and then, perform the following steps:
...
**Step 3:** Ensure a password-less SSH/SCP connection is established from the BART user on the BART host to the remote user on the remote host.

See the configuration section of the *EDB Postgres Backup and Recovery Installation and Upgrade Guide* for BART installation and configuration.
