...
To restore an incremental backup onto a remote host where BART has not been installed, follow the restore process steps outlined in [Restoring an Incremental Backup on a BART Host](../../03_overview/01_block-level_incremental_backup/#restoring_an_incremental_backup_on_a_bart_host) and then, perform the following steps:
