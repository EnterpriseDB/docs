...
> **Note:** While a BACKUP subcommand is in progress, no other processes must run in parallel. For more details about `BACKUP` subcommand, see the [Section - BACKUP](../../04_using_bart/03_basic_bart_subcommand_usage/#backup).
...
See the [BACKUP](../../04_using_bart/03_basic_bart_subcommand_usage/#backup) section for more information about using the `BACKUP` subcommand to take an incremental backup.
