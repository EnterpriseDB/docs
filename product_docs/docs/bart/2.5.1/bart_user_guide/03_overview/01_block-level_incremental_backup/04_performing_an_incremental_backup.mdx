...
The MBM files for the relevant WAL files are read, and the information is used to copy the modified blocks from the database cluster to the BART backup catalog. When compared to a full backup, the number and sizes of relation files can be significantly less for an incremental backup.
...
While a BACKUP subcommand is in progress, no other processes must run in parallel. For more details about `BACKUP` subcommand, see the [Section - BACKUP](../../04_using_bart/03_basic_bart_subcommand_usage/03_backup/#backup).
...
Specify the `-F p` option for plain text format as incremental backup can only be taken in the plain text format.

Specify the option `--check` before taking an incremental backup to verify if the required MBM files are present in the BART backup catalog. The `--parent` option must be specified when the `--check` option is used.

See the [BACKUP](../../04_using_bart/03_basic_bart_subcommand_usage/03_backup/#backup) section for more information about using the `BACKUP` subcommand to take an incremental backup.
