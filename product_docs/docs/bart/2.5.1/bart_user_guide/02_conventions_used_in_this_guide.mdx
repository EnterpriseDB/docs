...
-   The `--with-pg_basebackup` option is specified with the `BACKUP` subcommand for forcing the usage of `pg_basebackup` (see [Section - Backup](04_using_bart/03_basic_bart_subcommand_usage/03_backup/#backup)).
