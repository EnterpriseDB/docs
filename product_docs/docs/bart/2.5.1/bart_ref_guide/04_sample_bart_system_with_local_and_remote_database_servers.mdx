...
If the Postgres `archive_command` is already set, to prevent the `archive_command` from re-setting invoke the `INIT` subcommand with the `-- no-configure` option. For details, see [INIT](04_using_bart/03_basic_bart_subcommand_usage/#init) section.
