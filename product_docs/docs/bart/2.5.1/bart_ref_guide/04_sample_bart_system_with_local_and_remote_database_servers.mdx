...
-   [Replication Database User](#configuring_a_replication_database_user) Section provides example of how to configure the replication database user.
-   [WAL Archiving Configuration](#wal_archiving_configuration_parameters) Section provides example of WAL archiving configuration parameters in the `postgresql.conf` file to enable WAL archiving.
-   [BART Backup Catalog](#creating_the_bart_backup_catalog_backup_path) Section provides information about creating BART Backup Catalog.
-   [Database Servers](#starting_the_database_servers_with_wal_archiving) Section provides example of starting the database servers with WAL archiving.
-   [Backup](#taking_a_full_backup) Section illustrates the first full backup of the database servers.
-   [Point-In-Time Recovery](#using_point-in-time_recovery) Section demonstrates the point-in-time recovery operation on the remote PostgreSQL database server.
...
On the BART host where the public key file was just generated as shown in [Generating a Public Key File for the BART User Account](#generating_a_public_key_file_for_the_bart_user_account) Section create the authorized keys file by appending the public key file to any existing authorized keys file.
...
The public key file, `id_rsa.pub`, for BART user account `enterprisedb` on the BART host was generated in [Generating a Public Key File for the BART User Account](#generating_a_public_key_file_for_the_bart_user_account) Section, and is now copied to the remote Advanced Server host on `192.168.2.24`:
...
> **Note:** If the Postgres `archive_command` is already set, to prevent the `archive_command` from re-setting invoke the `INIT` subcommand with the `-- no-configure` option. For details, see [INIT](04_using_bart/03_basic_bart_subcommand_usage/#init) section.
...
> **Note:** Before invoking any BART subcommands, set up a profile under the BART user account’s home directory to set the `LD_LIBRARY_PATH` and `PATH` environment variables.
