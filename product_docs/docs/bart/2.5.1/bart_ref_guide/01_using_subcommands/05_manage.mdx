...
-   `-c { keep | nokeep }` or `--change-status { keep | nokeep }`
    ...
    -   Specify `keep` to change the backup status to `keep`. It is used to retain the backup indefinitely.
    -   Specify `nokeep` to change the status of any backup back to active status. You can then re-evaluate and possibly mark the backup as obsolete (according to the retention policy) using the `MANAGE` subcommand.

> **Note:** The `–c` option can only be used with the `-i` option.
