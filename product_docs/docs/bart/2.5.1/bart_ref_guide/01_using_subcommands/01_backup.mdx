...
> **Note:** While a `BACKUP` subcommand is in progress, no other subcommands (`INIT, DELETE, MANAGE, SHOW BACKUPS, VERIFY-CHKSUM`) must be issued. Any subcommands issued while a backup is in progress will skip and ignore the backups.
...
> **Note:** Specify `p` option while taking an incremental backup since an incremental backup can only be taken in plain text format.
