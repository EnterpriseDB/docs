...
**Step 1:** Install the latest version of BART. For information about how to install, see [installing BART](#installing_bart_using_an_rpm_package).
...
To specify an alternative configuration file name or location, use the `-c` option with BART subcommands. For more information about the `-c` option, see the EDB Postgres Backup and Recovery Us available at the [EDB website](https://www.enterprisedb.com/edb-docs/).

> **Note:** The `bart.cfg` configuration file is only required on the BART 2.6 host from which you will invoke BART subcommands. BART does not require the `bart.cfg` file on hosts on which an incremental backup will be restored.
