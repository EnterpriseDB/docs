...
When a [redundancy retention policy](../../03_using_bart/02_managing_backups_using_a_retention_policy/#redundancy_retention_policy) is used and the `MANAGE` subcommand is invoked, the status of the oldest `active` full backup is changed to `obsolete` if the number of full backups exceeds the maximum number specified by the `retention_policy` parameter in the BART configuration file.
...
See the *EDB Postgres Backup and Recovery Reference Guide* available at the [EDB website](https://www.enterprisedb.com/edb-docs/) for examples demonstrating use of the `MANAGE` and `DELETE` subcommands when a redundancy retention policy is in effect.

## Using a Recovery Window Retention Policy with Incremental Backups

If the `MANAGE` command is invoked when BART is configured to use a [recovery window retention policy](../../03_using_bart/02_managing_backups_using_a_retention_policy/#recovery_window_retention_policy), the status of `active` full backups are changed to `obsolete` if the date/time of the full backup is outside of the recovery window.
...
See the *EDB Postgres Backup and Recovery Reference Guide* available at the [EDB website](https://www.enterprisedb.com/edb-docs/) for examples demonstrating use of the `MANAGE` and `DELETE` subcommands when a recovery window retention policy is in effect.
