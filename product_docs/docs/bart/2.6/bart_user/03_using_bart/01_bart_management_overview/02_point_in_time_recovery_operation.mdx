...
1.  Copy any saved WAL files from Step 2 that were not archived to the BART backup catalog to the `restore_path/pg_wal` subdirectory (`pg_xlog` subdirectory if you are using server 9.6 version).
...
For a detailed description of the `RESTORE` subcommand, see [Basic BART Subcommand Usage](../../03_using_bart/03_basic_bart_subcommand_usage/#basic_bart_subcommand_usage). An example of a Point-in-Time Recovery operation is documented in the *EDB Postgres Backup and Recovery Reference Guide* available at the [EDB website](https://www.enterprisedb.com/edb-docs/). See [Restore](../../03_using_bart/03_basic_bart_subcommand_usage/#restore) for more information about using the `Restore` subcommand.
