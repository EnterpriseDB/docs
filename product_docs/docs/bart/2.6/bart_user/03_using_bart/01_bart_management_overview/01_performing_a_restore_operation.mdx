...
See [Restore](../../03_using_bart/03_basic_bart_subcommand_usage/#restore) for more information about using the BART `Restore` subcommand.
...
> **Note:** If you set the [backup](../../03_using_bart/03_basic_bart_subcommand_usage/#backup) `--checksum-algorithm=NONE` option, then you must specify the `--disable checksum` option while restoring a backup.
