...
5.  Install an Advanced Server or PostgreSQL database server.
    ...
    > **Note:** The BART host server is not required to have an Advanced Server or PostgreSQL installation, but must include a copy of the Postgres `libpq` library, the `pg_basebackup` utility program, and Boost Libraries 1.53 version for CentOS 7.
...
2.  Authorize <span class="title-ref">SSH/SCP access &lt;passwordless&gt;</span> between the server and the BART host without a password prompt.
...
4.  Append the client’s temporary public key file, `tmp.pub`, to the `authorized_keys` file:
...
An example of how to create a passwordless connection is documented in the *EDB Postgres Backup and Recovery Reference Guide*, available at the [EDB website](https://www.enterprisedb.com/edb-docs/).
...
1.  Ensure only the file owner (and not other groups or users) has access to `authorized_keys` file:
