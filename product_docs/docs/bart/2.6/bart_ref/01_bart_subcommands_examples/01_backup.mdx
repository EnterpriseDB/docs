...
> **Note:** To restore an incremental backup taken with the `--checksum-algorithm=NONE` option, you must specify [--disable-checksum while restoring](../01_bart_subcommands_examples/#restoring_an_incremental_backup_using_disable_checksum).
