...
> **Note:** If the Postgres `archive_command` is already set, invoke the `INIT` subcommand with the `-- no-configure` option to prevent the `archive_command` from being reset. For details, see [INIT](03_using_bart/03_basic_bart_subcommand_usage/#init).
