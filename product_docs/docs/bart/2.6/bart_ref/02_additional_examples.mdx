...
-   [Using a Redundancy Retention Policy](03_using_bart/02_managing_backups_using_a_retention_policy/#redundancy_retention_policy) provides an example of using the `MANAGE` and `DELETE` subcommands when a 3 backup redundancy retention policy is in effect.
-   [Using a Recovery Window Retention Policy](03_using_bart/02_managing_backups_using_a_retention_policy/#recovery_window_retention_policy) provides an example of using the `MANAGE` and `DELETE` subcommands when a 1-day recovery window retention policy is in effect.
