...
> **Note:** While a `BACKUP` is in progress, no other subcommands (`INIT, DELETE, MANAGE, SHOW BACKUPS, VERIFY-CHKSUM`) should be issued. Any subcommands issued while a backup is in progress will skip and ignore the backups.
...
> **Note:** An incremental backup can only be taken in plain text format (`p`).
