...
> **Note:** After upgrading to the latest BART version, you must take a new full backup of your system before performing an incremental backup.
...
> **Note:** The `bart.cfg` configuration file is only required on the BART 2.5.5 host from which you will invoke BART subcommands. BART does not require the `bart.cfg` file on hosts on which an incremental backup will be restored.
