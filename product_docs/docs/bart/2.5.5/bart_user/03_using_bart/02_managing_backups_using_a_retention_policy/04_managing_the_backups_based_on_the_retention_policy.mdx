...
> **Note:** Redundancy Retention Policy (Note1) : Deletion occurs only if the total number of backups for the specified database server is greater than the specified, maximum number of redundancy backups currently set with the `redundancy_policy` parameter in the BART configuration file.


<div id="note2" class="registered_link"></div>

> **Note:** Recovery Window Retention Policy (Note2): Deletion occurs only if the backup is not within the recovery window currently set with the `redundancy_policy` parameter in the BART configuration file.
