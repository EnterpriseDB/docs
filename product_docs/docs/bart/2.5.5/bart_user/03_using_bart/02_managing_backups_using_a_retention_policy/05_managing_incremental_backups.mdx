...
When a [redundancy retention policy](../../04_using_bart/02_managing_backups_using_a_retention_policy/#redundancy_retention_policy) is used and the `MANAGE` subcommand is invoked, the status of the oldest `active` full backup is changed to `obsolete` if the number of full backups exceeds the maximum number specified by the `retention_policy` parameter in the BART configuration file.
