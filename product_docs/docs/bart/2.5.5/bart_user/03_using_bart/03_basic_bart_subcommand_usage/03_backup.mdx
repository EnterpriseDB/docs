...
-   When you use BART to take a backup of:
    ...
    > -   PostgreSQL server version 9.5, only one backup per server may be in progress at any given time and if a backup is interrupted, you must manually run the pg_stop_backup() command to terminate the backup mode.
