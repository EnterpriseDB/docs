...
The `CHECK-CONFIG` subcommand checks the global parameter settings in the BART configuration file as well as the database server configuration for which the `-s` option is specified.
