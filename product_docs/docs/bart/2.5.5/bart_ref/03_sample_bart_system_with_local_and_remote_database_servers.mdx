...
Then, generate the public key file:
...
Before invoking any BART subcommands, set up a profile under the BART user account’s home directory to set the `LD_LIBRARY_PATH` and `PATH` environment variables. For more information regarding setting this variable, see the EDB Postgres Backup and Recovery Installation and Upgrade Guide available at the [EDB website](https://www.enterprisedb.com/edb-docs/).
...
This section demonstrates using the point-in-time recovery operation on the remote PostgreSQL database server. The following tables were created about two minutes apart with WAL archiving enabled:
