...
1. Navigate to the [BigAnimal portal](https://freetrial.biganimal.com/). Sign in with [your account](../create_an_account) if you need to.

2. Select the **Clusters** link on the left to navigate to the [Clusters](https://portal.biganimal.com/clusters) page.

3. Select **Create New Cluster**.
...
1. Select your **Cluster Type**. Pick **Single Node**. If you're interested in learning more about the BigAnimal's high-availability deployment options, see [Cluster types](/biganimal/release/overview/02_high_availability).
...
1. **Region** options depend on the provider you chose. Select the one closest to your application.

1. For **Instance Type** and **Storage**, only one option is available for each field during the trial, so just choose that. The full sets of instance types and storage options are available in the BigAnimal paid subscriptions.
