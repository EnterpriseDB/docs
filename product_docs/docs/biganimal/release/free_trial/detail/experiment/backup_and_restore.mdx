...
!!! Note
    For more on using the BigAnimal CLI, see [Creating a cluster on the command line](../create_a_cluster/create_cluster_cli) and the [BigAnimal CLI reference](/biganimal/latest/reference/cli/).

```shell
biganimal cluster delete --id p-xxxxxxxxxxx
```
