...
Review the EDB [Migration Handbook](/migrating/oracle/) for helpful considerations and information when migrating from Oracle. EDB also provides a tool, [Migration Portal](/migration_portal/latest), which provides the details for executing the migration steps: 
