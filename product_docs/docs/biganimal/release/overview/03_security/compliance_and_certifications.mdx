...
SOC 2 reports are unique to each organization. In line with specific business practices, each designs its own controls to comply with one or more of the trust service principles. BigAnimal is assessed on security, availability, and confidentiality trust service principles. A SOC 2 report arrives in two formats:
