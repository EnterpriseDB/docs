...
-   **Data encryption:** 
    ...
    - **Your own encryption key** - Optionally enable Transparent Data Encryption (TDE) at the cluster level or at the database level for BigAnimal's cloud account on AWS. You can't enable TDE on existing clusters. You can enable TDE, only while creating a cluster. To enable TDE, use your own encryption keys. Connect the encryption keys to BigAnimal at the project level and select those keys to encrypt the clusters.  
