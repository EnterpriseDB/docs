...
BigAnimal runs in your own cloud account or BigAnimal's cloud account, isolates your data from other users, and gives you control over our access to it. The key security features are:
...
-   **Database cluster permissions** The edb_admin account created during the *create cluster* process includes the CREATEDB and CREATEROLE database roles. We recommend using the edb_admin account to create a new application user and new application database for further isolation. See [Managing Postgres access](../../using_cluster/01_postgres_access/) for more information.
