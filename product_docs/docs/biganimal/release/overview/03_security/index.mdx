...
EDB supports enabling TDE with your own encryption key on EDB Postgres Advanced Server and EDB Postgres Extended Server versions 15 and later. Both the key and the cluster must be in the same region and hosted by the same underlying cloud provider. 
...
**BYOA or Bring your own access:** BigAnimal deploys the cluster on your own cloud provider account.
