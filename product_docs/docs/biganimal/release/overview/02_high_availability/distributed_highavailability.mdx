...
If read-only workloads are enabled, only the shadow nodes will serve the read-only workloads. In a two-node cluster, the single shadow node serves read-only workloads. In a three-node cluster, the two shadow node serves read-only workloads, unless a witness node is enabled. In a three-node cluster with a witness, read-only workloads are served by the single shadow node.

Similarly, if you have three data groups, and one of them is a witness, only the shadow data group will serve read-only workloads. 
