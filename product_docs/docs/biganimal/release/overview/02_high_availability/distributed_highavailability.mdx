...
!!!Note
   ...
   Operations against a distributed high-availability cluster leverage the [EDB Postgres Distributed group set leader](/pgd/latest/cli/command_ref/pgd_group_set_leader/) feature, which provides subsecond interruptions during planned lifecycle operations.
