...
When you enable read-only workloads, a read-only connection string is created. You can use this connection to route read-only requests through the shadow nodes (non-write leaders) to lighten the load on the write leaders. In a two-node cluster, the single shadow node (non-write leader) serves read-only workloads. In a three-node cluster, the two shadow nodes (non-write leaders) serve read-only workloads, unless a witness node is enabled. In a three-node cluster with a witness, read-only workloads are served by the single shadow node (non-write leader) only.

Similarly, if you have three data groups, the witness group will not serve any data requests. 
