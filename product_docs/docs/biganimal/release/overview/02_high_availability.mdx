...
EDB Postgres Distributed uses the feature [Commit Scopes](/pgd/latest/commit-scopes) to describe the behavior of COMMIT replication for [Group Commit](/pgd/latest/durability/group-commit/) and [Lag Control](/pgd/latest/durability/lag-control/). Group Commit is a synchronous commit behavior while Lag Control is the BigAnimal configuration for asynchronous commit behavior. To learn more about configuring Commit Scopes, see [Configuration](/pgd/latest/durability/commit-scopes/#configuration).

Group Commit is the default commit behavior for data nodes, but can also be used per [transaction](/pgd/latest/durability/group-commit/#configuration). [Transaction streaming](/pgd/latest/transaction-streaming/) is disabled for Group Commit.

!!! Note
    There is a default value set for `max_prepared_transactions` on Group Commit. Check that the `max_prepared_transactions` value is high enough to handle the number of transactions originating per node in you cluster. 