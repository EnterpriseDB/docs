...
While a single-region configuration with three data nodes (one lead and two shadow nodes each in separate availability zones) is available, if you're looking for a true active-active solution that protects against regional failures, select a three-region configuration with a data node, shadow node, and a witness node in one region, the same configuration in another region, and a witness node in a third region. 
