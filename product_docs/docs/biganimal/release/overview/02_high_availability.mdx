...
Since BigAnimal replicates to only one node synchronously, some standby replicas in three-node clusters may experience replication lag. Additionally, if you override the BigAnimal synchronous replication configuration, then the standby replicas exhibit inconsistencies. 
