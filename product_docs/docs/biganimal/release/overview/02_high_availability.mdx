...
EDB Postgres Distributed uses the feature [Commit Scopes](/pgd/latest/commit-scopes) for [Group Commit](/pgd/latest/durability/group-commit/) and [Lag Control](/pgd/latest/durability/lag-control/). Group Commit is the default commit behavior for data nodes, but can also be used per [transaction](/pgd/latest/durability/group-commit/#configuration). 

There is a default value for `max_prepared_transactions` on Group Commit. Also, transaction streaming is disabled. 