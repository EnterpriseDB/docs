...
- two data nodes, one a lead and one a shadow, and a witness node each in separate availability zones 

  ![region(2 data + 1 witness) +  (1 witness)](images/1dg-2dn-1wn-1wg.svg)

- three data nodes with one lead and two shadow nodes each in separate availability zones 

  ![region(3 data) +  (1 witness)](images/1dg-3dn-1wg.svg)

### Multiple data locations

A multiple data location configuration has two data groups and either:

- A 
  - 
  ![region(2 data + 1 shadow) + region(2 data + 1 shadow) + region(1 witness)](images/2dn-1wn-2dn-1wn-1wg.png)

- A true active-active production configuration with: 
