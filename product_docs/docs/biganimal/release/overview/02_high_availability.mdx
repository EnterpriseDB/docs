...
The high availability option is provided to minimize downtime in cases of failures. High-availability clusters—one *primary* and up to four *standby replicas*—are configured automatically, with standby replicas staying up to date through physical streaming replication. 
