...
Pricing is based on the number of virtual central processing units (vCPUs) provisioned for the database software offering. Consumption of vCPUs is metered hourly. A deployment is made up of either one instance or one primary and two standby replica instances of either PostgreSQL or EDB Postgres Advanced Server. For high availability cluster types, multiply the number of vCPUs per instance by three to calculate the full price for all resources used. For extreme high availability cluster types, add the costs for the EDB Postgres Distributed nodes that host data.
...
The additional cost for extreme high availability cluster types for each EDB Postgres Distributed node that hosts data is $0.0856 / vCPU / hour or $750 / vCPU / year.
