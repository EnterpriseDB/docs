...
1. On the **Setup Config** tab on the Set Up Identity Provider page in the BigAnimal(open the same link in the email sent from cloudcare@enterprisedb.com): 
   ...
   1. For any further [BigAnimal](https://portal.biganimal.com/overview) sign in, select **Sign in to your BigAnimal account** instead of the button of **Sign in for Azure Marketplace users**.
