...
1. On the **Setup Config** tab on the Set Up Identity Provider page in the BigAnimal(open the same link in the email sent from cloudcare@enterprisedb.com): 
   ...
   1. Select **Test Connection**. If the connection is successful, select **Sign in to your BigAnimal account** to complete the setup process in the BigAnimal portal(Do NOT click the button of **Sign in for Azure Marketplace users**).
