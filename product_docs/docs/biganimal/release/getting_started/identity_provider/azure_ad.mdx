...
1. Select **Properties** from the left navigation pane. On the Properties page, if you do not want all of the users in Azure AD to be able to use BigAnimal, answer `No` for the **Assignment required?** question. Otherwise, answer `Yes` and assign the specified users to this Enterprise Application explicitly on the Users and Groups page.
