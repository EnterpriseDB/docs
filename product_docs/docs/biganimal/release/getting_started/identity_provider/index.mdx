...
You add users through your identity provider. A user who is added in the identity provider is automatically added to BigAnimal. BigAnimal assigns them with the default role of reader. You manage roles and permissions from BigAnimal. See [Managing portal access](/biganimal/latest/administering_cluster/01_portal_access/). 
