...
1. Select **Done**. 
   ...
   If your domain doesn't appeared as verified after some time, you can debug whether your domain has the matching verification text field. Query it directly with DNS tools, such as nslookup.

   Select **Verify** next to the domain at `/settings/domains` to check whether this matches the required TXT field. Domains can have many TXT fields. As long as one matches, it works.
