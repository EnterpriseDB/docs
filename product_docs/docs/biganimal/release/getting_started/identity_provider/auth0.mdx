...
1. You will be copying and pasting between BigAnimal and Auth0. In a separate browser tab or window, log into [auth0.com](https://auth0.com/), navigate to the Applications page, and select **Create Application**.

1. In the Create Application dialog box, enter a name and choose **Native** as the application type. Select **Create**.
...
1. Select the **Settings** tab on the Addon: SAML2 Web App dialog box.
...
1. Copy the Audience URI from the Set Up Identity Provide page in BigAnimal and paste it as the audience value in the **Settings** field. The following attributions statements might need changes depending on the form of your user names. For example, user names from GitHub don't have last name and first name fields.
...
1. Select **Enable**.
1. Select the **Usage** tab, and copy the Identity Provider Login URL. 
   !!!note
