...
1. Choose **Enable AWS SSO** or if SSO is already enabled on your account, then continue to the next step. You may need to switch to a region where SSO is enabled (AWS Organizations only support SSO in one region at a time).
...
1. On the Setup Identity Provider page in BigAnimal, select the **Setup Config** tab. 
   ...
   1. Select **Choose File** and select the **AWS SSO certificate** from your files. 
