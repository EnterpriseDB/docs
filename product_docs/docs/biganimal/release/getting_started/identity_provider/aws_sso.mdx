---
title: "Using AWS IAM Identity Center as your identity provider"
navTitle: "AWS IAM Identity Center"
description: "Using AWS IAM Identity Center as your identity provider for your Azure account"
---

## Prerequisites

- To connect BigAnimal to AWS iAM Identity Center, your AWS account must have administrator access. 
...
## Set up BigAnimal with AWS IAM Identity Center
...
1. You will be copying and pasting between BigAnimal and AWS. In a separate browser tab or window, log into your AWS account and go to the [AWS SSO console](https://console.aws.amazon.com/singlesignon).
1. If SSO isn't already enabled on your account, select **Enable AWS SSO**. 
   !!! note
   You might need to switch to a region where SSO is enabled. AWS organizations support SSO in only one region at a time.
   !!!
...
1. On the Applications page, select **Add a new application**. 

1. On the Add New Application page:
...
1. At the bottom of the AWS page, select **Save changes**. 
1. Go to your newly created application (appearing under its display name) and select the **Attribute mappings** tab. 
1. Enter your desired attribute configuration. We recommend the following:
...
1. To allow users access to the application, on the **Assigned users** tab, select **Assign users**. 
1. On the **Configuration** tab, scroll down to the **AWS SSO metadata** section. 
   1. Copy the **AWS SSO sign-in URL**.

   2. To install the AWS SSO certificate, select **Download certificate**.
1. On the Setup Identity Provider page in BigAnimal, select the **Setup Config** tab. 
   ...
   1. For the **Request Binding**, select **HTTP-POST**.
   1. Enter a **Response Signature Algorithm**. We recommend rsa-sha256.
