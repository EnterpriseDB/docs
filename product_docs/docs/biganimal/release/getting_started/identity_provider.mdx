...
1. Configure your identity provider to send the SAML response. The SAML response must include the following attributes:
   ...
   Provide a `NameID` element in the `Subject` element. Provide your email ID as the value to `NameID`. BigAnimal uses the email ID you provide as your username and primary email, so `NameID` must be formatted like an email address.
...
1. Select **Test Connection**. If you connect to your identity provider successfully, your identity provider's login screen appears. If an error message appears, contact [Support](../overview/support). 
