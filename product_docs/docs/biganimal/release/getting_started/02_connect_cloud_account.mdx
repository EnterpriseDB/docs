...
To simplify the process of creating an Azure AD Application, EDB provides the `create-spn` script for Azure API users (available [here](https://github.com/EnterpriseDB/cloud-utilities/blob/main/azure/create-spn.sh)). The script automates the creation of the Active Directory Application. 
