...
To simplify the process of creating an Azure AD Application, EDB provides the `create-spn` script. The script automates the creation of the Active Directory Application. 

The help provides usage details:

  ```
  Usage:
    bash -d NAME -s SUBSCRIPTION_ID [-i CLIENT_ID] [options]

  Options:
    -d, --display-name: The name of Azure AD App.
    -s, --subscription: The Azure Subscription ID used by BigAnimal.
    -y, --years:        [Optional] The Number of years for which the credentials will be valid. Only accept positive integer value. Default: 1 year.
    -i, --id:           [Optional] The Application (client) ID of Service Principal will be updated.
...
You can run the script using either the:
- [Azure Cloud Shell](#running-the-script-using-azure-cloud-shell)
- [BigAnimal CLI](#running-the-script-using-the-biganimal-cli)

#### Running the script using Azure Cloud Shell

To run the script using Azure Cloud Shell:

1.  Open the [Azure Cloud Shell](https://shell.azure.com/) in your browser.

2.  From the Azure Cloud Shell, run the following command:  
    ```
    curl -sL https://raw.githubusercontent.com/EnterpriseDB/cloud-utilities/main/azure/create-spn.sh \
    | bash -s -- \
    --display-name <NAME> \
...
```
#### Running the script using the BigAnimal CLI

If you choose to download the script (available [here](https://github.com/EnterpriseDB/cloud-utilities/blob/main/azure)) and run it using the BigAnimal CLI, ensure that these utilities are installed on your machine:

  -   [jq command-line JSON processor](https://stedolan.github.io/jq/)
  -   [azure cli](https://docs.microsoft.com/en-us/cli/azure/) v2.26 or later
