...
!!! Note
    ...
    To run CLI commands for your AWS account, you must have the following AWS managed policies assigned (or an equivalent custom policy granting full access to resources):
...
1. Log in to BigAnimal.

1. On the Cloud Providers page in BigAnimal, select **Download BigAnimal CLI**. For more information, see [Using the CLI](../../reference/cli).
...
1. Input your cloud provider and account ID (if using AWS) or subscription ID (if using Azure). The CLI sets up the boundary policy, designates the BigAnimal role and access to your cloud provider, and checks and raises account limits as needed.
