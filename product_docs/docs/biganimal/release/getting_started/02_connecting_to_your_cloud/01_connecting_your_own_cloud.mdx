...
1. If the cloud readiness checks pass, the `ba-csp-setup` script is invoked to set up your cloud account. Connect your cloud account to BigAnimal with the following command. 
   ```shell
   biganimal connect-csp --provider  <cloud-service-provider> 
   ```
!!! Tip
    Your organization might require you to review the scripts that are invoked while connecting to your cloud account. To generate the scripts invoked by `setup-csp`, execute the command without the `--run` option. You can now execute `ba-csp-preflight` and then `ba-csp-setup` from your working directory. 
