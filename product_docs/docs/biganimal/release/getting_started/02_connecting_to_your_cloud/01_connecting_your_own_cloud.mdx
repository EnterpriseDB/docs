...
1. Run the `setup-csp` command to set up your cloud provider. The synatx is:
   ...
   ```
   !!! Tip
       Your organization might require you to review the scripts that are invoked while setting up your cloud account. To generate the scripts invoked by `setup-csp`, execute the command without the `--run` option. You can now review and execute `ba-csp-preflight` and then `ba-csp-setup` from your working directory. 
  
1. The command checks for cloud account readiness and displays the results. See [Configure your Azure subscription](/biganimal/release/getting_started/preparing_cloud_account/01_preparing_azure#configure-your-azure-subscription) or [Configure your AWS account](/biganimal/release/getting_started/preparing_cloud_account/02_preparing_aws/#configure-your-aws-account) to manually configure your cloud, if either of the following readiness checks are not met:    
...
1. If the cloud readiness checks pass, your cloud account is successfully set up. Connect your cloud account to BigAnimal with the following command.
   ```shell
   biganimal connect-csp --provider  <cloud-service-provider> 
   ```
   !!! Important
       Do not delete the `ba-passport.json` file created in your working directory during setup. It contains important identity and access management information used by `connect-csp` while connecting to your cloud.
