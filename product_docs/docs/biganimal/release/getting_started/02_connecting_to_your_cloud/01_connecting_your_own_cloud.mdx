...
To access BigAnimal for the first time, you have to set up your identity provider (IDP). After setting up your identity provider, your identity provider can add users and you can set up your cloud server provider (CSP) in the BigAnimal portal. 
...
!!! Note
    ...
    In order to run the commands for your AWS account, you must have the following AWS managed policies assigned (or an equivalent custom policy granting full access to resources):
    
    - `arn:aws:iam::aws:policy/IAMFullAccess`
    - `arn:aws:iam::aws:policy/ServiceQuotasFullAccess`
    
    To run the commands for your Azure account, you must have one of the following roles in Azure AD:
