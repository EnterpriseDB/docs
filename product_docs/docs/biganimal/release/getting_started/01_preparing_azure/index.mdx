...
2.  From the Azure Cloud Shell, run the following command:  
    ...
    -   `<instance-type>`: Enter the Azure VM instance type for the BigAnimal cluster. Running the `biganimal-preflight-azure` script with no arguments provides a list of possible VM instance types.
    ...
    -   `--onboard`: Checks if the user and subscription are correctly configured. The behavior of the script defaults to `--onboard` if you provide no other options.
