...
Every BigAnimal cluster with public network access is assigned a single public IP address, and this IP address counts against the quota for both **Basic** and **Standard** IP address types in a region. BigAnimal cannot create more clusters if this IP address limit is reached.
...
You might encounter an error while deploying your clusters if the SKU limit for virtual machine is reached in the region of your cluster.
