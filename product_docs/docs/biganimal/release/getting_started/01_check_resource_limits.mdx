...
---
Before you can use EDB Cloud in your Azure subscription to deploy your Postgres clusters in the designated regions, your Azure subscription has to meet certain requirements:

* [Registered *Azure Service providers*](#azure-service-providers)
* [Raising Azure resource limits on *vCPU*, and *IP Addresses* in the designated regions](#raising-azure-resource-limits)
* [Absence of any restrictions on *Virtual Machine SKU* in the designated regions](#virtual-machine-sku)


To automate EDB Cloud preflight chek, EDB provides shell script [azure-preflight.sh](https://github.com/EnterpriseDB/cloud-utilities/blob/main/azure/azure-preflight.sh). The comment's section of the script contains a description of its usage.
...
EDB Cloud deploys the “Infrastructure” to manage the PG clusters in your Azure subscription. This infrastructure is deployed in  every region you designated for the deployment of your Postgres cluster. The infrastructure contains VM(Virtual Machines, used as the AKS worker nodes) vCPU cores and Public IP(used as the Azure Standard LoadBalancer for the user to access its PG clusters).
