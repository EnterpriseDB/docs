...
### Google load balancer in GCP

BigAnimal creates a new load balancer using the Premium Network Service Tier for each cluster and tags using an unique identifier. The corresponding Frontend forwarding rule will be configured referrind the same identifier plus a cluster ID in the following format:
...
Because the load balancer IP address used in GCP is dynamic, make sure that your application uses the correct DNS name to access the network load balancer of a particular cluster. To be able to access it, make sure you are using the FQDN that BigAnimal provides in the Cluster Overview or Connect page.
