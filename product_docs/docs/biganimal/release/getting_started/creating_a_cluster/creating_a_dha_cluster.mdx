...
1. Volume Snapshots
   ...
   Enable the **Volume Snapshots** to take the snapshot backups. The snapshot backups are stored on the disk in the same region without degrading the performance. It may increase the storage costs as per your cloud service providers. By default, the snapshot backups are stored on the disk for 30 days.
