---
title: "Creating database links in AWS"

---

This topic describes how you can set up the infrastructure for linking an existing Oracle database to your cluster in AWS. This setup can allow you to use and evaluate your cluster without risking your production environment.
...
5. In the Backend pools tab, select **+ Add a backend pool**,  enter, or select the following information:
   ...
   2. Select **+Add** and add the virtual machine on which the Oracle database is deployed (`client-vm`). Ensure that the SKU of the virtual machine and the backend pool is the same, as noted in [Azure Load Balancer SKUs](https://docs.microsoft.com/en-us/azure/load-balancer/skus). 
