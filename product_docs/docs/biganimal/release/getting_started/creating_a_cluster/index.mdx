...
The Create Cluster documentation allows for customizable maintenance windows. When enabled, a 60-minute weekly window is set for maintenance upgrades, while disabling it means upgrades can occur at any time. It is important to note that the maintenance window only specifies the start time and not the end time of maintenance events. Additionally, maintenance events are not guaranteed to end before the user-specified end time, as stated in AWS's maintenance documentation.
