...
4.  In the **Instance Type** section, 
    - For Azure, select the category that works best for your applications and workload: memory optimized for large data sets, memory optimized for large data sets, or memory optimized for large data sets. Select the instance series and size. See [Sizes for virtual machines in Azure](https://docs.microsoft.com/en-us/azure/virtual-machines/sizes) for information to help you choose the appropriate instance type. 
    
    - For AWS, select the instance type (EC2 R5 or Rb5). 
