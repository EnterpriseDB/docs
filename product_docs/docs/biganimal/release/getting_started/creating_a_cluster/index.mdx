...
PgBouncer can manage your connections to Postgres databases and help your workloads run more efficiently. This tool is particularly useful if you plan to use more than a few hundred active connections. For more information, visit the [PgBouncer documentation](https://www.pgbouncer.org/community.html).

BigAnimal provisions up to three instances per PgBouncer-enabled cluster to ensure that performance is unaffected, so each availability zone receives its own instance of PgBouncer. If you want to customize your PgBouncer deployment, see the [How to configure PgBouncer with BigAnimal Cluster](https://support.biganimal.com/hc/en-us/articles/4848726654745-How-to-configure-PgBouncer-with-BigAnimal-Cluster) knowledgebase article.

Enable **PgBouncer** to have it manage your connections to Postgres databases and help your workloads run more efficiently. 

Use the **PgBouncer Configuration Settings** dropdown menu to set PgBouncer-specific GUCs. Select the Read-Write and Read-Only tabs according to the type of connection you wish to configure. 
