...
1. Select which type of cluster to deploy. 
     ...
     - [Single node](bigamimal/release/03_security/#single-node) creates a cluster with one primary and no standby servers. Suited for test environments where high availability might not be required. You can create single node clusters running EDB Postgres Advanced Server or PostgreSQL. 
...
1.  In the **Database Type** section:
    ...
    1.  Select the type of Postgres you want to use in the **Postgres Type** field:
        ...
        -   [EDB Postgres Advanced Server](/epas/latest/) is EDB’s secure, Oracle-compatible PostgreSQL. View [a quick demonstration of Oracle compatibility on BigAnimal](../../using_cluster/06_demonstration_oracle_compatibility). EDB Postgres Advanced Server is compatible with all three cluster types.
...
1.  In the **Region** section, select the region where you want to deploy your cluster. For the best performance, EDB strongly recommends that this region be the same as your other resources that communicate with your cluster. For a list of available regions, see [Supported regions](../../overview/03a_region_support).
