...
1.  In the **Storage** section, select your volume type from the **Volume Type** list. 
    ...
    - For AWS, BigAnimal currently supports General Purpose SSD (GP3), io2, io2 Block Express storage. In **Volume Properties**, select the disk size for your cluster and configure the IOPS for better performace.
<!-- Can you tune IOPS for GP3 as mentioned in the provisioning storage matrix--> 
<!-- Do we need to mention min max values?-->
<!-- Should we recommend user to configure vm IOPS to gain maximum performance?-->
