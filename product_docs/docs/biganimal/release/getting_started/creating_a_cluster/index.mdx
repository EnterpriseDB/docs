...
1.  In the **Storage** section, choose to use Write-Ahead Logs and then select your volume type from the **Volume Type** list. 
       - To add a separate storage volume for Write-Ahead Logs, under Storage click the checkbox. Creating a dedicated storage volume for WAL is optional. What is not optional is selecting your database storage type and volume properties. Azure offers Premium SSD Managed Disks and Ultra Disks for BigAnimal clusters. By enabling a separate Write-Ahead Log (WAL) storage volume, you can run transaction logging and database operations in parallel, enhancing cluster performance. However, enabling this incurs additional costs for a second storage volume. Refer to Azure [Managed Disks pricing](https://azure.microsoft.com/en-us/pricing/details/managed-disks/) docs for pricing and optimization information. Once digital self-service is released in Q2, users on digital-self service accounts will also be able to dedicate storage volumes for WALs in the same ways as for BYOA and azuremp accounts.
