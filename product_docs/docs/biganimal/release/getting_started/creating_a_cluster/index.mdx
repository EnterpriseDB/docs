...
1. Select which type of cluster to deploy. 
    ...
    -  [Extreme high availability](/biganimal/latest/overview/02_high_availability/#extreme-high-availability) creates a cluster configured with a leader node, three shadow nodes, and one witness node. This cluster uses EDB Postgres Distributed to deliver higher performance and faster recovery. You can create extreme high availability clusters with either PostgreSQL or Oracle compatibility. 
