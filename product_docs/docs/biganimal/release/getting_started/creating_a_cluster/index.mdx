...
!!! Tip
    BigAnimal might take a while to provision the infrastructure for a region before creating the first cluster of that region. To save time, you can activate the region and perform its provisioning in advance. See, [Activating regions](../activating_regions).
