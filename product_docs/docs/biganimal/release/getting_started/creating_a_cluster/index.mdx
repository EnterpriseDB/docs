...
- If your cloud provider is Azure, see [Raising your Azure resource limits](../01_check_resource_limits).
- If your cloud provider is AWS, see [AWS resources](https://docs.aws.amazon.com/general/latest/gr/aws-resources.html).
!!!
...
4.  In the **Instance Type** section, 
    - For Azure, select the category that works best for your applications and workload: memory optimized for large data sets, compute optimized for compute bound applications, or general purpose if you don't require memory or compute optimization. Select the instance series and size. See [Sizes for virtual machines in Azure](https://docs.microsoft.com/en-us/azure/virtual-machines/sizes) for information to help you choose the appropriate instance type. 
    
    - For AWS, select the instance series (AWS EC2 R5 or Rb5). 
5.  In the **Storage** section, select your volume type from the **Volume Type** list. In **Volume Properties**, select the type and amount of storage needed for your cluster.
    - For Azure, BigAnimal currently supports Azure Premium SSD storage types. See [the Azure documentation](https://docs.microsoft.com/en-us/azure/virtual-machines/disks-types#premium-ssds) for more information.

    - For AWS, select your storage type: io2 Provisioned IOPS SSD or io2 Block Express Provisioned IOPS SSD (only available for R5b class instance types). Choose your disk size. Configure your IOPs. BigAnimal automatically calculates throughput based on the IOPs setting.
