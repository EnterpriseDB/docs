...
1.  In the **Storage** section, select your volume type from the **Volume Type** list. For Azure, you can optionally choose to use a separate volume for write-ahead logs (WAL).       
       
    You can optionally enable a separate write-ahead log (WAL) storage volume to run transaction logging and database operations in parallel, enhancing performance for single node, high availability, and faraway replica clusters. This incurs additional costs for a second storage volume. However, depending on the volume size and properties, there may be cost savings associated with WAL separation. If using Azure, refer to Azure [Managed Disks pricing](https://azure.microsoft.com/en-us/pricing/details/managed-disks/) for pricing and optimization information. If you are using AWS, see [Amazon EBS pricing](https://aws.amazon.com/ebs/pricing/).
