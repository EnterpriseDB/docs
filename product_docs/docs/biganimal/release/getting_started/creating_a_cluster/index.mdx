...
Enable **Superuser Access** to grant superuser privileges to the edb_admin user. This option is available for single-node and high-availability clusters. See [Notes on the edb_admin user](/biganimal/latest/using_cluster/01_postgres_access/#notes-on-the-edb_admin-user).
