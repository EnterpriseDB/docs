...
1. Select which type of cluster to deploy.
     ...
     - *Single node cluster*
         Suited for test environments where high availability is not a primary concern. Creates a cluster with one primary and no standby servers.
    ...
    -  *Extreme High Availability Cluster* 
         Creates a cluster configured with a leader node, three shadow nodes and one witness node. This cluster uses a logical replication to deliver faster failover and recovery. 
