...
!!!Note "When using your own cloud"
...
- If your cloud provider is GCP, see [Preparing your GCP account](/biganimal/release/getting_started/preparing_cloud_account/preparing_gcp).
...
- Extreme High Availability cluster type
...
1.  In the **Database Type** section:
    ...
    1.  In the **Postgres Type** field, select the type of Postgres you want to use:
        ...
        -   **[PostgreSQL](/supported-open-source/postgresql/)** is the open-source, object-relational database management system. PostgreSQL is compatible with single-node and high-availability cluster types.
...
1.  In the **Storage** section, from the **Volume Type** list, select your volume type. 
       ...
       - For GCP, in  **Volume Type**, select **SSD Persistent Disk**. 
...
2.  ##### **Networking** section
    ...
    If you are using private networking and you're using BigAnimal's cloud account, you must enter your cloud service provider's account information. BigAnimal uses it to auto-approve your private endpoints and also to provide access for logging and telemetry. To be able to create your private endpoint, BigAnimal provides you with the corresponding service name, which you can use to connect to your cluster privately. 
...
!!! Note
    The **Read-only Workloads** option is available only for high-availability cluster types.
...
Use the **PgBouncer Configuration Settings** menu to set PgBouncer-specific settings. Select the **Read-Write** and **Read-Only** tabs according to the type of connection you want to configure. The **Read-Only** tab is available if you're creating a high-availability cluster and have enabled read-only workloads. 
...
Enable **Superuser Access** to grant superuser privileges to the edb_admin role. This option is available for single-node and high-availability clusters. See [Notes on the edb_admin role](/biganimal/latest/using_cluster/01_postgres_access/#notes-on-the-edb_admin-role).
