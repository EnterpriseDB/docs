...
!!! Important
    There are additional cost implications when you enable PgBouncer. For more details, see [Pricing and billing](../../pricing_and_billing).

Enable **PgBouncer** to have it manage your connections to Postgres databases and help your workloads run more efficiently. This tool is particularly useful if you plan to use more than a few hundred active connections. For more information, visit the [PgBouncer documentation](https://www.pgbouncer.org/community.html).
...
PgBouncer can be deployed at either the server side or client side, this implementation is a server side deployment, to provide customer an out-of-box use experience and advanced features like clustering. For more information on client side deployment, see [Configuring EDB PgBouncer for BigAnimal](../../using_cluster/02_connecting_your_cluster/pgbouncer_configuring). 
