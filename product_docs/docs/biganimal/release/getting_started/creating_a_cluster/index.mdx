...
1.  In the **Connections** section, you can enable Read-only Workloads. This feature directs read-only operations exclusively towards replicas. If enabled, you may have to raise the IP address resource limits for the cluster:
    ...
    What to be aware of when enabling read-only workloads:
      ...
      - Advisory locks aren't replicated between Postgres nodes, so advisory locks taken on a standby replica don't conflict with advisory locks taken on the primary or another standby replica.  We recommend that applications which rely on advisory locking avoid using read-only workloads for those transactions.
