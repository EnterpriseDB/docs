...
1.  In the **Storage** section, select your volume type from the **Volume Type** list. 
    ...
    - For AWS, BigAnimal currently supports General Purpose SSD (GP3). In **Volume Properties**, select the disk size for your cluster and configure the IOPS for better performace.
