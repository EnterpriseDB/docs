...
1. Select the type of cluster to deploy. 
    ...
    -  [Extreme high availability (Preview)](/biganimal/latest/overview/02_high_availability/#extreme-high-availability-beta) creates a cluster, powered by EDB Postgres Distributed, with three nodes and up to two BDR groups spread across three availability zones. This cluster uses EDB Postgres Distributed to deliver higher performance and faster recovery. You can create extreme high-availability clusters with either PostgreSQL or Oracle compatibility. 
...
1. If you selected an extreme high-availability cluster, select **Next: Data Groups** to create your data group and proceed to the [Data Groups tab](#data-groups-tab). 
...
This tab is specific to extreme high-availability clusters. 

### Node Settings

1. In the **Nodes** section, select the number of data nodes for your data group. 

   For more information on node architecture, see [Extreme high availability](/biganimal/latest/overview/02_high_availability/#extreme-high-availability-preview).

1. Follow steps 4 through 7 in the [Cluster Setting tab](#cluster-settings-tab). 

1. Select the settings for your data group as specified in the [Additional Settings tab](#additional-settings-tab).

### DB Configuration

Refer to [DB Configuration tab](#db-configuration-tab) and select **Create Data Group** when finished.

### Data Groups dashboard

On this dashboard, you can choose to modify your data groups, view a **Cluster Architecture Diagram**, and **Add a Data Group**.
