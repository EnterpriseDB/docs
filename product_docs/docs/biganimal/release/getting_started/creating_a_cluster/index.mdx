...
1.  Follow the on-screen guide to grant encrypt and decrypt permissions to your key. Some additional guidance:
    ...
    1.  Append a new policy statement where the `Principal.AWS` field contains the **Principal** identifier you copied to your clipboard and the `Principal.Action` field contains **kms:Encrypt** and **kms:Decrypt** permissions. 
