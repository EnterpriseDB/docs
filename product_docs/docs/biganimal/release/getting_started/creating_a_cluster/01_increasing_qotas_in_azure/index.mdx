---
title: Increasing qotas in Azure
---
...
For information about planning for qota requirements, see [Understanding qotas in Azure](../01_increasing_qotas_in_azure/01_understanding_qotas_in_azure) 

Before you use EDB Cloud to deploy your clusters:

1. [Check the qota requirements for your cluster](#check-qota-requirements).
1. [Remediate qota issues in your Azure subscription](#remediate-qota-issues).

## Check the qota requirements

EDB provides an [azure-preflight](https://github.com/EnterpriseDB/cloud-utilities/blob/main/azure/azure-preflight.sh) shell script to check your subscription automatically. Although EDB recommends using the the azure-preflight script, you can also use the Azure CLI or the Azure portal to manually check the requirements.
...
1. Next, run the **azure-preflight.sh** script. 
 ...
 Replace the following variables with:
   ...
   * **pg_type**: virtual machine instance of type E{N}Sv3 or D{N}Sv2 using {N} cores.
   * **end_point**: indicates either public or private endpoint
   * **--ha**: indicates that instance is run with high availability enabled
...
### Using the Azure protal or the the Azure CLI

#### Checking qota utilization for vCPUs and Public IP Addresses
...
#### Checking Azure Service Providers registerations using Azure CLI
...
### Registering Azure Service Providers using the Azure CLI
...
### Registering the Service Providers using Azure portal
