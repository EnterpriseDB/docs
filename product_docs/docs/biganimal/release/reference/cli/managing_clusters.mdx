...
Although the functionality is the same when using AWS or Google Cloud, there may be additional input flags based on cloud provider type. Use the `-h` or `--help` flags for more information on the CLI commands.  
...
│ pgd    │ Distributed High Availability             │ disabled │
