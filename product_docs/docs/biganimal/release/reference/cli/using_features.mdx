...
To update, delete, and restore faraway replicas, use the `faraway-replica update`, `faraway-replica delete`, and `faraway-replica restore` commands. See [Managing clusters with the CLI](/biganimal/latest/reference/cli/managing_clusters/) for more information.
...
For single-node and high-availability clusters, the syntax of the command is:
...
For extreme-high-availability clusters, the syntax of the command is:
