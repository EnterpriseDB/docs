...
```
!!!note
Avoid adding access key in the **import-access-key** command as a plain text in flag mode. Instead you can do **biganimal import-access-key --name <name> --key $(< file_contains_the_key.txt)** or use interactive mode to avoid leaking the key into your shell command history.
!!!
