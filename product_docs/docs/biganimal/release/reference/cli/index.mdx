...
You can create and manage login credentials using access key that allow users to access BigAnimal resources. It is recommended to use the access key authentication method. 

BigAnimal CLI provides following access key operations:

**Importing Access key** - This command supports two modes: flag mode and interactive mode.

This example shows us how to import access key in flag mode:
...
```
!!!note
Avoid adding access key in the **import-access-key** command as a plain text in flag mode. Instead you can do **biganimal credential import-access-key --name <name> --access-key $(< file_contains_the_key.txt)** or use interactive mode to avoid leaking the key into your shell command history.
!!!

This example show us how to import access key in interactive mode:
...
**Setting access key environment variable** - You can configure an environment variable **BA_ACCESS_KEY**. This setting overrides the default context credential. If you want to use default context credential, then remove this environment variable or set it to "". 

**Switching access keys** - You can import multiple access keys and then set one key as the default context credential for all commands. Use `credential show` to list all available credentials, and use `config set context_credential` to set a default credential.
