...
!!!Tip
You can turn off prompting by first using the `biganimal config set interactive_mode off` command. With prompting disabled, if there are any missing required flags, the CLI exits wiht error.
!!!
...
Check your cluster was created successfully using the `show-clusters` command stated in return message:
...
You can update the postgresql configuration of a biganimal cluster directly from CLI using following command.
...
!!!tip
CLI allows you to specify multiple `--pg-config` flags, or you can specify multiple configuration settings as a key-value arrary string separated by comman in one `--pg-config` flag. Some postgres settings can contain comma itself, in this case, you must specify it with a separate `--pg-config` flag.
!!!

### Restore a cluster
Biganimal continuously backs up your PostgrSQL clusters, you can restore a cluster from its backup with CLI to any point in time as long as backups are still retained in Blob storage. The restored cluster can be a in another region and with different configuration, in the restore command you can specify new properties of restored cluster, or you keep using the source cluster's configuration.
To be added. 
