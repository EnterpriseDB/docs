...
### Restore a cluster
Biganimal continuously backs up your PostgrSQL clusters. With CLI, you can restore a cluster from its backup to any point in time as long as backups are still retained in Blob storage. The restored cluster can be a in another region and with different configuration, you can specify new configurations in restore command.
