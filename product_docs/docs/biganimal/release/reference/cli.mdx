...
To create a cluster that is enabled for IAM authentication, set `--iam-authentication` flag on the create-cluster command or in the configuration file to `Yes` or `true`, respectively.
