...
A machine user is used to drive non-GUI access to BigAnimal by user customized automation script, BigAnimal CLI commands, Terraform manifest, or BigAnimal's first party binaries that runs from your environment.

- Any user with the organization's owner role can create, update and delete a machine user.
- The organization's owner can be a user authenticated via IDP or a machine user.
...
- BigAnimal's authorization system performs permission checks on both machine users and normal users.
- The machine user is associated to one organization only and can't switch to another organization.
- The machine user can't be invited and the only way to authenticate and authorize a machine user is access key.

### Quota

The number of machine users in one organization has a capacity:

- 1 per DSS free-trial organization
- 5 per DSS with credit card organization
- Unlimited for committed-spend organizations that includes:
  - CSP marketplace private offer
  - Direct purchase
...
1. Optionally, select the **Create Access Key** check box. 
    - If you select the check box, it creates the access key for this user.
    - If you don't select the check box, it doesn't create the access key while adding this user. You can create the access key for this user from the user's home page later on.
1. Provide the **Access Key Name**.
1. Select the **Expires in (Day/s)** for the access key.
1. Select **Add User** to save the settings and it provides the generated **Access Key** for the user. 

Copy and save this access key in a secure location. The access key is available only at the time you create it. If you lose your access key, you must delete the access key and create a new one. For more information, see [access key](../reference/access_key/).
