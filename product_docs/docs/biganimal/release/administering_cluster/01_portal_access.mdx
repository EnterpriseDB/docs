...
Each customer has a unique BigAnimal organization. Each organization will have at least one project by default; more projects can be created. An organization's database clusters are deployed and managed in the customer's cloud account within a project. Projects within an organization separate workflows to provide secure and isolated environments to ensure complete separation of customer data between different projects within an organization.

In order to access a BigAnimal organization, each user needs to be added to the organization's identity provider first. For more on how to set up the identity provider, see this page. Your identity provider establishes the identity of users that can log in to BigAnimal. 
...
- **Project editor**  &mdash; This role has edit privileges to the project, and can perform the following actions wihtin the project:
