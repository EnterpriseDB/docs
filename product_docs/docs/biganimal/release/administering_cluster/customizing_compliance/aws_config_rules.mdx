...
BigAnimal doesn’t customize your AWS Config rules to prevent conflicts with external workloads. At this time, there are no reported rule violations in Security Hub caused by BigAnimal. BigAnimal will add rules to avoid any potential issues as soon as they are identified and will document them here.
