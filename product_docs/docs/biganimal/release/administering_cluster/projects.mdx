...
1. Select the **Region** for the key. The interface only displays the regions available to the cloud account you configured.
   !!! Note 
   GCP offers the option to configure global keys. If you require a global key, do not select any region.
   !!! 
1. Fill out the remaining values. Each cloud provider has a different set of options. 
1. Select **Add Key** to finalize the key configuration.
