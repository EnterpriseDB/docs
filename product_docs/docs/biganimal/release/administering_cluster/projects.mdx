...
- The user must be added through your organization's identify provider.
...
- You must create a new Azure subscription or AWS account to create a new project. 
  ...
  !!!note
     A single Azure subscription or AWS account can only be used to register one project at a time. For example, if you have two projects and only need to deploy your clusters in AWS, you need to create a different AWS account for each project.

     If you want to reuse a cloud account (an Azure subscription or AWS account), you have to delete the project that was previously connected with that account first.
  !!!   
- Have an organization owner role.
