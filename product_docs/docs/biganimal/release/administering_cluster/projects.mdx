...
1. On the **Add a key** page, select:
   - **BigAnimal** &mdash; To use BigAnimal's default encryption.
...
Now use this TDE key, while creating a cluster. For more information, see [Creating a cluster](../getting_started/creating_a_cluster/index.mdx/#security).
...
1. Select a key and select **delete** icon for the key you want to delete. 
   
   You're prompted to type **delete <keyname>** in the field.

1. To delete the key, enter the text as instructed, and select **Yes, Delete key**. 

!!!note
A TDE key is deleted only when it isn't used and not associated to any cluster.
!!!