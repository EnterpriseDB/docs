...
Some data from Postgres monitoring of system views, tables, and functions is
...
The metrics may have these labels, represented
...
| `cnp_pg_stat_database_blks_hit`       | COUNTER | Number of times disk blocks were found already in the buffer cache so that a read wasn't necessary (this includes only hits in the Postgres buffer cache, not the operating system's file system cache) |
