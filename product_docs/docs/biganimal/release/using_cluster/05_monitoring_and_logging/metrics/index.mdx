...
views, and functions. The Postgres documentation serves is main reference for these metrics.
...
narrowly scoped individual metrics emitted. It isn't very useful to
...
The number of tables in your database will impact the number of metrics in your cloud logging platform thus
impacting your cloud provider costs for storing these metrics. To ensure stability of the metrics pipeline, metrics may be dropped when the number of tables in your database exceeds 2500.
...
removed or renamed. Where possible, we change the metric name when changing the
meaning or type of existing metrics.
...
See also [cnp_pg_stat_database](#group-cnp_pg_stat_database).
...
See also [cnp_pg_stat_replication](#group-cnp_pg_stat_replication), [cnp_pg_replication_slots](#group-cnp_pg_replication_slots).
...
See also [cnp_pg_stat_replication](#group-cnp_pg_stat_replication), [cnp_pg_replication](#group-cnp_pg_replication).
...
See also [cnp_pg_database](#group-np_pg_database).
...
See also [cnp_pg_statio_user_tables](#group-cnp_pg_statio_user_tables).
...
See also [cnp_pg_replication_slots](#group-cnp_pg_statio_user_tables), [cnp_pg_replication](#group-cnp_pg_replication).
...
See also [cnp_pg_stat_user_tables](#group-cnp_pg_stat_user_tables).
