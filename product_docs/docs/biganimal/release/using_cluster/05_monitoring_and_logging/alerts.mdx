...
For example, in this page we will set up an alert that is triggered when one of your cluster groups goes down. Assume that your cluster group has an ID = *p-gjlmcos8n7* and is on a subscription called *development*.
...
1. In the Condition tab, select **Add condition** to set when the alert rule is triggered. Select **Add condition** and then select **Custom log search** to monitor the Log analytics service.
