...
You can view logs from your PostgreSQL clusters using Azure Workbooks.

1.  Sign in to the [Azure portal](https://portal.azure.com).

2.  In the center search box at the top type in **workbook**. Hit enter.

3.  In this panel you may need to refine search by 'subscription' (if your account has more than one).
Also refine search by the region in which you deployed your cluster. i.e. 'eastus' in the search box type in customer to refine your search.

4.  Click on the corresponding link under the 'Name' column. Names in that column are following the format `${GUID} (xxx-rg-${region}-management-xxx-customer)`. For example `62b6d449-e0e5-480a-9809-cae2ff6510e9 (123-rg-australiaeast-management-abcd-customer)`

5.  On this expanded plane you will see bottom center **Open Workbook** button. Click this button. This will launch and interactive workbook with various metrics for you deployed clusters.
