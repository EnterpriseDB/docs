...
The query below shows if the pods are up as provided by [Kube State Metrics](https://github.com/kubernetes/kube-state-metrics/blob/master/docs/pod-metrics.md):
...
The query below shows Memory usage in megabytes as provided by [Kubernetes metrics](https://kubernetes.io/docs/concepts/cluster-administration/system-metrics/):
...
The query below shows the percentage of available storage used as provided by [Kubernetes metrics](https://kubernetes.io/docs/reference/instrumentation/metrics/):
...
The query below will show the replication lag as provided by the [CNP Replication Lag Metric](/biganimal/latest/using_cluster/05_monitoring_and_logging/metrics/#cnp_pg_replication) filtered by the `Postgres Instance ($instance)` pod:
...
To calculate the connections used as a percentage we need to leverage the [cnp_backends_total](/biganimal/latest/using_cluster/05_monitoring_and_logging/metrics/#cnp_backends) and [cnp_pg_settings_setting](/biganimal/latest/using_cluster/05_monitoring_and_logging/metrics/#cnp_pg_settings) metrics. The query below will calculate the percentage of connections used filtered by the `Postgres Instance ($instance)` pod:
...
The query below will show the database transaction ID age as provided by the [CNP Database xid Metric](/biganimal/latest/using_cluster/05_monitoring_and_logging/metrics/#cnp_pg_database) filtered by the `Postgres Instance ($instance)` pod:
...
The age will be relative to the workloads being run on your database and the following settings, we recommend you refer to the Postgres documentation for details:
