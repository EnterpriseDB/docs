...
The query below will show the replication lag as provided by the [CNP Replication Lag Metric](/biganimal/latest/using_cluster/05_monitoring_and_logging/metrics/#cnp_pg_replication). The example below is filtered by the `Postgres Instance ($instance)` pod:
...
The query below will show the database transaction ID age as provided by the [CNP Database xid Metric](/biganimal/latest/using_cluster/05_monitoring_and_logging/metrics/#cnp_pg_database):
