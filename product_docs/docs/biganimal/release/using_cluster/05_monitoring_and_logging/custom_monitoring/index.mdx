...
If you are using `custom monitoring` you will be able to access metrics in a prometheus format. We have provided some [example metrics](/biganimal/latest/using_cluster/05_monitoring_and_logging/custom_monitoring/example_metrics/) to help get you started.
...
* [Azure](/biganimal/release/using_cluster/02_connecting_your_cluster/01_connecting_from_azure/02_virtual_network_peering)
...
This refers to the self managed version of Grafana, not Grafana Cloud.

[Grafana Prometheus datasource](https://grafana.com/docs/grafana/latest/datasources/prometheus/)

## Logs

If you are using `custom monitoring`, we will store your logs in your cloud providers blob storage solution.
...
You could leverage azure functions to read the azure storage object, then use the API to upload the data to cloud watch.

### Patterns for accessing logs via a third party provider
