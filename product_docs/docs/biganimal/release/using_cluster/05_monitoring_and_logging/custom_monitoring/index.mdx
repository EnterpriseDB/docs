...
If you are using `customer metrics` you will be able to access metrics in a prometheus format. We have provided some [example metrics](/biganimal/latest/using_cluster/05_monitoring_and_logging/metrics/#cnp_pg_replication) to help get you started.
...
A common pattern for metric shipping is to have the vendor supplied agent to scrape the metrics endpoint and send the metrics to the desired platform.

![metrics pattern](images/metrics_pattern.png)

#### Datadog

[Datadog open mentrics integration](https://docs.datadoghq.com/integrations/openmetrics/)
...
N.B This refers to the self managed version, not Grafana Cloud.

[Grafana Prometheus datasource](https://grafana.com/docs/grafana/latest/datasources/prometheus/)

## logs

If you are using `customer metrics` , we will store you logs in your cloud providers blob storage soltuion.
