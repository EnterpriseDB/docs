...
4. You can modify the following settings on the corresponding tab of the Edit Cluster page:
   ...
   | Write-ahead logs (WALs)                              | Cluster Settings    | You can add a dedicated WAL volume to an existing single node, high availability, or faraway cluster. Adding a WAL volume to an existing cluster triggers a restart. Adding a WAL volume also incurs additional costs for a second storage volume. However, depending on the volume size and properties, there may be cost savings associated with WAL separation. If using Azure, refer to Azure [Managed Disks pricing](https://azure.microsoft.com/en-us/pricing/details/managed-disks/) for pricing and optimization information. If you are using AWS, see [Amazon EBS pricing](https://aws.amazon.com/ebs/pricing/).|
