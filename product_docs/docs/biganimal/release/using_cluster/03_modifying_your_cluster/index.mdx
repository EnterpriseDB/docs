...
4. You can modify the following settings on the corresponding tab of the Edit Cluster page:
   ...
   | Write-Ahead Logs (WALs)                              | Cluster Settings    | If you enable Write-Ahead Log (WAL) separation in your cluster, please note that you will not be able to disable it later. However, you can still adjust the size and input/output operations per second (IOPS) of the WAL volume as needed. It's important to keep in mind that creating a dedicated storage volume for WAL separation is optional, but selecting your database storage type and volume properties is not.
