...
!!! Note
    Any changes made to the cluster's instance type, volume type or volume properties are not automatically applied to its replicas's settings. To avoid lag during replication, ensure that your replica is at least as capable as its parent cluster. See, 
