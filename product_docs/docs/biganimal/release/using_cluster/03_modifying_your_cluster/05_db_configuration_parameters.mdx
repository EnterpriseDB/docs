...
In addition to entering specific values for parameters, for some parameters you can specify formulas to automatically calculate a value. You can use equations for parameters of type integer and real using the following operators: `+ - / * > >= < <= == != && || ! ? : ( )`. GUCs used in formulas must also be of type integer or real.
...
Formulas can calculate a value relative to the specifications you selected for the cluster or relative to two non-pseudo GUCs. If you resize your cluster, BigAnimal recalculates formulas using the new values for the pseudo GUCs. 
