...
Use the [CREATE EXTENSION](https://www.postgresql.org/docs/current/sql-createextension.html) command to install extensions. (One exception is the EDB Postgres Tuner (pg_tuner) extension. You enable edb_pg_tuner on the DB Configuration tab of the Create or Edit Cluster page of the BigAnimal portal.)

You need superuser privileges to install most extensions on BigAnimal. The table in [Postgres extensions available by deployment](/pg_extensions/) indicates whether an extension requires superuser privileges or not.
