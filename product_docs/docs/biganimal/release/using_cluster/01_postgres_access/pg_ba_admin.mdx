...
Learn about what operations the pg_ba role can perform and what Postgres built-in roles it has.

## What pg_ba_admin can do
The pg_ba_admin user can perform the following operations, for example:
- Create roles
- Create databases
- Create, modify or delete any non-role user, including setting passwords
- Impersonate any other user using SET ROLE without requiring a password
...
- Create foreign servers for foreign data wrappers (use may be confined by network access control rules)
...
- Create tablespaces, cast, operator class, access method, text search template
...
- Set parameters related to  pg_audit (PostgreSQL only) or edb_audit (EDB Postgres Advanced Server only)”- Create functions in non-trusted languages such as Python and C
- Install unsupported extensions (see [Supported Postgres extensions and tools](biganimal/latest/overview/extensions_tools/))
- Use SQL to set any superuser-only configuration parameters not explicitly listed as allowed (some parameters can be modified using the BigAnimal portal, see [Modifying database configuration parameters](/biganimal/latest/using_cluster/03_modifying_your_cluster/05_db_configuration_parameters/))
- Execute ALTER SYSTEM ... SET to modify the PostgreSQL configuration (use the BigAnimal portal instead, see [Modifying database configuration parameters](/biganimal/latest/using_cluster/03_modifying_your_cluster/05_db_configuration_parameters/)).
- Execute CREATE LANGUAGE with custom handlers (use CREATE EXTENSION for supported languages)
...
- Changed qreplace_function setting
