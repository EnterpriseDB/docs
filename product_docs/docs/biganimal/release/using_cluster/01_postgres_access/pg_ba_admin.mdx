...
Learn about what operations the pg_ba_admin role can perform and what Postgres built-in roles it has.

## What pg_ba_admin can do
A user with the pg_ba_admin role can perform many operations typically reserved for the Postgres superuser. For example:
...
- Change qreplace_function setting
...
A user with the pg_ba_admin role doesn't has, and can't be granted, the following roles that are predefined in Postgres:
