...
### pg_ba_admin
In order for us to effectively manage the cloud resources and ensure users are protected against security threats, BigAnimal provides a special administrative role, pg_ba_admin. The edbadmin user is a member of the pg_admin role. The pg_ba_admin role has privileges similar to superusers. The pg_ba_admin role shouldn't be be used for day-to-day application operations and access to the role controlled carefully. See [pg_ba_admin role](pg_ba_admin_user) for details.

### superuser
When using your own cloud account, you can grant the edb_admin role superuser privileges for a cluster. If you grant superuser privileges, you must take care to limit the number of connections used by superusers to avoid degraded service or compromising availability.
...
### Notes on the edbadmin role

  Refer to the [PostgreSQL documentation on superusers](https://www.postgresql.org/docs/current/role-attributes.html) for best practices. 
