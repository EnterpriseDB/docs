...
- Review the [tagging limitations of AWS](https://docs.aws.amazon.com/emr/latest/EMR-Serverless-UserGuide/tagging-restrictions.html).

- The **Key** cannot have `aws` or `managedby` in the prefix, as they are reserved by AWS and EDB Cloud Service. 

- In AWS, a resource can have a maximum number of 50 tags. The EDB Cloud Service applies some tags by default, so we recommend not adding more than 40 of your own tags. Take into account that the number of tags per resource is defined by the tags created in the EDB Console, in your cloud service provider's dashboard, with terraform, and other synced systems.
