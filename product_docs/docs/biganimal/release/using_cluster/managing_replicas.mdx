...
1. On the **Additional Settings** tab, 
   ...
   Under the **Security** section, enable the **Transparent Data Encryption (TDE)**. This option is available only when your primary cluster is TDE-enabled. It automatically enables TDE and allows you to select the encryption key from the available List.

   !!!Note 
   TDE key for faraway replicas must be same as the primary cluster encryption key. In case you use different key, the cluster provisioning fails.
   !!!
