...
1. Select the **Faraway Replica** icon next to a cluster to create its faraway replica. The Create Replica page appears. Certain fields aren't editable and inherit their values from the parent cluster such as the database type and most of the database configuration parameters.

1. You can't edit the **Cloud Provider** field. The cloud provider is inherited from the parent cluster. 
...
1. On the DB Configuration tab, to avoid replication issues when running a replica (standby server), keep the inherited value or increase it for the following database configuration parameters. Otherwise, queries are not allowed in the replica. 
   - max_connections
...
1. The cluster settings are populated with values inherited from the parent cluster. You can edit the cluster settings while creating your cluster. 
   ...
   !!! NoteNotes
       ...
       - While promoting a replica to a cluster, you can't modify the **Cloud Provider**, **Postgres Type**, **Postgres Version**, and **Region** fields.
...
1. In the **Replica Settings** tab, in the **Faraway Replica Name** field, enter the name for your replica.
...
1. Select the cluster with the replica you want to restore. In the **Overview** tab, you can see the cluster's replicas under **Faraway Replicas**.
...
1.  On the Restore Replica page: 
    ...
    3.  Review your selections and select **Restore Replica** to begin the restore process.  
...
1. Select the **Delete** icon next to the cluster with the replica you want to delete. 

1. Select the **Delete** icon next to the the replica you want to delete. 
