...
1. You can't edit the **Database Type** section. The database information is inherited from the parent cluster.
...
1. On the DB Configuration tab, to avoid replication issues when running a replica (standby server), keep the inherited value or increase it for the following database configuration parameters. Otherwise, queries are not allowed in the replica. None of the other parameters are editable for replicas.
   - max_connections
...
1. Enable Identity and Access Management (IAM) Authentication to turn on the ability to log in to Postgres using your AWS IAM credentials. See 
