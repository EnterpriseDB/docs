...
1. Identify the load balancer that's tagged with the ID of the cluster to which you want to connect. Note the name of that network load balancer.
...
1. Leave all the other fields with their default values and select **Create**. Note the **Service name** of the created AWS PrivateLink, because you need it for creating a VPC endpoint.

1. In the navigation pane, select **Endpoint Services**.

1. Select your endpoint service and, from the **Actions** list, and select **Allow principals**.
 
1. Add the AWS account to connect to the PrivateLink by specifying the ARN for the principal. The ARN must be in this format:
