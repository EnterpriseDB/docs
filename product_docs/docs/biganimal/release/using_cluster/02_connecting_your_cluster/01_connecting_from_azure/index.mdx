...
5. Enter the details for the private endpoint in the **Basics** tab:
   ...
   - Subscription &mdash; Select the subscription where your vm-client resides. In this case, it's `test`.

   - Resource group &mdash; Select a resource group in the same region where your vm-client resides. This exanple uses `rg-client`.
   ...
   - Network Interface Name &mdash; This takes the name of the private endpoint and appends it with `-nic`.

   - Region &mdash; The private endpoint must be in the same region as your VNet. In this case, it's `(Asia Pacific) Japan East`.
...
you created by entering the following details:
   ...
   - Subscription &mdash; Select the subscription in which the target BigAnimal Postgres cluster resides. In this example, it's `development`.
   - Resource type &mdash; Select **Microsoft.Network/privateLinkServices**. This is the type of resource you want to connect to using this private endpoint.
   - Resource &mdash; Select the Private Link service resource whose name starts with the cluster ID. In this case, it's **p-mckwlbakq5-rw-internal-lb**.
...
7. On the **Virtual Network** tab, enter the client VM’s Virtual Network details:  
   ...
   - Virtual Network &mdash; Enter the VM client’s virtual network. In this case, it's `vnet-client`.

   - Subnet &mdash; To deploy the private endpoint, you must select a virtual network subnet to receive the private IP address assignment. In this example, the snet client subnet was already defined and will be assigned the private IP address. However, if a subnet isn't yet defined, you can select the default subnet, and a private IP address will be assigned.
   - Private IP Configuration &mdash; This option defaults to **Dynamically allocate IP address**. This example uses the default.
   - Application security group &mdash; You can leave this blank, or you can create or assign an Application Security Group. In this example, it's blank.
