...
## Connect with a read-only connection string

Clusters that are read-only enabled display separate connection strings for Read-only and Read/write access. 

A Postgres physical standby receives the read-only connection, but doesn't filter to prevent commands that can modify the standby. The standby assumes the same privileges as the connection. 

To connect to your cluster with the read-only connection string, follow these steps:
...
!!! Note 
   ...
   We recommend that users have their applications run `SET default_transaction_read_only = true` for all read-only activity. For a libpq driver, add `options='-c default_transaction_read_only=true'` to the database connection string. Other drivers use similiar connection parameters. 
