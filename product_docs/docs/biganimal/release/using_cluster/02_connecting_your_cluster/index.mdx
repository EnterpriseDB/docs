...
## Connect with a read-only conenction string

Clusters that are read-only enabled dispaly separate connection strings for read-only and read-write access. 
...
1. Paste the read-only Service URI into your database driver.
