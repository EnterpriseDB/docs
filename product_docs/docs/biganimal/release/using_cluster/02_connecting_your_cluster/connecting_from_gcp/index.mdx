...
1. Create a connected endpoint in the client's VPC. The steps for creating a connected endpoint in the client's VPC are the same whether you're using BigAnimal's cloud or your cloud. See [Step 2: Create a Connected Endpoint for the VM client / application](#step-2-create-a-vpc-endpoint-in-the-clients-vpc).
...
- We recommend the PSC-connected endpoint method and it is most commonly used. This method is described in this topic. However, you can also use the [VPC peering](vpc_peering) connection method, if required by your organization.
...
- A BigAnimal Postgres cluster deployed with private connectivity.
- A VM with a client/application installed in your Google Cloud project.
- The VM’s VPC has a subnet in the same region as the BigAnimal cluster.


### Step 1: Create an endpoint service for your cluster (if using your own project)

!!! Note
    Performed in the Google Cloud project connected to your BigAnimal subscription.
...
1. Get the hostname of your Postgres cluster from the Connect tab of the Cluster page on the BigAnimal portal:
`P-mckwlbakq5.private.brcxzr08qr7rbei1.biganimal.io`.

1. Using Cloudshell, the command prompt, or other terminal, get the internal IP address of the host by performing a ping or nslookup or dig +short &lt;host> against the hostname: 
`10.247.200.9`.

1. In the Google Cloud portal, go Network Services > Load balancing.
...
1. Select **+ PUBLISH SERVICE**. 
   ...
   1. For Service Name, give the published service a name (`p-mckwlbakq5`).
   1. For Subnets, choose RESERVE NEW SUBNET from the dropdown.
...
1. In the Reserve subnet for Private Service Connect window, enter the following details, then select **ADD**.
   ...
   !!! Note Recommendations for IP Range:
       - Allocate at least 8 IP addresses to the CIDR, so the subnet mask should not be greater than 29.
...
1. ADD SERVICE and get the name of the service attachment.  You may need to select the newly created Published service to find the name of the service attachment. (Service Attachment: projects/development-001/regions/us-central1/serviceAttachments/p-mckwlbakq5).
...
!!! Note
    Performed in the Google Cloud project where your VM client / application resides.
...
1. Go to **Network Services -> Private Service Connect -> CONNECTED ENDPOINTS -> +CONNECT ENDPOINT** to create an endpoint with the VPC. 
   ...
   1. For the Endpoint name, use the name of your VM Client / App (test-app-1).
...
1. Connect to your BigAnimal cluster from your client application using the endpoint IP address (for example, `psql "postgres://edb_admin@<endpoint IDP>:5432/edb_admin?sslmode=require"`).
