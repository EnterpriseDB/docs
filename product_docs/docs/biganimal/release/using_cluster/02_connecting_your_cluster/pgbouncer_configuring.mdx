...
1. Update the PgBouncer configuration file `/etc/pgbouncer/pgbouncer.ini`:
   ...
   auth_file= 
   ```
For more information, see the [PgBouncer configuration file page](https://www.pgbouncer.org/config.html). 
...
For the next steps, you need PostgreSQL or another database driver. To install PostgreSQL, see [Downloads](https://www.postgresql.org/download/). 
