...
!!! Note
There are three EDB Postgres Distributed CLI commands that don't apply to BigAnimal: `create-proxy`, `delete-proxy` and `alter-proxy-option`.
!!!
...
`pgd switchover` changes the write leader of the group to another node without a failover.
