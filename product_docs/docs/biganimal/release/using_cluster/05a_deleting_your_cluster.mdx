...
!!!note
To restore a TDE enabled cluster, the TDE key must match with source cluster encryption key. In case a different key is used the restore operation fails.
!!!