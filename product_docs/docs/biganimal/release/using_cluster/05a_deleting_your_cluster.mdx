...
If you no longer need a particular cluster, you can delete it from the BigAnimal portal.  Deleted clusters can be restored after the cluster has been deleted for as long as the backup is available. The backup will remain available for the retention period set for each cluster, after which it will be deleted. To stop the management costs on the cluster, contact EDB's BigAnimal Support team; otherwise, costs may continue to accumulate.
