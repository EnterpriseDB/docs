...
With faraway replicas, the primary server writes to the archive which is moved to the object store. The standby (JASON, IS STANDBY SYNONYMOUS WITH FARAWAY REPLICA, HERE? IF SO, I THINK WE SHOULD USE FARAWAY REPLICA THROUGHOUT THIS SECTION INSTEAD OF STANDBY. DO YOU AGREE?) reads from the object store as files arrive. Prior to the archive replicating across to the standby, the log file must write at least 16MB (the WAL segment size) of data to the WAL to cut a new archive file. If the time interval for closing a WAL file is too large, it can introduce a delay until the faraway replica receives the latest record. This amount of time that it takes to fill up the 16MB log file and copy it to archive is the replication lag. The replication lag is the period of time when data can be lost. It can be minutes or hours depending on the amount of activity in your database. You need to measure the replication lag to determine if it is acceptable from a possible data loss perspective. If it isn't acceptable you should consider using an extreme-high-availability cluster. 

To determine the replication lag, you can compare the last log sequence number (LSN) in the 16MB log file on the primary to the LSN in the log file in the standby.  

### NEED TO DETERMINE WHAT, IF ANY, OF THIS CONTENT WE SHOULD RETAIN
DEE DEE'S COMMENT: I THINK THIS WAS JUST POORLY WRITTEN SENTENCE. I USED IT IN THE SECTION, BUT REMOVED THE IMPLICATION THAT YOU CAN SET THE INTERVAL.
If the time interval set for closing a WAL file is too large, it can introduce a delay until the faraway replica receives the latest record. 
____________________
DEE DEE'S COMMENT: THIS DOESN'T SEEM LIKE IT HAS ANYTHING TO DO WITH BACKUP AND RESTORE. IT IS OVERVIEW INFO BUT PROBABLY NOT WOTH KEEPING. 
Log shipping doesn't make any changes to the database tables. It facilitates:

- Low administration overhead
- Predictable timelines
- Minimal network requirements 
________________
DEE DEE'S COMMENT: I DON'T THINK THIS IS WORTH KEEPING EITHER. WE TALK ABOUT RESTORING FARAWAY REPLICAS IN THE MANAGING FARAWAY REPLICAS TOPIC. DOESN'T FEEL LIKE IT NEEDS CALLING OUT HERE THAT YOU CAN TAKE A BACKUP OF ONE. 
