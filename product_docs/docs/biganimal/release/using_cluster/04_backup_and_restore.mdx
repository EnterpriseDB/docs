...
BigAnimal supports the **Volume Snapshots**. Volume snapshots represents a snapshot of a volume on a storage system. Volume snapshots are very fast as compared to the base backups. You can opt for Volume snapshots from [Additonal tab]((../getting_started/creating_a_cluster/#additional-settings-tab)) while creating and editing a cluster.
...
#### Restor a distributed high-availability cluster