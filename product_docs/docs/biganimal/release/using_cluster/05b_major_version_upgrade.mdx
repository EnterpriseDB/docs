...
!!! Warning
Due to two different password authentications (one for the old instance and one for the new instance) entailed when piping the pg_dump command to the psql command, we recommend that you [utilize a Postgres password file (.pgpass) for authentication](https://www.postgresql.org/docs/16/libpq-pgpass.html) before performing the migration using the pg_dump command. If you do not, the command prompt may become unresponsive.
