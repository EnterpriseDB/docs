...
- Changes to system configuration (GUCs) made by `edb_admin` or other Postgres users will not be persisted though a reboot or maintenance. Use the BigAnimal portal to modify system configuration.
