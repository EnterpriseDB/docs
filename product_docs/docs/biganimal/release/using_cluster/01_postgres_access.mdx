...
For information on integrating with IAM using the CLI, see [Using IAM authentication on AWS](/biganimal/latest/reference/cli/using_features/#using-iam-authentication-on-aws) using the CLI.
