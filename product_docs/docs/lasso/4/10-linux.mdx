...
!!! Important
The same user that runs the PostgreSQL server process must run the command. 
...
in PostgreSQL 10 and later. Before 10, you must run as
a superuser. For more information, see 
[Default Roles](https://www.postgresql.org/docs/current/default-roles.html)in the PostgreSQLL documentation.
!!!

Lasso transparently gathers information related to
all supported versions of PGD and Pglogical.
...
Barman report is enabled by default if Lasso runs as the barman user
...
Lasso can also run on systems where PostgreSQL or
Barman aren't installedto gather all relevant information
