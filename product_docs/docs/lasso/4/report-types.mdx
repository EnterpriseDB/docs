...
In general, if you installed Patroni using EDB or PGDG-certified RPM and DEB
packages, all you need to do is execute Lasso.
...
1. Use the one provided to the `--patroni-configuration` option, if given.
2. Use the one provided to the `[patroni] -> configuration` setting in the Lasso
configuration file, if set.
3. Check the paths provided by packages. The configuration file is usually created
as `/etc/patroni/config.yml` or `/etc/patroni/patroni.yml`.
4. Check the list of running processes to identify the Patroni process, if any, and
use the configuration file retrieved from there.
