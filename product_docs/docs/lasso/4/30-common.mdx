...
Lasso uses the libpq environment variables in order to get the
...
command line options that might be required in your environment. Lasso will look
...
A template file for the configuration file looks like:
...
You can use the above template to set up your configuration file. You only need
to uncomment the desired parameters and set their value according to your
environment.

If you have installed Lasso from DEB or RPM packages, that same template configuration
file can be found at the path `/etc/edb-lasso.conf.templ`.

**IMPORTANT:** if you have installed Lasso from DEB or RPM packages, then you
will be required to have a configuration file that contains at least the customer
ID and token set. That is not required if you are running the Lasso disposable
...
This file must be returned to EDB engineers as part of the process,
or automatically sent with the `--upload` option. [^upload]

!!! Important
Some companies have specifically requested that Lasso
operates in an isolated network. In such a case, the `--upload` option is
not permitted (you can easily verify this setting from your company page
in the Portal or through the `--version` run-time option).
If you wish to take advantage of this feature, you must make
sure that the server that is analysed by Lasso can access the
`443` port of any of the front-line websites listed in Appendix A.
!!!

Lasso can also be used on a server which has no
PostgreSQL installation. In that case, you should use the
`--system-only` option, and the _TAR file_ produced will only contain
system-related information.

## Security considerations

When running queries in the database, Lasso will try to use a role that
has enough privileges to gather the required information from the tool which
metrics are being gathered.

Below are listed the tools and the roles that Lasso tries to use for
each of them; Lasso will try to use the first available role in each
tool role list:
...
Most of the BDR gatherings will try using `bdr_monitor` role, but one of them,
which is in charge of gathering conflicts, tries to use the role
`bdr_read_all_conflicts` for that purpose. This is the only exception.

Where "initial connection role" means the role that was provided through
Lasso CLI when running the tool -- usually `postgres` or `enterprisedb`.

In any of the cases it will use a read-only transaction while querying metrics
