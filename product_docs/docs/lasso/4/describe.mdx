...
Collects PGD Proxy configuration file that is found in the server
...
Report output:
  ...
  * file `/tools/pgd-proxy/systemd/<service_name>_cat.data`: Output of 'systemctl cat <service_name>
  * file `/tools/pgd-proxy/systemd/<service_name>_status.data`: Output of 'systemctl status <service_name>
...
- bdr.apply_log     &mdash; bdr.conflict_history     &mdash; bdr.consensus_kv_data
- bdr.internal_node_pre_commit     &mdash; bdr.replication_status     -
bdr.state_journal     &mdash; bdr.stat_activity
...
PostgreSQL workload characterisation using built-in wait events

Report output:

  * file `postgresql/running_waits_sample.out`: Workload characterisation using built-in wait events
...
Provides partitioning key details (all_part_key_columns, EPAS
specific)
...
All partitioned tables view (from all_part_tables, EPAS specific)
...
Provides subpartitioning key details (all_subpart_key_columns, EPAS
specific)
...
(all_tab_subpartitions, EPAS specific)
...
### EPAS specific dblink information (`postgresql_epas_dblink`)

Current EPAS dblink information from edb_dblink)
