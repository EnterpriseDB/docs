...
The EDB Postgres Distribute Command Line Interface (PDG CLI) is a tool to manage your BDR cluster. It allows you to run commands against BDR clusters. 
...
The easiest way to install and configure the PGD CLI is to use EDB's TPAexec utility for cluster deployment and management. For details, see the [TPAexec product page](https://access.2ndquadrant.com/customer_portal/sw/tpa/).
...
You can manually install the PGD CLI on any Linux machine using `.deb` and `.rpm` package managers available from the [BDR repository](https://techsupport.enterprisedb.com/software_subscriptions/add/products/bdr4/). For example:
...
The PGD CLI uses the Golang Postgres driver to connect to BDR nodes. Therefore, the database connectivity between cli node and BDR nodes should be present and working. The exact steps required for this configuration are highly dependent on the way your BDR cluster is configured and from where you are running cli; hence, it is outside the scope of this document.
...
| ------- | -------------- |
| tabular | Default format. Some columns may be truncated depending on your display. |
| json    | No truncation issues. Field order is preserved.|
| yaml    | Experimental and may not be fully supported in future versions. No truncation issues. Output fields presented in alphabetical order. Field order is not preserved. |
