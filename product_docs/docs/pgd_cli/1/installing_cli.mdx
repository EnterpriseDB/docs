...
If you are using TPAexec to deploy EDB Postgres Distributed then by default it installs the PGD CLI package (`edb-pgd-cli`) on each BDR node. It also creates the configuration file (`/etc/edb/pgd-config.yml`) prepopulated with the endpoints of each node in the cluster so no additional configuration is necessary.
