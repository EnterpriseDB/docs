...
-   **External table**: The WHPG table you use to read or write external data through PXF. Create a [readable or writable external table](/warehousepg/6/admin_guide/external/external-tables/) with a `pxf://` location, the same way as any other WHPG external table.
...
When you query a PXF external table with a `WHERE` clause, PXF can push part of that clause down to the external data source instead of transferring every row to WHPG and filtering there. WHPG controls pushdown through the [`gp_external_enable_filter_pushdown`](/warehousepg/6/ref_guide/config_params/guc-list.mdx#gp_external_enable_filter_pushdown) server configuration parameter. See [Compatibility](compatibility.mdx) to check which operators and data types support it. PXF falls back to filtering in WHPG for any part of the clause the connector or profile doesn't push down.
