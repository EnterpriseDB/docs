...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/admin_guide/managing/expand/expand-overview.md
...
-   The last step of the software phase is redistributing table data. Using the expansion control tables in the *gpexpand* schema as a guide, tables are redistributed. For each table:
    ...
    -   The [gpexand](../../../ref_guide/utility_guide/reference/gpexpand.mdx) utility redistributes the table data, across all of the servers, old and new, according to the distribution policy.
...
1.  To [create an expansion input file](expand-initialize.mdx):

    ```
    gpexpand -f <hosts_file>
    ```

2.  To [initialize segments and create the expansion schema](expand-initialize.mdx):
...
3.  [To redistribute table data](expand-redistribute.mdx):
    ...
    You may need to run `gpexpand` several times to complete the expansion in large-scale systems that require multiple redistribution sessions. `gpexpand` can benefit from explicit table redistribution ranking; see [Planning Table Redistribution](expand-planning.mdx).
...
For information about the [gpexpand](../../../ref_guide/utility_guide/reference/gpexpand.mdx) utility and the other utilities that are used for system expansion, see the *WarehousePG Utility Guide*.
