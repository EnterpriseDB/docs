...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/admin_guide/performance/wlmgmt_intro.md
...
For an automated way to determine the required values based on your specific hardware, use the [WarehousePG memory calculator](mem_calc.mdx). Input your system's RAM, swap, and segment count to get the optimized `gp_vmem_protect_limit` and `vm.overcommit_ratio` values.
