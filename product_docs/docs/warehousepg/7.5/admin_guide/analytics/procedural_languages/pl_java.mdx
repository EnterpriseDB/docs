...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/admin_guide/analytics/procedural_languages/pl_java.md
...
3.  Ensure that the environment variables `JAVA_HOME` and `LD_LIBRARY_PATH` are set properly in `$GPHOME/greenplum_path.sh` on all WarehousePG hosts.
    ...
    This example [gpsync](../../../ref_guide/utility_guide/reference/gpsync.mdx) command copies the file to all hosts specified in the file `gphosts_file`.
