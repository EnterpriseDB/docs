...
title: PL/R Language
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/admin_guide/analytics/procedural_languages/pl_r.md
...
> **Note** You can use the [gpssh](../../../ref_guide/utility_guide/reference/gpssh.mdx) utility to run bash shell commands on multiple remote hosts.
...
1.  To configure WarehousePG to use R, add the `R_HOME` environment variable to `$GPHOME/greenplum_path.sh` on all hosts. This example command returns the R home directory.
...
1.  Source `$GPHOME/greenplum_path.sh` and restart WarehousePG. For example, run these commands on the WarehousePG coordinator host.
...
1.  Copy the PL/R package to the WarehousePG coordinator host.

1.  Install the software extension package

1.  Source the file `$GPHOME/greenplum_path.sh`.
...
WarehousePG provides a collection of data science-related R packages that can be used with the WarehousePG PL/R language. For information about the packages, see [Data Science Package for R](/warehousepg/current/install_guide/).
...
1.  From the command line, use the `wget` utility to download the `tar.gz` files for the arm package to the WarehousePG coordinator host:
...
1.  Use the [gpsync](../../../ref_guide/utility_guide/reference/gpsync.mdx) utility and the `hosts_all` file to copy the `tar.gz` files to the same directory on all nodes of the WarehousePG cluster. The `hosts_all` file contains a list of all the WarehousePG segment hosts. You might require root access to do this.
...
1.  Use the `gpssh` utility in interactive mode to log into each WarehousePG segment host (`gpssh -f all_hosts`). Install the packages from the command prompt using the `R CMD INSTALL` command. Note that this may require root access. For example, this R install command installs the packages for the arm package.

    ```
    $R_HOME/bin/R CMD INSTALL Matrix_0.9996875-1.tar.gz   arm_1.5-03.tar.gz
    ```

1.  Ensure that the package is installed in the `$R_HOME/library` directory on all the segments (the `gpssh` can be used to install the package). For example, this `gpssh` command list the contents of the R library directory.
...
<https://cran.r-project.org/web/packages/PivotalR/> - The home page for PivotalR, a package that provides an R interface to operate on WarehousePG tables and views that is similar to the R data.frame. PivotalR also supports using the machine learning package [MADlib](https://madlib.apache.org/) directly from R.
