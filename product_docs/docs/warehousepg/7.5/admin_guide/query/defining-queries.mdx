...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/admin_guide/query/defining-queries.md
...
For predefined aggregate functions, see [Built-in Functions and Operators](functions-operators.mdx#built-in-functions-and-operators). You can also add custom aggregate functions.
...
Where `<window_function>` is one of the functions listed in [Table 4](functions-operators.mdx#window-functions) or a user-defined window function, `<expression>` is any value expression that does not contain a window expression, and `<window_specification>` is:
