...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/admin_guide/query/CTE-query.md
...
By default, WarehousePG enables the `RECURSIVE` keyword for the `WITH` clause. `RECURSIVE` can be deactivated by setting the server configuration parameter [gp_recursive_cte](../../ref_guide/config_params/guc-list.mdx) to `false`.
