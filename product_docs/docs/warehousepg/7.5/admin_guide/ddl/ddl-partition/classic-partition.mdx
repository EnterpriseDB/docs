...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/admin_guide/ddl/ddl-partition/classic-partition.md
...
If you are already familiar with the *classic* partitioning syntax, refer to [About Changes to Table Partitioning in WarehousePG 7](about-part-changes.mdx) for information about syntax and behavior changes introduced in version 7.

<a id="classic"></a>

## Creating Partitioned Tables with the Classic Syntax

This section includes several examples that use the classic syntax for creating a partitioned table.

<a id="topic67c"></a>

### Creating a Date-Range Partitioned Table
...
#### Creating a Numeric-Range Partitioned Table
...
> <a id="topic77c"></a>
>
> ## Partition Maintenance with the Classic Syntax
...
> **Note** You cannot add a partition to a partition design that has a default partition. You must split the default partition to add a partition. See [Splitting a Partition](#splitting-a-partition).
...
### Modifying a Sub-Partition Template
...
For information about exchanging and altering a leaf partition, see the [ALTER TABLE](../../../ref_guide/sql_commands/ALTER_TABLE.mdx) command reference.

For information about limitations of partitioned tables that contain a external table partition, see [Limitations of Partitioned Tables](./#limitations).
