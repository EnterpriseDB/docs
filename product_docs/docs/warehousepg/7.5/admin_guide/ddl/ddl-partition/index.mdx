...
navigation:
  ...
  - classic-partition
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/admin_guide/ddl/ddl-partition/index.md
...
## About the Table Partitioning Methods
...
## About Table Partitioning in WarehousePG
...
## Choosing the Partitioning Syntax
...
You partition a table when you create it with the [CREATE TABLE](../../../ref_guide/sql_commands/CREATE_TABLE.mdx) command. Before you create the table, you should:
...
3.  Determine the appropriate number of partitions and partition levels. For example, you may choose to create a table that is date-range partitioned by month, and then sub-partition the monthly partitions by sales region.
...
> **Important** The examples in the next section use the modern partitioning syntax. Refer to [Examples Using the Classic Partitioning Syntax](classic-partition.mdx) for information about the classic partitioning syntax and parallel examples.

<a id="topic66"></a>

## Creating Partitioned Tables with the Modern Syntax

The following subsections provide modern syntax examples for creating tables with various partition designs.

<a id="topic67"></a>

### Defining a Date-Range Partitioned Table
...
### Defining a Numeric-Range Partitioned Table
...
### Defining a List Partitioned Table
...
### Defining a Hash Partitioned Table
...
### Defining a Multi-level Partitioned Table
...
## Verifying the Partition Strategy

When a table is partitioned based on the query predicate, you can use [EXPLAIN](../../../ref_guide/sql_commands/EXPLAIN.mdx) to verify that the query optimizer scans only the relevant data to examine the query plan.
...
## About Viewing Your Partition Design

Partitioning information is stored in the [pg_partitioned_table](../../../ref_guide/system_catalogs/system_catalogs_definitions/pg_partitioned_table.mdx) system catalog, and in additional fields in the [pg_class](../../../ref_guide/system_catalogs/system_catalogs_definitions/pg_class.mdx) (`relispartition` and `relpartbound`) catalog.
...
<a id="topic76classic"></a>

### Viewing a Classic Syntax Sub-Partition Template

For a partitioned table that you create using the classic syntax, the [gp_partition_template](../../../ref_guide/system_catalogs/system_catalogs_definitions/gp_partition_template.mdx) system catalog table describes the relationship between the partitioned table and the sub-partition template defined at each level in the partition hierarchy.
...
### Constructing a pg_partitions-Equivalent Query

Refer to [Migrating Partition Maintenance Scripts to the New WarehousePG 7 Partitioning Catalogs](../../../install_guide/migrate-classic-partitioning.mdx) for information about mapping WarehousePG 6 partitioning catalogs to the new definitions in WarehousePG 7.
...
You can enable or disable partition pruning for the Postgres-based planner by setting the [enable_partition_pruning](../../../ref_guide/config_params/guc-list.mdx#enable_partition_pruning) server configuration parameter.
...
> **Important** The examples in the next section use the modern partitioning syntax. Refer to [Partition Maintenance with the Classic Syntax](classic-partition.mdx#topic77c) for information about the maintenance activities and commands for partitioned tables that you created using the classic partitioning syntax.

<a id="topic77"></a>

## Partition Maintenance with the Modern Syntax
