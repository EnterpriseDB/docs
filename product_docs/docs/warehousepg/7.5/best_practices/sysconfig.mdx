...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/best_practices/sysconfig.md
...
Configuration of the WarehousePG cluster is usually performed as root.
...
XFS is the file system used for WarehousePG data directories. Use the mount options described in [Configure Operating System](../install_guide/config_os.mdx).

<a id="port_config"></a>

## Port Configuration

See the [recommended OS parameter settings](../install_guide/config_os.mdx#topic3) in the *WarehousePG Installation Guide* for further details.

Set up `ip_local_port_range` so it does not conflict with the WarehousePG port ranges. For example, setting this range in `/etc/sysctl.conf`:

```
net.ipv4.ip_local_port_range = 10000  65535
```

you could set the WarehousePG base port numbers to these values.
...
See the [Recommended OS Parameters Settings](../install_guide/config_os.mdx#topic3) in the *WarehousePG Installation Guide* for further details.
...
<a id="os_mem_config"></a>

## OS Memory Configuration

The Linux sysctl `vm.overcommit_memory` and `vm.overcommit_ratio` variables affect how the operating system manages memory allocation. See the [`/etc/sysctl.conf`](../install_guide/config_os.mdx#sysctl_file) file parameters guidelines in the *WarehousePG Datatabase Installation Guide* for further details.
...
See also [Memory and Resource Management with Resource Queues](workloads.mdx).

<a id="shared_mem_config"></a>

## Shared Memory Settings

WarehousePG uses shared memory to communicate between `postgres` processes that are part of the same `postgres` instance. The following shared memory settings should be set in `sysctl` and are rarely modified. See the [`sysctl.conf`](../install_guide/config_os.mdx) file parameters in the *WarehousePG Installation Guide* for further details.
...
See [Setting the WarehousePG Recommended OS Parameters](../install_guide/config_os.mdx#recommended-os-parameters-settings) for more details.
...
1.  Calculate `gp_vmem`, the host memory available to WarehousePG.
...
3.  Calculate `gp_vmem_protect_limit` by dividing the total WarehousePG memory by the maximum number of acting primaries:
...
Also, see [Resource Management](workloads.mdx) for best practices for managing query memory using resource queues.

<a id="spill_files"></a>

## Resource Queue Spill File Configuration

WarehousePG creates *spill files* (also called *workfiles*) on disk if a query is allocated insufficient memory to run in memory. A single query can create no more than 100,000 spill files, by default, which is sufficient for the majority of queries.
...
A query could generate a large number of spill files if not enough memory is allocated to it or if data skew is present in the queried data. If a query creates more than the specified number of spill files, WarehousePG returns this error:
...
-   The `gp_workfile_entries` view contains one row for each operator using disk space for workfiles on a segment at the current time. See [How to Read Explain Plans](tuning_queries.mdx) for information about operators.
...
See the *WarehousePG Reference Guide* for descriptions of the columns in these views.
