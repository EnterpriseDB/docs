...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/security_guide/authentication.md
...
-   [Allowing Connections to WarehousePG](#allowing-connections-to-warehousepg)
-   [Editing the pg_hba.conf File](#editing-the-pg-hba-conf-file)
-   [Authentication Methods](#authentication-methods)
-   [Limiting Concurrent Connections](#limiting-concurrent-connections)
-   [Encrypting Client/Server Connections](#encrypting-clientserver-connections)

**Parent topic:** [WarehousePG Security Configuration Guide](./)

<a id="topic_ln1_ptd_jr"></a>
...
2.  Add a line to the file for each type of connection you want to allow. Records are read sequentially, so the order of the records is significant. Typically, earlier records will have tight connection match parameters and weaker authentication methods, while later records will have looser match parameters and stronger authentication methods. For example:
    ...
    # To use the more secure SCRAM-SHA-256 method instead,
    # replace md5 with scram-sha-256 in the line below
    host    all   dba   192.168.0.0/32  md5
    ```

<a id="topic_nyh_gwd_jr"></a>
...
scram-sha-256
Perform SCRAM-SHA-256 authentication as described in [RFC 5802](https://tools.ietf.org/html/rfc5802) to verify the user's password. SCRAM-SHA-256 authentication is a challenge-response scheme that prevents password sniffing on untrusted connections. It's more secure than the `md5` method, but older clients might not support it.

md5
Perform SCRAM-SHA-256 or MD5 authentication to verify the user's password. Falls back to a less secure challenge-response mechanism for users with an MD5-hashed password. The fallback mechanism also prevents password sniffing, but provides no protection if an attacker manages to steal the password hash from the server, and you can't use it when `db_user_namespace` is enabled. For all other users, `md5` works the same as `scram-sha-256`.
...
Client principals must have their WarehousePG user name as their first component, for example `gpusername@realm`. Alternatively, you can use a user name mapping to map from the first component of the principal name to the database user name. By default, WarehousePG does not check the realm of the client. If you have cross-realm authentication enabled and need to verify the realm, use the `krb_realm` parameter, or enable `include_realm` and use user name mapping to check the realm.

Make sure that your server keytab file is readable (and preferably only readable) by the `gpadmin` server account. The location of the key file is specified by the [krb_server_keyfile](../ref_guide/config_params/guc-list.mdx) configuration parameter. For security reasons, it is recommended to use a separate keytab just for the WarehousePG server rather than opening up permissions on the system keytab file.
...
<a id="openssl_config"></a>

### OpenSSL Configuration

You can make changes to the OpenSSL configuration by updating the `openssl.cnf` file under your OpenSSL installation directory, or the file referenced by `$OPENSSL_CONF`, if present, and then restarting the WarehousePG server.

<a id="create_a_cert"></a>
...
The default location for the following SSL server files is the WarehousePG coordinator data directory (`$COORDINATOR_DATA_DIRECTORY`):
...
<a id="topic_yxp_5h2_jr"></a>

## PAM-Based Authentication

The "PAM" (Pluggable Authentication Modules) authentication method validates username/password pairs, similar to basic authentication. To use PAM authentication, the user must already exist as a WarehousePG role name.

WarehousePG uses the `pamservice` authentication parameter to identify the service from which to obtain the PAM configuration.
...
WarehousePG does not install a PAM configuration file. If you choose to use PAM authentication with WarehousePG, you must identify the PAM service name for WarehousePG and create the associated PAM service configuration file and configure WarehousePG to use PAM authentication as described below:

1.  Log in to the WarehousePG coordinator host and set up your environment. For example:

    ```
    $ ssh gpadmin@<gpcoord>
    gpadmin@gpcoord$ . /usr/edb/whpg7/greenplum_path.sh
    ```

2.  Identify the `pamservice` name for WarehousePG. In this procedure, we choose the name `greenplum`.
...
5.  Add one or more entries to the `pg_hba.conf` configuration file to enable PAM authentication in WarehousePG. These entries must specify the `pam` *auth-method*. You must also specify the `pamservice=greenplum` *auth-option*. For example:
...
6.  Reload the WarehousePG configuration:

    ```
    $ gpstop -u
    ```

<a id="topic_ed4_d32_jr"></a>
...
1.  Stop your WarehousePG cluster:
...
4.  Restart your WarehousePG cluster:

    ```
    $ gpstart
    ```

<a id="topic_ibc_nl2_jr"></a>
...
> **Note** For information about encrypting data between the `gpfdist` server and WarehousePG segment hosts, see [Encrypting gpfdist Connections](encryption.mdx).

To enable SSL requires that OpenSSL be installed on both the client and the coordinator server systems. WarehousePG can be started with SSL enabled by setting the server configuration parameter `ssl=on` in the coordinator `postgresql.conf`. When starting in SSL mode, the server will look for the files `server.key` (server private key) and `server.crt` (server certificate) in the coordinator data directory. These files must be set up correctly before an SSL-enabled WarehousePG cluster can start.
...
A self-signed certificate can be used for testing, but a certificate signed by a certificate authority (CA) should be used in production, so the client can verify the identity of the server. Either a global or local CA can be used. If all the clients are local to the organization, a local CA is recommended. See [Creating a Self-Signed Certificate](#creating-a-self-signed-certificate) for steps to create a self-signed certificate.
