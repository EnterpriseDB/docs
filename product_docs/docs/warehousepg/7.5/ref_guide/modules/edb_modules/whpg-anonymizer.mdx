...
Refer to [Downloading and installing an extension](/warehousepg/current/ref_guide/modules/edb_modules/#downloading-and-installing-an-extension) for installation and setup instructions.
...
3.  Create the extension in your database and initialize it:
...
2.  **Define the masking policies:** Use `SECURITY LABEL` statements to map specific columns to masking static values or functions.
...
3.  **Assign subject roles:** Identify the database roles that must see masked data and label them as `MASKED`.

    ```sql
    SECURITY LABEL FOR anon ON ROLE <role_name> IS 'MASKED';
    ```

4.  **Remove or update policies:** If you need to change a rule or stop masking a specific column, use the `NULL` label or drop the rule.
...
2.  Create a sample table and populate it with sensitive data:
...
3.  Apply a static string rule, which masks names with a static string:
...
4.  Apply a functional masking rule, which masks phone numbers using a partial MD5 hash:
...
5.  Assign the `MASKED` label to the `analyst` role:
