...
title: System Views
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/ref_guide/system_catalogs/catalog_ref-views.md
...
<a id="gp_backend_memory_contexts"></a>
...
| column                       | type | references | description                                                                                                                                                        |
| ---------------------------- | ---- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `distributed_xid`            | xid  |            | The transaction ID used by the distributed transaction across the WarehousePG cluster.                                                                             |
...
<a id="gp_replication_origin_status"></a>
...
| `memory_quota`    | text | pg_resgroupcapability.value for pg_resgroupcapability.reslimittype = 5 | The memory quota (`MEMORY_QUOTA`) value specified for the resource group.                                                                                                                                                  |
...
| num_queued           | integer  |                     | The total number of queued transactions for the resource group since the WarehousePG cluster was last started, excluding the num_queueing. |
| num_executed         | integer  |                     | The total number of transactions run in the resource group since the WarehousePG cluster was last started, excluding the num_running.      |
| total_queue_duration | interval |                     | The total time any transaction was queued since the WarehousePG cluster was last started.                                                  |
...
<a id="gp_resgroup_status_per_host"></a>
...
<a id="gp_resgroup_status_per_segment"></a>
...
<a id="gp_suboverflowed_backend"></a>
...
<a id="gpexpandexpansion_progress"></a>
...
Status for specific tables involved in the expansion is stored in [gpexpand.status_detail](system_catalogs_definitions/gp_expansion_tables.mdx).
...
| `rsgid`            | oid         | pg_resgroup.oid     | Resource group OID or `0`.<br /><br />See [Note](#rsg_note).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `rsgname`          | text        | pg_resgroup.rsgname | Resource group name or `unknown`.<br /><br />See [Note](#rsg_note).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |

<a id="gp_stat_all_indexes"></a>
...
<a id="gp_stat_database_conflicts"></a>
...
<a id="gp_stat_progress_analyze"></a>
...
| `phase`                     | text    | Current processing phase. Refer to [ANALYZE Progress Reporting](../../admin_guide/managing/progress_reporting.mdx#analyze_progress) for detailed information about the phases.                                                 |
...
<a id="gp_stat_progress_basebackup"></a>
...
| `phase`                | text    | Current processing phase. Refer to [Base Backup Progress Reporting](../../admin_guide/managing/progress_reporting.mdx#basebackup_progress) for detailed information about the phases.                                                                                                                                                                                                                                                                                     |
...
<a id="gp_stat_progress_cluster"></a>
...
| `phase`               | text    | Current processing phase. Refer to [CLUSTER and VACUUM FULL Progress Reporting](../../admin_guide/managing/progress_reporting.mdx#cluster_progress) for detailed information about the phases.                                                                                                                                                                                                                                                                      |
...
| `heap_blks_total`     | bigint  | Total number of heap blocks in the table. This number is reported as of the beginning of `seq scanning heap`. For AO/CO tables, WarehousePG converts byte size into equivalent heap blocks in size.                                                                                                                                                                                                                                                                 |
| `heap_blks_scanned`   | bigint  | Number of heap blocks scanned. This counter only advances when the phase is `seq scanning heap`. For AO/CO tables, WarehousePG converts byte size into equivalent heap blocks in size.                                                                                                                                                                                                                                                                              |
...
<a id="gp_stat_progress_create_index"></a>
...
| `index_relid`        | oid     | The object identifer of the index being created or reindexed. Because WarehousePG does not support concurrent (re)indexing, this value is always `0`.                                                      |
| `command`            | text    | The name of the command that is running: `CREATE INDEX` or `REINDEX`.                                                                                                                                      |
| `phase`              | text    | Current processing phase of index creation. Refer to [CREATE INDEX Progress Reporting](../../admin_guide/managing/progress_reporting.mdx#create_index_progress) for detailed information about the phases. |
...
<a id="gp_stat_progress_dtx_recovery"></a>
...
<a id="gp_stat_progress_vacuum"></a>
...
| `phase`              | text    | Current processing phase of vacuum. Refer to [VACUUM Progress Reporting](../../admin_guide/managing/progress_reporting.mdx#vacuum_progress) for detailed information about the phases.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `index_vacuum_count` | bigint  | *Heap tables*: Number of completed index vacuum cycles. <br /><br />*AO/CO tables*: Collected when WarehousePG recycles a dead segment file, which may happen both, any, or neither, during `append-optimized pre-cleanup` phase and `append-optimized post-cleanup phase`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `max_dead_tuples`    | bigint  | *Heap tables*: Number of dead tuples that we can store before needing to perform an index vacuum cycle, based on [maintenance_work_mem](../config_params/guc-list.mdx#maintenance_work_mem). <br /><br />*AO/CO tables*: Collected at the beginning of the `append-optimized pre-cleanup` phase, this is the total number of tuples before the logical EOF of all segment files. The value should not change while `VACUUM` progresses.                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `wal_write_time`   | double precision         |            | Total amount of time spent writing WAL buffers to disk, in milliseconds (if [track_wal_io_timing](../config_params/guc-list.mdx#track_wal_io_timing) is enabled, otherwise zero). |
...
<a id="gp_stat_xact_all_tables"></a>
...
<a id="gp_stat_xact_sys_tables"></a>
...
<a id="gp_stat_xact_user_functions"></a>
...
<a id="gp_stat_xact_user_tables"></a>
...
<a id="gp_statio_all_sequences"></a>
...
<a id="gp_statio_sys_sequences"></a>
...
<a id="gp_statio_user_sequences"></a>
...
| ------------------------- | -------- | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `gp_segment_id`           | integer  |                                                                      | Unique identifier of a segment (or coordinator)                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `schemaname`              | name     | [pg_namespace](system_catalogs_definitions/pg_namespace.mdx).nspname | The name of the schema containing table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `tablename`               | name     | [pg_class](system_catalogs_definitions/pg_class.mdx).relname         | The name of the table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `attname`                 | name     | [pg_attribute](system_catalogs_definitions/pg_attribute.mdx).attname | The name of the column described by this row.                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
The maximum number of entries in the array fields can be controlled on a column-by-column basis using the `ALTER TABLE SET STATISTICS` command, or globally by setting the [default_statistics_target](../config_params/guc-list.mdx#default_statistics_target) run-time configuration parameter.
...
| `schemaname`             | name            | [pg_namespace](system_catalogs_definitions/pg_namespace.mdx).nspname         | The name of the schema containing table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `tablename`              | name            | [pg_class](system_catalogs_definitions/pg_class.mdx).relname                 | The name of the table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `statistics_schemaname`  | name            | [pg_namespace](system_catalogs_definitions/pg_namespace.mdx).nspname         | The name of the schema containing the extended statistic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `statistics_name`        | name            | [pg_statistic_ext](system_catalogs_definitions/pg_statistic_ext.mdx).stxname | The name of the extended statistic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `statistics_owner`       | oid             | [pg_authid](system_catalogs_definitions/pg_authid.mdx).oid                   | The owner of the extended statistic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `attnames`               | name\[]         | [pg_attribute](system_catalogs_definitions/pg_attribute.mdx).attname         | The names of the columns on which the extended statistics is defined.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
<a id="pg_backend_memory_contexts"></a>
