...
title: CREATE INDEX
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/ref_guide/sql_commands/CREATE_INDEX.md
...
Additional restrictions apply when unique indexes are applied to partitioned tables; see [CREATE TABLE](CREATE_TABLE.mdx).
...
tablespace_name
	The tablespace in which to create the index. If not specified, [default_tablespace](../config_params/guc-list.mdx) is consulted, or [temp_tablespaces](../config_params/guc-list.mdx) for indexes on temporary tables.
...
autosummarize
Defines whether a summarization run is queued for the previous page range whenever an insertion is detected on the next one. See [BRIN Index Maintenance] \([https://www.postgresql.org/docs/12/brin-intro.html#BRIN-OPERATION](https://www.postgresql.org/docs/12/brin-intro.html#BRIN-OPERATION)) in the PostgreSQL documentation for more information. The default is `off`.
...
The system regularly collects statistics on all of a table's columns. Newly-created non-expression indexes can immediately use these statistics to determine an index's usefulness. For new expression indexes, you must run [ANALYZE](ANALYZE.mdx) to generate statistics for these indexes.

For most index methods, the speed of creating an index is dependent on the setting of [maintenance_work_mem](../config_params/guc-list.mdx#maintenance_work_mem). Larger values will reduce the time needed for index creation, so long as you don't make it larger than the amount of memory really available, which would drive the machine into swapping.
...
Use [DROP INDEX](DROP_INDEX.mdx) to remove an index.
...
[ALTER INDEX](ALTER_INDEX.mdx), [DROP INDEX](DROP_INDEX.mdx)
