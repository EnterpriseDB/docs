...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/ref_guide/sql_commands/CREATE_PROCEDURE.md
...
This form of the `AS` clause is used for dynamically loadable C language procedures when the procedure name in the C language source code is not the same as the name of the SQL procedure. The string obj_file is the name of the file containing the dynamically loadable object, and is interpreted as for the [LOAD](LOAD.mdx) command. The string link_symbol is the name of the procedure in the C language source code. If the link symbol is omitted, it is assumed to be the same as the name of the SQL procedure being defined.
...
See [CREATE FUNCTION](CREATE_FUNCTION.mdx) for more details on function creation that also apply to procedures.

Use [CALL](CALL.mdx) to execute a procedure.
...
A `CREATE PROCEDURE` command is defined in the SQL standard. The WarehousePG version is similar but not fully compatible. For details see also [CREATE FUNCTION](CREATE_FUNCTION.mdx).

<a id="section10"></a>

## See Also

[ALTER PROCEDURE](ALTER_PROCEDURE.mdx), [DROP PROCEDURE](DROP_PROCEDURE.mdx), [CREATE FUNCTION](CREATE_FUNCTION.mdx), [CALL](CALL.mdx)
