...
title: COPY
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/ref_guide/sql_commands/COPY.md
...
query
A [SELECT](SELECT.mdx), [VALUES](VALUES.mdx), [INSERT](INSERT.mdx), [UPDATE](UPDATE.mdx), or [DELETE](DELETE.mdx) command whose results are to be copied. Note that parentheses are required around the query.
...
```
> **Note** When using `COPY FROM`, WarehousePG stores any data item that matches this string as a null value, so be sure that you use the same string that you specified in `COPY TO`.
```
...
ENCODING 'encoding_name'
Specifies that the file is encoded in the encoding_name. If this option is omitted, the current client encoding is used. See the [Notes](#section6) below for more details.
...
The limit for the number of initial rejected rows can be changed with the WarehousePG server configuration parameter [gp_initial_bad_row_limit](../config_params/guc-list.mdx#gp_initial_bad_row_limit) server configuration parameter documentation for information about the parameter.
...
For information about resource queues, refer to [Using Resource Queues](../../admin_guide/performance/wlmgmt/workload_mgmt.mdx) in the *WarehousePG Administrator Guide*.
...
Your designated delimiter_character is `|` (pipe character), and your designated escape character is `*` (asterisk). The formatted row in your data file would appear as follows:
