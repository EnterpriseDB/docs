...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/ref_guide/sql_commands/ALTER_DATABASE.md
...
It is also possible to tie a session default to a specific role rather than to a database; see [ALTER ROLE](ALTER_ROLE.mdx). Role-specific settings override database-specific ones if there is a conflict.
