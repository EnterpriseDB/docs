...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/ref_guide/sql_commands/ALTER_FUNCTION.md
...
argname
The name of an argument. Note that [ALTER FUNCTION](ALTER_FUNCTION.mdx) does not actually pay any attention to argument names, since only the argument data types are needed to determine the function's identity.
...
Change whether the function is a security definer or not. The key word `EXTERNAL` is ignored for SQL conformance. See [CREATE FUNCTION](CREATE_FUNCTION.mdx) for more information about this capability.

PARALLEL
Change whether the function is deemed safe for parallelism. See [CREATE FUNCTION](CREATE_FUNCTION.mdx) for details.
...
ROWS result_rows
Change the estimated number of rows returned by a set-returning function. See [CREATE FUNCTION](CREATE_FUNCTION.mdx) for more information.
...
[CREATE FUNCTION](CREATE_FUNCTION.mdx), [DROP FUNCTION](DROP_FUNCTION.mdx), [ALTER PROCEDURE](ALTER_PROCEDURE.mdx), [ALTER ROUTINE](ALTER_ROUTINE.mdx)
