...
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/admin_guide/workload_mgmt_resgroups.html.md
editTarget: originalFilePath

---

You use resource groups to set and enforce CPU, memory, and concurrent transaction limits in WarehousePG. After you define a resource group, you can then assign the group to one or more WarehousePG roles, or to an external component such as PL/Container, in order to control the resources used by those roles or components.
...
**Parent topic:** [Managing Resources](wlmgmt.mdx)
...
Nested cgroups do not affect memory limits for WarehousePG external components such as PL/Container. Memory limits for external components can only be managed if the cgroup that is used to manage WarehousePG resources is not nested, the cgroup is configured as a top-level cgroup.
...
## Using WarehousePG Command Center to Manage Resource Groups

Using WarehousePG Command Center, an administrator can create and manage resource groups, change roles' resource groups, and create workload management rules.

Workload management assignment rules assign transactions to different resource groups based on user-defined criteria. If no assignment rule is matched, WarehousePG assigns the transaction to the role's default resource group.

Refer to the [WarehousePG Command Center documentation](http://docs.vmware.com/en/VMware-Greenplum-Command-Center/index.html) for more information about creating and managing resource groups and workload management rules.
...
The [gp_resgroup_config](../ref_guide/system_catalogs/gp_resgroup_config.mdx) `gp_toolkit` system view displays the current limits for a resource group. To view the limits of all resource groups:
...
To view the resource group-to-role assignments, perform the following query on the [pg_roles](../ref_guide/system_catalogs/pg_roles.mdx) and [pg_resgroup](../ref_guide/system_catalogs/pg_resgroup.mdx) system catalog tables:
...
To view a resource group's running queries, pending queries, and how long the pending queries have been queued, examine the [pg_stat_activity](../ref_guide/system_catalogs/pg_stat_activity.mdx) system catalog table:
