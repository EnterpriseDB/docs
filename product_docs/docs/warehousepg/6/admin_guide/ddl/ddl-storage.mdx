...
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/admin_guide/ddl/ddl-storage.html.md
...
> **Note** \*QuickLZ compression is not available in the open source version of WarehousePG.
...
The `ALTER TABLE`command changes the definition of a table. Use `ALTER TABLE` to change table attributes such as column definitions, distribution policy, storage model, and partition structure (see also [Maintaining Partitioned Tables](ddl-partition.mdx)). For example, to add a not-null constraint to a table column:
...
See [Splitting a Partition](ddl-partition.mdx) to learn how to change the storage model of a partitioned table.
