...
A view is similar to a table, both are relations - that is "something with columns". All such objects are stored in the catalog table [`pg_class`](../../ref_guide/system_catalogs/pg_class.mdx). These are the general differences:
...
-   A view has an `ON SELECT` query rewrite rule called `_RETURN`.
    ...
    The rewrite rule contains the definition of the view and is stored in the `ev_action` column of the [`pg_rewrite`](../../ref_guide/system_catalogs/pg_rewrite.mdx) catalog table.
...
-   [`pg_class`](../../ref_guide/system_catalogs/pg_class.mdx) - object information including tables and views. The `relkind` column describes the type of object.
-   [`pg_depend`](../../ref_guide/system_catalogs/pg_depend.mdx) - object dependency information for database-specific (non-shared) objects.
-   [`pg_rewrite`](../../ref_guide/system_catalogs/pg_rewrite.mdx) - rewrite rules for tables and views.
-   [`pg_attribute`](../../ref_guide/system_catalogs/pg_attribute.mdx) - information about table columns.
-   [`pg_namespace`](../../ref_guide/system_catalogs/pg_namespace.mdx) - information about schemas (namespaces).
