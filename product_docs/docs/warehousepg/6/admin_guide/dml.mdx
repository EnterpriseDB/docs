...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/dml.md
...
-   [About Concurrency Control in WarehousePG](#topic2)
...
> **Note** By default WarehousePG acquires the more restrictive `EXCLUSIVE` lock (rather than `ROW EXCLUSIVE` in PostgreSQL) for `UPDATE`, `DELETE`, and `SELECT...FOR UPDATE` operations on heap tables. When the Global Deadlock Detector is enabled the lock mode for `UPDATE` and `DELETE` operations on heap tables is `ROW EXCLUSIVE`. See [Global Deadlock Detector](#topic_gdd). WarehousePG always holds a table-level lock with `SELECT...FOR UPDATE` statements.
...
To insert large amounts of data, use external tables or the `COPY` command. These load mechanisms are more efficient than `INSERT` for inserting large quantities of rows. See [Loading and Unloading Data](load) for more information about bulk data loading.
...
Using `UPDATE` in WarehousePG has the following restrictions:

-   While GPORCA supports updates to WarehousePG distribution key columns, the Postgres Planner does not.
...
Using `DELETE` in WarehousePG has similar restrictions to using `UPDATE`:
...
The following are the WarehousePG SQL transaction commands:
...
> **Note** WarehousePG uses the interval specified in the [deadlock_timeout](../ref_guide/config_params/guc-list.mdx) server configuration parameter for local deadlock detection. Because the local and global deadlock detection algorithms differ, the cancelled process(es) may differ depending upon which detector (local or global) WarehousePG triggers first.

> **Note** If the [lock_timeout](../ref_guide/config_params/guc-list.mdx) server configuration parameter is turned on and set to a value smaller than `deadlock_timeout` and `gp_global_deadlock_detector_period`, WarehousePG will cancel a statement before it would ever trigger a deadlock check in that session.
...
The `VACUUM` command collects table-level statistics such as the number of rows and pages. Vacuum all tables after loading data, including append-optimized tables. For information about recommended routine vacuum operations, see [Routine Vacuum and Analyze](managing/maintain.mdx).
...
When WarehousePG runs out of locks, the error message that you may observe references a shared memory error:
...
As the hint describes, consider increasing the [`max_locks_per_transaction`](../ref_guide/config_params/guc-list.mdx) server configuration parameter when you encounter this error.
