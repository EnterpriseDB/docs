...
2.  The following parameters are related to memory utilization:
    ...
    -   `gp_resqueue_memory_policy` - Enables WarehousePG memory management features.
        ...
        In WarehousePG4.2 and later, the distribution algorithm `eager_free` takes advantage of the fact that not all operators run at the same time. The query plan is divided into stages and WarehousePG eagerly frees memory allocated to a previous stage at the end of that stage's execution, then allocates the eagerly freed memory to the new stage.
