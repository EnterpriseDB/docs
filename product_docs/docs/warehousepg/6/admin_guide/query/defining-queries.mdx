...
-   WarehousePG does not support the following keywords: `ALL`, `DISTINCT`, and `OVER`. See [Table 5](functions-operators.mdx#in2073121) for more details.
...
Where *`window_function`* is one of the functions listed in [Table 4](functions-operators.mdx#in164369) or a user-defined window function, *`expression`* is any value expression that does not contain a window expression, and *`window_specification`* is:
