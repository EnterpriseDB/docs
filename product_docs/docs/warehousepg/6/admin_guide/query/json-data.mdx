...
> **Note** The [hstore module](/supported-open-source/warehousepg/extensions/bundled/hstore.mdx) contains functions that cast from `hstore` to `json`, so that `hstore` values converted via the JSON creation functions will be represented as JSON objects, not as primitive string values.
