...
title: Using WarehousePG MapReduce
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/query/gp-mapreduce.md
...
You configure a WarehousePG MapReduce job via a YAML-formatted configuration file, then pass the file to the WarehousePG MapReduce program, `gpmapreduce`, for execution by the WarehousePG parallel engine. The WarehousePG cluster distributes the input data, runs the program across a set of machines, handles machine failures, and manages the required inter-machine communication.

Refer to [gpmapreduce](../../ref_guide/utility_guide/reference/gpmapreduce.mdx) for details about running the WarehousePG MapReduce program.

**Parent topic:** [Querying Data](./)

<a id="topic2"></a>

## About the WarehousePG MapReduce Configuration File

This section explains some basics of the WarehousePG MapReduce configuration file format to help you get started creating your own WarehousePG MapReduce configuration files. WarehousePG uses the [YAML 1.1](http://yaml.org/spec/1.1/) document format and then implements its own schema for defining the various steps of a MapReduce job.

All WarehousePG MapReduce configuration files must first declare the version of the YAML specification they are using. After that, three dashes (`---`) denote the start of a document, and three dots (`...`) indicate the end of a document without starting a new one. (A document in this context is equivalent to a MapReduce job.) Comment lines are prefixed with a pound symbol (`#`). You can declare multiple WarehousePG MapReduce documents/jobs in the same file:
...
Within a WarehousePG MapReduce document, there are three basic types of data structures or *nodes*: *scalars*, *sequences* and *mappings*.
...
The WarehousePG MapReduce program processes the nodes of a document in order and uses indentation (spaces) to determine the document hierarchy and the relationships of the nodes to one another. The use of white space is significant. White space should not be used simply for formatting purposes, and tabs should not be used at all.

Refer to [gpmapreduce.yaml](../../ref_guide/utility_guide/reference/gpmapreduce-yaml.mdx) for detailed information about the WarehousePG MapReduce configuration file format and the keys and values supported.

<a id="topic59"></a>

## Example WarehousePG MapReduce Job

In this example, you create a MapReduce job that processes text documents and reports on the number of occurrences of certain keywords in each document. The documents and keywords are stored in separate WarehousePG tables that you create as part of the exercise.

This example MapReduce job utilizes the untrusted `plpythonu` language; as such, you must run the job as a user with WarehousePG administrative privileges.

1.  Log in to the WarehousePG coordinator host as the `gpadmin` administrative user and set up your environment. For example:
...
7.  Construct the MapReduce YAML configuration file. For example, open a file named `mymrjob.yaml` in the editor of your choice and copy/paste the following large text block:
    ...
    # Connect to WarehousePG using this database and role
    ...
    DEFINE:
     ...
     # 'documents' and 'keywords' tables.
      ...
      # within a WarehousePG MapReduce pipeline. It is like EXECUTION, but it is
      ...
      # One advantage of WarehousePG MapReduce is that MapReduce tasks can be
      ...
      # In WarehousePG MapReduce, a REDUCE function is comprised of one or more functions.
