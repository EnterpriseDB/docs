...
title: About WarehousePG Query Processing
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/query/parallel-proc.md
...
The coordinator receives, parses, and optimizes the query. The resulting query plan is either parallel or targeted. The coordinator dispatches parallel query plans to all segments, as shown in [Figure 1](#figure-1-dispatching-the-parallel-query-plan). The coordinator dispatches targeted query plans to a single segment, as shown in [Figure 2](#figure-2-dispatching-a-targeted-query-plan). Each segment is responsible for running local database operations on its own set of data.
...
![Dispatching the Parallel Query Plan](../../images/parallel_dispatch.png)
...
![Dispatching a Targeted Query Plan](../../images/targeted_dispatch.png)

<a id="topic3"></a>

## Understanding WarehousePG Query Plans
...
[Figure 3](#figure-3-query-slice-plan) shows the query plan. Each segment receives a copy of the query plan and works on it in parallel.
...
![Query Slice Plan](../../images/slice_plan.png)
...
This diagram shows the query worker processes on the coordinator and two segment instances for the query plan illustrated in [Figure 3](#figure-3-query-slice-plan).

![Query Worker Processes](../../images/gangs.png)
