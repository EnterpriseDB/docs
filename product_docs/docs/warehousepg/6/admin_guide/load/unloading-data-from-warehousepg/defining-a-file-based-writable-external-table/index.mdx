...
Use the `CREATE WRITABLE EXTERNAL TABLE` command to define the external table and specify the location and format of the output files. See [Using the WarehousePG Parallel File Server (gpfdist)](../../../external/using-gpfdist.mdx) for instructions on setting up gpfdist for use with an external table and [PXF for WarehousePG](/pxf/6/) for instructions on setting up PXF for use with an external table
