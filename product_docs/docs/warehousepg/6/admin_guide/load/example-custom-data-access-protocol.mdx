...
The following is the API for the WarehousePG custom data access protocol. The example protocol implementation [gpextprotocal.c](gpextprotocalc.mdx) is written in C and shows how the API can be used. For information about accessing a custom data access protocol, see [Using a Custom Protocol](using-a-custom-protocol.mdx).
...
The protocol corresponds to the example described in [Using a Custom Protocol](using-a-custom-protocol.mdx). The source code file name and shared object are `gpextprotocol.c` and `gpextprotocol.so`.
...
The example implementation [gpextprotocal.c](gpextprotocalc.mdx) uses `fopen()` and `fread()` to simulate a simple protocol that reads local files. In practice, however, the protocol would implement functionality such as a remote connection to some process over the network.
