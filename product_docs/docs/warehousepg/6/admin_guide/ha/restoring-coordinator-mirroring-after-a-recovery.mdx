...
4.  After the initialization completes, check the status of standby coordinator, mdw. Run [gpstate](../../utility_guide/ref/gpstate.mdx) with the `-f` option to check the standby coordinator status:
...
3.  After the utility completes, run `gpstate` with the `-b` option to display a summary of the system status:
    ...
    The coordinator instance status should be `Active`. When a standby coordinator is not configured, the command displays `No master standby configured` for the standby coordinator state.
