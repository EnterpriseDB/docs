...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/analytics/procedural_languages/pl_java.md
...
> **Note** When configuring host systems, you can use the [gpssh](../../../ref_guide/utility_guide/reference/gpssh.mdx) utility to run bash shell commands on multiple remote hosts.
...
3.  Ensure that the environment variables `JAVA_HOME` and `LD_LIBRARY_PATH` are set properly in `$GPHOME/greenplum_path.sh` on all WarehousePG hosts.
    ...
    This example [gpscp](../../../ref_guide/utility_guide/reference/gpscp.mdx) command copies the file to all hosts specified in the file `gphosts_file`.
...
2.  Copy your Java archives (JAR files) to the same directory on all WarehousePG hosts. This example uses the WarehousePG `gpscp` utility to copy the file `myclasses.jar` to the directory `$GPHOME/lib/postgresql/java/`:
    ...
    =:/usr/edb/whpg6/lib/postgresql/java/
    ```
...
If no databases have PL/Java as a registered language, remove the Java JAR files and uninstall the WarehousePG PL/Java extension.
...
3.  Uninstall the PL/Java extension. 
