...
title: PL/R Language
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/analytics/procedural_languages/pl_r.md
...
If no databases have PL/R as a registered language, uninstall the WarehousePG PL/R extension.
...
3.  Use the [gpscp](../../../ref_guide/utility_guide/reference/gpscp.mdx) utility and the `hosts_all` file to copy the `tar.gz` files to the same directory on all nodes of the WarehousePG cluster. The `hosts_all` file contains a list of all the WarehousePG segment hosts. You might require root access to do this.
