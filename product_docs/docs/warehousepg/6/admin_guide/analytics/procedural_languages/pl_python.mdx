...
To develop with Python modules, you can use WarehousePG utilities such as `gpssh` and `gpscp` to run commands or copy files to all hosts in the WarehousePG cluster. These sections describe how to use those utilities to install and use additional Python modules:
