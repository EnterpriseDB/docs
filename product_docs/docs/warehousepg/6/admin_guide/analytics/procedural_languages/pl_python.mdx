...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/analytics/procedural_languages/pl_python.md
...
PL/Python is a loadable procedural language. With the WarehousePG PL/Python extensions, you can write WarehousePG user-defined functions in Python that take advantage of Python features and modules to quickly build robust database applications.

You can run PL/Python code blocks as anonymous code blocks. See the [DO](../../../ref_guide/sql_commands/DO.mdx) command in the *WarehousePG Reference Guide*.

The WarehousePG PL/Python extensions are installed by default with WarehousePG. Two extensions are provided:

-   `plpythonu` supports developing functions using Python 2.7. WarehousePG installs a version of Python 2.7 for `plpythonu` at `$GPHOME/ext/python`.
-   `plpython3u`, introduced with WarehousePG 6.22, supports developing functions using Python 3.9. WarehousePG installs a compatible Python at `$GPHOME/ext/python3.9`.
...
WarehousePG installs compatible versions of Python 2.7 and 3.9 in `$GPHOME/ext`.
...
WarehousePG provides a collection of data science-related Python modules that you can use to easily develop PL/Python functions in WarehousePG. See [Python Data Science Module Packages](../../../install_guide/data_sci_pkgs/install_python_dsmod.mdx) for installation instructions and descriptions of the provided modules.

To develop with modules that are not part of th Python Data Science Module packages, you can use WarehousePG utilities such as `gpssh` and `gpscp` to run commands or copy files to all hosts in the WarehousePG cluster. These sections describe how to use those utilities to install and use additional Python modules:
...
As part of the WarehousePG installation, the `gpadmin` user environment is configured to use Python that is installed with WarehousePG. To check the Python environment, you can use the `which` command:

```
which python
```

The command returns the location of the Python installation. All WarehousePG installations include Python 2.7 installed as `$GPHOME/ext/python` and Python 3.9 installed as `$GPHOME/ext/python3.9`:

```
which python3.9
```

When running shell commands on remote hosts with `gpssh`, specify the `-s` option to source the `greenplum_path.sh` file before running commands on the remote hosts. For example, this command should display the Python installed with WarehousePG on each host specified in the `gpdb_hosts` file.
...
You can optionally run `gpssh` in interactive mode to display Python modules on remote hosts. This example starts `gpssh` in interactive mode and lists the Python modules on the WarehousePG host `sdw1`.
...
You can use `gpssh` to run the commands on the WarehousePG hosts. This example runs `gpssh` in interactive mode to install `pip` on the hosts listed in the file `gpdb_hosts`.
...
You can use `gpssh` to run the command on the WarehousePG hosts.
...
WarehousePG uses the value of `plpython3.python_path` to set `PLPYTHONPATH` in the environment used to create or call `plpython3u` functions.
...
You can use the WarehousePG utilities `gpssh` and `gpscp` to run commands on WarehousePG hosts and to copy files to the hosts.
...
Create a table that contains data on each WarehousePG segment instance. Depending on the size of your WarehousePG installation, you might need to generate more data to ensure data is distributed to all segment instances.
...
The `SELECT` command returns `SUCCESS` if the UDF imported the Python module on the WarehousePG segment instance. If the `SELECT` command returns `FAILURE`, you can find the segment host of the segment instance host. The WarehousePG cluster table `gp_segment_configuration` contains information about mirroring and segment configuration. This command returns the host name for a segment ID.
...
-   A problem accessing required libraries. For the NumPy example, a WarehousePG might have a problem accessing the OpenBLAS libraries or the Python libraries on a segment host.
...
-   If the Python `import` command does not return an error, environment variables might not be configured in the WarehousePG environment. For example, the WarehousePG might not have been restarted after installing the Python Package on the host system.
