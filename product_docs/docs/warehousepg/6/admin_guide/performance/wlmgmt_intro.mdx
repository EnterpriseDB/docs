...
title: WarehousePG Memory Overview
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/performance/wlmgmt_intro.md
...
![WarehousePG Segment Host Memory](../../images/segment_memory.png "WarehousePG Segment Host Memory")
...
The physical RAM and OS configuration are usually managed by the platform team and system administrators. See the [WarehousePG Installation Guide](../../install_guide/config_os.mdx#topic3) for the recommended kernel parameters and for how to set the `/etc/sysctl.conf` file parameters.
...
To calculate a safe value for `vm.overcommit_ratio` when resource queue-based resource management is active, first determine the total memory available to WarehousePG processes, `gp_vmem_rq`.
...
> **Note** The gp_vmem_protect_limit setting is enforced only when resource queue-based resource management is active in WarehousePG. WarehousePG ignores this configuration parameter when resource group-based resource management is active.
...
When resource group-based resource management is active in WarehousePG, the usable memory available on a host is a function of the amount of RAM and swap space configured for the system, as well as the `vm.overcommit_ratio` system parameter setting:
...
Assuming the default `gp_resource_group_memory_limit` value (.7), the memory allocated to a WarehousePG host with the example configuration is:
...
The memory available to a WarehousePG segment on a segment host is a function of the memory reserved for WarehousePG on the host and the number of active primary segments on the host. On cluster startup:
...
The `vm.overcommit_ratio` calculation for the example system when resource queue-based resource management is active in WarehousePG follows:
...
The `gp_vmem_protect_limit` calculation when resource queue-based resource management is active in WarehousePG:
