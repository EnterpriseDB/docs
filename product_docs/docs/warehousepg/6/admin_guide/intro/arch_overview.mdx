...
title: About the WarehousePG Architecture
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/intro/arch_overview.md
...
WarehousePG is based on PostgreSQL open-source technology. It is essentially several PostgreSQL disk-oriented database instances acting together as one cohesive database management system (DBMS). It is based on PostgreSQL 9.4, and in most cases is very similar to PostgreSQL with regard to SQL support, features, configuration options, and end-user functionality. Database users interact with WarehousePG as they would with a regular PostgreSQL DBMS.
...
-   [About the WarehousePG Coordinator](#arch_master)
...
## About the WarehousePG Coordinator
...
<a id="arch_segments"></a>

## About the WarehousePG Segments
...
When a user connects to the database via the WarehousePG coordinator and issues a query, processes are created in each segment database to handle the work of that query. For more information about query processes, see [About WarehousePG Query Processing](../query/parallel-proc.mdx).
...
When you deploy your WarehousePG database system, you have the option to configure *mirror* segments. Mirror segments allow database queries to fail over to a backup segment if the primary segment becomes unavailable.
...
This diagram shows how table data is distributed across the segments when the default group mirroring option is configured.

![Data Mirroring in WarehousePG](../../images/group_mirroring.png "Data Mirroring in WarehousePG")
...
<a id="arch_interconnect"></a>

## About the WarehousePG Interconnect
