...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/manage_access/client_auth/kerberos.md
...
2.  Create a principal for the WarehousePG service.
    ...
    The `postgres` part of the principal names matches the value of the WarehousePG `krb_srvname` server configuration parameter, which is `postgres` by default.

    The host name part of the principal name must match the output of the `hostname` command on the WarehousePG coordinator host. If the `hostname` command shows the fully qualified domain name (FQDN), use it in the principal name, for example `postgres/mdw.example.com@GPDB.KRB`.
...
4.  Modify the WarehousePG file `pg_hba.conf` to enable Kerberos support. For example, adding the following line to `pg_hba.conf` adds GSSAPI and Kerberos authentication support for connection requests from all users and hosts on the same network to all WarehousePG databases.
    ...
    Setting the `krb_realm` option to a realm name ensures that only users from that realm can successfully authenticate with Kerberos. Setting the `include_realm` option to `0` excludes the realm name from the authenticated user name. For information about the `pg_hba.conf` file, see [The pg_hba.conf file](https://www.postgresql.org/docs/9.4/auth-pg-hba-conf.html) in the PostgreSQL documentation.
...
8.  As a test, log in to the postgres database with the `gpadmin/admin` role:
    ...
    > **Note** When you start `psql` on the coordinator host, you must include the `-h <master-hostname>` option to force a TCP connection because Kerberos authentication does not work with local connections.
...
-   If the `map=<map-name>` option is specified, the Kerberos principal name is compared to entries labeled with the specified `<map-name>` in the `$MASTER_DATA_DIRECTORY/pg_ident.conf` file and replaced with the WarehousePG role name specified in the first matching entry.
