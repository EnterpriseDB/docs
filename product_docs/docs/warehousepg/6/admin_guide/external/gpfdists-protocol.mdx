...
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/admin_guide/external/g-gpfdists-protocol.html.md
...
To use it, you run the [gpfdist](../../utility_guide/ref/gpfdist.mdx) utility with the `--ssl` option. When specified in a URI, the `gpfdists://` protocol enables encrypted communication and secure identification of the file server and the WarehousePG to protect against attacks such as eavesdropping and man-in-the-middle attacks.
...
For an example of loading data into an external table security, see [Example 3—Multiple gpfdists instances](example-3-multiple-gpfdists-instances.mdx).
