...
title: Using the WarehousePG Parallel File Server (gpfdist)
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/external/using-gpfdist.md
...
-   Allow network traffic to use all ETL host network interfaces simultaneously. Run one instance of gpfdist for each interface on the ETL host, then declare the host name of each NIC in the `LOCATION` clause of your external table definition (see [Examples for Creating External Tables](external-tables/creating-external-tables---examples)).
...
For multiple gpfdist instances on the same ETL host (see [About gpfdist setup and performance](#about-gpfdist-setup-and-performance)), use a different base directory and port for each instance. For example:
