...
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/admin_guide/external/pxf-overview.html.md
...
> **Note** In previous versions of WarehousePG, you may have used the `gphdfs` external table protocol to access data stored in Hadoop. WarehousePG version 6.0.0 removes the `gphdfs` protocol. Use PXF and the `pxf` external table protocol to access Hadoop in WarehousePG version 6.x.
...
For detailed information about the architecture of and using PXF, refer to the [WarehousePG Platform Extension Framework (PXF)](https://docs.vmware.com/en/VMware-Greenplum-Platform-Extension-Framework/6.6/greenplum-platform-extension-framework/overview_pxf.html) documentation.
