...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/external/external-tables/gpfdists-protocol.md
...
To use it, you run the [gpfdist](../../../ref_guide/utility_guide/reference/gpfdist.mdx) utility with the `--ssl` option. When specified in a URI, the `gpfdists://` protocol enables encrypted communication and secure identification of the file server and the WarehousePG to protect against attacks such as eavesdropping and man-in-the-middle attacks.
...
For an example of loading data into an external table security, see [Example 3—Multiple gpfdists instances](creating-external-tables---examples/example-3-multiple-gpfdists-instances.mdx).
