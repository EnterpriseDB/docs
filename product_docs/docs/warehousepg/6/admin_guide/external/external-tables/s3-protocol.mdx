...
title: s3:// Protocol
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/external/external-tables/s3-protocol.md
...
> **Note** The `pxf` protocol can access data in S3 and other object store systems such as Azure, Google Cloud Storage, and Minio. The `pxf` protocol can also access data in external Hadoop systems (HDFS, Hive, HBase), and SQL databases. See [pxf:// Protocol](pxf-protocol.mdx).
...
2.  Declare the `s3` protocol and specify the read and write functions you created in the previous step:
    ...
    To allow only WarehousePG superusers to use the protocol, create it as follows:
...
5.  Create an s3 external table by specifying an `s3` protocol URL in the `CREATE EXTERNAL TABLE` command, `LOCATION` clause.
    ...
    For writable s3 tables, the protocol URL defines the S3 location in which WarehousePG writes the data files that back the table for `INSERT` operations. You can also specify a prefix that WarehousePG will add to the files that it creates. For example:
...
> **Note** Although the S3_prefix is an optional part of the syntax, you should always include an S3 prefix for both writable and read-only s3 tables to separate datasets as part of the [CREATE EXTERNAL TABLE](../../../ref_guide/sql_commands/CREATE_EXTERNAL_TABLE.mdx) syntax.
...
When you initiate an `INSERT` operation on a writable s3 table, each WarehousePG segment uploads a single file to the configured S3 bucket using the filename format `<prefix><segment_id><random>.<extension>[.gz]` where:
...
The environment variables must be set must and must be accessible to WarehousePG on all WarehousePG hosts.
...
-   You also have the option to use an `http/https` server to serve up the configuration file. In this scenario, you specify an `http/https` server URL in the `config_server` parameter. You are responsible for configuring and starting the server, and each WarehousePG segment host must be able to access the server. The following example specifies an IP address and port for an `https` server:

    ```
    LOCATION ('s3://s3-us-west-2.amazonaws.com/test/my_data config_server=https://203.0.113.0:8553')
    ```

<a id="section_tsq_n3t_3x">
