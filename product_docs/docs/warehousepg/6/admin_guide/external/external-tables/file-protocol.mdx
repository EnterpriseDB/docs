...
title: file:// Protocol
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/external/external-tables/file-protocol.md
editTarget: originalFilePath

---

You can use the `file://` protocol with a WarehousePG external table to read from one or more files located on each WarehousePG segment host. The `file://` protocol does not support writing to files.

When reading, the `file://` protocol uncompresses `gzip` (`.gz`), `bzip2` (.`bz2`), and `zstd` (`.zst`) files automatically.

You must provide a URI that specifies the location of an operating system file(s). The URI includes the host name, port, and path to the file. Each file must reside on a segment host in a location accessible by the WarehousePG superuser (`gpadmin`). The host name used in the URI must match a segment host name registered in the `gp_segment_configuration` system catalog table.
