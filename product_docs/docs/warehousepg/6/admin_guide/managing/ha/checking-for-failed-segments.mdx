...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/managing/ha/checking-for-failed-segments.md
...
On the coordinator host, run the [gpstate](../../../ref_guide/utility_guide/reference/gpstate.mdx) utility with the `-e` option to show segment instances with error conditions:
...
To get detailed information about failed segments, you can check the [gp_segment_configuration](../../../ref_guide/system_catalogs/system_catalogs_definitions/gp_segment_configuration.mdx) catalog table. For example:
...
Use the [gplogfilter](../../../ref_guide/utility_guide/reference/gplogfilter.mdx) utility to check the WarehousePG log files for additional information. To check the segment log files, run `gplogfilter` on the segment hosts using [gpssh](../../../ref_guide/utility_guide/reference/gpssh.mdx).
...
2.  Use `gpssh` to check for `WARNING`, `ERROR`, `FATAL`, or `PANIC` log level messages on each segment instance. For example:
    ...
    /usr/edb/whpg6/greenplum_path.sh ; gplogfilter -t 
