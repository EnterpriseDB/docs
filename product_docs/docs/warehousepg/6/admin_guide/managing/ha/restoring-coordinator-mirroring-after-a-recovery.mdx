...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/managing/ha/restoring-coordinator-mirroring-after-a-recovery.md
...
You must initialize a new standby coordinator to continue providing coordinator mirroring unless you have already done so while activating the prior standby coordinator. Run [gpinitstandby](../../../ref_guide/utility_guide/reference/gpinitstandby.mdx) on the active coordinator host to configure a new standby coordinator. See [Enabling Coordinator Mirroring](enabling-mirroring-in-warehousepg/enabling-coordinator-mirroring.mdx).
...
**Parent topic:** [Recovering a failed coordinator](recovering-a-failed-coordinator.mdx)
...
4.  After the initialization completes, check the status of standby coordinator, mdw. Run [gpstate](../../../ref_guide/utility_guide/reference/gpstate.mdx) with the `-f` option to check the standby coordinator status:
