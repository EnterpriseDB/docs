...
New hosts must exchange SSH keys with the existing hosts to enable WarehousePG administrative utilities to connect to all segments without a password prompt. Perform the key exchange process twice with the [gpssh-exkeys](../../../utility_guide/ref/gpssh-exkeys.mdx) utility.
...
1.  Use [gpssh](../../../utility_guide/ref/gpssh.mdx) to create the `gpadmin` user on all the new segment hosts (if it does not exist already). Use the list of new hosts you created for the key exchange. For example:
...
Use the [gpcheckperf](../../../utility_guide/ref/gpcheckperf.mdx) utility to test disk I/O and memory bandwidth.
