...
For the coordinator, standby coordinator, and segment instance, the first three fields, db_id, cont_id, and seg_address can be found in the [gp_segment_configuration](../../ref_guide/system_catalogs/gp_segment_configuration.mdx) catalog table. The fourth field, port, is the proxy port for the WarehousePG coordinator or a segment instance.
...
As an alternative, you can run the s[gpconfig](../../utility_guide/ref/gpconfig.mdx) utility to set the `gp_interconnect_proxy_addresses` parameter. To set the value as a string, the value is a single-quoted string that is enclosed in double quotes. The example WarehousePG cluster consists of a coordinator and a single segment instance.
