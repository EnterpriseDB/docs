...
title: Monitoring WarehousePG
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/managing/monitor.md
...
To see more detailed information about your WarehousePG array configuration, use `gpstate` with the `-s` option:
...
You can optionally set a WarehousePG configuration parameter, `gp_log_suboverflow_statement`, to record SQL statements that cause overflowed subtransactions. When this parameter is active, statements that cause overflow are recorded in server logs on the coordinator host and segment hosts with the text: `Statement caused suboverflow: <statement>`.  
