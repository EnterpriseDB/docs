...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/admin_guide/managing/access_db/pgbouncer.md
...
The following topics describe how to set up and use PgBouncer with WarehousePG. Refer to the [PgBouncer web site](https://pgbouncer.github.io) for information about using PgBouncer with PostgreSQL.
...
-   **If you are migrating to a WarehousePG version 6 or later**, you must shut down the PgBouncer instance in your old installation and reconfigure and restart PgBouncer in your new installation.
...
Refer to the [pgbouncer.ini](../../../ref_guide/utility_guide/reference/pgbouncer-ini.mdx) reference page for the PgBouncer configuration file format and the list of configuration properties it supports.
...
You can also obtain the MD5-encoded passwords of all WarehousePG users from the `pg_shadow` view.
...
If you enable LDAP authentication between `psql` and `pgbouncer` and you use `md5`, `password`, or `scram-sha-256` for authentication between PgBouncer and WarehousePG, ensure that you configure the latter password independently.
...
Refer to the WarehousePG [LDAP Authentication](../../../security-guide/Authenticate.mdx#ldap_auth) discussion for more information on configuring an HBA file for LDAP.
...
Starting in WarehousePG version 6.26, PgBouncer supports encrypted LDAP passwords. To utilize an encrypted LDAP password with PgBouncer, you must:
...
1.  Create a PgBouncer configuration file. For example, add the following text to a file named `pgbouncer.ini`:
    ...
    The file lists databases and their connection details. The file also configures the PgBouncer instance. Refer to the [pgbouncer.ini](../../../ref_guide/utility_guide/reference/pgbouncer-ini.mdx) reference page for information about the format and content of a PgBouncer configuration file.
...
3.  Launch `pgbouncer`:
    ...
    The `-d` option runs PgBouncer as a background (daemon) process. Refer to the [pgbouncer](../../../ref_guide/utility_guide/reference/pgbouncer.mdx) reference page for the `pgbouncer` command syntax and options.
...
For complete documentation of PgBouncer Administration Console commands, refer to the [pgbouncer-admin](../../../ref_guide/utility_guide/reference/pgbouncer-admin.mdx) command reference.
