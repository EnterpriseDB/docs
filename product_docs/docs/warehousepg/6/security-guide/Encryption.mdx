...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/6x/security-guide/Encryption.md
editTarget: originalFilePath

---

<a id="topic_th5_5bf_jr"></a>
...
<a id="ul_yyk_vq4_kr"></a><a id="ul_xdc_ycd_lr"></a><a id="ul_afc_bdd_lr"></a>
...
This section assumes you are installing WarehousePG on a Linux machine with the Gnu Privacy Guard (`gpg`) command line tool. Use the latest version of GPG to create keys. Download and install Gnu Privacy Guard (GPG) for your operating system from [https://www.gnupg.org/download/](https://www.gnupg.org/download/). On the GnuPG website you will find installers for popular Linux distributions and links for Windows and Mac OS X installers.

<a id="ol_vnm_h54_kr"></a><a id="ol_nty_qzd_lr"></a>
...
4.  Extract the public.key ID from the database:
    ...
    > **Note** Different keys may have the same ID. This is rare, but is a normal event. The client application should try to decrypt with each one to see which fits — like handling `ANYKEY`. See [pgp_key_id()](https://www.postgresql.org/docs/9.4/pgcrypto.html) in the pgcrypto documentation.
...
`ignore-cipher-failure` is dangerous. It makes pgcrypto ignore failures from the OpenSSL cipher in use, reproducing the incorrect decryption behavior from before the CVE-2026-14663 fix. A message recovered this way doesn't require the correct decryption key, so a successful decryption doesn't prove the plaintext came from someone who held the key. Use `ignore-cipher-failure` only to recover messages that were incorrectly encrypted because the cipher was unavailable under the OpenSSL configuration in use at the time.
!!!

If a message was correctly encrypted but its cipher is unavailable under the current OpenSSL configuration, `ignore-cipher-failure` doesn't help. Make the actual cipher available to OpenSSL instead, for example by enabling the appropriate provider. If "decrypting" a correctly encrypted message with `ignore-cipher-failure` happens to pass PGP's integrity checks, that result is coincidental and doesn't make the recovered plaintext trustworthy.
