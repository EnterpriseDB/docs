...
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/install_guide/platform-requirements-overview.md.hbs
editTarget: originalFilePath

---

This topic describes the WarehousePG 6 platform and operating system software requirements for deploying the software to VMware vSphere, to on-premise hardware, or to public cloud services such as AWS, GCP, or Azure.

<a id="topic13"></a>

## Operating System Requirements

WarehousePG 6 runs on the following operating system platforms:

-   Red Hat Enterprise Linux 64-bit 9.x
-   Red Hat Enterprise Linux 64-bit 8.7 or later (As of WarehousePG version 6.20. See the following [Note](#rhel-issues))
...
> **Note** If you use endpoint security software on your WarehousePG hosts, it may affect your database performance and stability. See [About Endpoint Security Sofware](../security-guide#endpoint_security) for more information.
...
> Significant WarehousePG performance degradation has been observed when enabling resource group-based workload management on RedHat 6.x and CentOS 6.x systems. This issue is caused by a Linux cgroup kernel bug. This kernel bug has been fixed in CentOS 7.x and Red Hat 7.x/8.x systems.
...
> **Caution** For WarehousePG that is installed on Red Hat Enterprise Linux 7.x or CentOS 7.x prior to 7.3, an operating system issue might cause WarehousePG that is running large workloads to hang in the workload. The WarehousePG issue is caused by Linux kernel bugs.
...
WarehousePG 6 requires the following software packages on RHEL/CentOS 6/7 systems which are installed automatically as dependencies when you install the WarehousePG RPM package):
...
-   bzip2
-   curl
-   compat-openssl11 (RHEL/Rocky 9)
-   iproute
-   krb5-devel
-   libcgroup (RHEL/CentOS 6)
-   libcgroup-tools (RHEL/CentOS 7 and RHEL/Rocky 8)
-   libcurl
-   libevent (RHEL/CentOS 7 and RHEL/Rocky 8)
-   libevent2 (RHEL/CentOS 6)
-   libuuid
-   libxml2
-   libyaml
-   libzstd (RHEL/Rocky 9)
-   less
-   net-tools (Debian/Fedora)
-   openldap
-   openssh
-   openssh-client
-   openssh-server
-   openssl
-   openssl-libs (RHEL/CentOS 7 and RHEL/Rocky 8)
-   perl
-   python3 (RHEL/Rocky 9)
-   readline
-   rsync
-   sed
-   tar
...
WarehousePG 6 client software requires these operating system packages:
...
VMware releases a Clients tool package on various platforms that can be used to access WarehousePG from a client system. The WarehousePG 6 Clients tool package is supported on the following platforms:
...
The WarehousePG 6 Clients package includes the client and loader programs provided in the WarehousePG 5 packages plus the addition of database/role/language commands and the WarehousePG Streaming Server command utilities. Refer to [WarehousePG Client and Loader Tools Package](/vmware/client_tool_guides/intro.html) for installation and usage details of the WarehousePG 6 Client tools.

<a id="topic_eyc_l2h_zz"></a>

### Extensions

This table lists the versions of the WarehousePG Extensions that are compatible with this release of WarehousePG 6.
...
<tbody class="tbody">
<tr class="row">
<td class="entry nocellnorowborder" style="vertical-align:top;" headers="d78288e683"><a class="xref" href="../analytics/pl_java.mdx">PL/Java</a></td>
...
<tr class="row">
<td class="entry nocellnorowborder" style="vertical-align:top;" headers="d78288e683"><a class="xref" href="install_python_dsmod.mdx">Python Data Science Module Package</a></td>
...
<tr class="row">
<td class="entry nocellnorowborder" style="vertical-align:top;" headers="d78288e683"><a class="xref" href="../analytics/pl_r.mdx">PL/R</a></td>
...
<tr class="row">
<td class="entry nocellnorowborder" style="vertical-align:top;" headers="d78288e683"><a class="xref" href="install_r_dslib.mdx">R Data Science Library Package</a></td>
...
<tr class="row">
<td class="entry nocellnorowborder" style="vertical-align:top;" headers="d78288e683"><a class="xref" href="../analytics/greenplum_r_client.html">WarehousePGR</a></td>
...
<tr class="row">
<td class="entry nocellnorowborder" style="vertical-align:top;" headers="d78288e683"><a class="xref" href="../analytics/madlib.mdx">MADlib Machine Learning</a></td>
...
<tr class="row">
<td class="entry row-nocellborder" style="vertical-align:top;" headers="d78288e683"><a class="xref" href="../analytics/postGIS.mdx">PostGIS Spatial and Geographic Objects</a></td>
...
For information about the Oracle Compatibility Functions, see [Oracle Compatibility Functions](../ref_guide/modules/orafce_ref.mdx).
...
<a id="topic_xpf_25b_hbb"></a>

### Data Connectors

-   WarehousePG Platform Extension Framework (PXF) - PXF provides access to Hadoop, object store, and SQL external data stores. Refer to [Accessing External Data with PXF](../admin_guide/external/pxf-overview.mdx) in the *WarehousePG Administrator Guide* for PXF configuration and usage information.

    > **Note** WarehousePG versions starting with 6.19.0 no longer bundle a version of PXF. You can install PXF in your WarehousePG cluster by installing [the independent distribution of PXF](https://docs.vmware.com/en/VMware-Greenplum-Platform-Extension-Framework/index.html) as described in the PXF documentation.
-   WarehousePG Streaming Server v1.5.3 - The WarehousePG Streaming Server is an ETL tool that provides high speed, parallel data transfer from Informatica, Kafka, Apache NiFi and custom client data sources to a WarehousePG cluster. Refer to the [WarehousePG Streaming Server](https://docs.vmware.com/en/VMware-Greenplum-Streaming-Server/index.html) Documentation for more information about this feature.
-   WarehousePG Streaming Server Kafka integration - The Kafka integration provides high speed, parallel data transfer from a Kafka cluster to a WarehousePG cluster for batch and streaming ETL operations. It requires Kafka version 0.11 or newer for exactly-once delivery assurance. Refer to the [WarehousePG Streaming Server](https://docs.vmware.com/en/VMware-Greenplum-Streaming-Server/index.html) Documentation for more information about this feature.
...
-   WarehousePG Informatica Connector v1.0.5 - The WarehousePG Connector for Informatica supports high speed data transfer from an Informatica PowerCenter cluster to a WarehousePG cluster for batch and streaming ETL operations.
...
-   Progress DataDirect ODBC Drivers v7.1.6+7.16.389 - The Progress DataDirect ODBC drivers enable third party applications to connect via a common interface to the WarehousePG cluster.
...
> **Note** WarehousePG 5.x clients (gpload, gpfdist) are supported with WarehousePG 6.x Server and Informatica PowerCenter and PowerExchange 10.4.

> **Note** WarehousePG 6 does not support the ODBC driver for Cognos Analytics V11.

Connecting to IBM Cognos software with an ODBC driver is not supported. WarehousePG supports connecting to IBM Cognos software with the DataDirect JDBC driver for WarehousePG. This driver is available as a download from [VMware Tanzu Network](https://network.pivotal.io/products/pivotal-gpdb).

<a id="topic_ncl_w1d_r1b"></a>

### WarehousePG Text

WarehousePG 6.0 through 6.4 are compatible with WarehousePG Text 3.3.1 through 3.4.1. WarehousePG 6.5 and later are compatible with WarehousePG Text 3.4.2 and later. See the [WarehousePG Text documentation](http://docs.vmware.com/en/VMware-Greenplum-Text/index.html) for additional compatibility information.

<a id="topic_zkq_j5b_hbb"></a>

### WarehousePG Command Center

WarehousePG 6.15 is compatible only with WarehousePG Command Center 6.4.0 and later. See the [WarehousePG Command Center documentation](http://docs.vmware.com/en/VMware-Greenplum-Command-Center/index.html) for additional compatibility information.

<a id="topic_tnl_3mx_zgb"></a>

## Hardware Requirements

The following table lists minimum recommended specifications for hardware servers intended to support WarehousePG on Linux systems in a production environment. All host servers in your WarehousePG cluster must have the same hardware and software configuration. WarehousePG also provides hardware build guides for its certified hardware platforms. It is recommended that you work with a WarehousePG clusters Engineer to review your anticipated environment to ensure an appropriate hardware configuration for WarehousePG.
...
<a id="topic_elb_4ss_n4b"></a>

### WarehousePG on DCA Systems

You must run WarehousePG version 6.9 or later on Dell EMC DCA systems, with software version 4.2.0.0 and later.

<a id="topic_pnz_5zd_xs"></a>

## Storage

The only file system supported for running WarehousePG is the XFS file system. All other file systems are explicitly *not* supported by VMware.
...
Other features of the shared storage such as de-duplication and/or replication are not directly supported by WarehousePG, but may be used with support of the storage vendor as long as they do not interfere with the expected operation of WarehousePG at the discretion of VMware.
...
WarehousePG provides access to HDFS with the [WarehousePG Platform Extension Framework (PXF)](https://docs.vmware.com/en/VMware-Greenplum-Platform-Extension-Framework/index.html).
...
{{> 'gpvirtual/vpshere/supported-platforms' }}
...
Throughput Optimized Disks (ST1) are a disk type designed for high throughput needs such as WarehousePG.  These disks are based on HDD rather than SSD, and are less expensive than GP2.  Use this disk type for the optimal performance of loading and querying data in AWS.
