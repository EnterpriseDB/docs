...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/install_guide/ansible-example.md
...
This Ansible playbook shows how tasks described in [Installing the WarehousePG Software](install_whpg.mdx) might be automated using [Ansible](https://docs.ansible.com).

> **Important** This playbook is provided as an *example only* to illustrate how WarehousePG cluster configuration and software installation tasks can be automated using provisioning tools such as Ansible, Chef, or Puppet.

The example playbook is designed for use with CentOS 7. It creates the `gpadmin` user, installs the WarehousePG software release, sets the owner and group of the installed software to `gpadmin`, and sets the Pam security limits for the `gpadmin` user.
...
## Ansible Playbook - WarehousePG Installation for CentOS 7
...
- hosts: all
  ...
  tasks:
    ...
    - name: find installed greenplum version
      shell: . /usr/edb/whpg6/greenplum_path.sh && /usr/edb/whpg6/bin/postgres --gp-version
