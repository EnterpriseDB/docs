...
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/install_guide/prep_os.html.md
...
For information about running WarehousePG in the cloud see *Cloud Services* in the [WarehousePG Partner Marketplace](https://pivotal.io/pivotal-greenplum/greenplum-partner-marketplace).
...
For `vm.overcommit_ratio` tuning and calculation recommendations with resource group-based resource management or resource queue-based resource management, refer to [Options for Configuring Segment Host Memory](../admin_guide/wlmgmt_intro.mdx) in the *WarehousePG Administrator Guide*.
...
2.  On the coordinator host, use your NTP daemon to synchronize the system clocks on all WarehousePG hosts. For example, using [gpssh](../utility_guide/ref/gpssh.mdx):
    If you are using the `ntpd` daemon:
