...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/6x/install_guide/config_os.md
...
1.  Make sure your host systems meet the requirements described in [Platform Requirements](platform-requirements.mdx).
...
Unless noted, these tasks should be performed for *all* hosts in your WarehousePG array (coordinator, standby coordinator, and segment hosts).

The WarehousePG host naming convention for the coordinator host is `mdw` and for the standby coordinator host is `smdw`.
...
> **Note** Automating the configuration steps described in this topic and [Installing the WarehousePG Software](install_whpg.mdx) with a system provisioning tool, such as Ansible, Chef, or Puppet, can save time and ensure a reliable and repeatable WarehousePG installation.
...
For all WarehousePG host systems running RHEL or CentOS, SELinux must either be `Disabled` or configured to allow unconfined access to WarehousePG processes, directories, and the gpadmin user.
...
If you decide to enable `iptables` with WarehousePG for security purposes, see [Enabling iptables (Optional)](enable_iptables.mdx) for important considerations and example configurations.
...
For `vm.overcommit_ratio` tuning and calculation recommendations with resource group-based resource management or resource queue-based resource management, refer to [Options for Configuring Segment Host Memory](../admin_guide/performance/wlmgmt_intro.mdx) in the *WarehousePG Administrator Guide*.
...
For information about the `gpinitsystem` cluster configuration file, see [Initializing WarehousePG](init_whpg.mdx).
...
For additional requirements and recommendations for cloud deployments, see [WarehousePG Cloud Technical Recommendations](platform-requirements.mdx#public-cloud).
...
-   The WarehousePG [gp_max_packet_size](../ref_guide/config_params/guc-list.mdx#gp_max_packet_size) server configuration parameter. The default max packet size is 8192. This default assumes a jumbo frame MTU.
...
2.  On the coordinator host, use your NTP daemon to synchronize the system clocks on all WarehousePG hosts. For example, using [gpssh](../ref_guide/utility_guide/reference/gpssh.mdx):
    If you are using the `ntpd` daemon:
...
> **Note** See [Example Ansible Playbook](ansible-example.mdx) for an example that shows how to automate the tasks of creating the `gpadmin` user and installing the WarehousePG software on all hosts in the cluster.
...
3.  Grant sudo access to the `gpadmin` user.
    ...
    This step is optional, but it's helpful, and it simplifies some procedures elsewhere in this guide, such as [Creating the Data Storage Areas](create_data_dirs.mdx), that run `sudo` commands on remote hosts through `gpssh`. If `gpadmin` doesn't have sudo access, you can't use `gpssh` or `gpscp` to run those commands. Because `gpssh` provides no terminal for a password prompt, the grant must let `gpadmin` run `sudo` without a password (`NOPASSWD`), as shown below, for those procedures to work.
    !!!
...
-   [Installing WarehousePG Software](install_whpg.mdx)
