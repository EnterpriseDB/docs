...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/6x/install_guide/minor_upgrade.md
...
!!!warning Upgrading to 6.27.6: Behavior changes to review

WarehousePG 6.27.6 changes the following behavior. Review each item before you upgrade.

-   Plain-text output from [`pg_dump`](../ref_guide/utility_guide/reference/pg_dump.mdx), [`pg_dumpall`](../ref_guide/utility_guide/reference/pg_dumpall.mdx), and [`pg_restore`](../ref_guide/utility_guide/reference/pg_restore.mdx) now begins with `\restrict <key>` and ends with `\unrestrict <key>`. Tooling that parses dump output needs adjusting for the new markers. Both the dumper and [`psql`](../ref_guide/utility_guide/reference/psql.mdx) used to restore the dump need to be on this version or later for the protection to apply.
-   [`CREATE OPERATOR`](../ref_guide/sql_commands/CREATE_OPERATOR.mdx) with a non-built-in `RESTRICT` or `JOIN` estimator now requires superuser privileges.
-   SQL-level calls to functions that take or return the `internal` type, and casts to and from `internal`, are rejected.
-   [`psql`](../ref_guide/utility_guide/reference/psql.mdx) now consumes in-line `COPY ... FROM STDIN` data even when the `COPY` command fails. Scripts must terminate in-line data with `\.`.
-   `pgcrypto` [PGP encryption](../security-guide/Encryption.mdx#encpgp) now returns an error when the cipher operation fails, instead of writing effectively unencrypted data. Use the new [`ignore-cipher-failure`](../security-guide/Encryption.mdx#recovering-data-encrypted-before-the-cve-2026-14663-fix) decryption option to read back data written by an affected build.
-   `tsvectorrecv()` now rejects empty lexemes and enforces the total-length limit consistently.
-   The new [`restrict_nonsystem_relation_kind`](../ref_guide/config_params/guc-list.mdx#restrict_nonsystem_relation_kind) configuration parameter defaults to empty, so it causes no behavior change unless you set it.
!!!
...
2.  Download your `6.target` package. See [Downloading the WarehousePG Server Software](install_whpg.mdx) for details on how to download the packages. From your jump host, copy the new package to the coordinator:
...
5.  Install WHPG `6.target` on the coordinator:
...
6.  Install WHPG `6.target` on segments: From the coordinator, run:
...
7.  Verify symbolic links: Ensure `/usr/local/greenplum-db` and `/usr/local/greenplum-db-clients` point to `/usr/edb/whpg6` across the cluster and that `gpadmin` is the owner of the installation directories:
...
4.  Handover: Formally notify the Infrastructure or Platform teams that the cluster is stable on version `6.target`.
