...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/install_guide/data_sci_pkgs/install_r_dslib.md
...
For information about the WarehousePG PL/R Language, see [WarehousePG PL/R Language Extension](../../admin_guide/analytics/procedural_languages/pl_r.mdx).
...
Before you install the R Data Science Library package, make sure that your WarehousePG is running, you have sourced `greenplum_path.sh`, and that the `$MASTER_DATA_DIRECTORY` and `$GPHOME` environment variables are set.
...
> **Note** `rjags` libraries are installed in the `$GPHOME/ext/DataScienceR/extlib/lib` directory. If you want to use `rjags` and your `$GPHOME` is not `/usr/edb/whpg6`, you must perform additional configuration steps to create a symbolic link from `$GPHOME` to `/usr/edb/whpg6` on each node in your WarehousePG cluster. For example:

```
$ gpssh -f all_hosts -e 'ln -s $GPHOME /usr/edb/whpg6'
$ gpssh -f all_hosts -e 'chown -h gpadmin /usr/edb/whpg6'

```

<a id="topic_removepdsl"></a>

## Uninstalling the R Data Science Library Package

Uninstall the R Data Science Library package. 

Re-source `greenplum_path.sh` and restart WarehousePG after you remove the R Data Science Library package:

```
$ . /usr/edb/whpg6/greenplum_path.sh
$ gpstop -r 
```
