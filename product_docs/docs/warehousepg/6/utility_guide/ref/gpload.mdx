...
title: gpload
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/utility_guide/ref/gpload.html.md
...
> **Note** `gpfdist` is compatible only with the WarehousePG major version in which it is shipped. For example, a `gpfdist` utility that is installed with WarehousePG 4.x cannot be used with WarehousePG 5.x or 6.x.

> **Note** The WarehousePG 5.22 and later `gpload` for Linux is compatible with WarehousePG 6.x. The WarehousePG 6.x `gpload` for both Linux and Windows is compatible with WarehousePG 5.x.
...
-   GPLOAD
    ...
    -   INPUT
        ...
        -   FILL_MISSING_FIELDS
            ...
            See the `FILL MISSING FIELDS` clause of the [CREATE EXTERNAL TABLE](../../ref_guide/sql_commands/CREATE_EXTERNAL_TABLE.mdx) command.
        ...
        -   LOG_ERRORS
            ...
            For more information about handling load errors, see "Loading and Unloading Data" in the *WarehousePG Administrator Guide*. For information about the `gp_read_error_log()` function, see the [CREATE EXTERNAL TABLE](../../ref_guide/sql_commands/CREATE_EXTERNAL_TABLE.mdx) command.
    ...
    -   OUTPUT
        ...
        -   MAPPING
            ...
            Where \\<expression></expression> is any expression that you would specify in the `SELECT` list of a query, such as a constant value, a column reference, an operator invocation, a function call, and so on.
...
Where \\<timestamp></timestamp> takes the form: `YYYY-MM-DD HH:MM:SS`, level is one of `DEBUG`, `LOG`, `INFO`, `ERROR`, and message is a normal text message.
...
[gpfdist](gpfdist.mdx), [CREATE EXTERNAL TABLE](../../ref_guide/sql_commands/CREATE_EXTERNAL_TABLE.mdx)
