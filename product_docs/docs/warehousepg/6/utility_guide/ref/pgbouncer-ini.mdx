...
-   auth_file
    ...
    The name of the file containing the user names and passwords to load. The file format is the same as the WarehousePG pg_auth file. Refer to the [PgBouncer Authentication File Format](../../admin_guide/access_db/pgbouncer.mdx#pgb_auth) for more information.

    Default: not set

-   auth_hba_file

    HBA configuration file to use when `auth_type` is `hba`. Refer to the [Configuring HBA-based Authentication for PgBouncer](../../admin_guide/access_db/pgbouncer.mdx#pgb_hba) and [Configuring LDAP-based Authentication for PgBouncer](../../admin_guide/access_db/pgbouncer.mdx#pgb_ldap) for more information.
