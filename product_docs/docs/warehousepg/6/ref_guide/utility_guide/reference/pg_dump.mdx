...
title: pg_dump
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/6x/ref_guide/utility_guide/reference/pg_dump.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="os137116"></a><a id="client_util_desc"></a>
...
<a id="client_util_synopsis"></a><div id="client_util_synopsis"></div>
...
Use `pg_dump` if you are migrating your data to another database vendor's system, or to another WarehousePG cluster with a different segment configuration (for example, if the system you are migrating to has greater or fewer segment instances). To restore, you must use the corresponding [pg_restore](pg_restore.mdx) utility (if the dump file is in archive format), or you can use a client program such as [psql](psql.mdx) (if the dump file is in plain text format).
...
-   \-F p|c|d|t | --format=plain|custom|directory|tar
    ...
    t | tar — Output a tar-format archive suitable for input into [pg_restore](pg_restore.mdx). The tar format is compatible with the directory format; extracting a tar-format archive produces a valid directory-format archive. However, the tar format does not support compression. Also, when using tar format the relative order of table data items cannot be changed during restore.
...
-   \-j njobs | --jobs=njobs
    ...
    `pg_dump` will open njobs + 1 connections to the database, so make sure your [max_connections](../../config_params/guc-list.mdx) setting is high enough to accommodate all connections.
...
-   \-o | --oids
    ...
    Dump object identifiers (OIDs) as part of the data for every table. Use of this option is not recommended for files that are intended to be restored into WarehousePG.
...
-   \--restrict-key=restrict_key
    ...
    Use the provided string as the `psql` `\restrict` key in the dump output. You can only specify this option for plain-text dumps, meaning when `--format` is set to `plain` or omitted. If you don't specify a restrict key, `pg_dump` generates a random one. Keys can contain only alphanumeric characters.

    This option is mainly for testing and other scenarios that need repeatable output, such as comparing dump files. Avoid it for general use, since a malicious server with advance knowledge of the key could inject code that runs on the machine that runs `psql` with the dump output.
...
Dump a WarehousePG in tar file format and include distribution policy information:
...
[pg_dumpall](pg_dumpall.mdx), [pg_restore](pg_restore.mdx), [psql](psql.mdx)
