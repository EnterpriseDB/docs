...
title: pg_restore
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/6x/ref_guide/utility_guide/reference/pg_restore.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="ov137116"></a><a id="client_util_desc"></a>
...
`pg_restore` is a utility for restoring a database from an archive created by [pg_dump](pg_dump.mdx) in one of the non-plain-text formats. It will issue the commands necessary to reconstruct the database to the state it was in at the time it was saved. The archive files also allow `pg_restore` to be selective about what is restored, or even to reorder the items prior to being restored.
...
-   \-d dbname | --dbname=dbname
    ...
    Connect to this database and restore directly into this database. This utility, like most other WarehousePG utilities, also uses the environment variables supported by `libpq`. However it does not read `PGDATABASE` when a database name is not supplied.
...
-   \-F c|d|t | --format={custom | directory | tar}
    ...
    The format of the archive produced by [pg_dump](pg_dump.mdx). It is not necessary to specify the format, since `pg_restore` will determine the format automatically. Format can be `custom`, `directory`, or `tar`.
...
-   \--restrict-key=restrict_key
    ...
    Use the provided string as the `psql` `\restrict` key when producing plain-text output. You can only specify this when `pg_restore` writes a script rather than connecting directly to a database. If you don't specify a restrict key, `pg_restore` generates a random one. Keys can contain only alphanumeric characters.

    This option is mainly for testing and other scenarios that need repeatable output. Avoid it for general use, since a malicious server with advance knowledge of the key could inject code that runs on the machine that runs `psql` with the output.
