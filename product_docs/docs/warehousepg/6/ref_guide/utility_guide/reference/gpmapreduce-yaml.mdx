...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/utility_guide/reference/gpmapreduce-yaml.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="ii20941"></a>
...
You specify the input, map and reduce tasks, and the output for the WarehousePG MapReduce `gpmapreduce` program in a YAML-formatted configuration file. (This reference page uses the name `gpmapreduce.yaml` when referring to this file; you may choose your own name for the file.)
...
-   VERSION
    ...
    Required. The version of the WarehousePG MapReduce YAML specification. Current supported versions are 1.0.0.1, 1.0.0.2, and 1.0.0.3.

-   DATABASE

    <a id="database"></a>

    Optional. Specifies which database in WarehousePG to connect to. If not specified, defaults to the default database or `$PGDATABASE` if set.

-   USER

    <a id="user"></a>

    Optional. Specifies which database role to use to connect. If not specified, defaults to the current user or `$PGUSER` if set. You must be a WarehousePG superuser to run functions written in untrusted Python and Perl. Regular database users can run functions written in trusted Perl. You also must be a database superuser to run MapReduce jobs that contain [FILE](#file), [GPFDIST](#gpfdist) and [EXEC](#exec) input types.

-   HOST

    <a id="host"></a>

    Optional. Specifies WarehousePG coordinator host name. If not specified, defaults to localhost or `$PGHOST` if set.

-   PORT

    <a id="port"></a>

    Optional. Specifies WarehousePG coordinator port. If not specified, defaults to 5432 or `$PGPORT` if set.
...
-   DEFINE
    ...
    -   FILE
        ...
        A sequence of one or more input files in the format: `seghostname:/path/to/filename`. You must be a WarehousePG superuser to run MapReduce jobs with `FILE` input. The file must reside on a WarehousePG segment host.

    -   GPFDIST

        <a id="gpfdist"></a>

        A sequence identifying one or more running `gpfdist` file servers in the format: `hostname[:port]/file_pattern`. You must be a WarehousePG superuser to run MapReduce jobs with `GPFDIST` input.
    ...
    -   EXEC
        ...
        An operating system command to run on the WarehousePG segment hosts. The command is run by all segment instances in the system by default. For example, if you have four segment instances per segment host, the command will be run four times on each host. You must be a WarehousePG superuser to run MapReduce jobs with `EXEC` input.
    ...
    -   ERROR_LIMIT
        ...
        If the input rows have format errors they will be discarded provided that the error limit count is not reached on any WarehousePG segment instance during input processing. If the error limit is not reached, all good rows will be processed and any error rows discarded.

    -   ENCODING

        <a id="encoding"></a>

        Character set encoding to use for the data. Specify a string constant (such as `'SQL_ASCII'`), an integer encoding number, or `DEFAULT` to use the default client encoding. See [Character Set Support](../../character_sets.mdx) for more information.
    ...
    If a `CONSOLIDATE` function is specified, `TRANSITION` processing is performed at the segment-level before redistributing the keys across the WarehousePG interconnect for final aggregation (two-phase aggregation). Only the resulting `state` value for a given key is redistributed, resulting in lower interconnect traffic and greater parallelism. `CONSOLIDATE` is handled like a `TRANSITION`, except that instead of `(state + value) => state`, it is `(state + state) => state`.
    ...
    -   TASK
        ...
        Optional. A `TASK` defines a complete end-to-end `INPUT`/`MAP`/`REDUCE` stage within a WarehousePG MapReduce job pipeline. It is similar to [EXECUTE](#execute) except it is not immediately run. A task object can be called as [INPUT](#input) to further processing stages.
...
-   EXECUTE
    ...
    Required. `EXECUTE` defines the final `INPUT`/`MAP`/`REDUCE` stage within a WarehousePG MapReduce job pipeline.
