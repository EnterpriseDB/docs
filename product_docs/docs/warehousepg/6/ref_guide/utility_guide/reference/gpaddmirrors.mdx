...
title: gpaddmirrors
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/utility_guide/reference/gpaddmirrors.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="jp20941"></a>
...
-   \-d coordinator_data_directory
    ...
    The coordinator data directory. If not specified, the value set for `$MASTER_DATA_DIRECTORY` will be used.
...
-   \-s (spread mirrors)
    ...
    Spreads the mirror segments across the available hosts. The default is to group a set of mirror segments together on an alternate host from their primary segment set. Mirror spreading will place each mirror on a different host within the WarehousePG array. Spreading is only allowed if there is a sufficient number of hosts in the array (number of hosts is greater than the number of segment instances per host).
...
For example, this is a segment instance configuration for a simple WarehousePG cluster. The segment host `gp6m` is configured with two NICs, `gp6m-1` and `gp6m-2`, where the WarehousePG cluster uses `gp6m-1` for the coordinator segment and `gp6m-2` for segment instances.
