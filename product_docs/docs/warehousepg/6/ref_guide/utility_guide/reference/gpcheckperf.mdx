...
title: gpcheckperf
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/utility_guide/reference/gpcheckperf.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="jq20941"></a>
...
Before using `gpcheckperf`, you must have a trusted host setup between the hosts involved in the performance test. You can use the utility `gpssh-exkeys` to update the known host files and exchange public keys between hosts if you have not done so already. Note that `gpcheckperf` calls to `gpssh` and `gpscp`, so these WarehousePG utilities must also be in your `$PATH`.
...
-   \-r ds{n|N|M}
    ...
    <a id="ul_jjn_vvt_mo"></a><a id="jq141318"></a>
    - Disk I/O test (`d`)
    <a id="jq141319"></a>
    - Stream test (`s`)
    - Network performance test in sequential (`n`), parallel (`N`), or full-matrix (`M`) mode. The optional `--duration` option specifies how long (in seconds) to run the network test. To use the parallel (`N`) mode, you must run the test on an even number of hosts. <br /><br />If you would rather use `netperf` ([https://github.com/HewlettPackard/netperf](https://github.com/HewlettPackard/netperf)) instead of the WarehousePG network test, you can download it and install it into `$GPHOME/bin/lib` on all WarehousePG hosts (coordinator and segments). You would then specify the optional `--netperf` option to use the `netperf` binary instead of the default `gpnetbench*` utilities.
