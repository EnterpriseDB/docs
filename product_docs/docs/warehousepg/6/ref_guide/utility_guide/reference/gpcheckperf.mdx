...
title: gpcheckperf
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/utility_guide/ref/gpcheckperf.html.md
...
-   \--netperf
    ...
    Specifies that the `netperf` binary should be used to perform the network test instead of the WarehousePG network test. To use this option, you must download `netperf` from <https://github.com/HewlettPackard/netperf> and install it into `$GPHOME/bin/lib` on all WarehousePG hosts (coordinator and segments).
