...
title: pgbouncer.ini
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/utility_guide/reference/pgbouncer-ini.md
...
-   auth_file
    ...
    The name of the file containing the user names and passwords to load. The file format is the same as the WarehousePG pg_auth file. Refer to the [PgBouncer Authentication File Format](../../../admin_guide/managing/access_db/pgbouncer.mdx#pgb_auth) for more information.

    Default: not set

-   auth_hba_file

    HBA configuration file to use when `auth_type` is `hba`. Refer to the [Configuring HBA-based Authentication for PgBouncer](../../../admin_guide/managing/access_db/pgbouncer.mdx#pgb_hba) and [Configuring LDAP-based Authentication for PgBouncer](../../../admin_guide/managing/access_db/pgbouncer.mdx#pgb_ldap) for more information.
...
-   auth_type
    ...
    <a id="ul_dgn_drd_bt"></a><a id="ul_hyh_1td_bt"></a>
...
-   auth_key_file
    ...
    If you are connecting to LDAP with an encrypted password, `auth_key_file` identifies the file system location of the encryption key. Refer to [About Specifying an Encrypted LDAP Password](../../../admin_guide/managing/access_db/pgbouncer.mdx#pgb_ldap_encrypt_passwd) for more information.

    Default: not set

-   auth_cipher

    If you are connecting to LDAP with an encrypted password, `auth_cipher` identifies the cipher algorithm for password authentication. PgBouncer accepts any cipher supported by OpenSSL on the system. When FIPS mode is enabled, specify only a cipher that is considered safe in FIPS mode. Refer to [About Specifying an Encrypted LDAP Password](../../../admin_guide/managing/access_db/pgbouncer.mdx#pgb_ldap_encrypt_passwd) for more information.
...
[pgbouncer](pgbouncer.mdx), [pgbouncer-admin](pgbouncer-admin.mdx), [PgBouncer Configuration Page](https://pgbouncer.github.io/config.html)
