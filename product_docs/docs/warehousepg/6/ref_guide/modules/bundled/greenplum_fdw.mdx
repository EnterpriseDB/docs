...
title: greenplum_fdw
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/6x/ref_guide/modules/bundled/greenplum_fdw.md
description: >-
  The greenplumfdw module is a foreign-data wrapper (FDW) that you can use to
  run queries between one or more WarehousePG version 6.20+ and 7 clusters.
...
Refer to [Installing Extensions](../../../install_guide/additional_modules) for more information about installing and registering modules in WarehousePG.
...
To access a remote WarehousePG cluster, you must first create a foreign server object which specifies the host, port, and database connection details. You provide these connection parameters in the `OPTIONS` clause of the [CREATE SERVER](../../sql_commands/CREATE_SERVER.mdx) command.
...
After you identify which users you will allow to access the remote WarehousePG cluster, you must create one or more mappings between a local WarehousePG user and a user on the remote WarehousePG cluster. You create these mappings with the [CREATE USER MAPPING](../../sql_commands/CREATE_USER_MAPPING.mdx) command.
