...
title: timestamp9
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/6x/ref_guide/modules/bundled/timestamp9.md
description: >-
  The timestamp9 module provides an efficient, nanosecond-precision timestamp
  data type and related functions and operators.
...
If no time zone is specified in the input string, then it is presumed to be in the time zone indicated by the system's [`TIMEZONE` server configuration parameter](../../config_params/guc-list.mdx#timezone) and is converted to UTC using the offset for the time zone.
...
You can set the [TimeZone](../../config_params/guc-list.mdx#timezone) server configuration parameter to specify the time zone that WarehousePG uses when it prints a `timestamp9` timestamp. When you set this parameter, WarehousePG displays the timestamp value in that time zone. For example:
