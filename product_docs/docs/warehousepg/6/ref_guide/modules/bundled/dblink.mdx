...
title: dblink
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/6x/ref_guide/modules/bundled/dblink.md
description: >-
  The dblink module supports connections to other WarehousePG databases from
  within a database session. These databases can reside in the same WarehousePG
  cluster, or in a remote system.
editTarget: originalFilePath

---

<a id="topic_lv4_czp_fz"></a>
...
> **Note** `dblink` is intended for database users to perform short ad hoc queries in other databases. `dblink` is not intended for use as a replacement for external tables or for administrative tools.
...
The `dblink` module is installed when you install WarehousePG. Before you can use any of the functions defined in the module, you must register the `dblink` extension in each database in which you want to use the functions. Refer to [Installing Extensions](../../../install_guide/additional_modules) for more information.
...
<a id="ol_dpt_ll3_5fb"></a><a id="ol_agf_qwj_x4b"></a>
...
Refer to [SSL Client Authentication](../../../security-guide/Authenticate.mdx#ssl_postgresql) for information about configuring WarehousePG to use SSL.
