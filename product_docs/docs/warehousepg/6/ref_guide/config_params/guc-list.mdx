...
| Value Range                                                                                                                                                                             | Default  | Set Classifications          |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------- |
| \\<format></format>, \\<date style=""><br /><br /></date>where:<br /><br />\\<format></format> is ISO, Postgres, SQL, or German<br /><br />\\<date style=""></date> is DMY, MDY, or YMD | ISO, MDY | coordinator, session, reload |
...
Selects the text search configuration that is used by those variants of the text search functions that do not have an explicit argument specifying the configuration. See [Using Full Text Search](../../admin_guide/textsearch#full-text-search) for further information. The built-in default is `pg_catalog.simple`, but `initdb` will initialize the configuration file with a setting that corresponds to the chosen `lc_ctype` locale, if a configuration matching that locale can be identified.
...
<a id="gp_enable_query_metrics">

</a>

<a id="gp_enable_query_metrics"></a>

## gp_enable_query_metrics

Enables collection of query metrics. When query metrics collection is enabled, WarehousePG collects metrics during query execution. The default is off.

After changing this configuration parameter, WarehousePG must be restarted for the change to take effect.

The WarehousePG metrics collection extension, when enabled, sends the collected metrics over UDP to a WarehousePG Command Center agent<sup>1</sup>.

> **Note** <sup>1</sup>The metrics collection extension is included in VMware's commercial version of WarehousePG. WarehousePG Command Center is supported only with WarehousePG.
...
If the value is `true`, WarehousePG ignores the clause, issues a warning, and runs the command without the `INTO ERROR TABLE` clause. In WarehousePG 5.x and later, you access the error log information with built-in SQL functions. See the [CREATE EXTERNAL TABLE](../sql_commands/CREATE_EXTERNAL_TABLE.mdx) or [COPY](../sql_commands/COPY.mdx) command.

You can set this value to `true` to avoid the WarehousePG error when you run applications that run `CREATE EXTERNAL TABLE` or `COPY` commands that include the WarehousePG 4.3.x `INTO ERROR TABLE` clause.
...
<a id="gp_instrument_shmem_size">

</a>

<a id="gp_instrument_shmem_size"></a>

## gp_instrument_shmem_size

The amount of shared memory, in kilobytes, allocated for query metrics. The default is 5120 and the maximum is 131072. At startup, if [gp_enable_query_metrics](#gp_enable_query_metrics) is set to on, WarehousePG allocates space in shared memory to save query metrics. This memory is organized as a header and a list of slots. The number of slots needed depends on the number of concurrent queries and the number of execution plan nodes per query. The default value, 5120, is based on a WarehousePG cluster that runs a maximum of about 250 concurrent queries with 120 nodes per query. If the `gp_enable_query_metrics` configuration parameter is off, or if the slots are exhausted, the metrics are maintained in local memory instead of in shared memory.
...
Setting the value to `true` uses the modulo hash algorithm that is compatible with WarehousePG 5.x and earlier releases.
...
| Value Range                      | Default | Set Classifications |
| -------------------------------- | ------- | ------------------- |
| \\<system dependent=""></system> |         | read only           |
...
| Value Range                      | Default | Set Classifications    |
| -------------------------------- | ------- | ---------------------- |
| \\<system dependent=""></system> |         | local, session, reload |
...
| Value Range                      | Default | Set Classifications    |
| -------------------------------- | ------- | ---------------------- |
| \\<system dependent=""></system> |         | local, system, restart |
...
| Value Range                      | Default | Set Classifications |
| -------------------------------- | ------- | ------------------- |
| \\<system dependent=""></system> | UTF8    | read only           |
