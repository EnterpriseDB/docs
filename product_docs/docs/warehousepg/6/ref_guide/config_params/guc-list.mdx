...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/6x/ref_guide/config_params/guc-list.md
...
Enable filter pushdown when reading data from external tables. If pushdown fails, a query is run without pushing filters to the external data source (instead, WarehousePG applies the same constraints to the result). See [Defining External Tables](../../admin_guide/external/external-tables) for more information.
...
Controls the time (in seconds) that WarehousePG waits before returning an error when WarehousePG is attempting to connect or write to a [gpfdist](../utility_guide/reference/gpfdist.mdx) server and `gpfdist` does not respond. The default value is 300 (5 minutes). A value of 0 deactivates the timeout.
...
Activates or deactivates  the enforcement of resource group concurrent transaction limits on WarehousePG resources. The default value is `false`, which enforces resource group transaction limits. Resource groups manage resources such as CPU, memory, and the number of concurrent transactions that are used by queries and external components.
...
Identifies the resource management scheme currently enabled in the WarehousePG cluster. The default scheme is to use resource queues. For information about WarehousePG resource management, see [Managing Resources](../../admin_guide/performance/wlmgmt).
...
For information about GPORCA, see [About GPORCA](../../admin_guide/query/query-piv-optimizer) in the *WarehousePG Administrator Guide*.
...
For information about setting the password hash algorithm to protect user passwords, see [Protecting Passwords in WarehousePG](../../admin_guide/manage_access/roles_privs.mdx#topic9).
...
Ensures that all identifiers are quoted, even if they are not keywords, when the database generates SQL. See also the `--quote-all-identifiers` option of [pg_dump](../utility_guide/reference/pg_dump.mdx) and [pg_dumpall](../utility_guide/reference/pg_dumpall.mdx).
...
<a id="restrict_nonsystem_relation_kind"></a>

## restrict_nonsystem_relation_kind

Restricts access to non-system relations of the specified kinds. Set to a comma-separated list containing `view`, `foreign-table`, or both, to prevent the backend from expanding non-system views or accessing non-system foreign tables. `pg_dump` sets this parameter for the duration of a dump to protect against a privilege-escalation attack from a maliciously crafted view or foreign table.

| Value Range                                       | Default | Set Classifications          |
| ------------------------------------------------- | ------- | ---------------------------- |
| a comma-separated list of `view`, `foreign-table` | (empty) | coordinator, session, reload |
...
For information about resource groups, see [Using Resource Groups](../../admin_guide/performance/wlmgmt/workload_mgmt_resgroups.mdx).
...
Enables the collection of information on the currently executing command of each session, along with the time when that command began execution. The default value is `true`. Only superusers can change this setting. See the [pg_stat_activity](../system_catalogs/system_catalogs_definitions/pg_stat_activity.mdx) view.
...
For information about the `gpfdists` protocol, see [gpfdists:// Protocol](../../admin_guide/external/external-tables/gpfdists-protocol.mdx). For information about running the `gpfdist` utility, see [gpfdist](../utility_guide/reference/gpfdist.mdx).
