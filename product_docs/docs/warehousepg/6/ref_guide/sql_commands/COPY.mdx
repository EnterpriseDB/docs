...
title: COPY
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/sql_commands/COPY.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="bk20941"></a><a id="sql_command_desc"></a>
...
`COPY` with a file name instructs the WarehousePG coordinator host to directly read from or write to a file. The file must be accessible to the coordinator host and the name must be specified from the viewpoint of the coordinator host.
...
-   PROGRAM 'command'
    ...
    Specify a command to run. In `COPY FROM`, the input is read from standard output of the command, and in `COPY TO`, the output is written to the standard input of the command. The command must be specified from the viewpoint of the WarehousePG coordinator host system, and must be executable by the WarehousePG administrator user (`gpadmin`).
...
-   IGNORE EXTERNAL PARTITIONS
    ...
    If this clause is not specified and WarehousePG attempts to copy data from a leaf child partition that is an external table, an error is returned.
...
When a `COPY FROM...ON SEGMENT` command is run, the server configuration parameter `gp_enable_segment_copy_checking` controls whether the table distribution policy (from the table `DISTRIBUTED` clause) is checked when data is copied into the table. The default is to check the distribution policy. An error is returned if the row of data violates the distribution policy for the segment instance. For information about the parameter, see [Server Configuration Parameters](../config_params).
...
<a id="ul_wk3_jdj_bp"></a><a id="ul_bf3_2pk_zs"></a>
...
-   Use the built-in SQL function `gp_read_error_log('table\_name')`. It requires `SELECT` privilege on table_name. This example displays the error log information for data loaded into table `ext_expenses` with a `COPY` command:
    ...
    For information about the error log format, see [Viewing Bad Rows in the Error Log](../../admin_guide/load/handling-load-errors/viewing-bad-rows-in-the-error-table-or-error-log.mdx) in the *WarehousePG Administrator Guide*.
...
For information about resource queues, see "Resource Management with Resource Queues" in the *WarehousePG Administrator Guide*.
