...
title: TRUNCATE
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/sql_commands/TRUNCATE.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="ep20941"></a><a id="sql_command_desc"></a>

Empties a table of all rows.

> **Note** WarehousePG does not enforce referential integrity syntax (foreign key constraints). As of version 6.12 `TRUNCATE` truncates a table that is referenced in a foreign key constraint even if the `CASCADE` option is omitted.
...
`TRUNCATE` quickly removes all rows from a table or set of tables. It has the same effect as an unqualified [DELETE](DELETE.mdx) on each table, but since it does not actually scan the tables it is faster. This is most useful on large tables.
...
[DELETE](DELETE.mdx), [DROP TABLE](DROP_TABLE.mdx)
