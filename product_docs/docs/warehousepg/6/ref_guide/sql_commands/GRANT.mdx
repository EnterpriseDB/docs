...
title: GRANT
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/sql_commands/GRANT.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="ds20941"></a><a id="sql_command_desc"></a>
...
WarehousePG grants default privileges on some types of objects to `PUBLIC`. No privileges are granted to `PUBLIC` by default on tables, table columns, sequences, foreign-data wrappers, foreign servers, large objects, schemas, or tablespaces. For other types of objects, the default privileges granted to `PUBLIC` are as follows:
...
The `PRIVILEGES` key word is required in the SQL standard, but optional in WarehousePG. The SQL standard does not support setting the privileges on more than one object per command.

WarehousePG allows an object owner to revoke their own ordinary privileges: for example, a table owner can make the table read-only to theirself by revoking their own `INSERT`, `UPDATE`, `DELETE`, and `TRUNCATE` privileges. This is not possible according to the SQL standard. WarehousePG treats the owner's privileges as having been granted by the owner to the owner; therefore they can revoke them too. In the SQL standard, the owner's privileges are granted by an assumed *system* entity.
...
[REVOKE](REVOKE.mdx), [CREATE ROLE](CREATE_ROLE.mdx), [ALTER ROLE](ALTER_ROLE.mdx)
