...
title: CREATE RULE
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/sql_commands/CREATE_RULE.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="ca20941"></a><a id="sql_command_desc"></a>
...
> **Note** A view that is simple enough to be automatically updatable (see [CREATE VIEW](CREATE_VIEW.mdx)) does not require a user-created rule in order to be updatable. While you can create an explicit rule anyway, the automatic update transformation will generally outperform an explicit rule.
...
It is very important to take care to avoid circular rules. For example, though each of the following two rule definitions are accepted by WarehousePG, the `SELECT` command would cause WarehousePG to report an error because of recursive expansion of a rule:
...
`CREATE RULE` is a WarehousePG language extension, as is the entire query rewrite system.
...
[ALTER RULE](ALTER_RULE.mdx), [DROP RULE](DROP_RULE.mdx)
