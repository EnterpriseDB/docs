...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/6x/ref_guide/sql_commands/CREATE_PROTOCOL.md
...
WarehousePG handles external tables of type `file`, `gpfdist`, and `gpfdists` internally. See [s3:// Protocol](../../admin_guide/external/external-tables/s3-protocol.mdx#configuring-the-s3-protocol) for information about enabling the `S3` protocol. Refer to [PXF for WarehousePG](/pxf/6/) for information about using the `pxf` protocol.
