...
title: EXPLAIN
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/ref_guide/sql_commands/EXPLAIN.html.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="dq20941"></a><a id="sql_command_desc"></a>
...
-   The time (in milliseconds) it took to retrieve the first row from the segment that produced the most rows, and the total time taken to retrieve all rows from that segment. The *\\<time></time> to first row* may be omitted if it is the same as the *\\<time></time> to end*.
