...
-   LOCATION ('protocol://\[host\[:port]]/path/file' \[, ...])
    ...
    If you use the `s3` protocol to read or write to S3, refer to [s3:// Protocol](../../admin_guide/external/s3-protocol.mdx) for additional information about the `s3` protocol `LOCATION` clause syntax.
...
When you specify the `LOG ERRORS` clause, WarehousePG captures errors that occur while reading the external table data. For information about the error log format, see [Viewing Bad Rows in the Error Log](../../admin_guide/load/viewing-bad-rows-in-the-error-table-or-error-log.mdx).
