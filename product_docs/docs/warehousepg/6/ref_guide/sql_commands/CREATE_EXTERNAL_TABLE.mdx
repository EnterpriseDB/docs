...
-   LOCATION ('protocol://\[host\[:port]]/path/file' \[, ...])
    ...
    If you use the `pxf` protocol to access an external data source, refer to [PXF for WarehousePG](/pxf/6/) for information about the `pxf` protocol.
...
-   FORMAT 'TEXT | CSV' (options)
    ...
    If you use the `pxf` protocol to access an external data source, refer to [PXF for WarehousePG](/pxf/6/) for information about using PXF.
