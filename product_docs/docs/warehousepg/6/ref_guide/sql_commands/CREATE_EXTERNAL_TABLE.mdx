...
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/ref_guide/sql_commands/CREATE_EXTERNAL_TABLE.html.md
...
-   EXECUTE 'command' \[ON ...]
    ...
    - SEGMENT segment\_id means the command will be run only once by the specified segment. You can determine a segment instance's ID by looking at the content number in the system catalog table [gp\_segment\_configuration](../system_catalogs/system_catalogs_definitions/gp_segment_configuration.mdx). The content ID of the WarehousePG coordinator is always `-1`.
