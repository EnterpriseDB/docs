...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/sql_commands/CREATE_EXTERNAL_TABLE.md
...
-   LOCATION ('protocol://\[host\[:port]]/path/file' \[, ...])
    ...
    If you use the `pxf` protocol to access an external data source, refer to [pxf:// Protocol](../../admin_guide/external/external-tables/pxf-protocol.mdx) for information about the `pxf` protocol.
...
-   FORMAT 'TEXT | CSV' (options)
    ...
    If you use the `pxf` protocol to access an external data source, refer to [Accessing External Data with PXF](../../admin_guide/external/pxf-overview.mdx) for information about using PXF.
