...
-   LOCATION ('protocol://\[host\[:port]]/path/file' \[, ...])
    ...
    If you use the `pxf` protocol to access an external data source, refer to [pxf:// Protocol](../../admin_guide/external/pxf-protocol.mdx) for information about the `pxf` protocol.

    If you use the `s3` protocol to read or write to S3, refer to [s3:// Protocol](../../admin_guide/external/s3-protocol.mdx#amazon-emr/section_stk_c2r_kx) for additional information about the `s3` protocol `LOCATION` clause syntax.
...
The limit for the number of initial rejected rows can be changed with the WarehousePG server configuration parameter `gp_initial_bad_row_limit`. See [Server Configuration Parameters](../config_params/guc_config.mdx) for information about the parameter.
...
When you specify the `LOG ERRORS` clause, WarehousePG captures errors that occur while reading the external table data. For information about the error log format, see [Viewing Bad Rows in the Error Log](../../admin_guide/load/viewing-bad-rows-in-the-error-table-or-error-log.mdx#topic58).
