...
title: UPDATE
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/sql_commands/UPDATE.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="eq20941"></a><a id="sql_command_desc"></a>
...
> **Note** As the default, WarehousePG acquires an `EXCLUSIVE` lock on tables for `UPDATE` operations on heap tables. When the Global Deadlock Detector is enabled, the lock mode for `UPDATE` operations on heap tables is `ROW EXCLUSIVE`. See [Global Deadlock Detector](../../admin_guide/dml.mdx#topic_gdd).
...
-   cursor_name
    ...
    The name of the cursor to use in a `WHERE CURRENT OF` condition. The row to be updated is the one most recently fetched from the cursor. The cursor must be a non-grouping query on the `UPDATE` command target table. See [DECLARE](DECLARE.mdx) for more information about creating cursors.
...
`SET` is not allowed on the WarehousePG distribution key columns of a table.
...
This command conforms to the SQL standard, except that the `FROM` clause is a WarehousePG extension.
...
[DECLARE](DECLARE.mdx), [DELETE](DELETE.mdx), [SELECT](SELECT.mdx), [INSERT](INSERT.mdx)
