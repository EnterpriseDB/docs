...
title: VALUES
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/sql_commands/VALUES.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="es20941"></a><a id="sql_command_desc"></a>
...
-   sort_expression
    ...
    An expression or integer constant indicating how to sort the result rows. This expression may refer to the columns of the `VALUES` result as `column1`, `column2`, etc. For more details, see "The ORDER BY Clause" in the parameters for [SELECT](SELECT.mdx).

-   operator

    A sorting operator. For more details, see "The ORDER BY Clause" in the parameters for [SELECT](SELECT.mdx).

-   LIMIT count

-   OFFSET start

    The maximum number of rows to return. For more details, see "The LIMIT Clause" in the parameters for [SELECT](SELECT.mdx).
...
`VALUES` conforms to the SQL standard. `LIMIT` and `OFFSET` are WarehousePG extensions; see also under [SELECT](SELECT.mdx).
...
[INSERT](INSERT.mdx), [SELECT](SELECT.mdx)
