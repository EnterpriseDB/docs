...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/sql_commands/ALTER_FOREIGN_DATA_WRAPPER.md
...
-   VALIDATOR validator_function
    ...
    Options to the foreign-data wrapper, servers, and user mappings may become invalid when you change the validator function. You must make sure that these options are correct before using the modified foreign-data wrapper. Note that WarehousePG checks any options specified in this `ALTER FOREIGN DATA WRAPPER` command using the new validator.
...
[CREATE FOREIGN DATA WRAPPER](CREATE_FOREIGN_DATA_WRAPPER.mdx), [DROP FOREIGN DATA WRAPPER](DROP_FOREIGN_DATA_WRAPPER.mdx)
