...
For writable S3 external tables, the `INSERT` operation uploads to one or more files in the configured S3 bucket, as described in [s3:// Protocol](../../admin_guide/external/s3-protocol.mdx). Pressing `Ctrl-c` cancels the `INSERT` and stops uploading to S3.
...
[COPY](COPY.mdx), [SELECT](SELECT.mdx), [CREATE EXTERNAL TABLE](CREATE_EXTERNAL_TABLE.mdx), [s3:// Protocol](../../admin_guide/external/s3-protocol.mdx)
