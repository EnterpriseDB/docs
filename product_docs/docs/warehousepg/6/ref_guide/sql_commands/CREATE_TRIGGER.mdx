...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/6x/ref_guide/sql_commands/CREATE_TRIGGER.md
editTarget: originalFilePath

---

<a id="topic1"></a><a id="cg20941"></a><a id="sql_command_desc"></a>
...
> Event Triggers, which capture only DDL events, *are* supported in WarehousePG. See the PostgreSQL documentation for [Event Triggers](https://www.postgresql.org/docs/9.4/event-triggers.html) for additional information.

[SELECT](SELECT.mdx) does not modify any rows so you can not create `SELECT` triggers. Rules and views are more appropriate in such cases.
...
[CREATE FUNCTION](CREATE_FUNCTION.mdx), [ALTER TRIGGER](ALTER_TRIGGER.mdx), [DROP TRIGGER](DROP_TRIGGER.mdx), [CREATE RULE](CREATE_RULE.mdx)
