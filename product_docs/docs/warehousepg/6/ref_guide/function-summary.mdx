...
> **Note** The WarehousePG MADlib Extension for Analytics provides additional advanced functions to perform statistical analysis and machine learning with WarehousePG data. See [MADlib Extension for Analytics](../analytics/madlib.mdx).
...
The following tables summarize the functions and operators that are provided for full text searching. See [Using Full Text Search](../admin_guide/textsearch) for a detailed explanation of WarehousePG's text search facility.
...
See [Range Types](datatype-range.mdx) for an overview of range types.
