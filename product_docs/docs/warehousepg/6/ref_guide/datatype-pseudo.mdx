...
title: Pseudo-Types
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/ref_guide/datatype-pseudo.html.md
...
The *anytable* type is used in some user-defined functions in the WarehousePG Text API. The following GPText example uses the `TABLE` function with the `SCATTER BY` clause in the GPText function `gptext.index()` to populate the index `mydb.mytest.articles` with data from the messages table:
...
For information about the function `gptext.index()`, see the WarehousePG Text documentation.
