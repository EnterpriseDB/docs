...
originalFilePath: >-
  https://github.com/warehouse-pg/warehouse-pg/gpdb-doc/markdown/best_practices/ha.html.md
...
Finally, through native API integration, WarehousePG can stream backups directly to the Dell EMC Data Domain enterprise backup platform.
...
-   WarehousePG customers should consider streaming backups to the Dell EMC Data Domain enterprise backup platform.
...
-   [Recovering from Segment Failures](../admin_guide/ha/recovering-from-segment-failures.mdx)

*WarehousePG Utility Guide*:

-   [gpstate](../utility_guide/ref/gpstate.mdx) - view state of the WarehousePG cluster
-   [gprecoverseg](../utility_guide/ref/gprecoverseg.mdx) - recover a failed segment
-   [gpactivatestandby](../utility_guide/ref/gpactivatestandby.mdx) - make the standby coordinator the active coordinator
...
Segment mirroring allows database queries to fail over to a backup segment if the primary segment fails or becomes unavailable. VMware requires mirroring for supported production WarehousePG clusters.
...
-   **Group mirroring** — Each host mirrors another host's primary segments. This is the default for [gpinitsystem](../utility_guide/ref/gpinitsystem.mdx) and [gpaddmirrors](../utility_guide/ref/gpaddmirrors.mdx).
...
You can design a custom mirroring configuration and use the WarehousePG `gpaddmirrors` or [gpmovemirrors](../utility_guide/ref/gpmovemirrors.mdx) utilities to set up the configuration.
