...
Override a specific choice the planner makes for a query, such as which scan method, join method, or join order to use, with an optimizer hint. WarehousePG reads hints through the [`pg_hint_plan`](/supported-open-source/warehousepg/extensions/bundled/pg_hint_plan.mdx) extension, placed in a SQL comment before the query.
...
See [About ORCA](query-piv-optimizer) for the two optimizers WarehousePG can use, and [Determining the Query Optimizer that is Used](query-piv-optimizer/query-piv-opt-fallback.mdx) if you need to confirm which one generated a plan. See [pg_hint_plan](/supported-open-source/warehousepg/extensions/bundled/pg_hint_plan.mdx) for how to load the extension and the configuration parameters that control it.
...
Load `pg_hint_plan` before writing a hint. See [Loading the extension](/supported-open-source/warehousepg/extensions/bundled/pg_hint_plan.mdx#loading-the-extension) for how to load it in a session, or configure it to load automatically for a database or user.
