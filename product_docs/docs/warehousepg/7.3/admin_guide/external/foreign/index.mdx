...
navigation:
  - devel-fdw
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/admin_guide/external/foreign/index.md
...
The WarehousePG distribution includes the [postgres_fdw](/supported-open-source/warehousepg/extensions/bundled/postgres_fdw.mdx) foreign data wrapper. If you use PXF to access external data, the [pxf_fdw](/pxf/6/foreign-data-wrapper.mdx) foreign data wrapper lets you query those same PXF-connected sources through standard foreign tables instead of `pxf://` external tables.
You can also download and install the EDB-provided module [whpg_fdw](/supported-open-source/warehousepg/extensions/additional/whpg_fdw).
