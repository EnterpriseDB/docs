...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/install_guide/config_os.md
...
1.  Make sure your host systems meet the requirements described in [On-Premise Hardware Requirements](platform-requirements.mdx#hardware-requirements).
...
> **Note** Automating the configuration steps described in this topic and [Installing WarehousePG](install_whpg.mdx) with a system provisioning tool, such as Ansible, Chef, or Puppet, can save time and ensure a reliable and repeatable WarehousePG installation.
...
For `vm.overcommit_ratio` tuning and calculation recommendations with resource group-based resource management or resource queue-based resource management, refer to [Options for Configuring Segment Host Memory](../admin_guide/performance/wlmgmt_intro.mdx) in the *WarehousePG Administrator Guide*. Use the [WarehousePG memory calculator](../admin_guide/performance/mem_calc.mdx) to calculate a recommended `vm.overcommit_ratio` value for a host based on its physical memory, swap space, and segment count.
...
For additional requirements and recommendations for cloud deployments, see [Public Cloud Requirements](platform-requirements.mdx#public-cloud).
...
-   Disk I/O scheduler
    ...
    | :--------------------------------- | :----------------------------- | :--------------------------- |
    | Non-Volatile Memory Express (NVMe) | RHEL 7<br />RHEL 8<br />RHEL 9 | `none`                       |
    | Solid-State Drives (SSD)           | RHEL 7                         | `noop`                       |
    | Solid-State Drives (SSD)           | RHEL 8<br />RHEL 9             | `none`                       |
    | Other                              | RHEL 7                         | `deadline`                   |
    | Other                              | RHEL 8<br />RHEL 9             | `mq-deadline`                |
...
Deactivate Transparent Huge Pages (THP) as it degrades WarehousePG performance. On systems that use `grub2`, use the system utility `grubby`. This command adds the parameter when run as root.
...
You can optionally give the `gpadmin` user sudo privilege, so that you can administer all hosts in the WarehousePG cluster as `gpadmin` using the `sudo`, `ssh/rsync`, and `gpssh/gpsync` commands.
...
3.  Grant sudo access to the `gpadmin` user.
    ...
    This step is optional, but it's helpful, and it simplifies some procedures elsewhere in this guide, such as [Creating the Data Storage Areas](create_data_dirs.mdx), that run `sudo` commands on remote hosts through `gpssh`. If `gpadmin` doesn't have sudo access, you can't use `gpssh` or `gpsync` to run those commands. Because `gpssh` provides no terminal for a password prompt, the grant must let `gpadmin` run `sudo` without a password (`NOPASSWD`), as shown below, for those procedures to work.
    !!!
