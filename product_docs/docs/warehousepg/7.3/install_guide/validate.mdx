...
title: Validating the WHPG Cluster
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/install_guide/validate.md
...
WarehousePG provides a management utility called [gpcheckperf](../ref_guide/utility_guide/reference/gpcheckperf.mdx), which can be used to identify hardware and system-level issues on the machines in your WarehousePG cluster. [gpcheckperf](../ref_guide/utility_guide/reference/gpcheckperf.mdx) starts a session on the specified hosts and runs the following performance tests:

-   Network Performance (`gpnetbench*`)
-   Disk I/O Performance (`dd` test)
-   Memory Bandwidth (`stream` test)

Before using `gpcheckperf`, you must have a trusted host setup between the hosts involved in the performance test. You can use the utility [gpssh-exkeys](../ref_guide/utility_guide/reference/gpssh-exkeys.mdx) to update the known host files and exchange public keys between hosts if you have not done so already. Note that `gpcheckperf` calls to [gpssh](../ref_guide/utility_guide/reference/gpssh.mdx) and [gpsync](../ref_guide/utility_guide/reference/gpsync.mdx), so these WarehousePG utilities must be in your `$PATH`.

-   **[Validating Network Performance](validate.mdx)**  

-   **[Validating Disk I/O and Memory Bandwidth](validate.mdx)**  
...
You would create four distinct host files for use with the [gpcheckperf](../ref_guide/utility_guide/reference/gpcheckperf.mdx) network test:
...
You would then run [gpcheckperf](../ref_guide/utility_guide/reference/gpcheckperf.mdx) once per subnet. For example (if testing an *even* number of hosts, run in parallel pairs test mode):
