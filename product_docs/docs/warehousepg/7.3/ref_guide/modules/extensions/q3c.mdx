...
title: q3c
displayBanner: ''
description: Learn how to use the q3c extension for spherical indexing of astronomical catalogs in WarehousePG.
---

The `q3c` module provides spherical indexing for large astronomical catalogs in WarehousePG (WHPG). It supports fast circular, elliptical, and polygonal searches on the sky, as well as fast cross-matches between catalogs, using the Q3C (Quad Tree Cube) scheme.
...
The WarehousePG `q3c` module is equivalent to version 2.0.3 of the [q3c extension](https://github.com/segasai/q3c) used with PostgreSQL. No WarehousePG or massively parallel processing (MPP)-specific considerations apply to the module.
...
## Preparing a table for q3c

Before running `q3c` searches, prepare tables with `ra` and `dec` columns (right ascension and declination, in degrees). For example:
...
Use `q3c` to run cone searches, cross-match catalogs, and account for proper motion in `table1` and `table2` from [Preparing a table for q3c](#preparing-a-table-for-q3c).
...
To upgrade `q3c`, download and install the new `edb-whpg7-q3c` package, then update the extension in each database where it's registered:
...
The `q3c` module installs a set of functions, all prefixed with `q3c_`, including index-aware search and cross-match functions, distance and coordinate-conversion functions, and `q3c_version()`. Refer to the [q3c functions reference](https://github.com/segasai/q3c#q3c-functions) on GitHub for the full list of functions and their arguments.
