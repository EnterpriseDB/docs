...
title: postgres_fdw
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/ref_guide/modules/bundled/postgres_fdw.md
redirects:
  - ../postgres_fdw
description: >-
  The postgresfdw module is a foreign-data wrapper (FDW) that you can use to
  access data stored in a remote PostgreSQL or WarehousePG.
...
Before you [create a foreign table](../../sql_commands/CREATE_FOREIGN_TABLE.mdx) using the `postgres_fdw` foreign-data wrapper, you must configure a server with the [CREATE SERVER](../../sql_commands/CREATE_SERVER.mdx) command. You can use a `postgres_fdw` foreign table to access data that is distributed across multiple remote PostgreSQL servers when you set certain WarehousePG and `postgres_fdw`-specific options on the `CREATE SERVER` command:
...
For more information about using foreign tables in WarehousePG, see [Accessing External Data with Foreign Tables](../../../admin_guide/external/foreign).
...
The `postgres_fdw` foreign-data wrapper is included in the WarehousePG open source github repository. You can view the [source code](https://github.com/greenplum-db/gpdb/tree/main/contrib/postgres_fdw) for the module which is located in the `contrib/` directory.
