...
title: pg_dump
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/ref_guide/utility_guide/reference/pg_dump.md
...
-   c \| custom — Output a custom archive suitable for input into [pg_restore](pg_restore.mdx). Together with the directory output format, this is the most flexible output format in that it allows manual selection and reordering of archived items during restore. This format is compressed by default.
...
Use the provided string as the `psql` `\restrict` key in the dump output. You can only specify this for plain-text dumps, meaning when `--format` is set to `plain` or omitted. If you don't specify a restrict key, `pg_dump` generates a random one. Keys can contain only alphanumeric characters.
