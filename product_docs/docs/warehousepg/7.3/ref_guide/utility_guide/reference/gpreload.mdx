...
title: gpreload
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/ref_guide/utility_guide/reference/gpreload.md
editTarget: originalFilePath

---

Reloads WarehousePG table data sorting the data based on specified columns.

> **Note** The `gpreload` utility is deprecated and will be removed in the next major release of WarehousePG. Use [ALTER TABLE REPACK BY COLUMNS](../../sql_commands/ALTER_TABLE.mdx) to physically reorder a table based on one or more columns. 
...
\-p port
The WarehousePG coordinator port. If not specified, the value of the `PGPORT` environment variable is used. If the value is not available, an error is returned.

{-t \| --table-file } path_to_file
The location and name of file containing list of schema qualified table names to reload and the column names to reorder from the WarehousePG. Only user defined tables are supported. Views or system catalog tables are not supported.
...
`CREATE TABLE` in the *WarehousePG Reference Guide*
