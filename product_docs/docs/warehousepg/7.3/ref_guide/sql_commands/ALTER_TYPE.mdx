...
title: ALTER TYPE
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/ref_guide/sql_commands/ALTER_TYPE.md
...
**`ADD ATTRIBUTE`**
Adds a new attribute to a composite type, using the same syntax as [CREATE TYPE](CREATE_TYPE.mdx).
...
> **Note** WarehousePG does not support adding storage options for row or composite types.
...
The variants to add and drop attributes are part of the SQL standard; the other variants are WarehousePG extensions.

<a id="section7"></a>

## See Also

[CREATE TYPE](CREATE_TYPE.mdx), [DROP TYPE](DROP_TYPE.mdx)
