...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/pxf/gphdfs-pxf-migrate.md
...
> **Note:** **Note:** If you are migrating `gphdfs` from a WarehousePG 5 installation, you perform the migration in the order above in your WarehousePG 5 cluster before you migrate data to WarehousePG 6.

> **Note:** **Note:** If you are migrating `gphdfs` from a WarehousePG 4 installation, you perform the migration in a similar order. However, since PXF is not available in WarehousePG 4, you must perform certain actions in the WarehousePG 6 installation before you migrate the data:
>
> 1.  WarehousePG 4: [Prepare](#prepare) for the migration.
>
> 2.  WarehousePG 6:
>
>     1.  Install and configure the WarehousePG 6 software.
>     2.  Map configuration properties, and then [install and set up PXF](#id_cfg_prop).
>     3.  [Create](#create_pxf_ext_table) a new `pxf` external table to replace each `gphdfs` external table.
>     4.  [Verify](#verify_pxf) access to Hadoop files with the `pxf` external tables.
>
> 3.  WarehousePG 4:
...
>
> 4.  Migrate WarehousePG 4 data to WarehousePG 6.
...
2.  If you are migrating from WarehousePG 4, *or* you have not previously used PXF in your WarehousePG 5 installation:
    ...
    2.  Inform PXF of the $JAVA_HOME setting by specifying its value in the `pxf-env.sh` configuration file.
        ...
        -   Edit the `pxf-env.sh` file on the WarehousePG coordinator node.
            ...
            ```shell
            gpadmin@gpcoordinator$ vi /usr/local/edb-whpg<whpg_major_version>-pxf/conf/pxf-env.sh
            ```
    ...
    3.  Register the PXF extension with WarehousePG:

        ```shell
        gpadmin@gpcoordinator$ pxf cluster register
        ```

    4.  Enable the PXF extension and grant users access to PXF.
...
<a id="map_location" class="no-quick-link"></a>
...
<sup>1</sup> Avro format `deflate` codec only.
...
<a id="map_format" class="no-quick-link"></a>
...
For text data, the `FORMAT` clause may identify a delimiter or other formatting option as described on the [CREATE EXTERNAL TABLE](../ref_guide/sql_commands/CREATE_EXTERNAL_TABLE.mdx) command reference page.

<a id="text_example" class="no-quick-link"></a>
...
<a id="remove_gphdfs_ext_table"></a>
