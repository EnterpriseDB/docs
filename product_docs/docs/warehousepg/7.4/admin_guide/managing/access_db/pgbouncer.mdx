...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/admin_guide/managing/access_db/pgbouncer.md
...
-   **If you are migrating to a WarehousePG version 5.8.x or earlier**, you can migrate PgBouncer without dropping connections. Launch the new PgBouncer process with the `-R` option and the configuration file that you started the process with:
...
Refer to the WarehousePG [LDAP Authentication](../../../security_guide/authentication.mdx#ldap_auth) discussion for more information on configuring an HBA file for LDAP.
