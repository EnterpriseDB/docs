...
navigation:
  - about-resgroups-changes
  - workload_mgmt_resgroups
  - workload_mgmt
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/admin_guide/performance/wlmgmt/index.md
...
You can use resource management features to limit the number of concurrent queries, the amount of memory used to run a query, and the relative amount of CPU devoted to processing a query. WarehousePG provides two schemes to manage resources: [Resource Queues](workload_mgmt.mdx) and [Resource Groups](workload_mgmt_resgroups.mdx).

Either the resource queue or the resource group management scheme can be active in WarehousePG; both schemes cannot be active at the same time. Set the server configuration parameter [gp_resource_manager](../../../ref_guide/config_params/guc-list.mdx#gp_resource_manager) to enable the preferred scheme in the WarehousePG cluster.
...
| External Components | None                                                                                                                    | Manage external component CPU resources                                                                                                                                                 |

-   **[About Changes to Resource Groups in WarehousePG 7](about-resgroups-changes.mdx)**  
    Compares resource group-based resource management between WarehousePG 6 and WarehousePG 7.
-   **[Using Resource Groups](workload_mgmt_resgroups.mdx)**  
    Configuring and using resource groups to manage CPU, memory, concurrency, and disk I/O.
-   **[Using Resource Queues](workload_mgmt.mdx)**  
