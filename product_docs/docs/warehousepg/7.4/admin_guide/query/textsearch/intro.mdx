...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/admin_guide/query/textsearch/intro.md
redirects:
  - ../analytics/text
...
WarehousePG provides a data type `tsvector` to store preprocessed documents, and a data type `tsquery` to store processed queries ([Text Search Data Types](../../../ref_guide/data_types/datatype-textsearch.mdx)). There are many functions and operators available for these data types ([Text Search Functions and Operators](../../../ref_guide/function-summary.mdx)), the most important of which is the match operator `@@`, which we introduce in [Basic Text Matching](#basic-text-matching). Full text searches can be accelerated using indexes ([GiST and GIN Indexes for Text Search](gist-gin.mdx)).
