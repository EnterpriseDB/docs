...
| Deep learning (CPU) | tensorflow-cpu, keras, tf-keras, torch, torchvision, torchaudio |
...
1.  From the coordinator, download the package from the EDB repository:
    ...
    sudo dnf download edb-whpg7-data-science-python3.11
    ```
    ...
    export EDB_REPO=gpsupp
    curl -1sSLf "https://downloads.enterprisedb.com/$EDB_SUBSCRIPTION_TOKEN/$EDB_REPO/setup.rpm.sh" | sudo -E bash
    sudo yumdownloader edb-whpg7-data-science-python3.11
    ```
...
```bash
gpssh -f all_hosts -e 'sudo dnf remove -y edb-whpg7-data-science-python3.11'
gpstop -r
```
...
| tf-keras | Keras 2 compatibility layer required by transformers and related libraries |
