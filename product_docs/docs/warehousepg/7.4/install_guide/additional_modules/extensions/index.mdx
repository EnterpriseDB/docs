...
Extensions require a separate package install before you can enable them with `CREATE EXTENSION`. Depending on the extension, you may need to install the package on all hosts or only on the coordinator. See the individual extension's [reference page](/supported-open-source/warehousepg/extensions/additional) for details.
