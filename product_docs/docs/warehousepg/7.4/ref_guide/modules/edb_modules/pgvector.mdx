...
title: pgvector
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/docs/7x/ref_guide/modules/pgvector/pgvector.md
redirects:
...
The WarehousePG pgvector module is equivalent to version 0.7.4. of the pgvector module used with PostgreSQL. See [Limitations](#limits) to understand the limitations of the WarehousePG version of the module.
...
Use [partitioning](/supported-open-source/warehousepg/warehousepg/admin_guide/ddl/ddl-partition/) to perform an approximate search on many different values of the `WHERE` columns:
...
Use pgvector together with [full-text search](/supported-open-source/warehousepg/warehousepg/admin_guide/textsearch/) for a hybrid search:
...
-   Vector indexes are not supported on Append-optimized (AO) tables.
