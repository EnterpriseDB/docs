...
The WarehousePG `whpg-q3c` module is equivalent to version 2.0.3 of the [q3c extension](https://github.com/segasai/q3c) used with PostgreSQL. No WarehousePG or massively parallel processing (MPP)-specific considerations apply to the module.
...
To upgrade `whpg-q3c`, download and install the new `edb-whpg7-q3c` package, then update the extension in each database where it's registered:
