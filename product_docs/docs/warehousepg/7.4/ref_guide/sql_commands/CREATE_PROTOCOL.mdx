...
For information about creating and enabling a custom data access protocol, refer to the [Example Custom Data Access Protocol](../../admin_guide/load/example-custom-data-access-protocol.mdx) documentation.
...
WarehousePG handles external tables of type `file`, `gpfdist`, and `gpfdists` internally. See [s3:// Protocol](../../admin_guide/external/s3-protocol.mdx) for information about enabling the `S3` protocol. Refer to [pxf:// Protocol](../../admin_guide/external/pxf-protocol.mdx) for information about using the `pxf` protocol.
