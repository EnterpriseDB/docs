...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/ref_guide/sql_commands/CREATE_PROTOCOL.md
...
For information about creating and enabling a custom data access protocol, refer to the [Example Custom Data Access Protocol](../../admin_guide/load/example-custom-data-access-protocol) documentation.
...
WarehousePG handles external tables of type `file`, `gpfdist`, and `gpfdists` internally. See [s3:// Protocol](../../admin_guide/external/external-tables/s3-protocol.mdx) for information about enabling the `S3` protocol. Refer to [PXF for WarehousePG](/pxf/6x/) for information about using the `pxf` protocol.
...
[ALTER PROTOCOL](ALTER_PROTOCOL.mdx), [CREATE EXTERNAL TABLE](CREATE_EXTERNAL_TABLE.mdx), [DROP PROTOCOL](DROP_PROTOCOL.mdx), [GRANT](GRANT.mdx)
