...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/ref_guide/config_params/guc-list.md
...
<a id="autovacuum_analyze_scale_factor"></a>
...
<a id="autovacuum_analyze_threshold"></a>
...
<a id="autovacuum_freeze_max_age"></a>
...
<a id="autovacuum_multixact_freeze_max_age"></a>
...
<a id="autovacuum_vacuum_cost_delay"></a>
...
<a id="autovacuum_vacuum_cost_limit"></a>
...
<a id="autovacuum_vacuum_scale_factor"></a>
...
<a id="autovacuum_vacuum_threshold"></a>
...
<a id="checkpoint_completion_target"></a>
...
<a id="debug_shareinput_xslice"></a>
...
<a id="default_table_access_method"></a>
...
<a id="gp_interconnect_address_type"></a>
...
<a id="gp_max_parallel_cursors"></a>
...
<a id="gp_postmaster_address_family"></a>
...
<a id="gp_print_create_gang_time"></a>
...
<a id="gp_resgroup_memory_query_fixed_mem"></a>
...
<a id="gp_resource_group_bypass_catalog_query"></a>
...
<a id="gp_resource_group_bypass_direct_dispatch"></a>
...
<a id="gp_resource_group_cgroup_parent"></a>
...
<a id="gp_resource_group_cpu_priority"></a>
...
<a id="gp_resource_group_move_timeout"></a>
...
Identifies the resource management scheme currently enabled in the WarehousePG cluster. For information about WarehousePG resource management, see [Managing Resources](../../admin_guide/performance/wlmgmt).
...
<a id="jit_optimize_above_cost"></a>
...
For information about the Postgres-based planner and GPORCA, see [SQL: Querying Data](../../admin_guide/query) in the *WarehousePG Administrator Guide*.
...
<a id="optimizer_discard_redistribute_hashjoin"></a>
...
<a id="optimizer_enable_dynamicindexonlyscan"></a>
...
<a id="optimizer_enable_foreign_table"></a>
...
<a id="optimizer_enable_coordinator_only_queries"></a>
...
<a id="optimizer_enable_push_join_below_union_all"></a>
...
<a id="optimizer_enable_replicated_table"></a>
...
<a id="optimizer_enable_right_outer_join"></a>
...
For information about GPORCA, see the [About GPORCA](../../admin_guide/query/query-piv-optimizer) topic.
...
<a id="optimizer_jit_above_cost"></a>
...
<a id="optimizer_jit_inline_above_cost"></a>
...
<a id="optimizer_jit_optimize_above_cost"></a>
...
<a id="optimizer_penalize_broadcast_threshold"></a>
...
<a id="ssl_max_protocol_version"></a>

## ssl_max_protocol_version

Sets the maximum TLS protocol version that WarehousePG accepts for secure connections. An empty string, the default, allows any protocol version up to the most recent version supported by the OpenSSL library used to build WarehousePG.

| Value Range                                          | Default | Set Classifications          |
| ---------------------------------------------------- | ------- | ---------------------------- |
| TLSv1, TLSv1.1, TLSv1.2, TLSv1.3, or an empty string |         | coordinator, system, restart |

<a id="ssl_min_protocol_version"></a>

## ssl_min_protocol_version

Sets the minimum TLS protocol version that WarehousePG accepts for secure connections. WarehousePG rejects connections that negotiate an earlier protocol version. See [Requiring a Minimum TLS Version](../../admin_guide/manage_access/client_auth#topic_ssl_min_version) in the *WarehousePG Administrator Guide* for an example that requires TLS 1.3.

!!!info Note

This parameter is not available in WarehousePG 6.
!!!
...
For the current read-only serializable transaction, specifies whether to defer until it can be executed with no possible serialization failures.

| Value Range | Default                                   | Set Classifications          |
| ----------- | ----------------------------------------- | ---------------------------- |
| Boolean     | value of `default_transaction_deferrable` | coordinator, session, reload |
...
<a id="vacuum_cleanup_index_scale_factor"></a>
...
For information about the `gpfdists` protocol and how this setting affects the certificate files required on WarehousePG segments, see [gpfdists:// Protocol](../../admin_guide/external/external-tables/gpfdists-protocol.mdx). For information about running the `gpfdist` utility, see [gpfdist](../utility_guide/reference/gpfdist.mdx).
...
<a id="whpg_dispatch_topology_file"></a>

## whpg_dispatch_topology_file

The path of the dispatch topology file. An empty value, the default, disables it.

This parameter serves hot-standby dispatchers only. When set, dispatch resolves every content's address from this file instead of selecting a row from [gp_segment_configuration](../system_catalogs/system_catalogs_definitions/gp_segment_configuration.mdx), with fail-closed validation and a `dbid` handshake. On any other node, the component-table build refuses to start. Each row in the file must carry the node's own identity, the `gp_dbid` value from `internal.auto.conf`. A relative path is relative to the data directory.

The topology file's state is exposed through the read-only [whpg_dispatch_topology_state](#whpg_dispatch_topology_state) parameter.
...
Shows the state of the [whpg_dispatch_topology_file](#whpg_dispatch_topology_file) parameter.

| Value Range             | Default  | Set Classifications |
| ----------------------- | -------- | ------------------- |
| inactive, active, error | inactive | read only           |
