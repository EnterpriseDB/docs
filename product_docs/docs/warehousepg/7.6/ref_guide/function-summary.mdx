...
> **Note** The [hstore](/supported-open-source/warehousepg/extensions/bundled/hstore.mdx) extension has a cast from `hstore` to `json`, so that `hstore` values converted via the JSON creation functions will be represented as JSON objects, not as primitive string values.
