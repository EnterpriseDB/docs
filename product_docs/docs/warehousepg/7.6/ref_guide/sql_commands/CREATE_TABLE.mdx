...
title: CREATE TABLE
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/ref_guide/sql_commands/CREATE_TABLE.md
...
> **Note** WarehousePG supports both *classic* and *modern* partitioning syntaxes. Refer to [Choosing the Partitioning Syntax](../../admin_guide/ddl/ddl-partition#choose) for more information, including guidance on selecting the appropriate syntax for a partitioned table.
...
data_type
The data type of the column. This may include array specifiers. For more information on the data types supported by WarehousePG, refer to the [Data Types](../data_types) documentation.
...
For more information about using column compression, refer to [Adding Column-Level Compression](../../admin_guide/ddl/ddl-storage.mdx#topic43).
...
Refer to [Partitioning Large Tables](../../admin_guide/ddl/ddl-partition) for further discussion on table partitioning.
...
The clauses `ALWAYS` and `BY DEFAULT` determine how the sequence value is given precedence over a user-specified value in an `INSERT` statement. If `ALWAYS` is specified, a user-specified value is only accepted if the `INSERT` statement specifies `OVERRIDING SYSTEM VALUE`. If `BY DEFAULT` is specified, then the user-specified value takes precedence. See [INSERT](INSERT.mdx) for details. (In the `COPY` command, ueser-specified values are always used regardless of this setting.)
You can use the optional sequence_options clause to override the options of the sequence. See [CREATE SEQUENCE](CREATE_SEQUENCE.mdx) for details.
...
**DELETE ROWS** - All rows in the temporary table will be deleted at the end of each transaction block. Essentially, WarehousePG performs an automatic [TRUNCATE](TRUNCATE.mdx) at each commit. When used on a partitioned table, this operation is not cascaded to its partitions.
...
TABLESPACE tablespace
The name of the tablespace in which the new table is to be created. If not specified, the database's  [default_tablespace](../config_params/guc-list.mdx#default_tablespace) is consulted, or [temp_tablespaces](../config_params/guc-list.mdx) if the table is temporary. For partitioned tables, since no storage is required for the table itself, the tablespace specified overrides `default_tablespace` as the default tablespace to use for any newly created partitions when no other tablespace is explicitly specified.

USING INDEX TABLESPACE tablespace
This clause allows selection of the tablespace in which the index associated with a `UNIQUE`, `PRIMARY KEY`, or `EXCLUDE` constraint will be created. If not specified, the database's  [default_tablespace](../config_params/guc-list.mdx#default_tablespace) is used, or [temp_tablespaces](../config_params/guc-list.mdx) if the table is temporary.
...
You can specify the defaults for some of the table storage options with the server configuration parameter [gp_default_storage_options](../config_params/guc-list.mdx#gp_default_storage_options). For information about setting default storage options, see [Notes](#section5).
...
Per-table value for the [autovacuum_vacuum_threshold](../config_params/guc-list.mdx#autovacuum_vacuum_threshold) server configuration parameter.

> **Note** WarehousePG accepts, but does not apply, values for these storage parameters.

autovacuum_vacuum_scale_factor, toast.autovacuum_vacuum_scale_factor (floating point)
Per-table value for the [autovacuum_vacuum_scale_factor](../config_params/guc-list.mdx#autovacuum_vacuum_scale_factor) server configuration parameter.
...
Per-table value for the [vacuum_freeze_min_age](../config_params/guc-list.mdx#vacuum_freeze_min_age) parameter. Note that autovacuum will ignore per-table `autovacuum_freeze_min_age` parameters that are larger than half of the system-wide `autovacuum_freeze_max_age` setting.

> **Note** WarehousePG accepts, but does not apply, values for these storage parameters.

autovacuum_freeze_max_age, toast.autovacuum_freeze_max_age (integer)
Per-table value for the [autovacuum_freeze_max_age](../config_params/guc-list.mdx#autovacuum_freeze_max_age) server configuration parameter. Note that autovacuum will ignore per-table `autovacuum_freeze_max_age` parameters that are larger than the system-wide setting (it can only be set smaller).
...
WarehousePG automatically creates an index for each unique constraint and primary key constraint to enforce uniqueness, so it is not necessary to create an index explicitly for primary key columns. (See [CREATE INDEX](CREATE_INDEX.mdx) for more information.)
...
Create an append-optimized table and add a unique index to it:
...
Define a primary key constraint inline on a row-oriented append-optimized table:
...
Define a primary key table constraint on a column-oriented append-optimized table:
...
Define a unique constraint inline on an append-optimized table:
...
Add a unique constraint to an existing append-optimized table:
...
[ALTER TABLE](ALTER_TABLE.mdx), [DROP TABLE](DROP_TABLE.mdx), [CREATE EXTERNAL TABLE](CREATE_EXTERNAL_TABLE.mdx), [CREATE TABLE AS](CREATE_TABLE_AS.mdx), [CREATE TABLESPACE](CREATE_TABLESPACE.mdx), [CREATE TYPE](CREATE_TYPE.mdx)
