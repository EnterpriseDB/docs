...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/ref_guide/sql_commands/CREATE_OPERATOR.md
...
-   A multicharacter operator name cannot end in `+` or `-`, unless the name also contains at least one of these characters: <code>~ ! @ # % ^ & | \` ?</code>
    ...
    For example, `@-` is an allowed operator name, but `*-` is not. This restriction allows WarehousePG to parse SQL-compliant commands without requiring spaces between tokens.
...
**Note:**  Right unary, also called postfix, operators are deprecated and may be removed in a future WarehousePG release.
...
You must be a superuser to specify a `RESTRICT` or `JOIN` estimator function that isn't one of the built-in selectivity estimators.
...
Use [DROP OPERATOR](DROP_OPERATOR.mdx) to delete user-defined operators from a database. Use [ALTER OPERATOR](ALTER_OPERATOR.mdx) to modify operators in a database.
...
`CREATE OPERATOR` is a WarehousePG extension to the SQL standard. The SQL standard does not provide for user-defined operators.

<a id="section8"></a>

## See Also

[CREATE FUNCTION](CREATE_FUNCTION.mdx), [CREATE TYPE](CREATE_TYPE.mdx), [ALTER OPERATOR](ALTER_OPERATOR.mdx), [CREATE OPERATOR CLASS](CREATE_OPERATOR_CLASS.mdx), [DROP OPERATOR](DROP_OPERATOR.mdx)
