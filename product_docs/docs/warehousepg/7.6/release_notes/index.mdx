...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/release_notes/index.md
...
| ----------------------------------- | ---------------- |
| [7.6.0-WHPG](#warehousepg-760-whpg) | 25 September 2026 |
...
| [7.3.2-WHPG](#warehousepg-732-whpg)    | 11 June 2026  |
...
This issue affects every WarehousePG 7 release before `7.6.0-WHPG`. WarehousePG 6 isn't affected. Upgrade to `7.6.0-WHPG` as soon as possible. See [Applying the file function privilege fix](../install_guide/minor_upgrade.mdx#applying-the-file-function-privilege-fix) for the required follow-up step and a mitigation script if you can't upgrade right away.
!!!

!!!info Note
WarehousePG 7.6.0 also changes some behaviors that need action when you upgrade, including dump output format, resource group `CPUSET` parsing, and `pgcrypto` and `psql` behavior. Review [Replacing binaries and restarting WHPG](../install_guide/minor_upgrade.mdx#replacing-binaries-and-restarting-whpg) before you upgrade.
!!!

### New features

-   Added support for [`CREATE TABLE ... AS ... DISTRIBUTED COORDINATOR ONLY`](../ref_guide/sql_commands/CREATE_TABLE_AS.mdx), giving `CREATE TABLE AS` a native coordinator-only path.
-   Added the [`whpg_dispatch_topology_file`](../ref_guide/config_params/guc-list.mdx#whpg_dispatch_topology_file) configuration parameter, along with the read-only [`whpg_dispatch_topology_state`](../ref_guide/config_params/guc-list.mdx#whpg_dispatch_topology_state), letting a hot-standby dispatcher resolve dispatch and interconnect addresses from a topology file instead of [`gp_segment_configuration`](../ref_guide/system_catalogs/system_catalogs_definitions/gp_segment_configuration.mdx).
-   Added PowerPC ppc64le support for WarehousePG 7. See [Platform Requirements](../install_guide/platform-requirements.mdx).
-   Added resource groups to the [`pg_stat_operations`](../ref_guide/system_catalogs/system_catalogs_definitions/pg_stat_operations.mdx) system view.
...
-   Fixed `pgcrypto` to fail cipher initialization errors during PGP encryption instead of emitting unencrypted data, and added the `ignore-cipher-failure=1` decryption option, passed in the `options` argument of `pgp_sym_decrypt()` and `pgp_pub_decrypt()`, to read back affected data, for [CVE-2026-14663](https://www.cve.org/CVERecord?id=CVE-2026-14663).
