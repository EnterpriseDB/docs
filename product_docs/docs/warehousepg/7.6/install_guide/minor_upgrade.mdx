...
originalFilePath: >-
  https://github.com/warehouse-pg/whpg-docs/blob/main/docs/whpg/7x/install_guide/minor_upgrade.md
...
-   Plain-text output from [`pg_dump`](../ref_guide/utility_guide/reference/pg_dump.mdx), [`pg_dumpall`](../ref_guide/utility_guide/reference/pg_dumpall.mdx), and [`pg_restore`](../ref_guide/utility_guide/reference/pg_restore.mdx) now begins with `\restrict <key>` and ends with `\unrestrict <key>`. Tooling that parses dump output needs adjusting for the new markers. Both the dumper and the `psql` used to restore the dump need to be on this version or later for the protection to apply. A dump from 7.6.0 restored with an older `psql` gets `invalid command \restrict` on the first line, and stops there if `ON_ERROR_STOP` is set.
-   [`CREATE OPERATOR`](../ref_guide/sql_commands/CREATE_OPERATOR.mdx) and [`ALTER OPERATOR`](../ref_guide/sql_commands/ALTER_OPERATOR.mdx) now require superuser privileges to attach a non-built-in selectivity estimator.
-   SQL-level calls to functions that take or return the `internal` type, and casts to and from `internal`, are rejected.
-   [`psql`](../ref_guide/utility_guide/reference/psql.mdx) now consumes in-line [`COPY ... FROM STDIN`](../ref_guide/sql_commands/COPY.mdx) data even when the `COPY` command fails. Scripts must terminate in-line data with `\.`.
-   `pgcrypto` [PGP encryption](../security_guide/encryption.mdx#encpgp) now returns an error when cipher initialization fails, instead of writing unencrypted data. Use the new `ignore-cipher-failure=1` decryption option, passed in the `options` argument of `pgp_sym_decrypt()` and `pgp_pub_decrypt()`, to read back data written by an affected build.
-   The resource group [`CPUSET`](../admin_guide/performance/wlmgmt/workload_mgmt_resgroups.mdx#cpuset) setting now parses in the documented order, `<coordinator_cores>;<segment_cores>`. Clusters that swapped the two halves of the value to compensate for the previous reversed behavior need to swap them back.
-   [`gp_toolkit`](../ref_guide/gp_toolkit.mdx) is now version 1.9. Run `ALTER EXTENSION gp_toolkit UPDATE` to pick up the corrected [`gp_toolkit.gp_workfile_entries`](../ref_guide/gp_toolkit.mdx#topic33) view.
...
6.  Install WHPG `7.target` on segments: From the coordinator, run:
    ...
    -   For RHEL 8, RHEL 9:
        ...
        gpssh -f hostfile_without_coord -e "sudo chown -R gpadmin:gpadmin /usr/local/greenplum-db
        ```
