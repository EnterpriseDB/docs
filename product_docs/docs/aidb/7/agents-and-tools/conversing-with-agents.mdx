---
title: Conversing with agents
navTitle: Conversing with agents
...
An agent's `input_token_budget`, `output_token_budget`, `max_iterations`, and `timeout` (all set on [`create_agent`/`update_agent`](creating-agents#budgets-iterations-and-timeouts)) cap one `agent_converse` call. `timeout` defaults to 300 seconds if unset. What happens when a budget is exceeded depends on `budget_strategy`:
...
`max_iterations` counts reasoning rounds (one model call plus any tools it calls in that round), not individual tool calls. A hard ceiling of 25 reasoning iterations always applies regardless of configuration, as a backstop against a runaway agent.

## How long context is handled
