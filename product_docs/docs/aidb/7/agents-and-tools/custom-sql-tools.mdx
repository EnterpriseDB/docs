...
A tool's name must be unique across every tool type — it can't collide with a built-in [native tool](tools#native-tools) or a tool already imported from an [MCP server](mcp-tools). `aidb.create_sql_tool()` rejects the call up front if it does.
