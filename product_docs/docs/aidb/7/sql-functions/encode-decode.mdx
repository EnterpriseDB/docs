...
Encodes multiple images into vectors in a single batched request. More efficient than calling `encode_image()` in a loop when processing many images.

```sql
SELECT * FROM aidb.encode_image_batch(
    inputs  => ARRAY[
        pg_read_binary_file('/path/to/image1.png')::BYTEA,
        pg_read_binary_file('/path/to/image2.png')::BYTEA
    ],
    options => '{"model": "my_clip_model"}'
);
...
For the full parameter reference, see [Models reference](../reference/models#aibdencode_image_batch).
