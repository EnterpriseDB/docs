...
Using a provider that doesn't support `decode_text` returns an error immediately — at function call time for `summarize_text()` and at pipeline creation time for pipeline use:
