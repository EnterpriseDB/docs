...
The model must support the `generate_text()` and `generate_text_batch()` operations. The following providers are compatible:

| Provider               | Type              | Description                                                              |
|------------------------|-------------------|---------------------------------------------------------------------------|
| `completions`          | Remote            | For any model that implements the OpenAI completions API.               |
| `openai_completions`   | Remote            | For any completions model on the OpenAI platform.                       |
| `nim_completions`      | Remote            | For any model that implements the OpenAI completions API on NVIDIA NIM.  |
| `openrouter_chat`      | Remote            | For any chat model available through the OpenRouter API.                |
| `gemini`               | Remote            | Google's family of generative models.                                    |
| `t5_local`             | Local (Candle)    | Ideal for translation, summarization, and question answering.           |
| `llama_instruct_local` | Local (Candle)    | Instruction variants of Meta's family of large language models.         |
| `llamacpp_generate`    | Local (llama.cpp) | LLM inference via llama.cpp (GGUF format).                              |

Embedding-only providers (`bert_local`, `*_embeddings`, `*_clip`, `*_reranking`) don't support summarization and will return an error if used.
...
- Good for: detailed summaries, meeting notes, and comprehensive reviews
...
Using a provider that doesn't support `generate_text` returns an error immediately — at function call time for `summarize_text()` and at pipeline creation time for pipeline use:
...
| `The requested adapter is not supported by the model provider: X`        | Embedding-only provider used for summarization      | Use `completions`, `openai_completions`, `t5_local`, or another language provider |
| `Invalid parameters: 'desired_length' must be greater than 0`             | Invalid chunk config                                | Use a positive integer for `desired_length`                                       |
| `Invalid parameters: 'max_length' must be greater than or equal to 'desired_length'` | `max_length` is smaller than `desired_length` | Set `max_length >= desired_length`                                                |
