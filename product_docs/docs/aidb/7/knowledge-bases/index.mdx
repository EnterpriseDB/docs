...
Once a pipeline has run, query the knowledge base using the retrieval functions below. Both functions use vector similarity to find results based on meaning rather than exact keywords, and support both `TEXT` and `BYTEA` (image) queries.
