...
description: "SQL patterns for combining vector search with relational filters, native FTS, RRF, and weighted linear fusion, with a worked example."
deepToC: true
redirects:
...
Use standard PostgreSQL `tsvector` / `ts_rank` as the keyword leg. This example assumes a `search_vector tsvector` column on the source table, populated and indexed as described in [Setting up native PostgreSQL FTS](set-up-native-fts).
