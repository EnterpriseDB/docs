...
- [Setting up user-managed BM25](set-up-bm25) — Install extensions, create a tokenizer, add a BM25 column and index to your source table.
- [Setting up native PostgreSQL FTS](set-up-native-fts) — Add a `tsvector` column, GIN index, and trigger to your source table.
- [Query patterns](query-patterns) — SQL patterns for every combination: relational filter, BM25 filter, RRF, weighted linear fusion.
