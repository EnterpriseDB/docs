...
A knowledge base can have more than one pipeline writing into it. Each pipeline keeps its own source, processing chain, and auto-processing mode, but all of them land embeddings in the same vector table. Retrieval queries the KB once and aggregates results across all attached pipelines.
...
## Attach a pipeline to an existing KB

The first pipeline creates the KB; subsequent pipelines attach by writing to the same `destination`. Use [`aidb.knowledge_base_config_from_kb()`](../data-pipelines/reference#aidbknowledge_base_config_from_kb) on the second and later pipelines so they inherit the KB's model, distance operator, and vector index.
...
Retrieval returns rows from both pipelines, with `pipeline_name` identifying the source:
...
Different step chains can share one KB — for example, one pipeline chunks long documents with `ChunkText` before the `KnowledgeBase` step while another embeds short rows directly. Per-pipeline intermediate results are returned in the `intermediate_steps` JSONB column from `aidb.retrieve_text()`.
...
Source tables and source volumes are never touched by these operations.
...
The shared vector table includes a `pipeline_id` column so each row can be traced back to its originating pipeline. The primary key is `(pipeline_id, source_id, part_ids)`.
