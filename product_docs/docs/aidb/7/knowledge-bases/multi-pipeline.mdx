...
Different step chains can share one KB — for example, one pipeline chunks long documents with `ChunkText` before the `KnowledgeBase` step while another embeds short rows directly. The `intermediate_steps` JSONB column from `aidb.retrieve_text()` returns per-pipeline intermediate results, for any step with [intermediate storage](../data-pipelines/pipeline-steps#intermediate-storage) enabled. `deduplicate` (on by default) is scoped per pipeline, not across the whole KB — see [Deduplication and intermediate results](../knowledge-bases#deduplication-and-intermediate-results).
