...
`aidb.pipeline_metrics` and `aidb.knowledge_base_metrics` compute their numbers on demand, at query time. For external monitoring — Prometheus scraping through the beacon agent, for example — polling those views on an interval means recomputing every pipeline's stats on every scrape, and requires the scraping role to reach each pipeline owner's underlying tables.
...
Either way, the change takes effect within a couple of minutes, without a restart. Superusers can set `aidb.enable_background_metrics_collector`; other roles need an explicit `GRANT SET ON PARAMETER aidb.enable_background_metrics_collector` first.
