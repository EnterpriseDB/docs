...
- **Observability**: It provides detailed status of your automated workflows. Since these processes are running in the background, observability allows you to track health, speed, and accuracy.
  ...
  - **[Audit & error logs](./error-log/)**: It captures exactly why a specific file failed to parse, for example, a corrupted PDF or a timeout, allowing for quick troubleshooting. Inspect, clear, and retry from a SQL surface.

To see orchestration applied in a complete pipeline workflow, see [Example](../example).