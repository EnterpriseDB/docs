...
The rendered page images can then pass directly to a `PeformOcr` step for text extraction.
...
**Helper function:** `aidb.pdf_to_image_config()`

| Parameter | Type      | Default  | Description                                    |
|-----------|-----------|----------|------------------------------------------------|
| `dpi`     | `INTEGER` | `NULL`   | Resolution in dots per inch used to render each page. Higher values produce sharper images at the cost of larger output size. Uses a system default when omitted.|

Output images are in PNG format, which is compatible with the `PerformOcr` step using NVIDIA NIM PaddleOCR.
...
`PdfToImage` unnests results — each page of the source PDF produces one output row containing the rendered page as a PNG image. 
