...
To attach a pipeline to an existing KB (so multiple pipelines share one vector table), use `aidb.knowledge_base_config_from_kb(data_format)` instead — it inherits the model, distance operator, and vector index from the KB. See [Multi-pipeline knowledge bases](../knowledge-bases/multi-pipeline).
