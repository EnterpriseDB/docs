...
Reference for agent-related functions, views, and types. For guide-style documentation, see [Agents and tools](../agents-and-tools).
...
| `model`                | TEXT    | Required             | Name of the model backing the agent. See [Choosing a model](../agents-and-tools/creating-agents#choosing-a-model). |
...
| `budget_strategy`      | TEXT    | `'attempt_complete'` | One of `'ignore'`, `'error'`, `'summarize'`, `'attempt_complete'`. See [Budgets and limits](../agents-and-tools/conversing-with-agents#budgets-and-limits). |
...
| `read_only`         | BOOLEAN | NULL     | Force [read-only mode](../agents-and-tools/conversing-with-agents#read-only-mode) on or off. Auto-enabled on a read replica when omitted. |
| `output_type`       | JSONB   | NULL     | Overrides the agent's configured [structured output](../agents-and-tools/creating-agents#structured-output) schema for this call only. |
| `debug`             | BOOLEAN | NULL     | If `true`, also emit every action as a `NOTICE`. See [Debug mode](../agents-and-tools/conversing-with-agents#debug-mode). |
...
See [Budgets and limits](../agents-and-tools/conversing-with-agents#budgets-and-limits) for what each value does.
