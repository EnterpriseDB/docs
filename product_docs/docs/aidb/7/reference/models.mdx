...
|----------|----------------------|
| `aidb.decode_text()`, `aidb.decode_text_batch()` | Yes — pass `aidb.inference_config()` (or raw JSON) as the `inference_config` argument. |
...
Probes a registered model to confirm it's functional, running a minimal test inference. Called automatically by `aidb.create_model()` unless `validate => false`; you can also call it directly to re-check an existing model.
...
`SETOF TEXT` — one row per input prompt, in input order. The result column is named `decode_text_batch`.

#### Example

```sql
SELECT aidb.decode_text_batch('t5_local', ARRAY[
...
!!!note
For agentic use with reliable, native tool calling on OpenAI models, use [`openai_responses`](#aidbopenai_responses_config) instead — see [Tool calling and structured output](#tool-calling-and-structured-output) for how the two providers differ.
!!!
