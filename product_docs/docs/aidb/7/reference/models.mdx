...
| `validate`            | BOOLEAN | `true`   | If `true`, probe the model at creation time to confirm it is functional. See [Validation](#validation). |
...
Probes a registered model to confirm it is functional, running a minimal test inference. Called automatically by `aidb.create_model()` unless `validate => false`; you can also call it directly to re-check an existing model.
