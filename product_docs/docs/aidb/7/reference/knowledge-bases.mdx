...
The legacy view names `aidb.knowledge_base_stats` and `aidb.kbstat` are kept as aliases of `aidb.knowledge_base_metrics` for backwards compatibility.
...
| `part_ids`      | bigint[]         | Which parts (e.g. chunk or page) of the source record produced the matching vector               |
