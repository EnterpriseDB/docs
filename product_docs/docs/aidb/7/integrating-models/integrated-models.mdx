...
| `lightonocr-2-1b-Q8_0`       | `llamacpp_ocr` (llama.cpp)        | OCR (text extraction from images)       |
...
| `llamacpp_reranking` | llama.cpp | Local reranking via llama.cpp (GGUF format). |
| `llamacpp_ocr` | llama.cpp | Local OCR (text extraction from images) via llama.cpp (GGUF format). |
...
llama.cpp runs GGUF-format model files — a common distribution format for quantized, open-weight models — through four providers: `llamacpp_generate` for text generation, `llamacpp_embeddings` for text embeddings, `llamacpp_reranking` for reranking, and `llamacpp_ocr` for OCR (text extraction from images). Point any of them at a GGUF file, downloaded from HuggingFace or already on disk. See [Default models](#default-models) above for AIDB's pre-registered GGUF models; use the recipes below to register a different one.
...
Extracts text from images using a local vision-language GGUF model plus its multimodal projector (mmproj) file:
...
SELECT aidb.create_model(
    ...
    config => jsonb_build_object(
        'model', 'ggml-org/LightOnOCR-2-1B-GGUF',
...
| Parameter           | Type    | Default                              | Description                                                                                                                          |
|----------------------|---------|-----------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------|
| `model`              | TEXT    | Required unless `local_path` is set    | HuggingFace repo id, for example, `ggml-org/LightOnOCR-2-1B-GGUF`.                                                                     |
| `model_file`         | TEXT    | Required unless `local_path` is set    | Filename of the base model `.gguf` inside the repo.                                                                                    |
| `mmproj_file`        | TEXT    | Required unless `mmproj_local_path` is set | Filename of the multimodal projector (mmproj) `.gguf`, in the same HuggingFace repo/revision as the base model.                    |
| `mmproj_local_path`  | TEXT    | NULL                                     | Absolute path to a local mmproj `.gguf` file. Overrides the HuggingFace-based `mmproj_file` download.                                |
| `revision`           | TEXT    | `main`                                   | HuggingFace revision.                                                                                                                   |
| `local_path`         | TEXT    | NULL                                     | Absolute path to a local base model `.gguf` file. Overrides `model`/`model_file`/`revision`.                                          |
| `cache_dir`          | TEXT    | NULL                                     | Local directory for caching downloaded model files.                                                                                     |
| `n_ctx`              | INTEGER | 8192                                     | Context window, in tokens.                                                                                                              |
| `prompt`             | TEXT    | Built-in transcription instruction      | OCR instruction sent with each image. Prompt influence is model-dependent.                                                             |
| `temperature`        | DOUBLE PRECISION | 0.0                             | Sampling temperature. Defaults to `0.0` for faithful transcription rather than creative sampling.                                       |
