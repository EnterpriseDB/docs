...
See the [Models reference](../reference/models#model-config-helpers) for full parameter details on all model config helper functions. Some functions (`decode_text()`, `summarize_text()`) also accept a per-call configuration override — see [Configuration model](../reference/models#configuration-model) for creation-time vs. call-time settings.
