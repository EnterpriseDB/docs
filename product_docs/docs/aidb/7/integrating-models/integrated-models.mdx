...
`llamacpp_generate` is also the only provider that supports `tools`, `tool_choice`, and `response_format` for tool calling and structured output — see [Tool calling and structured output](../reference/models#tool-calling-and-structured-output).
