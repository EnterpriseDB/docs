...
| Parameter                 | Type    | Default  | Description                                              |
|---------------------------|---------|----------|----------------------------------------------------------|
| `api_key`                 | TEXT    | Required | Has no default, but pass `NULL` and supply the real key via `create_model()`'s `credentials` argument instead — `config` can never contain `api_key`. |
