...
description: Pre-vetted model variants confirmed to work with AIDB, including local models (Candle and llama.cpp), remote NVIDIA NIM models available via HM Model Serving, and common OpenAI/Anthropic model families for AIDB's native agentic adapters.
deepToC: true
---
...
- **`openai_responses`** (+ `openai_responses_azure`) and **`anthropic_messages`** (+ `anthropic_messages_azure`/`anthropic_messages_bedrock`) — AIDB's native, agent-ready providers. Real `tools`/`tool_choice`/`response_format` support, both for direct calls and for agents created with [`aidb.create_agent()`](../agents-and-tools/creating-agents).
