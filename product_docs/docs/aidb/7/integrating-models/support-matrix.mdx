...
AIDB supports NVIDIA NIM microservices via several providers — see [External model connections](external-model-connections#nvidia-nim) for how to register one. All other models can be self-hosted using KServe and registered with AIDB using `aidb.create_model()`.
...
- **`openrouter_chat`** — a single gateway to OpenAI, Anthropic, and other vendors' models (including everything below) via OpenRouter's own model slugs, for example `openai/gpt-4o` or `anthropic/claude-opus-4-8`. Same simulated, not native, tool calling as `openai_completions`.
...
| Model identifier    | Native provider(s)                                                              | Also usable via   | Cloud host support (native)         | Description                                                                                   |
|----------------------|------------------------------------------------------------------------------------|----------------------|----------------------------------------|-------------------------------------------------------------------------------------------------|
| `claude-opus-4-8`    | `anthropic_messages`, `anthropic_messages_azure`, `anthropic_messages_bedrock`     | `openrouter_chat`    | Direct, Azure AI Foundry, AWS Bedrock  | Flagship Claude model for the most demanding reasoning and agentic tool-calling tasks.          |
