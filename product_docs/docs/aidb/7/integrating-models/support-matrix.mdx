...
- **`openai_responses`** (+ `openai_responses_azure`) and **`anthropic_messages`** (+ `anthropic_messages_azure`/`anthropic_messages_bedrock`) — AIDB's native, agent-ready providers. Real `tools`/`tool_choice`/`response_format` support, both for direct calls and for agents created with `aidb.create_agent()`.
