...
SELECT aidb.create_sql_tool(
    ...
    read_only        => true,  -- optional; enforced, not just descriptive (see below). Default: false
...
| `read_only`          | BOOLEAN | `false`  | If `true`, reject the statement at registration time unless it's provably read-only. |
...
`read_only` defaults to `false`. Set it to `true` to tell AIDB this tool never writes — this is enforced, not just descriptive: if `sql_statement` matches a recognized write pattern (`INSERT`, `UPDATE`, `DELETE`, and so on), registration fails immediately rather than letting a mislabeled tool through. A tool marked `read_only => true` is also usable by an agent running in [read-only mode](../agents/conversing-with-agents#read-only-mode); every other SQL tool is excluded from a read-only run, the same as an unclassified native or MCP tool.
