...
- **Catalog discovery tools** live, exact lookups against the Postgres catalog. They need no setup, always reflect the current state of the database, and match by exact name or SQL `LIKE` pattern.
- **Semantic knowledge base tools** semantic (vector) search over a crawled snapshot of the schema. They require a pre-built, opt-in knowledge base (and a re-crawl to pick up structural changes), but can answer natural-language questions like "which table holds customer contact info?" that exact catalog lookups can't.
