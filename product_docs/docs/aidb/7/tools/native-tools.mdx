...
AIDB ships with a large built-in catalog of native tools — mostly thin wrappers around existing AIDB functionality, so an agent can operate on pipelines, knowledge bases, models, and volumes the same way you would from SQL directly. This is one of three tool types AIDB supports — see [Tools](../tools) for the full catalog, including [custom SQL tools](../custom-sql-tools) and [MCP tools](../mcp-tools). `sleep` is always available to every agent regardless of its configured `tools` list (it backs the reasoning loop's own ability to pause and resume). Every other native tool must be explicitly listed in `tools` to be usable.
