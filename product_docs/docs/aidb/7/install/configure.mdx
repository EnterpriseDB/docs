...
Lower this on a shared or resource-constrained database server to prevent local model inference from saturating available CPUs:

```sql
SET aidb.max_threads = 4;
```

A session-local `SET` applies immediately, to that backend only. Setting it with `ALTER SYSTEM` or in `postgresql.conf` applies after `pg_reload_conf()` (or `SIGHUP`), and propagates to all backends and background workers — no restart required.
...
| Parameter                   | Type    | Default | Range | Description                                                                                                                                                                    |
|------------------------------|---------|---------|-------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `aidb.enable_llamacpp_logs` | boolean | `false` | —     | Controls whether llama.cpp's own diagnostic output (GGUF metadata, tensor list, backend and model initialization) is written to the server log. `Suset`. Read once at extension initialization — see below. |
...
Unlike the other parameters on this page, `aidb.enable_llamacpp_logs` is `Suset` (a superuser, or a role granted `SET` on the parameter, can change it) and is read once when the extension initializes. Setting it with `SET`, `ALTER SYSTEM`, or editing `postgresql.conf` and reloading (`pg_reload_conf()`) does **not** take effect until Postgres is restarted:
...
Applies only to `llamacpp_generate` and `llamacpp_embeddings` models — it has no effect on Candle-based models (`bert_local`, `clip_local`, `t5_local`, `llama_instruct_local`). See [Local models — Troubleshooting](../integrating-models/integrated-models#troubleshooting) for when to use this.
