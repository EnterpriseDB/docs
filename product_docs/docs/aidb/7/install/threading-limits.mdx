...
[`aidb.max_io_threads`](configure#aidbmax_io_threads) caps AIDB's async I/O thread pools — used for calling remote model providers over HTTP (OpenAI-compatible endpoints, NVIDIA NIM, Gemini, and OpenRouter) and for volume/object-store I/O against PGFS-backed volumes.
...
See [Threading limits](configure#threading-limits) for the parameter reference (type, default, range).
