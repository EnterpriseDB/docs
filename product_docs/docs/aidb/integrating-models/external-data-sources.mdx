...
navTitle: External data sources
description: How to connect AIDB pipelines to external storage — S3-compatible object stores, Google Cloud Storage, Azure Blob Storage, and local file systems — using PGFS storage locations and AIDB volumes.
---

AIDB pipelines can read from two types of data source: Postgres tables and external storage volumes. 
External storage — S3-compatible object stores, Google Cloud Storage, Azure Blob Storage, and local file systems — is accessed through the Postgres File System (PGFS) extension, which maps external storage into Postgres as *storage locations*. 
AIDB then wraps each storage location in a *volume* that pipelines reference by name.
...
For local file system access, the allowed base paths must be declared in `postgresql.conf` before creating the storage location. PGFS restricts access to these paths for security.
