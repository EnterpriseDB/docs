...
Aggregate version of `aidb.summarize_text`. Summarizes text across multiple rows using a standard SQL aggregate pattern.
...
SELECT
    ...
    aidb.summarize_text_aggregate(
        text_column,
        '{"model": "my_t5_model"}'::jsonb ORDER BY id
    ) AS summary
FROM my_table
GROUP BY category;
```

Accepts the same options as `aidb.summarize_text`. Rows are processed in the order specified by the `ORDER BY` clause inside the aggregate call.
