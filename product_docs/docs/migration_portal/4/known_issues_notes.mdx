...
When a new version of Migration Portal gets released, there is a chance that users might face an issue where they are not able to open Migration Portal as they might get a `White label Error Page` or some other error which will prevent them from using Migration Portal. The browser's stored cache data causes this error. Users are requested to clear the cache data from the browser history or use the Incognito/Private window to access the Migration Portal in such a scenario.
