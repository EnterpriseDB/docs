...
Next, determine the number of MB your environment needs to increase the allocated storage space:
...
1.  Calculate the total number of MB your environment requires for the UNDO tablespace. For example, if the transaction volume generates X MB of undo data every minute, and the requirement to retain it for Y minutes:
