...
The Migration Portal will migrate non-DEFAULT profiles assigned to the schema users. Oracle profiles include both password and resource limits.  EDB Postgres Advanced Server only supports the Oracle password related limits.  Resource limits are extracted with the profiles, but only the password limits are uploaded, assessed, and migrated by the Migration Portal.  Also, in the Oracle database, the DEFAULT profile may have been overriden with new limit values.  The DEFAULT profile is not migrated by the Migration Portal.  If it is desired that the DEFAULT profile in EPAS be configured with any limit overrides that have been applied to the Oracle DEFAULT profile, they will have to be applied manually in the target EPAS database by using the ALTER PROFILE statement.
