...
While assessing source schemas with Migration Portal, Object Definitions can be incomplete or the DDL assessment is terminated when the source DDL contains reserved words as object names.

If you encounter an issue where some part of the Object Definition is missing or the definition gets terminated before reaching the actual end of the DDL, you are possibly using keywords reserved by EDB Postgres Advanced Server. For example, when you use `CONSTRAINT` as a column name and then use it in a `TABLE`, `VIEW` or `MVIEW` definition, the definition is terminated before reaching the end of the DDL.
