...
- You might see some errors while trying to extract `ROLES`, `SYSTEM GRANTS ON ROLES`, `OBJECT GRANTS ON ROLES` and `ROLE GRANTS` from Oracle 11g using `impdp` or `expdp` in schema mode. The workaround is to use the `full=y` option, which extracts all the schemas and the mentioned object types without error.
