...
!!!note 
   You can see which DDLs have been removed in the assessment log files.
!!!

The following 

- Objects related to materialized views, for example, backend `TABLE` or `INDEX` statements created to support `MVIEW`. 
- All Queue-related objects.
- Objects with nestled tables.
- Objects related to XML-Type tables.
- Types dependant on SYSTEM schemas.
