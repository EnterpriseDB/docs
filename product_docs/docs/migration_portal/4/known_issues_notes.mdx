...
Next, determine the number of MB your environment required and increase the allocated storage space:

1.  Monitor the volume of undo data generated by transactions:
    ...
    !!!note 
    The resulting value reflects the used MB over a time interval specified in your system's `v$undostat` view.
    !!!

1.  Calculate the total number of MB your environment requires for the UNDO tablespace. For example, if the transaction volume generates X MB of undo data every minute, and the requirement is to retain the data for Y minutes:
