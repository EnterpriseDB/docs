...
An *instance* is a registered upstream data source — either an EDB Hybrid Manager (HM) deployment or a standalone Grafana Loki endpoint. The viewer can connect to multiple instances at the same time, aggregating clusters and sessions across all of them.

You manage instances on the **Configure** page. Reach it from the home page's **Configure Instances** button, or from the header settings menu by choosing **Configure instance**.
...
You create the machine user API key in the HM admin portal. The viewer stores it server-side and never returns it to the browser. For more on credential handling, see [Security and data handling](./security_and_data).
...
Instance configuration is stored in the backend's Postgres database. Access keys are kept server-side only and are never sent to the browser — the access key field is cleared on the server and omitted from responses. Each instance is assigned a unique identifier in the form `inst_` followed by 12 hexadecimal characters.
