...
This gives you two independent enforcement layers:
...
Configure each Airman MCP instance with three settings that determine what it can do and how its activity is tagged in the viewer.
...
Airman MCP runs in either restricted or unrestricted mode. Set the mode with the `--accessmode` flag when starting the Airman MCP process — **[ENGINEERING: confirm the exact invocation, for example as a CLI argument, environment variable, or config file entry]**.
...
Set the database connection in your Airman MCP configuration to use the login user you created — **[ENGINEERING: confirm the exact setting name, for example `AIRMAN_MCP_DATABASE_URL` as an environment variable or a config file field]**:
...
3. Deploy a new Airman MCP instance with `AIRMAN_MCP_PURPOSE` set to the new purpose label and the database connection pointing at the new login user — **[ENGINEERING: add a cross-reference to Airman MCP deployment instructions or a deployment example]**.
