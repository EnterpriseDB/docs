...
Airman MCP runs in either restricted or unrestricted mode. Set the mode with the `--access-mode` flag when starting the Airman MCP process — **[ENGINEERING: confirm the exact invocation, for example as a CLI argument, environment variable, or config file entry]**.
