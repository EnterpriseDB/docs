...
```
!!! Tip
    For an open source JDBC 3.0 type 4 driver for Microsoft SQL Server and Sybase ASE, see [http://jtds.sourceforge.net/](http://jtds.sourceforge.net/).
...
If you're connecting to a SQL Server database, `SRC_DB_URL` takes the form of a JTDS URL or JDBC URL, as follows:
...
By default Microsoft JDBC Driver for SQL Server uses TLS encryption for all data communication between client and SQL Server. By setting encrypt property to false, it won’t use TLS encryption. User can enable and disable it as per their requirement.

 ```text
 jdbc:sqlserver://<server_name>:<port>[;<database_id>][;encrypt=false]
 ```
 
!!! Tip
    For more information about connecting to a Microsoft SQL Server using JDBC type 4 driver, see [Building the connection URL](https://docs.microsoft.com/en-us/sql/connect/jdbc/building-the-connection-url?view=sql-server-ver16).
