...
```text
jdbc:<postgres_type>://<host_name>[:<port>]/<database_id>
```
...
-   `<host_name>` The name or IP address of the host where the source server is running. You can get the value for the URL from the EDB Cloud portal. In EDB Cloud, select your cluster and go to the **Connect** tab and copy the **Service URI** value. You will need to replace `postgres` with either `edb` or `postgresl` (see `postgres_type` above).

-   `<port>` The port number that the database listener is monitoring.

-   `<database_id>` The name of the source database.

-   `TARGET_DB_USER` should specify the name of a privileged database user.
