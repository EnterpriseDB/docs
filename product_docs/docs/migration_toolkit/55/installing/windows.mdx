...
You must also set the Java executable path in a configuration file found at `C:\Program Files\edb\mtk\etc\sysconfig\edbmtk-<version>.config`. An entry in that configuration file should reflect the location of the Java executable. For example:

`JAVA_EXECUTABLE_PATH="C:\Program Files (x86)\Java\jre-1.8\bin\java.exe"`
