...
You can include multiple custom type mappings in `<property_file>`. Specify each entry in the file on a separate line, in a key=value pair. The left side of each pair selects the columns with a regular expression. The right side of each pair names the data type for that column to assume.
