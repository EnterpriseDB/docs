...
!!! Note
    The `-allschemas` parameter is supported only for the Oracle, EDB Postgres Advanced Server, and PostgreSQL source database. It is not supported for Sybase, MS SQL Server, and MySQL source databases.

Disable the user-created triggers while migrating the data. However, as PostgreSQL/EDB Postgres Advanced Server does not allow disabling the inherited child partition triggers using DISABLE TRIGGER USER clause in ALTER TABLE, the triggers will get fired during data migration on partition tables (that may cause unexpected results).
...
| [Parallel Data Loading Options](#parallel-loading-options)          | -loaderCount, -parallelLoadRowLimit, -tableLoaderLimit                                                                                                                                                                                                                                                                                                                                                                                                             |
...
When dealing with a relatively large number of tables and/or large size tables, normal data migration takes a long time. As a solution to this problem, MTK has come up with a parallel data loading algorithm that allows migrating data in parallel at the schema level and at the table level. 
...
!!! Note
    - Parallel loading is useful for large size tables only. For small size tables, parallel threads may take more time. Thus it is recommended to use parallel data loading at the table level for relatively large size tables.

    - Parallel data loading is not applicable for tables without a primary/unique key.

    - `fastCopy` parameter cannot be used with parallel data loading.

    - Parallel loading functionality applies only to the source Oracle, EDB Postgres Advanced Server, and PostgreSQL databases.

Following options control the way MTK migrates data in parallel.

`-loaderCount [<value>]`

Use the `-loaderCount` option to specify the number of parallel threads available in the pool to perform data load in parallel. This option is particularly useful if the host system (that is running Migration Toolkit) has high-end CPU and RAM resources. While `value` may be any non-zero, positive number, we recommend that value should not exceed the number of CPU cores. For example, a dual-core CPU should have an optimal value of `2`. Default is `1`.
...
Use the `-parallelLoadRowLimit` option to specify the minimum number of rows required in a table to perform data load in parallel at the table level. The `value` parameter should be greater than 0. Default is 100,000. 

For example, if the value of `-parallelLoadRowLimit` is 10,000, only tables with the number of rows 10,000 and above will be loaded in parallel (at the table level). The other tables will be migrated in sequential mode or at the schema level.

`-tableLoaderLimit [<value>]`

Use the `-tableLoaderLimit` option to specify the maximum number of threads that may be assigned from the thread pool to load a single table data in parallel chunks (at the table level). The `value` parameter should not be greater than the value of the `-loaderCount`. Default value is equal to the value of the `-loaderCount` option. This option is implicitly changed based on the non-default custom value of `-loaderCount`. 

For example, if the `-loaderCount` is set to 4 and `-tableLoaderLimit` is not specified, the `-tableLoaderLimit` will also be set to 4. Hence, if all threads are not meant to be used for a single table load, reduce the `-tableLoaderLimit` to a lower value.

**High-level Overview of the Parallel Data Loading Algorithm**

The algorithm works in three steps:

**Step 1:** A list of tables and table chunks is created from the ordered (sorted by the table name) list of tables that have to be migrated. The list is created based on the `-parallelLoadRowLimit` and `-tableLoaderLimit` option values.

**Step 2:** A thread pool is created based on the `-loaderCount` option value.

**Step 3:** The parallel data loading algorithm creates tasks for each table/chunk from the list of tables and table chunks and executes them in the thread pool (as many tasks as the number of threads are executed in parallel). As soon as a task execution completes, its thread starts executing the next task from the list. Parallel data loading completes as soon as the last task from the list is completed.


**Example:**

In this scenario, there are four tables `tab1, tab2, tab3, tab4` to be migrated with parallel data loading:
...
Table `tab1` has 50,000 rows, table `tab2` has 9,500 rows, table `tab3` has 50,001 rows and table `tab4` has 9,999 rows. The list of tables for Step1 is `tab1, tab2, tab3`, and `tab4`.

Only tables `tab1` and `tab3` qualify for parallel loading on table level (`-parallelLoadRowLimit 10000`)

As the value of the `-tableLoaderLimit`  option is 2, table `tab1` is loaded in two chunks, `tab1_chunk1` and `tab1_chunk2`. The number of rows in each chunk is determined based on the `totalRowCount/tableLoaderLimit`. In this case, both chunks have 25,000 rows (50,000/2). 
...
**Step 1:** Here is the the list of tables and table chunks created:
...
**Step 2:** Thread pool with four threads `T1, T2, T3, T4` is created.
 
Step 3: Tasks migrating tables/chunks from the list created in step 1 are executed in the thread pool:
...
Thread `T3` finishes first and it starts executing `tab3_chunk2` (next chunk in the list):
...
Thread `T1` finishes second and it starts executing `tab4`:
...
If you change command options to have `-tableLoaderLimit` 1:
...
You can have pure schema level parallelism. Each table is processed simultaneously in one thread:
