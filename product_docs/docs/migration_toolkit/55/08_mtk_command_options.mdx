...
!!! Note
    ...
    - Migration Toolkit disables the user-created triggers while migrating the data. However, as PostgreSQL/EDB Postres Advanced Server does not allow disabling inherited child partition triggers using ``DISABLE TRIGGER USER` clause in the `ALTER TABLE`, the triggers will get fired during data migration on partition tables (that may cause unexpected results).
