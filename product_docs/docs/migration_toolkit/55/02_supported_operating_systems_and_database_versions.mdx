...
!!! Note
All the PostgreSQL and EDB Postgres Advanced Server versions available as EDB Postgres® AI Cloud Services single-node, primary/standby high-availability, and distributed high-availability cluster types (formerly BigAnimal cluster types) are also supported for use with Migration Toolkit. See the (managed database cloud service) [documentation](/biganimal/latest) for more information about [supported cluster types](/biganimal/latest/overview/02_high_availability/). See the [database version policy documentation](/biganimal/latest/overview/05_database_version_policy/) for the versions of PostgreSQL and EDB Postgres Advanced Server available in EDB Postgres® AI Cloud Services.

The superuser role isn't available in EDB Postgres Advanced Server distributed high-availability cluster databases managed by EDB Postgres® AI Cloud Services (formerly BigAnimal). As a result, the following Migration Toolkit capabilities that require superuser access aren't supported when targeting EDB Postgres® AI Cloud Services distributed high-availability clusters:
