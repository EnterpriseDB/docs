...
!!! Note
The superuser role is not available for BigAnimal EDB Postgres Advanced Server distributed high-availability cluster databases. As a result, the following Migration Toolkit capabilities that require superuser access are not supported when targeting BigAnimal distributed high-availability clusters:
1. the `-copyViaDBLinkOra` option that enables data migration using db_link_ora extension
2. the Oracle compatible SQL command `CREATE DATABASE LINK`
!!!
