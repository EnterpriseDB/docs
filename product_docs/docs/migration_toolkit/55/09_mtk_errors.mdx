...
Before using Migration Toolkit, you must download and install the appropriate JDBC driver for the database that you are migrating from. See [Installing a JDBC driver](/migration_toolkit/latest/05_installing_mtk/installing_jdbc_driver) for complete instructions.
