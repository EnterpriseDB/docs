...
-   Use the `-connRetryCount <connection_attempts>` option to specify the number of retry attempts to perform. 
-   Use the `-connRetryInterval <seconds>` option to specify the seconds to wait before each reconnection attempt.
-   Use the `-abortConnOnFailure <true/false>` option to specify if to abort the migration when all reconnection attempts failed.
    The default is `true`, which aborts the session if the connection fails after the the specified `-connRetryCount` threshold.
-   Use the `-pgIdleTxSessionTimeOut <seconds>` to specify the PostgreSQL or EDB Postgres Advanced Server `idle_in_transaction_session_timeout`.
