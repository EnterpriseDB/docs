...
Append migration options when you run Migration Toolkit to control details of the migration. For example, to migrate all schemas in a database, append the `-allSchemas` option to the command:
...
| [Schema object selection options](#schema-object-selection-options) | -allTables, -tables, -excludeTables,<br /><br />-constraints, -ignoreCheckConstFilter,<br /><br />-skipCKConst, -skipFKConst,<br /><br />-skipColDefaultClause,<br /><br />-indexes, -triggers,<br /><br />-allViews, -views, -excludeViews,<br /><br />-allSequences, -sequences,<br /><br />-allProcs, -procs,<br /><br />-allFuncs, -funcs,<br /><br />-checkFunctionBodies,<br /><br />-allPackages, -packages,<br /><br />-allDomains,<br /><br />-allQueues, -queues,<br /><br />-allRules, |
...
!!! Note 
    The following examples invoke the Migration Toolkit in Linux. To invoke Migration Toolkit in Windows, use the `runMTK.bat` command instead of the `runMTK.sh` command.
...
The `-sourcedbtype` option specifies the source database type. For `source_type`, use one of the following values: `mysql`, `oracle`, `sqlserver`, `sybase`, `postgresql` or `enterprisedb`. `source_type` is not case sensitive. By default, `source_type` is `oracle`.

`-targetdbtype <target_type>`

The `-targetdbtype` option specifies the target database type. For `target_type`, use one of the following values: `enterprisedb`, `postgres`, or `postgresql`. `target_type` is not case sensitive. By default, `target_type` is `enterprisedb`.
...
You can't specify `information-schema`, `dbo`, `sys`, or `pg_catalog` as target schema names. These schema names are reserved for meta-data storage in EDB Postgres Advanced Server.
...
Import the selected tables from the source schema. `table_list` is a comma-separated list of table names with no intervening space characters (e.g., `-tables emp,dept,acctg`).

`-excludeTables <table_list>`

Exclude the selected tables from migration. The `table_list` is a comma-separated list of table names with no intervening space characters (e.g., `-excludeTables emp,jobhist`). This option is applicable when all tables from a schema are selected for migration via `-allTables` option.
...
By default, Migration Toolkit doesn't implement migration of check constraints and default clauses from a Sybase database. Include the `‑ignoreCheckConstFilter` parameter when specifying the `-constraints` parameter to migrate constraints and default clauses from a Sybase database.
...
Import the specified materialized or dynamic views from the source schema. Oracle and Postgres materialized views are supported. `view_list` is a comma-separated list of view names with no intervening space characters (e.g., `-views all_emp,mgmt_list,acct_list`).

`-excludeViews <view_list>`

Exclude the selected views from migration. The `view_list` is a comma-separated list of view names with no intervening space characters (e.g., `-excludeViews all_emp,acct_list`). This option is applicable when all views from a schema are selected for migration via `-allViews` option.
...
Specify the batch size in MB to use in the COPY command. Any value greater than 0 is valid. The default batch size is 8 MB.
...
Following entries in the properties file migrate only the relevant data from EMPLOYEES and the DEPARTMENTS tables.
...
`-customColTypeMappingFile <property_file>`
 ...
 Specify each entry in the file on a separate line, in a key=value pair. The left side of each pair selects the columns with a regular expression. The right side of each pair names the data type for that column to assume. When used in the `<property_file>`, the `'\\'` characters act as an escape string irrespective of the operating system.

<div id="parallel_loading" class="registered_link"></div>

## Migration options for parallel data loading

When dealing with a relatively large number of tables or large size tables, normal data migration takes a long time. As a solution to this problem, Migration Toolkit has a parallel data loading algorithm that allows migrating data in parallel at the schema level and at the table level. 
...
!!! Note
    - Parallel loading is useful for large tables only. For small tables, parallel threads might take more time. Thus we recommend using parallel data loading at the table level for relatively large tables.

    - Parallel data loading doesn't apply to tables without a primary/unique key.

    - `fastCopy` parameter can't be used with parallel data loading.
...
Use the `-loaderCount` option to specify the number of parallel threads available in the pool to perform data load in parallel. This option is particularly useful if the host system that is running Migration Toolkit has high-end CPU and RAM resources. While `value` can be any nonzero, positive number, we recommend that value not exceed the number of CPU cores. For example, a dual-core CPU has an optimal value of `2`. The default is `1`.
...
For example, if the value of `-parallelLoadRowLimit` is 10,000, only tables with the number of rows 10,000 and above are loaded in parallel at the table level. The other tables migrate in sequential mode or at the schema level.
...
1.  A list of tables and table chunks is created from the ordered list of tables to be migrated, sorted by the table name. The list is created based on the `-parallelLoadRowLimit` and `-tableLoaderLimit` option values.
...
If you change command options to set `-tableLoaderLimit` to `1`:
...
Include the `-importPartitionAsTable` parameter to import the contents of a partitioned table that resides on an Oracle host into a single nonpartitioned table. `table_list` is a comma-separated list of table names with no intervening space characters (e.g., `-importPartitionAsTable emp,dept,acctg`).
...
The uppercase naming convention is preserved for tables, views, sequences, procedures, functions, triggers, packages, and so on. For these database objects, the uppercase naming convention is applied to:
...
Use these migration options view Migration Toolkit help and version information. You can also use these options to control Migration Toolkit feedback and logging options.
...
Include this option to have the schema definition (DDL script) of any failed objects saved to a file. The file is saved under the same path used for the error logs and is named in the format:
...
This example performs an Oracle to EDB Postgres Advanced Server migration.
