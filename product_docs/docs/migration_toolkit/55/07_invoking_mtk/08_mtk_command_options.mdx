...
Whenever there is a connection failure with the target database, Migration Tookit automatically attempts to reconnect to the 
target database to ensure the migration completes without skipping any tables. When the connection is reestablished, 
Migration Toolkit resumes the migration and performs the data copy for the remaining tables.

Migration Toolkit resumes the migration based on the mode that it was using to migrate the data:

-   If Migration Toolkit was using CopyAPI to migrate the data, it continues copying data from the last failed row. 
    CopyAPI is compatible with PostgreSQL and EDB Postgres Advanced Server as target databases.
...
!!!note Scope and limitations
   ...
   Issues with the schema migration are not supported.

   Modality scope: This reconnection capability is available with the data migration mode (`-dataOnly`). 
   It is also available when you run a migration **without** both the `-dataOnly` and `-schemaOnly` options.
!!!   
...
The VALUE parameter should be greater than 0, and the default is 180 seconds.
