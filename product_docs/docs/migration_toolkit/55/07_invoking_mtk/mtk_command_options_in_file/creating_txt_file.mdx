...
For example, if you run the following command, Migration Toolkit first recognizes the `-sourcedbtype oracle` option, and then reads the contents of `example.options` in order from top to bottom. The last parameter is the schema scope (`<schema_name>` or `-allSchemas`).
