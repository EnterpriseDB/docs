...
For example, if you run the following command, MTK first recognizes the `-sourcedbtype oracle` option, then reads the contents of `example.options` in order from top to bottom, and then reads the `-schemaOnly` option. It will then write the migration to a `schema_name`.
