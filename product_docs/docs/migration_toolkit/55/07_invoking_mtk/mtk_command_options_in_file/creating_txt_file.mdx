...
### Don't specify an option in the options file and command line
...
For example, if you run the following command, MTK first recognizes the `-offlineMigration` option, then reads the contents of `example.options` in order from top to bottom, and then reads `-safeMode <schema_name>`.

```shell
runMTK.sh -sourcedbtype oracle -optionsFile example.options -schemaOnly schema_name
```

If you want an option to be executed last, you can either put it at the end of the `example.options` file and add no further options after `-optionsFile` in the command line. For example, options related to specifying **schemas** must be placed last. 
...
Or you can place the options flag in the command line after specifying the `-optionsFile`: 
