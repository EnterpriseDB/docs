...
!!!note
   Provide the full path for `<example>.options` if you're running the Migration Toolkit command from a different folder than where you stored the file.
!!!
...
-   If you want to migrate all schemas: Add the `-allSchemas` option at the command line or add `allSchemas` in your options file. Regardless of the method you use, this option must be the last parameter.
