...
2.  Open the `pg_hba.conf` file and modify the `IDENT` entries.
...
3.  Restart the EDB Postgres Advanced Server service before invoking Migration Toolkit.
...
**Note**: This guide uses the term Stack Builder to refer to either StackBuilder Plus (distributed with EDB Postgres Advanced Server) or Stack Builder (distributed with the PostgreSQL one-click installer from EnterpriseDB).
