...
-   <span class="title-ref">Installing Migration Toolkit on a CentOS or RHEL\_PPCLE host &lt;rhel\_centos\_ppcle\_installation&gt;</span>
...
2.  Restart the EDB Postgres Advanced Server service before invoking Migration Toolkit.
...
1.  After creating the `advance-toolchain.repo` file, use your choice of editor to ensure that the value of the `enabled` parameter is `1`, and replace the `username` and `password` placeholders in the `baseurl` specification with the name and password of a registered EnterpriseDB user.
...
> 1.  After saving your changes to the configuration file, install the EPEL package:
>
> > ``` text
> > yum -y install https://dl.fedoraproject.org/pub/epel/
> > epel-release-latest-7.noarch.rpm
...
1.  If you are using RHEL 7 ppc64le, you must enable an additional repository to resolve dependencies:
...
1.  To install Migration Toolkit, run the following command.
...
1.  The command creates the repository configuration files in the /etc/zypp/repos.d directory. Use the following command to refresh the metadata on your SLES host to include the EnterpriseDB repository:
...
1.  You must also add SUSEConnect and the SUSE Package Hub extension to the SLES host, and register the host with SUSE, allowing access to SUSE repositories. Use the commands:
...
1.  Use the following command to refresh the metadata:
...
1.  Now you can use the zypper utility to install a Migration Toolkit:
