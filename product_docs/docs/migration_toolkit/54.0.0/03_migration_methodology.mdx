...
2.  Identify potential migration problems. If it is an Oracle-to-EDB Postgres Advanced Server migration, consult the [EnterpriseDB documentation](https://www.enterprisedb.com/edb-docs/p/edb-postgres-advanced-server) for complete details about the compatibility features supported in EDB Postgres Advanced Server. Consider using EnterpriseDB's migration assessment service to assist in this review.
