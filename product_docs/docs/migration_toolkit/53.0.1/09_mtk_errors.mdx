...
Before using Migration Toolkit, you must download and install the appropriate JDBC driver for the database that you are migrating from. See [Installing Source-Specific Drivers](../edb_migration_toolkit/#installing_drivers) for complete instructions.
...
To resolve this error, edit the toolkit.properties file, specifying valid connection information for the target database in the `TARGET_DB_URL` property. For information about forming a JDBC URL for Advanced Server, see [Defining an Advanced Server URL](../edb_migration_toolkit/#define_epas_url).
