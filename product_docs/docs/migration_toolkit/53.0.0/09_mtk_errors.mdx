...
To resolve this error, edit the toolkit.properties file, specifying valid connection information for the target database in the `TARGET_DB_URL` property. For information about forming a JDBC URL for Advanced Server, see [Defining an Advanced Server URL](06_building_toolkit.properties_file/#define_epas_url).
