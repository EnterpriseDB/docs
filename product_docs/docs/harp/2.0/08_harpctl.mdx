...
* `logs` - Returns important events logged in the DCS
...
Promotes the local or specified node to Lead Master in the current Location. 
Since this is a requested event, it invokes a smooth handover where:
...
2. The promoted node will be the only valid candidate to take the Lead Master
   ...
   other nodes will ignore the unset Lead Master lease.
    - If CAMO is enabled, the promoted node will temporarily disable client
      ...
      Lead Master lease.
3. HARP Proxy will `PAUSE` connections to allow ongoing transactions to 
   complete. Once the Lead Master lease is claimed by the promoted node, it
...
immediate transition to the promoted node.
...
Sets cluster-related attributes only. There's only one of these at the moment,
but future versions of HARP may add more.
...
Sets location-related attributes only. There are none of these at the moment,
and calling this command will result in an error regarding unrecognized options.

!!! Note
    This is a placeholder for future capabilities.
