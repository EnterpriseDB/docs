...
## EDB Postgres Distributed driver compatibility

The `bdr` native consensus layer is available from EPD versions 
[3.6.21](/bdr/3.7/release-notes/#bdr-362) 
and [3.7.3](/bdr/3.7/release-notes/#bdr-373).

For the purpose of maintaining a voting quorum, EPD Logical Standby 
nodes don't participate in consensus communications in an EPD cluster. Don't count these in the total node list to fulfill DCS quorum requirements.
...
coinciding with the primary EPD nodes, and a fifth DCS node (such as an EPD 
witness) elsewhere. Using such a design, a network partition between the two 
EPD data centers doesn't disrupt consensus thanks to the independently
located node.
...
This isn't possible when using EPD as the DCS, as EPD maintains a consensus 
