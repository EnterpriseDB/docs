...
- **`driver`**: Required type of consensus layer to use. 
  ...
  full EDB Postgres Distributed (EPD, formerly known as BDR) member nodes to maintain quorum during database maintenance.
...
- **`name`**: Required name of the Postgres node represented by this Manager.
  ...
  also serves to name this Manager. If this is an EPD node, it must match the 
...
- **`start_command`**: This can be used instead of the information in DCS for
  starting the database to monitor. This is required if using EPD as the
  consensus layer.
...
- **`status_command`**: This can be used instead of the information in DCS for
  ...
  required if using EPD as the consensus layer.
...
- **`type`**: Specifies whether to use pgbouncer or the experimental built-in passthrough proxy. All proxies must use the same proxy type. We recommend to experimenting with only the simple proxy in combination with the experimental EPD DCS.
...
- **`camo_enforcement`**: Whether to strictly enforce CAMO queue state.
  When set to `strict`, HARP never allows switchover or failover to an EPD 
...
- **`maximum_camo_lag`**: Highest allowable variance (in bytes) between last
  ...
  Thus this applies only to EPD clusters where `pg2q.enable_camo` is set.
