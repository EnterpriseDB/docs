...
HARP is a new approach to high availability for EDB Postgres Distributed (EPD)
...
that is "in charge." For an EPD cluster consisting of multiple nodes, this
...
Even in multi-master-capable approaches such as EPD, it can be help to
reduce the amount of necessary conflict management to derive identical data
across the cluster. In clusters that consist of multiple EPD nodes per physical
location or region, this usually means a single EPD node acts as a "leader" and
...
Postgres node to represent the entire cluster or a local EPD region. Any
...
While using EPD as the consensus layer, each server node resembles this
variant instead:

![HARP Unit w/EPD Consensus](images/ha-unit-bdr.png)

In either case, each unit consists of the following elements:

* A Postgres or EDB Postgres Advanced Server instance
...
This is a typical design using two EPD nodes in a single data center organized in a lead master/shadow master configuration:

![HARP Cluster](images/ha-ao.png)

When using EPD  as the HARP consensus layer, at least three
fully qualified EPD nodes must be present to ensure a quorum majority. (Not shown in the diagram are connections between EPD nodes.)

![HARP Cluster w/EPD Consensus](images/ha-ao-bdr.png)

## How it works

When managing an EPD cluster, HARP maintains at most one leader node per
defined location. This is referred to as the lead master. Other EPD
...
EDB Postgres Distributed Always-On group as can exist in a single data center. When any
...
!!! Note
    To operate and manage cluster state, EPD contains its own
...
HARP was primarily designed to represent a EDB Postgres Distributed Always-On architecture that
resides in two or more data centers and consists of at least five EPD
...
![EDB Postgres Distributed Always-On Reference Architecture](images/bdr-ao-spec.png)

In this diagram, HARP Manager exists on EPD Nodes 1-4. The initial state 
of the cluster is that EPD Node 1 is the lead master of DC A, and EPD 
Node 3 is the lead master of DC B.

This configuration results in any HARP Proxy resource in DC A connecting to EPD Node 1 
and the HARP Proxy resource in DC B connecting to EPD Node 3.
...
For multiple EPD nodes to be eligible to take the lead master lock in
a location, you must define a location in the `config.yml` configuration
file.

To reproduce the EDB Postgres Distributed Always-On reference architecture shown in the diagram, include these lines in the `config.yml`
configuration for EPD Nodes 1 and 2:

```yaml
location: dca
```

For EPD Nodes 3 and 4, add:
...
location to the EPD node. This is done by calling the following SQL
API function while connected to the EPD node. So for EPD Nodes 1 and 2, you
might do this:
