...
!!! Note
    ...
    to EDB Postgres Distributed (EPD, formerly known as BDR) cluster deployments. If you can't access the TPAExec URL,
...
either `bdr` or `etcd`. If using fewer than three EPD nodes, you might need to rely on `etcd`. Otherwise any EPD service outage reduces the
...
use more than two nodes so that EPD's consensus layer remains responsive during node
maintenance or outages.

To set EPD as the consensus layer, include this in the `config.yml` 
configuration file:
...
The endpoints for a EPD consensus layer follow the
