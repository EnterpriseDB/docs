...
Postgres or EDB Postgres Distributed (EPD formerly known as BDR) as needed to maintain node status in the consensus
layer.
...
EPD nodes have metadata and views that are visible only when certain roles are
...
The `bdr_monitor` EPD role is meant for status monitoring tools to maintain
ongoing information on cluster operation, thus it is well-suited to HARP.

## EPD consensus permissions

When the `dcs.driver` configuration parameter is set to `bdr`, HARP 
uses EPD as the consensus layer. As such, it requires access to API 
...
Currently access to the EPD consensus model requires superuser equivalent
permission.

!!! Important
    EPD superusers *are not* Postgres superusers. The `bdr_superuser` role is
    merely granted elevated privileges in EPD, such as access to
