...
**PGFS storage abstraction**: The [Postgres File System (PGFS)](/pg_extensions/pgfs/) is a cloud-native storage layer that provides a unified interface between Postgres and object storage (S3, GCS, Azure). It handles authentication, network resilience, and the mapping of remote objects to local table structures.

**Delta Lake**: An open-source storage framework that brings ACID transactions to data lakes.

**Transaction log**: A central metadata record &mdash; such as the Delta Log in Delta Lake or snapshots in Iceberg &mdash; that tracks every change to the table, ensuring data consistency.
...
**Iceberg REST:** A standardized API used by PGAA to communicate with external catalogs. It allows EDB to integrate with any catalog service that supports the REST specification, such as Lakekeeper, Tabular, Snowflake Polaris, or MinIO AIStor.
...
**Metadata caching:** Maintains a local, high-speed copy of the data lake's structural information, such as file manifests, schema definitions, and partition locations. Since Iceberg and Delta Lake tables can consist of thousands of individual files, caching this map lets the query planner perform partition pruning and file-level filtering almost instantaneously, without reaching out to the storage provider on every query. Catalogs with many tables or under high query concurrency benefit most, since cached entries are revalidated with conditional (ETag) requests rather than refetched outright. See [Configuring metadata caching](../catalog#configuring-metadata-caching).
