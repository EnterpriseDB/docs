...
| Iceberg | Read | Iceberg REST catalog | Seafowl  <br /> Spark | Yes |
...
| Iceberg | Write (via `CTAS`) | Local, S3, GCP, Azure (via storage locations) | Postgres engine via CompatScan | Yes |
| Iceberg | Write (via `CTAS`) | Iceberg REST catalog | Postgres engine via CompatScan | Yes |
| Delta Lake | Write (via `CTAS`) | Local, S3, GCP, Azure (via storage locations) | Postgres engine via CompatScan | Yes |
