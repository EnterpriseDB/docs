...
| Iceberg | Write (via `CTAS`) | Local, S3, GCP, Azure (via storage locations) | Postgres | Yes |
| Iceberg | Write (via `CTAS`) | Iceberg REST catalog | Postgres | Yes |
| Delta Lake | Write (via `CTAS`) | Local, S3, GCP, Azure (via storage locations) | Postgres | Yes |
