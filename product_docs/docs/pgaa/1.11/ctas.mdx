...
!!!Note
    On WarehousePG, a single writer, the coordinator on WarehousePG 7.6 and later or one designated segment on WarehousePG earlier than 7.6, writes the CTAS results. See [Writing to object storage using CTAS](whpg/reading#writing-to-object-storage-using-ctas).
