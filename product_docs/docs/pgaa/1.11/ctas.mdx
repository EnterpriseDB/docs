...
!!!Note
    On WarehousePG, a single writer, the coordinator on WarehousePG later than 7.5 or one designated segment on WarehousePG 7.5 or earlier, writes the CTAS results. See [Writing to object storage using CTAS](whpg/reading#writing-to-object-storage-using-ctas).
