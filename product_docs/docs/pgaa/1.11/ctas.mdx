...
!!! Note
    ...
    CTAS is designed for immutable, write-once operations. For more advanced write capabilities, such as continuous `INSERT`, `UPDATE`, or `DELETE` operations on object storage, refer to the [PGD integration](pgd).

!!!Note
    On WarehousePG, a single writer writes the CTAS results. On WarehousePG 7.6 and later versions, the writer is the coordinator. On WarehousePG versions earlier than 7.6, the writer is the designated segment. See [Writing to object storage using CTAS](whpg/reading#writing-to-object-storage-using-ctas).
...
To write to object storage, you must [create a PGFS storage location](/pg_extensions/pgfs/using/#creating-a-storage-location). This defines the target bucket and provides the necessary credentials. For example:
...
## Monitoring CTAS progress

For large CTAS operations, PGAA emits `NOTICE` messages to help you track progress without needing to poll system views or query logs. Progress is reported every 1 million rows or every 10 minutes, whichever comes first, followed by a final completion message:

```
NOTICE: CTAS progress: 1000000 rows processed
NOTICE: CTAS progress: 2000000 rows processed
NOTICE: CTAS completed: 2500000 rows processed
```
