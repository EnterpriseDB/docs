...
description: Understand how PGD and PGAA work together to enable cost-efficient data lifecycle management with tiered storage.
deepToC: true
---
...
For details, see [Implementing tiered tables](/pgd/latest/tiered-storage/tiered-tables/) in the PGD documentation.
...
!!! Warning
Don't rename a table configured for analytics replication, or the schema that contains it. PGD tracks replicated tables by name, so a rename interrupts synchronization with the object storage copy and can leave stale or duplicate data behind.
!!!

For details, see [Replicating to analytics](/pgd/latest/tiered-storage/replicate/) in the PGD documentation.
...
For details, see [Offloading to analytics](/pgd/latest/tiered-storage/offload/) in the PGD documentation.
...
- **No-catalog offload**: Store metadata alongside data files using [storage locations](/pgaa/latest/read_no_catalog/#creating-a-storage-location) to connect directly to object storage. Local filesystem storage locations aren't supported for this method in multi-node PGD deployments. Use a shared object storage provider (S3, GCS, or Azure).
