...
You can also write new data with CTAS. PGAA assigns the resulting table a distribution policy that keeps it consistent across the cluster so it can be queried and joined like any other PGAA table. See [Writing to object storage using CTAS](reading#writing-to-object-storage-using-ctas).
