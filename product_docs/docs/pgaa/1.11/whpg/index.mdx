...
- **Write to object storage using CTAS:** Use `CREATE TABLE AS SELECT` (CTAS) to export query results into Iceberg or Delta Lake format in your data lake. See [Writing to object storage using CTAS](reading#writing-to-object-storage-using-ctas).
...
Tables mapped to a storage location or an Iceberg catalog are read-only. The data must already exist in object storage, for example written by PGD or another pipeline.

You can also write new data with CTAS. A single writer, the coordinator on WarehousePG later than 7.5 or one designated segment on WarehousePG 7.5 or earlier, writes the data, and PGAA assigns the resulting table a distribution policy that keeps it consistent across the cluster so it can be queried and joined like any other PGAA table. See [Writing to object storage using CTAS](reading#writing-to-object-storage-using-ctas).
