...
To upgrade an existing PGAA installation on WarehousePG to a newer version, complete the steps in this section instead of [Installing the extension](#installing-the-extension).

1. On the coordinator, download the new package version:
...
1. Transfer and upgrade the package on all hosts, including the coordinator and every segment host:

    ```shell
    gpssh -f all_hosts -e "scp gpadmin@$(hostname):~/edb-whpg7-pgaa*.rpm /tmp/ && sudo dnf install -y /tmp/edb-whpg7-pgaa*.rpm"
    ```

1. Restart the Seafowl service on all hosts to load the new binary:

    ```shell
    gpssh -f all_hosts -e 'sudo systemctl restart seafowl'
    ```

1. Restart the WarehousePG cluster to load the new `pgaa` library:

    ```shell
    gpstop -ra
    ```

1. Connect to the coordinator and update the extension in each database where PGAA is installed:
...
!!!Note
Upgrade the package on every host, coordinator and segments, and complete both restarts before running `ALTER EXTENSION`. Running it against a `pgaa` library that hasn't been upgraded yet fails with an error such as `could not find function "..._wrapper" in file ...`, because the SQL migration scripts expect functions that only exist in the newer library.
!!!
