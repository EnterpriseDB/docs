...
- For WarehousePG earlier than 7.6, one designated segment is the single writer, and CTAS broadcasts the full `SELECT` result set to every segment first, so write performance is slower for large result sets.
