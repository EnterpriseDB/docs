...
Write query results to object storage using `CREATE TABLE AS SELECT` (CTAS), targeting either a standalone storage location or an Iceberg catalog, with the same syntax as on any other PGAA installation. A single writer writes the data, rather than every segment in parallel, and PGAA gives the resulting table a distribution policy that keeps it consistent across the cluster so it can be queried and joined like any other PGAA table.

Which host acts as the single writer, and how it receives the `SELECT` results, depends on the WarehousePG version:

- For WarehousePG 7.6 and later, the coordinator is the single writer, and CTAS sends the `SELECT` results to the coordinator only, with no broadcast to segments.
- For WarehousePG earlier than 7.6, one designated segment is the single writer, and CTAS broadcasts the full `SELECT` result set to every segment first, so every segment except the writer receives the full result set only to discard it, wasting network and compute resources.
