...
- For WarehousePG 7.5.1 and later, the coordinator is the single writer, and CTAS sends the `SELECT` results to the coordinator only, with no broadcast to segments.
- For WarehousePG earlier than 7.5.1, one designated segment is the single writer, and CTAS broadcasts the full `SELECT` result set to every segment first, so write performance is slower for large result sets.
