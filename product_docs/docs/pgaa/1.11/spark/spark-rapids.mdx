...
2.  Confirm that [NVIDIA RAPIDS](#enabling-nvidia-rapids) is properly installed and active within your Spark cluster. 
3.  [Connect Postgres to Spark](#connecting-postgres-to-spark).
4.  [Configure Postgres's source data](#configuring-the-source-data). 
5.  [Run analytical queries](#running-analytical-queries) and verify that the workload is utilizing GPU acceleration.
...
2.  **GPU discovery:** [Install the GPU discovery script](https://docs.nvidia.com/spark-rapids/user-guide/latest/getting-started/on-premise.html#install-the-gpu-discovery-script) that allows Spark to identify the available GPUs. 

3.  **Apply Spark configuration:** Enable the following parameters as a minimum. For a full list of optimization settings, see the [RAPIDS configuration](https://nvidia.github.io/spark-rapids/docs/configs.html) and [Tuning guide](https://docs.nvidia.com/spark-rapids/user-guide/latest/tuning-guide.html).
...
4.  **Container provisioning:** If deploying Spark via Docker, modify your container manifest to grant Spark access to the host GPUs.
...
2.  Confirm that Postgres can communicate with the Spark cluster by executing a simple version check through the PGAA interface:
...
1.  Create a [PGFS storage location](/pg_extensions/pgfs/using/#creating-a-storage-location) in your Postgres database that points to the bucket containing your analytical data. For example, with a public bucket:
...
2.  Create tables using the `PGAA` access method. Specify the storage location created in the previous step, the path to either a single Parquet file or a directory containing multiple Parquet files, and the format (currently Parquet). For example:
...
2.  Navigate to the SQL tab and select your recently executed query.
3.  Inspect the plan. If RAPIDS is active, the query nodes will be prefixed with **Gpu**. For example, `GpuFilter`, `GpuBatchScan`, `GpuUnion`, etcetera. 
