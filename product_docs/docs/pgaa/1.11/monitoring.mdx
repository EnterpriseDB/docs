...
### Controlling Iceberg manifest concurrency

Tables affected by the small file problem also accumulate many Iceberg manifest entries. Seafowl reads and decodes these manifests to plan scans and maintenance operations, and doing so for a large number of manifests at once can drive up memory use and generate a burst of object store requests. Bound this concurrency with [`pgaa.autostart_seafowl_concurrent_manifest_ops`](reference/configuration_parameters#pgaaautostart_seafowl_concurrent_manifest_ops), which caps the number of concurrent manifest object-store operations. Lower it to reduce peak memory and request bursts on tables with very large manifests, or raise it to speed up planning on tables with many small manifests, provided your object store and host have the capacity.
...
!!! Note
On WarehousePG, background maintenance workers run on the coordinator only and do not run on segment hosts. Before calling `pgaa.launch_task()`, install the `pgcrypto` extension on the coordinator: `CREATE EXTENSION IF NOT EXISTS pgcrypto;`.
!!!
...
- **Z-Order (Delta only):** Reorganizes data across multiple columns so related values are stored physically close together. This enables data skipping, where the engine ignores entire files that don't match query filters.
- **Vacuum (Delta only):** Deletes data files no longer associated with active snapshots and older than a specified retention period. This reclaims object storage space.
- **Purge (Delta only):** An administrative tool to explicitly remove data from a specific storage path, useful for manual cleanup of failed exports.
...
- Use vacuum to delete files that are no longer part of the active table state and are older than 7 days to reclaim storage space:
...
For complex maintenance operations not yet supported by the native background worker, use the following functions.

**pgaa.spark_sql()**

Run Iceberg compaction routines or Spark functions that aren't available in Postgres via the configured Spark Connect endpoint. See [Spark procedures](https://iceberg.apache.org/docs/latest/spark-procedures/) for a list of the available procedures.
...
!!! Note
    To use this function, you must set the `pgaa.spark_connect_url` configuration parameter to point to an available Spark Connect service.


**pgaa.execute_compaction() (deprecated)**

The `pgaa.execute_compaction()` function consolidates fragmented data files into larger, more efficient Parquet files, improving the performance and storage efficiency of an analytical Iceberg or Delta table:
...
See the [function reference section](reference/functions/) for a complete list of supported options and examples.
