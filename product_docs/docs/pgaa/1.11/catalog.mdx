...
PGAA supports reading from and writing (using [CTAS](ctas/#using-an-iceberg-catalog)) to Iceberg tables using the following catalog implementations: 
- **REST catalogs:** Any service following the Apache Iceberg REST API, such as Lakekeeper, Tabular, Apache Polaris, Project Nessie, or MinIO AIStor. 
...
- `iceberg-rest`: For Iceberg REST catalog (Lakekeeper, Snowflake Polaris, Tabular, MinIO AIStor).
...
A REST catalog typically holds the object store credentials itself, or authenticates to the object store using a method such as IMDSv2 or WebIdentity. When a client requests a table, the catalog checks access and vends short-term, table-scoped credentials back to the client. This credential-vending model is the recommended architecture because the catalog governs data access centrally, and the client never needs to hold long-lived object store credentials, only an API token to authenticate with the catalog.

Some REST catalog implementations don't vend credentials and instead expect the client to supply its own object store credentials directly. In that case, add a `store_config` object to `catalog_options` with the object store connection details:
...
Where:
- `type`: The object store type, for example `s3` for an S3-compatible store.
- `bucket`: The name of the bucket holding the catalog's Iceberg tables.
- `access_key_id` and `secret_access_key`: The credentials used to access the bucket.
- `region`: The object store region.
- `endpoint`: The object store endpoint, required for S3-compatible stores such as MinIO.
- `allow_http`: Set to `"true"` to allow non-TLS connections to the endpoint.
...
- Test connectivity and write permissions of your configuration. The function returns `NULL` if the connection is successful. For example:
...
Configure Seafowl to cache the Iceberg table metadata it retrieves from your REST catalog, cutting round trips to the catalog service during query planning. Catalogs with many tables, or under high query concurrency, benefit the most, since cached entries are revalidated with conditional (ETag) requests rather than refetched outright. Tune caching with the following parameters:

- [`pgaa.autostart_seafowl_enable_metadata_cache`](reference/configuration_parameters#pgaaautostart_seafowl_enable_metadata_cache): Turns the cache on or off. Enabled by default.
- [`pgaa.autostart_seafowl_metadata_cache_capacity_mb`](reference/configuration_parameters#pgaaautostart_seafowl_metadata_cache_capacity_mb): Sets the maximum amount of metadata the cache retains before evicting older entries. Raise this parameter for catalogs with many tables.
- [`pgaa.autostart_seafowl_metadata_cache_ttl_s`](reference/configuration_parameters#pgaaautostart_seafowl_metadata_cache_ttl_s): Sets how long a cached entry is trusted before Seafowl refreshes it, independent of ETag revalidation. Set to `0` to rely solely on ETag revalidation.
...
- [Creating a catalog-managed table](#creating-catalog-managed-tables) to register a single table with a high level of granularity. 
...
- `pgaa.metastore_sync_poll_rate_s`: Controls the polling frequency.
- `pgaa.metastore_sync_stale_duration_s`: Defines the threshold for metadata freshness.
