...
| `pgd.purge_analytics_target` | boolean | Delete any pre-existing tables from the object store directory or catalog entry. See [Implementing tiered tables](../pgd/tiered-tables) for details. |
...
Transition tables between transactional and analytical storage by altering the table's access method. This provides a direct way to manually move data between the local Postgres disk and object storage. See [Offloading to analytics](../pgd/offload) for details.
