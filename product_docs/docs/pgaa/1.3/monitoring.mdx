...
Iceberg maintenance is performed synchronously using the Spark engine. Use the function `pgaa.spark_sql()` to run Spark SQL queries directly. See the function reference section for [pgaa.spark_sql()](reference/functions#pgaaspark_sql) for a complete list of supported options and examples.
...
!!! Note
    You must set the `pgaa.spark_connect_url` configuration parameter to point to an available Spark Connect service.
