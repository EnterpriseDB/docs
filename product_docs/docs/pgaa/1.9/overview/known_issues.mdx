...
- The gprestore option `--create-db` isn't supported for PGAA tables. You must manually create the database the PGAA and PGFS extensions before running gprestore. See [Backing up and restoring](../whpg/backup) for details.
[BEAC-1378]: # 
- `INSERT ... SELECT` from PGAA tables fails with `ExtensibleNodeMethods "SeafowlCompatScan" was not registered`. This is a known issue with no workaround at this time.
