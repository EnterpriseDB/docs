...
!!!Note
The `pgaa.autostart_seafowl` parameters are not available on WarehousePG. On WarehousePG, Seafowl is managed as a `systemd` service on each host. See [Configuring and monitoring](../whpg/managing#seafowl-management) for details.
!!!
