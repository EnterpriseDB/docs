...
Postgres Analytics Accelerator (PGAA) extends WarehousePG with lakehouse capabilities, enabling distributed analytical queries against data in object storage without separate infrastructure or data movement. 

## Capabilities

- **[Query object storage directly from Postgres](reading#querying-object-storage-directly):** Register an S3, Google Cloud Storage (GCS), or Azure path as a storage location, create a PGAA table pointing to it, and query Parquet, Delta Lake, or Iceberg files with standard SQL without the need for ETL or data import.
- **[Connect to an Iceberg catalog](reading#connecting-to-an-iceberg-catalog):** Attach PGAA to an external Iceberg REST catalog for table discovery, schema and partition metadata, and authentication to the underlying object storage. When EDB Postgres Distributed (PGD) replicates data to object storage, WarehousePG can attach to the shared catalog and read that data at MPP scale, enabling a combined OLTP and analytical workload across both platforms.
- **[Accelerate with Spark](reading#accelerating-with-spark):** For large workloads, or if you already operate a Spark cluster, route queries through Spark Connect instead of Seafowl. If your cluster includes GPU-enabled nodes, RAPIDS acceleration is available on top of Spark for even higher throughput on compute-intensive queries.
- **[Back up and restore](backup):** Use `gpbackup` to capture PGAA table definitions and storage bindings. After a restore, tables immediately reference the correct object storage paths with no manual reconfiguration required.
- **[Monitor and maintain](../monitoring):** Monitor table health and run maintenance operations such as compaction and vacuum on PGAA tables. On WarehousePG, maintenance workers run on the coordinator only.
...
- **CompatScan:** Used when the query involves operations Seafowl cannot handle, such as a join with a native WarehousePG heap table. PGAA proposes two candidate execution plans to the WarehousePG planner, which picks the cheaper one:

    - **Strewn locus (MPP):** the scan is distributed across all segment hosts, each reading a portion of the data in parallel.
    - **General locus (coordinator-only):** the scan runs entirely on the coordinator. With [aggregation pushdown](../optimize_performance#configuring-compute-pushdowns) enabled, the coordinator processes aggregations before returning results, which is often cheaper than the MPP path.
...
| Seafowl | CompatScan | Coordinator only (General locus) or MPP across segments (Strewn locus), based on cost. |
