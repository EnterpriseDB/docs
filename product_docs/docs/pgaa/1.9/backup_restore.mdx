...
#### Performing a PITR restore after offloading tables

When performing [point-in-time recovery (PITR)](/pgd/latest/lifecycle/backup-restore/) using Barman's `--target-time` option, a target time that falls during a `pgaa.convert_to_analytics()` operation can leave the deployment in a partially recovered state.

Offloading a table to object storage with `pgaa.convert_to_analytics()` removes the table's local heap copy and creates Iceberg data files and a REST catalog entry in object storage. During recovery, Postgres rolls back the `pgaa.table_mapping` insert and the heap truncate, but can't roll back the Iceberg data files and REST catalog entry already written to object storage. The heap table holds its original rows while the Iceberg data and catalog entry remain as orphans.
...
For the `pgaa.convert_to_analytics()` flow, see [Replicating with PGD](pgd).
