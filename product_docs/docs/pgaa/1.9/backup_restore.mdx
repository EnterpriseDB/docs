...
Follow the [physical restore procedure](/pgd/latest/backup-restore/) for PGD clusters. This procedure assumes you have a coordinated backup from all nodes in the cluster. See [Backing up a PGD cluster](#backing-up-a-pgd-cluster) for details. When you restore all nodes together, PGAA catalog state is consistent across the restored nodes and post-restore writes replicate normally. Keep the following PGAA-specific points in mind:

- On manual restores, set `max_worker_processes` before starting Postgres. `barman recover` handles this adjustment automatically. See [Setting `max_worker_processes` on a manual restore](/pgd/latest/backup-restore/#setting-max_worker_processes-on-a-manual-restore) in the PGD documentation for details.
...
When performing [point-in-time recovery (PITR)](/pgd/latest/backup-restore/) using Barman's `--target-time` option, a target time that falls during a `pgaa.convert_to_analytics()` operation can leave the deployment in a partially recovered state.
