...
Refer to [PGFS functions for Pipelines](/pg_extensions/pgfs/using) for details on how to create storage locations for PGAA.
...
!!! Note
In a PGD environment, this function relies on BDR and isn't available when restoring a single node in isolation. See [Restoring a single PGD node in isolation](../backup_restore#restoring-a-single-pgd-node-in-isolation) for alternatives.
!!!
...
### pgaa.lakehouse_table_stats_uncached()

Returns the same storage statistics as [`pgaa.lakehouse_table_stats()`](#pgaalakehouse_table_stats), but always queries object storage directly instead of returning a cached result. Use this function when you need the current state and can't wait for [`pgaa.lakehouse_table_stats_cache_ttl_s`](configuration_parameters#pgaalakehouse_table_stats_cache_ttl_s) to expire.

**Synopsis**

```sql
SELECT * FROM pgaa.lakehouse_table_stats_uncached('table_name'::regclass);
```
...
-   For REST catalogs
    ...
    -   `danger_accept_invalid_certs`: If set to `true`, Postgres skips SSL certificate validation. Use this only for internal testing or with self-signed certificates; never use it for sensitive public connections.
        ...
        Most REST catalogs vend short-term, table-scoped object store credentials to the client, so no further configuration is needed. If your REST catalog doesn't vend credentials, add a `store_config` object with the object store connection details:
        ...
        "store_config": {
            ...
            "region": "us-east-1",
            "endpoint": "https://your-object-store-endpoint.com",
            "allow_http": "true"
        }
        }
        ```
    ...
    -   `store_config.type`: The object store type, for example `s3` for an S3-compatible store.

    -   `store_config.bucket`: The name of the bucket holding the catalog's Iceberg tables.

    -   `store_config.access_key_id` and `store_config.secret_access_key`: The credentials used to access the bucket.

    -   `store_config.region`: (Optional) The object store region.

    -   `store_config.endpoint`: (Optional) The object store endpoint, required for S3-compatible stores such as MinIO.

    -   `store_config.allow_http`: (Optional) Set to `"true"` to allow non-TLS connections to the endpoint.

        `store_config` supports the same object store providers as PGFS. For the connection options for Azure or Google Cloud Storage, see the [PGFS storage provider documentation](/pg_extensions/pgfs/using/#storage-provider-types).
...
!!! Note
`pgaa.attach_catalog()` is not supported on WarehousePG. Use [pgaa.import_catalog()](#pgaaimport_catalog) to manually import catalog metadata instead.
!!!
...
Performs compaction to improve the performance and storage efficiency of an analytical table. Runs as a native background task and supports both Iceberg and Delta tables.

!!! Note

    The function `pgaa.execute_compaction()` is deprecated in favor of `pgaa.launch_task()`.
...
!!! Note
    ...
    The parameter `settings` is currently not supported. Use [pgaa.spark_sql()](#pgaaspark_sql) for more granular control over compaction behavior.
...
Schedules a background maintenance task for an analytical Delta or Iceberg table. 
...
!!! Note
On WarehousePG, background maintenance workers run on the coordinator only and do not run on segment hosts. Additionally, this function requires the `pgcrypto` extension on the coordinator, as it uses `gen_random_uuid()` internally. Before calling this function, run `CREATE EXTENSION IF NOT EXISTS pgcrypto;` on the coordinator.
!!!
...
-   `compaction`: Merges small files into larger ones to speed up analytical scans. The available `task_options` are: 
    ...
    -   `target_size` (Delta only): Specifies the size of the output files in bytes.
    -   `preserve_insertion_order` (Delta only): Whether to maintain the existing sort order of rows.
    -   `max_concurrent_tasks` (Delta only): Limits the number of parallel tasks the executor can run.
    -   `max_spill_size` (Delta only): Sets the maximum data size in bytes allowed to spill to disk during the process.
    -   `min_commit_interval` (Delta only): Sets the minimum wait time in seconds between committing updates to the Delta log.
    -   `dry_run`: If `true`, calculates and logs the operation, but does not perform compaction.


-   `zorder` (Delta only): A clustering technique that reorganizes data across multiple columns to improve "data skipping" for queries with filters on those columns. The available `task_options` are:
...
-   `vacuum` (Delta only): Deletes old data files that are no longer referenced by the Delta transaction logs, freeing up space in object storage. The available `task_options` are:
...
-   `purge` (Delta only): Explicitly removes data from a specific storage path. The available `task_options` are:
...
Converts a heap table into a PGD-managed tiered table with automatic partitioning and analytics offloading. This function automates the creation of a partitioned staging table, migrates existing data, and configures the lifecycle policies for both hot and cold tiers. See [Implementing tiered tables](/pgd/latest/tiered-storage/tiered-tables/) for details.
...
Disables PGD replication to object storage for an HTAP table. This function reverts the table from an HTAP state back to a Heap state by stopping the continuous replication stream to object storage. See [Replicating to analytics](/pgd/latest/tiered-storage/replicate/) for details.
...
Enables PGD replication for a heap table to object storage for analytics. This function transitions a standard Heap table to an HTAP table by initiating a continuous replication stream to the analytics engine. See [Replicating to analytics](/pgd/latest/tiered-storage/replicate/) for details.
...
!!! Note
When using Barman to back up a PGD cluster with HTAP tables, a point-in-time recovery (PITR) with a target time that falls during this operation can leave the deployment in a partially recovered state. See [Known limitation: PITR restore after enabling analytics replication](../backup_restore#known-limitation-pitr-restore-after-enabling-analytics-replication) for details.
!!!
...
Runs a synthetic, pgbench-like replication benchmark to measure the performance of data ingestion into a specified storage location or catalog. It calculates the initial data load speed and the sustained transaction throughput.

!!! Note Internal function

    This function is primarily intended for internal testing and is still under development.
...
| Parameter          | Type      | Description                                                                                                                  |
| ------------------ | --------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `storage_location` | `TEXT`    | The name of the PGFS destination storage location where the benchmark data will be written. Mutually exclusive with `catalog`. |
| `catalog`          | `TEXT`    | The name of the Iceberg catalog to use as the benchmark target. Mutually exclusive with `storage_location`. |
| `format`           | `TEXT`    | The table format to use for the benchmark. Default is `iceberg`.                                                             |
| `scale`            | `INTEGER` | The scale factor for the benchmark, determining the size of the initial dataset. Default is 100.                             |
| `transactions`     | `INTEGER` | The total number of transactions to simulate during the throughput test. Default is 100000.                                  |
| `dry_run`          | `BOOLEAN` | If `true`, the function validates the parameters and connection without performing the actual benchmark. Default is `false`. |

**Returns**

| Column                       | Type               | Description                                                                    |
| ---------------------------- | ------------------ | ------------------------------------------------------------------------------ |
| `initial load speed  (MB/s)` | `DOUBLE PRECISION` | The rate at which the initial dataset was written to storage.                  |
| `throughput (TPS)`           | `DOUBLE PRECISION` | The sustained number of transactions per second achieved during the benchmark. |
