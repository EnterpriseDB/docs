...
!!! Note
    Automatic catalog sync using `pgaa.attach_catalog()` isn't supported for PGAA on WarehousePG. You must manually run `pgaa.import_catalog()` whenever the catalog changes.
