...
The Postgres Analytics Accelerator (PGAA) supports writing directly to your data lake using the `CREATE TABLE AS SELECT` (CTAS) syntax. This enables you to transform Postgres data and save it into high-performance columnar formats like Iceberg or Delta Lake on cloud storage.
...
### Creating the CTAS table

Create a table with the `USING PGAA` and `AS` clauses that exports its result set directly to a structured table format in object storage. For example:
...
If you have integrated an Iceberg catalog, you can write to it by specifying the catalog name. In this mode, the catalog automatically manages the storage paths and table versioning for you.

Writing to a table in object storage via CTAS and using an Iceberg catalog requires two steps: first, configure a catalog connection, and then execute the SQL command that maps your query results via that catalog.
...
Create a table with the `USING PGAA` and `AS` clauses that exports its result set directly to a configured Iceberg catalog. For example:
...
- `pgaa.format`: Sets the destination table format (`iceberg`).
- `pgaa.managed_by`: Specifies your external Iceberg catalog.
- `pgaa.catalog_namespace`: Defines the namespace where the table will reside within your Iceberg catalog.
- `pgaa.catalog_table`: Specifies the name of the table to be created in your data lake.
...
- **No automatic overwrites:** To prevent accidental data loss, the command will fail by default if data already exists at the specified `pgaa.path`. If you intend to replace the existing data, you must explicitly set the `pgaa.purge_data_if_exists` parameter to true in the `WITH` clause of your CTAS statement.
