...
- **Parquet:** A columnar storage file format available for use as a standalone format. Unlike Iceberg or Delta, which manage a collection of files via metadata, a Parquet-based storage location maps directly to individual files or directories. It is highly efficient for heavy analytical queries because it allows the PGAA engine to read only the specific columns required for a query.
...
You must [create a PGFS storage location](/edb-postgres-ai/latest/ai-factory/pipeline/pgfs/functions/#creating-a-storage-location) to tell Postgres where your bucket is and provide the necessary credentials. For example, with a public S3 bucket:
...
!!! Note 
    If you leave the column definitions empty (`()`), PGAA will automatically discover the schema from the Iceberg or Delta metadata files when you first query the table. For standalone Parquet files, you must explicitly define the columns in your `CREATE TABLE` statement. In both cases, the PGAA engine maps columns by name to the fields found in the remote Iceberg/Delta metadata or Parquet headers.
...
You must manually resolve interference as the data lake evolves:
- Use `ALTER TABLE` commands in Postgres to add, drop, or modify columns to match the remote schema.
- For Iceberg and Delta tables, the most common fix is to drop and recreate the table using empty parentheses `()`. Upon the first query, PGAA will perform a fresh discovery of the remote metadata.
