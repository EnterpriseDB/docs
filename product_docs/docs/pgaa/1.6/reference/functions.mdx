...
Converts a heap table into a PGD-managed tiered table with automatic partitioning and analytics offloading. This function automates the creation of a partitioned staging table, migrates existing data, and configures the lifecycle policies for both hot and cold tiers. See [Creating tired tables](../pgd/tiered-tables) for details.
...
Returns real-time, uncached lakehouse statistics for an HTAP table. This function directly queries the storage backend to provide the most current snapshot and total data size.
