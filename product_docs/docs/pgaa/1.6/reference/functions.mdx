...
`pgaa.launch_task(regclass, text, jsonb, timestamp)`: 'Schedules a background maintenance task for a PGAA Delta table. 

Task types: ''compaction'', ''zorder'', ''vacuum'', ''purge''. 
