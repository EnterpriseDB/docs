...
| `replication_status`         | `pgaa.replication_status` | The current state of data movement (`enabled`, `disabled`, or `initial_offload`). |
...
See [pgaa.add_catalog()](#pgaa.add_catalog) for for a detailed breakdown of the required JSON fields for each catalog type.
...
Enables continuous metadata synchronization for a previously registered Iceberg catalog (using [pgaa.add_catalog()](#pgaa.add_catalog)). Once attached, PGAA automatically monitors the remote catalog for changes and updates the local PostgreSQL metadata accordingly. See [Catalog synchronization](configuration_parameters/#catalog-synchronization) for polling rate configuration.
...
Executes a Spark SQL query directly on the configured Spark Connect endpoint. It requires that the configuration parameter `pgaa.executor_engine` is set to 'spark_connect'.
...
```sql
-- Multi-catalog or default version
...
Performs compaction to improve the performance and storage efficiency of an analytical Iceberg table. It requires that the configuration parameter `pgaa.executor_engine` is set to 'spark_connect'.

**Synopsis**

```sql
CALL pgaa.execute_compaction('table_name'::regclass, 'settings'::json);
```
...
| `settings` | `JSON` | Optional. A JSON object defining compaction behavior. Default is `NULL`. |

Parameters for the optional JSON `settings` are:
...
- `max_concurrent_tasks`: Limits on the number of parallel Spark tasks.
- `max_spill_size`: Sets the maximum data size in bytes allowed to spill to disk during the process.
- `min_commit_interval`: Sets the minimum time in seconds between commits to the Iceberg log.
...
```sql
CALL pgaa.execute_compaction(
    'lakehouse.sales_data'::regclass, 
    '{
        "target_size": 1073741824,
        "preserve_insertion_order": false,
        "max_concurrent_tasks": 4,
...
| Parameter                 | Type     | Description                                           
| ---------------------- | -------- | ----------------------------------------------------- 
| `table_name` | `REGCLASS` | The name or OID of the analytical table to compact. |
