...
- **Postgres**: Version 16 or later. Any supported Postgres distributions, and running on Debian 11/12 or Ubuntu 22/24 on amd64 architectures, and RHEL 8/9 on both amd64 and arm64.
- **PGAA extension**: Version 1.6 or later installed on your Postgres database.
...
1. Create tables using the `PGAA` access method. Specify the storage location created in the previous step, the path to either a single Parquet file or a directory containing multiple Parquet files, and the format (currently Parquet). For example:
  ...
  CREATE TABLE public.customer_address () USING PGAA
      WITH (pgaa.storage_location = 'my-sample-data', pgaa.path = 'tpcds_sf_10/customer_address', pgaa.format = 'parquet');
  CREATE TABLE public.customer_demographics () USING PGAA
      WITH (pgaa.storage_location = 'my-sample-data', pgaa.path = 'tpcds_sf_10/customer_demographics', pgaa.format = 'parquet');
  ...
  CREATE TABLE public.warehouse () USING PGAA
      WITH (pgaa.storage_location = 'my-sample-data', pgaa.path = 'tpcds_sf_10/warehouse', pgaa.format = 'parquet');
  CREATE TABLE public.ship_mode () USING PGAA
      WITH (pgaa.storage_location = 'my-sample-data', pgaa.path = 'tpcds_sf_10/ship_mode', pgaa.format = 'parquet');
