...
-   Read-only queries for Iceberg tables in Iceberg REST catalogs (OAuth2 authentication is not supported).
!!!
...
2.  [Connect Postgres to Spark](#connecting-postgres-to-spark)
3.  [Configure Postgres's source data](#configuring-the-source-data)
4.  [Run analytical queries](#running-analytical-queries)
5.  [Inspect the query plan](#inspecting-the-query-plan)
...
2.  Validate the connection by running a simple version check through the PGAA interface:
...
2.  Create the necessary catalog-managed tables. Alternatively, attach or import the catalog.
