...
You use PGAA to point to the Spark Connect endpoint from Postgres and to a storage location in object storage, then PGAA uses Spark as the query engine to perform the analytical queries.
