...
PostgreSQL Analytics Acceleration(PGAA) supports integration with third-party Apache Iceberg® catalogs, enabling seamless data lake integration for analytical workloads. 
PGAA supports reading from and writing (using CTAS) to Iceberg tables through external catalog services, allowing you to leverage existing data lake infrastructure alongside your PostgreSQL deployments.
...
Supported catalogs --> confirm this.

## Prerequisites

- PGAA installed
- Catalog requirements???
- Object storage: FS, S3, GCP, Azure
- Data format: Iceberg, Delta, (Parquet?)


## Configure the Catalog Connection

If you are using a REST catalog, you need to obtain and regularly refresh an OAuth token.
...
1. Add -  pgaa.add_catalog
1. Attach - pgaa.attach_catalog
1. List - 
1. Verify - 
1. Import
...
If the catalog already has tables, you can simply query the imported tables directly:
