...
In formats like Iceberg or Delta Lake, the total size often exceeds the snapshot size to support time-travel and rollbacks. A significant gap between these values indicates a buildup of historical overhead and serves as a signal to perform a `VACUUM` or compaction. Monitoring these metrics helps you maintain a lean, high-performing data lake. 
...
- Set specific file limits to Spark to define the file size strategy:

    ```sql
    SELECT pgaa.execute_compaction(
        table_name => 'lakehouse.inventory_iceberg'::regclass,
        options    => '{"target-file-size-bytes": "536870912"}' -- 512MB
    );
    ```
