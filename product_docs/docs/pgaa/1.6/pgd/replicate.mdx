...
Additionally, you can enable the `bdr.prefer_analytics_engine` parameter to route queries to the PGAA engine (Seafowl or Spark):
...
Disable the `bdr.prefer_analytics_engine` parameter so queries are again executed by the standard Postgres.
