...
- **Monitoring & capacity:** Track partition growth and storage savings by comparing local versus remote data sizes. Monitor background worker logs to identify offload failures or object storage permission issues.
- **Scheduling:** Execute bulk offload or restore operations during low-traffic maintenance windows to minimize impact on production workloads.
- **Lifecycle maintenance:** Periodically verify object storage connectivity and ensure the storage layer is optimized for analytical query performance by managing file consistency in the cold tier.
- **Resilience & backups:** Continue standard backup routines for the hot transactional tier. Leverage the native durability of object storage for the cold tier and validate access paths by inspecting query plans for historical data.