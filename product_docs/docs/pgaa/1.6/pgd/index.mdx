---
title: Replicating with PGD
navTitle: Replicating with PGD
...
Maintain high availability and cost efficiency by proactively managing your tiered environment according to these guidelines.
- **Monitor capacity and health:** Regularly track partition growth and storage savings by comparing local versus remote data sizes.
- **Schedule offloads strategically:** Execute bulk offload or restore operations during low-traffic maintenance windows to minimize resource contention on production workloads.
- **Maintain storage lifecycle:** Periodically verify object storage connectivity. Ensure the storage layer remains optimized for analytical query performance by managing file consistency in the cold tier.
- **Validate resilience and recovery:** While standard backup routines should continue for the hot transactional tier, you should leverage the native durability of object storage for the cold tier. Periodically validate access paths by inspecting query plans for historical data.