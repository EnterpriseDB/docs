...
When used in conjunction with a catalog service, tiered tables create a unified view in the catalog that performs a `UNION` of all partitions. You can enable the `bdr.prefer_analytics_engin`e parameter to route queries on the entire tiered table to the PGAA engine (Seafowl or Spark):
