...
When used in conjunction with a catalog service, you can enable the `bdr.prefer_analytics_engine` parameter to route queries on tables in the cold partitions to the PGAA engine (Seafowl or Spark):

```sql
SET bdr.prefer_analytics_engine = TRUE;
```

If `enable_replication` is `true` and this parameter is enabled, the system routes queries for the entire table to the analytics engine. The catalog provides the engine with a view of both offloaded data and the replicated hot data.
