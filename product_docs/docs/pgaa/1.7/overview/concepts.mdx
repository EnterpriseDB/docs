...
**PGFS storage abstraction**: The [Postgres File System (PGFS)](/edb-postgres-ai/latest/ai-factory/pipeline/pgfs/) is a cloud-native storage layer that provides a unified interface between Postgres and object storage (S3, GCS, Azure). It handles authentication, network resilience, and the mapping of remote objects to local table structures.
