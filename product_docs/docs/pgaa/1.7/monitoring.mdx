...
### Advanced Spark-based maintenance

For complex Iceberg maintenance operations not yet supported by the native background worker, use the following functions.

!!! Note
    To use these functions, you must set the `pgaa.spark_connect_url` configuration parameter to point to an available Spark Connect service.
