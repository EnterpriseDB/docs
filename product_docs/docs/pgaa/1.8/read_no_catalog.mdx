...
- **Parquet:** The underlying columnar storage format used by Iceberg and Delta Lake data lakes. When used as a standalone format without a management layer, PGAA maps directly to the physical files or directories. 
...
The process involves two steps: defining the physical storage access via a storage location and mapping the remote metadata to a local definition.

### Creating a storage location

Storage locations act as aliases for your cloud buckets. You must create a PGFS storage location to tell Postgres where your bucket is and provide the necessary credentials. You can create a storage location with the `pgfs.create_storage_location()` function using the following syntax:
...
For example, with a public S3 bucket:
...
For a comprehensive list of configuration parameters and supported storage providers, see [Creating a PGFS storage location](/pg_extensions/pgfs/using/#creating-a-storage-location).

After creating a storage location, verify its existence with the following command:
