...
-   Column names with a dollar sign (`$`) at the start or immediately following a dot, for example, `$field` or `nested.$field`, aren't supported. However, `$` is permitted in other positions, such as `field$name`.
