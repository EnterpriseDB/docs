...
MongoDB Foreign Data Wrapper allows the pushdown of the `WHERE` clause only when clauses include the comparison expressions that have a column and a constant as arguments. `WHERE` clause pushdown isn't supported where the constant is an array. From version 5.5.0 and later, we now support recursive operator expressions, boolean expressions, Relabel types, and Var's on both sides of an operator.
