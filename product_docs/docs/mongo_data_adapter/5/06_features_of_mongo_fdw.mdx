...
MongoDB Foreign Data Wrapper supports retrieving sub-json, by allowing you to retrieve sub-fields from a document.
You can retrieve sub-fields in a collection residing in MongoDB Foreign Data Wrapper as explained in the following example:
...
1.  Create a forign table. To access a sub-field use the dot (".") in the columnn name as shown below:
