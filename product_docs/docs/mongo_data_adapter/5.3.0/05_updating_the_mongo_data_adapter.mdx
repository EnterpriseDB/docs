...
-   On RHEL or CentOS 7 on PPCLE:
    ...
    where `xx` is the server version number and  `yy` is Advance Toolchain major version number. For EDB Postgres Advanced Server versions 9.6 to 11, `yy` must be 10 and for EDB Postgres Advanced Server version 12 and later, `yy` must be 11. 
