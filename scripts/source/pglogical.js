...
(async () => {
  ...
  const mdIndex = yaml.load(
    await fs.readFile(path.resolve(basePath, "pglogical.yml"), "utf8"),
  );
    ...
    for (let i = 0; i < tree.children.length; ++i) {
      ...
      if (
        ...
        node.value = node.value.replace(
          ...
          `<AuthenticatedContentPlaceholder target="https://www.2ndquadrant.com/en/resources/pglogical/pglogical-docs/${filename.replace(
            ".md",
            "/",
          )}"`,
        );
