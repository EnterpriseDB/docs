...
cd $DESTINATION_CHECKOUT/product_docs/docs/tpa/22/
node $DESTINATION_CHECKOUT/scripts/source/files-to-ignore.mjs \
  "$DESTINATION_CHECKOUT/product_docs/docs/tpa/22/" \
...
node $DESTINATION_CHECKOUT/scripts/source/merge-indexes.mjs \
  ...
  "$DESTINATION_CHECKOUT/product_docs/docs/tpa/22/index.mdx" \
  "$SOURCE_CHECKOUT/docs/src/index.mdx" \
  >> $SOURCE_CHECKOUT/files-to-ignore.txt

node $DESTINATION_CHECKOUT/scripts/source/merge-indexes.mjs \
  "$SOURCE_CHECKOUT/docs/src/reference/index.mdx" \
  "$DESTINATION_CHECKOUT/product_docs/docs/tpa/22/reference/index.mdx" \
  "$SOURCE_CHECKOUT/docs/src/reference/index.mdx" \
  >> $SOURCE_CHECKOUT/files-to-ignore.txt

rsync -av --delete --exclude="*.md" --exclude="architectures" --exclude="templates" --exclude-from=$SOURCE_CHECKOUT/files-to-ignore.txt src/ $DESTINATION_CHECKOUT/product_docs/docs/tpa/22/
