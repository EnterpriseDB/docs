...
function remarkMdxEmbeddedHast()
{
  ...
  function transformer(tree, file)
  {
    ...
    function isUsableHast(root)
    {
      ...
      return root.children[0].data?.position?.closing 
    ...
    function isOpeningTag(root, sourceHtml)
    {
      ...
      if (voidElements.includes(root.children[0].tagName?.toLowerCase())) return false;

      // of course, also shouldn't have children, and shouldn't have a closing
      return !root.children[0]?.children?.length
    ...
    function captureToEnd(node, index, parent, hast)
    {
      ...
      while (endIndex<parent.children.length && parent.children[endIndex].value !== valueToMatch)
      {
      ...
      replacement.children[0].children = parent.children.splice(index+1, endIndex-index-1);
