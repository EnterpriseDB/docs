    ...
    function indexer(node) {
      if (node.properties && node.properties.id) {
        if (
          ...
          node.properties.className.includes("registered_link")
        )
          index.indexRegisteredId(file.path, node.properties.id);
        else index.indexId(file.path, node.properties.id);
  ...
  function transformer(tree, file) {
    ...
    function visitor(node) {
      if (
        ...
        relativeUrl.test(node.properties.href)
      ) {
    ...
    function testId(filepath, id) {
      index.stats.relativeLinksChecked =
        (index.stats.relativeLinksChecked || 0) + 1;
      ...
      if (byFile && !byFile.invalid) {
        index.stats.targetsFoundInSameFile =
          (index.stats.targetsFoundInSameFile || 0) + 1;
        //console.log(`found ${id} in same file`);
        return generateRelativeUrl("", "", byFile.id);
      } else if (bySubdir && !bySubdir.invalid) {
        index.stats.targetsFoundInSiblingFile =
          (index.stats.targetsFoundInSiblingFile || 0) + 1;
        ...
        index.stats.targetsFoundRegisteredInSameProductAndVersion =
          (index.stats.targetsFoundRegisteredInSameProductAndVersion || 0) + 1;
      ...
      // - cannot match across versions for the same product
        ...
        index.stats.targetsFoundRegisteredCrossProductVersion =
          (index.stats.targetsFoundRegisteredCrossProductVersion || 0) + 1;
...
const index = {
  ...
  keyProductMaxVersion: (product, id) => {
    index.versionsForProduct[product] = index.versionsForProduct[product] || [];
    return index.keyProductVersion(
      product,
      index.versionsForProduct[product].sort(versionComp).pop(),
      id,
    );
  },
  ...
  keySubdir: (subdir, id, isIndex) =>
    "dir:" + (isIndex ? "i:" : "") + subdir + index.keyId(id),
  // index files get to "live" in both the directory they're in, and that directory's parent - but the former takes precedence.
  lookupIdBySubdir: (id, subdir) =>
    index.linkIdByKey[index.keySubdir(subdir, id)] ||
    index.linkIdByKey[index.keySubdir(subdir, id, true)],

  keyFilepath: (filepath, id) => filepath + index.keyId(id),
  lookupIdByFile: (id, filepath) =>
    index.linkIdByKey[index.keyFilepath(filepath, id)],

  productVersionForPath: (filepath) => {
    let [, prod, ver] = filepath.match(
      /^product_docs\/docs\/([^\/]+)\/([\d\.]+)\//,
    ) || [null, null, null];
  ...
  indexRegisteredId: (filepath, id) => {
    ...
    if (prod && ver) {
      ...
      if (maxKey === key) index.addValue(index.keyId(id), value);
      index.stats.globalIdsIndexed = (index.stats.globalIdsIndexed || 0) + 1;
    } else console.error("bad path for registered ID: " + filepath);
  ...
  indexId: (filepath, id) => {
    ...
    if (path.basename(filepath) === "index.mdx") subdir = path.dirname(subdir);
...
function generateRelativeUrl(source, dest, id) {
  ...
  while (
    pos < sourceChunks.length &&
    pos < destChunks.length &&
  ...
  while (pos < sourceChunks.length || pos < destChunks.length) {
    ...
    if (pos < sourceChunks.length - 1) result.unshift("..");
  ...
  if (!result.length && source !== dest) result.push(".");
  // append ID and run away
  return (
    result.join("/") + (result.length ? "/" : "") + "#" + encodeURIComponent(id)
  );
}
