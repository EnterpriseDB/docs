...
function transformer(tree, file) {
  ...
  visit(tree, (node, index, parent) => {
    ...
    visit(node, "element", (node) => {
      ...
      let entries = mdast2string(node).split(' ').map(s => s.trim());
      let context = entries.length && index > 0 && mdast2string(parent.children[index-1]);

      replacement = {
        type: 'list',
        ordered: false,
        children: entries.map(item => getListItemForName(item, path.dirname(file.path)))
      };

      console.error(`Double-TOC in ${file.path}:${node.position.start.line}:${node.position.start.column}` + '\n' + entries.join() + '\nCONTEXT: ' + context);
    });
  ...
  {
    let matches = glob.sync([basepath + '/*_' + name + '/index.mdx', basepath + '/??_' + name + '.mdx']);
    if (!matches.length)
    {
      console.error("NO MATCH for " + name + " in " + basepath);
      return {type: 'listItem', children: [{type: 'text', value: 'NOT FOUND: ' + name}]};
    }
  ...
  {
    return (fs.readFileSync(filename, 'utf8').match(/^title: (.+)$/m)||[''])[1].trim().replace(/^["']|['"]$/g, '');
  }
}