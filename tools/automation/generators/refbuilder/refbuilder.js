...
function processDirectory(sourceDir, prefix) {
  ...
  var isTemplate =
    fs.existsSync(rootIndexTemplatePath) &&
    fs.statSync(rootIndexTemplatePath).isFile();
  var isIndex =
    fs.existsSync(rootIndexPath) && fs.statSync(rootIndexPath).isFile();
  ...
  if (isTemplate || isIndex) {
    ...
    if (navItems != undefined) {
      // If no navItems, fall back on directory order
...
function parse(prefix, filepath) {
  ...
  indexDepth = digested.attributes.indexdepth
    ? digested.attributes.indexdepth
    : 5;
  firstIndex = -1;
  rootIsHeading = digested.attributes.rootisheading;
  refprefix = digested.attributes.refprefix;
  ...
  lexedMD.forEach((element) => {
    if (element.type == "heading" && element.depth <= indexDepth) {
      if (firstIndex == -1) {
        firstIndex = element.depth;
      }
      if (rootIsHeading && element.depth == firstIndex) {
        buffer = buffer.concat(
          `### [${element.text}](${itemCleaned}#${anchorize(element.text)})\n`,
        );
      } else {
