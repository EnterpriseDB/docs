...
try {
  branch = execSync("git rev-parse --abbrev-ref HEAD", { cwd: basePath })
    ...
    .trim();
  renames = execSync(
    `git log --diff-filter=R --name-status --pretty=format: ${baseBranch}..${branch}`,
    { cwd: basePath },
  )
    .toString()
    .split("\n")
...
const run = async () => {
  const fsPathToURLPath = (fsPath) => {
    ...
    // 4. strip trailing /
    return fsPath
      .replace(/^advocacy_docs|^product_docs\/docs|\/index\.mdx$|\.mdx$/g, "")
      .replace(/\/$/, "");
  };
    ...
    const redirectPath =
      ...
      oldUrlPathSplit[2] === newUrlPathSplit[2]
        ? path.join(
            ...
            oldUrlPathSplit[1],
            "latest",
            ...oldUrlPathSplit.slice(3),
            "/",
          )
        : path.relative(newUrlPath, oldUrlPath) + "/";
    mapCurrentPathToRedirect[key] = mapCurrentPathToRedirect[key] || [];
    mapCurrentPathToRedirect[key].push(redirectPath);
  }
  ...
  for (const sourcePath of sourceFiles) {
    const redirectPaths =
      mapCurrentPathToRedirect[path.relative(basePath, sourcePath)] || [];
    if (redirectPaths.length) found++;

    const origContent = await readFile(sourcePath, "utf8");
    const newContent = await replaceRedirect(origContent, redirectPaths, {
      sourcePath,
      branch,
    });
  ...
  console.log(
    `${renames.length} renames identified, ${found} redirects identified, ${updated} files updated`,
  );
};

const replaceRedirect = async (
  content,
  redirectPaths,
  { sourcePath, branch },
) => {
...
${error.message}`
  ...
  if (redirectPaths.length && !redirects?.items) {
    const seq = frontmatter.createNode(redirectPaths);
    for (let item of seq.items) item.comment = tagComment;
  ...
  for (let i = 0; i < redirects?.items?.length; ++i) {
    ...
    if (redirectPaths.length) {
      redirects.items[i].value = redirectPaths[found];
    } else {
  ...
  if (!found && !redirectPaths.length) return content;

  // there are redirects, but not *this* redirect, so add a new one
  if (!found && redirectPaths.length) {
    for (let redirect of redirectPaths) {
      const item = new yaml.Scalar(redirect);
  ...
  return content.replace(
    frontmatterRE,
    (_, open, fmYaml, close) =>
      open + frontmatter.toString({ lineWidth: 0 }) + close,
  );
};
