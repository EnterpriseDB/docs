...
`CASE` clauses can be used wherever an expression is valid. *`condition`* is an expression that returns a `BOOLEAN` result. If the result is `TRUE` then the value of the CA~SE expression is the *`result`* that follows the condition. If the result is `FALSE` any subsequent `WHEN` clauses are searched in the same manner. If no `WHEN` *`condition`* is `TRUE` then the value of the `CASE` expression is the *`result`* in the `ELSE` clause. If the `ELSE` clause is omitted and no condition matches, the result is `NULL`.
