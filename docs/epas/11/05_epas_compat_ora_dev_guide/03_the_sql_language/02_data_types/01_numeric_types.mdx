...
| `ROWID`                     | 4 bytes      | Unsigned 4 bit integer.                                      | 0 to 4,294,967,295               |

The following sections describe the types in detail.

## Integer Types

The type `BINARY_INTEGER`, `INTEGER`, `PLS_INTEGER`, and `ROWID` stores whole numbers (without fractional components) as specified under Table 2-2 *Numeric* *Types*. Attempts to store values outside of the allowed range will result in an error.
