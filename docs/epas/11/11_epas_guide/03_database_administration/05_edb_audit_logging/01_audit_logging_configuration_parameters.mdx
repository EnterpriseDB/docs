...
Use the following configuration parameters to control database auditing. See Section [3.1.2](https://www.enterprisedb.com/edb-docs/d/edb-postgres-advanced-server/user-guides/user-guide/11/EDB_Postgres_Advanced_Server_Guide.1.022.html#pID0E05WF0HA) to determine if a change to the configuration parameter takes effect immediately, or if the configuration needs to be reloaded, or if the database server needs to be restarted.
...
> This configuration parameter is used to specify auditing of different categories of SQL statements. Various combinations of the following values may be specified: `none`, `dml`, `insert`, `update`, `delete`, `truncate`, `select`, `error`, `rollback`, `ddl`, `create`, `drop`, `alter`, `grant`, `revoke`, and `all`. The default is `ddl` and `error`. See Section [3.5.2](https://www.enterprisedb.com/edb-docs/d/edb-postgres-advanced-server/user-guides/user-guide/11/EDB_Postgres_Advanced_Server_Guide.1.040.html#pID0E0NNE0HA) for information regarding the setting of this parameter.
...
> Bulk processing logs the resulting statements into both the Advanced Server log file and the EDB Audit log file. However, logging each and every statement in bulk processing is costly. This can be controlled by the `edb_log_every_bulk_value` configuration parameter. When set to tru~e, each and every statement in bulk processing is logged. When set to `false`, a log message is recorded once per bulk processing. In addition, the duration is emitted once per bulk processing. Default is `false`.

`edb_audit_destination`

> Specifies whether the audit log information is to be recorded in the directory as given by the `edb_audit_directory` parameter or to the directory and file managed by the *syslog* process. Set to fi~le to use the directory specified by `edb_audit_directory`, which is the default setting. Set to syslog to use the `syslog` process and its location as configured in the `/etc/syslog.conf` file. **Note:** In recent Linux versions, syslog has been replaced by *rsyslog* and the configuration file is in `/etc/rsyslog.conf`.
