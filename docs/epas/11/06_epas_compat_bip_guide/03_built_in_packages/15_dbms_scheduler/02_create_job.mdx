...
<div id="create_job" class="registered_link"></div>
...
> *`job_type`* specifies the type of job. The current implementation of CRE~ATE_JOB supports a job type of `PLSQL_BLOCK` or `STORED_PROCEDURE`.

*`job_action`*

> If *`job_type`* is PLS~QL_BLOCK, *`job_action`* specifies the content of the PL/SQL block that will be invoked when the job executes. The block must be terminated with a semi-colon (;).
>
