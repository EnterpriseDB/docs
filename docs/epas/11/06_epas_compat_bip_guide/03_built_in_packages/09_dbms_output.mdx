...
<div id="dbms_output" class="registered_link"></div>
...
> G`ET_LINE(`*`line`* `OUT VARCHAR2,` *`status`* `OUT INTEGER)`
...
> Table receiving the lines of text from the message buffer. See `CHARARR` for a description of *li`n`es*.
